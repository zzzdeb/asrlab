...
class VectorAccumulator {
  typedef VectorAccumulator<InputType, PlusOperation, PlusWeightedOperation>
      Self;
...
public:
  VectorAccumulator(ComponentIndex size = 0) : weight_(0) { resize(size); }
  ...
  void accumulate(const std::vector<InputType> &v) {
    unrolledTransform(sum_.begin(), sum_.end(), v.begin(), sum_.begin(),
                      PlusOperation());
    ++weight_;
  }
  void accumulate(const std::vector<InputType> &v, Weight weight) {
    unrolledTransform(sum_.begin(), sum_.end(), v.begin(), sum_.begin(),
                      PlusWeightedOperation(weight));
    weight_ += weight;
  }
  void accumulate(const Self &v) {
    unrolledTransform(sum_.begin(), sum_.end(), v.sum_.begin(), sum_.begin(),
                      std::plus<SumType>());
    weight_ += v.weight_;
  }
  ...
  void read(Core::BinaryInputStream &i, u32 version) {
    ...
    i >> size;
    sum_.resize(size);
    for (typename std::vector<SumType>::iterator it = sum_.begin();
         it != sum_.end(); it++)
      i >> (*it);
    if (version > 0) {
      i >> weight_;
  ...
  void write(Core::BinaryOutputStream &o) const {
    o << (u32)sum_.size();
    std::copy(sum_.begin(), sum_.end(),
              Core::BinaryOutputStream::Iterator<SumType>(o));
    o << weight_;
  }
  void write(Core::XmlWriter &o,
             const std::string &name = "vector-accumulator") const {
    o << Core::XmlOpen(name) + Core::XmlAttribute("size", sum_.size()) +
             Core::XmlAttribute("weight", weight_);
