...
public:
  _const_iterator(const BundleArchive &a) : a_(a) {
    ...
    if (iterArchive_ != a_.archiveFiles_.end()) {
      curArchive_ =
          Archive::create(a_.config, *iterArchive_, Archive::AccessModeRead);
      verify(curArchive_);
  ...
  virtual ~_const_iterator() { delete curArchive_; }
...
Core::BundleArchive::BundleArchive(const Configuration &config,
                                   const std::string &path, AccessMode access)
...
Core::BundleArchive::~BundleArchive() {
  for (ArchiveCache::iterator i = archiveCache_.begin();
       i != archiveCache_.end(); ++i) {
    delete i->second;
  }
}
...
bool Core::BundleArchive::createIndex() {
  for (u32 i = 0; i < archiveFiles_.size(); ++i) {
    Archive *archive =
        Archive::create(config, archiveFiles_[i], AccessModeRead);
    if (!archive)
      return false;
...
bool Core::BundleArchive::writeIndex(const std::string &filename) {
  ...
  std::copy(archiveFiles_.begin(), archiveFiles_.end(),
            std::ostream_iterator<std::string>(fout, "\n"));
  fout << fileMap_.size() << "\n";
  for (StringHashMap<u32>::const_iterator i = fileMap_.begin();
       i != fileMap_.end(); ++i) {
...
bool Core::BundleArchive::readIndex(const std::string &filename) {
  ...
  if (archiveFiles_.size() != tmp)
    return false;
  for (std::vector<std::string>::const_iterator f = archiveFiles_.begin();
       f != archiveFiles_.end(); ++f) {
    std::string buffer;
    fin >> buffer;
    if (*f != buffer)
      return false;
...
BundleArchive::ArchiveRef
Core::BundleArchive::getArchive(const std::string &file) const {
  ...
  if (archive == archiveCache_.end()) {
    ArchiveRef archive =
        Archive::create(config, archiveFiles_[idx->second], AccessModeRead);
    verify(archive);
...
bool Core::BundleArchive::discover(const std::string &name,
                                   Sizes &sizes) const {
...
bool Core::BundleArchive::write(const std::string &name, const std::string &b,
                                const Sizes &sizes) {
...
bool Core::BundleArchive::clear() { return false; }

bool Core::BundleArchive::recover() { return false; }
