  ...
  struct Statistics {
    ...
    void inc(Operation op) {
      ...
      case PseudoBwdMatch:
        ++nPseudoBwdMatch;
        break;
      case BwdMatchWithoutSplit:
        ++nBwdMatchWithoutSplit;
        break;
      case BwdMatchWithSplit:
        ++nBwdMatchWithSplit;
        break;
      }
    }
...
public:
  ...
  bool allowBwdMatch() const { return allowBwdMatch_; }
...
const Core::ParameterBool
    ClusterBuilder::paramBwdMatch("allow-bwd-match", "allow backward matches",
                                  false);
...
protected:
  ...
  template <class T> inline T l1norm(const T &val1, const T &val2) {
  ...
  */
  inline Score _similarity(const Arc &hyp, const Arc &ref) const {
    ...
    if (hyp.label == ref.label)
      score *= 2.5;
    /*
    dbg("sim(" << l_->getInputAlphabet()->symbol(hyp.label) << ":" << hyp.begin
    << "-" << hyp.end << ", "
        << l_->getInputAlphabet()->symbol(ref.label) << ":" << ref.begin << "-"
    << ref.end << ") = " << score);
    */
    return score;
  }
  inline Score _similarity(const Arc &hyp, Score beginTime) const {
    ...
    dbg("sim(" << l_->getInputAlphabet()->symbol(hyp.label) << ":" << hyp.begin
    << "-" << hyp.end << ", "
        << "<new arc cluster>" << ":" << beginTime << "-" << hyp.end << ") = "
    << score);
    */
    return score;
  }
  ...
  */
  inline Score similarity(const Arc &hyp, const Arc &ref) const {
    Score overlapScore =
              std::min(hyp.end, ref.end) - std::max(hyp.begin, ref.begin),
          diffScore = (std::max(hyp.begin, ref.begin) -
                       std::min(hyp.begin, ref.begin)) +
                      (std::max(hyp.end, ref.end) - std::min(hyp.end, ref.end));
  ...
  inline Score similarity(const Arc &hyp, Score beginTime) const {
    Score overlapScore = hyp.end - beginTime, diffScore = beginTime - hyp.begin;
  ...
  inline void updateFwdMatch(const ArcId hypId, const Arc &hyp,
                             const ArcId refId, const Arc &ref,
                             Match &bestFwdMatch) const {
  ...
  inline void updateBwdMatch(const ArcId hypId, const Arc &hyp,
                             const ArcId refId, const Arc &ref,
                             Match &bestBwdMatch) const {
    // bwd matches are expensive: try to avoid them
    if (hyp.begin < ref.end) {
      Score hypLen = hyp.end - hyp.begin, refLen = ref.end - ref.begin,
            overlap = ref.end - hyp.begin;
      if (((hyp.label == ref.label) && (overlap > 0.25 * hypLen) &&
           (overlap > 0.25 * refLen)) ||
          ((overlap > 0.5 * hypLen) && (overlap > 0.5 * refLen))) {
        Score score = similarity(hyp, ref);
  ...
  */
  bool checkPostCondition(const StateCluster *initial,
...
private:
  ClusteredLattice() { nullArcCluster_ = 0; }
  void cluster(ConstLatticeRef l, ConstFwdBwdRef fb, bool allowBwdMatch,
               ClusterBuilder::Statistics &statistics);
...
public:
  ...
  Probability calcConfidence(ProbabilityList::const_iterator itP,
                             ProbabilityList::const_iterator endP) const {
    switch (u32(endP - itP)) {
    case 0:
      return 1.0;
    case 1:
  ...
  Score calcScore(ProbabilityList::const_iterator itP,
                  ProbabilityList::const_iterator endP,
                  Score alpha = 0.05) const {
    switch (u32(endP - itP)) {
    case 0:
      return 0.0;
    case 1:
  ...
  std::pair<ConstConfusionNetworkRef, ConstLatticeRef>
  getNormalizedCn(ScoreId confidenceId, bool mapping = false) const;
  ConstConfusionNetworkRef getCn(ScoreId posteriorId = Semiring::InvalidId,
                                 bool mapping = false) const;
  ConstLatticeRef getClusteredLattice() const;
};
const ClusteredLattice::ArcId ClusteredLattice::InvalidArcId =
    Core::Type<ArcId>::max;
const ClusteredLattice::StateId ClusteredLattice::InvalidStateId =
    Core::Type<StateId>::max;

ClusteredLatticeRef ClusterBuilder::cluster(ConstLatticeRef l,
                                            ConstFwdBwdRef fb) {
...
ConstLatticeRef ClusteredLattice::getLattice() const { return l_; }

bool ClusteredLattice::checkPostCondition(const StateCluster *initial,
                                          bool verbose) const {
  ...
  bool good = true;
  u32 clusterId = 0;
  for (const StateCluster *cluster = initial;;
       cluster = cluster->next->next, ++clusterId) {
    if (log)
    ...
    if (cluster->id != clusterId) {
      Core::Application::us()->warning(
          "State cluster id mismatch: expected %d, got %d.", clusterId,
          cluster->id);
      good = false;
    }
    for (StateCluster::const_iterator s = cluster->begin(),
                                      end_s = cluster->end();
         s != end_s; ++s) {
      ...
      if (state.cluster->id != cluster->id) {
        Core::Application::us()->warning(
            "State cluster mismatch: expected %d, got %d.", cluster->id,
            state.cluster->id);
        good = false;
      }
    }
    ...
    if (cluster->next->id != clusterId + 1) {
      Core::Application::us()->warning(
          "Arc cluster id mismatch: expected %d, got %d.", (clusterId + 1),
          cluster->next->id);
      good = false;
    }
    if (cluster->next->empty()) {
      Core::Application::us()->warning("Arc cluster %d is empty.",
                                       cluster->next->id);
      good = false;
    }
    ClusterId prevToClusterId = 0;
    ClusterId maxFromClusterId = 0, minToClusterId = Core::Type<ClusterId>::max;
    for (ArcCluster::const_iterator a = cluster->next->begin(),
                                    a_end = cluster->next->end();
         a != a_end; ++a) {
      const Arc &arc = arcs_[*a];
      if (log)
        *log << "\tcheck arc " << arc.from << "-" << arc.to << ", "
             << "[" << states_[arc.from].cluster->id << ":" << arc.cluster->id
             << ":" << states_[arc.to].cluster->id << "], "
             << l_->getInputAlphabet()->symbol(arc.label) << ":" << arc.begin
             << "-" << arc.end << "\n";
      if (arc.cluster->id != cluster->next->id) {
        Core::Application::us()->warning(
            "Arc cluster mismatch: expected %d, got %d.", cluster->next->id,
            arc.cluster->id);
        good = false;
      }
      ...
      if (fromClusterId >= toClusterId) {
        Core::Application::us()->warning(
            "State cluster order violation: expected %d < %d.", fromClusterId,
            toClusterId);
        good = false;
      }
      if ((arc.cluster->id <= fromClusterId) ||
          (toClusterId < arc.cluster->id)) {
        Core::Application::us()->warning(
            "Arc cluster out of bounds: %d not in (%d,%d].", arc.cluster->id,
      ...
      /*
        ...
        "Inner arc cluster order violation: expected %d < %d.", prevToClusterId,
        toClusterId);
      */
    ...
    if (maxFromClusterId >= minToClusterId) {
      Core::Application::us()->warning(
          "Arc cluster consistency violation: expected %d < %d.",
          maxFromClusterId, minToClusterId);
      good = false;
    }
  }
  return good;
...
ClusteredLattice::ArcCluster *
ClusteredLattice::remerge(ArcCluster *newBwdArcCluster) {
  if (!newBwdArcCluster)
    return newBwdArcCluster;
  ClusterId leftBwdClusterId = newBwdArcCluster->prev->id,
            rightBwdClusterId = newBwdArcCluster->next->id;
  bool leftMerge = true, rightMerge = true;
  if (newBwdArcCluster->prev->prev) {
    for (ArcCluster::const_iterator itBwdArc = newBwdArcCluster->begin(),
                                    endBwdArc = newBwdArcCluster->end();
    ...
    if (leftMerge)
      for (ArcCluster::const_iterator
               itLeftBwdArc = newBwdArcCluster->prev->prev->begin(),
  ...
  if (newBwdArcCluster->next->next) {
    ...
    if (rightMerge)
      for (ArcCluster::const_iterator
               itRightBwdArc = newBwdArcCluster->next->next->begin(),
               endRightBwdArc = newBwdArcCluster->next->next->end();
           itRightBwdArc != endRightBwdArc; ++itRightBwdArc) {
        if (states_[arcs_[*itRightBwdArc].from].cluster->id ==
            rightBwdClusterId) {
          rightMerge = false;
          break;
        }
  ...
  if (leftMerge && rightMerge) {
    Score leftScore = Core::Type<Score>::min,
          rightScore = Core::Type<Score>::min;
    ArcCluster::const_iterator beginLeftBwdArc =
                                   newBwdArcCluster->prev->prev->begin(),
                               endLeftBwdArc =
                                   newBwdArcCluster->prev->prev->end();
    for (ArcCluster::const_iterator itBwdArc = newBwdArcCluster->begin(),
                                    endBwdArc = newBwdArcCluster->end();
         itBwdArc != endBwdArc; ++itBwdArc) {
      const Arc &bwdArc = arcs_[*itBwdArc];
      for (ArcCluster::const_iterator itLeftBwdArc = beginLeftBwdArc;
           itLeftBwdArc != endLeftBwdArc; ++itLeftBwdArc) {
    ...
    ArcCluster::const_iterator beginRightBwdArc =
                                   newBwdArcCluster->next->next->begin(),
                               endRightBwdArc =
                                   newBwdArcCluster->next->next->end();
    for (ArcCluster::const_iterator itBwdArc = newBwdArcCluster->begin(),
                                    endBwdArc = newBwdArcCluster->end();
         itBwdArc != endBwdArc; ++itBwdArc) {
      const Arc &bwdArc = arcs_[*itBwdArc];
      for (ArcCluster::const_iterator itRightBwdArc = beginRightBwdArc;
           itRightBwdArc != endRightBwdArc; ++itRightBwdArc) {
  ...
  if (rightMerge || leftMerge) {
    ...
    if (leftMerge) {
      ...
      for (ArcCluster::iterator itBwdArc = newBwdArcCluster->begin(),
                                endBwdArc = newBwdArcCluster->end();
           itBwdArc != endBwdArc; ++itBwdArc)
      ...
      for (StateCluster::iterator itBwd = newBwdArcCluster->next->begin(),
                                  endBwd = newBwdArcCluster->next->end();
           itBwd != endBwd; ++itBwd)
      ...
      for (ArcCluster::iterator itRightBwdArc = rightBwdArcCluster->begin(),
                                endRightBwdArc = rightBwdArcCluster->end();
           itRightBwdArc != endRightBwdArc; ++itRightBwdArc)
      ...
      for (StateCluster::iterator
               itRightBwd = rightBwdArcCluster->next->begin(),
               endRightBwd = rightBwdArcCluster->next->end();
    ...
    for (ArcCluster *itCluster = newBwdArcCluster; itCluster;
         itCluster = itCluster->next->next)
      itCluster->id = itCluster->next->id = itCluster->prev->id + 1;
  }
  return newBwdArcCluster;
}

void ClusteredLattice::cluster(ConstLatticeRef l, ConstFwdBwdRef fb,
                               bool allowBwdMatch,
                               ClusterBuilder::Statistics &statistics) {
  l_ = l;
  ...
  */
  Core::ProgressIndicator pi(
      Core::form("CN(%d states/ %.2f sec)",
                 ...
                 f32(boundaries.time(chronologicalSort->back()) -
                     boundaries.time(chronologicalSort->front())) /
                     100.0),
      "states");
  ...
  for (StateMap::const_iterator itSid = chronologicalSort->begin(),
    ...
    for (Flf::State::const_iterator a = sr->begin(), end_a = sr->end();
      ...
      if (states_[arc.to].cluster)
        Core::Application::us()->criticalError(
            "CN builder: Arc violates topological order; probably a back "
            "pointing arc (%d-%d).",
    ...
    // std::sort(states_[sid].bwd.begin(), states_[sid].bwd.end(),
    // reverseBeginWeakOrder);
    for (ArcIdList::const_iterator beginAid = states_[sid].bwd.begin(),
                                   itAid = states_[sid].bwd.begin(),
                                   endAid = states_[sid].bwd.end();
         itAid != endAid; ++itAid) {
      ArcId arcId = *itAid;
      Arc &arc = arcs_[arcId];
      verify(arc.to == sid);
      ...
      */
      ...
      if (arc.begin == arc.end) {
          Core::Application::us()->error(
              "CN builder: Zero-length arc detected(%s,%d-%d); zero-length arcs
      might cause malfunctions in the alignment algorithm.",
              l->getInputAlphabet()->symbol(arc.label).c_str(), s32(arc.begin),
      s32(arc.end)); nullArcCluster_->add(arcId, arc); statistics.inc(
      ClusterBuilder::Statistics::NullArc); continue;
      }
      */
      ...
      */
      ClusterBuilder::Statistics::Operation op =
          ClusterBuilder::Statistics::None;
      ...
      for (ArcIdList::const_iterator itAid2 = beginAid; itAid2 != itAid;
           ++itAid2) {
        ArcId arcId2 = *itAid2;
      ...
      for (StateCluster *itCluster = cluster;
           allowBwdMatch || (itCluster != fromCluster);
           itCluster = itCluster->prev->prev) {
        for (StateIdList::const_iterator itSid2 = itCluster->begin(),
                                         endSid2 = itCluster->end();
             itSid2 != endSid2; ++itSid2)
          if (*itSid2 != arc.from) {
            const State &state2 = states_[*itSid2];
            for (ArcIdList::const_iterator itAid2 = state2.bwd.begin(),
                                           endAid2 = state2.bwd.end();
      ...
      */
      ...
      if (bestBwdMatch.score > bestFwdMatch.score) {
        ...
        if ((bestFwdMatch.id != InvalidArcId) &&
            (states_[arcs_[bestBwdMatch.id].to].cluster->id >=
             arcs_[bestFwdMatch.id].cluster->id)) {
          /*
            bwd. match -> add to existing cluster;
            correct previous match of best bwd. arc and its nearest neighbours
          */
          /*
            Find representative for arcs remaining in bwd. cluster (stay) and
            arcs going with best bwd. arc (move); if stay representative is
            closer to best bwd. arc than new arc => cancel bwd. match.
          */
          ClusterId fwdArcClusterId = arcs_[bestFwdMatch.id].cluster->id;
          ArcId stayRepresentativeId = InvalidArcId,
                moveRepresentativeId = arcId /* bestBwdMatch.id ? */;
          Score moveScore = similarity(bestBwdArc, arc),
                bestStayScore = Core::Type<Score>::min;
          for (ArcCluster::iterator itStayArc = bwdArcCluster->begin(),
                                    endStayCluster = bwdArcCluster->end();
               itStayArc != endStayCluster; itStayArc++) {
          ...
          if (bestStayScore == Core::Type<Score>::min) {
            ...
            */
            ...
            u32 nMove = 0;
            const Arc &stayRepresentative = arcs_[stayRepresentativeId],
                      &moveRepresentative = arcs_[moveRepresentativeId];
            for (ArcCluster::iterator itBwdArc = bwdArcCluster->begin(),
                                      endBwdArc = bwdArcCluster->end();
                 itBwdArc != endBwdArc; ++itBwdArc) {
              if (*itBwdArc == bestBwdMatch.id) {
                ...
                if (toClusterId >= fwdArcClusterId) {
                  Score stayScore = similarity(bwdArc, stayRepresentative),
                        moveScore = similarity(bwdArc, moveRepresentative);
            ...
            newBwdArcCluster = bwdArcCluster->prev->next =
                bwdArcCluster->next->prev =
                    new ArcCluster(bwdArcCluster->prev, bwdArcCluster->next);
            newBwdArcCluster->id = bwdArcCluster->id;
            newBwdArcCluster->reserve(bwdArcCluster->size() - nMove);
            for (ArcCluster::const_iterator itBwdArc = bwdArcCluster->begin(),
                                            endBwdArc = bwdArcCluster->end();
                 itBwdArc != endBwdArc; ++itBwdArc) {
              Arc &bwdArc = arcs_[*itBwdArc];
              if (bwdArc.cluster != 0)
            ...
            // dbg("cluster=" << cluster);
            for (cluster = initial; cluster->next;
          ...
          */
          ...
          Score moveScore = similarity(bestBwdArc, arc),
                bestStayScore = Core::Type<Score>::min;
          verify(states_[arcs_[bestBwdMatch.id].to].cluster->id >=
                 fromCluster->id);
          for (ArcCluster::iterator itStayArc = bwdArcCluster->begin(),
                                    endStayCluster = bwdArcCluster->end();
               itStayArc != endStayCluster; itStayArc++) {
          ...
          if (bestStayScore == Core::Type<Score>::min) {
            ...
            */
            ...
            /*
              ...
              where the current arc starts at (1a) and the best matching arc
              ends at (1b)
            */
            newBwdArcCluster = bwdArcCluster->prev->next =
                bwdArcCluster->next->prev =
                    new ArcCluster(bwdArcCluster->prev, bwdArcCluster->next);
            newBwdArcCluster->id = bwdArcCluster->id;
            StateCluster *leftStateCluster =
                             new StateCluster(fromCluster->prev, 0),
                         *rightStateCluster =
                             new StateCluster(0, fromCluster->next);
            fromCluster->prev->next = leftStateCluster;
            if (fromCluster->next)
              fromCluster->next->prev = rightStateCluster;
            if (cluster == fromCluster)
              cluster = rightStateCluster;
            ArcCluster *newArcCluster = leftStateCluster->next =
                rightStateCluster->prev =
                    new ArcCluster(leftStateCluster, rightStateCluster);
            leftStateCluster->id = fromCluster->id;
            for (ArcCluster *itCluster = newArcCluster; itCluster;
                 itCluster = itCluster->next->next)
              itCluster->id = itCluster->next->id = itCluster->prev->id + 1;
            Time bwdSplitOverlapBegin = states_[arc.from].time,
                 bwdSplitOverlapEnd = states_[bestBwdArc.to].time;
            verify(bwdSplitOverlapBegin < bwdSplitOverlapEnd);
            for (StateIdList::const_iterator itFrom = fromCluster->begin(),
                                             endFrom = fromCluster->end();
                 itFrom != endFrom; ++itFrom) {
            ...
            for (ArcCluster::iterator itBwdArc = bwdArcCluster->begin(),
              ...
              if (bwdArc.cluster != 0)
                ...
                verify((bwdArc.to == sid) ||
                       (states_[bwdArc.to].cluster->id >= newArcCluster->id));
        ...
        if (op == ClusterBuilder::Statistics::PseudoBwdMatch) {
          ...
          for (ArcIdList::const_iterator itAid2 = fromState.bwd.begin(),
                                         endAid2 = fromState.bwd.end();
               itAid2 != endAid2; ++itAid2)
          ...
          if (!isConsistent) {
            /*
              ...
              l_->getInputAlphabet()->symbol(arc.label) << ":" << arc.begin <<
              "-" << arc.end << ")" << "\n"
              << "from " << fromCluster->id << " to " <<
              arcs_[bestBwdMatch.id].cluster->prev->id << "\n";
            */
            Core::Application::us()->warning(
                "Give up on bwd. match, because CN has a really odd structure; "
                "fall back to no/fwd. match.");
            bestBwdMatch.score = Core::Type<Score>::min;
            bestBwdMatch.id = InvalidArcId;
            op = ClusterBuilder::Statistics::None;
          } else {
      ...
      */
      /*
      Core::Application::us()->log() << "new arc " << arcId << " (" <<
      l_->getInputAlphabet()->symbol(arc.label) << ":" << arc.begin << "-" <<
      arc.end << ")\n"
                                     << "operation " << op << "\n";
      checkPostCondition(initial, true);
      */
      /*
      Core::Application::us()->log() << "added arc " << arcId << " " <<
      states_[arc.from].id << "-" << states_[arc.to].id << " (" <<
      l_->getInputAlphabet()->symbol(arc.label) << ":" << arc.begin << "-" <<
      arc.end << ", "
                                     << "[" << states_[arc.from].cluster->id <<
      ":" << arc.cluster->id << ":" << states_[arc.to].cluster->id << "])\n"
                                     << "operation " << op << "\n";
...
std::pair<ConstConfusionNetworkRef, ConstLatticeRef>
ClusteredLattice::getNormalizedCn(ScoreId confidenceId, bool mapping) const {
  ...
  s->setProperties(Fsa::PropertyAcyclic | Fsa::PropertyLinear,
                   Fsa::PropertyAll);
  ...
  s->setState(sp);
  Time minEndTime = 0,
       preferredEndTime = 0; // Attention: preferredEndTime >= minEndTime
  ...
  for (u32 i = 0; i < arcClusters_.size(); ++i) {
    ...
    for (ArcCluster::const_iterator itArc = arcCluster.begin(),
                                    endArc = arcCluster.end();
         itArc != endArc; ++itArc) {
      ...
      if (ccnArc.label != bestCurrentLabelCcnArc.label) {
        ...
        Score start = ::exp(posteriorScore - colBegin->get()),
              end = ::exp(posteriorScore - colEnd->get());
        colBegin->reset();
        colEnd->reset();
        ...
        slot.push_back(ConfusionNetwork::Arc(
            bestCurrentLabelCcnArc.label, scores, Time(Core::round(start)),
            std::max(Time(1), Time(Core::round(end - start)))));
      ...
      } else if (ccnArc.posteriorScore <
                 bestCurrentLabelCcnArc.posteriorScore) {
        bestCurrentLabelCcnArcId = ccnArcId;
      }
    ...
    if (sum != 1.0) {
      ...
      if (normalize) {
        Score norm = 1.0 / sum;
        if (confidenceId != Semiring::InvalidId)
          for (ConfusionNetwork::Slot::iterator itSlot = slot.begin(),
                                                endSlot = slot.end();
    ...
    // add best arc
    if ((bestCcnArcId != InvalidArcId) &&
        (arcs_[bestCcnArcId].label != Fsa::Epsilon)) {
      const Arc &bestCcnArc = arcs_[bestCcnArcId];
      Time beginTime = Time(Core::round(bestCcnArc.begin)),
           endTime = Time(Core::round(bestCcnArc.end));
      if (preferredEndTime < beginTime) {
        b->set(sp->id(), Boundary(preferredEndTime));
        ScoresRef scores = semiring->one();
        if (confidenceId != Semiring::InvalidId) {
          Score epsConfidence = calcConfidence(pendingEpsConfidences.begin(),
                                               pendingEpsConfidences.end());
          scores = semiring->clone(semiring->one());
          scores->set(confidenceId, epsConfidence);
        }
        ...
        preferredEndTime =
            std::max(minEndTime, (preferredEndTime + beginTime) / 2);
      b->set(sp->id(), Boundary(preferredEndTime));
      if (confidenceId != Semiring::InvalidId) {
        bestCcnArcScores = semiring->clone(bestCcnArcScores);
        bestCcnArcScores->set(confidenceId, bestConfidence);
      }
      sp->newArc(sp->id() + 1, bestCcnArcScores, bestCcnArc.label,
                 bestCcnArc.label);
      sp = new Flf::State(sp->id() + 1);
      s->setState(sp);
  ...
  if ((confidenceId != Semiring::InvalidId) && !pendingEpsConfidences.empty()) {
    scores = semiring->clone(scores);
    scores->set(confidenceId, calcConfidence(pendingEpsConfidences.begin(),
                                             pendingEpsConfidences.end()));
    pendingEpsConfidences.clear();
  }
  sp->setFinal(scores);
...
ConfusionNetwork::ConstMapPropertiesRef
ClusteredLattice::getMapping(bool reduce) const {
  ConfusionNetwork::MapProperties *mapProps =
  ...
  ConfusionNetwork::MapProperties::Map &lat2cn = mapProps->lat2cn;
  {
    ...
    lat2cn.resize(stateIndex.back(), ConfusionNetwork::MapProperties::Mapping(
                                         Fsa::InvalidStateId, 0));
  }
  if (reduce) {
    for (Fsa::StateId sid = 0; sid < arcClusters_.size(); ++sid) {
      ...
      for (ArcCluster::const_iterator itArc = arcCluster.begin(),
        ...
        lat2cn[stateIndex[ccnArc.from] + ccnArc.aid] =
            ConfusionNetwork::MapProperties::Mapping(sid, Fsa::InvalidStateId);
      }
    }
  } else {
    ...
    ConfusionNetwork::MapProperties::Map &cn2lat = mapProps->cn2lat;
    {
      ...
      cn2lat.resize(slotIndex.back(), ConfusionNetwork::MapProperties::Mapping(
                                          Fsa::InvalidStateId, 0));
    }
    for (Fsa::StateId sid = 0; sid < arcClusters_.size(); ++sid) {
      const ArcCluster &arcCluster = *arcClusters_[sid];
      ...
      for (ArcCluster::const_iterator itArc = arcCluster.begin(),
                                      endArc = arcCluster.end();
           itArc != endArc; ++itArc, ++aid) {
        const Arc &ccnArc = arcs_[*itArc];
        verify(slotIndex[sid] + aid < cn2lat.size());
        cn2lat[slotIndex[sid] + aid] =
            ConfusionNetwork::MapProperties::Mapping(ccnArc.from, ccnArc.aid);
        verify(stateIndex[ccnArc.from] + ccnArc.aid < lat2cn.size());
        lat2cn[stateIndex[ccnArc.from] + ccnArc.aid] =
            ConfusionNetwork::MapProperties::Mapping(sid, aid);
      }
    }
  }
...
ConstConfusionNetworkRef ClusteredLattice::getCn(ScoreId posteriorId,
                                                 bool mapping) const {
  if (stateClusters_.empty())
  ...
  for (u32 i = 0; i < arcClusters_.size(); ++i) {
    ...
    for (ArcCluster::const_iterator itArc = arcCluster.begin(),
      ...
      slot.push_back(ConfusionNetwork::Arc(
          arc.label, scores, Time(arc.begin), Time(arc.end - arc.begin),
...
ConstLatticeRef ClusteredLattice::getClusteredLattice() const {
  ...
  for (Fsa::StateId sid = 0; sid < stateClusters_.size(); ++sid) {
    ...
    s->setState(sp);
    Time tSum = 0;
    for (StateCluster::const_iterator itState = cluster.begin(),
                                      endState = cluster.end();
         itState != endState; ++itState)
    ...
    if (t <= lastT) {
      /*
      Core::Application::us()->warning(
          "Clustered lattice of \"%s\" violates time constraints: %d > %d
      violated", l_->describe().c_str(), t, lastT);
      */
      t = lastT + 1;
    }
  ...
  for (ArcIdList::const_iterator itAid = sortedAids.begin(),
                                 endAid = sortedAids.end();
       itAid != endAid; ++itAid) {
    const Arc &arc = arcs_[*itAid];
    u32 startCluster = states_[arc.from].cluster->id,
        endCluster = states_[arc.to].cluster->id;
...
private:
  ...
  ConstConfusionNetworkRef getNormalizedCn() {
    if (!normalizedCn_) {
      std::pair<ConstConfusionNetworkRef, ConstLatticeRef> result =
          clusteredLattice()->getNormalizedCn(getConfidenceId(), map_);
...
public:
  StateClusterCnBuilderNode(const std::string &name,
...
const Core::ParameterString
    StateClusterCnBuilderNode::paramConfKey("confidence-key",
                                            "store confidence score", "");
const Core::ParameterBool StateClusterCnBuilderNode::paramMap(
    "map", "map lattice to CN and vice versa", false);
const Core::ParameterBool
    StateClusterCnBuilderNode::paramRemoveNullArcs("remove-null-arcs",
