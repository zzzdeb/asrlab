...
const Core::ParameterBool paramCollectorNodeCheckSameLength(
    "check-same-length",
    "if true, error raised if input streams are of different length", false);
const Core::ParameterString
    paramCollectorNodeTimestampPortName("timestamp-port",
                                        "name of port to take timestamps from");
...
template <class I, class O> class CollectorNode : public SourceNode {
...
protected:
   ...
   *    if Illegal: @param startTime is the minimal start time and @param
   * endTime the maximal end time; if not Illegal: timestamp of timestampPortId_
   * is copied into @param startTime and @param endTime.
   *
...
CollectorNode<I, O>::CollectorNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c),
      ...
      timestampPortId_(IllegalPortId), needInit_(true) {
  setCheckSameLength(paramCollectorNodeCheckSameLength(c));
  setTimestampPortName(paramCollectorNodeTimestampPortName(c));
}

template <class I, class O> void CollectorNode<I, O>::init() {
  if (!timestampPortName_.empty()) {
    std::map<std::string, Flow::PortId>::iterator p =
        inputs_.find(timestampPortName_);
    if (p == inputs_.end())
      error("Could not find input port '%s' as time-stamp-port.",
            timestampPortName_.c_str());
    else
...
template <class I, class O> bool CollectorNode<I, O>::putNullData() {
  if (checkSameLength_) {
...
template <class I, class O>
bool CollectorNode<I, O>::setParameter(const std::string &name,
...
template <class I, class O> bool CollectorNode<I, O>::configure() {
