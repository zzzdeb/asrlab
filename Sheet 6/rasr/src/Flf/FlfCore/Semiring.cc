...
void incScoresRefCounter() { ++nScoresRef_; }
void decScoresRefCounter() { --nScoresRef_; }
u32 nScoresRef() { return nScoresRef_; }
...
Semiring::Semiring(size_t n, const ScoreList &scales, const KeyList &keys)
    : Precursor(), n_(n), tolerance_(DefaultTolerance),
      scales_(n, DefaultScale), keys_(n, UndefinedKey) {
  if (!scales.empty())
    setScales(scales);
  if (!keys.empty())
    setKeys(keys);
...
std::string Semiring::name() const {
  ...
  for (ScoreList::const_iterator itScale = scales_.begin();
       itScale != scales_.end(); ++itScale, ++itKey, ++id) {
    oss << ",";
    if (itKey->empty())
      oss << id;
    else
...
void Semiring::setKeys(const KeyList &keys) const {
  ...
  for (KeyList::const_iterator it = keys.begin(); it != keys.end(); ++it, ++id)
    if (*it != UndefinedKey)
      if (!setKey(id, *it))
        Core::Application::us()->error("Failed to set name \"%s\" for "
                                       "dimension %d, name is already in use.",
                                       it->c_str(), id);
}
...
bool Semiring::hasId(ScoreId id) const { return size_t(id) < size(); }
...
void Semiring::compress(ByteVector &v, const ScoresRef &a) const {
  if (a == one_)
    ...
    v.push_back(3);
    for (Scores::const_iterator it = begin(a), itEnd = end(a); it != itEnd;
         ++it) {
...
size_t Semiring::compressedSize() const { return 1 + size() * sizeof(u32); }
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
ConstSemiringRef Semiring::create(Fsa::SemiringType type, size_t n,
...
const Core::ParameterString paramType("type", "semiring type", "");
const Core::ParameterInt paramTolerance("tolerance",
                                        "tolerance used for score comparisons",
                                        Semiring::DefaultTolerance);
const Core::ParameterStringVector paramKeys("keys", "list of keys");
const Core::ParameterFloat paramScale("scale", "scale", Semiring::DefaultScale);
ConstSemiringRef Semiring::create(const Core::Configuration &config) {
  ...
  for (KeyList::const_iterator itKey = _keys.begin(); itKey != _keys.end();
       ++itKey, ++itScale)
    *itScale = Score(paramScale(Core::Configuration(config, *itKey)));
  ConstSemiringRef _semiring =
      Semiring::create(_type, _scales.size(), _scales, _keys);
...
void Semiring::write(Core::XmlWriter &xml) const {
  ...
  for (u32 i = 0; i < size(); ++i)
    xml << Core::XmlOpen("dimension") << Core::XmlFull("key", keys()[i])
        << Core::XmlFull("scale", scales()[i]) << Core::XmlClose("dimension");
...
public:
  ...
  void appendCdata(const char *ch, int len) { cdata_.append(ch, len); }
  ...
  void type() {
    ...
    if ((type_ = getSemiringType(typeName)) == Fsa::SemiringTypeUnknown)
      parser()->criticalError("Semiring type \"%s\" is unknown",
                              cdata_.c_str());
    cdata_.clear();
  }
...
Semiring::XmlElement::XmlElement(Core::XmlContext *context)
  ...
  addChild(new Core::XmlMixedElementRelay(
      "type", internal_, 0, endHandler(&Internal::type),
      charactersHandler(&Internal::appendCdata), XML_NO_MORE_CHILDREN));
  addChild(new Core::XmlMixedElementRelay(
      "tolerance", internal_, 0, endHandler(&Internal::tolerance),
      charactersHandler(&Internal::appendCdata), XML_NO_MORE_CHILDREN));
  addChild(new Core::XmlMixedElementRelay(
      "dimension", internal_, 0, endHandler(&Internal::dimension), 0,
      new Core::XmlMixedElementRelay(
          "key", internal_, 0, endHandler(&Internal::key),
          charactersHandler(&Internal::appendCdata), XML_NO_MORE_CHILDREN),
      new Core::XmlMixedElementRelay(
          "scale", internal_, 0, endHandler(&Internal::scale),
          charactersHandler(&Internal::appendCdata), XML_NO_MORE_CHILDREN),
      XML_NO_MORE_CHILDREN));
}

Semiring::XmlElement::~XmlElement() { delete internal_; }
...
ConstSemiringRef Semiring::read(std::istream &is) {
  if (!semiringXmlParser)
    semiringXmlParser = new SemiringXmlParser(Core::Configuration(
        Core::Application::us()->getConfiguration(), "semiring-parser"));
...
Fsa::SemiringType LogSemiring::type() const { return Fsa::SemiringTypeLog; }

std::string LogSemiring::name() const { return "log" + Semiring::name(); }
...
ScoresRef LogSemiring::collect(const ScoresRef &a, const ScoresRef &b) const {
  ...
  // verify(Core::isAlmostEqualUlp(logAdd(project(a), project(b)), project(c),
  // 1000));
  return c;
}
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
TropicalSemiring::TropicalSemiring(size_t n, const ScoreList &scales,
                                   const KeyList &keys)
    : Semiring(n, scales, keys) {}
...
ScoresRef TropicalSemiring::collect(const ScoresRef &a,
                                    const ScoresRef &b) const {
  if (b.get() == zero_.get())
    return a;
  if (a.get() == zero_.get())
    return b;
...
ConstSemiringRef cloneSemiring(ConstSemiringRef sr1) {
  ConstSemiringRef sr2 =
      Semiring::create(sr1->type(), sr1->size(), sr1->scales(), sr1->keys());
  sr2->setTolerance(sr1->tolerance());
  return sr2;
}

ConstSemiringRef appendSemiring(ConstSemiringRef sr1, Score scale,
                                const Key &key) {
  ...
  keys.push_back(key);
  ConstSemiringRef sr2 =
      Semiring::create(sr1->type(), sr1->size() + 1, scales, keys);
  sr2->setTolerance(sr1->tolerance());
  return sr2;
}
...
ConstSemiringRef appendSemiring(ConstSemiringRef sr1, ConstSemiringRef sr2) {
  ...
  if (sr1->type() != sr2->type())
    std::cerr << "In append semirings: Semiring type mismatch; use semiring "
                 "type of left argument"
              << std::endl;
  if (sr1->tolerance() != sr2->tolerance())
    std::cerr << "In append semirings: Tolerance mismatch; use tolerance of "
                 "left argument"
              << std::endl;
  ConstSemiringRef sr3 =
      Semiring::create(sr1->type(), sr1->size() + sr2->size(), scales, keys);
  sr3->setTolerance(sr1->tolerance());
  return sr3;
}

ConstSemiringRef rescaleSemiring(ConstSemiringRef semiring,
                                 const ScoreList &scales, const KeyList &keys) {
...
ConstSemiringRef rescaleSemiring(ConstSemiringRef semiring, ScoreId id,
                                 Score scale, const Key &key) {
  ConstSemiringRef _semiring = cloneSemiring(semiring);
...
ConstSemiringRef toLogSemiring(ConstSemiringRef semiring, Score scale) {
  ...
  if (scale == 0.0) {
    ...
    for (ScoreList::const_iterator it = scales.begin(); it != scales.end();
         ++it)
...
ConstSemiringRef toTropicalSemiring(ConstSemiringRef semiring) {
  if (semiring->type() == Fsa::SemiringTypeTropical)
    return semiring;
  ConstSemiringRef tropicalSemiring =
      Semiring::create(Fsa::SemiringTypeTropical, semiring->size(),
                       semiring->scales(), semiring->keys());
