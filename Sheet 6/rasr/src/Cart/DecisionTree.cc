...
void Cart::writeXml(Core::XmlWriter &xml, const QuestionRefList &questionRefs) {
  ...
  for (QuestionRefList::const_iterator it = questionRefs.begin();
    ...
    // question indicies are not used in the current implementation, leave it
    // unset
    // (*it)->index = index;
    (*it)->writeXml(xml);
  }
...
// ============================================================================
const Question::QuestionIndex Question::InvalidQuestionIndex =
    Core::Type<Question::QuestionIndex>::max;
...
// ============================================================================
ScalarQuestion::ScalarQuestion(PropertyMapRef map, const std::string &key,
                               const std::string &value,
                               const std::string &desc)
    : Question(map, desc), keyIndex_(map_->key(key)),
...
void ScalarQuestion::writeXml(Core::XmlWriter &xml) const {
  ...
  xml << xmlOpen << Core::XmlFull("key", map_->key(keyIndex_))
...
// ============================================================================
SetQuestion::SetQuestion(PropertyMapRef map, const std::string &key,
                         const std::vector<std::string> &values,
                         const std::string &desc)
    : Question(map, desc), keyIndex_(map_->key(key)), begin_(0), end_(0) {
  verify(map_->isDefined(keyIndex_));
  begin_ = new ValueIndex[values.size()];
  end_ = begin_ + values.size();
  std::vector<std::string>::const_iterator it;
  ValueIndex *itt;
  for (it = values.begin(), itt = begin_; it != values.end(); ++it, ++itt) {
...
void SetQuestion::writeXml(Core::XmlWriter &xml) const {
  ...
  if (!desc_.empty())
    xmlOpen.operator+(Core::XmlAttribute("description", desc_));
  xml << xmlOpen << Core::XmlFull("key", map_->key(keyIndex_))
      << Core::XmlFull("values", values2str()) << Core::XmlClose("question");
}
// ============================================================================
...
void TrainingInformation::writeXml(Core::XmlWriter &xml) const {
  xml << Core::XmlOpen("information") << Core::XmlFull("order", order)
      << Core::XmlFull("size", size) << Core::XmlFull("score", score)
...
const Core::ParameterString
    DecisionTree::paramCartFilename("decision-tree-file",
                                    "name of decision tree (aka cart) file");

DecisionTree::DecisionTree(const Core::Configuration &config,
                           PropertyMapRef map, QuestionRefList *questionRefs,
                           Node *root)
    : Precursor(config), map_(), questionRefs_(0) {
  setMap(map);
  if (questionRefs)
...
const DecisionTree::Node *DecisionTree::find(const Properties &props) const {
  ...
  while (!node->isLeaf())
    ...
    case RASR_FALSE:
      ...
      break;
    case RASR_UNDEF:
      warning() << "undefined answer to \"" << *questions[node->id()]
                << "\", assume false";
      node = node->rightChild_;
      break;
    }
...
u32 drawDecisionTreeNode(std::ostream &out, const DecisionTree::Node &node,
                         u32 id = 0) {
  if (node.isLeaf()) {
    out << "n" << id++ << " [shape=doublecircle label=" << node.id() << "]"
        << std::endl;
  } else {
    u32 nodeId = id++;
    out << "n" << nodeId << " [shape=circle label=" << node.id() << "]"
        << std::endl;
...
void writeDecisionTreeNode(std::ostream &out, const DecisionTree::Node &node,
                           const u32 depth = 0) {
...
void DecisionTree::writeXml(Core::XmlWriter &xml,
