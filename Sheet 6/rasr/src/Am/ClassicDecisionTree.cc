...
void mapBoundaries(PropertyMap::StringList &keys,
  ...
  pairs[3].second =
      PropertyMap::Index(Allophone::isInitialPhone | Allophone::isFinalPhone);
}
...
PropertyMap::PropertyMap() : Precursor(), termIndex(0), termString("#") {}

PropertyMap::PropertyMap(ClassicStateModelRef stateModel)
    : Precursor(), termIndex(0), termString("#") {
  StringList keys;
...
// ============================================================================
void logKey(Core::XmlWriter &xml, const PropertyMap &map, PropertyMap::Index i,
...
void PropertyMapDiff::logDiff(Core::XmlWriter &xml, PropertyMap::Index l,
                              PropertyMap::Index r) {
...
PropertyMapDiff::PropertyMapDiff(const Core::Configuration &config,
  ...
  if (map1.historySize() != map2.historySize()) {
    xml << Core::XmlOpen("diff") << Core::XmlOpen("left")
        << Core::XmlFull("history-size", map1.historySize())
        << Core::XmlClose("left") << Core::XmlOpen("right")
        << Core::XmlFull("history-size", map2.historySize())
        << Core::XmlClose("right") << Core::XmlClose("diff");
    differences_ |= 8;
  }
  l += map1.historySize();
  r += map2.historySize();
  ...
  for (; l < map1.size() && r < map2.size(); ++l, ++r) {
    if (map1.key(l) != map2.key(r)) {
      xml << Core::XmlOpen("diff") << Core::XmlOpen("left")
          << Core::XmlFull("condtion", map1.key(l)) << Core::XmlClose("left")
          << Core::XmlOpen("right") << Core::XmlFull("condtion", map2.key(r))
          << Core::XmlClose("right") << Core::XmlClose("diff");
      differences_ |= 32;
    }
  }
  if ((l != map1.size()) || (r != map2.size())) {
    xml << Core::XmlOpen("diff") << Core::XmlOpen("left")
        << Core::XmlFull("number-of-conditions", map1.size() -
                                                     map1.historySize() -
                                                     map1.futureSize() - 3)
        << Core::XmlClose("left") << Core::XmlOpen("right")
        << Core::XmlFull("number-of-conditions", map2.size() -
                                                     map2.historySize() -
                                                     map2.futureSize() - 3)
        << Core::XmlClose("right") << Core::XmlClose("diff");
    differences_ |= 32;
