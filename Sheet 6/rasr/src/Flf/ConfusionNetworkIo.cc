...
/**
 * Iterate through cn.
 * If cn is normalized, then the slot is sorted in descending order by
 *scores[posteriorId].
 **/
...
private:
  struct ProbabilityWeakOrder {
    ...
    bool operator()(const ConfusionNetwork::Arc &a1,
                    const ConfusionNetwork::Arc &a2) const {
  ...
  void setSlot() {
    if (it_ != end_) {
      if (posteriorId_ != Semiring::InvalidId) {
        sortedSlot_ = *it_;
        std::sort(sortedSlot_.begin(), sortedSlot_.end(),
                  ProbabilityWeakOrder(posteriorId_));
        slot_ = &sortedSlot_;
      } else
        slot_ = &*it_;
    } else
...
public:
  const ConfusionNetwork::Slot &operator*() const { return *slot_; }
  bool operator!=(const CnSortingIterator &it) const { return it_ != it.it_; }
  void operator++() {
    verify(it_ != end_);
    ++it_;
    setSlot();
...
**/
namespace {
void writeConfusionNetworkHeaderAsText(std::ostream &os,
...
void writeConfusionNetworkArcAsText(std::ostream &os,
                                    Fsa::ConstAlphabetRef alphabet,
                                    ConstSemiringRef semiring,
                                    const ConfusionNetwork::Arc &arc) {
  ...
  if (arc.duration != Speech::InvalidTimeframeIndex)
    ...
    os << "\t"
       << "inf";
  os << "\t"
     << ((arc.label == Fsa::Epsilon) ? "@" : alphabet->symbol(arc.label));
...
void writeConfusionNetworkAsText(std::ostream &os, ConstConfusionNetworkRef cn,
  ...
  if (!cn)
    return;
  if (cn->isNormalized())
    os << "# Normalized CN (posterior key is \""
       << cn->semiring->key(cn->normalizedProperties->posteriorId) << "\")."
       << std::endl;
  ...
  for (CnSortingIterator itSlot = CnSortingIterator::begin(cn),
    ...
    if (oracleProps) {
      ...
      writeConfusionNetworkArcAsText(os, alphabet, oracleProps->semiring,
                                     oracleProps->alignment[i]);
    }
...
/**
  ...
  <cn version="1.1">
    ...
    <body [n="#slots"]>
      <slot [n="#arcs"]>
        [<oracle> <word> ... </word> <begin> ... </begin> <end> ... </end>
<from> ... </from> <to> ... </to> <score> ... </score> ... </oracle>] <arc>
<word> ... </word> <begin> ... </begin> <end> ... </end> <from> ... </from> <to>
... </to> <score> ... </score> ... </arc>
        ...
      </slot>
      ...
...
private:
  ...
  void init() {
    ...
    symbolMap_ =
        Lexicon::us()->symbolMap(Lexicon::us()->alphabetId(cn_->alphabet));
  }
    ...
    if (oracleAlignment_)
      if (cn_->size() != oracleAlignment_->size())
        parser()->criticalError(
            "CN and oracle alignment must have same length (%d != %d)",
  ...
  void normalized(const Core::XmlAttributes atts) {
    ...
    if (!c)
      parser()->criticalError(
          "\"normalized\" tag requires \"posterior-key\" attribute.");
  ...
  void endOracle() {
    ...
    oracleSemiring_ = oracleAlignmentProperties->semiring =
        oracleSemiringXmlElement_->semiring();
    oracleAlignment_ = &oracleAlignmentProperties->alignment;
    oracleAlignment_->reserve(cn_->size());
    cn_->oracleAlignmentProperties =
        ConfusionNetwork::ConstOracleAlignmentPropertiesRef(
  ...
  void endArc() {
    cn_->back().push_back(
        ConfusionNetwork::Arc(label_, scores_, begin_, duration_, from_, to_));
    resetArc();
  }
  ...
  void endScore() {
    if (itScore_ == endScore_)
      parser()->criticalError(
          "Semiring has \"%s\" only %d dimensions, but more scores found.",
...
public:
  ConfusionNetworkXmlParser(const Core::Configuration &config)
    ...
    Core::XmlMixedElement *headerElement = new Core::XmlMixedElementRelay(
        "head", this, startHandler(&Self::startHeader),
        endHandler(&Self::endHeader), 0,
        XML_CHILD(new Core::XmlMixedElementRelay("alphabet", this,
                                                 startHandler(&Self::alphabet),
                                                 0, 0, XML_NO_MORE_CHILDREN)),
        ...
        XML_CHILD(new Core::XmlMixedElementRelay("n-best", this,
                                                 startHandler(&Self::nBest), 0,
                                                 0, XML_NO_MORE_CHILDREN)),
        XML_CHILD(new Core::XmlMixedElementRelay(
            "oracle", this, 0, endHandler(&Self::endOracle), 0,
            XML_CHILD(oracleSemiringXmlElement_), XML_NO_MORE_CHILDREN)),
        XML_NO_MORE_CHILDREN);
    Core::XmlMixedElement *bodyElement = new Core::XmlMixedElementRelay(
        "body", this, startHandler(&Self::startBody), 0, 0,
        XML_CHILD(new Core::XmlMixedElementRelay(
            "slot", this, startHandler(&Self::startSlot),
            endHandler(&Self::endSlot), 0,
            XML_CHILD(new Core::XmlMixedElementRelay(
                "oracle", this, startHandler(&Self::startOracleArc),
                endHandler(&Self::endOracleArc), 0,
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "word", this, 0, endHandler(&Self::endWord),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "begin", this, 0, endHandler(&Self::endBegin),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "duration", this, 0, endHandler(&Self::endDuration),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "from", this, 0, endHandler(&Self::endFrom),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "to", this, 0, endHandler(&Self::endTo),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "score", this, 0, endHandler(&Self::endScore),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_NO_MORE_CHILDREN)),
            XML_CHILD(new Core::XmlMixedElementRelay(
                "arc", this, startHandler(&Self::startArc),
                endHandler(&Self::endArc), 0,
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "word", this, 0, endHandler(&Self::endWord),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
            ...
            XML_NO_MORE_CHILDREN)),
        XML_NO_MORE_CHILDREN);
    Core::XmlRegularElementRelay *rootElement =
        new Core::XmlRegularElementRelay("cn", this,
                                         startHandler(&Self::startRoot), 0);
...
ConstSemiringRef emptySemiring_ =
    Semiring::create(Fsa::SemiringTypeTropical, 0);
} // namespace

ConstConfusionNetworkRef readConfusionNetworkFromXml(std::istream &is,
                                                     ConfusionNetworkRef cn) {
  if (!parser_)
    parser_ = new ConfusionNetworkXmlParser(Core::Configuration(
        Core::Application::us()->getConfiguration(), "cn-parser"));
  if (!cn) {
...
// -------------------------------------------------------------------------
namespace {
void writeConfusionNetworkArcContentAsXml(Core::XmlWriter &xml,
                                          const ConfusionNetwork::Arc arc,
                                          Fsa::ConstAlphabetRef alphabet,
                                          ConstSemiringRef semiring) {
  ...
  if (arc.begin != Speech::InvalidTimeframeIndex)
    xml << Core::XmlFull("begin", Core::form("%.3f", (f32(arc.begin) / 100.0)))
        << Core::XmlFull("duration",
                         Core::form("%.3f", (f32(arc.duration) / 100.0)));
  if ((arc.from != Core::Type<u32>::max) && (arc.to != Core::Type<u32>::max))
    xml << Core::XmlFull("from", arc.from) << Core::XmlFull("to", arc.to);
  for (Scores::const_iterator itScore = arc.scores->begin(),
                              endScore = arc.scores->begin() + semiring->size();
       itScore != endScore; ++itScore)
...
void writeConfusionNetworkAsXml(Core::XmlWriter &xml,
                                ConstConfusionNetworkRef cn,
                                ConstSegmentRef segment, u32 ioFlag) {
  if (!cn)
    return;
  {
    ...
    if (segment) {
      ...
      if (segment->hasStartTime() && segment->hasEndTime())
        xmlCnOpen +
            Core::XmlAttribute("start",
                               Core::form("%.3f", segment->startTime())) +
  ...
  if (ioFlag & IoFlag::WriteBody) {
    ...
    const ConfusionNetwork::OracleAlignmentProperties::OracleAlignment
        *oracleAlignment = 0;
    if (cn->isOracleAlignment()) {
    ...
    for (CnSortingIterator itSlot = CnSortingIterator::begin(cn),
      ...
      if (oracleAlignment) {
        xml << Core::XmlOpen("oracle");
        writeConfusionNetworkArcContentAsXml(xml, (*oracleAlignment)[i],
                                             alphabet, oracleSemiring);
        xml << Core::XmlClose("oracle");
      }
      for (ConfusionNetwork::Slot::const_iterator itArc = slot.begin(),
                                                  endArc = slot.end();
...
private:
  void dump(std::ostream &os, ConstConfusionNetworkRef cn) {
    ConstSegmentRef segment =
        connected(1) ? requestSegment(1) : ConstSegmentRef();
    switch (format_) {
...
public:
  DumpConfusionNetworkNode(const std::string &name,
                           const Core::Configuration &config)
      : Node(name, config), dumpChannel_(config, "dump") {}
  virtual ~DumpConfusionNetworkNode() {}

  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(0))
      criticalError("Need a confusion network as input at port 0");
    format_ = IoFormat::CnFormat(
        IoFormat::paramCnFormat(config, IoFormat::CnFormatXml));
    hasCn_ = false;
...
NodeRef createDumpConfusionNetworkNode(const std::string &name,
                                       const Core::Configuration &config) {
