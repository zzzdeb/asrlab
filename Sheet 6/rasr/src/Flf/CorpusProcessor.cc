...
// -------------------------------------------------------------------------
Processor *CorpusProcessor::create(const Core::Configuration &config,
                                   Network *network) {
  ...
  for (Network::NodeList::iterator it = network->nodes().begin();
    ...
    SpeechSegmentNode *speechSegNode =
        dynamic_cast<SpeechSegmentNode *>(it->get());
    if (speechSegNode)
...
CorpusProcessor::CorpusProcessor(const Core::Configuration &config,
                                 Network *network,
                                 SpeechSegmentNodeList *speechSegNodes)
    : Core::Component(config),
      Processor(config, network), Speech::CorpusProcessor(config),
      corpusVisitor_(0), speechSegNodes_(speechSegNodes), good_(true) {}
...
// -------------------------------------------------------------------------
SpeechSegmentNode::SpeechSegmentNode(const std::string &name,
