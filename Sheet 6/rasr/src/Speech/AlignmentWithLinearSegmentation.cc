...
const Core::ParameterInt LinearSegmenter::Delimiter::paramNumberOfIterations(
    ...
    "number of iterations to calculate approximate silence/speech/silence "
    "segmentation",
    3, 0);

const Core::ParameterFloat LinearSegmenter::Delimiter::paramPenalty(
    "penalty", "penalty for second order statistic, zero for no penalty", 100.0,
    0);

const Core::ParameterFloat
    LinearSegmenter::Delimiter::paramMinimumSpeechProportion(
        "minimum-speech-proportion", "minimum proportion of speech in segment",
        0.7, 0.0, 1.0);

LinearSegmenter::Delimiter::Delimiter(const Core::Configuration &c)
    : Core::Component(c), numberOfIterations_(paramNumberOfIterations(config)),
...
const Core::ParameterInt LinearSegmenter::paramMaximumSegmentLength(
    ...
    "maximum number of timeframes a segment may have to enter the linear "
    "segmentation",
    Core::Type<s32>::max, 0);

const Core::ParameterBool LinearSegmenter::paramShouldUseSietill(
    "should-use-sietill", "delimitation for Sietill is special", false);

LinearSegmenter::LinearSegmenter(const Core::Configuration &c)
    : Core::Component(c), delimiter_(0), silence_(Fsa::InvalidLabelId),
...
LinearSegmenter::~LinearSegmenter() { delete delimiter_; }

u32 LinearSegmenter::nFeatures() const { return delimiter_->nFeatures(); }
...
void LinearSegmenter::feed(f32 f) { delimiter_->feed(f); }
 ...
 */
AlignmentWithLinearSegmentationNode::AlignmentWithLinearSegmentationNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c), segmenter_(select("linear-segmenter")) {
}
...
void AlignmentWithLinearSegmentationNode::initialize() {
  ...
  allophoneStateGraphBuilder_ = new AllophoneStateGraphBuilder(
      ...
      modelCombination.acousticModel(), true);
  ...
  segmenter_.setSilence(
      modelCombination.acousticModel()->silenceAllophoneStateIndex());
...
bool AlignmentWithLinearSegmentationNode::work(Flow::PortId p) {
  ...
  if (!segmenter_.getAlignment(alignment->data()))
    warning("The segment %s (%d) has been discarded.", segmentId_.c_str(),
            segmenter_.nFeatures());
