...
private:
  struct StateIdKey {
    u32 operator()(Fsa::StateId s) { return s; }
  };
...
template <class _Automaton> class NormalizeMapping : public Fsa::Mapping {
public:
  typedef typename _Automaton::ConstRef _ConstAutomatonRef;
...
public:
  ...
  virtual void dumpMemoryUsage(Core::XmlWriter &o) const {
    ...
    o << Core::XmlFull("states", accAndCoacc_.size() / 8)
      << Core::XmlClose("trim");
  }
  virtual std::string describe() const {
...
template <class _Automaton>
typename _Automaton::ConstRef trim(typename _Automaton::ConstRef f,
                                   bool progress) {
  return
      typename _Automaton::ConstRef(new TrimAutomaton<_Automaton>(f, progress));
}
...
public:
  PartialAutomaton(_ConstAutomatonRef f, Fsa::StateId newInitialStateId)
      : Precursor(f), newInitialStateId_(newInitialStateId),
        additionalFinalWeight_(f->semiring()->one()) {}
  PartialAutomaton(_ConstAutomatonRef f, Fsa::StateId newInitialStateId,
                   typename _Automaton::Weight additionalFinalWeight)
      : Precursor(f), newInitialStateId_(newInitialStateId),
        additionalFinalWeight_(additionalFinalWeight) {}
  virtual Fsa::StateId initialStateId() const { return newInitialStateId_; }
  virtual std::string describe() const {
    return Core::form("partial(%s,%d)", Precursor::fsa_->describe().c_str(),
                      newInitialStateId_);
  }
  virtual typename _Automaton::ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (sp->isFinal()) {
      ...
      _sp->setWeight(Precursor::fsa_->semiring()->extend(
          _sp->weight(), additionalFinalWeight_));
...
partial(typename _Automaton::ConstRef f,
        Fsa::StateId initialStateOfPartialAutomaton) {
  return typename _Automaton::ConstRef(
      new PartialAutomaton<_Automaton>(f, initialStateOfPartialAutomaton));
}
...
partial(typename _Automaton::ConstRef f,
        Fsa::StateId initialStateOfPartialAutomaton,
        typename _Automaton::Weight additionalFinalWeight) {
  return typename _Automaton::ConstRef(new PartialAutomaton<_Automaton>(
      f, initialStateOfPartialAutomaton, additionalFinalWeight));
}
...
public:
  ...
  virtual std::string describe() const {
    return Core::form("hypothesis(%s,%d)", Precursor::fsa_->describe().c_str(),
                      n_);
  }
...
template <class _Automaton>
typename _Automaton::ConstRef
hypothesis(typename _Automaton::ConstRef f,
           Fsa::StateId initialStateOfHypothesisAutomaton) {
  return typename _Automaton::ConstRef(new HypothesisAutomaton<_Automaton>(
      f, initialStateOfHypothesisAutomaton));
}
...
public:
  ...
  virtual std::string describe() const {
    return Core::form(
        "changeSemiring(%s;%s,%s)", Precursor::fsa_->describe().c_str(),
        Precursor::fsa_->semiring()->name().c_str(), semiring_->name().c_str());
  }
};

template <class _Automaton>
typename _Automaton::ConstRef
changeSemiring(typename _Automaton::ConstRef f,
               typename _Automaton::ConstSemiringRef semiring) {
  return typename _Automaton::ConstRef(
      new ChangeSemiringAutomaton<_Automaton>(f, semiring));
}
...
class ConverterAutomaton
    : public WrapperAutomaton<_AutomatonFrom, _AutomatonTo> {
...
public:
  ConverterAutomaton(_ConstFromAutomatonRef f, _ConstSemiringRef semiring,
                     const _Mapping &map)
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    for (typename _FromState::const_iterator a = fromSp->begin();
         a != fromSp->end(); ++a)
  ...
  virtual std::string describe() const {
    return Core::form(
        "changeSemiringAndSet(%s;%s,%s)", Precursor::fsa_->describe().c_str(),
        Precursor::fsa_->semiring()->name().c_str(), semiring_->name().c_str());
  }
};
...
convert(Core::Ref<const _AutomatonFrom> f,
        Core::Ref<const typename _AutomatonTo::Semiring> semiring,
        const _Mapping &map) {
  return Core::Ref<const _AutomatonTo>(
      new ConverterAutomaton<_AutomatonFrom, _AutomatonTo, _Mapping>(
          f, semiring, map));
}
