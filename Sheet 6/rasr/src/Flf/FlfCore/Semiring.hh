...
public:
  Semiring(size_t n, const ScoreList &scales = ScoreList(),
  ...
  virtual std::string describe(const ScoresRef &a,
                               Fsa::Hint hint = Fsa::HintNone) const;
   ...
   * create semirings
   **/
  static ConstSemiringRef create(Fsa::SemiringType, size_t n,
...
public:
  LogSemiring(size_t n, const ScoreList &scales = ScoreList(),
              const KeyList &keys = KeyList());
  virtual ~LogSemiring() {}
...
public:
  TropicalSemiring(size_t n, const ScoreList &scales = ScoreList(),
                   const KeyList &keys = KeyList());
 ...
 * If scale is not set, the dimensional scales are normalized, i.e. scale = 1.0
 * / (sum of dimensional scales). Thus, a scale of 1.0 leaves the dimensional
 *scales untouched.
 *
 **/
ConstSemiringRef cloneSemiring(ConstSemiringRef semiring);
ConstSemiringRef appendSemiring(ConstSemiringRef semiring1,
                                ConstSemiringRef semiring2);
ConstSemiringRef appendSemiring(ConstSemiringRef semiring,
                                Score scale = Semiring::DefaultScale,
...
ConstSemiringRef rescaleSemiring(ConstSemiringRef semiring, ScoreId id,
                                 ...
                                 const Key &key = Semiring::UndefinedKey);
ConstSemiringRef
toLogSemiring(ConstSemiringRef semiring,
              Score scale = 0.0); // scale == 0.0 --> 1/<max-scale>
