...
public:
  LevenshteinAutomaton(ConstAlphabetRef ref, ConstAlphabetRef test, f32 delCost,
    ...
    for (Alphabet::const_iterator i = inputAlphabet_->begin();
         i != inputAlphabet_->end(); ++i) {
      if (mapping[LabelId(i)] != InvalidLabelId)
        sp->newArc(0, Weight(corCost), LabelId(i),
                   mapping[LabelId(i)]);                   // match
      sp->newArc(0, Weight(delCost), LabelId(i), Epsilon); // deletion
      for (Alphabet::const_iterator j = outputAlphabet_->begin();
           j != outputAlphabet_->end(); ++j)
        if (mapping[LabelId(i)] != j)
          sp->newArc(0, Weight(subCost), LabelId(i),
                     LabelId(j)); // substitution
    }
    for (Alphabet::const_iterator i = outputAlphabet_->begin();
         i != outputAlphabet_->end(); ++i)
...
ConstAutomatonRef levenshtein(ConstAutomatonRef ref, ConstAutomatonRef test,
                              ...
                              f32 corCost) {
  LevenshteinAutomaton *tmp = new LevenshteinAutomaton(
      ref->getOutputAlphabet(), test->getInputAlphabet(), delCost, insCost,
      subCost, corCost);
