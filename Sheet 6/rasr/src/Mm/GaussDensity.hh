...
public:
  ...
  Weight pseudoLogPrior(const std::vector<VarianceType> &variances) const {
    ...
    for (ComponentIndex cmp = 0; cmp < variances.size(); ++cmp) {
      result += 0.5 * (scales_[cmp] * scales_[cmp] * variances[cmp] +
                       log(2 * M_PI * variances[cmp]));
    }
    return result;
  }
};
...
public:
   ...
   *  Remark: output type is const reference for the moment, might need to get
   * changed.
   */
  ...
  virtual bool write(std::ostream &o) const {
    Core::Application::us()->error(
        "write() not yet implented for (inherited) Covariance");
    return false;
  }
  virtual bool read(std::istream &i) {
    Core::Application::us()->error(
        "read() not yet implented for (inherited) Covariance");
    return false;
  }
};
inline std::ostream &operator<<(std::ostream &o, const Covariance &c) {
  if (!c.write(o))
    Core::Application::us()->error(
        "an error occures while printing Covariance on stream..");
  return o;
};
inline std::istream &operator>>(std::istream &i, Covariance &c) {
  if (!c.read(i))
    Core::Application::us()->error(
        "an error occures while scaning Covariance from stream..");
  return i;
};
...
public:
  ...
  DiagonalCovariance(const std::vector<VarianceType> &v,
                     const std::vector<f64> &w)
      : buffer_(v), featureWeights_(w) {}
  DiagonalCovariance(const std::vector<VarianceType> &v) : buffer_(v) {
