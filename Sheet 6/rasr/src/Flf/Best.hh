 ...
 * Dijkstra:
 *   - aka Ftl::firstBest
 *   - extend path scores, find path with minimum w.r.t. to projection of
 *extended path scores
 ...
 *   - project scores, sum projected scores, find path with minimum sum of
 *projected path scores For the tropical semiring and if the projection of each
 *score is positive, then all three algorithms yield the same result.
 **/
typedef enum {
  Dijkstra,
  BellmanFord,
...
ConstLatticeRef best(ConstLatticeRef l,
                     SingleSourceShortestPathAlgorithm = Dijkstra);
...
typedef Core::Ref<const AllPairsShortestDistance>
...
public:
  ...
  static ConstAllPairsShortestDistanceRef
  create(ConstLatticeRef l, Time timeThreshold = Core::Type<Time>::max);
};
