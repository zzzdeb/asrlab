...
const char *Choice::endMark() { return (const char *)0; }
...
Choice::Choice() : illegalIdentifier(Choice::IllegalIdentifier) {}
...
Choice::Choice(const std::vector<std::string> &keys)
  ...
  for (std::vector<std::string>::const_iterator it = keys.begin();
       it != keys.end(); ++it, ++value)
    addChoice(it->c_str(), value);
}

Choice::Value Choice::addChoice(const char *ident) {
  Value maxValue = 0;
  for (std::set<Item, comp_by_value>::const_iterator it =
           items_by_value.begin();
       it != items_by_value.end(); ++it)
...
const std::string &Choice::operator[](const Value &value) const {
  std::set<Item, comp_by_value>::iterator it =
      items_by_value.find(Item("", value));
...
const Choice Core::boolChoice("1", true, "0", false, "yes", true, "no", false,
                              "true", true, "false", false, "on", true, "off",
                              false, Choice::endMark());

const Choice Core::infinityChoice("infinite", 1, "+infinite", 1, "-infinite", 2,
                                  "infinity", 1, "+infinity", 1, "-infinity", 2,
                                  "inf", 1, "+inf", 1, "-inf", 2, "unlimited",
                                  1, Choice::endMark());
