 ...
 */
SegmentwiseGmmTrainer::SegmentwiseGmmTrainer(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), featureDescription_(*this),
      initialized_(false), mixtureSetTrainer_(0) {
  initializeMixtureSetTrainer();
}

SegmentwiseGmmTrainer::~SegmentwiseGmmTrainer() { delete mixtureSetTrainer_; }

void SegmentwiseGmmTrainer::setFeatureDescription(
    const Mm::FeatureDescription &description) {
  Core::Ref<const Mm::FeatureScorerScaling> featureScorer(
      required_cast(const Mm::FeatureScorerScaling *,
  ...
  if (!initialized_) {
    ...
    mixtureSetTrainer_->initializeAccumulation(
        acousticModel()->nEmissions(), dimension,
    ...
    mixtureSetTrainer_->setAssigningFeatureScorer(
        featureScorer->assigningFeatureScorer());
  }
}
...
protected:
  virtual void accumulate(Core::Ref<const Feature::Vector> f,
                          Mm::MixtureIndex m, Mm::Weight w) {
...
GmmAccumulator *SegmentwiseGmmTrainer::createAcc() {
  return new GmmAccumulator(segmentwiseFeatureExtractor()->features(portId()),
                            alignmentGenerator(), mixtureSetTrainer_,
                            weightThreshold(), acousticModel());
}
...
void MinimumErrorSegmentwiseGmmTrainer::processWordLattice(
  ...
  if (!lattice->hasPart(Lattice::WordLattice::accuracyFsa)) {
    criticalError("lattice has no \" ")
        << Lattice::WordLattice::accuracyFsa << "\" part";
  }

  // calculate the posterior automaton with expectation semiring of Q(Z), with Z
  // = (A,P*A) A = accuracies, P = scaled joint probabilities
  ...
  Fsa::ConstAutomatonRef fsa = Fsa::posteriorE(
      ...
      lattice->part(Lattice::WordLattice::accuracyFsa), expectation, true,
      posteriorTolerance());

  /**
   * accumulation of the covariance Cov(A, gradient( log(P) ) )
   */
  // pass over all arcs with negative arc weights (negative accuracy =
  // denominator)
  accumulateDenominator(Fsa::multiply(fsa, Fsa::Weight(f32(-1))),
                        denominator->wordBoundaries());
  // pass over all arcs with positive arc weights (positive accuracy =
  // numerator)
 ...
 * factory
 */
SegmentwiseGmmTrainer *SegmentwiseGmmTrainer::createSegmentwiseGmmTrainer(
