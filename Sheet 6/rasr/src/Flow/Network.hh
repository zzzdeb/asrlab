...
private:
  ...
  bool dump(const bool initialCall, Core::XmlChannel &dumpChannel,
            std::set<std::string> *dumpedNetworks = 0);
  ...
  public:
    ...
    inline bool operator<(const Parameter &p) const { return name_ < p.name_; }
    friend inline std::ostream &operator<<(std::ostream &o,
                                           const Parameter &p) {
      o << p.name_ << " [ ";
      for (std::vector<Use>::const_iterator u = p.uses_.begin();
           u != p.uses_.end(); ++u)
        o << u->by << ":" << u->as << " ";
      return o << "]";
    }
  ...
  protected:
    ...
    AbstractNode *node_;
    PortId nodePort_;
    /** shows if the link is connected, i.e. port is used either by putData or
     * getData.
     */
    bool linkConnected_;
  ...
  public:
    ...
    int operator==(const Port &p) const { return name_ == p.name_; }

    bool operator<(const Port &p) const { return name_ < p.name_; }
...
private:
   ...
   *  2) creates a link for the port which gets connected to the input of the
   * repeater
  ...
  bool connectNodeInput(const std::string &nodeName,
                        const std::string &portName, Link *l);
  /** Connects a link to a node output */
  bool connectNodeOutput(const std::string &nodeName,
                         const std::string &portName, Link *l);

  /** Connects the link of the input port @param port to its repeater node.
   *  During initilization, input ports gets connected to the repeater node by a
   * link. This link is used to put data and attributes to the network. The link
   * is automatically disconnected if the network is embedded and the input port
   * is connected to a predecessor node.
   */
   ...
   *  During initilization, output ports gets connected to their source node by
   * a link. This link is used to get data and attributes from the network. The
   * link is automatically disconnected if the network is embedded and the
   * output port is connected to a successor node. In this way, we can prevent
   * that data objects are put into this link since these will never be taken
   * out. To reactivate the port for external usage see avtivateOutput.
   */
  ...
  bool setUserDefinedParameter(const std::string &name,
...
protected:
  ...
  bool validInputPort(PortId in) const { return 0 <= in && in < nInputs(); }
...
public:
   ...
   */
  bool addParameterUse(AbstractNode *node, const std::string &name,
   ...
   *  In this way, we can prevent that data objects are put into the port link
   * since these will never be taken out. Use this function only if you
   * explicitely would like to read data from an embedded network.
   */
...
public:
  ...
  NodeAttributes &addNodeAttributes(const std::string &name,
