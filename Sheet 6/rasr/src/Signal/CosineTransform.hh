...
/** Discrete Cosine Transform with an optional warping of the input 'omega'
 * range. */
...
private:
   ...
   */
  void initNplusOneData(size_t inputSize, size_t outputSize,
...
public:
  ...
  void init(InputType inputType, size_t inputSize, size_t outputSize,
            bool normalize = false) {
    init(inputType, inputSize, outputSize, normalize,
         Math::AnalyticFunctionFactory::createIdentity(), true);
  }
  void init(InputType inputType, size_t inputSize, size_t outputSize,
            bool normalize, Math::UnaryAnalyticFunctionRef,
            bool shouldWarpDifferentialUnit);

  /** Calculates cosinus transform of @param in.
   *  If normalization is set at initialization, the result (@param out) is is
   * divided by N. Remark: Normalization is not included in the transformation
   * matrix, as it neither is in (inverse) FFT.
   */
...
private:
   ...
   *    to inverse Fourier transform. Thus, input is assumed to be in frequency
   * domain and output will be transformed into time domain. Further assumption
   * is that input is only the first half of a symmetric function (similar to
   * amplitude spectrum.). Formulas:
   *    -"N+1" input type: time-sample-rate = 2 * (inputSize - 1) /
   * frequency-sample-rate.
   *    -"even about N-1/2" input type: time-sample-rate = 2 * inputSize /
   * frequency-sample-rate.
   */
