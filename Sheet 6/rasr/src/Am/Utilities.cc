...
LexiconUtilities::LexiconUtilities(const Core::Configuration &c,
                                   Bliss::LexiconRef l)
    : Component(c), lexicon_(l) {}
...
LexiconUtilities::determineSilencePronunciation() const {
  ...
  if (!silenceLemma)
    ...
    warning("Special lemma \"silence\" has multiple pronunciations."
            " Using /%s/.",
            silenceLemma->pronunciations()
                .first->pronunciation()
                ->format(lexicon_->phonemeInventory())
                .c_str());
  return silenceLemma->pronunciations().first;
}

Fsa::LabelId LexiconUtilities::determineSilenceLemmaPronunciationId() const {
  const Bliss::LemmaPronunciation *silencePronunciation =
      determineSilencePronunciation();
  if (silencePronunciation)
...
void LexiconUtilities::getInitialAndFinalPhonemes(
  ...
  for (Core::tie(pron, pron_end) = lexicon_->pronunciations(); pron != pron_end;
       ++pron) {
  ...
  for (Core::tie(phon, phon_end) = lexicon_->phonemeInventory()->phonemes();
       phon != phon_end; ++phon) {
  ...
  log("%zd distinct context dependent initial phonemes, %zd distinct context "
      "dependent final phomemes",
