...
PeakDetection::PeakDetection()
    : minPosition_(0), maxPosition_(0), maxWidthHalf_(0),
      heightAverageWidthHalf_(0), widthHeightFactor_(.5), mountainWidth_(false),
...
u32 PeakDetection::getMaximalPeakIndex(const std::vector<Amplitude> &v) const {
  ...
  for (u32 peakBegin = 1; peakBegin + 1 < v.size(); ++peakBegin) {
    ...
    if (v[peakBegin - 1] < v[peakBegin] && v[peakBegin] >= v[peakBegin + 1]) {
      ...
      for (; peakEnd + 1 < v.size() && v[peakEnd] == v[peakEnd + 1]; ++peakEnd)
        ;
      ...
      if (peakEnd + 1 < v.size() && v[peakEnd] > v[peakEnd + 1] &&
        ...
        maximumPeakPosition =
            std::min(std::max(maximumPeakPosition, minPosition_), maxPosition_);
      }
...
PeakDetection::getMaximum(const std::vector<Amplitude> &v) {
  if (needInit_)
    init();
...
PeakDetection::Amplitude
PeakDetection::getMaximalPeakValue(const std::vector<Amplitude> &v) {
  if (needInit_)
    init();
...
PeakDetection::Amplitude
PeakDetection::getAverageHeight(const std::vector<Amplitude> &v,
                                const u32 peakIndex) const {
  if (continuousHeightAverageWidth_ > 0) {
    u32 width = heightAverageWidthHalf_ - 1;

    // sum over [peakIndex - width .. peakIndex - 2] within
    // [minPosition_..maxPosition_]
    u32 firstLeft =
        peakIndex > minPosition_ + width ? peakIndex - width : minPosition_;
    u32 lastLeft = peakIndex > minPosition_ + 1 ? peakIndex - 1 : minPosition_;
    Amplitude sumLeft =
        std::accumulate(v.begin() + firstLeft, v.begin() + lastLeft, 0.0);

    // sum over [peakIndex + 2 .. peakIndex + width] within
    // [minPosition_..maxPosition_]
    u32 firstRight = std::min(peakIndex + 2, maxPosition_ + 1);
    u32 lastRight = std::min(peakIndex + width, maxPosition_) + 1;
    Amplitude sumRight =
        std::accumulate(v.begin() + firstRight, v.begin() + lastRight, 0.0);

    return (sumLeft + sumRight) /
           Amplitude(lastLeft - firstLeft + lastRight - firstRight);

  } else if (continuousHeightAverageWidth_ == 0) {
    return std::accumulate(v.begin() + minPosition_,
                           v.begin() + maxPosition_ + 1, 0.0) /
...
PeakDetection::Amplitude
PeakDetection::getNormalizedMaximum(const std::vector<Amplitude> &v) {
  if (needInit_)
    init();
...
PeakDetection::Amplitude
PeakDetection::getWidthHeight(const Amplitude peakValue,
                              const Amplitude averageHeight) const {
  ...
  if (peakValue > averageHeight && widthHeightFactor_ < 1.0)
    return widthHeightFactor_ * peakValue +
           (1.0 - widthHeightFactor_) * averageHeight;

  // widthHeight must be less peakValue.
  // In case of constant signal or widthHeightFactor_ == 1.0 take the 3/4 of
  // peakValue.
...
void PeakDetection::init() {
  ...
  heightAverageWidthHalf_ =
      (u32)rint(continuousHeightAverageWidth_ * sampleRate_ / 2.0);
...
ParameterFloat PeakDetectionNode::paramMinPosition(
    "min-position",
    "min peak position in continous units depending on previous nodes");

ParameterFloat PeakDetectionNode::paramMaxPosition(
    "max-position",
    "max peak position in continous units depending on previous nodes");

ParameterFloat PeakDetectionNode::paramMaxWidth(
    "max-width", "width with which bandwith is normed (in continous units "
                 "depending on previous nodes)");

ParameterFloat PeakDetectionNode::paramHeightAverageWidth(
    "height-average-width",
    "width over which heigth average is taken (in cont. units depending on "
    "prev. nodes, 0: [min-peak-pos..max-peak=pos])",
    0, -2);

ParameterFloat PeakDetectionNode::paramWidthHeightFactor(
    "width-height-factor",
    "widthHeight = factor * peakValue + (1 - factor) * averageHeight (factor = "
    "1: peakValue * 3/4)",
    1, 0, 1);

ParameterBool PeakDetectionNode::paramMountainWidth(
    "use-mountain-width",
    "peak/mountain (includes close peaks) width for width confidence", true);
...
bool PeakDetectionNode::setParameter(const std::string &name,
...
bool PeakDetectionNode::putData(const Flow::PortId port,
                                const Flow::Timestamp &timestamp, const f32 v) {
