...
protected:
  ...
  virtual std::string format(HistoryHandle) const { return "singleton"; }
};
...
protected:
  virtual HistoryHandle acquire(HistoryHandle hd) { return hd; }
...
protected:
  virtual HistoryHandle acquire(HistoryHandle hd) {
    const Core::ReferenceCounted *rc =
        static_cast<const Core::ReferenceCounted *>(hd);
    rc->acquireReference();
    return hd;
  }
