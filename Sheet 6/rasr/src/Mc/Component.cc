...
const Core::ParameterFloat Component::paramScale("scale", "log-linear scale",
                                                 1.0);

const Core::ParameterString
    ScaleUpdate::paramMapFilename("scales-file",
                                  "XML file with model-name scale pairs");
...
bool ScaleUpdate::setMap(const Core::Configuration &c) {
  ...
  ScaleMap scaleMap;
  Core::XmlMapDocument<ScaleMap> parser(c, scaleMap, "model-combination",
                                        "model", "name", "scale");
  parser.parseFile(filename.c_str());

  ScaleMap *extendedMap = new ScaleMap;
  for (ScaleMap::const_iterator scale = scaleMap.begin();
       scale != scaleMap.end(); ++scale) {
...
//========================================================================================================
Component::Component(const Core::Configuration &c) : Core::Component(c) {
...
void Component::setScale(Scale ownScale, Scale parentScale,
                         ScaleUpdate update) {
  ownScale_ = ownScale;
