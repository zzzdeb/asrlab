...
AffineFeatureTransformAccumulator::AffineFeatureTransformAccumulator()
    : featureDimension_(0), modelDimension_(0) {}

AffineFeatureTransformAccumulator::AffineFeatureTransformAccumulator(
    size_t featureDimension, size_t modelDimension, const std::string &key)
    : featureDimension_(featureDimension), modelDimension_(modelDimension),
      key_(key), beta_(0), globalVarianceOptimization_(false),
...
void AffineFeatureTransformAccumulator::accumulate(
    Core::Ref<const Feature::Vector> feature, DensityIndex density,
    Core::Ref<MixtureSet> mixture) {
  ...
  if (globalVarianceOptimization_ | mixture->nCovariances() == 1) {
    ...
    for (u32 i = 0; i < modelDimension_; i++)
      for (u32 j = 0; j < featureDimension_ + 1; j++)
        kAccumulator_[i][j] +=
            Sum(mean[i]) / Sum(variance[i]) * Sum(extendedFeature[j]);
  } else {
    for (u32 i = 0; i < modelDimension_; i++) {
      for (u32 j = 0; j < featureDimension_ + 1; j++) {
        kAccumulator_[i][j] +=
        ...
        for (u32 k = j; k < featureDimension_ + 1; k++)
          gAccumulator_[i][j][k] += Sum(extendedFeature[j]) *
                                    Sum(extendedFeature[k]) / Sum(variance[i]);
      }
    }
  }
  ...
  for (u32 i = 0; i < featureDimension_ + 1; i++) {
    ...
    for (u32 j = i; j < featureDimension_ + 1; j++)
      globalCovarianceAccumulator_[i][j] +=
          Sum(extendedFeature[i]) * Sum(extendedFeature[j]);
  }
}

void AffineFeatureTransformAccumulator::accumulate(
    Core::Ref<const Feature::Vector> feature, DensityIndex density,
    Core::Ref<MixtureSet> mixture, Mm::Weight weight) {
  verify(featureDimension_ != 0);
  ...
  if (globalVarianceOptimization_) {
    for (u32 i = 0; i < modelDimension_; i++) {
      ...
      for (u32 j = 0; j < featureDimension_ + 1; j++) {
        for (u32 k = j; k < featureDimension_ + 1; k++) {
          gAccumulator_[i][j][k] =
              globalCovarianceAccumulator_[j][k] / Sum(globalModelVariance_[i]);
        }
      }
    }
  }
...
AffineFeatureTransformAccumulator::estimate(int iterations, Weight minObsWeight,
  ...
  if (hldaCriterion) {
    ...
    for (u32 i = modelDimension_; i < featureDimension_; i++) {
      Sum componentVariance = (globalCovarianceAccumulator_[i + 1][i + 1] -
                               globalMeanAccumulator_[i + 1] *
                                   globalMeanAccumulator_[i + 1] / beta_) /
                              beta_;
      Sum componentMean = (globalMeanAccumulator_[i + 1] / beta_);

      kAccumulator_[i] =
          globalMeanAccumulator_ * componentMean / componentVariance;
  ...
  Math::Matrix<Sum> meanSquare(globalMeanAccumulator_ / beta_,
                               globalMeanAccumulator_ / beta_);
  Math::Matrix<Sum> globalCovariance(globalCovarianceAccumulator_ / beta_ -
                                     meanSquare);
  ...
  Math::Matrix<Sum> halfCovarianceTransposed(linearTransform *
                                             globalCovariance);
  Math::Matrix<Sum> transformedCovariance(linearTransform *
                                          halfCovarianceTransposed.transpose());

  for (int i = 0; i < iterations; i++) {
    for (u32 row = 0; row < targetDimension; row++) {

      // Compute extended generalised cofactor row vector. Note that scaling is
      // arbitrary (see eq A.9 in Gales), so transpose of inverse is used
      // instead.
      Math::Matrix<Sum> linearTransform(transform);
      linearTransform.removeColumn(0);
      ...
      if (targetDimension < featureDimension_)
        // The exact motivations for these formulas should be written down
        // somewere... pseudoInverse = linearTransform * globalCovariance *
        // linearTransform.transpose();
      ...
      if (targetDimension < featureDimension_)
        // pseudoInverse= globalCovariance * linearTransform.transpose() *
        // pseudoInverse;
        cofactors =
            halfCovarianceTransposed.transpose() * pseudoInverse.column(row);
      else
      ...
      for (u32 j = 0; j < featureDimension_ + 1; j++) {
        for (u32 k = 0; k < featureDimension_ + 1; k++) {
          ...
          epsilon2 +=
              cofactors[j] * gInverse[row][j][k] * kAccumulator_[row][k];
        }
      }

      // Solve for alpha1 and alpha2
      Sum alpha1 =
          (-epsilon2 + std::sqrt(epsilon2 * epsilon2 + 4 * epsilon1 * beta_)) /
          (2 * epsilon1);
      Sum alpha2 =
          (-epsilon2 - std::sqrt(epsilon2 * epsilon2 + 4 * epsilon1 * beta_)) /
          (2 * epsilon1);
      ...
      Sum l1 = beta_ * std::log(std::abs(alpha1 * epsilon1 + epsilon2)) -
               1 / 2 * alpha1 * alpha1 * epsilon1;
      Sum l2 = beta_ * std::log(std::abs(alpha2 * epsilon1 + epsilon2)) -
               1 / 2 * alpha2 * alpha2 * epsilon1;
      Sum alpha = l1 > l2 ? alpha1 : alpha2;

      // Update matrix row using alpha, accumulators and cofactors
      Math::Vector<Sum> newRow(gInverse[row] *
                               (cofactors * alpha + kAccumulator_[row]));
      ...
      transformedCovariance.setRow(row,
                                   halfCovarianceTransposed * newRowLinear);
...
Sum AffineFeatureTransformAccumulator::score(Transform &transform,
                                             Criterion criterion) {
  verify(featureDimension_ != 0);
  ...
  if (criterion == naive)
    ...
    jacobian = Math::Lapack::logDeterminant(linearTransform * globalCovariance *
                                            linearTransform.transpose()) /
               2;
  else if (criterion == hlda)
...
void AffineFeatureTransformAccumulator::combine(
    AbstractAdaptationAccumulator &abstractOther, Mm::Weight weight) {
...
bool AffineFeatureTransformAccumulator::read(Core::BinaryInputStream &is) {
  is >> featureDimension_ >> modelDimension_ >> beta_ >> gAccumulator_ >>
      kAccumulator_ >> globalMeanAccumulator_ >> globalCovarianceAccumulator_ >>
      globalModelVariance_ >> globalVarianceOptimization_;
  return is.good();
}

bool AffineFeatureTransformAccumulator::write(
    Core::BinaryOutputStream &os) const {
  os << featureDimension_ << modelDimension_ << beta_ << gAccumulator_
     << kAccumulator_ << globalMeanAccumulator_ << globalCovarianceAccumulator_
     << globalModelVariance_ << globalVarianceOptimization_;
  return os.good();
}
...
void AffineFeatureTransformAccumulator::dump(Core::XmlOutputStream &os) {
  os << Core::XmlOpen("affine-feature-transform-accumulator") +
            Core::XmlAttribute("key", key_) +
