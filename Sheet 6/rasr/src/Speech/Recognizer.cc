...
const Core::Choice Recognizer::searchTypeChoice_(
    ...
    "teaching-word-conditioned-tree-search",
    Search::TeachingWordConditionedTreeSearchType, "fsa-search",
    Search::FsaSearchType, "expanding-fsa-search",
    Search::ExpandingFsaSearchType, "linear-search", Search::LinearSearchType,
...
const Core::ParameterChoice
    Recognizer::paramSearch("search-type", &searchTypeChoice_,
...
void Recognizer::createRecognizer() {
  delete recognizer_;
  recognizer_ = 0;
  recognizer_ = Search::Module::instance().createRecognizer(
      static_cast<Search::SearchType>(paramSearch(config)),
      select("recognizer"));
}

void Recognizer::initializeRecognizer(
    Am::AcousticModel::Mode acousticModelMode) {
  createRecognizer();
  ModelCombination modelCombination(select("model-combination"),
                                    recognizer_->modelCombinationNeeded(),
                                    acousticModelMode);
...
const Core::ParameterBool OfflineRecognizer::paramStoreTracebacks(
    "store-tracebacks", "store recognition tracebacks in archive", false);
...
const Core::ParameterString OfflineRecognizer::paramLayerName(
    "layer-name",
    "name to distinguish results of differently parameterized passes over same "
    "corpus",
...
const Core::ParameterBool
    OfflineRecognizer::paramEvaluteResult("evaluate-result",
                                          "evaluate recognition results", true);
...
OfflineRecognizer::OfflineRecognizer(const Core::Configuration &c,
                                     Am::AcousticModel::Mode acousticModelMode)
    : Core::Component(c), Precursor(c), Recognizer(c),
      partialResultInterval_(Core::Type<f32>::max), lastPartialResult_(0),
      shouldEvaluateResult_(true), latticeArchiveWriter_(0),
      tracebackArchiveWriter_(0), tracebackChannel_(config, "traceback"),
  ...
  if (paramStoreLattices(config)) {
    ...
    latticeArchiveWriter_ =
        Lattice::Archive::openForWriting(select("lattice-archive"), lexicon_);
  ...
  if (paramStoreTracebacks(config)) {
    ...
    tracebackArchiveWriter_ =
        Lattice::Archive::openForWriting(select("traceback-archive"), lexicon_);
...
void OfflineRecognizer::processResult(Bliss::SpeechSegment *s) {
  ...
  /*! \todo Should stop timer here: evaluation may take some time we don't want
   * to count it. */
  ...
  if (lattice) {
    ...
    if (tracebackArchiveWriter_) {
      tracebackArchiveWriter_->store(s->fullName(),
                                     traceback_.wordLattice(lexicon_));
    }
  }

  if (shouldEvaluateResult_) {
    evaluator_->setReferenceTranscription(s->orth());
    evaluator_->evaluate(traceback_.lemmaPronunciationAcceptor(lexicon_),
                         "single best");
...
void OfflineRecognizer::addPartialToTraceback(
    Recognizer::Traceback &partialTraceback) {
  if (!traceback_.empty() &&
...
void OfflineRecognizer::processFeatureTimestamp(
    const Flow::Timestamp &timestamp) {
  if (tracebackChannel_.isOpen())
...
void OfflineRecognizer::setFeatureDescription(
...
void OfflineRecognizer::logTraceback(const Recognizer::Traceback &traceback) {
  ...
  for (std::vector<Search::SearchAlgorithm::TracebackItem>::const_iterator tbi =
           traceback.begin();
       tbi != traceback.end(); ++tbi) {
    if (tbi->pronunciation) {
      tracebackChannel_
          << Core::XmlOpen("item") + Core::XmlAttribute("type", "pronunciation")
          << Core::XmlFull(
                 "orth",
                 tbi->pronunciation->lemma()->preferredOrthographicForm())
          << Core::XmlFull("phon", tbi->pronunciation->pronunciation()->format(
                                       lexicon_->phonemeInventory()))
          << Core::XmlFull("score",
                           f32(tbi->score.acoustic - previousScore.acoustic)) +
...
const Core::ParameterBool ConstrainedOfflineRecognizer::paramUseLanguageModel(
    "use-language-model",
    "factor to scale fsa scores with in case the lm is not used", true);
...
ConstrainedOfflineRecognizer::ConstrainedOfflineRecognizer(
    const Core::Configuration &c, Am::AcousticModel::Mode acousticModelMode)
    : Core::Component(c), Precursor(c, acousticModelMode),
      latticeArchiveReader_(0), scale_(Fsa::Weight((f32)paramScale(config))),
      fsaPrefix_(paramFsaPrefix(config)) {
  log("opening lattice archive");
  latticeArchiveReader_ = Lattice::Archive::openForReading(
      select("constrained-lattice-archive"), lexicon_);
  ...
  lemmaPronunciationToLemmaTransducer_ = Fsa::cache(
      Fsa::multiply(lexicon_->createLemmaPronunciationToLemmaTransducer(),
                    Fsa::Weight(f32(0))));

  lemmaToSyntacticTokenTransducer_ = Fsa::cache(Fsa::multiply(
      lexicon_->createLemmaToSyntacticTokenTransducer(), Fsa::Weight(f32(0))));

  if (paramUseLanguageModel(config)) {
    Core::Ref<Lm::ScaledLanguageModel> lm =
        Lm::Module::instance().createScaledLanguageModel(select("lm"),
                                                         lexicon_);
    lmFsa_ = lm->getFsa();
  }
}
...
void ConstrainedOfflineRecognizer::enterSpeechSegment(Bliss::SpeechSegment *s) {
  ...
  Fsa::ConstAutomatonRef f =
      latticeArchiveReader_->get(s->fullName(), fsaPrefix_)->mainPart();

  // map to syntactic tokens
  if (f->getInputAlphabet() ==
      lemmaPronunciationToLemmaTransducer_->getInputAlphabet()) {
    f = Fsa::composeMatching(f, lemmaPronunciationToLemmaTransducer_);
  }
  require(f->getOutputAlphabet() ==
          lemmaToSyntacticTokenTransducer_->getInputAlphabet());
  f = Fsa::projectOutput(
      Fsa::composeMatching(f, lemmaToSyntacticTokenTransducer_));

  // restrict search space to word sequences in the automaton f
  Fsa::ConstAutomatonRef g = Fsa::multiply(f, scale_);
  /*
   * not yet checked: is new implementation more efficient than the old miminize
   * implementation, i.e.,
   * Fsa::determinize(Fsa::transpose(Fsa::determinize(Fsa::transpose(g))))?
   */
