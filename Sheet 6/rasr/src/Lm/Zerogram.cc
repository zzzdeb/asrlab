...
std::string Zerogram::format(HistoryHandle) const { return "zerogram history"; }
...
public:
  ZerogramAutomaton(Bliss::LexiconRef lexicon, Score score)
    ...
    Core::Ref<const Bliss::SyntacticTokenAlphabet> si =
        lexicon->syntacticTokenAlphabet();
    for (Bliss::Lexicon::SyntacticTokenIterator i =
             lexicon_->syntacticTokens().first;
