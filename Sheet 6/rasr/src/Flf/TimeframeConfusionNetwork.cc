...
private:
  ConstSemiringRef confSemiring() const {
    if (!confSemiring_) {
      confSemiring_ = Semiring::create(Fsa::SemiringTypeTropical, 1);
  ...
  Fsa::StateId addStateAndArc(Fsa::StateId sid, StaticLattice *s,
                              StaticBoundaries *b, Probability p,
...
public:
  ...
  ConstLatticeRef convert(ConstPosteriorCnRef cn) const {
    ...
    for (Fsa::StateId sid = 0; sid < cn->size(); ++sid) {
      ...
      for (PosteriorCn::Slot::const_iterator itPdf = pdf.begin();
           itPdf != pdf.end(); ++itPdf) {
        ScoresRef score = confSemiring()->create();
    ...
    s->setStateFinal(s->newState(cn->size()),
                     confSemiring()->clone(confSemiring()->one()));
  ...
  ConstLatticeRef decode(ConstPosteriorCnRef cn) const {
    ...
    if (hasPendingEps)
      sid = addStateAndArc(sid, s, b, epsPosterior, Fsa::Epsilon);
    s->setStateFinal(s->newState(sid),
                     confSemiring()->clone(confSemiring()->one()));
    b->set(sid, Boundary(Time(sid)));
    s->setInitialStateId(0);
    return ConstLatticeRef(s);
  ...
  for (u32 i = 0; i < cn.size(); ++i) {
    ...
    if (sum < 0.0)
      Core::Application::us()->warning(
          "In normalizePosteriorCn: Negative sum %f", sum);
    for (PosteriorCn::Slot::iterator itPdf = pdf.begin(); itPdf != pdf.end();
         ++itPdf)
      itPdf->score /= sum;
  }
}
...
// -------------------------------------------------------------------------
const Core::ParameterFloat FramePosteriorCnFeatures::paramAlpha("alpha",
                                                                "alpha", 0.0);
Score FramePosteriorCnFeatures::smooth(Score error, Score duration,
                                       Score alpha) {
...
FramePosteriorCnFeatures::~FramePosteriorCnFeatures() { delete internal_; }

void FramePosteriorCnFeatures::update(Fsa::LabelId label, Time begin,
                                      Time duration) {
...
FramePosteriorCnFeaturesRef
FramePosteriorCnFeatures::create(ConstPosteriorCnRef cn) {
  verify(cn);
...
// -------------------------------------------------------------------------
namespace {
const Core::ParameterFloat paramGamma("gamma", "gamma correction", 1.0);
} // namespace
// -------------------------------------------------------------------------
...
public:
  ExtendByFramePosteriorConfidenceLattice(ConstLatticeRef l,
                                          ConstPosteriorCnRef cn, ScoreId id,
...
ConstLatticeRef extendByFCnConfidence(ConstLatticeRef l, ConstPosteriorCnRef cn,
                                      ScoreId id, RescoreMode rescoreMode) {
  return ConstLatticeRef(
      new ExtendByFramePosteriorConfidenceLattice(l, cn, id, rescoreMode));
}
...
public:
  ExtendByPosteriorCnConfidenceNode(const std::string &name,
                                    const Core::Configuration &config)
      : Precursor(name, config) {}
  ~ExtendByPosteriorCnConfidenceNode() {}
};
NodeRef
createExtendByPosteriorCnConfidenceNode(const std::string &name,
                                        const Core::Configuration &config) {
...
public:
  struct FeatureIds {
    ...
    Probability alpha;
    FeatureIds()
        : confidenceId(Semiring::InvalidId), errorId(Semiring::InvalidId),
          alpha(0.05) {}
  };
...
public:
  FramePosteriorCnFeatureLattice(ConstLatticeRef l, ConstPosteriorCnRef cn,
  ...
  virtual void rescore(State *sp) const {
    ...
    for (State::iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if (featureIds_.errorId != Semiring::InvalidId)
        a->weight_->set(featureIds_.errorId,
                        features_->error(featureIds_.alpha));
    }
  }

  virtual std::string describe() const {
    return Core::form("addFramePosteriorCnFeatures(%s)",
                      fsa_->describe().c_str());
  }
};
...
protected:
  ConstLatticeRef rescore(ConstLatticeRef l) {
    ...
    gammaCorrection(cn, gamma_, true);
    return ConstLatticeRef(
        new FramePosteriorCnFeatureLattice(l, cn, lastIds_, rescoreMode));
  }

public:
  FramePosteriorCnFeatureNode(const std::string &name,
                              const Core::Configuration &config)
      : Precursor(name, config) {}
  ...
  void init(const std::vector<std::string> &arguments) {
    ...
    lastIds_.alpha =
        FramePosteriorCnFeatures::paramAlpha(select("error"), 0.05);
    msg << "error smoothing alpha is " << lastIds_.alpha;
  }
};
const Core::ParameterString
    FramePosteriorCnFeatureNode::paramConfidenceKey("confidence-key",
                                                    "fCN confidence", "");
...
NodeRef createFramePosteriorCnFeatureNode(const std::string &name,
                                          const Core::Configuration &config) {
