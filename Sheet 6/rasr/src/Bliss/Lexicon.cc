...
// ===========================================================================
Lemma::Lemma() : Token(), pronunciations_(0) {}

Lemma::~Lemma() {}

void Lemma::setName(Symbol symbol) { setSymbol(symbol); }

u32 Lemma::nPronunciations() const {
  u32 result = 0;
  for (PronunciationIterator pi(pronunciations_);
       pi != PronunciationIterator(0); ++pi)
    ++result;
  return result;
}
...
bool Pronunciation::Equality::operator()(const Phoneme::Id *ll,
                                         const Phoneme::Id *rr) const {
  if (ll == rr)
    return true;
...
bool Pronunciation::Equality::operator()(const Pronunciation *l,
                                         const Pronunciation *r) const {
...
std::string Pronunciation::format(
...
Lexicon::Lexicon(const Configuration &c)
    : Component(c), symbolSequences_(symbols_), internal_(0) {
  internal_ = new Internal;
}

Lexicon::~Lexicon() {
  for (PronunciationList::const_iterator p = pronunciations_.begin();
       p != pronunciations_.end(); ++p)
    delete *p;
  delete internal_;
}
...
void Lexicon::setOrthographicForms(Lemma *lemma,
                                   const std::vector<std::string> &orths) {
  require(lemma);
  for (std::vector<std::string>::const_iterator orth = orths.begin();
       orth != orths.end(); ++orth) {
...
void Lexicon::setPhonemeInventory(Core::Ref<const PhonemeInventory> pi) {
  require(
      !phonemeInventory_); // changing it could invalidate all pronunciaitions
  phonemeInventory_ = pi;
}

Pronunciation *
Lexicon::getOrCreatePronunciation(const std::vector<Phoneme::Id> &phonemes) {
...
void Lexicon::parsePronunciation(const std::string &phonStr,
...
void Lexicon::addPronunciation(Lemma *lemma, Pronunciation *pron, f32 weight) {
  ...
  LemmaPronunciation *lp = lemmaPronunciations_.add(
      LemmaPronunciation(lemmaPronunciationsByIndex_.size()));
...
void Lexicon::normalizePronunciationWeights(Lemma *lemma) {
  if (lemma->nPronunciations() == 0)
    return;
  f32 sum = 0.0;
  for (LemmaPronunciation *lp = lemma->pronunciations_; lp;
       lp = lp->nextForThisLemma_)
    sum += lp->pronunciationProbability();
  if (sum <= 0.0) {
    error("pronunciation probabilities for lemma %s cannot be normalized: sum "
          "of weights is %f",
...
void Lexicon::setSyntacticTokenSequence(Lemma *lemma,
                                        const std::vector<std::string> &synt) {
  require(lemma);
  synts_.start();
  for (std::vector<std::string>::const_iterator i = synt.begin();
       i != synt.end(); ++i) {
    SyntacticToken *token = getOrCreateSyntacticToken(symbols_[*i]);
    synts_.grow(token);
    token->addLemma(lemma);
  }
  SyntacticTokenSequence tokenSequence(synts_.currentBegin(),
                                       synts_.currentEnd());
  synts_.finish();
...
void Lexicon::setDefaultSyntacticToken(Lemma *lemma) {
  require(lemma);
  require(lemma->nOrthographicForms());
  SyntacticToken *token =
      getOrCreateSyntacticToken(lemma->preferredOrthographicForm());
  token->addLemma(lemma);
...
void Lexicon::addEvaluationTokenSequence(Lemma *lemma,
                                         const std::vector<std::string> &eval) {
  require(lemma);
  evals_.start();
  for (std::vector<std::string>::const_iterator i = eval.begin();
       i != eval.end(); ++i) {
    EvaluationToken *token = getOrCreateEvaluationToken(symbols_[*i]);
    evals_.grow(token);
  }
  EvaluationTokenSequence tokenSequence(evals_.currentBegin(),
                                        evals_.currentEnd());
  evals_.finish();
...
void Lexicon::setDefaultEvaluationToken(Lemma *lemma) {
  ...
  EvaluationToken *token =
      getOrCreateEvaluationToken(lemma->preferredOrthographicForm());
...
void Lexicon::writeXml(Core::XmlWriter &os) const {
  ...
  for (tie(l, l_end) = lemmas(); l != l_end; ++l) {
    ...
    for (tie(p, p_end) = lemma->pronunciations(); p != p_end; ++p) {
      os << Core::XmlFull("phon",
                          p->pronunciation()->format(phonemeInventory())) +
    ...
    if (sts.size()) {
      ...
      for (SyntacticTokenSequence::Iterator st = sts.begin(); st != sts.end();
           ++st) {
    ...
    for (tie(e, e_end) = lemma->evaluationTokenSequences(); e != e_end; ++e) {
      if (e->size()) {
        ...
        for (EvaluationTokenSequence::Iterator et = e->begin(); et != e->end();
             ++et) {
...
void Lexicon::logStatistics() const {
  ...
  msg << "\nnumber of phonemes:                    "
      << ((phonemeInventory()) ? phonemeInventory()->nPhonemes() : 0)
  ...
  for (tie(p, p_end) = pronunciations(); p != p_end; ++p)
    nPhonemes += (*p)->length();
  msg << "\naverage number of phonemes per pronunciation: "
      << f32(nPhonemes) / nPronunciations();
}
...
Core::Ref<const LemmaAlphabet> Lexicon::lemmaAlphabet() const {
  ...
  if (internal_->lemmaAlphabet_)
    ...
    internal_->lemmaAlphabet_ = result =
        Core::ref(new LemmaAlphabet(LexiconRef(this)));
  return result;
}

Core::Ref<const LemmaPronunciationAlphabet>
Lexicon::lemmaPronunciationAlphabet() const {
  ...
  if (internal_->lemmaPronunciationAlphabet_)
    ...
    internal_->lemmaPronunciationAlphabet_ = result =
        Core::ref(new LemmaPronunciationAlphabet(LexiconRef(this)));
  return result;
}

Core::Ref<const SyntacticTokenAlphabet>
Lexicon::syntacticTokenAlphabet() const {
  Core::Ref<const SyntacticTokenAlphabet> result;
  if (internal_->syntacticTokenAlphabet_)
    result = internal_->syntacticTokenAlphabet_;
  else
    internal_->syntacticTokenAlphabet_ = result =
        Core::ref(new SyntacticTokenAlphabet(ref(this)));
  return result;
}

Core::Ref<const EvaluationTokenAlphabet>
Lexicon::evaluationTokenAlphabet() const {
  ...
  if (internal_->evaluationTokenAlphabet_)
    ...
    internal_->evaluationTokenAlphabet_ = result =
        Core::ref(new EvaluationTokenAlphabet(ref(this)));
  return result;
}
...
Core::Ref<const LetterAlphabet> Lexicon::letterAlphabet() const {
  ...
  if (internal_->letterAlphabet_)
    ...
    internal_->letterAlphabet_ = result =
        Core::ref(new LetterAlphabet(ref(this)));
  return result;
}
 ...
 */
Core::Ref<Fsa::Automaton>
Lexicon::createLemmaToOrthographyTransducer(bool shallIncludeVariants) const {
  ...
  for (LemmaIterator lemma = lemmas().first; lemma != lemmas().second;
       ++lemma) {
    const OrthographicFormList &forms((*lemma)->orthographicForms());
    OrthographicFormList::Iterator orth = forms.begin();
    OrthographicFormList::Iterator orth_end =
        (shallIncludeVariants) ? forms.end() : (forms.begin() + 1);
...
struct Lexicon::Internal::PronunciationSuffix {
  ...
  struct Equality : Pronunciation::Equality {
    bool operator()(const PronunciationSuffix &ll,
                    const PronunciationSuffix &rr) const {
...
class Lexicon::Internal::PronunciationSuffixMap
    : public Core::hash_map<PronunciationSuffix, Fsa::StateId,
...
Lexicon::createPhonemeToLemmaPronunciationTransducer(
  ...
  return createPhonemeToLemmaPronunciationTransducer(
      phonemeInventory()->phonemeAlphabet()->nDisambiguators(),
      shallDisambiguate, false);
}

Core::Ref<PhonemeToLemmaPronunciationTransducer>
Lexicon::createPhonemeToLemmaPronunciationTransducer(
    size_t nDisambiguators, bool shallDisambiguate,
    bool markInitialPhonesAndSilence) const {
  ...
  Core::Ref<PhonemeToLemmaPronunciationTransducer> result =
      Core::ref(new PhonemeToLemmaPronunciationTransducer);
  result->setType(Fsa::TypeTransducer);
  result->setSemiring(Fsa::TropicalSemiring);
  ...
  for (PronunciationIterator pron = pronunciations().first;
       pron != pronunciations().second; ++pron) {
    suffix.homophoneIndex = 0;
    for (Pronunciation::LemmaIterator l = (*pron)->lemmas().first;
         l != (*pron)->lemmas().second; ++l) {
      const LemmaPronunciation *lp = l;
      Fsa::LabelId out = lp->id();
      if ((*pron)->phonemes()[0] != Phoneme::term) {
        Fsa::State *from = initial;
        Fsa::State *next = from;
        for (suffix.phonemes = (*pron)->phonemes();
             next && *suffix.phonemes != Phoneme::term;) {
          ...
          if (to == Fsa::InvalidStateId) {
            ...
            if (*suffix.phonemes == Phoneme::term) {
              next->newArc(initial->id(), weight,
                           pi->disambiguator(suffix.homophoneIndex),
                           Fsa::Epsilon);
            }
            to = next->id();
        ...
        warning("empty pronunciation of lemma '%s' added to transducer (may "
                "not be determinizable?)",
                l->lemma()->name().str());
        initial->newArc(initial->id(), weight,
                        pi->disambiguator(suffix.homophoneIndex), out);
      }
  ...
  for (size_t d = 0; d < nDisambiguators; ++d)
    initial->newArc(initial->id(), weight,
                    pi->disambiguator(nLexiconDisambiguators + d),
                    li->disambiguator(d));
...
/**
 * - disambiguator(1) between empty-pronunciation and corresponding
 *non-empy-pronuncaition
 * - disambiguator(0) else
 **/
Core::Ref<PhonemeToLemmaPronunciationTransducer>
Lexicon::createPhonemeToLemmaPronunciationAndStickPunctuationTransducer()
    const {
  Core::Ref<const PhonemeAlphabet> pi = phonemeInventory()->phonemeAlphabet();
  Core::Ref<const LemmaPronunciationAlphabet> li = lemmaPronunciationAlphabet();
        ...
        for (suffix.phonemes = (*pron)->phonemes();
          ...
          if (to == Fsa::InvalidStateId) {
            ...
            if (*suffix.phonemes == Phoneme::term) {
              ...
              next->newArc(postPunctuation->id(), weight, disambig1Id,
                           Fsa::Epsilon);
            }
            to = next->id();
            suffixState.add(suffix, to);
...
// ---------------------------------------------------------------------------
Core::Ref<LemmaPronunciationToLemmaTransducer>
Lexicon::createLemmaPronunciationToLemmaTransducer(u32 nDisambiguators) const {
  Core::Ref<const LemmaPronunciationAlphabet> lpa =
      lemmaPronunciationAlphabet();
  Core::Ref<const LemmaAlphabet> li = lemmaAlphabet();
  LemmaPronunciationToLemmaTransducer *result =
      new LemmaPronunciationToLemmaTransducer;
  result->setType(Fsa::TypeTransducer);
  result->setSemiring(Fsa::TropicalSemiring);
  ...
  for (LemmaPronunciationList::const_iterator lp =
           lemmaPronunciationsByIndex_.begin();
       lp != lemmaPronunciationsByIndex_.end(); ++lp) {
    state->newArc(state->id(), Fsa::Weight((*lp)->pronunciationScore()),
                  lpa->index(*lp), (*lp)->lemma()->id());
  }
  for (u32 l = 0; l < nDisambiguators; ++l)
    state->newArc(state->id(), result->semiring()->one(), lpa->disambiguator(l),
                  li->disambiguator(l));
  result->normalize();
  result->setProperties(Fsa::PropertyAcyclic | Fsa::PropertyLinear, 0);
  return Core::ref(result);
...
// ---------------------------------------------------------------------------
Core::Ref<LemmaToSyntacticTokenTransducer>
Lexicon::createLemmaToSyntacticTokenTransducer(
  ...
  for (LemmaIterator l = lemmas().first; l != lemmas().second; ++l) {
    ...
    if (useEmptySyntacticTokenSequences && tokens.isEpsilon()) {
      initial->newArc(initial->id(), result->semiring()->one(), li->index(*l),
                      Fsa::Epsilon);
    } else {
      Fsa::State *from = initial;
      for (u32 i = 0; i < tokens.length(); ++i) {
        Fsa::State *to =
            (i < tokens.length() - 1) ? result->newState() : initial;
        Fsa::LabelId in = (i == 0) ? li->index(*l) : Fsa::Epsilon;
        from->newArc(to->id(), Fsa::Weight(tokens[i]->classEmissionScore()), in,
                     si->index(tokens[i]));
        from = to;
      }
    }
  ...
  for (size_t l = 0; l < nDisambiguators; ++l) {
    initial->newArc(initial->id(), result->semiring()->one(),
                    li->disambiguator(l), si->disambiguator(l));
  }
  result->normalize();
  result->setProperties(Fsa::PropertyAcyclic | Fsa::PropertyLinear, 0);
  return Core::ref(result);
...
Core::Ref<LemmaToEvaluationTokenTransducer>
Lexicon::createLemmaToEvaluationTokenTransducer(bool allowAlternatives) const {
  Core::Ref<const LemmaAlphabet> li = lemmaAlphabet();
  Core::Ref<const EvaluationTokenAlphabet> ei = evaluationTokenAlphabet();

  LemmaToEvaluationTokenTransducer *result =
      new LemmaToEvaluationTokenTransducer;
  result->setType(Fsa::TypeTransducer);
  result->setSemiring(Fsa::TropicalSemiring);
  result->setInputAlphabet(li);
    ...
    for (tie(e, e_end) = (*l)->evaluationTokenSequences(); e != e_end; ++e) {
      if (e->isEpsilon()) {
        ...
        for (u32 i = 0; i < e->length(); ++i) {
          ...
          from->newArc(to->id(), result->semiring()->one(),
...
Core::Ref<LemmaToEvaluationTokenTransducer>
Lexicon::createLemmaToPreferredEvaluationTokenSequenceTransducer() const {
