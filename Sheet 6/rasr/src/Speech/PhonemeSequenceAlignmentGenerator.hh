...
public:
  struct Key {
    ...
    std::string string() const {
      return Core::form("%d|%d|%d|%d|%d", _id, _tbeg, _tend, _leftContext,
                        _rightContext);
    }
    bool operator==(const Key &rhs) const {
      return ((_id == rhs._id) && (_tbeg == rhs._tbeg) &&
              (_tend == rhs._tend) && (_leftContext == rhs._leftContext) &&
              (_rightContext == rhs._rightContext));
    }

    friend Core::BinaryInputStream &operator>>(Core::BinaryInputStream &i,
                                               Key &key) {
      return (i >> key._id >> key._tbeg >> key._tend >> key._leftContext >>
              key._rightContext);
    }
    friend Core::BinaryOutputStream &operator<<(Core::BinaryOutputStream &o,
                                                const Key &key) {
      return (o << key._id << key._tbeg << key._tend << key._leftContext
                << key._rightContext);
    }
  };
  ...
  struct KeyHash {
    size_t operator()(const Key &key) const {
      return (key._id & 0x0FFF) | ((key._tbeg & 0x03FF) << 12) |
             ((key._tend & 0x03FF) << 22);
    }
  };
...
public:
  PhonemeSequenceAlignmentGenerator(
      ...
      Core::Ref<const ModelCombination> = Core::Ref<const ModelCombination>());
  ...
  const Alignment *getAlignment(const Bliss::LemmaPronunciation &,
                                TimeframeIndex tbeg, TimeframeIndex tend);
  const Alignment *
  getAlignment(const Bliss::Coarticulated<Bliss::LemmaPronunciation> &,
               TimeframeIndex tbeg, TimeframeIndex tend);
  void useAlignmentCache(bool use) { useAlignmentCache_ = use; }
