...
public:
  ...
  private:
    Boundaries(ConstBoundariesRef srcBoundaries, s32 startTime, s32 endTime)
        : srcBoundaries_(srcBoundaries), startTime_(startTime),
          endTime_(endTime), boundaries_(0) {
      initialBoundary_.setTime(0);
      finalBoundary_.setTime(endTime_ - startTime_);
      finalTm1Boundary_.setTime((startTime_ == endTime_)
                                    ? InvalidTime
                                    : Time(endTime_ - startTime_ - 1));
    }
...
protected:
  ScoresRef sssp(Fsa::StateId initSid) const {
    ...
    while (!Q.empty()) {
      ...
      if (sr->isFinal())
        ssspScore = semiring_->collect(ssspScore,
                                       semiring_->extend(score, sr->weight()));
  ...
  inline void addArc(Action action, State *from, const State *to,
                     ScoresRef score, Fsa::LabelId input,
    ...
    case ActionAddFinalArc: {
      ScoresRef finalScore =
          to->hasArcs() ? sssp(to->id())
                        : (to->isFinal() ? to->weight() : semiring_->zero());
    ...
    case ActionAddSentenceEndArc: {
      ...
      // true if lattice is trim
      // verify(semiring_->compare(finalScore, semiring_->zero()) != 0);
      from->newArc(1, semiring_->extend(score, finalScore), sentenceEndInput_,
                   sentenceEndOutput_);
      break;
    }
    default:
  ...
  State *buildInitialState(State *sp) const {
    ...
    verify(initialT == 0);
    if (startTime_ < 0) {
      sp->newArc(srcInitialSid + 2, semiring_->clone(semiring_->one()),
                 Fsa::Epsilon, Fsa::Epsilon);
    } else if (startTime_ == 0) {
      buildState(sp, srcInitialSid);
    } else {
      TopologicalOrderQueueRef initialTopologicalQeue =
          createTopologicalOrderQueue(fsa_, topologicalOrderMap_);
      TopologicalOrderQueue &Q = *initialTopologicalQeue;
      Core::Vector<ScoresRef> initialSsspScores(topologicalOrderMap_->maxSid +
                                                1);
      ...
      while (!Q.empty()) {
        ...
        for (State::const_iterator a = sr->begin(), a_end = sr->end();
             a != a_end; ++a) {
          ...
          if (targetT > startTime_) {
            ...
            if (forceSentenceEndSymbol_) {
              ...
              else if (((targetT >= endTime_) ||
                        (targetSr->isFinal() && !targetSr->hasArcs())) &&
                       ((a->input() == sentenceEndInput_) ||
                        (a->output() == sentenceEndOutput_)))
            ...
            addArc(action, sp, targetSr.get(), arcScore, a->input(),
                   a->output());
          } else {
            if (!initialSsspScores[a->target()]) {
              initialSsspScores[a->target()] =
                  semiring_->extend(score, a->weight());
              Q.insert(a->target());
            } else
              initialSsspScores[a->target()] =
                  semiring_->collect(initialSsspScores[a->target()],
                                     semiring_->extend(score, a->weight()));
          }
        }
      }
  ...
  State *buildState(State *sp, Fsa::StateId srcSid) const {
    ...
    for (State::const_iterator a = srcSr->begin(), a_end = srcSr->end();
         a != a_end; ++a) {
      ...
      if (forceSentenceEndSymbol_) {
        ...
        if ((targetT >= endTime_) ||
            (targetSr->isFinal() && !targetSr->hasArcs() &&
             ((a->input() == Fsa::Epsilon) && (a->output() == Fsa::Epsilon))))
          action = ActionAddFinalArc;
        else
          action = ActionAddArc;
      }
...
public:
  FittingLattice(ConstLatticeRef l, s32 startTime, s32 endTime,
                 bool forceSentenceEndSymbol)
      : SlaveLattice(l), startTime_(startTime), endTime_(endTime),
        forceSentenceEndSymbol_(forceSentenceEndSymbol) {
    topologicalOrderMap_ = findTopologicalOrder(l);
    verify(topologicalOrderMap_ &&
           (topologicalOrderMap_->maxSid != Fsa::InvalidStateId));
    if (startTime_ == Core::Type<s32>::max)
      startTime_ = 0;
    if (endTime_ == Core::Type<s32>::max) {
      endTime_ = 0;
      Fsa::StateId sid = 0;
      for (StateMap::const_iterator it = topologicalOrderMap_->begin(),
                                    it_end = topologicalOrderMap_->end();
           it != it_end; ++it, ++sid)
    ...
    if (forceSentenceEndSymbol_) {
      ...
      sentenceEndOutput_ =
          (l->type() == Fsa::TypeAcceptor) ? sentenceEndInput_ : Fsa::Epsilon;
    } else
    ...
    setBoundaries(Flf::ConstBoundariesRef(
        new Boundaries(srcBoundaries_, startTime_, endTime_)));
    if (startTime_ == endTime_) {
      ...
      if (semiring_->compare(finalScores, semiring_->one()) != 0)
        Core::Application::us()->error(
            "Fitting lattice: Result is a single state lattice with final "
            "weight %s != %s;\n"
            "discard final weight.",
            semiring_
                ->describe(finalScores, Fsa::HintShowDetails | HintUnscaled)
                .c_str(),
            semiring_
                ->describe(semiring_->one(),
                           Fsa::HintShowDetails | HintUnscaled)
                .c_str());
      initialSr_ =
          ConstStateRef(new State(0, Fsa::StateTagFinal, semiring_->one()));
    } else if ((startTime_ + 1 == endTime_) && forceSentenceEndSymbol_) {
      State *sp = new State(0);
      sp->newArc(1, sssp(fsa_->initialStateId()), sentenceEndInput_,
                 sentenceEndOutput_);
  ...
  virtual std::string describe() const {
    return Core::form("fit-lattice(%s,%d-%d)", fsa_->describe().c_str(),
                      startTime_, endTime_);
  }
};

ConstLatticeRef fit(ConstLatticeRef l, bool forceSentenceEndLabels) {
  return fit(l, Core::Type<s32>::max, Core::Type<s32>::max,
             forceSentenceEndLabels);
}

ConstLatticeRef fit(ConstLatticeRef l, s32 startTime, s32 endTime,
                    bool forceSentenceEndLabels) {
  verify_(l->hasProperty(Fsa::PropertyAcyclic));
  return ConstLatticeRef(
      new FittingLattice(l, startTime, endTime, forceSentenceEndLabels));
}
// -------------------------------------------------------------------------
...
const Core::ParameterBool FitLatticeNode::paramForceSentenceEndLabels(
    "force-sentence-end-labels", "put sentence end labels at final arcs",
    false);
NodeRef createFitLatticeNode(const std::string &name,
...
bool isFinite(ConstSemiringRef semiring, ScoresRef scores) {
  for (Scores::const_iterator w = scores->begin(),
                              w_end = scores->begin() + semiring->size();
       w != w_end; ++w)
    if ((*w == Semiring::Zero) || (*w == Semiring::Invalid) ||
        (std::isfinite(*w) == 0))
      return false;
  return true;
}
namespace {
typedef enum { None, White, Gray, Black } Color;
}
StaticLatticeRef cleanUp(ConstLatticeRef l) {
  ...
  Fsa::ConstAlphabetRef inputA = l->getInputAlphabet(),
                        outputA = isAcceptor ? l->getInputAlphabet()
                                             : l->getOutputAlphabet();
    ...
    if (C[sid] == White) {
      ...
      for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
        if (isValidTarget(a->target())) {
          ...
          if (isValidArc) {
            if ((C[a->target()] == Gray) || !isValidLabel(inputA, a->input()) ||
        ...
        for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
          ...
          if (discard) {
            ...
            Core::Application::us()->warning(
                "Discard arc from=%d,to=%d,i/o=%s(%d):%s(%d),w=%s", sr->id(),
                a->target(), inputA->symbol(a->input()).c_str(), a->input(),
                outputA->symbol(a->output()).c_str(), a->output(),
                semiring
                    ->describe(a->weight(), Fsa::HintShowDetails | HintUnscaled)
                    .c_str());
          } else {
            ...
            if (isAcceptor && (a->input() != a->output())) {
              ++nCorrected;
              Core::Application::us()->warning(
                  "Acceptor arc has different input and output; correct.");
              newArc.output_ = newArc.input_;
            }
          }
        }
        verify(nDiscarded + nCorrected > 0);
        Core::Application::us()->log("For state %d: %d arcs discarded and %d "
                                     "arcs corrected of %d original arcs",
                                     sr->id(), sr->nArcs(), nDiscarded,
                                     nCorrected);
      } else
        sp = new State(*sr);
      s->setState(sp);
...
NodeRef createCleanUpNode(const std::string &name,
                          const Core::Configuration &config) {
