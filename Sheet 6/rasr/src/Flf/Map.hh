...
ConstLatticeRef mapInput(Fsa::ConstAutomatonRef targetToInputFsa,
                         ConstLatticeRef l);
...
ConstLatticeRef mapOutput(ConstLatticeRef l,
                          Fsa::ConstAutomatonRef outputToTargetFsa);
...
public:
  struct ExtendedLabel {
    ...
    ExtendedLabel(Fsa::LabelId label, f32 length)
        : label(label), length(length), initial(Boundary::Transit::InvalidId),
          final(Boundary::Transit::InvalidId) {}
    ExtendedLabel(Fsa::LabelId label, f32 length, Bliss::Phoneme::Id initial,
                  Bliss::Phoneme::Id final)
...
public:
  ...
  u32 nMappings() const;
  const Mapping &identity() const { return Identity; }
...
public:
  ...
  static LabelMapRef
  createCompoundWordSplittingMap(Lexicon::AlphabetId alphabetId);
  static LabelMapRef
  load(const Core::Configuration &config,
       Fsa::ConstAlphabetRef defaultAlphabet = Fsa::ConstAlphabetRef());
};
