...
/**
   ...
   <properties-definition>
       <key> central </key> <value-map> <value id="-1"> # </value> <value
   id="0"> a </value> </value-map>
       <!--
...
public:
  PropertyMap()
      : undefinedIndex(Core::Choice::IllegalValue),
        undefinedString(Core::Choice::IllegalIdentifier), keys_(0), values_(0) {
  }

  PropertyMap(const StringList &keys, const ListOfIndexedStringList &values)
      : undefinedIndex(Core::Choice::IllegalValue),
        undefinedString(Core::Choice::IllegalIdentifier), keys_(0), values_(0) {
    set(keys, values);
  }

  virtual ~PropertyMap() { reset(); }

  virtual void set(const StringList &keys,
                   const ListOfIndexedStringList &values);

  // true, iff keys_ (and values_) are not set
  bool empty() const { return keys_ == 0; }
...
private:
  ...
  void logKey(Core::XmlWriter &xml, const std::string &name,
              const PropertyMap &map, std::string key);

  void logUniq(Core::XmlWriter &xml, std::string key, u16 diff);

  void logDiff(Core::XmlWriter &xml, std::string key);

public:
  PropertyMapDiff(const Core::Configuration &config, const PropertyMap &map1,
...
public:
  Properties(PropertyMapRef map) : map_(map) {}
  virtual ~Properties() {}
  ...
  */
  struct PtrEqual {
    bool operator()(const Properties *const props1,
                    const Properties *const props2) const {
...
public:
  ...
  StoredProperties(PropertyMapRef map, const StringList &keys,
