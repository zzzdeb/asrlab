 ...
 */
AlignmentTransformNode::AlignmentTransformNode(const Core::Configuration &c)
...
bool AlignmentGammaCorrectionNode::setParameter(const std::string &name,
...
const Core::ParameterChoice AlignmentCombineItemsNode::paramSemiringType(
    "semiring-type", &Fsa::SemiringTypeChoice,
    "define semiring to combine items", Fsa::SemiringTypeProbability);

AlignmentCombineItemsNode::AlignmentCombineItemsNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c),
      semiring_(
          Fsa::getSemiring((Fsa::SemiringType)paramSemiringType(config))) {}
...
const Core::ParameterFloat AlignmentFilterWeightsNode::paramMinWeight(
    "min-weight",
    "only alignment items with a weight greater or equal than min-weight "
    "survive",
    0.0);

const Core::ParameterFloat AlignmentFilterWeightsNode::paramMaxWeight(
    "max-weight",
    "only alignment items with a weight less or equal than max-weight survive",
    1.0);
...
bool AlignmentFilterWeightsNode::setParameter(const std::string &name,
                                              const std::string &value) {
...
const Core::ParameterFloat AlignmentResetWeightsNode::paramPreviousWeight(
    "previous-weight",
    "only alignment items with a weight smaller than this weight will be "
    "reseted",
    0.0);
...
const Core::Choice AlignmentResetWeightsNode::choiceMode(
    "larger-than", modeLargerThan, "smaller-than", modeSmallerThan,
    "tied-larger-than", modeTiedLargerThan, "tied-smaller-than",
    modeTiedSmallerThan, "nothing", modeNone, Core::Choice::endMark());

const Core::ParameterChoice AlignmentResetWeightsNode::paramMode(
    "mode", &choiceMode, "select function to treat alignment weights",
    modeNone);
...
AlignmentResetWeightsNode::AlignmentResetWeightsNode(
    ...
    : Core::Component(c), Precursor(c),
      ...
      newWeight_(paramNewWeight(config)), mode_(paramMode(config)) {
  if (mode_ == modeTiedSmallerThan || mode_ == modeTiedLargerThan) {
...
bool AlignmentResetWeightsNode::setParameter(const std::string &name,
                                             const std::string &value) {
...
void AlignmentResetWeightsNode::transform(Alignment &a) {
  if (mode_ == modeSmallerThan)
    a.resetWeightsSmallerThan(Mm::Weight(previousWeight_),
                              Mm::Weight(newWeight_));
  else if (mode_ == modeLargerThan)
    a.resetWeightsLargerThan(Mm::Weight(previousWeight_),
                             Mm::Weight(newWeight_));
  else if (mode_ == modeTiedSmallerThan || mode_ == modeTiedLargerThan) {
    ...
    for (frame_i = frames.begin(); frame_i != frames.end(); ++frame_i) {
      Alignment::iterator state_i,
          state_i_end; // start- and end-states for timeframe i
      Core::hash_map<u32, f32> frameStateProb;
      Core::hash_map<u32, u32> frameStateCounts;
      for (Core::tie(state_i, state_i_end) = *frame_i; state_i != state_i_end;
           ++state_i) {
...
AlignmentMultiplyWeightsNode::AlignmentMultiplyWeightsNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c), factor_(paramFactor(config)) {}

bool AlignmentMultiplyWeightsNode::setParameter(const std::string &name,
                                                const std::string &value) {
...
AlignmentAddWeightNode::AlignmentAddWeightNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), offset_(paramOffset(config)) {}

bool AlignmentAddWeightNode::setParameter(const std::string &name,
                                          const std::string &value) {
...
void AlignmentAddWeightNode::transform(Alignment &a) { a.addWeight(offset_); }

/** AlignmentExpmNode
 */
AlignmentExpmNode::AlignmentExpmNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c) {}

void AlignmentExpmNode::transform(Alignment &a) { a.expm(); }
...
void AlignmentRemoveEmissionScoreNode::checkFeatureDependencies(
...
void AlignmentRemoveEmissionScoreNode::transform(Alignment &a) {
  ...
  while (getData(1, in)) {
    ...
    Mm::FeatureScorer::Scorer scorer(
        acousticModel_->featureScorer()->getScorer(f));
    while (currentAlignmentItem != a.end() &&
           currentAlignmentItem->time == currentFeatureId) {
      verify(currentAlignmentItem == a.begin()
                 ? true
                 : (currentAlignmentItem - 1)->time <=
                       currentAlignmentItem->time);
      f32 emScore = scorer->score(
          acousticModel_->emissionIndex(currentAlignmentItem->emission));
  ...
  while (getData(0, ina) && getData(1, inb)) {
    ...
    for (i = framesa.begin(); i != framesa.end() && s != b.end(); ++i) {
      ...
      for (Core::tie(j, j_end) = *i; j != j_end; ++j) {
        if (j->time != s->time) {
          warning("alignment times differ: single-path=%d multi-path=%d",
                  s->time, j->time);
        } else {
          if (acousticModel_->emissionIndex(s->emission) ==
              acousticModel_->emissionIndex(j->emission))
            s->weight += j->weight;
        }
      }
      s++;
    ...
    Flow::DataAdaptor<Alignment> *alignment =
        new Flow::DataAdaptor<Alignment>(b);
    putData(0, alignment);
  }
