...
struct EditDistance::Trace : public Core::ReferenceCounted,
  ...
  Core::Ref<Trace> back;
  Trace(const Core::Ref<Trace> &_back, const Bliss::Token *_a,
        const Bliss::Token *_b, EditOperation _op)
...
struct EditDistance::Hyp {
  ...
  struct PriorityFunction {
    bool operator()(const Hyp &lhs, const Hyp &rhs) const {
      ...
      */
      return (lhs.cost == rhs.cost) ? (lhs.nRef > rhs.nRef)
                                    : (lhs.cost < rhs.cost);
    }
  };
};
...
const Core::Choice
    EditDistance::AlignmentFormatChoice("bliss", EditDistance::FormatBliss,
                                        "nist", EditDistance::FormatNist,
                                        Core::Choice::endMark());
const Core::ParameterChoice
    EditDistance::paramAlignmentFormat("format", &AlignmentFormatChoice,
                                       "format of alignment output",
                                       FormatBliss);
const Core::ParameterInt
    EditDistance::paramInsertionCost("ins-cost",
                                     "alignnent cost for insertions", 1, 0);
const Core::ParameterInt
    EditDistance::paramDeletionCost("del-cost", "alignnent cost for deletions",
                                    1, 0);
...
EditDistance::EditDistance(const Core::Configuration &c)
    : Core::Component(c), statSearchSpace_("potential search space"),
      statMaxStackSize_("maximum stack size    "),
      statNExpansions_("number of expansions  "), debugChannel_(c, "debug") {
...
EditDistance::EditCost EditDistance::subCost(const Token *a,
                                             const Token *b) const {
  require_(a && b && (a != b));
...
EditDistance::EditCost EditDistance::cost(const Token *a,
                                          const Token *b) const {
  if (a) {
    if (b)
 ...
 */
void EditDistance::align(Fsa::ConstAutomatonRef a, Fsa::ConstAutomatonRef b,
                         Alignment &out) const {
  ...
  Core::TracedPriorityQueue<Hyp, State, Hyp::KeyFunction, Hyp::PriorityFunction,
                            State::Hash>
      stack;
  ...
  const TokenAlphabet *aa =
      dynamic_cast<const TokenAlphabet *>(a->getOutputAlphabet().get());
  const TokenAlphabet *ba =
      dynamic_cast<const TokenAlphabet *>(b->getInputAlphabet().get());
  require(aa);
  require(ba);
  require(aa == ba);
  ...
  while (!stack.empty()) {
    ...
    for (ea = va->begin(); ea != va->end(); ++ea) {
      ...
      nh.back = Core::ref(
          new Trace(current.back, aa->token(ea->output()), 0, opCost.first));
    ...
    for (eb = vb->begin(); eb != vb->end(); ++eb) {
      ...
      nh.back = Core::ref(
          new Trace(current.back, 0, ba->token(eb->input()), opCost.first));
      stack.insertOrRelax(nh);
    }
    ...
    for (ea = va->begin(); ea != va->end(); ++ea) {
      ...
      for (eb = vb->begin(); eb != vb->end(); ++eb) {
        ...
        nh.back = Core::ref(new Trace(current.back, aa->token(ea->output()),
                                      ba->token(eb->input()), opCost.first));
        stack.insertOrRelax(nh);
      }
    }
  }
  ...
  if (stack.empty()) {
    error(
        "No final state found.  Apparently one automaton has no final states.");
    Hyp empty;
    empty.score = 0.0;
    empty.cost = 0;
    empty.nRef = 0;
...
void ErrorStatistic::clear() {
  nLeftTokens_ = nRightTokens_ = nInsertions_ = nDeletions_ = nSubstitutions_ =
      0;
}

ErrorStatistic::ErrorStatistic()
    : nLeftTokens_(0), nRightTokens_(0), nInsertions_(0), nDeletions_(0),
      nSubstitutions_(0) {}
...
void ErrorStatistic::operator+=(const EditDistance::Alignment &a) {
  for (EditDistance::Alignment::const_iterator i = a.begin(); i != a.end();
       ++i) {
    switch (i->op) {
