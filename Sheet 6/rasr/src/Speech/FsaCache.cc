...
const Core::ParameterString FsaCache::paramPath("path",
                                                "path of model acceptor cache");

const Core::ParameterBool
    FsaCache::paramReadOnly("read-only", "if true cache is only read", false);

FsaCache::FsaCache(const Core::Configuration &c,
                   Fsa::StoredComponents whatToStore)
    : Core::Component(c), archive_(0), whatToStore_(whatToStore),
      readOnly_(false) {
...
FsaCache::~FsaCache() { delete archive_; }
...
Fsa::ConstAutomatonRef FsaCache::read(const std::string &id) {
  ...
  if (reader.isOpen()) {
    ...
    if (!Fsa::readBinary(result.get(), reader)) {
      result.reset();
      error("Failed to read the object \"%s\" from cache \"%s\"", id.c_str(),
            archive_->path().c_str());
    } else {
      if (inputAlphabet_)
...
void FsaCache::write(const std::string &id, Fsa::ConstAutomatonRef fsa) {
  ...
  if (!Fsa::writeBinary(fsa, writer, whatToStore_)) {
    error("Failed to write the object \"%s\" to cache \"%s\"", id.c_str(),
          archive_->path().c_str());
  }
}
