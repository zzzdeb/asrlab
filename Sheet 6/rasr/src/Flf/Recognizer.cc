...
class Recognizer : public Speech::Recognizer,
                   public SegmentwiseSpeechProcessor {
public:
...
protected:
  void enterSpeechSegment(const Bliss::SpeechSegment *segment) {
    if (!segment->orth().empty()) {
      clog() << Core::XmlOpen("orth") +
                    Core::XmlAttribute("source", "reference")
             << segment->orth() << Core::XmlClose("orth");
    }
  ...
  void
  addPartialToTraceback(Search::SearchAlgorithm::Traceback &partialTraceback) {
    if (!traceback_.empty() &&
        traceback_.back().time == partialTraceback.front().time)
      partialTraceback.erase(partialTraceback.begin());
    traceback_.insert(traceback_.end(), partialTraceback.begin(),
                      partialTraceback.end());
  }

  virtual void process(const FeatureList &features) {
    Core::Ref<const Mm::ScaledFeatureScorer> scorer =
        acousticModel_->featureScorer();
    for (FeatureList::const_iterator itFeature = features.begin(),
                                     endFeature = features.end();
      ...
      if (partialResultInterval_ < Core::Type<f32>::max) {
        if (f->timestamp().startTime() - lastPartialResult_ >
            partialResultInterval_) {
  ...
  /*
    ...
    Afterwards:
    am_score = emission_scale * emission + transition_scale * transition +
    pronunciation_scale * pronunciation am_scale = 1.0 lm_score = lm lm_scale =
    lm_scale
  */
  ScoresRef buildScore(Fsa::LabelId label, Score amRecogScore,
                       Score lmRecogScore) {
  ...
  ConstLatticeRef buildLattice() {
    Core::Ref<const ::Lattice::WordLattice> lattice =
        recognizer_->getCurrentWordLattice();
    Core::Ref<const ::Lattice::WordBoundaries> boundaries =
        lattice->wordBoundaries();
    Fsa::ConstAutomatonRef amFsa =
    ...
    s->setProperties(Fsa::PropertyAcyclic | PropertyCrossWord,
                     Fsa::PropertyAll);
    ...
    Core::Vector<Fsa::StateId> sidMap(amFsa->initialStateId() + 1,
                                      Fsa::InvalidStateId);
    ...
    while (!S.isEmpty()) {
      ...
      s->setState(sp);
      b->set(sp->id(), Boundary(boundary.time(),
                                Boundary::Transit(boundary.transit().final,
                                                  boundary.transit().initial)));
      if (amSr->isFinal()) {
        sp->newArc(
            1, buildScore(Fsa::InvalidLabelId, amSr->weight(), lmSr->weight()),
            sentenceEndLabel_);
        finalTime = std::max(finalTime, boundary.time());
      }
      for (Fsa::State::const_iterator am_a = amSr->begin(),
                                      lm_a = lmSr->begin();
           (am_a != amSr->end()) && (lm_a != lmSr->end()); ++am_a, ++lm_a) {
        ...
        if (targetAmSr->isFinal() && targetLmSr->isFinal()) {
          if (am_a->input() == Fsa::Epsilon) {
            ScoresRef scores =
                buildScore(am_a->input(), am_a->weight(), lm_a->weight());
    ...
    if (applyNonWordClosureFilter_) {
      ...
      StaticLatticeRef filteredLattice =
          applyEpsClosureWeakDeterminizationFilter(l);
      trimInPlace(filteredLattice);
      l = projectOutput(filteredLattice);
    }
    if (addConfidenceScores_ || applyPosteriorPruning_) {
      std::pair<ConstLatticeRef, ConstFwdBwdRef> latAndFb =
          FwdBwd::build(l, posteriorSemiring_);
      l = latAndFb.first;
      ConstFwdBwdRef fb = latAndFb.second;
      if (addConfidenceScores_) {
        ConstPosteriorCnRef cn = buildFramePosteriorCn(l, fb);
        l = extendByFCnConfidence(l, cn, confidenceId_,
                                  RescoreModeInPlaceCache);
        l = persistent(l);
      }
  ...
  void logTraceback(const Search::SearchAlgorithm::Traceback &traceback) {
    tracebackChannel_ << Core::XmlOpen("traceback") +
                             Core::XmlAttribute("type", "xml");
    u32 previousIndex = traceback.begin()->time;
    Search::SearchAlgorithm::ScoreVector previousScore(0.0, 0.0);
    for (std::vector<Search::SearchAlgorithm::TracebackItem>::const_iterator
             tbi = traceback.begin();
         tbi != traceback.end(); ++tbi) {
      if (tbi->pronunciation) {
        tracebackChannel_
            << Core::XmlOpen("item") +
                   Core::XmlAttribute("type", "pronunciation")
            << Core::XmlFull(
                   "orth",
                   tbi->pronunciation->lemma()->preferredOrthographicForm())
            << Core::XmlFull("phon",
                             tbi->pronunciation->pronunciation()->format(
                                 lexicon_->phonemeInventory()))
            << Core::XmlFull(
                   "score", f32(tbi->score.acoustic - previousScore.acoustic)) +
        ...
        if (previousIndex < tbi->time)
          tracebackChannel_
              << Core::XmlEmpty("samples") +
                     Core::XmlAttribute(
                         "start",
                         f32(featureTimes_[previousIndex].startTime())) +
                     Core::XmlAttribute(
                         "end", f32(featureTimes_[tbi->time - 1].endTime()))
              << Core::XmlEmpty("features") +
                     Core::XmlAttribute("start", previousIndex) +
...
public:
  Recognizer(const Core::Configuration &config, ModelCombinationRef mc)
      : Core::Component(config), Speech::Recognizer(config),
        SegmentwiseSpeechProcessor(config, mc), mc_(mc),
    ...
    applyPosteriorPruning_ = paramApplyPosteriorPruning(config);
    {
      ...
      const Bliss::Lemma *special =
          mc_->lexicon()->specialLemma("sentence-end");
      if (special) {
        Bliss::Lemma::LemmaPronunciationRange lpRange =
            special->pronunciations();
        if (lpRange.first != lpRange.second)
          sentenceEndLabel_ = lpRange.first->id();
      }
      msg << "Sentence end symbol is \""
          << lpAlphabet_->symbol(sentenceEndLabel_) << "\".\n";
      ...
      if (applyNonWordClosureFilter_) {
        nonWordToEpsilonMap_ = LabelMap::createNonWordToEpsilonMap(
            Lexicon::LemmaPronunciationAlphabetId);
        msg << "Non-word-closure filter is active.\n";
      }
      if (addConfidenceScores_) {
        msg << "Confidence score calculation is active (Attention: Confidence "
               "scores are calculated on lemma pronunciations). \n";
      }
      if (applyPosteriorPruning_) {
  ...
  void setGrammar(Fsa::ConstAutomatonRef fsa) {
    require(Lexicon::us()->alphabetId(fsa->getInputAlphabet()) ==
            Lexicon::SyntacticTokenAlphabetId);
...
const Core::ParameterBool Recognizer::paramPronunciationScore(
    ...
    "add an extra dimension containing the pronunciation score", false);
const Core::ParameterBool Recognizer::paramApplyNonWordClosureFilter(
    "apply-non-word-closure-filter", "apply the non word closure filter",
    false);
const Core::ParameterBool Recognizer::paramConfidenceScore(
    "add-confidence-score",
    "add an extra dimension containing the confidence score", false);
const Core::ParameterFloat
    Recognizer::paramAlpha("alpha",
                           "scale dimensions for posterior calculation", 0.0);
const Core::ParameterBool
    Recognizer::paramApplyPosteriorPruning("apply-posterior-pruning",
                                           "prune posterior", false);
const Core::ParameterFloat
    Recognizer::paramThreshold("threshold", "posterior pruning threshold", 200);
...
protected:
  ScaledLanguageModelRef buildEmptyFsaGrammarLm() {
    ...
    Lm::FsaLm *fsaLm =
        new Lm::FsaLm(select("lm"), Bliss::LexiconRef(Lexicon::us()));
    fsaLm->setFsa(f);
    ScaledLanguageModelRef lm =
        Lm::Module::instance().createScaledLanguageModel(
            select("lm"), LanguageModelRef(fsaLm));
    return lm;
  }
  ...
  Fsa::ConstAutomatonRef buildFsaGrammar(ConstLatticeRef l) {
    ...
    */
    ...
    LatticeCounts bestCounts =
        count(applyOneToOneLabelMap(best(l), nonWordToEpsilonMap));
    u32 nBestWords = bestCounts.nArcs_ - bestCounts.nIEps_;
    LatticeCounts latticeCounts = count(l);
    f32 latticeDensity = (nBestWords == 0)
                             ? f32(0)
                             : f32(latticeCounts.nArcs_) / f32(nBestWords);
    ...
    */
    ...
    if (latticeCounts.nArcs_ > grammarMaxArcs_) {
        ...
        density = (nBestWords == 0) ? f32(0) : f32(latticeCounts.nArcs_) /
    f32(nBestWords); grammarChannel_ <<
    Core::XmlOpen("lattice-after-non-word-closure-determinization-filter");
        info(l, grammarChannel_);
        grammarChannel_ << Core::XmlFull("density", density);
        grammarChannel_ <<
    Core::XmlClose("lattice-after-non-word-closure-determinization-filter");
    }
    */

    f32 desiredDensity = std::min(latticeDensity, grammarDensity_);
    if ((desiredDensity < latticeDensity) ||
        (latticeCounts.nArcs_ > grammarMaxArcs_)) {
      StaticLatticeRef tmpL = StaticLatticeRef(new StaticLattice),
                       prunedL = StaticLatticeRef(new StaticLattice);
      std::pair<ConstLatticeRef, ConstFwdBwdRef> fbResult =
          FwdBwd::build(l, toLogSemiring(l->semiring()));
      l = fbResult.first;
      ...
      for (;;) {
        persistent(pruneByFwdBwdScores(l, fbResult.second, threshold),
                   tmpL.get(), 0);
        trimInPlace(tmpL);
        ...
        if (tmpL->initialStateId() == Fsa::InvalidStateId) {
          ...
          tmpDensity = (nBestWords == 0)
                           ? f32(0)
                           : f32(tmpCounts.nArcs_) / f32(nBestWords);
        }
        grammarChannel_ << Core::XmlFull(
            "dynamic-pruning",
            Core::form("threshold = %f, #arcs = %d, density = %f", threshold,
                       s32(tmpCounts.nArcs_), tmpDensity));
        if (tmpCounts.nArcs_ >= grammarMaxArcs_) {
          if ((prunedCounts.nArcs_ == 0) ||
              ((tmpCounts.nArcs_ - grammarMaxArcs_) <
               (grammarMaxArcs_ - prunedCounts.nArcs_))) {
      ...
      grammarChannel_ << Core::XmlFull(
          "dynamic-pruning",
          Core::form("final threshold = %f, #arcs = %d, density = %f",
                     threshold, s32(prunedCounts.nArcs_), prunedDensity));
      l = prunedL;
      latticeCounts = prunedCounts;
    ...
    */
    ...
    // grammarChannel_ <<
    // Core::XmlOpen("grammar-after-transposed-determinization"); Fsa::info(g,
    // grammarChannel_); grammarChannel_ <<
    // Core::XmlClose("grammar-after-transposed-determinization");
    g->addProperties(Fsa::PropertyAcyclic); // just to make sure
    g = Fsa::staticCopy(Fsa::determinize(g));
    Fsa::trimInPlace(g);
...
public:
  ...
  virtual ~RecognizerNode() { delete recognizer_; }
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!Lexicon::us()->isReadOnly())
      warning("Lexicon is not read-only, "
              "dynamically added/modified lemmas are not considered by the "
              "recognizer.");
    ...
    if (connected(0)) {
      ...
      grammarMaxArcs_ =
          (tmp == Core::Type<s32>::max) ? Core::Type<size_t>::max : size_t(tmp);
      grammarDensity_ = paramGrammarDensity(select("grammar"));
      grammarPruningStepWidth_ =
          paramGrammarPruningStepWidth(select("grammar"));
      if (!grammarKey_.empty())
  ...
  virtual void finalize() { recognizer_->reset(); }

  virtual ConstLatticeRef sendLattice(Port to) {
    if (isBuffered_)
      return l_;
    const Bliss::SpeechSegment *segment =
        static_cast<const Bliss::SpeechSegment *>(requestData(1));
...
const Core::ParameterString
    RecognizerNode::paramGrammarKey("key",
                                    "dimension storing the grammar scores", "");
const Core::ParameterInt RecognizerNode::paramGrammarArcsLimit(
    "arcs-limit", "maximum number of arcs in grammar", Core::Type<s32>::max, 0,
    Core::Type<s32>::max);
const Core::ParameterFloat
    RecognizerNode::paramGrammarDensity("density", "desired density",
                                        Core::Type<f32>::max, 1.0,
                                        Core::Type<f32>::max);
const Core::ParameterFloat RecognizerNode::paramGrammarPruningStepWidth(
    "pruning-step", "step width between two prunings", 2.5, 0.0,
    Core::Type<f32>::max);
NodeRef createRecognizerNode(const std::string &name,
