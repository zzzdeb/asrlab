...
void EmissionAlphabet::writeXml(Core::XmlWriter &os) const {
  os << Core::XmlOpenComment() << nMixtures_ << " emission labels, "
...
const Core::ParameterString ClassicStateTying::paramFilename(
    "file", "external source defining the state tying");

Core::Ref<const ClassicStateTying>
ClassicStateTying::createClassicStateTyingRef(
  ...
  ClassicStateTying *result = Am::Module::instance().getStateTying(
      static_cast<ClassicAcousticModel::StateTyingType>(
          ClassicAcousticModel::paramType(config)),
...
ClassicStateTying::createMixtureToAllophoneStateTransducer(
    s32 nDisambiguators) const {
  ConstEmissionAlphabetRef emissionAlphabet =
      ConstEmissionAlphabetRef(new EmissionAlphabet(nClasses()));
  ...
  f->setProperties(Fsa::PropertyAcyclic | Fsa::PropertyLinear,
                   Fsa::PropertyNone);
  ...
  if (nDisambiguators > 0) {
    ...
    for (Fsa::LabelId d = 0; d < Fsa::LabelId(nDisambiguators); ++d) {
      initial->newArc(root->id(), One, emissionAlphabet->disambiguator(d),
                      alphabetRef_->disambiguator(d));
      root->newArc(final->id(), One, emissionAlphabet->disambiguator(d),
                   alphabetRef_->disambiguator(d));
    }
  } else
  ...
  for (std::pair<AllophoneStateIterator, AllophoneStateIterator> it =
           alphabetRef_->allophoneStates();
       it.first != it.second; ++it.first)
    root->newArc(root->id(), One, classify(it.first.allophoneState()),
                 it.first.id());
...
MonophoneStateTying::MonophoneStateTying(const Core::Configuration &config,
    ...
    : Core::Component(config), ClassicStateTying(config, stateModel) {
  nPhonemes_ = Mm::MixtureIndex(
      stateModel->phonology().getPhonemeInventory()->nPhonemes());
  nClasses_ = 0;
  ...
  for (Bliss::Phoneme::Id id = 1; id <= nPhonemes_; ++id) {
    ...
    for (int state = 0;
         state < stateModel->hmmTopologySet().get(id)->nPhoneStates();
         ++state, ++nClasses_) {
...
bool LutStateTying::loadLut(const std::string &filename) {
  ...
  */
  ...
  Core::ProgressIndicator *progress =
      new Core::ProgressIndicator("state tying lookup from file");
  progress->start();
  u32 nLine = 0;
  while (cis) {
    ++nLine;
    ...
    if (!line.empty() && (*line.c_str() != '#')) {
      ...
      if (fields.size() != 2) {
        criticalError("failure while reading line %d from file \"%s\"", nLine,
                      filename.c_str());
        return false;
      }
      allophoneStateString = fields[0], Core::strconv(fields[1], mixtureId);
      AllophoneStateIndex allophoneStateId =
          alphabetRef_->index(allophoneStateString);
