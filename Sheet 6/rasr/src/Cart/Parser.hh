...
/**
   ...
   <key> central </key> <value-map> <value id="-1"> # </value> <value id="0"> a
   </value> </value-map>
   <!--
...
public:
  XmlIndexedStringBuilderElement(const char *name, XmlContext *context)
      : Precursor(name, context), pair_(new IndexedString()), index_(0),
        ok_(true) {}
  ~XmlIndexedStringBuilderElement() { delete pair_; }
  ...
  void characters(const char *ch, int len) { pair_->first.append(ch, len); }
};
...
public:
  XmlPropertiesDefinitionBuilderElement(const char *name,
...
protected:
  ...
  void addQuestion(const std::string &key,
...
public:
  XmlIgnoreInformationBuilderElement(const char *name, XmlContext *context)
...
protected:
  void order(const u32 &u) { info_->order = u32(u); }

  void size(const u32 &u) { info_->size = u32(u); }

  void score(const f64 &f) { info_->score = Score(f); }

public:
  XmlTrainingInformationBuilderElement(const char *name, XmlContext *context);
...
public:
  ...
  XmlElement *getXmlElement() { return infoBuilder_; }

  BuildDelegation<Information> *getBuildDelegation() { return infoBuilder_; }
};
...
protected:
  void info(Information *info) { node_->setInfo(info); }

public:
  XmlBinaryTreeRootBuilderElement(const char *name, XmlContext *context,
...
public:
  XmlDecisionTreeBuilderElement(const char *name, XmlContext *context,
                                InformationBuilder *infoBuilder = 0);
...
public:
  XmlDecisionTreeParser(const Core::Configuration &config,
      ...
      : Precursor(config) {
    treeBuilder_ =
        new XmlDecisionTreeBuilderElement("decision-tree", this, infoBuilder);
    setRoot(treeBuilder_);
  }

  ~XmlDecisionTreeParser() { delete treeBuilder_; }
...
public:
  ...
  void set(PropertyMapRef mapRef) { mapRef_ = mapRef; }
...
public:
  XmlFloatBoxBuilderElement(const char *name, XmlContext *context)
      : Precursor(name, context), nRows_(0), nCols_(0), cdata_() {}
...
public:
  XmlExampleListParser(const Core::Configuration &config) : Precursor(config) {
    exampleListBuilder_ =
        new XmlExampleListBuilderElement("example-list", this);
    setRoot(exampleListBuilder_);
  }
  ~XmlExampleListParser() { delete exampleListBuilder_; }
...
public:
  XmlExampleListMerger(const Core::Configuration &config, ExampleList *examples)
      : Precursor(config) {
    exampleListMerger_ = new XmlExampleListMergerElement("example-list", this);
    exampleListMerger_->set(examples);
    setRoot(exampleListMerger_);
  }
  ~XmlExampleListMerger() { delete exampleListMerger_; }
...
protected:
  void minObs(const u32 &minObs) { step_->minObs = minObs; }

  void minGain(const f64 &minGain) { step_->minGain = minGain; }
...
public:
  XmlTrainingPlanParser(const Core::Configuration &config) : Precursor(config) {
    trainingPlanBuilder_ =
        new XmlTrainingPlanBuilderElement("decision-tree-training", this);
    setRoot(trainingPlanBuilder_);
  }

  ~XmlTrainingPlanParser() { delete trainingPlanBuilder_; }
  ...
  bool parseStream(std::istream &i) { return parseStream(i, new TrainingPlan); }
