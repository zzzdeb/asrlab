...
ConstWordLatticeRef transpose(ConstWordLatticeRef l, bool progress,
                              WordBoundary final) {
  ...
  for (size_t i = 0; i < l->nParts(); ++i) {
    ...
    if (!result) {
      ...
      Core::Ref<WordBoundaries> wordBoundaries(
          new WordBoundaries(*(l->wordBoundaries())));
...
ConstWordLatticeRef unite(const Core::Vector<ConstWordLatticeRef> &lattices) {
  if (lattices.size() > 0) {
    ...
    Core::Vector<Core::Ref<const WordBoundaries> > wordBoundaries(
        lattices.size());
