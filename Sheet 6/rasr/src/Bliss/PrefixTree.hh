...
template <> struct PrefixTreeSymbolTraits<std::string> {
...
template <> struct PrefixTreeSymbolTraits<const char *> {
  typedef char Symbol;
  typedef int Index;
  static const int term = 0;
  static const int first = 0;
...
template <class S, class I, class K = typename Core::identity<I>,
...
public:
  ...
  ItemRange lookup(const SymbolString &seq, SymbolIndex &maxLen) const;
  // lookup, set all matches, return length of longest match
  SymbolIndex lookup(const SymbolString &seq, std::vector<ItemRange> &matches,
