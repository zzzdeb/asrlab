...
struct vectorToAlternatingComplexVector : std::unary_function<T, T> {
  ...
  void operator()(const std::vector<T> &x, std::vector<T> &result) {
    ...
    Math::transformToAlternatingComplex(
        x.begin(), x.end(), result.begin(),
        std::bind2nd(Math::makeComplex<T>(), 0));
  }
};

/** converts alternating complex vector to vector of objects of the class
   complex<T> length of output is set according to the number of input complex
   numbers.
*/
template <class T>
struct alternatingComplexVectorToComplexVector
    : std::unary_function<T, std::complex<T> > {
...
struct complexVectorToAlternatingComplexVector
    : std::unary_function<std::complex<T>, T> {
 ...
 */
template <class Function> class ComplexVectorFunctionNode : public SleeveNode {
public:
