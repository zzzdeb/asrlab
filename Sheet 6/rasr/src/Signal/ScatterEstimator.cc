...
const Core::ParameterBool ScatterMatrixEstimator::paramShallNormalize(
    "shall-normalize",
    "If yes, scatter matrices are divided by number of observations", true);

ScatterMatrixEstimator::ScatterMatrixEstimator(
    const Core::Configuration &configuration)
    : Precursor(configuration), featureDimension_(0), needInit_(true) {}
...
bool ScatterMatrixEstimator::accumulate(
    const ScatterMatrixEstimator &estimator) {
...
bool ScatterMatrixEstimator::write(const std::string &filename,
  ...
  bool success = true;
  if (Math::Module::instance().formats().write(filename, scatterMatrix,
                                               paramOutputPrecision(config))) {
    log("The %s scatter matrix is written to '%s'.", scatterType.c_str(),
        filename.c_str());
  } else {
    error("Failed to write the %s scatter matrix to '%s'.", scatterType.c_str(),
          filename.c_str());
    success = false;
  }
  return success;
...
const Core::ParameterFloat
    TotalScatterMatrixEstimator::paramElementThresholdMin(
        "element-threshold-min", "Min threshold for every covariance entry",
        Core::Type<Sum>::min);

TotalScatterMatrixEstimator::TotalScatterMatrixEstimator(
    const Core::Configuration &configuration)
    : Core::Component(configuration), Precursor(configuration) {}
...
bool TotalScatterMatrixEstimator::write() {
  ...
  if (!finalize(covarianceMatrix))
    success = false;
  if (!Precursor::write(paramFilename(config), totalScatterType,
                        covarianceMatrix))
    success = false;
  return success;
}
...
const Core::ParameterString
    ScatterMatricesEstimator::paramBetweenClassScatterFilename(
        "between-class-scatter-matrix-file",
        "output filename for between class scatter matrix");
const Core::ParameterString
    ScatterMatricesEstimator::paramWithinClassScatterFilename(
        "within-class-scatter-matrix-file",
        "output filename for within class scatter matrix");
...
const Core::ParameterString
    ScatterMatricesEstimator::paramOldAccumulatorFilename(
        "old-accumulator-file", "input filename for accumulators");
const Core::ParameterString
    ScatterMatricesEstimator::paramNewAccumulatorFilename(
        "new-accumulator-file", "output filename for accumulators");
const Core::ParameterStringVector
    ScatterMatricesEstimator::paramAccumulatorFilesToCombine(
        "accumulator-files-to-combine", "combine with current estimator", " ",
        1);

ScatterMatricesEstimator::ScatterMatricesEstimator(
    const Core::Configuration &configuration)
    : Core::Component(configuration), Precursor(configuration), nClasses_(0) {}
...
void ScatterMatricesEstimator::initialize(bool deepInitialization) {
  ...
  for (ClassIndex classIndex = 0; classIndex < nClasses_; classIndex++) {
    ...
    std::fill(vectorSums_[classIndex].begin(), vectorSums_[classIndex].end(),
              Sum(0));
...
void ScatterMatricesEstimator::accumulate(ClassIndex classIndex,
                                          const Math::Vector<Data> &x) {
...
bool ScatterMatricesEstimator::accumulate(
    const ScatterMatricesEstimator &estimator) {
...
bool ScatterMatricesEstimator::finalize(
  ...
  Math::Matrix<Sum> totalMeanPart =
      Math::vectorInnerProduct(totalVectorSum, totalVectorSum) /
      (Sum)totalCount;
  ...
  for (ClassIndex classIndex = 0; classIndex < nClasses_; ++classIndex) {
    if (counts_[classIndex] > 0) {
      classMeanPart += Math::vectorInnerProduct(vectorSums_[classIndex],
                                                vectorSums_[classIndex]) /
...
Math::Vector<ScatterMatricesEstimator::Sum>
ScatterMatricesEstimator::getTotalVectorSum() const {
...
bool ScatterMatricesEstimator::writeMatrices() {
  ...
  ScatterMatrix withinClassScatterMatrix;
  ScatterMatrix totalScatterMatrix;
  if (!finalize(betweenClassScatterMatrix, withinClassScatterMatrix,
                totalScatterMatrix))
    return false;
  bool success = true;
  if (!paramBetweenClassScatterFilename(config).empty())
  ...
  if (!paramTotalScatterFilename(config).empty())
    if (!Precursor::write(paramTotalScatterFilename(config), totalScatterType,
                          totalScatterMatrix))
      success = false;
  return success;
}
...
bool ScatterMatricesEstimator::loadAccumulatorFile(
...
void ScatterMatricesEstimator::addAccumulatorFiles(
    const std::vector<std::string> &filenames) {
  if (filenames.empty())
    return;
  loadAccumulatorFile(filenames.front());
  for (std::vector<std::string>::const_iterator itFilename =
           filenames.begin() + 1;
