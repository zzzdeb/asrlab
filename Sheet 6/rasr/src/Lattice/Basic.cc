  ...
  if (lattice) {
    ...
    if (lattice->wordBoundaries()) {
      return resolveMorphism(ConstWordLatticeRef(normalized),
                             lattice->wordBoundaries(), Fsa::mapNormalized(n));
    } else {
...
ConstWordLatticeRef trim(ConstWordLatticeRef lattice, bool progress) {
  Trimmer t(progress);
...
ConstWordLatticeRef partial(ConstWordLatticeRef lattice, Fsa::StateId initial) {
  Cutter c(initial, lattice->part(0)->semiring()->one());
  return apply(lattice, c);
}

ConstWordLatticeRef partial(ConstWordLatticeRef lattice, Fsa::StateId initial,
                            Fsa::Weight additionalFinalWeight) {
...
ConstWordLatticeRef changeSemiring(ConstWordLatticeRef lattice,
                                   Fsa::ConstSemiringRef semiring) {
...
public:
  ...
  u32 nTimeframes() const { return maxTimeframeIndex_ - minTimeframeIndex_; }
