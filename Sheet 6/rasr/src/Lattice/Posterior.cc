...
ConstWordLatticeRef posterior(ConstWordLatticeRef lattice,
                              Fsa::Weight &totalInv, s32 tol) {
  WordLattice *result = new WordLattice();
  result->setWordBoundaries(lattice->wordBoundaries());
  result->setFsa(Fsa::posterior64(lattice->mainPart(), totalInv, tol),
                 "posterior");
  return ConstWordLatticeRef(result);
}

ConstWordLatticeRef posterior(ConstWordLatticeRef lattice, f64 &totalInv,
                              s32 tol) {
  WordLattice *result = new WordLattice();
  result->setWordBoundaries(lattice->wordBoundaries());
  result->setFsa(Fsa::posterior64(lattice->mainPart(), totalInv, tol),
...
CheckPosteriorDfsState::CheckPosteriorDfsState(ConstWordLatticeRef lattice,
                                               s32 tolerance)
    : DfsState(lattice), accumulators_(lattice->maximumTime()),
...
void CheckPosteriorDfsState::finish() {
  ...
  for (Speech::TimeframeIndex t = 0; t < accumulators_.size() - 1; ++t) {
    ...
    std::cout << "accumulated-word-posterior[" << t << "]=" << sum << std::endl;
...
ConstWordLatticeRef prune(ConstWordLatticeRef lattice,
                          const Fsa::Weight &threshold, bool relative,
                          bool backward) {
  if (f32(threshold) < Core::Type<f32>::max) {
    ...
    if (backward)
      result->setFsa(
          Fsa::prunePosterior(lattice->mainPart(), threshold, relative),
          lattice->mainName());
    else
      result->setFsa(Fsa::pruneSync(lattice->mainPart(), threshold),
                     lattice->mainName());
    return ConstWordLatticeRef(result);
  }
  return lattice;
