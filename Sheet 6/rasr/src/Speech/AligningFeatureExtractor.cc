...
const Core::ParameterString AligningFeatureExtractor::paramAlignmentPortName(
    "alignment-port-name", "name of the main data source port", "alignments");

const Core::ParameterBool
    AligningFeatureExtractor::paramEnforceWeightedProcessing(
        "enforce-weighted-processing",
        "enforce weighted processing even for weights=1 etc.", false);

const Core::ParameterString AligningFeatureExtractor::paramAlignment2PortName(
    "alignment-2-port-name", "name of the second data source port",
    "alignments-2");
...
AligningFeatureExtractor::AligningFeatureExtractor(
    ...
    : Core::Component(c), Precursor(c),
      ...
      processWeighted_(paramEnforceWeightedProcessing(config)), alignment_(0),
      alignment2PortId_(Flow::IllegalPortId), alignment2_(0) {
...
void AligningFeatureExtractor::setFeatureDescription(
  ...
  if (!processWeighted_) {
    if (currentAlignmentItem_ != alignment_->end()) {
      ...
      alignedFeatureProcessor_.processAlignedFeature(
          f, currentAlignmentItem_->emission);
      ++currentAlignmentItem_;
    } else {
      error("Alignment (size=%zd) shorter than the feature stream.",
            alignment_->size());
    }
  } else {
    if (!alignment2_) {
...
void AligningFeatureExtractor::unaryProcessFeature(Core::Ref<const Feature> f) {
  ...
  while (currentAlignmentItem_ != alignment_->end() &&
         currentAlignmentItem_->time == currentFeatureId_) {
    verify(currentAlignmentItem_ == alignment_->begin()
               ? true
               : (currentAlignmentItem_ - 1)->time <=
                     currentAlignmentItem_->time);
    alignedFeatureProcessor_.processAlignedFeature(
        f, currentAlignmentItem_->emission, currentAlignmentItem_->weight);
    ++currentAlignmentItem_;
  }
}

void AligningFeatureExtractor::binaryProcessFeature(
    Core::Ref<const Feature> f) {
  verify(alignment2_);
...
bool AligningFeatureExtractor::initializeAlignment() {
  ...
  processWeighted_ = (processWeighted_ || alignment_->hasWeights() ||
                      (currentAlignmentItem_->time != 0));
