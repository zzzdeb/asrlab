 ...
 * postcondition: for each state holds that a non-matching arc is never followed
by a matching arc
...
template <class WeakOrder> class ArcSortLattice : public ModifyLattice {
private:
...
template <class ArcFilter> class ArcRemovalLattice : public SlaveLattice {
private:
...
public:
  ArcRemovalLattice(ConstLatticeRef l)
      : SlaveLattice(cache(ConstLatticeRef(new ArcSortLattice<WeakOrder>(l)))),
        semiring_(l->semiring()), weakOrder_(l) {
    ...
    while (!topQ.empty()) {
      ...
      for (; (a != a_end) && (a->input() == Fsa::Epsilon) &&
        ...
        if (!closureScores_[a->target()]) {
          ...
          closureScores_[a->target()] =
              semiring_->collect(closureScores_[a->target()],
                                 semiring_->extend(score, a->weight()));
      for (; a != a_end; ++a) {
        State::iterator pos = sp->lower_bound(*a, weakOrder_);
        if ((pos == sp->end()) || (a->target() != pos->target()) ||
            (a->input() != pos->input()) || (a->output() != pos->output())) {
          sp->insert(pos, *a)->weight_ = semiring_->extend(score, a->weight());
        } else
  ...
  virtual std::string describe() const {
    return Core::form("%s(%s)", ArcFilter::describe().c_str(),
                      fsa_->describe().c_str());
  }
};
// -------------------------------------------------------------------------
...
const Core::ParameterBool ArcRemovalNode::paramToLogSemiring("log-semiring",
                                                             "use log semiring",
                                                             false);
const Core::ParameterFloat ArcRemovalNode::paramAlpha(
    "alpha", "scale dimensions for posterior calculation", 0.0);
// -------------------------------------------------------------------------
...
NodeRef createEpsilonRemovalNode(const std::string &name,
...
public:
  ...
  struct WeakOrder {
    ...
    bool operator()(const Arc &a1, const Arc &a2) const {
      Time t1 = boundaries.get(a1.target()).time(),
           t2 = boundaries.get(a1.target()).time();
      if (t1 < t2)
        return true;
      if (t1 > t2)
        return false;
...
public:
  NullArcsRemovalNode(const std::string &name,
                      const Core::Configuration &config)
      : ArcRemovalNode(name, config) {}
  ~NullArcsRemovalNode() {}
};
NodeRef createNullArcsRemovalNode(const std::string &name,
                                  const Core::Configuration &config) {
