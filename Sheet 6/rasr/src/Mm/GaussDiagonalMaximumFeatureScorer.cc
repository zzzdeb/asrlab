...
GaussDiagonalMaximumFeatureScorer::Context::Context(
    const FeatureVector &featureVector,
    const GaussDiagonalMaximumFeatureScorer *featureScorer, size_t cacheSize)
...
const Core::ParameterFloat
    GaussDiagonalMaximumFeatureScorer::paramMixtureWeightScale(
        "mixture-weight-scale", "scaling of the logarithmized mixture weights",
        1.0, Core::Type<Score>::epsilon);

const Core::ParameterFloat
    GaussDiagonalMaximumFeatureScorer::paramGaussianScale(
        "gaussian-scale", "scaling of the logarithmized gaussian probability",
        1.0, Core::Type<Score>::epsilon);
...
GaussDiagonalMaximumFeatureScorer::GaussDiagonalMaximumFeatureScorer(
    ...
    : Core::Component(c), Precursor(c),
      ...
      gaussianScale_(std::sqrt(paramGaussianScale(c))), dimension_(0) {
  init(*mixtureSet);
}
...
GaussDiagonalMaximumFeatureScorer::calculateScoreAndDensity(
...
Score GaussDiagonalMaximumFeatureScorer::distance(
    const std::vector<FeatureType> &feature, const std::vector<MeanType> &mean,
...
GaussDiagonalSumFeatureScorer::GaussDiagonalSumFeatureScorer(
    const Core::Configuration &c, Core::Ref<const MixtureSet> mixtureSet)
    : Core::Component(c), Precursor(c, mixtureSet),
...
AssigningFeatureScorer::ScoreAndBestDensity
GaussDiagonalSumFeatureScorer::calculateScoreAndDensity(
    const CachedAssigningContextScorer *cs, MixtureIndex mixtureIndex) const {
