...
Core::ParameterFloat CutterNode::paramStartTime("start-time",
                                                "start-time of output", 0.0);
Core::ParameterFloat CutterNode::paramEndTime("end-time", "end-time of output",
                                              Core::Type<f32>::max);
Core::ParameterString
    CutterNode::paramId("id", "changing the id resets the cached features");

bool CutterNode::setParameter(const std::string &name,
...
void CutterNode::seekToStartTime() {
  ...
  if (currentItem->startTime() > startTime_) {
    while (position_ > 0 &&
           DataPtr<Timestamp>(featureSequence_[position_])->startTime() >
               startTime_)
      position_--;
  } else if (currentItem->startTime() < startTime_) {
    while (position_ < featureSequence_.size() &&
           DataPtr<Timestamp>(featureSequence_[position_])->startTime() <
               startTime_)
      position_++;
  }
...
bool CutterNode::work(PortId output) {
  ...
  if (readPosition < featureSequence_.size() &&
      DataPtr<Timestamp>(featureSequence_[readPosition])->startTime() <=
          endTime_)
