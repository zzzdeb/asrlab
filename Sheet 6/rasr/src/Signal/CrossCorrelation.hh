...
/** normalizeCrossCorrelationEstimate: normalizes cross-correlation estimate by
 * number of addands Leads to the so called unbiased estimate of the
 * cross-correaltion function
 */
template <class Argument>
class normalizeCrossCorrelationEstimate
    : std::binary_function<Argument, s32, Argument> {
  s32 sizeX_;
  s32 sizeY_;

public:
  normalizeCrossCorrelationEstimate(const std::vector<Argument> &x,
...
/** normalizeCrossCorrelationUpperBound: normalizes upper bound of
 * cross-correlation to 1 |Rxy(m)| / sqrt(Rxx(0)Ryy(0)) <= 1 With other words,
 * the two sequences x, y are normalized such that the zeroth lag of their
...
public:
  normalizeCrossCorrelationUpperBound(const std::vector<Argument> &x,
                                      const std::vector<Argument> &y) {
...
/** normalizeCrossSquareDifferenceUpperBound: normalizes upper bound of
 * cross-square-difference to 1. 0 <= Qxy(m) / (sqrt(Rxx(0)) + sqrt(Ryy(0)))^2
 * <= 1
 */
template <class Argument>
...
/** normalizeCrossAbsoluteDifferenceUpperBound: normalizes upper bound of
 * cross-absolute-difference to 1 0 <= Dxy(m) / [sum( abs(x(t)) ) + sum(
 * abs(y(t)) )] <= 1
 */
template <class Argument>
...
public:
  normalizeCrossAbsoluteDifferenceUpperBound(const std::vector<Argument> &x,
                                             const std::vector<Argument> &y) {
    upperBound_ = 0;
    for (typename std::vector<Argument>::const_iterator i = x.begin();
         i != x.end(); ++i)
      upperBound_ += Core::abs(*i);
    for (typename std::vector<Argument>::const_iterator i = y.begin();
         i != y.end(); ++i)
      upperBound_ += Core::abs(*i);
  }
  Argument operator()(Argument element, s32) { return element / upperBound_; }
...
/** CrossCorrelation
 * Rxy(m) = sum_{t = max(0, -m)}^{min(sizeX_ - m, sizeY_)} similarity(x_{t + m},
 * y_{t})
 *
 * where:
 *   "similarty" can be:
 ...
 *
 *   m is element of [begin..end); begin and end are give in continuous units
 * (e.g.: sec, Hz)
 *
 * normalitions:
...
private:
   ...
   * @param product is a binary functor implementing an arbitrary similarity
   * function
   *
   * remark:
   *   This implementation compares only the overlapping part of the shifted x
   * and y. It does not extend any of the signals by zeros. This does not make
   * any difference if applying 'multiplication' as similarity function but it
   * do make a significant difference for absolute-difference kind of similarity
   * functions.
   */
  template <class OperationProduct>
  void crossCorrelation(const std::vector<Data> &x, const std::vector<Data> &y,
                        std::vector<Data> &Rxy,
                        OperationProduct product) const {
   ...
   * @param normalization is an arbitrary binary functor with parameter
   * (element, m)
   */
  template <class OperationNormalization>
  void normalize(const std::vector<Data> &x, const std::vector<Data> &y,
                 std::vector<Data> &Rxy,
                 OperationNormalization normalization) const {
  ...
  void crossCorrelationWithMultiplication(const std::vector<Data> &x,
                                          const std::vector<Data> &y,
                                          std::vector<Data> &Rxy);
  void crossCorrelationWithAbsoluteDifference(const std::vector<Data> &x,
                                              const std::vector<Data> &y,
                                              std::vector<Data> &Rxy) const;

  void normalizeUpperBound(const std::vector<Data> &x,
                           const std::vector<Data> &y,
                           std::vector<Data> &Rxy) const;
...
public:
  ...
  void apply(const std::vector<Data> &x, const std::vector<Data> &y,
             std::vector<Data> &Rxy);
  void apply(const std::vector<Data> &x, const std::vector<Data> &y,
             std::vector<Data> &Rxy, Core::Component &errorChannels);

  void setBegin(s32 begin) { begin_ = begin; }
  void setEnd(s32 end) { end_ = end; }

  void setSimilarityFunctionType(
      const SimilarityFunctionType similarityFunctionType) {
...
public:
   ...
   *  @param nComponents is number of autocorrelation components (order + 1 for
   * AR estimation).
   *  @param amplitude is the half(odd number of elements) of the amplitude
   * spectrum.
   */
  bool init(size_t nComponents, const std::vector<Data> &amplitude);
  /** Retrives the autocorrelation for the frequency band [@param
   * bandBegin..@param bandEnd) */
...
private:
   ...
   *  Note: if not 0, this parameter overwrites the interval given by
   * continuousBegin_ and continuousend_.
   */
...
private:
  ...
  void init();
  bool getData(Flow::DataPtr<Flow::Vector<f32> > &x,
