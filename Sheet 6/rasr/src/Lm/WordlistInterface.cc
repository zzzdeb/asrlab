...
const Core::ParameterInt WordlistInterfaceLm::paramHistoryLimit(
    "history-limit", "maximum length of history considered (m-grammity - 1)",
    historyLengthLimit, 0, historyLengthLimit);
const Core::ParameterBool WordlistInterfaceLm::paramUseBackingOff(
    "use-backing-off", "use backing off to reduce length of histories", false);
...
WordlistInterfaceLm::WordlistInterfaceLm(const Core::Configuration &c,
                                         Bliss::LexiconRef l)
    : Core::Component(c), IndexMappedLm(c, l), wl(0), backingOffCounterBase_(0),
      backingOffCounter_(0), backingOffStatistics_(c, "backing-off-statistics",
  ...
  if (useBackingOff_) {
    ...
    for (size_t i = 0; i < maxHistoryLength() + 1; ++i) {
      backingOffCounter_[i] = backingOffCounterBase_ + i * maxHistoryLength();
...
WordlistInterfaceLm::~WordlistInterfaceLm() {
  if (useBackingOff_) {
    backingOffStatistics_ << XmlOpen("backing-off-statistics") +
                                 XmlAttribute("history",
                                              maxHistoryLength() + 1);
    for (size_t i = 0; i < maxHistoryLength() + 1; ++i) {
      backingOffStatistics_
          << XmlOpen("history") + XmlAttribute("length", i + 1);
...
void WordlistInterfaceLm::setSignificantHistoryLength(u32 l) {
  ...
  if (l > historyLengthLimit)
    warning("support for %d-grams is not implemented (increase "
            "WordlistInterfaceLm::historyLengthLimit)",
            l + 1);
...
bool WordlistInterfaceLm::isEquivalent(HistoryHandle hda,
                                       HistoryHandle hdb) const {
...
History WordlistInterfaceLm::startHistory() const {
  ...
  HistoryDescriptor *result =
      new HistoryDescriptor(std::min(u32(1), maxHistoryLength_));
  if (result->length > 0)
...
u32 WordlistInterfaceLm::backingOffHistoryLength(
    const HistoryDescriptor *h) const {
  return h->length;
}
...
History WordlistInterfaceLm::extendedHistory(const History &h, Token w) const {
  ...
  HistoryDescriptor *result =
      new HistoryDescriptor(std::min(cd->length + 1, maxHistoryLength_));
...
std::string WordlistInterfaceLm::formatHistory(const History &h) const {
  ...
  for (unsigned int i = 0; i < hd->length; i++) {
    result += Core::form("%s (%d), ", internalClassName(hd->history[i]),
                         hd->history[i]);
  }
  return result;
}
