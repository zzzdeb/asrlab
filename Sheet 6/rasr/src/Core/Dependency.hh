...
public:
   ...
   *  Before appending this set, each name in @param dependencySet is extended
   * by @param name.
   *
  ...
  /** @return is true if all dependencies in @param required are greater or
     equal. you should use this method with care.
   */
  ...
  friend std::ostream &operator<<(std::ostream &o, const DependencySet &s) {
    for (DependencySet::Dependencies::const_iterator d =
             s.dependencies_.begin();
         d != s.dependencies_.end(); ++d)
