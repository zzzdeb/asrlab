...
template <class T> int getrf(Matrix<T> &A, Vector<int> &pivotIndices) {
 ...
 *          The factors L and U from the factorization A = P*L*U written in the
 * same matrix with the unit diagonal elements of L not stored.
 *
...
int getrs(Matrix<T> &LU, Matrix<T> &B, Vector<int> &pivotIndices) {
  ...
  getrs(&trans, &n, &nrhs, LU.buffer(), &lda, pivotIndices.buffer(), B.buffer(),
        &ldb, &info);
