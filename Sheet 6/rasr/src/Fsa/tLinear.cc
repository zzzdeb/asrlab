...
namespace Ftl {
template <class _Automaton> bool isLinear(typename _Automaton::ConstRef la) {
...
void getLinearInput(typename _Automaton::ConstRef la,
...
template <class _Automaton>
void getLinearOutput(typename _Automaton::ConstRef la,
                     std::vector<Fsa::LabelId> &result) {
  require(hasProperties<_Automaton>(la, Fsa::PropertyLinear));
  //	require(!isEmpty<_Automaton>(la));
  typename _Automaton::ConstStateRef s = la->getState(la->initialStateId());
