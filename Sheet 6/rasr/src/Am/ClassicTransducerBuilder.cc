...
private:
  struct PhoneBoundaryStateSetDescriptor {
    ...
    PhoneBoundaryStateSetDescriptor(const PhoneBoundaryStateDescriptor &pbsd)
...
private:
  u32 &nPhoneBoundaryStates(const PhoneBoundaryStateDescriptor &pbsd) {
    return nPhoneBoundaryStates_
        .insert(std::make_pair(PhoneBoundaryStateSetDescriptor(pbsd), 0))
        .first->second;
  }
...
public:
  Statistics() : nAllophones_(0) {}

  void accumulate(const PhoneBoundaryStateDescriptor &pbsd) {
    nPhoneBoundaryStates(pbsd) += 1;
  }
  void setNumberOfAllophones(size_t n) { nAllophones_ = n; }

  void reset() { nPhoneBoundaryStates_.clear(); }
  void write(Core::XmlWriter &) const;
};

Core::XmlOpen
ClassicTransducerBuilder::Statistics::PhoneBoundaryStateSetDescriptor::
    xmlAttributes(const Core::XmlOpen &xmlOpen) const {
  ...
  return Core::XmlOpen(xmlOpen) +
         Core::XmlAttribute("boundary", phoneBoundaryValue) +
         Core::XmlAttribute("coarticulated", isCoarticulatedValue);
}

void ClassicTransducerBuilder::Statistics::write(Core::XmlWriter &os) const {
  os << Core::XmlOpen("statistics") +
            Core::XmlAttribute("type", "state-model-transducer");
...
void ClassicTransducerBuilder::Statistics::writeNumberOfPhoneBoundaryStates(
  ...
  for (NumberOfPhoneBoundaryStates::const_iterator i =
    ...
    os << i->first.xmlAttributes(Core::XmlOpen("number-of-states")) << i->second
       << Core::XmlClose("number-of-states");
  }
}

// ---------------------------------------------------------------------------
ClassicTransducerBuilder::ClassicTransducerBuilder(
    Core::Ref<const ClassicAcousticModel> model)
...
ClassicTransducerBuilder::~ClassicTransducerBuilder() { delete statistics_; }
...
void ClassicTransducerBuilder::setupWordEnd(
    Fsa::State *s, const PhoneBoundaryStateDescriptor &pbsd) {
  if (acceptCoarticulatedSinglePronunciation_) {
    s->addTags(Fsa::StateTagFinal);
    s->weight_ = product_->semiring()->one();
  } else {
...
Fsa::State *
ClassicTransducerBuilder::phoneStartState(const Allophone *allophone) {
  ...
  /**
   *  Note: non-coarticulated phonemes mostly occur as initial/final phoneme
   * (e.i. word start/end) but there exit seldom non-coarticulated within word
   * phonemes  as well (e.g. more than one subsequent noise phonemes).
   */
  if (pbsd.context.history.empty())
...
Fsa::State *
ClassicTransducerBuilder::phoneEndState(const Allophone *allophone) {
  PhoneBoundaryStateDescriptor pbsd;
  pbsd.context = allophone->context();
...
void ClassicTransducerBuilder::buildAllophone(const Allophone *allophone) {
  ...
  buildAllophone(allophone, phoneStartState(allophone),
                 phoneEndState(allophone));
}

Fsa::State *ClassicTransducerBuilder::buildAllophone(const Allophone *allophone,
                                                     Fsa::State *start) {
...
void ClassicTransducerBuilder::buildAllophone(const Allophone *allophone,
  ...
  case inputAcceptsAllophones: {
    const Bliss::Phoneme *phoneme =
        model_->phonemeInventory()->phoneme(allophone->phoneme());
    verify(phoneme);
...
Fsa::LabelId
ClassicTransducerBuilder::allophoneStateLabel(const AllophoneState &as) {
  switch (inputType_) {
  case inputAcceptsAllophoneStates:
...
size_t ClassicTransducerBuilder::hashSequence(const AllophoneSuffix &as) {
  ...
  for (s16 state = as.allophoneState.state();
       state < as.hmmTopology->nPhoneStates(); ++state) {
...
int ClassicTransducerBuilder::compareSequences(const AllophoneSuffix &ll,
  ...
  for (;;) {
    ...
    if (rstate == rr.hmmTopology->nPhoneStates())
      return -1;
    Fsa::LabelId llLabel = allophoneStateLabel(allophoneStates_->allophoneState(
        ll.allophoneState.allophone(), lstate));
    Fsa::LabelId rrLabel = allophoneStateLabel(allophoneStates_->allophoneState(
        rr.allophoneState.allophone(), rstate));
    if (llLabel != rrLabel)
...
Fsa::State *ClassicTransducerBuilder::buildSilenceAndNoiseLoops(
    const Bliss::PhonemeInventory &phonemeInventory, const Phonology &phonology,
    Fsa::State *s) {
  ...
  for (; silencePronunciationIt != silencesAndNoises_->end();
       ++silencePronunciationIt) {
...
void ClassicTransducerBuilder::buildPronunciation(
    const Bliss::Coarticulated<Bliss::Pronunciation>
        &coarticulatedPronunciation) {
  //    require(nDisambiguators_ == 0); // nDisambiguators_ effects the
  //    application of the transition model.
  setDisambiguators(0);
  ...
  if (p.length() == 1) {
    Allophone allophone(phonology(p, 0),
    ...
    {
      Allophone initialAllophone(phonology(p, 0), Allophone::isInitialPhone);
      if (leftContext != Bliss::Phoneme::term &&
          model_->isAcrossWordModelEnabled() &&
          phonemeInventory.phoneme(initialAllophone.phoneme())
              ->isContextDependent()) {
...

    {
      ...
      if (rightContext != Bliss::Phoneme::term &&
          model_->isAcrossWordModelEnabled() &&
          phonemeInventory.phoneme(finalAllophone.phoneme())
              ->isContextDependent()) {
...
ClassicTransducerBuilder::applyTransitionModel(Fsa::ConstAutomatonRef ff) {
  require(model_->transitionModel_);
  require((ff->inputAlphabet() == allophoneStates_) ||
          (ff->inputAlphabet() == emissions_));

  const Allophone *silenceAllophone = allophones_->allophone(
      Allophone(Phonology::Allophone(model_->silence_),
                Allophone::isInitialPhone | Allophone::isFinalPhone));
  AllophoneState silenceState =
      allophoneStates_->allophoneState(silenceAllophone, 0);
  ...
  return model_->transitionModel_->apply(ff, silenceLabel,
                                         (nDisambiguators_ == 0));
}
      ...
      for (Fsa::State::iterator a = sp->begin(); a != sp->end(); ++a) {
        if (a->input() != Fsa::Epsilon) {
          if (!allophoneStates_->isDisambiguator(a->input()))
            a->input_ = model_->stateTyingRef_->classifyIndex(a->input());
          else
            a->input_ = emissions_->disambiguator(
                a->input() - allophoneStates_->nClasses() - 1);
        }
      }
    }
    pi.notify(s);
...
Fsa::ConstAutomatonRef
ClassicTransducerBuilder::createAllophoneLoopTransducer() {
  createEmptyTransducer();
...
/**
 *  - union of closure of reduced HMM state sequences for each (used, unique)
 *allophone
 *  - add disambiguation symbols
 *  - still not determinized on state level! (would result in less efficient
 *composition)
 *
 ...
 **/
Fsa::ConstAutomatonRef
ClassicTransducerBuilder::createEmissionLoopTransducer(bool transitionModel) {
    ...
    for (int s = 0; s < nStates; ++s) {
      ...
      if (transitionModel) {
        ...
        if (j == minimizedStates.end()) {
          ...
          // loop transition
          to->newArc(to->id(), Fsa::Weight((*tdp)[StateTransitionModel::loop]),
                     input, Fsa::Epsilon);
          if (isLastArc)
            to->newArc(initial->id(), a->semiring()->one(), Fsa::Epsilon,
                       Fsa::Epsilon);
        } else {
          to = j->second;
          s = nStates;
        ...
        if (isFirstArc) {
          fw = (*(*model_->transitionModel_)[TransitionModel::entryM1])
              [StateTransitionModel::forward];
        ...
        // skip transition
        if (((*tdp)[StateTransitionModel::skip] <
             Core::Type<StateTransitionModel::Score>::max) &&
            (fromSkip)) {
          fromSkip->newArc(
              to->id(), Fsa::Weight((*tdp)[StateTransitionModel::skip]), input,
              (fromSkip == initial) ? aa->index(*ai) : Fsa::Epsilon);
        }
      } else {
        if (j == minimizedStates.end()) {
          if (isLastArc)
      ...
      // forward
      from->newArc(to->id(), Fsa::Weight(fw), input,
                   isFirstArc ? aa->index(*ai) : Fsa::Epsilon);
      fromSkip = from;
      from = to;
    }
  }
  for (u32 d = 0; d < nDisambiguators_; ++d)
    if (transitionModel)
      initial->newArc(initial->id(), a->semiring()->one(), ea->disambiguator(d),
                      aa->disambiguator(d));
    else
      initial->newArc(initial->id(), a->semiring()->one(),
                      asa->disambiguator(d), aa->disambiguator(d));
...
struct PhoneContext : public Phones {
  ...
  PhoneContext(size_t n, bool boundary = false,
               Fsa::LabelId disambiguator = Fsa::Epsilon)
      : Phones(n, Bliss::Phoneme::term), boundary_(boundary),
...
Fsa::ConstAutomatonRef
ClassicTransducerBuilder::createMinimizedContextDependencyTransducer(
    Fsa::LabelId initialPhoneAndSilenceOffset) {
  bool coartSilence =
      false; // not used yet, but enables the support for cooarticulated silence
  ...
  for (AllophoneAlphabet::AllophoneList::const_iterator ai =
    ...
    const Bliss::Phoneme *phoneme =
        model_->phonemeInventory()->phoneme(a->phoneme());
  ...
  initialCoartPhones.erase(
      std::unique(initialCoartPhones.begin(), initialCoartPhones.end()),
      initialCoartPhones.end());
  std::sort(initialNonCoartPhones.begin(), initialNonCoartPhones.end());
  initialNonCoartPhones.erase(
      std::unique(initialNonCoartPhones.begin(), initialNonCoartPhones.end()),
      initialNonCoartPhones.end());

  const Allophone *silenceAllophone = allophones_->allophone(
      Allophone(Phonology::Allophone(model_->silence_), 0));
  ...
  Fsa::State *initial =
      new Fsa::State(stateMap.insert(pc),
  ...
  if (model_->isAcrossWordModelEnabled()) {
    ...
    for (Core::hash_map<Bliss::Phoneme::Id, const Allophone *>::const_iterator
      ...
      initial->newArc(initial->id(), semiring->one(),
                      allophones_->index(i->second),
                      initialPhoneAndSilenceOffset + Fsa::LabelId(i->first));
  }
  ...
  // - if c == #, we draw an arc to the final state f and an additional silence
  // arc to #,si
    ...
    if (model_->isAcrossWordModelEnabled()) {
      ...
      if ((*ai)->central() == silenceAllophone->central()) {
        if ((((*ai)->history().size() > 0) && ((*ai)->future().size() == 0)) ||
          ...
          continue;
        if ((((*ai)->boundary & Allophone::isInitialPhone) &&
             !((*ai)->boundary & Allophone::isFinalPhone)) ||
            (!((*ai)->boundary & Allophone::isInitialPhone) &&
             ((*ai)->boundary & Allophone::isFinalPhone)))
          continue;
      }
      if (((*ai)->boundary & Allophone::isInitialPhone) &&
          (nonCoartPhones.find((*ai)->central()) != nonCoartPhones.end()))
        continue;

      // create "from" state
      Fsa::StateId sFrom = stateMap.insert(
          PhoneContext(phones.begin(), phones.end() - 1,
                       (*ai)->boundary & Allophone::isInitialPhone));
      if (!_c->state(sFrom)) {
        ...
        if ((!(*ai)->boundary & Allophone::isInitialPhone) &&
            (*ai)->central() == silenceAllophone->central() &&
            ((*ai)->history().size() == 0)) {
          for (std::vector<Bliss::Phoneme::Id>::const_iterator p =
                   initialCoartPhones.begin();
               p != initialCoartPhones.end(); ++p) {
            PhoneContext pc(maxHistory + maxFuture, false);
            pc[maxHistory] = Fsa::LabelId(*p);
            _c->state(sFrom)->newArc(stateMap.insert(pc), semiring->one(),
                                     allophones_->index(silenceAllophone),
                                     Fsa::LabelId(*p));
          }
        }
      }
      ...
      if ((*ai)->boundary & Allophone::isFinalPhone) {
        ...
        if (((*ai)->future().size() == 0) ||
            ((*ai)->future()[0] == Bliss::Phoneme::term))
          sp->newArc(initial->id(), semiring->one(), allophones_->index(*ai),
                     Fsa::Epsilon);
        else
          sp->newArc(stateMap.insert(
                         PhoneContext(phones.begin() + 1, phones.end(), true)),
                     semiring->one(), allophones_->index(*ai),
        ...
        if (((*ai)->future().size() == 0) ||
          ...
          if ((*ai)->central() != silenceAllophone->central()) {
            ...
            sp->newArc(stateMap.insert(pc), semiring->one(),
                       allophones_->index(*ai), silenceAllophone->central());
          } else
            sp->newArc(sFrom, semiring->one(), allophones_->index(*ai),
                       silenceAllophone->central());
        } else
          sp->newArc(
              stateMap.insert(PhoneContext(phones.begin() + 1, phones.end())),
              semiring->one(), allophones_->index(*ai), phones[maxHistory + 1]);
      }
      ...
      if ((*ai)->boundary & Allophone::isFinalPhone) {
        ...
        for (size_t d = 0; d < nDisambiguators_; ++d) {
          require((*ai)->future()[0] ==
                  Bliss::Phoneme::term); // allow only for boundary phone
          sTo = stateMap.insert(PhoneContext(
              pc, true,
              d + 1)); // use generic phone context of all term symbols
          if (!_c->state(sTo)) {
            Fsa::State *state =
                new Fsa::State(sTo, Fsa::StateTagFinal, semiring->one());
            _c->setState(state);
            nonCoartBoundaries.push_back(
                NoCoartBoundary(state, allophones_->disambiguator(d),
                                phonemes_->disambiguator(d)));
            /*std::cout << "disambiguator d=" << d << "  -> " <<
               allophones_->disambiguator(d) << " "
                      <<
               allophones_->isDisambiguator(allophones_->disambiguator(d)) << "
               symbol="
                      << allophones_->symbol(allophones_->disambiguator(d)) <<
               std::endl; */
          }
          ...
          /*
             ...
             allophones_->symbol(allophones_->index(*ai))
                            ...
                            << phonemes_->symbol(phonemes_->disambiguator(d)) <<
             std::endl;
          */
        }
      } else
    ...
    if (_c->state(s)) {
      if (model_->isAcrossWordModelEnabled()) {
        ...
        /*std::cout << "(D) from=" << initial->id() << " to=" <<  s
                              ...
                              << phonemes_->symbol(Fsa::LabelId(*p)) <<
           std::endl;
        */
      }
      for (std::vector<NoCoartBoundary>::const_iterator wb =
        ...
        // std::cout << "nonCoartBoundary: input=" << wb->input_ << " -> " <<
        // allophones_->symbol(wb->input_) << "  phoneDisambiguator=" <<
        // wb->phoneDisambiguator_ << std::endl;
        wb->state_->newArc(s, semiring->one(), wb->input_, Fsa::LabelId(*p));
        /*
                std::cout << "(E) from=" << wb->state_->id() << " to=" <<  s <<
           " input="
                          << wb->input_ << " " <<
           allophones_->symbol(wb->input_)
                          << " output=" << Fsa::LabelId(*p) << " "
