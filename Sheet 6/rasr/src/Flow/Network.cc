...
Network::Port::Port(const std::string &name)
    : /*****************************************************************************/
      name_(name), link_(0), node_(0), nodePort_(IllegalPortId),
...
const Core::ParameterString
    Network::paramFilename("file", "name of Flow network file to load");
/*****************************************************************************/
...
Network::Network(const Core::Configuration &c, bool shouldLoad)
    : /*****************************************************************************/
      Component(c), Precursor(c), dumpChannel_(c, "flow-dump-channel"),
      typeName_("network"), started_(false), lastNonData_(Data::eos()) {
  if (shouldLoad)
...
/*****************************************************************************/
{
  ...
  for (std::list<Link *>::const_iterator it = links_.begin();
       it != links_.end(); it++)
    delete *it;
...
/*****************************************************************************/
bool Network::addLink(const std::string &fromNodeName,
                      const std::string &fromPortName,
                      const std::string &toNodeName,
                      const std::string &toPortName, u32 buffer)
/*****************************************************************************/
{
  Link *l = new Link();
  bool result = true;
  ...
  if (toNodeName == typeName_) {
    if (fromNodeName == typeName_) {
      if (!connectOutputFromInside(inputRepeaterPrefix + fromPortName, "data",
                                   toPortName, l))
        result = false;
    } else {
...
/*********************************************************************************/
bool Network::connectInputFromInside(const std::string &portName, Link *l,
                                     u32 buffer)
/*********************************************************************************/
{
...
bool Network::connectOutputFromInside(const std::string &fromNodeName,
                                      const std::string &fromPortName,
                                      const std::string &toPortName, Link *l)
/*****************************************************************************/
{
  ...
  if (fromPort == IllegalPortId) {
    error("could not connect output '%s' at node '%s'.", fromPortName.c_str(),
          fromNodeName.c_str());
    return false;
  }
...
/************************************************************************************************/
bool Network::connectNodeInput(const std::string &nodeName,
                               const std::string &portName, Link *l)
/************************************************************************************************/
{
  ...
  if ((toPort == IllegalPortId) ||
    ...
    error("could not connect input '%s' at node '%s'.", portName.c_str(),
          nodeName.c_str());
    return false;
  }
  return true;
...
/**************************************************************************************************/
bool Network::connectNodeOutput(const std::string &nodeName,
                                const std::string &portName, Link *l)
/**************************************************************************************************/
{
  ...
  if ((fromPort == IllegalPortId) ||
    ...
    error("could not connect output '%s' at node '%s'.", portName.c_str(),
          nodeName.c_str());
    return false;
  }
  return true;
...
/*****************************************************************************/
{
  ...
  // Note: 'in' is actually input port of the network. It will be used in the
  // function work().
...
/*************************************************************************************/
bool Network::addParameterUse(AbstractNode *node, const std::string &name,
...
/*****************************************************************************/
{
  ...
  if (!port.linkConnected()) {
    ...
    error("Output port '%s' is not connected to any node or port.",
          port.name().c_str());
    return "";
  }
  Link *l = port.link();
...
/*****************************************************************************/
bool Network::setUserDefinedParameter(const std::string &name,
...
/*****************************************************************************/
{
  ...
  if (found != 0) {
    ...
    for (std::vector<Parameter::Use>::const_iterator used = list.begin();
         used != list.end(); ++used) {
...
/*****************************************************************************/
{
  ...
  /*
   * Input ports are initialized for external usage, i.e. the input ports are
   * used directly by external calls to Network::putData. If the network is
   * embedded in an other network then the port 'in' is deactivated, i.e. the
   * link of the port is disconnected from its to-node, and the new link 'l' is
...
{
  require(l != 0);
  verify(l->getFromNode() !=
         this); // It would imply that 'l' is a input port link.
...
/*****************************************************************************/
{
  ...
  Port &port = outputs_[out];
  if (port.node() == 0) {
    error("Output port \"%s\" not connected to any source node.",
          port.name().c_str());
    return IllegalPortId;
  }

  /*
   * Output ports are initialized for external usage, i.e. the output ports are
   * used directly by external calls to Network::getData. If the network is
   * embedded in an other network then the port 'out' is deactivated, i.e. the
   * link of the port is disconnected from its from-node, and the new link 'l'
   * is directly connected to this node.
   */
  if (port.linkConnected()) {
    verify(l != port.link());
...
/*****************************************************************************/
{
  ...
  verify(l->getToNode() !=
         this); // It would imply that 'l' is a output port link.
...
/*****************************************************************************/
{
  // This function is only called if an input port link of the network has no
  // attributes.
  for (std::vector<Port>::iterator port = inputs_.begin();
       port != inputs_.end(); ++port) {
...
{
  /*
  for (list<AbstractNode*>::const_iterator n = nodes_.begin(); n !=
  nodes_.end(); n++) {
          (*n)->Start();
  }
  */
  ...
  for (std::list<AbstractNode *>::const_iterator n = nodes_.begin();
       n != nodes_.end(); n++) {
  ...
  // loop over all sinks
  while (sinks.size()) {
    for (std::list<AbstractNode *>::iterator n = sinks.begin();
         n != sinks.end(); n++) {
      if (!(*n)->work(0)) {
        sinks.erase(n);
        break;
...
/*****************************************************************************/
bool Network::dump(const bool initialCall, Core::XmlChannel &dumpChannel,
...
/*****************************************************************************/
{
  ...
  for (std::list<AbstractNode *>::const_iterator it = nodes_.begin();
    ...
    if (Network *n = dynamic_cast<Network *>(*it)) {
      ...
      std::set<std::string>::const_iterator it_dumped =
          dumped->find(n->filename());
  ...
  for (std::list<AbstractNode *>::const_iterator it = nodes_.begin();
    ...
    for (UnresolvedAttributes::const_iterator a = atts.begin(); a != atts.end();
         ++a) {
...
/*****************************************************************************/
{
  ...
  o << "node(s):" << std::endl;
  std::copy(n.nodes_.begin(), n.nodes_.end(),
            std::ostream_iterator<AbstractNode *>(o, "\n"));
  o << "link(s):" << std::endl;
  std::copy(n.links_.begin(), n.links_.end(),
            std::ostream_iterator<Link *>(o, "\n"));
...
NetworkTemplate::NetworkTemplate(const char *typname, NodeBuilder *build)
    : typeName_(typname), builder_(build)
/*****************************************************************************/
{}

/*****************************************************************************/
NetworkTemplate::NodeAttributes &
NetworkTemplate::addNodeAttributes(const std::string &name,
...
/*****************************************************************************/
bool NetworkTemplate::addLink(const std::string &fromNodeName,
                              const std::string &fromPortName,
                              const std::string &toNodeName,
                              const std::string &toPortName, u32 buffer)
/*****************************************************************************/
...
/*****************************************************************************/
{
  ...
  for (vector<NodeAttributes>::const_iterator it = savedAttributes_.begin();
    ...
    for (UnresolvedAttributes::const_iterator attribute =
             it->attributes.begin();
  ...
  for (vector<LinkParameter>::const_iterator it = savedLinks_.begin();
    ...
    status =
        network.addLink(it->from_n, it->from_p, it->to_n, it->to_p, it->buffer);
    if (!status)
