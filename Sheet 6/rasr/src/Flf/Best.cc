...
struct TraceElement {
  ...
  Fsa::StateId aid;
  TraceElement()
      : score(Semiring::Max), sid(Fsa::InvalidStateId),
...
std::pair<ConstLatticeRef, Score> bestProjection(ConstLatticeRef l) {
  ...
  for (u32 i = 0; i < topologicalSort->size(); ++i) {
    ...
    */
    ...
    if (sr->isFinal()) {
      const Score score =
          add(currentTrace.score, semiring.project(sr->weight()));
    ...
    Fsa::StateId aid = 0;
    for (State::const_iterator a = sr->begin(), end = sr->end(); a != end;
         ++a, ++aid) {
      ...
      std::cout <<
      Lexicon::us()->phonemeInventory()->phonemeAlphabet()->symbol(leftBoundary.transit().initial)
      ...
      */
      ...
      const Score score =
          add(currentTrace.score, semiring.project(a->weight()));
  ...
  if (bestTrace.sid == Fsa::InvalidStateId) {
    Core::Application::us()->error(
        "No best path found; all pathes have infinite score.");
    return std::make_pair(ConstLatticeRef(), Semiring::Max);
  }
  StaticLattice *s = new StaticLattice(l->type());
  s->setProperties(l->knownProperties() | Fsa::PropertyLinear,
                   l->properties() | Fsa::PropertyLinear);
...
void dumpBest(ConstLatticeRef l, std::ostream &os) {
  ...
  os << "\""
     << ", score=" << semiring->project(sum) << ", scores="
     << semiring->describe(sum, Fsa::HintShowDetails | HintUnscaled)
     << std::endl;
}
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
ConstLatticeRef best(ConstLatticeRef l,
                     SingleSourceShortestPathAlgorithm algo) {
  if (!l || (l->initialStateId() == Fsa::InvalidStateId))
...
// -------------------------------------------------------------------------
namespace {
const Core::Choice choiceSsspAlgorithm("dijkstra", Dijkstra, "bellman-ford",
                                       BellmanFord, "projecting-bellman-ford",
                                       ProjectingBellmanFord,
                                       Core::Choice::endMark());
const Core::ParameterString
    paramSsspAlgorithm("algorithm", "single source shortest path algorithm",
                       "dijkstra");
SingleSourceShortestPathAlgorithm
getSsspAlgorithm(const Core::Configuration &config) {
  Core::Choice::Value ssspChoice =
      choiceSsspAlgorithm[paramSsspAlgorithm(config)];
...
NodeRef createBestNode(const std::string &name,
...
private:
  struct SortDistanceByState {
    bool operator()(const AllPairsShortestDistance::Distance &d1,
                    const AllPairsShortestDistance::Distance &d2) const {
...
public:
  ...
  AllPairsShortestDistance::const_iterator begin() const {
    if (!distance_) {
      AllPairsShortestDistance::Distance *itDist = distance_ =
          new Distance[size()];
      for (const_iterator itMap = Precursor::begin(), endMap = Precursor::end();
           itMap != endMap; ++itMap, ++itDist) {
...
AllPairsShortestDistance::~AllPairsShortestDistance() { delete internal_; }

Score AllPairsShortestDistance::get(Fsa::StateId from, Fsa::StateId to) const {
  return internal_->get(StateIdPair(from, to));
}

AllPairsShortestDistance::const_iterator
AllPairsShortestDistance::begin() const {
...
struct StateIdScorePair {
  Fsa::StateId sid;
  Score score;
  StateIdScorePair(Fsa::StateId sid, Score score) : sid(sid), score(score) {}
  bool operator<(const StateIdScorePair &pair) const { return sid < pair.sid; }
};
typedef std::vector<StateIdScorePair> StateIdScorePairList;
} // namespace

ConstAllPairsShortestDistanceRef
AllPairsShortestDistance::create(ConstLatticeRef lRef, Time timeThreshold) {
  ...
  for (StateMap::const_iterator itSid = topologicalSort->begin(),
    ...
    StateIdScorePairList::iterator itPair =
        std::lower_bound(from.begin(), from.end(), d);
    ...
    for (State::const_iterator a = sr->begin(), a_begin = sr->begin(),
      ...
      for (StateIdScorePairList::const_iterator itFrom = from.begin(),
                                                endFrom = from.end();
    ...
    for (StateIdScorePairList::const_iterator itFrom = from.begin(),
         ...
         itFrom != endFrom; ++itFrom)
      internal->insert(
          std::make_pair(StateIdPair(itFrom->sid, sid), itFrom->score));
  }
  return ConstAllPairsShortestDistanceRef(
      new AllPairsShortestDistance(internal));
}
// -------------------------------------------------------------------------
...
protected:
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    if (!hasDumped_) {
      ConstAllPairsShortestDistanceRef D =
          AllPairsShortestDistance::create(l, timeThreshold_);
      if (connected(1))
        printSegmentHeader(dump_, requestSegment(1));
      dump_ << "# from\tto\tscore\n";
      for (AllPairsShortestDistance::const_iterator itD = D->begin(),
                                                    endD = D->end();
           itD != endD; ++itD)
...
public:
  DumpAllPairsShortestDistanceNode(const std::string &name,
                                   const Core::Configuration &config)
  ...
  virtual void sync() { hasDumped_ = false; }
};
const Core::ParameterFloat DumpAllPairsShortestDistanceNode::paramTimeThreshold(
    "time-threshold", "time threshold", Core::Type<f32>::max);
NodeRef
createDumpAllPairsShortestDistanceNode(const std::string &name,
                                       const Core::Configuration &config) {
