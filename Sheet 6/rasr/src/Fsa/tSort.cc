...
public:
  SortByInputAndOutputAutomaton(_ConstAutomatonRef f) : Precursor(f) {
    this->setProperties(Fsa::PropertySorted,
                        Fsa::PropertySortedByInputAndOutput);
  }
  virtual void modifyState(_State *sp) const {
...
template <class _Automaton>
class SortByInputAndOutputAndTargetAutomaton
...
public:
  SortByInputAndOutputAndTargetAutomaton(_ConstAutomatonRef f) : Precursor(f) {
    this->setProperties(Fsa::PropertySorted,
                        Fsa::PropertySortedByInputAndOutputAndTarget);
  }
  virtual void modifyState(_State *sp) const {
    sp->sort(byInputAndOutputAndTarget<_Automaton>());
  }
  virtual std::string describe() const {
    return "sort(" + Precursor::fsa_->describe() +
...
template <class _Automaton>
typename _Automaton::ConstRef sort(typename _Automaton::ConstRef f,
                                   Fsa::SortType type) {
  switch (type) {
  ...
  case Fsa::SortTypeByArc:
    if (f->hasProperty(Fsa::PropertySortedByArc))
      ...
      return
          typename _Automaton::ConstRef(new SortByArcAutomaton<_Automaton>(f));
  case Fsa::SortTypeByInput:
    if (f->hasProperty(Fsa::PropertySortedByArc) ||
      ...
      return typename _Automaton::ConstRef(
          new SortByInputAutomaton<_Automaton>(f));
...
public:
  ...
  virtual void exploreNonTreeArc(_ConstStateRef from, const _Arc &a) {
    if (color(a.target()) == Precursor::Gray)
      isCyclic_ = true;
  }
};

template <class _Automaton>
Fsa::StateMap topologicallySort(typename _Automaton::ConstRef f,
                                bool progress = false) {
...
template <class _Automaton> bool isAcyclic(typename _Automaton::ConstRef f) {
  TopologicallySortDfsState<_Automaton> v(f);
  v.dfs();
  return !v.isCyclic_;
}
