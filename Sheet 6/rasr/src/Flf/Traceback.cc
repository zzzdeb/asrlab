...
protected:
  void dumpBlissTraceback(ConstLatticeRef l, Core::XmlWriter &os) {
    if (l && (l->initialStateId() != Fsa::InvalidStateId)) {
      ...
      for (Fsa::StateId i = 1, n = initialSr->nArcs(); i <= n; ++i) {
        ...
        traceback.clear();
        traceback.push_back(Search::SearchAlgorithm::TracebackItem(
            0, 0, Search::SearchAlgorithm::ScoreVector(0.0, 0),
        ...
        traceback.push_back(Search::SearchAlgorithm::TracebackItem(
            0, b.time(), Search::SearchAlgorithm::ScoreVector(score, 0),
            Search::SearchAlgorithm::TracebackItem::Transit()));
  ...
  struct CtmPrinter {
    ...
    void print(f32 start, f32 duration, const std::string &word,
               const Scores &scores, Type type = TypeInvalid) {
      os << name << " " << track << " " << Core::form("%.3f", start) << " "
         << Core::form("%.3f", duration) << " " << word;
  ...
  void dumpCtmTraceback(ConstLatticeRef l, std::ostream &os) {
    ...
    if (segment_) {
      if (segment_->hasRecordingId())
        ...
        cp.name =
            Core::stripExtension(Core::baseName(segment_->audioFilename()));
    ...
    if (l && (l->initialStateId() != Fsa::InvalidStateId)) {
      ...
      case Lexicon::LemmaAlphabetId:
        ...
        nonWordToEpsilonMap =
            LabelMap::createNonWordToEpsilonMap(Lexicon::LemmaAlphabetId);
        break;
      case Lexicon::LemmaPronunciationAlphabetId:
        alphabet = lpAlphabet = Lexicon::us()->lemmaPronunciationAlphabet();
        nonWordToEpsilonMap = LabelMap::createNonWordToEpsilonMap(
            Lexicon::LemmaPronunciationAlphabetId);
        break;
      default:
        if (dumpPhonemeAlignment_ || dumpSubwordAlignment_)
          criticalError("Acoustic alignment requires lemma or "
                        "lemma-pronunciation as input, not %s",
      ...
      ConstStateRef initialSr = l->getState(l->initialStateId());
      if (!l->hasProperty(Fsa::PropertyLinear))
        printAsText(os << ";; ", segment_)
            << " [1.." << initialSr->nArcs() << "]-best" << std::endl;
      for (Fsa::StateId i = 1, n = initialSr->nArcs(); i <= n; ++i) {
        ConstLatticeRef p;
        ConstStateRef sr;
        if (l->hasProperty(Fsa::PropertyLinear)) {
        ...
        for (; sr->hasArcs(); sr = p->getState(sr->begin()->target())) {
          verify(sr->nArcs() == 1);
          const Arc &a = *sr->begin();
          const Boundary &leftBoundary = boundaries.get(sr->id()),
                         &rightBoundary = boundaries.get(a.target());
          ...
          if (wordBegin < wordEnd) {
            if (lAlphabet || lpAlphabet) {
              ...
              if ((Fsa::FirstLabelId <= a.input()) &&
                ...
                if (dumpOrthography_) {
                  word = (lAlphabet)
                             ? std::string(lAlphabet->lemma(a.input())
                                               ->preferredOrthographicForm())
                             : lpAlphabet->lemmaPronunciation(a.input())
                                   ->lemma()
                                   ->preferredOrthographicForm();
                } else
                  ...
                  word = Core::form(
                      ...
                      ((leftBoundary.transit().initial == Bliss::Phoneme::term)
                           ...
                           : Lexicon::us()
                                 ->phonemeInventory()
                                 ->phonemeAlphabet()
                                 ->symbol(leftBoundary.transit().initial)
                                 .c_str()),
                      ...
                      ((rightBoundary.transit().initial == Bliss::Phoneme::term)
                           ...
                           : Lexicon::us()
                                 ->phonemeInventory()
                                 ->phonemeAlphabet()
                                 ->symbol(rightBoundary.transit().final)
                                 .c_str()));
                }
                if ((*nonWordToEpsilonMap)[a.input()].empty()) {
                  ...
                  verify_((*nonWordToEpsilonMap)[a.input()].front().label ==
                          Fsa::Epsilon);
              ...
              if (type != CtmPrinter::TypeInvalid) {
                f32 absoluteBegin = begin + wordBegin,
                    duration = wordEnd - wordBegin;
                cp.print(absoluteBegin, duration, word, *a.weight(),
                         (dumpType_ ? type : CtmPrinter::TypeInvalid));
                if (dumpPhonemeAlignment_) {
                  if (!dumpPhonemeAlignment(cp, latticeAlignment, sr,
                                            absoluteBegin, duration))
                    warning() << "No phoneme alignment available for \""
                              << absoluteBegin << " " << duration << " " << word
                              << "\"";
                }
                if (dumpSubwordAlignment_) {
                  if (!dumpSubwordAlignment(cp, latticeAlignment, sr,
                                            absoluteBegin, duration))
                    warning() << "No sub-word alignment available for \""
                              << absoluteBegin << " " << duration << " " << word
                              << "\"";
                }
              }
              ...
              if ((Fsa::FirstLabelId <= a.input()) &&
                  (a.input() <= Fsa::LastLabelId))
                cp.print(begin + wordBegin, wordEnd - wordBegin,
                         alphabet->symbol(a.input()), *a.weight());
              else if (dumpEps_)
                cp.print(begin + wordBegin, wordEnd - wordBegin, epsSymbol_,
                         *a.weight());
            }
          }
        }
  ...
  bool dumpPhonemeAlignment(CtmPrinter &cp,
                            ConstLatticeAlignmentRef latticeAlignment,
                            ConstStateRef sr, f32 absoluteBegin, f32 duration) {
    return false;
  }

  bool dumpSubwordAlignment(CtmPrinter &cp,
                            ConstLatticeAlignmentRef latticeAlignment,
                            ConstStateRef sr, f32 absoluteBegin, f32 duration) {
    return false;
  }
  ...
  void dumpOrthTraceback(ConstLatticeRef l, Core::XmlWriter &os) {
    if (l && (l->initialStateId() != Fsa::InvalidStateId)) {
      ...
      for (ConstStateRef sr = initialSr; sr->hasArcs();
           sr = p->getState(sr->begin()->target())) {
        verify(sr->nArcs() == 1);
        const Arc &a = *sr->begin();
...
public:
  ...
  virtual ~DumpTracebackNode() { delete dumpXmlWriter_; }

  virtual void init(const std::vector<std::string> &arguments) {
    unkLemma_ = (Lexicon::us()->unkLemmaId() == Fsa::InvalidLabelId)
                    ? 0
                    : Lexicon::us()->lemmaAlphabet()->lemma(
                          Lexicon::us()->unkLemmaId());
    unkLemmaPron_ =
        (Lexicon::us()->unkLemmaPronunciationId() == Fsa::InvalidLabelId)
            ? 0
            : Lexicon::us()->lemmaPronunciationAlphabet()->lemmaPronunciation(
                  Lexicon::us()->unkLemmaPronunciationId());
...
const Core::Choice DumpTracebackNode::CtmPrinter::TypeChoice(
    "invalid", TypeInvalid, "lex", TypeLexical, "sub-lex-begin",
    TypeSubLexicalBegin, "sub-lex-within", TypeSubLexicalWithin, "sub-lex-end",
    TypeSubLexicalEnd, "non-lex", TypeNonLexical, "fp", TypeHesitation, "phone",
    TypePhone, Core::Choice::endMark());
const Core::Choice DumpTracebackNode::TracebackFormatChoice =
    Core::Choice("bliss", 0, "ctm", 1, "corpus", 2, Core::Choice::endMark());
const Core::ParameterChoice DumpTracebackNode::paramTracebackFormat(
    "format", &DumpTracebackNode::TracebackFormatChoice, "traceback format", 1);
const Core::ParameterBool DumpTracebackNode::paramDumpOrthography(
    "dump-orthography",
    "dump preferred orthography instead of lattice input alphabet", true);
const Core::ParameterBool
    DumpTracebackNode::paramDumpCoarticulation("dump-coarticulation",
                                               "dump coarticualtion", false);
const Core::ParameterBool DumpTracebackNode::paramDumpNonWord("dump-non-word",
                                                              "dump non word",
                                                              false);
const Core::ParameterBool
    DumpTracebackNode::paramDumpEps("dump-eps", "dump epsilon", false);
...
const Core::ParameterStringVector DumpTracebackNode::paramScoreKeys(
    "scores", "dimension of scores to be dumped; default is no scores", "");
const Core::ParameterBool DumpTracebackNode::paramDumpType("dump-type",
                                                           "dump type", false);
const Core::ParameterBool DumpTracebackNode::paramDumpPhonemeAlignment(
    "dump-phoneme-alignment", "dump phoneme alignment", false);
const Core::ParameterBool DumpTracebackNode::paramDumpSubwordAlignment(
    "dump-subword-alignment", "dump subword alignment", false);
