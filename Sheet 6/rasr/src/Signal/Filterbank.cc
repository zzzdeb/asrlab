...
FilterBank::Filter::Filter(size_t start, size_t end,
                           const std::vector<FilterWeight> &weights)
    : start_(start), end_(end), weights_(weights) {
  require(end_ >= start_);
  require(end_ - start_ == weights_.size());
}

void FilterBank::Filter::normalize(
    FilterBank::NormalizationType normalizationType) {
  if (normalizationType != FilterBank::normalizeNone) {
    ...
    if (normalizationType == FilterBank::normalizeSurface) {
      normalizationTerm =
          std::accumulate(weights_.begin(), weights_.end(), (FilterWeight)0);
    } else
      defect();
    std::transform(
        weights_.begin(), weights_.end(), weights_.begin(),
 ...
 *    1) normalizedCenterPosition: return the position of the center of the
 * filter in percent. -0 -> filter has no left flank. -1 -> filter has no right
 * flank. 2) weight(frequency): returns weight for the given frequency.
 */
...
Core::Ref<FilterBank::Filter> FilterBank::FilterBuilder::create(
  ...
  discreteToContinuousFunction_ =
      Math::nest(warpingFunction, discreteToContinuousFunction);
  ...
  if (continuousToDiscreteFunction_) {
    if (shouldWarpDifferentialUnit) {
      if (warpingFunction->derive())
        derivedWarpingFunction_ =
            Math::nest(warpingFunction->derive(), discreteToContinuousFunction);
    } else {
      derivedWarpingFunction_ =
          Math::AnalyticFunctionFactory::createConstant(1);
    }
...
bool FilterBank::FilterBuilder::setStart() {
  ...
  error("Start point of the filter at center %f became negative (%d).", center_,
        (int)start);
  return false;
}
...
bool FilterBank::FilterBuilder::setEnd() {
  Frequency end = continuousToDiscreteFunction_->value(
      std::min(center_ + (1.0 - normalizedCenterPosition()) * width_,
               maximumFrequency_));
  ...
  error("Inconsistent end point of the filter at center %f: start=%zd end=%d.",
        center_, start_, (int)end);
  return false;
}
...
FilterBank::FilterWeight SymmetricalTriangularFilterBuilder::weight(
    FilterBank::Frequency frequency) const {
  FilterBank::FilterWeight result =
      (FilterBank::Frequency)1 - Core::abs(frequency - center_) / (width_ / 2);
  /* start_ and end_ control that 'frequency' stays within the interval
   * [center-halfWidth..center+halfWidth], thus the resulting weight has to be
   * larger or equal to zero. Nevertheless rounding error can yield slightly
   * negative weight which will be clipped to zero.
   */
...
FilterBank::FilterWeight
TrapezeFilterBuilder::weight(FilterBank::Frequency frequency) const {
...
public:
  ...
  // Start and end boundaries are different to the normal way, so change them
  // here. Same for weights
  virtual bool setStart();
  virtual bool setEnd();
};

FilterBank::FilterWeight
TrapezeRastaFilterBuilder::weight(FilterBank::Frequency frequency) const {
  FilterBank::Frequency relativeFrequency = frequency - center_;
  FilterBank::Frequency middleBorder =
      normalizedMiddleBorder() * width_; // => 0.5
...
protected:
  Boundary(const Core::Configuration &c)
      : Core::Component(c), filterWidth_(0), spacing_(0),
        normalizedCenterPosition_(0), maximumFrequency_(0) {}
...
void FilterBank::Boundary::init(Frequency filterWidth, Frequency spacing,
                                Frequency normalizedCenterPosition) {
...
bool FilterBank::Boundary::init(
    ...
    Frequency normalizedCenterPosition, Frequency maximumFrequency,
    Math::UnaryAnalyticFunctionRef warpingFunction) {
...
bool FilterBank::Boundary::setWarpingFunction(
    Math::UnaryAnalyticFunctionRef warpingFunction) {
...
FilterBank::Boundary::Frequency
FilterBank::Boundary::postprocessNumberOfFilters(Frequency nFilters) const {
  if (nFilters < 1)
    return 1;
...
/**
 *  Include Boundary: left and right boundaries of input are included but with
 * small weights. -First filter starts at center equals spacing. -Last filter is
 * chosen such that its left border is smaller than maximal-frequency and its
 * right border is larger or equal to the maximal-frequency.
 *  =>Number of filters = ceil(maximum-frequency - (1.0 - normalized-center) *
 * filter-width) / spacing) -Sample rate of output vector: 1.0 / spacing. -Note
 * that if spacing > normalized-center * filter-width, the leftmost few input
 * values will not contribute to the results.
 */
...
size_t IncludeBoundary::getNumberOfFilters() const {
  return (size_t)Core::ceil(postprocessNumberOfFilters(
...
/**
 *  Stretch-To-Cover: filter width and spacing are stretched to cover the
 * complete input exactly, thus left and right boundaries of input are included
 * with weight 0. -Basic idea to ensure the complete coverage of the whole
 * interval [0..maximal-frequency] with minimal number of filters. -First filter
 * starts at left-border equals 0. -Last filter is chosen such that its right
 * border is smaller or equal to maximal-frequency and the right-border of the
 * next filter would be larger than maximal-frequency.
 *  =>Number of filters = floor((maximal-frequency - filter-width) / spacing +
 * 1) -Finally, spacing and filter width are multiplied by a number > 1 such
 * that right border of the last filter falls on the maximal-frequency. -Sample
 * rate of output vector: since spacing is changed according to the input
 * length, the output sample-rate cannot be retrieved before the first input
 * arrives, thus it will be set to 1. -Pre-warping of centers is supported in
 * this boundary type since pre-warping introduces non-linearities.
 */
...
bool StretchToCover::init(Frequency filterWidth, Frequency spacing,
                          Frequency normalizedCenterPosition,
                          Frequency maximumFrequency,
                          Math::UnaryAnalyticFunctionRef warpingFunction) {
  if (warpingFunction) // unwarping of center not possible in this boundary
                       // type.
    return false;
  ...
  Frequency coverage =
      (spacing_ * (nFilters - 1) + filterWidth_) / maximumFrequency_;
  if (nFilters == 1 && Core::isSignificantlyGreater(coverage, 1))
    return true; // If single filter ensures full coverage, then do not shrink
                 // it.

  require(Core::isAlmostEqual(coverage, 1) ||
          coverage < 1); // If fails then nFilters was too high
...
/**
 *  Emphasize Boundary: left and right boundaries of input are included with
 * large weights. -First filter starts at center equals 0. -Last filter is
 * chosen such that its center is smaller than maximal-frequency + spacing.
...
FilterBank::FilterBank(const Core::Configuration &c)
    : Core::Component(c), builder_(0), boundary_(0), filterWidth_(0),
      spacing_(0), maximumFrequency_(0), shouldWarpDifferenctialUnit_(true),
      shouldWarpCenterPositions_(true), normalizationType_(normalizeNone),
      needInit_(true) {}
...
bool FilterBank::init() {
  if (!builder_ || !boundary_ || (maximumFrequency_ == 0) ||
      !boundary_->init(filterWidth_, spacing_,
                       ...
                       shouldWarpCenterPositions_
                           ? Math::UnaryAnalyticFunctionRef()
                           : warpingFunction_)) {
    return false;
  }
  ...
  for (size_t i = 0; i < filters_.size(); ++i) {
    filters_[i] =
        builder_->create(boundary_->center(i), boundary_->filterWidth(),
                         maximumFrequency_, discreteToContinuousFunction_,
...
void FilterBank::dump(Core::XmlWriter &o) {
  if (needInit_)
    init();
  o << Core::XmlOpen("filter-bank") +
           Core::XmlAttribute("warping-function", warpingFunctionName_);
  for (size_t f = 0; f < filters_.size(); ++f)
    filters_[f]->dump(o);
...
//============================================================================================
const Core::Choice FilterBankNode::choiceFilterType("triangular",
                                                    typeTriangular, "trapeze",
                                                    typeTrapeze, "trapezeRasta",
                                                    typeRastaTrapeze,
                                                    Core::Choice::endMark());
const Core::ParameterChoice FilterBankNode::paramFilterType("type",
                                                            &choiceFilterType,
                                                            "filter bank type",
                                                            typeTriangular);
...
const Core::Choice FilterBankNode::choiceBoundaryType(
    "include-boundary", FilterBank::includeBoundary, "stretch-to-cover",
    FilterBank::stretchToCover, "emphasize-boundary",
    FilterBank::emphasizeBoundary, Core::Choice::endMark());
const Core::ParameterChoice
    FilterBankNode::paramBoundaryType("boundary", &choiceBoundaryType,
                                      "boundary type",
                                      FilterBank::stretchToCover);

const Core::ParameterFloat FilterBankNode::paramFilteringInterval(
    "filtering-interval", "Filters are placed only inside this interval.",
    Core::Type<f32>::max, 0);

const Core::Choice FilterBankNode::choiceNormalizationType(
    "none", FilterBank::normalizeNone, "surface", FilterBank::normalizeSurface,
    Core::Choice::endMark());
const Core::ParameterChoice FilterBankNode::paramNormalizationType(
    "normalization", &choiceNormalizationType, "filterbank type",
    FilterBank::normalizeNone);

const Core::ParameterString
    FilterBankNode::paramWarpingFunction("warping-function",
                                         "warping function declaration");
const Core::ParameterBool FilterBankNode::paramShouldWarpDifferentialUnit(
    "warp-differential-unit",
    "Controls if derivative of warping function is applied.", true);
const Core::ParameterBool FilterBankNode::paramShouldWarpCenterPositions(
    "warp-center-positions", "Controls if filter center position are warped.",
    true);

FilterBankNode::FilterBankNode(const Core::Configuration &c)
    : Core::Component(c), Node(c), FilterBank(c), StringExpressionNode(c, 1),
      filteringInterval_(Core::Type<f32>::max), inputSize_(0), sampleRate_(0),
      needInit_(true), dumpChannel_(c, "dump-filters") {
  addInput(0);
  addOutput(0);
  ...
  setNormalizationType(
      (FilterBank::NormalizationType)paramNormalizationType(c));
...
void FilterBankNode::createAnalyticFunction(
  ...
  factory.setMaximalArgument(
      discreteToContinuousFunction->value(inputSize_ - 1));
...
bool FilterBankNode::setParameter(const std::string &name,
  ...
  if (paramFilterType.match(name))
    ...
    setNormalizationType(
        (FilterBank::NormalizationType)paramNormalizationType(value));
...
bool FilterBankNode::work(Flow::PortId p) {
  ...
  if (in->size() != inputSize_) {
    criticalError(
        "Input size (%zd) does not match the expected input size (%d)",
        in->size(), inputSize_);
  }
