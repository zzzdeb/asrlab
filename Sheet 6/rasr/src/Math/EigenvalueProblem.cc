...
const Core::Choice EigenvalueProblem::choiceEigenvectorNormalizationType(
    "none", nonEigenvectorNormalization, "unity-length", unityLength,
    "unity-diagonal", unityDiagonal, Core::Choice::endMark());
...
const Core::ParameterFloat
    EigenvalueProblem::paramDiagonalNormalizationTolerance(
        "eigenvector-normalization-tolerance",
        "tolerance used when inverting the diagonal.", 0, 0);
const Core::ParameterFloat EigenvalueProblem::paramVerificationTolerance(
    "verification-tolerance", "tolerance used when verifying the solution.",
    Core::Type<f64>::max, 0);
const Core::ParameterFloat EigenvalueProblem::paramRelativeImaginaryMaximum(
    "relative-imaginary-maximum",
    "while copiing real part of a container, values are reported which "
    "abs(imag) > max(abs(container)) * this.",
    1, 0, 1);
const Core::Choice EigenvalueProblem::choiceType("general", typeGeneral,
                                                 "symmetric", typeSymmetric,
                                                 "symmetric-positive-definite",
                                                 typeSymmetricPositiveDefinite,
                                                 Core::Choice::endMark());
const Core::ParameterChoice
    EigenvalueProblem::paramType("type", &choiceType,
                                 "type of eigenvalue problem", typeGeneral);
const Core::Choice EigenvalueProblem::choiceEigenvalueSortType(
    "none", noSort, "decreasing", decreasing, "increasing", increasing,
    Core::Choice::endMark());
...
EigenvalueProblem::EigenvalueProblem(const Core::Configuration &c)
    : Core::Component(c),
      eigenvectorNormalizationType_(
          (EigenvectorNormalizationType)paramEigenvectorNormalization(c)),
...
bool EigenvalueProblem::solveSymmetricAndFinalize(
    const Matrix<ValueType> &a, Vector<ValueType> &eigenvalues,
...
bool EigenvalueProblem::normalizeDiagonal(const Matrix<ValueType> &a,
                                          Matrix<ValueType> &eigenvectors) {
  bool result = true;
  ...
  for (size_t i = 0; i < diagonal.size(); ++i) {
    if (diagonal[i] < 0 &&
        Core::abs(diagonal[i]) > diagonalNormalizationTolerance_) {
      error("Failed to normalize eigenvectors because "
            "because %dth diagonal element %e is negative.",
            i, diagonal[i]);
      result = false;
    }
  }
  if (result) {
    ...
    Message scalingMessage(
        log("Eigenvector scaling factors relative to their lengths:\n"));
    for (size_t i = 0; i < diagonal.size(); ++i) {
      if (diagonal[i] == 0)
...
bool EigenvalueProblem::isCorrectSolution(
    const Matrix<ValueType> &a, const Vector<ValueType> &eigenvalues,
    const Matrix<ValueType> &eigenvectors) {
  if ((a.nColumns() != eigenvectors.nRows()) ||
      (eigenvectors.nColumns() != eigenvalues.size())) {
    error("Dimension mismatch while verifying solution: A[%zd, %zd], X[%zd, "
          "%zd], lambda[%zd].)",
          a.nRows(), a.nColumns(), eigenvectors.nRows(),
          eigenvectors.nColumns(), eigenvalues.size());
    return false;
  }
  Matrix<ValueType> aTimesEigenvectors(a * eigenvectors);
  Matrix<ValueType> eigenvectorsTimesValues(eigenvectors *
                                            makeDiagonalMatrix(eigenvalues));
  return areEqual(aTimesEigenvectors, eigenvectorsTimesValues);
}

bool EigenvalueProblem::areEqual(const Matrix<ValueType> &x,
                                 const Matrix<ValueType> &y) {
  std::pair<size_t, size_t> notEqual =
      x.isAlmostEqual(y, verificationTolerance_);

  if (notEqual.first != x.nRows()) {
    error("Checking solution failed in row=%zd columns=%zd (%e != %e at "
          "tolerance %e).",
          notEqual.first, notEqual.second, x[notEqual.first][notEqual.second],
          y[notEqual.first][notEqual.second], verificationTolerance_);
    return false;
  }
  return true;
}

void EigenvalueProblem::sortEigenvalues(Vector<ValueType> &eigenvalues,
                                        Matrix<ValueType> &eigenvectors) {
  if (eigenvalueSortType_ != noSort) {
    ...
    std::transform(eigenvalueIndexMap.begin(), eigenvalueIndexMap.end(),
                   permutation.begin(), select2nd<MapItem>());
    eigenvalues.permuteElements(permutation);
    eigenvectors.permuteColumns(permutation);
  }
}

bool EigenvalueProblem::real(const Vector<std::complex<ValueType> > &c,
                             Vector<ValueType> &r) {
  ValueType imaginaryMaximum =
      Core::maxAbsoluteElement(c) * relativeImaginaryMaximum_;
  bool success = true;
  r.resize(c.size());
  ...
  for (ci = c.begin(), ri = r.begin(); ci != c.end(); ++ci, ++ri) {
    ...
    if (absImag > imaginaryMaximum) {
      error("The %zdth imaginary element (%e) is larger than imaginary-maximum "
            "(%e).",
...
bool EigenvalueProblem::real(const Matrix<std::complex<ValueType> > &c,
                             Matrix<ValueType> &r) {
  ValueType imaginaryMaximum =
      Math::maxAbsoluteElement(c) * relativeImaginaryMaximum_;
  bool success = true;
  ...
  for (size_t row = 0; row < r.nRows(); ++row) {
    for (size_t column = 0; column < r.nColumns(); ++column) {
      ...
      if (absImag > imaginaryMaximum) {
        error("The (%zd, %zd) imaginary element (%e) is larger than "
              "imaginary-maximum (%e).",
...
const Core::ParameterBool
    GeneralizedEigenvalueProblem::paramNormalizeEigenvectorsUsingB(
        "normalize-eigenvectors-using-B",
        "If yes, matrix B is used in eigenvector normalization instead of A.",
        false);

GeneralizedEigenvalueProblem::GeneralizedEigenvalueProblem(
    const Core::Configuration &c)
    : Core::Component(c), EigenvalueProblem(c),
...
bool GeneralizedEigenvalueProblem::finalizeEigenvaluesAndVectors(
  ...
  if (normalizeEigenvectorsUsingB_)
    m = &b;
  return EigenvalueProblem::finalizeEigenvaluesAndVectors(*m, eigenvalues,
                                                          eigenvectors);
}
...
bool GeneralizedEigenvalueProblem::isCorrectSolution(
  ...
  if ((a.nColumns() != eigenvectors.nRows()) ||
      ...
      (eigenvectors.nColumns() != eigenvalues.size())) {
    error("Dimension mismatch while verifying solution: A[%zd, %zd], B[%zd, "
          "%zd], X[%zd, %zd], lambda[%zd].)",
