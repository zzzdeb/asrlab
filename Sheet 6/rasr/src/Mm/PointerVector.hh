 ...
 */
template <class T> class PointerVector : private std::vector<T *> {
public:
  typedef std::vector<T *> Parent;
  typedef typename Parent::const_iterator ConstantIterator;

private:
  /** Copying of pointer tables hard to solve generally because of inheritance
   * and it is not (yet) used in this library.
   */
...
public:
   ...
   *  If @param newIndex is larger than the last index, zero elements are
   * inserted.
  ...
  /** @return type is T* to prevent the pointer value from changing at @param
   * index.
   */
...
template <class T> void PointerVector<T>::clear() {
