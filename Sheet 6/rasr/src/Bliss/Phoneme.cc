...
Phoneme::Phoneme() : Token(), isContextDependent_(true) {}
...
PhonemeInventory::PhonemeInventory() : internal_(0) {
...
PhonemeInventory::PhonemeInventory(const PhonemeInventory &phonemeInventory) {
  ...
  for (Core::tie(pi, pi_end) = phonemeInventory.phonemes(); pi != pi_end;
       ++pi) {
...
PhonemeInventory::~PhonemeInventory() { delete internal_; }
...
void PhonemeInventory::writeXml(Core::XmlWriter &os) const {
  ...
  for (Core::tie(pi, pi_end) = phonemes(); pi != pi_end; ++pi) {
    os << Core::XmlOpen("phoneme") << Core::XmlFull("symbol", (*pi)->symbol());
...
Core::Ref<const PhonemeAlphabet> PhonemeInventory::phonemeAlphabet() const {
  ...
  if (internal_->phonemeAlphabet_)
    ...
    internal_->phonemeAlphabet_ = result =
        Core::ref(new PhonemeAlphabet(Core::ref(this)));
  return result;
}
