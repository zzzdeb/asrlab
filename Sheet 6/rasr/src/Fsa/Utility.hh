...
private:
  struct StringHashKey_ {
    LabelIdStrings &s_;
    StringHashKey_(LabelIdStrings &s) : s_(s) {}
    u32 operator()(Id s) {
      u32 value = 0;
  ...
  struct StringHashEqual_ {
    LabelIdStrings &s_;
    StringHashEqual_(LabelIdStrings &s) : s_(s) {}
...
public:
  ...
  Id start() const { return strings_.size(); }
  ...
  void append(LabelId l) { strings_.push_back(l); }

  Id append(Core::Vector<LabelId>::const_iterator i);

  void discard(Id start) { strings_.resize(start); }

  const_iterator begin(Id s) const { return strings_.begin() + s; }
