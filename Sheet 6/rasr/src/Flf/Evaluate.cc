...
const Core::ParameterBool
    Evaluator::paramSingleBest("single-best",
                               "decode lattice and evaluate result", true);
const Core::ParameterBool
    Evaluator::paramBestInLattice("best-in-lattice", "evaluate lattice", true);
const Core::Choice Evaluator::choiceSsspAlgorithm("dijkstra", Dijkstra,
...
const Core::ParameterChoice
    Evaluator::paramSsspAlgorithm("algorithm", &choiceSsspAlgorithm,
                                  "single source shortest path algorithm",
                                  Dijkstra);

Evaluator::Evaluator(const Core::Configuration &config, const std::string &name)
    : Core::Component(config), LayeredComponent(config, name),
      blissEvaluator_(0), lastLemmaUpdates_(0),
      ssspAlgorithm_(static_cast<SingleSourceShortestPathAlgorithm>(
          paramSsspAlgorithm(config))) {
...
Evaluator::~Evaluator() { delete blissEvaluator_; }
...
void Evaluator::evaluate(ConstLatticeRef l) {
  ...
  if (logSingleBest_) {
    ConstLatticeRef eval = (l && !l->hasProperty(Fsa::PropertyLinear))
                               ? best(l, ssspAlgorithm_)
                               : l;
...
void evaluate(ConstLatticeRef l, const std::string &ref) {
  Core::Configuration _config(Core::Application::us()->getConfiguration(),
                              "evaluator");
...
public:
  ...
  virtual ~EvaluatorNode() { delete evaluator_; }
  virtual void init(const std::vector<std::string> &arguments) {
    if (!(connected(1) || connected(2)))
      criticalError(
          "EvaluatorNode: Need a data source either at port 1 or port 2");
  ...
  void sync() { validReference_ = false; }
};

NodeRef createEvaluatorNode(const std::string &name,
