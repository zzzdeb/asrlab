...
PackedAutomaton::PackedAutomaton() { clear(); }
...
bool PackedAutomaton::hasState(StateId s) const { return bool(getState(s)); }
...
size_t PackedAutomaton::getMemoryUsed() const {
  ...
  return size + sizeof(u8) * arcs_.size() +
         sizeof(PackedState) * states_.size() + 4 * sizeof(size_t);
}

float PackedAutomaton::getStateRatio() const {
  return (nStates_ * sizeof(State)) /
         float(pStates_ + nStates_ * sizeof(PackedState));
}
...
float PackedAutomaton::getRatio() const {
  return (sizeof(Arc) * nArcs_ + nStates_ * sizeof(State)) /
         float(arcs_.size() + nStates_ * sizeof(PackedState));
}

void PackedAutomaton::dumpMemoryUsage(Core::XmlWriter &o) const {
  o << Core::XmlOpen("packed") << Core::XmlFull("state-ratio", getStateRatio())
    << Core::XmlFull("arc-ratio", getArcRatio())
    << Core::XmlFull("ratio", getRatio())
    << Core::XmlFull("total", getMemoryUsed()) << Core::XmlClose("packed");
}
