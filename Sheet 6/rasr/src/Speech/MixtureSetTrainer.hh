...
protected:
  virtual Mm::AbstractMixtureSetEstimator *
  createMixtureSetEstimator() const = 0;
  ...
  void setAssigningFeatureScorer(
      ...
      Core::Ref<const Mm::AssigningFeatureScorer> assigningFeatureScorer =
          Core::Ref<Mm::AssigningFeatureScorer>());
  ...
  virtual const Core::Ref<Mm::MixtureSet> getMixtureSet(size_t nMixtures,
                                                        size_t dimension);
...
public:
  ...
  void write(Core::XmlWriter &o) const {
    verify(estimator_ != 0);
    estimator_->write(o);
  }
