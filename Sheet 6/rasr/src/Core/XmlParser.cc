...
/** xmlChar to char */
const char *x2c(const xmlChar *x) { return reinterpret_cast<const char *>(x); }
...
xmlParserInputPtr
XmlParser::SAX_resolveEntity_callback(void *ctx, const xmlChar *publicId,
                                      const xmlChar *systemId) {
  return static_cast<self *>(ctx)->resolveEntity(x2c(publicId), x2c(systemId));
}

void XmlParser::SAX_internalSubset_callback(void *ctx, const xmlChar *name,
                                            const xmlChar *ExternalID,
                                            const xmlChar *SystemID) {
  static_cast<self *>(ctx)->internalSubset(x2c(name), x2c(ExternalID),
                                           x2c(SystemID));
}

void XmlParser::SAX_externalSubset_callback(void *ctx, const xmlChar *name,
                                            const xmlChar *ExternalID,
                                            const xmlChar *SystemID) {
  static_cast<self *>(ctx)->externalSubset(x2c(name), x2c(ExternalID),
                                           x2c(SystemID));
}
...
xmlEntityPtr XmlParser::SAX_getParameterEntity_callback(void *ctx,
                                                        const xmlChar *name) {
  return static_cast<self *>(ctx)->getParameterEntity(x2c(name));
}

void XmlParser::SAX_entityDecl_callback(void *ctx, const xmlChar *name,
                                        int type, const xmlChar *publicId,
                                        const xmlChar *systemId,
                                        xmlChar *content) {
  static_cast<self *>(ctx)->entityDecl(x2c(name), type, x2c(publicId),
                                       x2c(systemId), x2c(content));
}

void XmlParser::SAX_notationDecl_callback(void *ctx, const xmlChar *name,
                                          const xmlChar *publicId,
                                          const xmlChar *systemId) {
  static_cast<self *>(ctx)->notationDecl(x2c(name), x2c(publicId),
                                         x2c(systemId));
}

void XmlParser::SAX_attributeDecl_callback(void *ctx, const xmlChar *elem,
                                           const xmlChar *name, int type,
                                           int def, const xmlChar *defaultValue,
                                           xmlEnumerationPtr tree) {
  static_cast<self *>(ctx)->attributeDecl(x2c(elem), x2c(name), type, def,
                                          x2c(defaultValue), tree);
}

void XmlParser::SAX_elementDecl_callback(void *ctx, const xmlChar *name,
                                         int type,
                                         xmlElementContentPtr content) {
  static_cast<self *>(ctx)->elementDecl(x2c(name), type, content);
}

void XmlParser::SAX_unparsedEntityDecl_callback(void *ctx, const xmlChar *name,
                                                const xmlChar *publicId,
                                                const xmlChar *systemId,
...
void XmlParser::SAX_startElement_callback(void *ctx, const xmlChar *name,
                                          const xmlChar **atts) {
...
void XmlParser::SAX_ignorableWhitespace_callback(void *ctx, const xmlChar *ch,
                                                 int len) {
  static_cast<self *>(ctx)->ignorableWhitespace(x2c(ch), len);
}

void XmlParser::SAX_processingInstruction_callback(void *ctx,
...
void XmlParser::externalSubset(const char *name, const char *ExternalID,
                               const char *SystemID) {
...
xmlEntityPtr XmlParser::getParameterEntity(const char *name) { return NULL; }

void XmlParser::entityDecl(const char *name, int type, const char *publicId,
                           const char *systemId, const char *content) {
  //  ::entityDecl(ctxt_, name, type, publicId, systemId, content) ;
}

void XmlParser::attributeDecl(const char *elem, const char *name, int type,
                              int def, const char *defaultValue,
                              xmlEnumerationPtr tree) {
  //  ::attributeDecl(ctxt_, elem, name, type, def, defaultValue, tree) ;
}

void XmlParser::elementDecl(const char *name, int type,
                            xmlElementContentPtr content) {
  //  ::elementDecl(ctxt_, name, type, content) ;
}

void XmlParser::notationDecl(const char *name, const char *publicId,
                             const char *systemId) {
  //  ::notationDecl(ctxt_, name, publicId, systemId) ;
}

void XmlParser::unparsedEntityDecl(const char *name, const char *publicId,
                                   const char *systemId,
...
XmlChannel *XmlParser::vErrorMessage(ErrorType mt, const char *msg,
                                     va_list ap) const {
  XmlChannel *chn = Component::vErrorMessage(mt, msg, ap);
  if (ctxt_ != NULL) {
    *chn << " in \"" << x2c(xmlSAX2GetSystemId(ctxt_)) << "\""
         << " line " << xmlSAX2GetLineNumber(ctxt_) << " column "
         << xmlSAX2GetColumnNumber(ctxt_);
  }
  return chn;
}
 ...
 *    - xmlCreatePushParserCtxt needs at least 4 bytes to allow content encoding
 * detection
...
int XmlParser::parseStream(std::istream &is, const std::string &filename) {
  ...
  ctxt_ = xmlCreatePushParserCtxt(const_cast<xmlSAXHandler *>(&SAX_callbacks),
                                  static_cast<void *>(this), buffer, nRead,
                                  filename.c_str());
  if (ctxt_ == NULL)
    error("Failed to setup XML parser");
...
#if GCC_v3
  /** in gcc-3.1 istream::readsome(...) returns always zero. It worked fine in
   * gcc-3.0.4. */
...
XmlElementGarbageCollector::~XmlElementGarbageCollector() {
  for (std::vector<XmlElement *>::iterator i = xmlElements_.begin();
       i != xmlElements_.end(); ++i)
    delete *i;
}

// ===========================================================================
XmlElement::XmlElement(const char *_name, XmlContext *_context)
    : context_(_context), name_(_name) {}
...
XmlDocumentElement::XmlDocumentElement(XmlElement *_root)
    : XmlParentElement(" ROOT ", this), root_(_root) {}

void XmlDocumentElement::setRoot(XmlElement *_root) { root_ = _root; }
...
// ===========================================================================
XmlEmptyElement::XmlEmptyElement(const char *_name, XmlContext *_context,
                                 bool allowWhitespace)
    : XmlElement(_name, _context), allowWhitespace_(allowWhitespace) {}
...
void XmlEmptyElement::characters(const char *ch, int len) {
  if (!allowWhitespace_ ||
      std::string((char *)ch, len).find_first_not_of(utf8::whitespace) !=
          std::string::npos)
    // validityError
    parser()->error("Element \"%s\" is declared to have empty content", name());
}
...
XmlFlattenElementRelay::XmlFlattenElementRelay(
    const char *_name, XmlContext *_context,
    XmlContext::StartHandler _startHandler, XmlContext::EndHandler _endHandler)
...
XmlElement *XmlParentElement::unexpectedElement() { return unexpectedElement_; }
...
void XmlMixedElement::addChild(XmlElement *decl) { children_.push_back(decl); }
...
XmlMixedElementRelay::XmlMixedElementRelay(
    const char *_name, XmlContext *_context,
    XmlContext::StartHandler _startHandler, XmlContext::EndHandler _endHandler,
    XmlContext::CharactersHandler _charactersHandler, XmlElement *_child, ...)
    : XmlMixedElement(_name, _context) {
  va_list ap;
...
void XmlRegularElement::characters(const char *ch, int len) {
  if (std::string((char *)ch, len).find_first_not_of(utf8::whitespace) !=
      std::string::npos) {
    // validity error
...
void XmlRegularElement::draw(std::ostream &os) const {
  ...
  for (t = transitions_.begin(); t != transitions_.end(); ++t) {
    if (t->child) {
      os << t->from << " -> " << t->to << " [label=\"" << t->child->name()
         << "\"];" << std::endl;
    } else {
...
void XmlSchemaParser::startElement(const char *_name,
...
void XmlSchemaParser::endElement(const char *name) {
  ...
  if (parse_stack_.top()->matches(name)) {
    ...
    // well-formedness error
    error("Found mismatched closing tag: \"%s\" instead of \"%s\"", name,
          parse_stack_.top()->name());
  }
}
...
XmlSchemaParser::XmlSchemaParser(const Configuration &c)
    : XmlParser(c), XmlDocumentElement() {}
