...
public:
  ComposeAutomaton(Fsa::ConstAutomatonRef left,
                   Core::Ref<const Lm::LanguageModel> right, Score lmScale,
                   Score syntaxEmissionScale)
      : left_(left), right_(right), lmScale_(lmScale),
        syntaxEmissionScale_(syntaxEmissionScale) {
    setProperties(Fsa::PropertyStorage | Fsa::PropertyCached, 0);
    setProperties(Fsa::PropertyAcyclic,
                  Fsa::hasProperties(left_, Fsa::PropertyAcyclic));
  }
  ...
  virtual Fsa::Type type() const { return left_->type(); }

  virtual Fsa::ConstSemiringRef semiring() const { return left_->semiring(); }
  ...
  virtual void dumpState(Fsa::StateId si, std::ostream &os) const {
    ...
    os << ", " << right_->formatHistory(states_[si].right) << ")";
  }
  ...
  static ComposeAutomaton *
  createComposeAutomaton(Fsa::ConstAutomatonRef left,
  ...
  void addScore(Fsa::Arc &a, Lm::History &history) const {
    if (a.output() != Fsa::Epsilon) {
      ...
      Lm::addLemmaScore(right_, lmScale_,
                        ...
                        syntaxEmissionScale_, history, score);
...
protected:
  void addScore(Fsa::Arc &a, Lm::History &history) const {
    ...
    if (pronunciation) {
      Lm::Score score = f32(a.weight_);
      Lm::addLemmaPronunciationScore(right_, lmScale_, pronunciation,
                                     pronunciationScale_, syntaxEmissionScale_,
                                     history, score);
      a.weight_ = Fsa::Weight(score);
    }
  }
...
public:
  LemmaPronunciationComposeAutomaton(Fsa::ConstAutomatonRef left,
                                     Core::Ref<const Lm::LanguageModel> right,
                                     Score lmScale, Score pronunciationScale,
                                     Score syntaxEmissionScale)
      : Precursor(left, right, lmScale, syntaxEmissionScale),
    ...
    left_ = Fsa::mapInput(
        left_, right_->lexicon()->lemmaPronunciationAlphabet(), true);
    left_ = Fsa::cache(left_);
  }
...
ComposeAutomaton *ComposeAutomaton::createComposeAutomaton(
    Fsa::ConstAutomatonRef left, Core::Ref<const Lm::LanguageModel> right,
    Score lmScale, Score syntaxEmissionScale) {
  ComposeAutomaton *composeAutomaton = 0;
  if (left->getOutputAlphabet() == right->lexicon()->lemmaAlphabet()) {
    composeAutomaton =
        new LemmaComposeAutomaton(left, right, lmScale, syntaxEmissionScale);
  } else if (left->getInputAlphabet() ==
             right->lexicon()->lemmaPronunciationAlphabet()) {
    composeAutomaton = new LemmaPronunciationComposeAutomaton(
        left, right, lmScale, 0.0, syntaxEmissionScale);
  } else {
    defect();
  }
...
public:
  LeftComposeMapping(Core::Ref<const Lm::ComposeAutomaton> f) : fsa_(f) {
    require(fsa_);
  }
...
namespace Lm {
Core::Ref<const ComposeAutomaton> compose(Fsa::ConstAutomatonRef left,
...
Core::Ref<const ComposeAutomaton>
compose(Fsa::ConstAutomatonRef left, Core::Ref<const ScaledLanguageModel> right,
        Score syntaxEmissionScale) {
  return Core::ref(LmComposeInternal::ComposeAutomaton::createComposeAutomaton(
...
Core::Ref<const ComposeAutomaton>
composePron(Fsa::ConstAutomatonRef left,
            Core::Ref<const ScaledLanguageModel> right,
            Score pronunciationScale, Score syntaxEmissionScale) {
  require(left->getInputAlphabet() ==
          right->lexicon()->lemmaPronunciationAlphabet());
  return Core::ref(new LmComposeInternal::LemmaPronunciationComposeAutomaton(
      left, right->unscaled(), right->scale(), pronunciationScale,
      syntaxEmissionScale));
}
