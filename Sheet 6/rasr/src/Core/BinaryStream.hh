...
/**
 * @file
 * @todo CodeSmell: wrapper and file stream roles should be separated:
...
protected:
  ...
  BinaryStreamIos(const std::string &, std::ios_base::openmode,
...
public:
  ...
  explicit BinaryOutputStream(std::ostream &stream,
                              Endianess endianess = defaultEndianess)
...
template <class T>
BinaryOutputStream &operator<<(BinaryOutputStream &o,
                               const std::vector<T> &vec) {
  o << (u32)vec.size();
  for (typename std::vector<T>::const_iterator p = vec.begin(); p != vec.end();
       ++p)
    o << *p;
  return o;
}
...
BinaryOutputStream &operator<<(BinaryOutputStream &o,
  ...
  o << (u32)m.size();
  for (typename std::map<T1, T2>::const_iterator p = m.begin(); p != m.end();
       ++p)
    o << p->first << p->second;
  return o;
}
...
BinaryInputStream &operator>>(BinaryInputStream &is, std::vector<T> &vec) {
  ...
  is >> size;
  vec.resize(size);
  for (typename std::vector<T>::iterator iter = vec.begin(); iter != vec.end();
       ++iter) {
    is >> (*iter);
  }
  return is;
