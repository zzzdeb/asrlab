...
public:
  enum Type { typeGeneral, typeSymmetric, typeSymmetricPositiveDefinite };
...
public:
  ...
  /**
   *  Solves the symmetric eigenvalue problem and performs finalization of
   * results. If @return is false, depending on the type of error, results can
   * be stored in
   ...
   *  2) Verifies solution.
   */
  bool finalize(const Matrix<ValueType> &a, Vector<ValueType> &eigenvalues,
                Matrix<ValueType> &eigenvectors);
  /**
   ...
   */
  bool real(const Vector<std::complex<ValueType> > &, Vector<ValueType> &);
  /**
   *  Extracts real part of the input matrix.
   *  If one of the imaginary parts are larger than imaginaryMaximum_
   *  then @return is false.
   */
  bool real(const Matrix<std::complex<ValueType> > &, Matrix<ValueType> &);
};
...
public:
  ...
  virtual bool solveSymmetric(const Matrix<ValueType> &a,
                              ...
                              Matrix<ValueType> &eigenvectors) = 0;
  /**
   *  Solves the symmetric generalized eigenvalue problem and performs
   * finalization of results. If @return is false, depending on the type of
   * error, results can be stored in
   *  @c eigenvalues and @c eigenvectors
   */
  bool solveSymmetricAndFinalize(const Matrix<ValueType> &a,
                                 const Matrix<ValueType> &b,
   ...
   *  2) Verifies solution.
   */
  bool finalize(const Matrix<ValueType> &a, const Matrix<ValueType> &b,
                Vector<ValueType> &eigenvalues,
                Matrix<ValueType> &eigenvectors);
   ...
   *  Verifies if eigenvectors and eigenvalues satisfy A*X = Lambda*B*X
   */
  bool isCorrectSolution(const Matrix<ValueType> &a, const Matrix<ValueType> &b,
                         const Vector<ValueType> &eigenvalues,
                         const Matrix<ValueType> &eigenvectors);
};
} // namespace Math
