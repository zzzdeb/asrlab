...
namespace Ftl {
template <class _Automaton> class Resources {
...
public:
  ...
  typedef bool (*Writer)(const Self &, _ConstAutomatonRef, std::ostream &,
                         Fsa::StoredComponents, bool progress);
  ...
  struct Format {
    ...
    Format(const std::string &name, const std::string &desc, Reader reader,
           Writer writer)
        : name(name), desc(desc), reader(reader), writer(writer) {}
  };

private:
  typedef Core::hash_map<std::string, _ConstSemiringRef, Core::StringHash>
      SemiringMap;
...
public:
  ...
  Resources(const Core::Configuration &config) : config_(&config) {
    defaultFormat_ = 0;
  }

  const Core::Configuration &getConfiguration() const { return *config_; }

  bool registerSemiring(const std::string &name, _ConstSemiringRef semiring,
  ...
  _ConstSemiringRef getDefaultSemiring() const { return defaultSemiring_; }
  /**
   * \note for backward compatibility only
   */
  bool registerSemiring(Fsa::SemiringType type, _ConstSemiringRef semiring,
                        bool isDefault = false) {
  ...
  bool registerFormat(const std::string &name, Format *format,
                      bool isDefault = false) {
    if (isDefault)
