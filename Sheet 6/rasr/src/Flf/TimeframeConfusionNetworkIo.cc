...
private:
  PosteriorCnSortingIterator(ConstPosteriorCnRef cn) : cn_(cn), slot_(0) {
    verify(cn);
  }
...
public:
  const PosteriorCn::Slot &operator*() const { return slot_; }
...
/**
  ...
  ...
**/
void writePosteriorCnAsText(std::ostream &os, ConstPosteriorCnRef cn,
  ...
  u32 i = 0;
  for (PosteriorCnSortingIterator
           itSlot = PosteriorCnSortingIterator::begin(cn),
           endSlot = PosteriorCnSortingIterator::end(cn);
       itSlot != endSlot; ++itSlot, ++i) {
    os << "# slot " << (i + 1) << std::endl;
    const PosteriorCn::Slot &slot = *itSlot;
    for (PosteriorCn::Slot::const_iterator itArc = slot.begin(),
                                           endArc = slot.end();
         itArc != endArc; ++itArc)
      os << ((itArc->label == Fsa::Epsilon) ? "@"
                                            : alphabet->symbol(itArc->label))
...
private:
  ...
  void endSlot() { std::sort(cn_->back().begin(), cn_->back().end()); }
...
public:
  PosteriorCnXmlParser(const Core::Configuration &config)
        ...
        XML_CHILD(new Core::XmlMixedElementRelay(
            ...
            XML_CHILD(new Core::XmlMixedElementRelay(
                ...
                XML_CHILD(new Core::XmlMixedElementRelay(
                    ...
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "probability", this, 0, endHandler(&Self::endProbability),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_NO_MORE_CHILDREN)),
            XML_NO_MORE_CHILDREN)),
    ...
    Core::XmlRegularElementRelay *rootElement =
        new Core::XmlRegularElementRelay("posterior-cn", this,
...
ConstPosteriorCnRef readPosteriorCnFromXml(std::istream &is,
                                           PosteriorCnRef cn) {
  if (!parser_)
    parser_ = new PosteriorCnXmlParser(Core::Configuration(
        Core::Application::us()->getConfiguration(), "posterior-cn-parser"));
  if (!cn) {
...
// -------------------------------------------------------------------------
void writePosteriorCnAsXml(Core::XmlWriter &xml, ConstPosteriorCnRef cn,
...
// -------------------------------------------------------------------------
void writePosteriorCnAsFlowAlignment(Flow::Cache &cache, ConstPosteriorCnRef cn,
                                     ConstSegmentRef segment) {
  ...
  Core::Ref<const Flow::Attributes> flowAttrRef =
      Core::Ref<const Flow::Attributes>(flowAttr);
  flowAttr->set("datatype", "flow-alignment");
  Flow::DataAdaptor<Speech::Alignment> *flowAlignment =
      new Flow::DataAdaptor<Speech::Alignment>();
  Flow::DataPtr<Flow::Data> flowAlignmentRef =
      Flow::DataPtr<Flow::Data>(flowAlignment);
  ...
  for (PosteriorCnSortingIterator
    ...
    for (PosteriorCn::Slot::const_iterator itArc = slot.begin(),
                                           endArc = slot.end();
         itArc != endArc; ++itArc)
      if ((Fsa::FirstLabelId <= itArc->label) &&
          (itArc->label <= Fsa::LastLabelId))
        alignment.push_back(
            Speech::AlignmentItem(t, itArc->label, itArc->score));
  }
  alignment.sortItems();
...
public:
  DumpPosteriorCnNode(const std::string &name,
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    format_ = IoFormat::PosteriorCnFormat(IoFormat::paramPosteriorCnFormat(
        config, IoFormat::PosteriorCnFormatXml));
    hasCn_ = false;
