...
public:
  ...
  HistoryHandle handle() const { return desc_; }
  ...
  bool isValid() const { return mang_ != &sentinel; }
  ...
  HistoryHash hashKey() const { return mang_->hashKey(desc_); }

  struct Hash {
    HistoryHash operator()(const Lm::History &h) const { return h.hashKey(); }
  };
   ...
   */
  std::string format() const { return mang_->format(desc_); }
};
...
protected:
  ...
  void setTokenInventoryAndAlphabet(const TokenInventory *_tokenInventory,
                                    Fsa::ConstAlphabetRef _tokenAlphabet);

  Token getToken(const std::string &word) const {
    require_(tokenInventory_);
    return (*tokenInventory_)[word];
  }

  virtual Token getSpecialToken(const std::string &name,
  ...
  */
  const Bliss::SyntacticToken *getSpecialSyntacticToken(const std::string &name,
                                                        bool required) const;
   ...
   * must be issued.)
   */
  Token fallbackToken() const { return fallbackToken_; }

public:
  Token sentenceBeginToken() const { return sentenceBeginToken_; }
  Token sentenceEndToken() const { return sentenceEndToken_; }
...
protected:
  ...
  */
   ...
   * the lexicon.
   */
  void requireSentenceBoundaryToken() { requiresSentenceBoundaryToken_ = true; }

  LanguageModel(const Core::Configuration &c, Bliss::LexiconRef l);
...
public:
   ...
   */
  virtual CompiledBatchRequest *compileBatchRequest(const BatchRequest &,
...
inline void extendHistoryByLemmaPronunciation(
    ...
    const Bliss::LemmaPronunciation *pronunciation, History &history) {
