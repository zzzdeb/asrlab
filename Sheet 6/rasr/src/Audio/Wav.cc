...
sf_count_t &sfSamples(SF_INFO &info) { return info.frames; }
...
inline void sfSetWavFormat(SF_INFO &info) { info.format = SF_FORMAT_WAV; }
#else
// old version of libsndfile

typedef void SndFileHandle;
inline u32 sfGetSampleSize(const SF_INFO &info) { return info.pcmbitwidth; }
inline void sfSetSampleSize(SF_INFO &info, u32 s) { info.pcmbitwidth = s; }
u32 &sfSamples(SF_INFO &info) { return info.samples; }
...
bool WavInputNode::openFile_() {
  ...
  if (sf_ == 0) {
    criticalError("could not open wav file '%s' for reading",
                  filename_.c_str());
    return false;
  }
...
u32 WavInputNode::readTyped(u32 nSamples, Flow::Timestamp *&d) {
  ...
  int bytesRead = sf_read_raw(static_cast<SndFileHandle *>(sf_), &*(v->begin()),
                              bytesToRead);
...
template <typename T> bool WavOutputNode::writeTyped(const Flow::Data *_in) {
  const Flow::Vector<T> *in(dynamic_cast<const Flow::Vector<T> *>(_in));
  size_t bytesToWrite = sizeof(T) * in->size();
  size_t bytesWritten = sf_write_raw(static_cast<SndFileHandle *>(sf_),
                                     (void *)&*(in->begin()), bytesToWrite);
