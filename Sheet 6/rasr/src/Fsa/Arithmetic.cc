...
struct Multiplier {
  ...
  Weight operator()(const Weight &w) const { return Weight(f32(f32(w) * c)); }
  std::string describe() const { return Core::form("multiply(%f)", c); }
};
ConstAutomatonRef multiply(ConstAutomatonRef f, Weight value) {
  if ((f->semiring() != LogSemiring) && (f->semiring() != TropicalSemiring)) {
    std::cerr << "input to multiply must be log or tropical semiring"
              << std::endl;
...
struct ApplyExp {
  ...
  std::string describe() const { return Core::form("expm"); }
};
ConstAutomatonRef expm(ConstAutomatonRef f) {
  if ((f->semiring() != LogSemiring) && (f->semiring() != TropicalSemiring)) {
    std::cerr << "input to exponentiate must be log or tropical semiring"
              << std::endl;
    return ConstAutomatonRef();
  }
...
struct ApplyLog {
  ...
  std::string describe() const { return Core::form("logm"); }
};
...
public:
  ...
  virtual std::string describe() const {
    return Core::form("extend-by-fsa(%s,%s)", fsa_->describe().c_str(),
                      fsa2_->describe().c_str());
  }
};
...
struct GreaterEqual {
  ...
  Weight operator()(const Weight &w) const { return Weight(f32(f32(w) >= t)); }
  std::string describe() const { return Core::form("greater-equal(%f)", t); }
};
ConstAutomatonRef isGreaterEqual(ConstAutomatonRef f, Weight threshold) {
  if ((f->semiring() != LogSemiring) && (f->semiring() != TropicalSemiring)) {
    std::cerr << "input for greater_equal must be log or tropical semiring"
              << std::endl;
    return ConstAutomatonRef();
  }
  return Ftl::modifyWeight<Automaton, GreaterEqual>(
      f, GreaterEqual(f32(threshold)));
}
