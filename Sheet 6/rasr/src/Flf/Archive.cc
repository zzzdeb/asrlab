...
// -------------------------------------------------------------------------
const Core::ParameterBool paramInfo("info", "info", false);

const Core::ParameterString Archive::paramPath("path", "path to lattices");
const Core::ParameterString Archive::paramFile("file", "file", "");
const Core::ParameterString Archive::paramSuffix("suffix", "suffix", "");

Archive::Archive(const Core::Configuration &config, const std::string &pathname)
    : Core::Component(config), archive(0), isClosed_(false),
      pathname_(pathname) {
...
const std::string &Archive::path() const { return pathname_; }
...
LatticeArchive::getReader(const Core::Configuration &config) {
  ...
  case LatticeFormatFlf:
    archiveReader =
        new FlfArchiveReader(Core::Configuration(config, "flf"), pathname);
    break;
  case LatticeFormatHtkSlf:
    archiveReader =
        new HtkSlfArchiveReader(Core::Configuration(config, "htk"), pathname);
    break;
  default:
    defect();
  }
...
LatticeArchive::getWriter(const Core::Configuration &config) {
  std::string pathname = paramPath(config);
  if (pathname.empty())
    return 0;
  ...
  case LatticeFormatFlf:
    archiveWriter =
        new FlfArchiveWriter(Core::Configuration(config, "flf"), pathname);
    break;
  case LatticeFormatHtkSlf:
    archiveWriter =
        new HtkSlfArchiveWriter(Core::Configuration(config, "htk"), pathname);
    break;
  case LatticeFormatLatticeProcessor:
    archiveWriter = new LatticeProcessorArchiveWriter(
        Core::Configuration(config, "lattice-processor"), pathname);
    break;
  default:
    defect();
  }
...
// -------------------------------------------------------------------------
const Core::ParameterString LatticeArchiveReader::paramSemiring("semiring-type",
                                                                "semiring type",
                                                                "tropical");
const Core::ParameterString LatticeArchiveReader::paramKeys("keys", "keys", "");
const Core::ParameterFloat
    LatticeArchiveReader::paramScale("scale", "scale",
...
public:
  LatticeArchiveReaderNode(const std::string &name,
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!reader_ || reader_->hasFatalErrors())
      criticalError("Failed to open archive \"%s\"  for reading.",
                    readerPath.c_str());
    else
...
NodeRef createLatticeArchiveReaderNode(const std::string &name,
                                       const Core::Configuration &config) {
...
// -------------------------------------------------------------------------
LatticeArchiveWriter::LatticeArchiveWriter(const Core::Configuration &config,
                                           const std::string &pathname)
    : Precursor(config, pathname) {}
...
public:
  LatticeArchiveWriterNode(const std::string &name,
                           const Core::Configuration &config)
...
NodeRef createLatticeArchiveWriterNode(const std::string &name,
                                       const Core::Configuration &config) {
...
*/
const Core::ParameterFloat
    LatticeProcessorArchiveWriter::paramPronunciationScale(
        "pronunciation-scale", "pronunciation scale", Semiring::UndefinedScale);

LatticeProcessorArchiveWriter::LatticeProcessorArchiveWriter(
    const Core::Configuration &config, const std::string &pathname)
    : Precursor(config, pathname) {
  pronunciationScale_ =
      paramPronunciationScale(config, Semiring::UndefinedScale);
  ...
  Core::Ref<Fsa::StorageAutomaton> f =
      Core::ref(new Fsa::StaticAutomaton(Fsa::TypeAcceptor));
...
void LatticeProcessorArchiveWriter::store(const std::string &id,
                                          ConstLatticeRef l) {
  ...
  l = extendByPronunciationScore(l, Lexicon::us()->lemmaPronunciationAlphabet(),
                                 lmId, pronunciationScale_,
...
ConfusionNetworkArchive::getReader(const Core::Configuration &config,
                                   CnFormat format) {
  std::string pathname = paramPath(config);
  if (pathname.empty())
    return 0;
...
ConfusionNetworkArchive::getWriter(const Core::Configuration &config,
                                   CnFormat format) {
  std::string pathname = paramPath(config);
  if (pathname.empty())
    return 0;
...
ConfusionNetworkArchiveReader::ConfusionNetworkArchiveReader(
  ...
  if (format != CnFormatXml)
    criticalError("Format \"%s\" is not supported yet.",
                  choiceCnFormat[format].c_str());
  archive =
      Core::Archive::create(config, pathname, Core::Archive::AccessModeRead);
  if (!archive)
    criticalError("Failed to open CN archive \"%s\" for reading",
                  pathname.c_str());
  getHeader_ = true;
}
ConfusionNetworkArchiveReader::~ConfusionNetworkArchiveReader() {}
...
ConfusionNetworkArchiveReader::get(const std::string &id) {
...
void ConfusionNetworkArchiveWriter::store(const std::string &id,
...
public:
  ConfusionNetworkArchiveReaderNode(const std::string &name,
                                    const Core::Configuration &config)
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(1) && !connected(2))
      criticalError("Data source either at port 1 or port 2 required.");
    IoFormat::CnFormat format =
        IoFormat::CnFormat(IoFormat::paramCnFormat(config));
    std::string readerPath = Archive::paramPath(config);
...
NodeRef
createConfusionNetworkArchiveReaderNode(const std::string &name,
                                        const Core::Configuration &config) {
...
public:
  ConfusionNetworkArchiveWriterNode(const std::string &name,
                                    const Core::Configuration &config)
...
NodeRef
createConfusionNetworkArchiveWriterNode(const std::string &name,
                                        const Core::Configuration &config) {
 ...
 **/
// -------------------------------------------------------------------------
PosteriorCnArchive::PosteriorCnArchive(const Core::Configuration &config,
                                       const std::string &pathname,
...
PosteriorCnArchive::getReader(const Core::Configuration &config,
                              PosteriorCnFormat format) {
  std::string pathname = paramPath(config);
  if (pathname.empty())
    return 0;
...
PosteriorCnArchive::getWriter(const Core::Configuration &config,
                              PosteriorCnFormat format) {
  std::string pathname = paramPath(config);
  if (pathname.empty())
    return 0;
...
PosteriorCnArchiveReader::PosteriorCnArchiveReader(
  ...
  if (format != PosteriorCnFormatXml)
    criticalError("Format \"%s\" is not supported yet.",
                  choicePosteriorCnFormat[format].c_str());
  archive =
      Core::Archive::create(config, pathname, Core::Archive::AccessModeRead);
  if (!archive)
    criticalError("Failed to open fCN archive \"%s\" for reading",
                  pathname.c_str());
  getHeader_ = true;
}
PosteriorCnArchiveReader::~PosteriorCnArchiveReader() {}
...
PosteriorCnArchiveWriter::PosteriorCnArchiveWriter(
  ...
  archive =
      Core::Archive::create(config, pathname, Core::Archive::AccessModeWrite);
  if (!archive)
    criticalError("Failed to open posterior CN archive \"%s\" for writing",
                  pathname.c_str());
  margin = paramMargin(config);
  indentation = paramIndentation(config);
  storeHeader_ = true;
...
void PosteriorCnArchiveWriter::store(const std::string &id,
...
public:
  PosteriorCnArchiveReaderNode(const std::string &name,
                               const Core::Configuration &config)
      : Precursor(name, config), archiveReader_(0) {
    isValid_ = false;
  }
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(1) && !connected(2))
      criticalError("Data source either at port 1 or port 2 required.");
    IoFormat::PosteriorCnFormat format =
        IoFormat::PosteriorCnFormat(IoFormat::paramPosteriorCnFormat(config));
...
NodeRef createPosteriorCnArchiveReaderNode(const std::string &name,
                                           const Core::Configuration &config) {
...
public:
  PosteriorCnArchiveWriterNode(const std::string &name,
                               const Core::Configuration &config)
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    case IoFormat::PosteriorCnFormatFlowAlignment: {
      ...
      if (!flowCache_->isOpen())
        criticalError("Failed to open flow archive \"%s\".",
                      flowCache_->path().c_str());
    } break;
    default:
      defect();
    }
...
NodeRef createPosteriorCnArchiveWriterNode(const std::string &name,
                                           const Core::Configuration &config) {
