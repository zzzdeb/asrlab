...
bool SSE2L2NormCodeGenerator::appendDuplicate(int block) {
  ...
  */
  static const u8 code[2][4] = {{0x66, 0x0f, 0x6f, 0xd0},
                                {0x66, 0x0f, 0x6f, 0xf4}};
...
bool SSE2L2NormCodeGenerator::appendSubtract(int block) {
  /*  block = 0
      psubusb xmm0,xmm1        ; xmm0 = x - u (SIMD subtraction of packed
     unsigned integers) psubusb xmm1,xmm2        ; xmm1 = u - x
  ...
  */
  static const u8 code[2][8] = {
      {0x66, 0x0f, 0xd8, 0xc1, 0x66, 0x0f, 0xd8, 0xca},
      {0x66, 0x0f, 0xd8, 0xe5, 0x66, 0x0f, 0xd8, 0xee}};
  return append(code[block], sizeof(code[block]));
}
...
bool SSE2L2NormCodeGenerator::appendPrepareForUnpack(int block) {
  ...
  */
  ...
  static const u8 code[2][12] = {
      {0x66, 0x0f, 0xeb, 0xc8, 0x66, 0x0f, 0x6f, 0xd1, 0x66, 0x0f, 0xef, 0xc0},
      {0x66, 0x0f, 0xeb, 0xec, 0x66, 0x0f, 0x6f, 0xf5, 0x66, 0x0f, 0xef, 0xe4}};
  return append(code[block], sizeof(code[block]));
}
...
bool SSE2L2NormCodeGenerator::appendUnpackAndAdd(int block) {
  /*  block = 0
      ...
      punpckhbw xmm1,xmm0      ; unpack and interleave high order bytes from
     xmm1 with 0 punpcklbw xmm2,xmm0      ; unpack and interleave low  order
     bytes from xmm2 with 0
                            ;
  ...
  */
  static const u8 code[2][16] = {
      {0x66, 0x0f, 0x68, 0xc8, 0x66, 0x0f, 0x60, 0xd0, 0x66, 0x0f, 0xf5, 0xc9,
       0x66, 0x0f, 0xf5, 0xd2},
      {0x66, 0x0f, 0x68, 0xec, 0x66, 0x0f, 0x60, 0xf4, 0x66, 0x0f, 0xf5, 0xed,
       0x66, 0x0f, 0xf5, 0xf6}};
  return append(code[block], sizeof(code[block]));
}
...
bool SSE2L2NormCodeGenerator::appendAccumulate(int block) {
  ...
  */
  static const u8 code[2][8] = {
      {0x66, 0x0f, 0xfe, 0xf9, 0x66, 0x0f, 0xfe, 0xfa},
      {0x66, 0x0f, 0xfe, 0xfd, 0x66, 0x0f, 0xfe, 0xfe}};
  return append(code[block], sizeof(code[block]));
}

bool SSE2L2NormCodeGenerator::appendEpilog() {
  /*	; calculate results as sum overt the 4 double words in xmm7 (s3 s2 s1
     s0) pshufd  xmm1,xmm7,238    ; shuffle xmm7 in xmm1 : s2 s3 s2 s3 pshufd
     xmm2,xmm7,68     ; shuffle xmm7 in xmm2 : s0 s1 s0 s1 paddd   xmm1,xmm2 ;
     add double words: xmm1 (s2 + s0), (s3 + s1), (s2 + s0), (s3 + s1) pshufd
     xmm2,xmm1,177    ; shuffle xmm1 in xmm2 : (s3 + s1), (s2 + s0), (s3 + s1),
     (s2 + s0) paddd   xmm1,xmm2        ; add double words: xmm1 (s1 + s2 +s3 +
     s4) x 4 ; create return value movd    eax,xmm1         ; copy lowest double
     word of xmm1 to eax = result
...
SSE2L2NormCodeGenerator::SSE2L2NormCodeGenerator(const Core::Configuration &c,
                                                 size_t d)
    : CodeGenerator(c) {
