...
/** Derived Mel warping function core: d f_{Mel} / df = 1.0 / log(10) / (700.0 +
 * f) */
 ...
 *  E(f) *= 9.58e26 (last pole's omega) to get values in order of magnitude of 1
 * at 4000 Hz.
 */
 ...
 *  E(f) = (omega^4 * (omega^2 + 56.8e6)) / ((omega^2 + 6.3e6))^2 * (omega^2 +
 * 0.38e9))
 */
 ...
 * T * x))), where 'a' controls curvature, and 'T' is the periodicity of the
 * arctan term. Hint: pont T/2 is mapped on itself.
 */
...
public:
  ...
  virtual Result value(Argument x) const {
    return x - T_ / M_PI *
                   atan(a_ * sin(2 * M_PI / T_ * x) /
                        (1 + a_ * cos(2 * M_PI / T_ * x)));
  }
  virtual UnaryAnalyticFunctionRef invert() const {
