...
void ISmoothingCovarianceEstimator::setICovariance(
    const Covariance *covariance) {
  require(covariance);
  verify(iCovariance_.empty());
  iCovariance_.resize(covariance->dimension());
  std::copy(covariance->diagonal().begin(), covariance->diagonal().end(),
            iCovariance_.begin());
}
...
ISmoothingCovarianceEstimator::iSum(
  ...
  if (!iSum_) {
    ...
    std::transform(
        iSum_->begin(), iSum_->end(), iSum_->begin(),
    ...
    for (; m != meanSet.end(); ++m) {
      ...
      std::transform(iSum_->begin(), iSum_->end(),
                     meanEstimator->iMean().begin(), iSum_->begin(),
...
Sum ISmoothingCovarianceEstimator::getObjectiveFunction(
  ...
  for (ComponentIndex i = 0; i < parent_->sum().size(); ++i) {
    ...
    CovarianceToMeanSetMap::MeanSet::const_iterator m = meanSet.begin();
    for (; m != meanSet.end(); ++m) {
      const MeanType prevMean =
          required_cast(const DiscriminativeMeanEstimator *, m->get())
              ->previousMean(i);
