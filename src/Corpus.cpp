...
void CorpusDescription::read(std::shared_ptr<const Lexicon> lexicon) {
  ...
  std::vector<Configuration> segment_configs = corpus_config.get_array<Configuration>("segments");
...
Segment CorpusDescription::read_segment(Configuration const& config, std::shared_ptr<const Lexicon> lexicon) {
  Segment result;
...
void Corpus::read(CorpusDescription const& corpus_description, std::string const& feature_path, SignalAnalysis& analyzer) {
  ...
  frame_duration_         = 1000 * static_cast<double>(analyzer.window_shift) / analyzer.sample_rate;
...
size_t Corpus::get_total_frame_count() const {
  return feature_offsets_.back() / features_per_timeframe_;
}
