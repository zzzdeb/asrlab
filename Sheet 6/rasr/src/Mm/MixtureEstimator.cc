 ...
 */
void AbstractMixtureEstimator::addDensity(
    Core::Ref<GaussDensityEstimator> densityEstimator) {
  require(std::find_if(
              densityEstimators_.begin(), densityEstimators_.end(),
...
void AbstractMixtureEstimator::removeDensitiesWithZeroWeight() {
  ...
  for (DensityIndex dns = 0; dns < densityEstimators_.size();) {
    if (densityEstimators_[dns]->mean()->weight() == 0 && dns != densityMax) {
      removeDensity(dns);
      if (densityMax > dns)
        --densityMax;
    } else {
...
void AbstractMixtureEstimator::removeDensitiesWithLowWeight(
  ...
  DensityIndex densityMax = densityIndexWithMaxWeight();
  Weight minWeight =
      std::max(minObservationWeight, getWeight() * minRelativeWeight);
  for (DensityIndex dns = 0; dns < densityEstimators_.size();) {
...
void AbstractMixtureEstimator::accumulate(DensityIndex indexInMixture,
...
void AbstractMixtureEstimator::accumulate(
  ...
  std::transform(weights_.begin(), weights_.end(), toAdd.weights_.begin(),
...
bool AbstractMixtureEstimator::equalTopology(
  ...
  if (nDensities() != toCompare.nDensities())
    return false;
  for (DensityIndex dns = 0; dns < densityEstimators_.size(); ++dns) {
    if (densityMap[densityEstimators_[dns]] !=
        densityMapToCompare[toCompare.densityEstimators_[dns]])
      return false;
  }
  return true;
}

void AbstractMixtureEstimator::addMixture(
    const AbstractMixtureEstimator &toAdd) {
  for (DensityIndex i = 0; i < toAdd.nDensities(); ++i) {
    Core::Ref<GaussDensityEstimator> densityEstimator =
        toAdd.densityEstimators()[i];
    require(std::find_if(
                densityEstimators_.begin(), densityEstimators_.end(),
                std::bind2nd(std::equal_to<Core::Ref<GaussDensityEstimator> >(),
                             densityEstimator)) == densityEstimators_.end());
...
DensityIndex
AbstractMixtureEstimator::accumulate(Core::BinaryInputStreams &is,
