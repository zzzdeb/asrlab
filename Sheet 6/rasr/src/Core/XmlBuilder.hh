...
public:
  ...
  template <class T> static Handler handler(void (T::*h)(HandlerArgument)) {
...
public:
  virtual void start(const XmlAttributes atts) { Precursor::start(atts); }
  ...
  XmlBuilderElementTemplate(const char *_name, XmlContext *_context,
                            Handler _handler = 0)
 ...
 */
template <typename P, class CM, class CP> class XmlBuilderElement;
...
public:
  ...
  XmlBuilderElement(const char *_name, XmlContext *_context,
                    Handler _handler = 0)
...
class XmlBuilderElement<P, CM, CreateUsingNew>
    : public XmlBuilderElementTemplate<CM, P, std::auto_ptr<P>,
                                       std::auto_ptr<P> &> {
...
public:
  ...
  typedef CreationHandlerResult (XmlContext::*CreationHandler)(
      const XmlAttributes atts);
  template <class T>
  static CreationHandler
...
{
    typedef XmlBuilderElement<std::string, XmlEmptyElement, CreateStatic>
Precursor; public:
...
template <typename P> struct StringConversion {
  bool operator()(const std::string &in, P &out) const {
    return strconv(in, out);
  }
};

template <> struct StringConversion<std::string> {
...
protected:
  ...
  XmlDataOnlyBuilderElement(const char *_name, XmlContext *_context,
                            Handler _handler, Conversion _converter)
      : Precursor(_name, _context, _handler), converter_(_converter) {}

public:
  virtual void start(const XmlAttributes atts) {
    XmlBuilderElementTemplate<XmlEmptyElement, P, P,
                              const P &>::Precursor::start(atts);
    str_ = std::string();
  }
  ...
  XmlDataOnlyBuilderElement(const char *_name, XmlContext *_context,
                            typename Precursor::Handler _handler = 0)
      : Precursor(_name, _context, _handler) {}
};

template <typename P> struct ChoiceConversion {
  const Choice choice;
  ...
  ChoiceConversion(const Choice &choice) : choice(choice) {}
};
...
public:
  XmlChoiceBuilderElement(const char *_name, XmlContext *_context,
                          const Choice &_choice, Handler _handler = 0)
      : Precursor(_name, _context, _handler, ChoiceConversion<P>(_choice)) {}

  const Choice &choice() { return Precursor::converter_.choice; }
};
