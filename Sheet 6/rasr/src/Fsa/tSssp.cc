...
public:
  SsspQueue() : head_(Fsa::InvalidStateId), tail_(Fsa::InvalidStateId), n_(0) {}
  ...
  Fsa::StateId size() const { return n_; }
...
public:
  FifoSsspQueue(Fsa::StateId maxStateId) : SsspQueue(maxStateId) {}
...
public:
  ...
  std::string name() const { return "topo"; }
  void enqueue(Fsa::StateId s) {
    if (next_[s] == Fsa::InvalidStateId) {
...
StatePotentials<typename _Automaton::Weight>
ssspLoop(Queue &q, typename _Automaton::ConstRef f, Fsa::StateId start,
  ...
  // d[q] denotes the tentative shortest distance from the source s to q (cite
  // from [Mohri])
  _StatePotentials d(maxStateId + 1, semiring->zero());
  // r[q] keeps track of the sum of the weights +-added to d[q] since the last
  // queue extraction of q (cite from [Mohri])
  ...
  // although in the original paper by Mohri and Riley, uncommenting the
  // following gives wrong initial and final weights when pushing: d[start] =
  // f->semiring()->one();
  return d;
};
...
StatePotentials<typename _Automaton::Weight>
sssp(typename _Automaton::ConstRef f, Fsa::StateId start,
     const SsspArcFilter<_Automaton> &arcFilter, bool progress) {
  if (hasProperties<_Automaton>(f, Fsa::PropertyAcyclic)) {
    ...
    if (q.maxStateId() != Fsa::InvalidStateId)
      return ssspLoop<_Automaton, TopologicalSsspQueue>(q, f, start, arcFilter,
                                                        progress);
  }
  // in case of a wrongly tagged automaton we gently fall back to a standard
  // filo queue
  Fsa::AutomatonCounts counts = count<_Automaton>(f, progress);
...
template <class _Automaton>
StatePotentials<typename _Automaton::Weight>
sssp(typename _Automaton::ConstRef f, bool progress) {
  return sssp<_Automaton>(f, f->initialStateId(), SsspArcFilter<_Automaton>(),
                          progress);
}

template <class _Automaton>
StatePotentials<typename _Automaton::Weight>
ssspBackward(typename _Automaton::ConstRef f,
             const SsspArcFilter<_Automaton> &arcFilter, bool progress) {
  typedef typename _Automaton::Weight _Weight;
  typedef typename _Automaton::State _State;
  typedef typename _Automaton::ConstStateRef _ConstStateRef;
  ...
  _StatePotentials d(maxStateId + 1, semiring->zero());
  _StatePotentials r(maxStateId + 1, semiring->zero());
  std::vector<bool> changed(maxStateId + 1, false),
      oldChanged(maxStateId + 1, false);
  ...
  while (potentialsHaveChanged) {
    ...
    for (Fsa::StateId s = 0; s <= maxStateId; ++s) {
      ...
      if (sp) {
        ...
        for (typename _State::const_iterator a = sp->begin(); a != sp->end();
             ++a)
        ...
        if (targetHasChanged) {
          ...
          for (typename _State::const_iterator a = sp->begin(); a != sp->end();
            ...
            if (arcFilter(*a)) {
              D = semiring->collect(
                  D, semiring->extend(a->weight(), r[a->target()]));
              R = semiring->collect(
                  R, semiring->extend(a->weight(), r[a->target()]));
            }
...
template <class _Automaton>
StatePotentials<typename _Automaton::Weight>
ssspBackward(typename _Automaton::ConstRef f, bool progress) {
  return ssspBackward<_Automaton>(f, SsspArcFilter<_Automaton>(), progress);
}

// together with the final weight correction above we also changed the
// calculation of the final weights in the two automata below. the uncommented
// code is the original one from the literature. residual initial weights are
// ignored.
template <class _Automaton>
...
public:
  PushToFinalAutomaton(_ConstAutomatonRef f, bool progress)
      : Precursor(f), semiring_(f->semiring()),
  ...
  virtual void modifyState(_State *sp) const {
    ...
    for (typename _State::iterator a = sp->begin(); a != sp->end(); ++a)
      a->weight_ =
          semiring_->extend(semiring_->extend(potentials_[s], a->weight()),
                            semiring_->invert(potentials_[a->target()]));
    if (s == initialStateId_)
      sp->weight_ = semiring_->one(); // extend(sp->weight_, potentials_[s]);
    if (sp->isFinal())
      sp->weight_ =
          semiring_->extend(semiring_->extend(potentials_[s], sp->weight_),
                            semiring_->invert(potentials_[initialStateId_]));
    // if (sp->isFinal()) sp->weight_ = semiring()->extend(sp->weight_,
    // semiring()->invert(potentials_[s]));
  }
...
template <class _Automaton>
typename _Automaton::ConstRef pushToFinal(typename _Automaton::ConstRef f,
                                          bool progress) {
  return typename _Automaton::ConstRef(
      new PushToFinalAutomaton<_Automaton>(f, progress));
}
...
public:
  ...
  PushToInitialAutomaton(
      _ConstAutomatonRef f,
      const StatePotentials<typename _Automaton::Weight> &potentials,
      bool progress)
      : Precursor(f), semiring_(f->semiring()),
        initialStateId_(f->initialStateId()), potentials_(potentials) {}
  virtual void modifyState(_State *sp) const {
    Fsa::StateId s = sp->id();
    for (typename _State::iterator a = sp->begin(); a != sp->end(); ++a) {
      a->weight_ = semiring_->extend(
          semiring_->invert(potentials_[s]),
          semiring_->extend(a->weight(), potentials_[a->target()]));
    }
    if (s == initialStateId_)
      sp->weight_ = semiring_->one(); // extend(sp->weight_, potentials_[s]);
    if (sp->isFinal())
      sp->weight_ = semiring_->extend(
          semiring_->extend(sp->weight_, potentials_[initialStateId_]),
          semiring_->invert(potentials_[s]));
    // if (sp->isFinal()) sp->weight_ =
    // semiring()->extend(semiring()->invert(potentials_[s]), sp->weight_);
  }
  virtual std::string describe() const {
...
template <class _Automaton>
typename _Automaton::ConstRef pushToInitial(typename _Automaton::ConstRef f,
                                            bool progress) {
  return typename _Automaton::ConstRef(
      new PushToInitialAutomaton<_Automaton>(f, progress));
}
...
public:
  PosteriorAutomaton(_ConstAutomatonRef f) : Precursor(f) {
    forwardPotentials_ = sssp<_Automaton>(f);
    backwardPotentials_ = sssp<_Automaton>(transpose<_Automaton>(f));
    totalInv_ =
        this->semiring()->invert(backwardPotentials_[f->initialStateId()]);
  }
  ...
  virtual void modifyState(_State *sp) const {
    Fsa::StateId s = sp->id();
    for (typename _State::iterator a = sp->begin(); a != sp->end(); ++a) {
      a->weight_ = this->semiring()->extend(a->weight(),
                                            backwardPotentials_[a->target()]);
      a->weight_ = this->semiring()->extend(forwardPotentials_[s], a->weight());
      a->weight_ = this->semiring()->extend(totalInv_, a->weight());
    }
  }
  const _Weight &totalInv() const { return totalInv_; }
  virtual std::string describe() const {
...
posterior(typename _Automaton::ConstRef f,
          const StatePotentials<typename _Automaton::Weight> &forward) {
  return typename _Automaton::ConstRef(
      new PosteriorAutomaton<_Automaton>(f, forward));
}
...
private:
  Count infinity() const { return Core::Type<Count>::max; }

public:
  virtual std::string name() const { return "count-paths"; }
  virtual Count create() const { return 0; }
  virtual Count clone(const Count &a) const { return a; }
  virtual Count invalid() const { return Count(Core::Type<Count>::min); }
  virtual Count zero() const { return 0; }
  virtual Count one() const { return 1; }
  virtual Count max() const { return infinity(); }
  ...
  virtual Count invert(const Count &a) const { return -a; }
  virtual int compare(const Count &a, const Count &b) const {
    return a < b ? -1 : (a > b ? 1 : 0);
  }
  virtual size_t hash(const Count &a) const { return size_t(a); }
  virtual bool isDefault(const Count &a) const { return a == defaultWeight(); }
...
public:
  CountPathsAutomaton(typename _SrcAutomaton::ConstRef f) : Precursor(f) {
    if (!(semiring_))
      semiring_ = _ConstSemiringRef(new CountPathsSemiring_());
    superFinalState_ =
        _ConstStateRef(new _State(0, Fsa::StateTagFinal, semiring()->one()));
  }
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    sp->setId(s);
    for (typename _SrcAutomaton::State::const_iterator a = fromSp->begin();
         a != fromSp->end(); ++a)
...
template <class _SrcAutomaton>
size_t countPaths(typename _SrcAutomaton::ConstRef f) {
