...
public:
  ToWordBoundariesDfs(Fsa::ConstAutomatonRef f,
                      Flf::ConstBoundariesRef boundaries)
      : Fsa::DfsState(f), boundaries_(boundaries),
  ...
  virtual void discoverState(Fsa::ConstStateRef sp) {
    ...
    Lattice::WordBoundary wordBoundary(
        boundaries_->time(s), std::make_pair(transit.final, transit.initial));
...
public:
  FromWordBoundariesDfs(Lattice::ConstWordLatticeRef lattice)
      : Fsa::DfsState(lattice->part(0)), boundaries_(new Flf::StaticBoundaries),
...
Flf::ConstLatticeRef fromWordLattice(Lattice::ConstWordLatticeRef lattice) {
  ...
  Flf::ConstLatticeRef l = Flf::fromFsaVector(
      fsas,
      Flf::Semiring::create(getSemiringType(fsas[0]->semiring()), // hack
                            fsas.size(), Flf::ScoreList(fsas.size(), 1.0),
                            keys));
...
Core::Choice ModelCombinationNode::choiceMode(
    "lexicon", ModelCombination::useLexicon, "lexicon+am",
    ModelCombination::useAcousticModel, "lexicon+lm",
    ModelCombination::useLanguageModel, "complete", ModelCombination::complete,
    Core::Choice::endMark());

Core::ParameterChoice
    ModelCombinationNode::paramMode("mode", &choiceMode, "parts to load",
                                    ModelCombination::useLexicon);

Core::Choice ModelCombinationNode::choiceAcousticModelMode(
    "complete", Am::AcousticModel::complete, "no-emissions",
    Am::AcousticModel::noEmissions, "no-state-tying",
    Am::AcousticModel::noStateTying, "no-state-transition",
    Am::AcousticModel::noStateTransition, "no-emissions+no-state-tying",
    Am::AcousticModel::noEmissions | Am::AcousticModel::noStateTying,
    "no-emissions+no-state-transition",
    Am::AcousticModel::noEmissions | Am::AcousticModel::noStateTransition,
    "no-state-tying+no-state-transition",
    Am::AcousticModel::noStateTying | Am::AcousticModel::noStateTransition,
    "no-emissions+no-state-tying+no-state-transition",
    Am::AcousticModel::noEmissions | Am::AcousticModel::noStateTying |
        Am::AcousticModel::noStateTransition,
    Core::Choice::endMark());

Core::ParameterChoice ModelCombinationNode::paramAcousticModelMode(
    "acoustic-model-mode", &choiceAcousticModelMode,
    "parts to load from acoustic model", Am::AcousticModel::complete);

ModelCombinationNode::ModelCombinationNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), mode_(paramMode(config)),
      acousticModelMode_(paramAcousticModelMode(config)), needInit_(true) {
  addInputs(0);
  addOutputs(1);
}
...
bool ModelCombinationNode::configure() {
  ...
  attributes->set("datatype",
                  Flow::DataAdaptor<ModelCombinationRef>::type()->name());
  return putOutputAttributes(0, attributes);
}

void ModelCombinationNode::initialize() {
  modelCombination_ = ModelCombinationRef(new ModelCombination(
      select("model-combination"), mode_, acousticModelMode_));
...
const Core::ParameterString
    LatticeIoNode::paramSegmentId("id", "segment identifier for caches.");
...
bool LatticeIoNode::setParameter(const std::string &name,
...
bool LatticeIoNode::configure() {
  ...
  attributes->set("datatype",
                  Flow::DataAdaptor<Flf::ConstLatticeRef>::type()->name());
  return putOutputAttributes(0, attributes);
}
...
const Core::ParameterStringVector LatticeReadNode::paramReaders(
    "readers", "set of lattice extractors, type=reader", ",");

LatticeReadNode::LatticeReadNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), latticeArchiveReader_(0) {
  addOutputs(1);
  readers_ = paramReaders(config);
}

LatticeReadNode::~LatticeReadNode() { delete latticeArchiveReader_; }
...
bool LatticeReadNode::work(Flow::PortId p) {
  ...
  Lattice::ConstWordLatticeRef lattice =
      latticeArchiveReader_->get(segmentId_, readers_);
  if (!lattice || !lattice->nParts()) {
    error("lattice could not be read");
    return putEos(0);
  }
  Flow::DataAdaptor<Flf::ConstLatticeRef> *out =
      new Flow::DataAdaptor<Flf::ConstLatticeRef>();
...
LatticeWriteNode::LatticeWriteNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), latticeArchiveWriter_(0) {
  addInputs(1);
}
LatticeWriteNode::~LatticeWriteNode() { delete latticeArchiveWriter_; }
...
bool LatticeWriteNode::configure() {
  ...
  if (!configureDatatype(attributes,
                         Flow::DataAdaptor<Flf::ConstLatticeRef>::type())) {
    return false;
  }
 ...
 */
const Core::ParameterFloatVector
    LatticeSimpleModifyNode::paramScales("scales", "set the semiring scales");
...
bool LatticeSimpleModifyNode::setParameter(const std::string &name,
                                           const std::string &value) {
...
bool LatticeSimpleModifyNode::work(Flow::PortId p) {
  ...
  while (getData(0, in)) {
    ...
    l = changeSemiring(l, Flf::Semiring::create(Fsa::SemiringTypeLog,
                                                scales_.size(), scales_,
                                                l->semiring()->keys()));
...
const Core::ParameterBool LatticeTransformNode::paramAppendScore(
    "append-score",
    "append posterior scores to lattice (increase the lattice layers)", false);

LatticeTransformNode::LatticeTransformNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), appendScore_(paramAppendScore(c)) {
  addInputs(1);
  addOutputs(1);
}
...
bool LatticeTransformNode::work(Flow::PortId p) {
  ...
  if (appendScore_) {
    ...
    l = Flf::fromFsa(transform(toFsa(l)),
                     Flf::Semiring::create(l->semiring()->type(), 1), 0);
  }
 ...
 */
LatticeTransform2Node::LatticeTransform2Node(const Core::Configuration &c)
    : Core::Component(c), Precursor(c) {
  addInputs(1);
}
...
bool LatticeTransform2Node::work(Flow::PortId p) {
  ...
  if (appendScore_) {
    ...
    l = Flf::fromFsa(
        transform(Flf::toFsa(in1->data()), Flf::toFsa(in2->data())),
        Flf::Semiring::create(l->semiring()->type(), 1), 0);
  }
...
LatticeWordPosteriorNode::transform(Fsa::ConstAutomatonRef fsa) const {
...
Fsa::ConstAutomatonRef
LatticeCopyNode::transform(Fsa::ConstAutomatonRef fsa) const {
...
Fsa::ConstAutomatonRef
LatticeCacheNode::transform(Fsa::ConstAutomatonRef fsa) const {
...
Fsa::ConstAutomatonRef
LatticeExpmNode::transform(Fsa::ConstAutomatonRef fsa) const {
  return Fsa::expm(fsa);
}
...
const Core::ParameterBool LatticeExpectationPosteriorNode::paramNormalize(
    "normalize", "posteriors are v-normalized, i.e., mean = 0", true);

LatticeExpectationPosteriorNode::LatticeExpectationPosteriorNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c), normalize_(paramNormalize(config)) {}

Fsa::ConstAutomatonRef
LatticeExpectationPosteriorNode::transform(Fsa::ConstAutomatonRef fsa1,
                                           Fsa::ConstAutomatonRef fsa2) const {
  Fsa::Weight dummy;
 ...
 */
const Core::ParameterString
    LatticeDumpCtmNode::paramSegmentId("id", "segment identifier.");

const Core::ParameterString
    LatticeDumpCtmNode::paramTrackId("track",
                                     "audio track or channel identifier.", "1");

LatticeDumpCtmNode::LatticeDumpCtmNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), ctmChannel_(config, "ctm"),
      segmentId_(paramSegmentId(config)), trackId_(paramTrackId(config)) {
  addInputs(2);
  addOutputs(1);
}

bool LatticeDumpCtmNode::setParameter(const std::string &name,
                                      const std::string &value) {
  if (paramSegmentId.match(name))
    segmentId_ = paramSegmentId(value);
...
bool LatticeDumpCtmNode::work(Flow::PortId p) {
  ...
  while (getData(0, in)) {
    ...
    for (Flf::ConstStateRef sr = l->getState(l->initialStateId());
         sr->hasArcs(); sr = l->getState(sr->begin()->target())) {
      ...
      if (ctmChannel_.isOpen() && a.input() != Fsa::Epsilon) {
        ctmChannel_ << segmentId_ << " " << trackId_ << " "
