...
CrossCorrelation::CrossCorrelation()
    : begin_(0), end_(0), similarityFunctionType_(Multiplication), power_(1.0),
      normalizationType_(None), useFastFourierTransform_(true) {}
...
void CrossCorrelation::crossCorrelation(const std::vector<Data> &x,
  ...
  if (&x == &y) { // autocorrelation
    Math::transformAlternatingComplexToAlternatingComplex(
        X_.begin(), X_.end(), X_.begin(), X_.begin(),
        Math::conjugateMultiplies<Data>());
  } else {
    fft.transform(Y_ = y);
    Math::transformAlternatingComplexToAlternatingComplex(
        X_.begin(), X_.end(), Y_.begin(), X_.begin(),
        Math::conjugateMultiplies<Data>());
  }
...
void CrossCorrelation::crossCorrelationWithMultiplication(
    const std::vector<Data> &x, const std::vector<Data> &y,
    std::vector<Data> &Rxy) {
...
void CrossCorrelation::crossCorrelationWithAbsoluteDifference(
    const std::vector<Data> &x, const std::vector<Data> &y,
...
void CrossCorrelation::normalizeUpperBound(const std::vector<Data> &x,
  ...
  if (similarityFunctionType_ == Multiplication)
    ...
    if (power_ == 1.0) {
      normalize(x, y, Rxy,
                normalizeCrossAbsoluteDifferenceUpperBound<Data>(x, y));
    } else if (power_ == 2.0) {
      normalize(x, y, Rxy,
                normalizeCrossSquareDifferenceUpperBound<Data>(x, y));
    } else
...
void CrossCorrelation::apply(const std::vector<Data> &x,
                             const std::vector<Data> &y, std::vector<Data> &Rxy,
                             Core::Component &errorChannels) {
  if (begin_ > end_)
    errorChannels.error(
        "Discrete begin time (%d) is larger then Discrete end time (%d).",
        begin_, end_);
  if ((similarityFunctionType_ != Multiplication ||
       !useFastFourierTransform_) &&
      (begin_ <= -((s32)y.size()) || end_ > (s32)x.size())) {
    errorChannels.error("Cannot calculate the interval [%d..%d) with x.size "
                        "(%zd) and y.size (%zd).",
                        begin_, end_, x.size(), y.size());
  }
  if (Core::isMalformed(x.begin(), x.end()) ||
      Core::isMalformed(y.begin(), y.end()))
...
//========================================================================================================
bool BandpassAutocorrelation::work(u32 bandBegin, u32 bandEnd,
                                   std::vector<Data> &autocorrelation) {
...
bool BandpassAutocorrelation::init(size_t nComponents,
                                   const std::vector<Data> &amplitude) {
  ...
  for (size_t n = 0; n < nComponents; ++n) {
     ...
     * The amplitude spectrum is symetric, so the same data [0..N/2] can be
     * used, just shifted ones: positive discrete freq.  = [0..N/2-1], negative
     * negative discrete freq.  [1..N/2] */
    f64 kernel = 1.0;
    f64 conjugateKernel;

    R_[n][0] = 0.0;
    size_t discreteFrequency;
    for (discreteFrequency = 0; discreteFrequency < nSamples / 2;
         discreteFrequency++) {
      conjugateKernel =
          cos(2.0 * M_PI / nSamples * (discreteFrequency + 1) * n);
      R_[n][discreteFrequency + 1] =
          R_[n][discreteFrequency] +
          1.0 / nSamples *
              (amplitudeSquare_[discreteFrequency] * kernel +
...
const ParameterFloat CrossCorrelationNode::paramBegin(
    "begin",
    "correlation is calculated for [begin..end) (in continous unit depending "
    "on previous nodes)",
    0);
...
const ParameterInt CrossCorrelationNode::paramNumberOfCoefficients(
    "nr-coefficients",
    "correlation is calculated for 0, 1, 2, ..nr-coefficients-1 discrete "
    "values",
    0);

const Choice CrossCorrelationNode::choiceSimilarityFunctionType(
    "multiplication", Multiplication, "absolute-difference", AbsoluteDifference,
    Choice::endMark());
const ParameterChoice CrossCorrelationNode::paramSimilarityFunctionType(
    "similarity-function", &choiceSimilarityFunctionType,
    "type of similarity function", Multiplication);

const ParameterFloat
    CrossCorrelationNode::paramPower("power", "power of similarity function",
                                     1.0);

const Choice CrossCorrelationNode::choiceNormalizationType(
    "none", None, "unbiased-estimate", UnbiasedEstimate, "upper-bound",
    UpperBound, Choice::endMark());
...
CrossCorrelationNode::CrossCorrelationNode(const Core::Configuration &c)
    : Core::Component(c), Node(c), continuousBegin_(0), continuousEnd_(0),
      sampleRate_(0), nCoefficients_(0), needInit_(true) {
  setContinuousBegin(paramBegin(c));
  setContinuousEnd(paramEnd(c));
  setNumberOfCoefficients(paramNumberOfCoefficients(c));
  setSimilarityFunctionType(
      (SimilarityFunctionType)paramSimilarityFunctionType(c));
...
void CrossCorrelationNode::init() {
  ...
  if (nCoefficients_ != 0) {
    if (continuousBegin_ != 0 || continuousEnd_ != 0) {
      warning(
          "Ambiguous parameters: begin=%f, end=%f vs. nr-coefficients=%zd.\n"
...
bool CrossCorrelationNode::setParameter(const std::string &name,
  ...
  if (paramBegin.match(name))
    ...
    setSimilarityFunctionType(
        (SimilarityFunctionType)paramSimilarityFunctionType(value));
