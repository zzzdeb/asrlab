...
public:
  Question(PropertyMapRef map, const std::string &desc = "")
...
public:
  ScalarQuestion(PropertyMapRef map, const std::string &key,
...
public:
  SetQuestion(PropertyMapRef map, const std::string &key,
              const std::vector<std::string> &values, const std::string &desc);
  ~SetQuestion() {
    delete[] begin_;
    begin_ = 0;
    end_ = 0;
...
struct TrainingInformation : public Information {
  ...
  TrainingInformation()
      : order(InvalidOrder), size(InvalidSize), score(InvalidScore) {}
  TrainingInformation(u32 order, u32 size, f64 score)
      : order(order), size(size), score(score) {}
...
public:
  DecisionTree(const Core::Configuration &config,
               ...
               QuestionRefList *questionRefs = 0, Node *root = 0);
  ...
  virtual void writeXml(Core::XmlWriter &xml,
                        const std::string &name = "decision-tree") const;
...
inline std::ostream &operator<<(std::ostream &out,
                                const Cart::QuestionRefList &q) {
  Cart::write(out, q);
  return out;
}

inline std::ostream &operator<<(std::ostream &out,
                                const Cart::DecisionTree::Path &p) {
  p.write(out);
  return out;
}
