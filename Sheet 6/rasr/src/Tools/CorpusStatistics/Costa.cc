...
CorpusVisitor::CorpusVisitor(const Core::Configuration &c)
    : Precursor(c), extraction_(0), recordingsChannel_(config, "recordings") {
...
void CorpusVisitor::enterRecording(Bliss::Recording *r) {
  ...
  if (extraction_) {
    ...
    std::istringstream durationAttr(
        extraction_->getAttribute(dataPort_, "total-duration"));
    Bliss::Time duration;
...
const Core::ParameterBool Costa::paramLmStatistics(
    "lm-statistics", "create statistics about language model properties",
    false);
