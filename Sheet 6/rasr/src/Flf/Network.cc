...
bool NetworkCrawler::init(Node *node,
...
void NetworkCrawler::dump(std::ostream &os) const {
  ...
  u32 nid = 0;
  for (std::vector<u32>::const_iterator it = visited_.begin();
       it != visited_.end(); ++it, ++nid)
    if (*it != 0)
      ordered[*it - 1] = network->nodes()[nid];
  u32 order = 1;
  for (std::vector<NodeRef>::const_iterator it = ordered.begin();
       it != ordered.end(); ++it, ++order)
...
void Node::dump(std::ostream &o) const {
  ...
  if (!out_.empty()) {
    ...
    for (LinkList::const_iterator it = out_.begin(); it != out_.end(); ++it)
      if (it->node)
        o << "    " << it->from << " --> " << it->node->name << ":" << it->to
          << std::endl;
  } else
    o << "  no outgoing links" << std::endl;
}

bool Node::init_(NetworkCrawler &crawler,
                 const std::vector<std::string> &arguments) {
  bool _good = true;
  for (LinkList::const_iterator it = in_.begin(); it != in_.end(); ++it)
    if (it->node)
...
ConstPosteriorCnRef Node::sendPosteriorCn(Port to) {
  criticalError(
      "Node \"%s\" provides no data of type \"posterior CN\" at port %d.",
      name.c_str(), to);
...
const void *Node::sendData(Port to) {
  criticalError(
      "Node \"%s\" provides no data of type \"const void*\" at port %d.",
      name.c_str(), to);
  return 0;
}
void Node::pull() {
...
// -------------------------------------------------------------------------
const Core::ParameterBool FilterNode::paramInfo("info", "info", false);
const Core::ParameterString FilterNode::paramInfoType("info-type", "info type",
                                                      "normal");

bool FilterNode::init_(NetworkCrawler &crawler,
                       const std::vector<std::string> &arguments) {
  if (!connected(0))
...
private:
  ...
  typedef Core::hash_map<std::string, ColoredNode, Core::StringHash>
      ColoredNodeMap;
...
private:
  void parse(const Core::Configuration &config) {
    ...
    for (std::vector<std::string>::const_iterator it = initials.begin();
         it != initials.end(); ++it) {
      ...
      if (!node) {
        network_.criticalError("Node \"%s\" does either not exist or does not "
    ...
    while (!S.empty()) {
      ...
      if (srcColoredNode->second.color == White) {
        ...
        std::vector<std::string> links =
            paramLinks(Core::Configuration(config, node->name));
        if (links.empty()) {
          ...
          for (std::vector<std::string>::const_iterator it = links.begin();
            ...
            if (trgColoredNode == coloredNodes.end()) {
              ...
              if (!target) {
                network_.criticalError("Node \"%s\" does either not exist or "
                                       "does not specify a type",
                                       name.c_str());
                continue;
              }
              coloredNodes.insert(
                  std::make_pair(name, ColoredNode(target, White)));
              S.push_back(target);
            } else {
    ...
    for (ColoredNodeMap::iterator itColoredNode = coloredNodes.begin();
      ...
      network_.nodes_[itColoredNode->second.node->id_] =
          itColoredNode->second.node;
    }
  }

public:
  NetworkParser(Network &network) : NodeFactory(), network_(network) {
    parse(network.config);
  }
};
const Core::ParameterStringVector
    NetworkParser::paramInitialNodes("initial-nodes",
                                     "initial nodes; list of node names", "");
const Core::ParameterStringVector NetworkParser::paramLinks(
    "links", "links; list of target nodes ([port->]name[:port])", "");

Network::Network(const Core::Configuration &config) : Core::Component(config) {}
...
bool Network::init(NetworkCrawler &crawler,
                   const std::vector<std::string> &arguments) {
  bool good = true;
  for (NodeList::iterator it = finalNodes_.begin(); it != finalNodes_.end();
       ++it)
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(0))
      warning("DummyFilterNode: No incoming lattice at port 0, node will crash "
              "on lattice request.");
...
NodeRef createDummyFilterNode(const std::string &name,
...
NodeRef createLatticeBufferNode(const std::string &name,
                                const Core::Configuration &config) {
...
public:
  typedef enum { SinkTypeLattice, SinkTypeCn, SinkTypePosteriorCn } SinkType;
...
protected:
  void reportEmpty(Port from) {
    if (warnOnEmpty_)
      warning("Empty %s from \"%s:%d\"", choiceSinkType[type_].c_str(),
              in()[from].node->name.c_str(), in()[from].from);
    if (errorOnEmpty_)
      criticalError("Empty %s from \"%s:%d\"", choiceSinkType[type_].c_str(),
                    in()[from].node->name.c_str(), in()[from].from);
  }
...
const Core::Choice Sink::choiceSinkType("lattice", Sink::SinkTypeLattice, "CN",
                                        Sink::SinkTypeCn, "fCN",
                                        Sink::SinkTypePosteriorCn,
                                        Core::Choice::endMark());
const Core::ParameterChoice Sink::paramSinkType("sink-type",
                                                &Sink::choiceSinkType,
                                                "type of input",
                                                Sink::SinkTypeLattice);
const Core::ParameterBool
    Sink::paramWarnOnEmpty("warn-on-empty",
                           "warn on empty lattice/cn/posterior-cn", true);
const Core::ParameterBool
    Sink::paramErrorOnEmpty("error-on-empty",
                            "error on empty lattice/cn/posterior-cn", false);
// deprecated
const Core::ParameterBool Sink::paramWarnOnEmptyLattice("warn-on-empty-lattice",
                                                        "warn on empty lattice",
                                                        true);
const Core::ParameterBool
    Sink::paramErrorOnEmptyLattice("error-on-empty-lattice",
  ...
  void dumpProperties(ConstLatticeRef l, std::ostream &os) {
    ...
    for (Fsa::Property prop = 1; prop != 0; prop <<= 1)
      if (_knownProperties & prop)
        os << "  0x" << std::hex << std::setw(8) << std::setfill(' ')
           << std::right << prop << ": "
...
public:
  PropertiesNode(const std::string &name, const Core::Configuration &config)
      : FilterNode(name, config), knownProperties_(0), knownPropertyValues_(0),
...
const Core::Choice PropertiesNode::tertiaryLogicChoice(
    "0", 0, "1", 1, "2", 2, "no", 0, "yes", 1, "dunno", 2, "false", 0, "true",
    1, "unknown", 2, "off", 0, "on", 1, Core::Choice::endMark());
const Core::ParameterBool
    PropertiesNode::paramDumpProperties("dump", "dump properties", false);

NodeRef createPropertiesNode(const std::string &name,
                             const Core::Configuration &config) {
...
public:
    ...
    bool hasInput = false;
    {
      ...
      for (u32 i = 0; i <= 9; ++i)
        if (connected(i)) {
          ...
          switch (i) {
          case 0:
            msg << "Get bliss speech segment at port 0.\n";
            break;
          case 1:
            msg << "Get audio file name  at port 1.\n";
            break;
          case 2:
            msg << "Get start time at port 2.\n";
            break;
          case 3:
            msg << "Get end time at port 3.\n";
            break;
          case 4:
            msg << "Get track at port 4.\n";
            break;
          case 5:
            msg << "Get orthography at port 5.\n";
            break;
          case 6:
            msg << "Get speaker id at port 6.\n";
            break;
          case 7:
            msg << "Get condition id at port 7.\n";
            break;
          case 8:
            msg << "Get recording id at port 8.\n";
            break;
          case 9:
            msg << "Get segment id at port 9.\n";
            break;
          }
        }
    }
  ...
  virtual void sync() { segment_.reset(); }

  virtual ConstSegmentRef sendSegment(Port to) {
    if (!segment_) {
      Segment *segment = new Segment;
      if (connected(0))
        segment->setBlissSpeechSegment(
            static_cast<const Bliss::SpeechSegment *>(requestData(0)));
      if (connected(1))
      ...
      if (progressChannel_.isOpen()) {
        ...
        if (segment->hasRecordingId())
          xmlSegment +
              Core::XmlAttribute("recording-id", segment_->recordingId());
        ...
        if (segment->hasStartTime() && segment->hasEndTime())
          xmlSegment + Core::XmlAttribute("start", segment_->startTime()) +
              Core::XmlAttribute("end", segment_->endTime());
        if (segment->hasSpeakerId())
          xmlSegment + Core::XmlAttribute("speaker-id", segment_->speakerId());
        if (segment->hasConditionId())
          xmlSegment +
              Core::XmlAttribute("condition-id", segment_->conditionId());
...
NodeRef createSegmentBuilderNode(const std::string &name,
                                 const Core::Configuration &config) {
...
private:
  ConstSemiringRef buildSemiring(ConstSemiringRef semiring1,
                                 ConstSemiringRef semiring2) {
    if (semiring_ &&
        ((semiring1_.get() == semiring1.get()) ||
    ...
    scales.insert(scales.end(), semiring2_->scales().begin(),
                  semiring2_->scales().end());
    KeyList keys(semiring1_->keys());
    keys.insert(keys.end(), semiring2_->keys().begin(),
                semiring2_->keys().end());
    semiring_ =
        Semiring::create(semiring1_->type(),
                         semiring1_->size() + semiring2_->size(), scales, keys);
    return semiring_;
  }
...
public:
  ...
  virtual ConstLatticeRef sendLattice(Port to) {
    if (!appendL_) {
      ...
      if (l1 && l2)
        appendL_ =
            appendScores(l1, l2, buildSemiring(l1->semiring(), l2->semiring()));
    }
    return appendL_;
  }

  virtual void sync() { appendL_.reset(); }
};
NodeRef createAppendScoresNode(const std::string &name,
                               const Core::Configuration &config) {
