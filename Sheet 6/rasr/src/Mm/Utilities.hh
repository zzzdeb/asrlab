...
double logNorm(InputIterator beginDiagonal, InputIterator endDiagonal,
               WeightIterator beginWeights, WeightIterator endWeights) {
  double result = 0;
...
template <class InputIterator, class WeightIterator>
double
gaussLogNormFactor(InputIterator beginDiagonal, InputIterator endDiagonal,
                   WeightIterator beginWeights, WeightIterator endWeights) {
  return (double)std::distance(beginDiagonal, endDiagonal) *
             log((double)2 * M_PI) +
         logNorm(beginDiagonal, endDiagonal, beginWeights, endWeights);
}

/** 1 / sqrt(x) */
template <class T> struct inverseSquareRoot : public std::unary_function<T, T> {
...
public:
  normalizedMinus(T normalization) : normalization_(normalization) {
...
/** x + weight*y */
template <class T> struct plusWeighted : public std::binary_function<T, T, T> {
  T weight_;
...
/** x + y^2 */
template <class T> struct plusSquare : public std::binary_function<T, T, T> {
...
/** -factor * log (x) */
template <class T> class minusLog : public std::unary_function<T, T> {
  T factor_;
 ...
 *
 *  Remark: mean{x} is expected to be 0, so offset is simply to half of the
 * integer interval.
 */
...
public:
  ...
  QuantizedType operator()(ValueType x) {
    static const int offset = (int)round(
        (Core::Type<QuantizedType>::max + Core::Type<QuantizedType>::min + 1) /
        2);
...
inline DistanceType unrolledVectorDistance(const T *a, const T *b,
                                           size_t dimension) {
 ...
 *  -objects_ is no STL vector to avoid any calls to the constructor of the
 * objects when after the memory.
 */
template <class O> class Cache {
 ...
 * objects when after the memory.
 */
template <class O> class Cache2D {
...
public:
  Cache2D(size_t first_level_size)
      : isCalculated_(first_level_size,
                      std::make_pair(false, std::vector<bool>())),
        freedStorage_(false) {
 ...
 */
template <class Type> class DynamicCache {
private:
  std::vector<bool> isCalculated_;
...
public:
  ...
  const Type &operator[](size_t index) const { return objects_[index]; }
