...
protected:
  ...
  u32 length_;
  /** number of elements removed from the beginning of the buffer after a call
   * to get */
  u32 shift_;
   ...
   *  if false, segments are delivered until the last segment contains the last
   * sample
   */
  bool flushAll_;
...
private:
  /** copies @param length element form the beginning of the buffer to @param
   * out and sets the beginning and end time of @param out
   */
...
public:
   ...
   *
   *  Caution: an assertion fails if the buffer is larger then 2 * max(length_,
   * shift_)
   */
