...
/**
 *
 * Collection of fast hash functions taken from
 http://www.concentric.net/~Ttwang/tech/inthash.htm.
 *

 32 bit Mix Functions

 Based on an original suggestion on Robert Jenkin's part in 1997, I have done
 some research for a version of the integer hash function. This is my latest
 version as of January 2007. The specific value of the bit shifts are obtained
 from running the accompanied search program.
 ...
 By taking advantages of the native instructions such as 'add complement', and
 'shift & add', the above hash function runs in 11 machine cycles on HP 9000
 workstations.

 Having more rounds will strengthen the hash function by making the result more
 random looking, but performance will be slowed down accordingly. Simulation
 seems to prefer small shift amounts for inner rounds, and large shift amounts
 for outer rounds. Robert Jenkins' 32 bit integer hash function
 ...
 This version of integer hash function uses operations with integer constants to
 help producing a hash value. I suspect the actual values of the magic constants
 are not very important. Even using 16 bit constants may still work pretty well.

 These magic constants open up the construction of perfect integer hash
 functions. A test program can vary the magic constants until a set of perfect
 hashes are found. Using Multiplication for Hashing

 Using multiplication requires a mechanism to transport changes from high bit
 positions to low bit positions. Bit reversal is best, but is slow to implement.
 A viable alternative is left shifts.

 Using multiplication presents some sort of dilemma. Certain machine platforms
 supports integer multiplication in hardware, and an integer multiplication can
 be completed in 4 or less cycles. But on some other platforms an integer
 multiplication could take 8 or more cycles to complete. On the other hand,
 integer hash functions implemented with bit shifts perform equally well on all
 platforms.

 A compromise is to multiply the key with a 'sparse' bit pattern, where on
 machines without fast integer multiplier they can be replaced with a 'shift &
 add' sequence. An example is to multiply the key with (4096 + 8 + 1), with an
 equivalent expression of (key + (key << 3)) + (key << 12).

 On most machines a bit shift of 3 bits or less, following by an addition can be
 performed in one cycle. For example, Pentium's 'lea' instruction can be used to
 good effect to compute a 'shift & add' in one cycle.
 ...
 One such use for this kind of hash function is to hash a 64 bit virtual address
 to a hash table index. Because the output of the hash function is narrower than
 the input, the result is no longer one-to-one.
 ...
 *
 **/
template <class T, class WeakOrder> class BinaryPriorityQueue {
public:
  typedef std::vector<T> Heap;

private:
  WeakOrder lessThan_;
  Heap heap_; // for the sake of simple index arithmetic the first element in
              // the heap must never be used (aka pseudo sentinel), i.e.
              // heap_[0] == T()
public:
 ...
 *
 * ToTry:
 * - Replace binary look ups by hash maps (in PrefixTree::Node::arcs,
 *PrefixTree::Node::suffixes, Detour::forks)
 *
 **/
...
public:
  ...
  const Data &get(const Key &key, const Data &def = Data()) const {
    if (Precursor::size() <= 8) {
      typename Precursor::const_iterator it = Precursor::begin(),
                                         it_end = Precursor::end();
...
public:
  ...
  public:
    ...
    struct Node {
      ...
      std::pair<Node *, bool> add(Fsa::LabelId label) {
        ...
        ArcList::iterator itArc =
            std::lower_bound(arcs.begin(), arcs.end(), Arc(label));
      ...
      bool addSuffix(Fsa::StateId suffixSid) {
        StateIdList::iterator itSuffix =
            std::lower_bound(suffixes.begin(), suffixes.end(), suffixSid);
  ...
  public:
    ...
    ~PrefixTree() {
      ...
      while (!S.empty()) {
        Node *node = S.back();
        S.pop_back();
        for (Node::ArcList::iterator a = node->arcs.begin(),
                                     a_end = node->arcs.end();
             a != a_end; ++a)
          S.push_back(a->to);
        delete node;
  ...
  struct Node {
    ...
    Node()
        : begin(0), end(0), fwdScore(Core::Type<Score>::max),
          bwdScore(Core::Type<Score>::max), dissenters(0) {}
  ...
  public:
    ...
    Score detourScore;
    const Arc *dissenter;
    Path() : n(0), fatherId(0), detourScore(0.0), dissenter(0) {}
    Path(Number n, u32 fatherId, Score detourScore, const Arc *dissenter,
         bool isDuplicate)
        : n(n), fatherId(fatherId), detourScore(detourScore),
          dissenter(dissenter) {}
  };
  ...
  public:
    DissenterList() : Q_() { initialHeap_ = Q_.heap(); }
  ...
  protected:
    void update() {
      for (dissenter_ = (*dissenters_)[i_]; dissenter_ != 0;
           dissenter_ = (*dissenters_)[++i_]) {
  ...
  public:
    Detour(Fsa::StateId initialNid, PrefixTree::Node *initialPrefix, u32 pathId,
           Score pathScore, const DissenterList *dissenters,
           const ForkList &forks)
        : initialNid_(initialNid), initialPrefix_(initialPrefix),
          pathId_(pathId), pathScore_(pathScore), dissenters_(dissenters),
          forks_(forks), i_(0) {
      update();
    }
    // const member access
...
public:
  NBestBuilder(ConstLatticeRef l, u32 n, bool removeDuplicates = true,
               bool ignoreNonWords = true)
...
const NBestBuilder::Path::Number NBestBuilder::Path::InvalidNumber =
    Core::Type<Number>::max;

/**
 * Calculate
 * - state fwd./bwd. scores
 * - arc dissenter score: additional cost of using arc instead of (locally) best
 *path
 **/
void NBestBuilder::initialize(bool ignoreNonWords) {
  const Lattice &l = *l_;
  const ScoreList &scales = l.semiring()->scales();
  LabelMapRef nonWordMapRef =
      ignoreNonWords ? LabelMap::createNonWordToEpsilonMap(
  ...
  // count arcs
  u32 nArcs = 0;
  for (StateMap::const_iterator itSid = topSort->begin(),
                                endSid = topSort->end();
       itSid != endSid; ++itSid)
  ...
  for (StateMap::const_iterator itSid = topSort->begin(),
    ...
    u32 arcId = 0;
    for (State::const_iterator a = sr->begin(), a_end = sr->end(); a != a_end;
         ++a, ++node.end, ++arcId) {
      Arc &arc = *node.end;
      arc.id = arcId;
      arc.from = sid;
      arc.to = a->target();
      arc.label = nonWordMapRef
                      ? ((*nonWordMapRef)[a->input()].empty() ? a->input()
                                                              : Fsa::Epsilon)
                      : a->input();
  ...
  for (StateMap::const_reverse_iterator itSid = topSort->rbegin(),
                                        endSid = topSort->rend();
       itSid != endSid; ++itSid) {
    Fsa::StateId sid = *itSid;
    Node &node = nodes_[sid];
  ...
  verify(Core::isAlmostEqualUlp(f32(bestScore_),
                                f32(nodes_[topSort->front()].bwdScore), 100));
}
...
void NBestBuilder::find(u32 n) {
  ...
  // add first best, i.e. initialize
  {
    ...
    PrefixTree::Node *prefix = prefixTree_.root(),
                     *initialPrefix = prefixTree_.root();
    ...
    for (StateIdList::const_reverse_iterator itNid = S.rbegin() + 1,
                                             endNid = S.rend();
         itNid != endNid; ++itNid) {
    ...
    Detour *detour =
        new Detour(initialSid, initialPrefix, 0, 0.0, dissenters, F);
    S.clear();
    F.clear();
    if (!detour->empty())
      Q.append(detour);
  ...
  while ((validPathIds_.size() < n) && !Q.empty() &&
         (paths_.size() < maxPaths)) {
    Detour *detour = Q.top();
    verify(!detour->empty());
    const Score pathScore = detour->score();
    const Arc *dissenter = detour->dissenter();
    std::pair<PrefixTree::Node *, bool> prefix =
        std::make_pair(detour->prefix(), false);
    ...
    if (isUnique || !removeDuplicates_) {
      ...
      verify_(!isUnique);
    paths_.push_back(
        Path(n, detour->pathId(), pathScore, dissenter, !isUnique));
    Detour *newDetour = new Detour(dissenter->to, newPrefix, pathId, pathScore,
                                   newDissenters, F);
    S.clear();
    F.clear();
    detour->next();
  ...
  if (paths_.size() == maxPaths)
    Core::Application::us()->warning(
        "Did not find %d valid paths after searching %d paths; give up.", n,
        maxPaths);
  for (DetourPtrQueue::Heap::iterator itDetourPtr = Q.heap().begin() + 1,
                                      endDetourPtr = Q.heap().end();
       itDetourPtr != endDetourPtr; ++itDetourPtr)
    delete *itDetourPtr;
  Q.clear();
}
...
StaticLatticeRef NBestBuilder::lattice(ScoreId scoreId) {
  ...
  std::vector<std::pair<Fsa::StateId, ScoresRef> > suffix2lattice(
      nodesSize_, std::make_pair(Fsa::InvalidStateId, ScoresRef()));
  // add best; prepare suffix representation
  {
    ...
    ScoresRef suffixSum;
    for (Fsa::StateId internalSid = internalInitialSid, internalTargetSid;;
         internalSid = internalTargetSid) {
    ...
    for (u32 suffixSid = nextSid - 2; suffixSid >= firstSuffixSid;
         --suffixSid) {
      std::pair<Fsa::StateId, ScoresRef> &suffix2latticeElement =
          suffix2lattice[lattice2internal[suffixSid]];
      suffixSum = suffix2latticeElement.second =
          semiring.extend(suffix2latticeElement.second, suffixSum);
    }
    std::pair<Fsa::StateId, ScoresRef> &suffix2latticeElement =
        suffix2lattice[lattice2internal[1]];
    suffixSum = suffix2latticeElement.second =
        semiring.extend(suffix2latticeElement.second, suffixSum);
  ...
  for (PathIdList::const_iterator itPathId = validPathIds_.begin() + 1;
       itPathId != validPathIds_.end(); ++itPathId) {
    ...
    for (ConstArcPtrList::const_reverse_iterator
             itDissenter = dissenters.rbegin(),
             endDissenter = dissenters.rend();
         ;) {
      ...
      if ((itDissenter == endDissenter) &&
          (suffix2lattice[dissenter.to].first != Fsa::InvalidStateId)) {
        std::pair<Fsa::StateId, ScoresRef> &suffix2latticeElement =
            suffix2lattice[dissenter.to];
        sp->newArc(suffix2latticeElement.first, a.weight(), a.input(),
                   a.output());
        sum = semiring.extend(semiring.extend(sum, a.weight()),
                              suffix2latticeElement.second);
        sp = 0;
        break;
      }
      sp->newArc(nextSid, a.weight(), a.input(), a.output());
      ...
      if (itDissenter == endDissenter)
        break;
      for (Fsa::StateId internalSid = dissenter.to, internalTargetSid,
                        endSid = (*itDissenter)->from;
    ...
    if (sp) {
      ...
      ScoresRef suffixSum;
      for (Fsa::StateId internalSid = dissenters.front()->to, internalTargetSid;
           ; internalSid = internalTargetSid) {
        const Node &node = nodes_[internalSid];
        internalTargetSid = node.begin->to;
        ConstStateRef sr = l.getState(internalSid);
        ...
        std::pair<Fsa::StateId, ScoresRef> &suffix2latticeTarget =
            suffix2lattice[internalTargetSid];
        if (suffix2latticeTarget.first != Fsa::InvalidStateId) {
          sp->newArc(suffix2latticeTarget.first, a.weight(), a.input(),
                     a.output());
        ...
        if (!nodes_[internalTargetSid].hasArcs()) {
          ...
          sp->setFinal(sr->weight());
          suffix2lattice[internalTargetSid] =
              std::make_pair(sp->id(), sr->weight());
          suffixSum = sr->weight();
          lastSuffixSid = nextSid - 2;
          break;
        }
      }
      for (u32 suffixSid = lastSuffixSid; suffixSid >= firstSuffixSid;
           --suffixSid) {
        std::pair<Fsa::StateId, ScoresRef> &suffix2latticeElement =
            suffix2lattice[lattice2internal[suffixSid]];
        suffixSum = suffix2latticeElement.second =
...
/*
ConstLatticeRef nbest(ConstLatticeRef l, u32 n, bool removeDuplicates, bool
ignoreNonWords, ScoreId scoreId) { NBestBuilder builder(l, n, removeDuplicates,
ignoreNonWords); return builder.lattice(scoreId);
}
*/
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
ConstLatticeRef nbest(ConstLatticeRef l, u32 n, bool removeDuplicates,
                      NBestAlgorithm algo) {
...
// -------------------------------------------------------------------------
namespace {
const Core::Choice choiceNBestAlgorithm("eppstein", Eppstein, "more", Mori,
                                        Core::Choice::endMark());
const Core::ParameterString paramNBestAlgorithm("algorithm", "n-best algorithm",
                                                "eppstein");
NBestAlgorithm getNBestAlgorithm(const Core::Configuration &config) {
  Core::Choice::Value nBestChoice =
      choiceNBestAlgorithm[paramNBestAlgorithm(config)];
  ...
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    if (!isValid_) {
      ...
      if (!scoreKey_.empty()) {
        ...
        if (scoreId == Semiring::InvalidId)
          criticalError("Semiring \"%s\" has no dimension \"%s\".",
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    verify(n_ >= 1);
    if (n_ == 1)
      warning("N-Best list generation is called with n=1; did you forget to "
              "specify n?");
    ...
    if (ignoreNonWords_)
      msg << "- Ignore non words, i.e. treat two hypothesis as duplicates, if "
             "they only differ in non words.\n";
    if (!scoreKey_.empty())
...
const Core::ParameterInt NBestNode::paramN("n", "(up to) n best hypothesis", 1);
const Core::ParameterBool NBestNode::paramRemoveDuplicates(
    "remove-duplicates", "remove duplicate pathes from the n-best list", true);
const Core::ParameterBool NBestNode::paramIgnoreNonWords(
    "ignore-non-words",
    "ignore non words, i.e. treat two pathes as duplicates, if they only "
    "differ in non words",
    true);
const Core::ParameterString
    NBestNode::paramScoreKey("score-key", "store path score in this dimension",
                             "");

NodeRef createNBestNode(const std::string &name,
...
NodeRef createSelectNBestNode(const std::string &name,
                              const Core::Configuration &config) {
...
protected:
  void dumpNBest(ConstLatticeRef l, std::ostream &os) {
    ...
    if (scoreKeys_.empty()) {
      ...
      for (KeyList::const_iterator itKey = scoreKeys_.begin();
           itKey != scoreKeys_.end(); ++itKey) {
    ...
    os << "# ";
    for (ScoreIdList::const_iterator itId = scoreIds.begin();
         itId != scoreIds.end(); ++itId)
      os << semiring->key(*itId) << "/" << semiring->scale(*itId) << "\t";
    os << "N";
    os << std::endl;
    std::vector<Fsa::LabelId> labels;
    for (State::const_iterator a = begin->begin(), a_end = begin->end();
         a != a_end; ++a) {
      ...
      os << labels.size() << "\t";
      os << "<s>";
      for (std::vector<Fsa::LabelId>::const_iterator it = labels.begin(),
                                                     it_end = labels.end();
           it != it_end; ++it)
...
const Core::ParameterStringVector DumpNBestNode::paramScoreKeys(
    "scores", "dimension of scores to be dumped; default is all scores", "");
NodeRef createDumpNBestNode(const std::string &name,
                            const Core::Configuration &config) {
