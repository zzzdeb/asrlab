...
struct u16_char_traits {
  ...
  static void assign(u16 &__c1, const u16 &__c2) { __c1 = __c2; }

  static bool eq(const u16 &__c1, const u16 &__c2) { return __c1 == __c2; }

  static bool lt(const u16 &__c1, const u16 &__c2) { return __c1 < __c2; }
  ...
  static u16 to_char_type(const int &__c) { return static_cast<u16>(__c); }

  // To keep both the byte 0xff and the eof symbol 0xffffffff
  // from ending up as 0xffffffff.
  static int to_int_type(const u16 &__c) { return static_cast<int>(__c); }

  static bool eq_int_type(const int &__c1, const int &__c2) {
    return __c1 == __c2;
  }

  static int eof() { return static_cast<int>(EOF); }

  static int not_eof(const int &__c) { return (__c == eof()) ? 0 : __c; }
};
...
public:
  void setContextDependent(bool cd) { isContextDependent_ = cd; }
  bool isContextDependent() const { return isContextDependent_; }
...
public:
  ...
  u32 nPhonemes() const { return phonemes_.size() - 1; }
...
template <typename T> class PhonemeMap {
public:
  typedef T Value;
...
public:
  PhonemeMap(Core::Ref<const PhonemeInventory> pi) : store_(pi->nPhonemes()) {}
