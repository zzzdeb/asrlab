...
private:
  ...
  Network *createNetworkNode(const std::string &name,
                             const std::string &networkName);
  Node *createFilterNode(const std::string &name,
                         const std::string &filterName);

  bool findNetworkFile(const std::string &pathname,
...
public:
  ...
  // methods called by NetworkTemplate while initializing a network from a
  // template
   ...
   *    -parameter resolved in the configuration file (e.g. in flow-file " ...
   * filter="$(filter-type) ..." and in config-file "... name.filter-type =
   * signal-window ...).
   */
  AbstractNode *
  createNode(const NetworkTemplate::NodeAttributes &nodeAttributes);
...
public:
   ...
   *
   * Subnetworks:
   * If subnetwork name does not have an extension value
   * paramNetworkFileExtension is appended. XML description file is checked in
