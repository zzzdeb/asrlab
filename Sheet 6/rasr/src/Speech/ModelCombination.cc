...
const Core::ParameterFloat ModelCombination::paramPronunciationScale(
    "pronunciation-scale",
    "scaling exponent for lemma pronunciation probabilities", 0.0);

ModelCombination::ModelCombination(const Core::Configuration &c, Mode mode,
  ...
  if (mode & useLanguageModel) {
    setLanguageModel(Lm::Module::instance().createScaledLanguageModel(
        select("lm"), lexicon_));
    if (!languageModel_)
...
void ModelCombination::setAcousticModel(
    Core::Ref<Am::AcousticModel> acousticModel) {
...
void ModelCombination::setLanguageModel(
    Core::Ref<Lm::ScaledLanguageModel> languageModel) {
...
void ModelCombination::distributeScaleUpdate(
    const Mc::ScaleUpdate &scaleUpdate) {
  if (lexicon_) {
    Mm::Score scale;
    if (scaleUpdate.findScale(config.prepareResourceName(
                                  fullName(), paramPronunciationScale.name()),
                              scale)) {
...
void ModelCombination::getDependencies(
