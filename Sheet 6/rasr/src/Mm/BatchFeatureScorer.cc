...
BatchFeatureScorerBase::BatchFeatureScorerBase(const Core::Configuration &c)
    : Core::Component(c), FeatureScorer(c), scores_(0), paddedDimension_(0),
      dimension_(0), nMixtures_(0), nDensities_(0), currentFeature_(0),
      buffered_(0), bufferSize_(paramBufferSize(c)) {
  log("batch feature scorer using buffer size %d", bufferSize_);
}

BatchFeatureScorerBase::~BatchFeatureScorerBase() { ::free(scores_); }
...
BatchFeatureScorerBase::getScorer(const FeatureVector &f) const {
  require(bufferFilled());
  s32 posToAdd =
      currentFeature_ ? (currentFeature_ - 1) % bufferSize_ : bufferSize_ - 1;
...
Score BatchFeatureScorerBase::getScore(EmissionIndex e, u32 featureIndex,
                                       u32 length) const {
...
struct BatchFeatureScorerBase::AllDensitySelector {
  bool operator()(u32 bufferIndex, size_t density) const { return true; }
...
BatchFloatFeatureScorer::BatchFloatFeatureScorer(const Core::Configuration &c)
    : Core::Component(c), BatchFeatureScorerBase(c), features_(0), means_(0),
      variance_(0), constants_(0) {}
...
void BatchFloatFeatureScorer::setFeature(size_t pos,
...
void BatchFloatFeatureScorer::init(const MixtureSet &mixtureSet) {
  ...
  std::copy(covariance.inverseSquareRootDiagonal().begin(),
            covariance.inverseSquareRootDiagonal().end(), variance_);
  const float logNormFactor = covariance.logNormalizationFactor();

  Core::allocateAlignedVector<f32>(&features_, bufferSize_ * paddedDimension_,
                                   0.0, 16);
  Core::allocateAlignedVector<f32>(&means_, nDensities_ * paddedDimension_, 0.0,
                                   16);
  ...
  for (size_t m = 0; m < nMixtures_; ++m) {
    ...
    for (size_t dns = 0; dns < mixture.nDensities(); ++dns) {
      const GaussDensity &density =
          *mixtureSet.density(mixture.densityIndex(dns));
...
void BatchFloatFeatureScorer::fillScoreCache(EmissionIndex e, u32 featureIndex,
                                             u32 length) const {
  fillScoreCacheTpl<AllDensitySelector>(e, featureIndex, length,
                                        AllDensitySelector());
}
...
void BatchFloatFeatureScorer::fillScoreCacheTpl(
    EmissionIndex e, u32 featureIndex, u32 length,
    const DensitySelector &selector) const {
  ...
  for (size_t dns = offsets_[e]; dns < endDns; ++dns) {
    ...
    for (u32 t = startIdx; t < endIdx; ++t) {
      ...
      for (int d = 0; d < paddedDimension_; d += BlockSize) {
        ...
        x2 =
            _mm_sub_ps(_mm_load_ps(mean + d + 4), _mm_load_ps(feature + d + 4));
...
void BatchPreselectionFloatFeatureScorer::init(const MixtureSet &mixtureSet) {
  ...
  std::fill(activeClusters_,
            activeClusters_ + bufferSize_ * clustering_->nClusters(), false);
}

void BatchPreselectionFloatFeatureScorer::setFeature(
    size_t pos, const FeatureVector &f) const {
  BatchFloatFeatureScorer::setFeature(pos, f);
  clustering_->selectClusters(activeClusters_ +
                                  (pos * clustering_->nClusters()),
                              features_ + (pos * paddedDimension_));
}

class BatchPreselectionFloatFeatureScorer::ClusterDensitySelection {
public:
  ClusterDensitySelection(const bool *activeClusters,
                          const DensityClusteringBase &clustering)
      : activeClusters_(activeClusters), clustering_(clustering) {}

  bool operator()(u32 bufferIndex, size_t density) const {
    return activeClusters_[bufferIndex * clustering_.nClusters() +
                           clustering_.clusterIndexForDensity(density)];
  }
...
void BatchPreselectionFloatFeatureScorer::fillScoreCache(EmissionIndex e,
                                                         u32 featureIndex,
                                                         u32 length) const {
  fillScoreCacheTpl(e, featureIndex, length,
                    ClusterDensitySelection(activeClusters_, *clustering_));
...
struct BatchIntFeatureScorer::MultiplyAndQuantize {
  u8 operator()(f32 a, f32 b) const { return quantize<f32, u8>()(a * b); }
};

BatchIntFeatureScorer::BatchIntFeatureScorer(const Core::Configuration &c)
    : Core::Component(c), BatchFeatureScorerBase(c), features_(0), means_(0),
      variance_(0), constants_(0) {}
...
f32 BatchIntFeatureScorer::quantizationScale(
    const MixtureSet &mixtureSet) const {
  require(variance_);
...
void BatchIntFeatureScorer::init(const MixtureSet &mixtureSet) {
  ...
  const float logNormFactor =
      covariance.logNormalizationFactor() * scaleSquared;

  // allocate feature buffer and means in the same memory area.
  Core::allocateAlignedVector<QuantizedType>(
      &features_,
      bufferSize_ * paddedDimension_ + nDensities_ * paddedDimension_, 0, 16);
...
void BatchIntFeatureScorer::setFeature(size_t pos,
                                       const FeatureVector &f) const {
  verify(pos < bufferSize_);
  QuantizedType *feature = features_ + (pos * paddedDimension_);
  memset(feature, 0, sizeof(QuantizedType) * paddedDimension_);
  std::transform(f.begin(), f.end(), variance_, feature, MultiplyAndQuantize());
}
...
template <class DensitySelector>
void BatchIntFeatureScorer::fillScoreCacheTpl(
    EmissionIndex e, u32 featureIndex, u32 length,
    const DensitySelector &selector) const {
  const size_t startIdx = featureIndex;
  const size_t endIdx = featureIndex + length;
  ...
  for (u32 t = startIdx; t < endIdx; ++t) {
    ...
    _mm_prefetch(const_cast<char *>(reinterpret_cast<const char *>(mean)),
                 _MM_HINT_T1);
    ...
    _mm_prefetch(const_cast<char *>(reinterpret_cast<const char *>(result)),
                 _MM_HINT_NTA);
    *result = static_cast<f32>(best) / scale_;
  }
}

void BatchIntFeatureScorer::fillScoreCache(EmissionIndex e, u32 featureIndex,
                                           u32 length) const {
  fillScoreCacheTpl<AllDensitySelector>(e, featureIndex, length,
                                        AllDensitySelector());
}
...
void BatchPreselectionIntFeatureScorer::setFeature(
    size_t pos, const FeatureVector &f) const {
...
public:
  ClusterIterator(const bool *activeClusters,
                  const DensityClusteringBase &clustering)
  ...
  bool value() const { return activeClusters_[offset_ + *cluster_]; }
...
void BatchPreselectionIntFeatureScorer::fillScoreCache(EmissionIndex e,
                                                       u32 featureIndex,
                                                       u32 length) const {
  fillScoreCacheTpl(e, featureIndex, length,
                    ClusterIterator(activeClusters_, *clustering_));
}

// ================================================================

const size_t BatchUnrolledIntFeatureScorer::Dimension =
    3 * BatchUnrolledIntFeatureScorer::BlockSize;
...
void BatchUnrolledIntFeatureScorer::init(const MixtureSet &mixtureSet) {
  ...
  if (paddedDimension_ > Dimension) {
    criticalError(
        "This feature scorer supports only features with max. %d components",
        (int)Dimension);
  }
}

void BatchUnrolledIntFeatureScorer::fillScoreCache(EmissionIndex e,
                                                   u32 featureIndex,
                                                   u32 length) const {
  const size_t startIdx = featureIndex;
  const size_t endIdx = featureIndex + length;
  ...
  for (u32 t = startIdx; t < endIdx; ++t) {
    ...
    for (size_t dns = startDns; dns < endDns; ++dns) {
      __m128i sum = _mm_setzero_si128();
      {
        ...
        m2 =
            _mm_load_si128(reinterpret_cast<const __m128i *>(mean + BlockSize));
        x2 = _mm_load_si128(
            reinterpret_cast<const __m128i *>(feature + BlockSize));
        addDistance(m1, x1, sum);
        m3 = _mm_load_si128(
            reinterpret_cast<const __m128i *>(mean + 2 * BlockSize));
        x3 = _mm_load_si128(
            reinterpret_cast<const __m128i *>(feature + 2 * BlockSize));
