...
const Core::ParameterInt
    EbwDiscriminativeMixtureEstimator::paramNumberOfIterations(
        "number-of-iterations",
        "number of iterations in the Cambridge re-estimation procedure of "
        "mixture weights",
        100, 0);

EbwDiscriminativeMixtureEstimator::EbwDiscriminativeMixtureEstimator(
    const Core::Configuration &c)
    : Precursor(c), nIterations_(paramNumberOfIterations(c)) {}

void EbwDiscriminativeMixtureEstimator::removeDensity(
    DensityIndex indexInMixture) {
...
void EbwDiscriminativeMixtureEstimator::estimateMixtureWeights(
    std::vector<Weight> &weights, bool normalizeWeights) const {
  /**
  ...
  if (nDensities() > 1 and getWeight() > Core::Type<Weight>::epsilon) {
      ...
      if (normalizeWeights) {
        ...
        if (sum > Core::Type<Weight>::delta) {
          std::transform(weights.begin(), weights.end(), weights.begin(),
                         std::bind2nd(std::divides<Weight>(), sum));
        } else {
          Core::Application::us()->warning(
              "cannot normalize mixture weights because of vanishing sum");
    ...
    weights.resize(nDensities(),
                   normalizeWeights ? 1 / Weight(nDensities()) : 1);
  }
}

void EbwDiscriminativeMixtureEstimator::addDensity(
...
void EbwDiscriminativeMixtureEstimator::accumulate(
    const AbstractMixtureEstimator &toAdd) {
  ...
  std::transform(denWeights_.begin(), denWeights_.end(),
                 _toAdd->denWeights_.begin(), denWeights_.begin(),
                 std::plus<Weight>());
}

void EbwDiscriminativeMixtureEstimator::accumulateDenominator(
    DensityIndex indexInMixture, const FeatureVector &featureVector,
    Weight weight) {
  ...
  required_cast(DiscriminativeGaussDensityEstimator *,
                densityEstimators_[indexInMixture].get())
...
void EbwDiscriminativeMixtureEstimator::write(
  ...
  for (DensityIndex dns = 0; dns < densityEstimators_.size(); ++dns) {
    o << Core::XmlEmpty("density") +
             Core::XmlAttribute("estimator-index",
...
bool EbwDiscriminativeMixtureEstimator::equalWeights(
    const AbstractMixtureEstimator &toCompare) const {
  return Precursor::equalWeights(toCompare) &&
         required_cast(const EbwDiscriminativeMixtureEstimator *, &toCompare)
                 ->denWeights_ == denWeights_;
}

DensityIndex
EbwDiscriminativeMixtureEstimator::accumulate(Core::BinaryInputStreams &is,
...
void EbwDiscriminativeMixtureEstimatorWithISmoothing::removeDensity(
    DensityIndex indexInMixture) {
