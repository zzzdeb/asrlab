...
const Network &Processor::network() const { return *network_; }

const NetworkCrawler &Processor::crawler() const { return *crawler_; }
...
public:
  ...
  virtual bool good() { return itBatch_ != batches_.end(); }
  ...
  virtual std::string sendString(Port to) { return getArgument(to); }
};
const std::string BatchNode::emptyString = std::string();
const Core::ParameterString BatchNode::paramFile("file", "text file", "");
const Core::ParameterString
    BatchNode::paramEncoding("encoding", "encoding of file", "utf-8");

NodeRef createBatchNode(const std::string &name,
