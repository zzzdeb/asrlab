...
template <class _Weight> struct Accumulator {
  virtual ~Accumulator() {}
...
template <class _Weight> class Semiring : public Core::ReferenceCounted {
...
public:
  ...
  Fsa::Property
  getProperties(Fsa::Property properties = Fsa::PropertyAll) const {
  ...
  virtual _Weight invCollect(const _Weight &a, const _Weight &b) const {
    std::cerr << "method \"negCollect\" is not supported" << std::endl;
    return invalid();
  }
  ...
  virtual _Weight defaultWeight() const { return one(); }
...
protected:
  struct AnchoredAccumulator : public Ftl::Accumulator<_Weight> {
    typedef _Weight (ConstSelf::*Function)(const _Weight &a,
...
template <class _Weight> class SemiringAdapter : public Semiring<_Weight> {
protected:
...
public:
  ...
  SemiringAdapter(Fsa::Property properties) : Precursor(properties) {}
