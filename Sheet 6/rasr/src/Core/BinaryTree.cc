...
BinaryTree::BinaryTree(const TreeStructure &s) : root_(0) {
  cutTreeCounter_ = 0;
  build(s);
}
...
void BinaryTree::buildTreeStructure(void) const {
  ...
  while (!stack.empty()) {
    ...
    if ((node->left_ != 0) && (node->right_ != 0)) {
      ...
      treeStructure.push_back(
          TreeStructureEntry(node->id_, node->leafNumber_)); // leaf
  }
}
...
BinaryTree::LeafIndexVector BinaryTree::prune(Id maxLeafs) {
  ...
  BinaryTree::LeafIndexVector newLeafId(
      new Math::ReferenceCountedVector<BinaryTree::LeafNumber>(nLeafs_,
                                                               invalidId));
...
void BinaryTree::cutTree(Node *node, Id lastNode, Id cutoff,
