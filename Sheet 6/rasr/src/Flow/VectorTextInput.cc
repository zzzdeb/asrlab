...
protected:
  virtual void startDocument() { n_->timeInS_ = 0; }

  virtual void endDocument() { verify(!n_->out_); }
...
const Core::ParameterString
    VectorTextInputNodeBase::paramFileName("file", "input text file name");
const Core::ParameterFloat
    VectorTextInputNodeBase::paramLength("length", "frame length in s", 1, 0);
const Core::ParameterFloat
    VectorTextInputNodeBase::paramShift("shift", "frame shift in s", 0, 0);
const Core::ParameterInt
    VectorTextInputNodeBase::paramSampleRate("sample-rate", "sample-rate");

VectorTextInputNodeBase::VectorTextInputNodeBase(const Core::Configuration &c,
                                                 const Datatype *type)
    : Core::Component(c), SourceNode(c), timeInS_(0), shiftInS_(0),
      lengthInS_(0), sampleRate_(0), type_(type),
...
bool VectorTextInputNodeBase::setParameter(const std::string &name,
...
bool VectorTextInputNodeBase::work(PortId p) {
  if (!fileName_.empty()) {
    if (fileinfoChannel_.isOpen()) {
      fileinfoChannel_ << (Core::XmlEmpty("reading") +
                           Core::XmlAttribute("file", fileName_));
    }
