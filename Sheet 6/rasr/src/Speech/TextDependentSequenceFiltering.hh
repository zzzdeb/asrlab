...
/**
 *  Converts alignment into sequence selection and/or calculates ratio of the
 * not-filtered timeframes. Input: default port: alignment Outputs: default
 * port: vector-bool i.e. selection of remaining timeframes ratio port: f32,
 * i.e. #remaining / #all timeframes.
 *
 *  -Selection set types:
 ...
 *    -parameter 'symbols' is list of symbols inlcuded in the set (e.g. in
 * configuration files: phoneme-selection.symbols=a v f; in flow files:
 * phoneme-selection.symbols="a v f") -parameter 'complement': if true the
 * symbol set is complemented (e.g. in configuration files:
 *     state-selection.complement=true; in flow files:
 * state-selection.complement="true"); Note: for empty sets, 'complement' is set
 * automatically to 'true'. -Selection sets are in 'AND' relation, thus those
 * timeframes are selected whose allophone states are included in all sets.
 *  -Sequence selection object is a vector of bool values. Timeframes not
 * included in one of the selection sets are marked by false and timeframes
 * included in all selection sets are marked by true.
...
private:
  ...
  public:
     ...
     *  Note: initilization is not possible in the constructor, because of the
     * virtual functions. Calling of initialize is thus essential after having
     * set the parameters.
     */
    void initialize();
    ...
    virtual bool contains(const Am::AllophoneState &as,
                          s16 position = 0) const = 0;
    bool contains(Am::AllophoneStateIndex emission, s16 position = 0) const {
      return contains(allophoneStateAlphabet_->allophoneState(emission),
                      position);
    }
  ...
  private:
    /**
     *   All phonemes of the phoneme inventory of the lexicon are mapped to a
     * bool value. Workaround: bool is not supported by Bliss::PhonemeMap, so we
     * have use char now.
     */
  ...
  public:
    ...
    virtual bool contains(const Am::AllophoneState &as,
                          s16 position = 0) const {
      return Precursor::complementIfRequired(set_.find(as.state()) !=
                                             set_.end());
    }
