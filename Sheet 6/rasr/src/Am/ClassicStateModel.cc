...
const Core::ParameterInt Phonology::paramHistoryLength(
    "history-length", "maximum history length in context phonology", 1);
const Core::ParameterInt Phonology::paramFutureLength(
    "future-length", "maximum future length in context phonology", 1);
const Core::ParameterBool Phonology::paramCrossWord(
    "cross-word",
    "model context dependencies between words; at maximum one phoneme", true);

Phonology::Phonology(const Core::Configuration &config,
                     Bliss::PhonemeInventoryRef pi)
    : Precursor(pi, paramHistoryLength(config), paramFutureLength(config)),
      isCrossWord_(paramCrossWord(config)) {}

void Phonology::setCrossWord(bool isCrossWord) { isCrossWord_ = isCrossWord; }
...
const Core::ParameterString AllophoneAlphabet::paramAddFromFile(
    "add-from-file", "index all the allophones listed in <file>", "");
const Core::ParameterBool AllophoneAlphabet::paramAddFromLexicon(
    "add-from-lexicon", "index all allophones occuring in the current lexicon",
    true);
const Core::ParameterBool
    AllophoneAlphabet::paramAddAll("add-all", "index all possible allophones",
                                   false);
...
AllophoneAlphabet::AllophoneAlphabet(const Core::Configuration &config,
                                     ConstPhonologyRef phonology,
                                     Bliss::LexiconRef lexicon)
    : Fsa::Alphabet(), Core::Component(config), phonology_(phonology),
  ...
  if (paramAddAll(config)) {
    add(pi_);
    log("%d allophones after adding all allophones", (int)allophones_.size());
  }
  setSilence(lexicon);
}
...
AllophoneAlphabet::~AllophoneAlphabet() {
  ...
  if (!filename.empty()) {
    log("store %d allophones to \"%s\"", (int)allophones_.size(),
        filename.c_str());
    store(filename);
  }
  clear();
...
std::pair<AllophoneAlphabet::AllophoneMap::iterator, bool>
AllophoneAlphabet::insert(const Allophone *allo) const {
...
void AllophoneAlphabet::clear() {
  allophoneMap_.clear();
  for (AllophoneList::const_iterator a = allophones_.begin();
       a != allophones_.end(); ++a)
    delete *a;
  allophones_.clear();
  nDisambiguators_ = 0;
...
void AllophoneAlphabet::add(Core::Ref<const Bliss::PhonemeInventory> pi) {
  ...
  for (Core::tie(itPhoneme, endPhoneme) = pi->phonemes();
       itPhoneme != endPhoneme; ++itPhoneme) {
  ...
  Core::ProgressIndicator *progress =
      new Core::ProgressIndicator("index all allophones");
  progress->start();
  for (s32 i = 0; i <= phonology_->maximumFutureLength(); ++i) {
    for (s32 j = 0; j <= phonology_->maximumHistoryLength(); ++j) {
      for (;;) {
        ...
        for (PhonemeIdList::iterator itP = phonemeIds.begin();
             itP != phonemeIds.end(); ++itP) {
  ...
  for (PhonemeIdList::iterator itP = ciPhonemeIds.begin();
       itP != ciPhonemeIds.end(); ++itP) {
...
*/
std::pair<AllophoneAlphabet::AllophoneMap::iterator, bool>
AllophoneAlphabet::createAndInsert(const std::vector<bool> &cd,
                                   const Bliss::Pronunciation &p, s32 b, s32 c,
                                   s32 e, s16 boundary) const {
...
void AllophoneAlphabet::add(Bliss::LexiconRef lexicon) {
  ...
  Bliss::PhonemeInventory::PhonemeIterator itPhoneme, endPhoneme;
  for (Core::tie(itPhoneme, endPhoneme) = pi_->phonemes();
       itPhoneme != endPhoneme; ++itPhoneme)
  ...
  Core::ProgressIndicator *progress;
  progress =
      new Core::ProgressIndicator("index allophones from lexicon: within word");
  ...
  for (Core::tie(lexicon_pron, lexicon_pron_end) = lexicon->pronunciations();
    ...
    for (std::vector<s32>::const_iterator itLastPronId = contextChanges.begin();
      ...
      s16 initialFlag = (firstPronId == 0) ? Allophone::isInitialPhone
                                           : Allophone::isWithinPhone;
      s16 finalFlag = (lastPronId == maxPronId) ? Allophone::isFinalPhone
                                                : Allophone::isWithinPhone;
      ...
      if (firstPronId == lastPronId) {
        ...
        if (insAllo.second) {
          ...
          if (isLeftCw && isRightCw && (historyLength > 0) &&
              (futureLength > 0))
      ...
      if (historyLength == 0) {
        // initial allophone
        insAllo = createAndInsert(cd, p, firstPronId, firstPronId,
                                  std::min(e, lastPronId), initialFlag);
        ...
        for (++c, ++e; c < std::min(firstPronId + historyLength, lastPronId);
             ++c, ++e) {
          insAllo =
              createAndInsert(cd, p, firstPronId, c, std::min(e, lastPronId),
                              Allophone::isWithinPhone);
          if (insAllo.second) {
            if (isLeftCw)
              leftCwAllophones.push_back(insAllo.first->first);
      ...
      if (futureLength == 0) {
        ...
        // right cw allophones
        for (; c < lastPronId; ++b, ++c) {
          insAllo = createAndInsert(cd, p, b, c, lastPronId,
                                    Allophone::isWithinPhone);
  ...
  if (isCrossWord_) {
    typedef std::vector<Bliss::Phoneme::Id> PhonemeIdList;
    PhonemeIdList initialPhonemes(initialPhonemeSet.begin(),
                                  initialPhonemeSet.end());
    PhonemeIdList finalPhonemes(finalPhonemeSet.begin(), finalPhonemeSet.end());

    progress = new Core::ProgressIndicator(
        "index allophones from lexicon: across word");
    progress->start();
    // left cw allophones
...
void AllophoneAlphabet::load(const std::string &filename) {
  ...
  if (!allophones_.empty())
    warning("Indices of allophones in file \"%s\" seems NOT to match indices "
            "in current allophone alphabet",
            filename.c_str());
  Core::CompressedInputStream cis(filename);
  std::string line;
  Core::ProgressIndicator *progress =
      new Core::ProgressIndicator("index allophones from file");
  progress->start();
  while (cis) {
    ...
    if (!line.empty() && (*line.c_str() != '#')) {
      if (!insert(fromString(line)).second)
        warning(
            "Indices of allophones in file \"%s\" seems NOT to match indices "
            "in current allophone alphabet; allophone %s does already exist",
...
void AllophoneAlphabet::setSilence(Bliss::LexiconRef lexicon) {
  ...
  if (silLemma) {
    ...
    for (Core::tie(lemma_pron, lemma_pron_end) = silLemma->pronunciations();
      ...
      for (const Bliss::Phoneme::Id *itPhoneme =
               (*lemma_pron).pronunciation()->phonemes();
...
Allophone AllophoneAlphabet::fromString(const std::string &s) const {
  ...
  if (!phoneme)
    criticalError("unknown phoneme: '%s'",
                  std::string(c, std::string::size_type(d - c)).c_str());
...
void AllophoneAlphabet::writeXml(Core::XmlWriter &os) const {
  os << Core::XmlOpenComment() << allophones_.size() << " allophone labels, "
     << nDisambiguators_ << " disambiguation symbols" << Core::XmlCloseComment()
     << "\n";
...
void AllophoneStateIterator::resetState() const {
  if (itAllo_ == endAllo_)
    ...
    endState_ =
        asAlphabet_->hmmTopologies_->get((*itAllo_)->central())->nPhoneStates();
  state_ = 0;
}
...
AllophoneStateAlphabet::AllophoneStateAlphabet(
    ...
    : Fsa::Alphabet(), Core::Component(config),
      allophoneAlphabet_(allophoneAlphabet), hmmTopologies_(hmmTopologies) {}

Fsa::LabelId AllophoneStateAlphabet::nClasses() const {
  Fsa::LabelId nStates = 0;
  for (AllophoneAlphabet::AllophoneList::const_iterator itAllo =
           allophoneAlphabet_->allophones().begin();
...
Fsa::LabelId
AllophoneStateAlphabet::index(const AllophoneState &alloState) const {
  return allophoneAlphabet_->index(*alloState.allophone()) | alloState.state()
                                                                 << 26;
}

Fsa::LabelId AllophoneStateAlphabet::index(const Allophone *allo,
                                           s16 state) const {
...
AllophoneState AllophoneStateAlphabet::allophoneState(Fsa::LabelId id) const {
  return AllophoneState(allophoneAlphabet_->allophone(id),
                        (id & StateMask) >> 26);
}

AllophoneState AllophoneStateAlphabet::allophoneState(const Allophone *allo,
                                                      s16 state) const {
  verify(allo && (state <= MaxStateId));
  return AllophoneState(allo, state);
}

std::pair<Fsa::LabelId, s16> AllophoneStateAlphabet::allophoneIndexAndState(
    Fsa::LabelId allophoneStateId) const {
  return std::make_pair(allophoneStateId & AllophoneAlphabet::IdMask,
                        (allophoneStateId & StateMask) >> 26);
}

std::string
AllophoneStateAlphabet::toString(const AllophoneState &alloState) const {
...
Fsa::LabelId AllophoneStateAlphabet::index(const std::string &symbol) const {
  ...
  // Example:
  // a{b+c}@i@f.10
  // <allo-center><allo-context-delimiter
  // [{]><allo-left><allo-neighbor-delimiter
  // [+]><allo-right><allo-context-delimiter [}]>[@i][@f]<state-delimiter
  // [.]><state-number>

  // we search for last occurrence of '.' in order to allow more than 9 HMM
  // states
  ...
  // lengths!
  ensure((found != std::string::npos) &&
         (found > 5)); // '5' depends again on other delimiter lengths
  DBG(DEBUG_ALLOPHONE_INFO) << VAR(symbol.substr(0, found)) << " "
                            << VAR(symbol.substr(found + 1)) << ENDDBG;
...
std::pair<AllophoneStateIterator, AllophoneStateIterator>
AllophoneStateAlphabet::allophoneStates() const {
...
void AllophoneStateAlphabet::writeXml(Core::XmlWriter &os) const {
  os << Core::XmlOpenComment() << nClasses()
     << " allophone state labels, "
...
struct IndexedAllophoneState {
  ...
  bool operator<(const IndexedAllophoneState &i) const { return id < i.id; }
};
...
void AllophoneStateAlphabet::dump(Core::XmlWriter &xml) const {
  ...
  for (std::pair<AllophoneStateIterator, AllophoneStateIterator> it =
    ...
    indexedAllophoneStates.push_back(
        IndexedAllophoneState(it.first.id(), it.first.allophoneState()));
  std::sort(indexedAllophoneStates.begin(), indexedAllophoneStates.end());
  xml << Core::XmlOpen("allophone-state-list");
  for (IndexedAllophoneStateList::const_iterator it =
           indexedAllophoneStates.begin();
       it != indexedAllophoneStates.end(); ++it)
    xml << Core::form("%9d. %s\n", it->id,
                      toString(it->allophoneState).c_str());
...
ClassicStateModel::ClassicStateModel(
    ...
    HmmTopologySetRef hmmTopologySetRef, std::vector<std::string> conditions)
    : phonologyRef_(phonologyRef), piRef_(phonologyRef->getPhonemeInventory()),
      allophoneAlphabetRef_(allophoneAlphabetRef),
      allophoneStateAlphabetRef_(allophoneStateAlphabetRef),
      hmmTopologySetRef_(hmmTopologySetRef), conditions_(conditions) {
  verify(phonologyRef_ && allophoneAlphabetRef_ && allophoneStateAlphabetRef_ &&
         hmmTopologySetRef_);
}

AllophoneState ClassicStateModel::allophoneState(const Allophone *allo,
                                                 s16 state) const {
  return allophoneStateAlphabetRef_->allophoneState(allo, state);
}

const ClassicHmmTopology *
ClassicStateModel::hmmTopology(const Allophone *allo) const {
