...
public:
    ...
    LinObj(std::shared_ptr<BaseT> data, std::valarray<size_t> shape, const size_t& start = 0) : data(data) {
    ...
    void operator+=(const LinObj& o) { (*data)[slice] += o.get(); }
...
public:
    ...
    Vector sumx() const;
    Vector at(size_t i, size_t j) const;
    // for(size_t j = 0; j < seq_len; j++) {
    //   auto outoffset = slice.start() +
    //   inoffset =
    //   auto out = output[];
    //   auto in = input_buffer_[std::slice(,,1)];
