...
const Core::ParameterBool
    DeterminizeNode::paramToLogSemiring("log-semiring", "use log semiring",
                                        false);
const Core::ParameterFloat DeterminizeNode::paramAlpha(
    "alpha", "scale dimensions for posterior calculation", 0.0);

NodeRef createDeterminizeNode(const std::string &name,
...
protected:
  virtual ConstLatticeRef filter(ConstLatticeRef l) { return minimize(l); }
...
NodeRef createMinimizeNode(const std::string &name,
                           const Core::Configuration &config) {
