 ...
 *   -Filters have a central point and relative to this a left(negative number)
 * and a right border. Outside of the borders (f < left-border and f >
 * right-border) filter-weights are zero. Left- and right-borders are configured
 * by 'normalizedCenterPosition', @see FilterBuilder. -Filters are placed in a
 * constant distant given by the spacing.
 *   @param shouldWarpCenterPositions_ controls over which axis the spacing is
 * constant. -yes:filters are placed in a constant distant over the warped axis.
 *    -no:filters are placed in a constant distant over the original unwarped
 * axis. Implementation is done by simply pre-warping the center position, since
 * the inverse warping applied which building the filters will yield equidistant
 * filters over the original unwarped axis. Note that the filter widths are
 * warped in this cases. -If spacing is set to zero, it will be set
 * automatically to normalized-center * filter-width, i.e. the size of the left
 * flank. -Parts of filters reaching out of the interval [0..maximal-frequency]
 * are assumed to be zero. -Position of first and last filters are controlled by
 * different boundary type classes,
...
public:
  ...
  void setWarpingFunction(const std::string &name,
                          Math::UnaryAnalyticFunctionRef f) {
    require(f);
 ...
 *
 *   Parameters:
 *     warping-function: is a string containing a declaration of a function
 * conform to Math::AnalyticFunctionFactory. Example: nest(linear-2(1.2, 0.875),
 * mel) This node class is derived from Flow::StringExpressionNode, thus it
 * supports parameter resolving with values arriving one or more ports. Example:
 *                       nest(linear-2($input(warping-factor), 0.875), mel). In
 * this example the node creates an input port called warping-factor and the
 * values arriving will be substituted to the placeholder
 * $input(warping-factor). The main input of the node and the different
 * "parameter-ports" are synchronized, thus time-stamps of the main port are
 * ensured to fall within the start and end times of the current parameter
 * value.
 *
 */
