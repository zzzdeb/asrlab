...
const LabelIdStrings::Id LabelIdStrings::Invalid =
    Core::Type<LabelIdStrings::Id>::max;

LabelIdStrings::Id
LabelIdStrings::append(Core::Vector<LabelId>::const_iterator i) {
  Id output = start();
...
void LabelIdStrings::dump(std::ostream &os, Id id,
                          ConstAlphabetRef alphabet) const {
  if (alphabet)
...
QualifiedFilename splitQualifiedFilename(const std::string &file,
                                         const std::string &defaultQualifier) {
  std::string::size_type colonPos = file.find(':');
  if (colonPos == std::string::npos)
    return QualifiedFilename(defaultQualifier, file);
  else
    return QualifiedFilename(file.substr(0, colonPos),
                             file.substr(colonPos + 1));
}
} // namespace Fsa
