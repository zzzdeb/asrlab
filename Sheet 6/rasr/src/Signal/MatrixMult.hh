...
// $Id: MatrixMult.hh 7257 2009-07-10 12:17:00Z rybach $
...
const Core::ParameterString
    paramMatrixMultiplicationFileName("file", "name of matrix file to load");
...
const Core::ParameterBool paramIgnoreTimestamps(
    "ignore-timestamps",
    "ignore timestamps, i.e. update matrix for every frame", false);
const Core::ParameterInt paramMultipleDataProcessing(
    "multi-data",
    "multiple data processing, i.e. more than one vector at a time, this "
    "parameter can only be set using a config file",
    1);
 ...
 *   Since MatrixMultiplicationNode derives from Flow::StringExpressionNode, @c
 * file can contain placeholders for input ports. Example:
 * file="$input(warping-factor).matrix". In this case, node has got an extra
 * input port called warping-factor. The placeholder $input(warping-factor) will
 * be replaced by the values which arrives on the input port. For more details
 ...
 *   The result of the multiplication of input vector 0 is then available at
 * output port 0. Not that all vectors will multiplied with the very same
 * matrix.
 *
 * Inputs: one or more
 * Outputs: one or more
...
MatrixMultiplicationNode<T>::MatrixMultiplicationNode(
    ...
    : Component(c), Node(c),
      StringExpressionNode(c, paramMultipleDataProcessing(c)), matrixCache_(0),
...
template <class T> MatrixMultiplicationNode<T>::~MatrixMultiplicationNode() {
  delete matrixCache_;
}
...
template <class T> bool MatrixMultiplicationNode<T>::configure() {
...
template <class T>
bool MatrixMultiplicationNode<T>::setParameter(const std::string &name,
                                               const std::string &value) {
  if (paramMatrixMultiplicationFileName.match(name))
    Flow::StringExpressionNode::setTemplate(
        paramMatrixMultiplicationFileName(value));
...
template <class T> bool MatrixMultiplicationNode<T>::work(Flow::PortId p) {
  ...
  for (s32 i = 0; i < multiData_; ++i) {
    if (inputConnected((Flow::PortId)i)) {
      if (in[portNumber]->size() != currentMatrix_->nColumns()) {
        error() << "vector/matrix dimension mismatch: vector(at port "
                << portNumber << " ): " << in[portNumber]->size() << ", matrix "
                << currentMatrix_->nColumns() << " columns";
        return putDataAllPorts(Flow::Data::eos());
      } // end inner if
      ++portNumber;
    } // end connected if
  ...
  for (s32 i = 0; i < multiData_; ++i) {
    if (nOutputLinks((Flow::PortId)i)) {
      Flow::Vector<T> *out =
          new Flow::Vector<T>((*currentMatrix_) * (*(in[portNumber])));
