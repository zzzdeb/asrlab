...
struct CorpusStatisticsVisitor::SentenceStatistics {
  ...
  SentenceStatistics()
      : nLemmas(0), nSyntacticTokens(0), avgEvaluationTokens(0.0),
        nUnkLemmas(0), nUnkSyntacticTokens(0), avgUnkEvaluationTokens(0.0),
        score(0.0) {}
  ...
  f32 syntPerplexity() const {
    return (nSyntacticTokens) ? exp(score / f32(nSyntacticTokens))
                              : Core::Type<f32>::max;
  }
  f32 syntCrossEntropy() const {
    return (nSyntacticTokens) ? score / f32(nSyntacticTokens) / ln2
                              : Core::Type<f32>::max;
  }
  f32 evalPerplexity() const {
    return (avgEvaluationTokens) ? exp(score / avgEvaluationTokens)
                                 : Core::Type<f32>::max;
  }
  f32 evalCrossEntropy() const {
    return (avgEvaluationTokens) ? score / avgEvaluationTokens / ln2
                                 : Core::Type<f32>::max;
  }
};
...
struct CorpusStatisticsVisitor::TextStatistics
  ...
  TextStatistics() : SentenceStatistics(), nSentences(0) {}
...
const Core::ParameterBool CorpusStatisticsVisitor::paramIgnoreUnknowns(
    "ignore-unknowns", "ignore lemmas containing unknown syntactic token(s)",
    true);

const Core::ParameterBool
    CorpusStatisticsVisitor::paramUseClassEmissionProbabilities(
        "use-class-emission-probabilities", "use class emission probabilities",
        true);

CorpusStatisticsVisitor::CorpusStatisticsVisitor(const Core::Configuration &c,
                                                 Bliss::LexiconRef l)
    : Bliss::CorpusStatisticsVisitor(c), lexicon_(l),
      sentenceChannel_(c, "sentence"), orthographicParser_(c, l) {
  ...
  what_ = (paramUseClassEmissionProbabilities(config)) ? ReportLanguageModel
                                                       : ReportClassModel;
 ...
 * tokens.
 */
CorpusStatisticsVisitor::SentenceStatistics
CorpusStatisticsVisitor::buildSentenceStatistics(
    Fsa::ConstAutomatonRef sentence, Lm::Score syntaxEmissionScale,
  ...
  Core::Ref<const Bliss::LemmaAlphabet> lemmaAlphabet =
      ref(dynamic_cast<const Bliss::LemmaAlphabet *>(
          sentence->getInputAlphabet().get()));
  std::vector<const Bliss::Lemma *> lemmas;
  for (std::vector<Fsa::LabelId>::const_iterator li = labels.begin();
       li != labels.end(); ++li)
    lemmas.push_back(lemmaAlphabet->lemma(*li));

  // process LM events
  History h = lm_->startHistory();
  for (std::vector<const Bliss::Lemma *>::const_iterator ll = lemmas.begin();
       ll != lemmas.end(); ++ll) {
    const Bliss::SyntacticTokenSequence syntacticTokens(
        (*ll)->syntacticTokenSequence());
    ...
    for (Core::tie(e, e_end) = (*ll)->evaluationTokenSequences(); e != e_end;
         ++e) {
    ...
    f32 avgEvaluationTokens =
        nEvaluationTokenSequences
            ? f32(nEvaluationTokens) / f32(nEvaluationTokenSequences)
            : 0;
    bool hasAnythingToReport =
        (nSyntacticTokens > 0) || (nEvaluationTokens > 0);

    if (sentenceChannel_.isOpen() && hasAnythingToReport)
      sentenceChannel_ << (Core::XmlOpen("lemma") +
    ...
    for (u32 ti = 0; ti < syntacticTokens.length(); ++ti) {
      ...
      if (sentenceChannel_.isOpen()) {
        sentenceChannel_ << Core::XmlOpen("event")
                         << Core::XmlFull("token", st->symbol());
        if (classMapping_)
          sentenceChannel_ << Core::XmlFull(
              "class", classMapping_->classToken(st)->symbol());
        sentenceChannel_ << Core::XmlFull("history", lm_->formatHistory(h))
    ...
    if (sentenceChannel_.isOpen() && hasAnythingToReport)
      sentenceChannel_ << Core::XmlFull("words", avgEvaluationTokens)
  ...
  if (sentenceChannel_.isOpen()) {
    sentenceChannel_ << Core::XmlOpen("sentence-end") << Core::XmlOpen("event");
    if (lm_->sentenceEndToken()) {
      sentenceChannel_ << Core::XmlFull("token",
                                        lm_->sentenceEndToken()->symbol());
      if (classMapping_)
        sentenceChannel_ << Core::XmlFull("class",
                                          lm_->sentenceEndToken()->symbol());
    } else
      sentenceChannel_ << Core::XmlFull("token", "n/a");
    sentenceChannel_ << Core::XmlFull("history", lm_->formatHistory(h))
...
void CorpusStatisticsVisitor::visitSpeechSegment(
    Bliss::SpeechSegment *segment) {
  sentenceChannel_ << Core::XmlOpen("orth") +
                          Core::XmlAttribute("source", "reference")
                   << segment->orth() << Core::XmlClose("orth");

  Fsa::ConstAutomatonRef sentence =
      orthographicParser_.createLemmaAcceptor(segment->orth());

  if (what_ & ReportLanguageModel) {
    if (sentenceChannel_.isOpen())
      sentenceChannel_ << (Core::XmlOpen("sentence-statistics") +
                           Core::XmlAttribute("model", "language model"));
    SentenceStatistics lmStats = buildSentenceStatistics(
        sentence, syntaxEmissionScale_, classEmissionScale_);
  ...
  if (what_ & ReportClassModel) {
    if (sentenceChannel_.isOpen())
      sentenceChannel_ << (Core::XmlOpen("sentence-statistics") +
                           Core::XmlAttribute(
                               "model",
                               (classLm_ ? "class model" : "syntax model")));
...
void CorpusStatisticsVisitor::writeReport(Core::XmlWriter &xml,
                                          TextStatistics &stats) const {
...
void CorpusStatisticsVisitor::writeReport(Core::XmlWriter &xml) const {
  if (what_ & ReportLanguageModel) {
    xml << (Core::XmlOpen("statistics") +
            Core::XmlAttribute("model", "language model"));
    writeReport(xml, *lmStats_);
    xml << Core::XmlClose("statistics");
  }
  if (what_ & ReportClassModel) {
    xml << (Core::XmlOpen("sentence-statistics") +
            Core::XmlAttribute("model",
                               (classLm_ ? "class model" : "syntax model")));
