...
public:
  ...
  public:
    Reader(const Core::Configuration &c) : Core::Component(c) {}
    virtual ~Reader() {}
    virtual bool read(const std::string &filename,
                      MixtureSetRef &result) const = 0;
    template <class T> static Reader *create(const Core::Configuration &c) {
      return new T(c);
    }
  };
  /**
  ...
  public:
    RefReader(const Core::Configuration &c) : Reader(c) {}
    virtual ~RefReader() {}
  ...
  public:
    FormatReader(const Core::Configuration &c) : Reader(c) {}
  ...
  public:
    CmfReader(const Core::Configuration &c) : Reader(c) {}
    virtual ~CmfReader() {}
    virtual bool read(const std::string &filename, MixtureSetRef &result) const;
  };
  /**
  ...
  public:
    LambdaReader(const Core::Configuration &c) : Reader(c) {}
    virtual ~LambdaReader() {}
    virtual bool read(const std::string &filename, MixtureSetRef &result) const;
    Core::Ref<LogLinearMixtureSet>
    readLogLinearMixtureSet(const std::string &filename) const;
  };
  /**
  ...
  public:
    MixtureSetEstimatorReader(const Core::Configuration &c) : Reader(c) {}
    virtual ~MixtureSetEstimatorReader() {}
    virtual bool read(const std::string &filename, MixtureSetRef &result) const;
    bool readMixtureSetEstimator(const std::string &filename,
                                 AbstractMixtureSetEstimator &estimator) const;
  };
...
private:
   ...
   * filename extension
   */
  template <class T> void registerReader(const std::string &id) {
    reader_->registerClass(id, Reader::create<T>);
  }
};

template <class MixtureType>
Core::Ref<MixtureType>
MixtureSetReader::read(const std::string &filename) const {
  MixtureSetRef result;
  Reader *reader =
      reader_->getObject(Core::filenameExtension(filename), config);
  if (!reader) {
