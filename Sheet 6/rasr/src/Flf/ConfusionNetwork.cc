...
const Core::ParameterString paramAlgorithm("algorithm", "algorithm",
                                           "arc-clustering");
} // namespace
...
ConfusionNetworkFactoryRef
ConfusionNetworkFactory::create(const Core::Configuration &config) {
  ...
  if (cnAlgorithmChoice == Core::Choice::IllegalValue)
    Core::Application::us()->criticalError("Unknwon CN algorithm \"%s\".",
  ...
  case CnAlgorithmStateClustering:
    Core::Application::us()->criticalError("Algorithm not supported yet.");
    break;
...
ConfusionNetworkFactoryRef
ConfusionNetworkFactory::create(CnAlgorithmType cnAlgorithm) {
  ConfusionNetworkFactory *cnFactory = 0;
  switch (cnAlgorithm) {
  case CnAlgorithmStateClustering:
    Core::Application::us()->criticalError("Algorithm not supported yet.");
...
// -------------------------------------------------------------------------
ConstConfusionNetworkRef sausageLattice2cn(ConstLatticeRef l,
                                           bool keepEpsArcs) {
  ...
  for (ConstStateRef sr = l->getState(l->initialStateId()), nextSr;
    ...
    bool hasSlot = false;
    for (State::const_iterator a = sr->begin(), end_a = sr->end(); a != end_a;
         ++a) {
      ...
      if (keepEpsArcs || (a->input() != Fsa::Epsilon)) {
        ...
        cn->back().push_back(
            ConfusionNetwork::Arc(a->input(), a->weight(), begin, duration,
...
private:
  Score calcConfidence(ScoreList::const_iterator itP,
                       ScoreList::const_iterator endP) const {
    switch (u32(endP - itP)) {
    case 0:
      return 1.0;
    case 1:
...
private:
  ...
  public:
    struct SortByLabel {
      bool operator()(const ConfusionNetwork::Arc *arc1,
                      const ConfusionNetwork::Arc *arc2) const {
...
public:
  ConstConfusionNetworkRef normalize(ConstConfusionNetworkRef cn,
                                     ScoreId posteriorId) const {
    if (!cn)
      return ConstConfusionNetworkRef();
    if (posteriorId == Semiring::InvalidId)
      Core::Application::us()->criticalError(
          "No dimension storing a posterior probability distribution is given; "
          "in order to normalize a CN a prob. dist. over all arcs in the slot "
          "is required, where lacking prob. mass is assigned to eps.");
    ConfusionNetwork *normCn = new ConfusionNetwork(cn->size());
    normCn->alphabet = cn->alphabet;
    normCn->semiring = cn->semiring;
    normCn->normalizedProperties =
    ...
    for (u32 i = 0; i < normCn->size(); ++i) {
      ...
      if (!slot.empty()) {
        ...
        for (ConfusionNetwork::Slot::const_iterator itArc = slot.begin(),
                                                    endArc = slot.end();
             itArc != endArc; ++itArc, ++itArcPtr)
          *itArcPtr = &*itArc;
        std::sort(arcPtrs.begin(), arcPtrs.begin() + slot.size(),
                  ArcPtrList::SortByLabel());
        ...
        for (ArcPtrList::const_iterator
                 itArcPtr = arcPtrs.begin() + 1,
                 endArcPtr = arcPtrs.begin() + slot.size();
        ...
        if (sum != 1.0) {
            ...
            if (normSlot.front().label == Fsa::Epsilon) {
              ...
              normSlot.insert(
                  ...
                  ConfusionNetwork::Arc(
                      Fsa::Epsilon, scores,
                      (normSlot.front().begin + normSlot.front().duration) / 2,
                      0));
            } else
              normalize = true;
          }
          if (normalize) {
            Score norm = 1.0 / sum;
            for (ConfusionNetwork::Slot::iterator itArc = normSlot.begin(),
                                                  endArc = normSlot.end();
                 itArc != endArc; ++itArc)
        ...
        normSlot.push_back(ConfusionNetwork::Arc(
            Fsa::Epsilon, scores, Speech::InvalidTimeframeIndex, 0));
      }
    }
    if (cn->hasMap()) {
      ConfusionNetwork::MapProperties *mapProperties =
          new ConfusionNetwork::MapProperties(*cn->mapProperties);
      mapProperties->reduce(); // normalized CN contains only "lattice-state+arc
                               // -> slot" information
      normCn->mapProperties =
  ...
  public:
      MaximumPosteriorProbabilityDecoder(ConstConfusionNetworkRef cn, ScoreId
  posteriorId, const ScoreIdList systemPosteriorIds) : cn_(cn),
  posteriorId_(posteriorId) {} virtual Fsa::LabelId label(u32 iSlot) const {
      }
  };
  ...
  public:
      InversePosteriorProbabilityEntropyDecoder(ConstConfusionNetworkRef cn,
  ScoreId posteriorId, const ScoreIdList systemPosteriorIds) : cn_(cn),
  posteriorId_(posteriorId), systemPosteriorIds_(systemPosteriorIds),
  ...
  */
  ...
  ConstLatticeRef decode(ConstConfusionNetworkRef cn,
                         ScoreId posteriorId) const {
    if (!cn)
      return ConstLatticeRef();
    if (posteriorId == Semiring::InvalidId) {
    ...
    verify((posteriorId != Semiring::InvalidId) && cn->isNormalized() &&
           (cn->normalizedProperties->posteriorId == posteriorId));
    ...
    for (u32 i = 0; i < cn->size(); ++i) {
      ...
      for (ConfusionNetwork::Slot::const_iterator itPdf = pdf.begin();
           itPdf != pdf.end(); ++itPdf) {
      ...
      if (itBestPdf->label != Fsa::Epsilon) {
        if (preferredEndTime < itBestPdf->begin) {
          ...
          ScoresRef scores = semiring->clone(semiring->one());
          scores->set(posteriorId, calcConfidence(pendingEpsPosteriors.begin(),
                                                  pendingEpsPosteriors.end()));
          ...
          preferredEndTime =
              std::max(minEndTime, (preferredEndTime + itBestPdf->begin) / 2);
        b->set(sp->id(), Boundary(preferredEndTime));
        sp->newArc(sp->id() + 1, itBestPdf->scores, itBestPdf->label,
                   itBestPdf->label);
        sp = new Flf::State(sp->id() + 1);
        s->setState(sp);
        minEndTime = preferredEndTime + 1;
        preferredEndTime =
            std::max(minEndTime, itBestPdf->begin + itBestPdf->duration);
      } else
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(0) && !connected(1))
      criticalError(
          "Require either CN at port 0 or sausage lattice at port 1.");
...
const Core::ParameterString
    CnDecoderNode::paramPosteriorKey("posterior-key", "posterior key", "");
NodeRef createCnDecoderNode(const std::string &name,
...
CnFeatureConfiguration::CnFeatureConfiguration()
    : cn(), cnPosteriorId(Semiring::InvalidId), compose(false),
      duplicateOutput(false), confidenceId(Semiring::InvalidId),
      scoreId(Semiring::InvalidId), errId(Semiring::InvalidId),
      oracleOutput(false), slotEntropyId(Semiring::InvalidId),
      slotId(Semiring::InvalidId), nonEpsSlotId(Semiring::InvalidId),
...
public:
  CnFeatures(CnFeatureConfiguration cnConfig) : cnConfig_(cnConfig) {
    if (!cnConfig_.cn || !cnConfig_.cn->hasMap())
      Core::Application::us()->criticalError(
          "Confusion Network with lattice/CN map required.");
    if ((cnConfig_.confidenceId != Semiring::InvalidId) ||
        (cnConfig_.scoreId != Semiring::InvalidId) ||
        (cnConfig_.errId != Semiring::InvalidId) || (cnConfig_.oracleOutput) ||
      ...
      if ((cnConfig_.errId != Semiring::InvalidId) || cnConfig_.oracleOutput) {
        ...
        if (!cnConfig_.cn->isOracleAlignment())
          Core::Application::us()->criticalError(
              "Confusion Network with oracle alignment required.");
      }
      ...
      if ((cnConfig_.confidenceId != Semiring::InvalidId) ||
        ...
        if (!cnConfig_.cn->isNormalized()) {
          if (cnConfig_.cnPosteriorId == Semiring::InvalidId)
            Core::Application::us()->criticalError(
                "Posterior dimension for confusion network normalization "
                "required.");
          normalizedCn_ = normalizeCn(cnConfig_.cn, cnConfig_.cnPosteriorId);
        } else {
          cnConfig_.cnPosteriorId =
              cnConfig_.cn->normalizedProperties->posteriorId;
          normalizedCn_ = cnConfig_.cn;
        }
        needsPosterior_ = (cnConfig_.confidenceId != Semiring::InvalidId) ||
                          (cnConfig_.scoreId != Semiring::InvalidId);
      }
      // mapping to eps-slot free CN required
      if (cnConfig_.nonEpsSlotId != Semiring::InvalidId) {
        slotIdToNonEpsilonSlotIdMap_.resize(cnConfig_.cn->size(),
                                            Fsa::InvalidStateId);
        Fsa::StateId nonEpsSlotId = 0;
        std::vector<Fsa::StateId>::iterator itNonEpsilonSlotId =
            slotIdToNonEpsilonSlotIdMap_.begin();
        for (ConfusionNetwork::const_iterator
                 itSlot = cnConfig_.cn->begin(),
                 endSlot = cnConfig_.cn->end(),
                 itNormalizedSlot = normalizedCn_->begin();
             itSlot != endSlot;
             ++itSlot, ++itNormalizedSlot, ++itNonEpsilonSlotId) {
          if ((itNormalizedSlot->front().label == Fsa::Epsilon) &&
              (itNormalizedSlot->front().scores->get(cnConfig_.cnPosteriorId) >=
               cnConfig_.epsSlotThreshold))
            continue;
  ...
  const CnFeatureConfiguration &cnConfig() const { return cnConfig_; }

  const ConfusionNetwork &cn() const { return *cnConfig_.cn; }
  ...
  bool needsModification() const { return needsModification_; }
  ...
  Score cost(Fsa::LabelId hyp, Fsa::LabelId ref) const {
    return (hyp == ref) ? CorCost
                        : ((ref == Fsa::Epsilon)
                               ? InsCost
                               : ((hyp == Fsa::Epsilon) ? DelCost : SubCost));
  }

  void modify(Arc &arc, Fsa::StateId slotId, Fsa::StateId fromSlotId,
              Fsa::StateId toSlotId) const {
    ...
    verify((fromSlotId != Fsa::InvalidStateId) &&
           (toSlotId != Fsa::InvalidStateId));
    verify((fromSlotId <= slotId) && (slotId <= toSlotId) &&
           (toSlotId < cnConfig_.cn->size()));
    // oracle based features
    if (needsOracle_) {
      const ConfusionNetwork::OracleAlignmentProperties &props =
          oracleProperties();
    ...
    if (needsPosterior_) {
      const ConfusionNetwork::NormalizedProperties &props =
          normalizedProperties();
      // slot confidence
      if (cnConfig_.confidenceId != Semiring::InvalidId) {
        Score posterior =
            props.posteriorScore((*normalizedCn_)[slotId], arc.input());
        arc.weight()->set(cnConfig_.confidenceId, posterior);
      }
      // score over spanned slots
      if (cnConfig_.scoreId != Semiring::InvalidId) {
        Score score = 0.0;
        for (Fsa::StateId thisSlotId = fromSlotId; thisSlotId <= toSlotId;
             ++thisSlotId) {
...
// -------------------------------------------------------------------------
ConstLatticeRef composeAndAddCnFeatures(ConstLatticeRef l,
                                        const CnFeatures &cnFeatures) {
  if (!l)
    return ConstLatticeRef();
  StaticLattice *s = new StaticLattice;
  StaticBoundaries *b = new StaticBoundaries;
  ...
  for (Fsa::StateId sid = 0; sid < s->size(); ++sid) {
    ...
    if (!sp)
      continue;
    if (sp->hasArcs()) {
      ConfusionNetwork::MapProperties::Map::const_iterator itMap =
          cnFeatures.mapProperties().state(sp->id());
      for (State::const_iterator a = sp->begin(), end = sp->end(); a != end;
           ++a, ++itMap) {
  ...
  if (!cnFeatures.cnConfig().compose) {
    ...
    for (Fsa::StateId sid = 0; sid < s->size(); ++sid) {
      ...
      ConfusionNetwork::MapProperties::Map::const_iterator itMap =
          cnFeatures.mapProperties().state(sp->id());
      for (State::iterator a = sp->begin(), end = sp->end(); a != end;
           ++a, ++itMap) {
        Fsa::StateId slotId = itMap->sid;
        ...
        cnFeatures.modify(
            *a, slotId,
            ...
            (nextSlotId == Core::Type<Fsa::StateId>::max) ? slotId
                                                          : nextSlotId - 1);
      }
    }
  } else {
    s->setDescription(
        Core::form("composeAndAddCnFeatures(%s)", s->describe().c_str()));
    ...
    for (Fsa::StateId sid = 0; sid < endSid; ++sid) {
      ...
      Fsa::StateId fromSlotId = minOut[sid];
      verify(fromSlotId !=
             Core::Type<Fsa::StateId>::max); // can happen, if lattice has
                                             // (many) arcs of length 0
      ConfusionNetwork::MapProperties::Map::const_iterator itMap =
          cnFeatures.mapProperties().state(sp->id());
      for (State::iterator a = sp->begin(), end = sp->end(); a != end;
           ++a, ++itMap) {
        ...
        if ((fromSlotId != slotId) || (slotId != toSlotId)) {
          const Arc tmpArc = *a;
          Fsa::LabelId epsOutputLabel =
              (duplicateOutput && (l->type() != Fsa::TypeAcceptor))
                  ? tmpArc.output()
                  : Fsa::Epsilon;
          ...
          if (thisSlotId < slotId) {
            ...
            for (++thisSlotId; thisSlotId < slotId; ++thisSlotId) {
              ...
              s->setState(nextSp);
              b->set(nextSp->id(), fromBoundary);
              thisArc = thisSp->newArc(nextSp->id(), semiring.one(),
                                       Fsa::Epsilon, epsOutputLabel);
              cnFeatures.modify(*thisArc, thisSlotId);
              thisSp = nextSp;
            }
          ...
          if (thisSlotId < toSlotId) {
            ...
            for (++thisSlotId; thisSlotId <= toSlotId; ++thisSlotId) {
              ...
              thisArc->target_ = nextSp->id();
              thisArc = nextSp->newArc(Fsa::InvalidStateId, semiring.one(),
                                       Fsa::Epsilon, epsOutputLabel);
              cnFeatures.modify(*thisArc, thisSlotId);
            }
...
ConstLatticeRef
composeAndAddCnFeatures(ConstLatticeRef l,
                        const CnFeatureConfiguration &cnConfig) {
...
protected:
  ConstLatticeRef filter(ConstLatticeRef l) {
    ...
    if (!lastCnSemiring_ ||
        (lastCnSemiring_.get() != cnConfig_.cn->semiring.get())) {
      lastCnSemiring_ = cnConfig_.cn->semiring;
      cnConfig_.cnPosteriorId = lastCnSemiring_->id(cnPosteriorKey_);
    }
    if (!cnConfig_.semiring ||
        (cnConfig_.semiring.get() != l->semiring().get())) {
  ...
  void init(const std::vector<std::string> &arguments) {
    ...
    if (cnConfig_.compose) {
      ...
      if (cnConfig_.duplicateOutput)
        msg << "Preserve output-label (in some configurations, the output "
               "label identifies the system the arcs comes from)\n";
    }
    cnPosteriorKey_ = paramPosteriorKey(select("cn"));
    if (cnPosteriorKey_.empty())
      msg << "CN posterior key: " << cnPosteriorKey_ << "\n";
    // else criticalError("Definition of a key pointing at a posterior
    // probability distribution in the CN is mandatory.");
    ...
    if (cnConfig_.oracleOutput && cnConfig_.duplicateOutput)
      criticalError(
          "Oracle output and duplicate output are mutually exclusive.");
  }
};
const Core::ParameterString
    CnFeatureNode::paramPosteriorKey("posterior-key", "posterior key", "");
const Core::ParameterString CnFeatureNode::paramKey("key", "key", "");
const Core::ParameterBool
    CnFeatureNode::paramOracleOutput("oracle-output",
                                     "store oracle as output label", false);
const Core::ParameterFloat CnFeatureNode::paramThreshold("threshold",
                                                         "threshold");
const Core::ParameterBool CnFeatureNode::paramCompose("compose", "compose",
                                                      false);
const Core::ParameterBool
    CnFeatureNode::paramDuplicateOutput("duplicate-output",
                                        "duplicate output labels", false);
NodeRef createCnFeatureNode(const std::string &name,
                            const Core::Configuration &config) {
