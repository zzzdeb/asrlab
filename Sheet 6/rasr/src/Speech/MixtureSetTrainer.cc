...
MixtureSetTrainer::MixtureSetTrainer(const Core::Configuration &configuration)
    : Core::Component(configuration), estimator_(0) {}
...
//     size_t nMixtures, size_t dimension, Mm::AbstractMixtureSetEstimator
//     &estimator)
// {
...
MixtureSetTrainer::getMixtureSet(size_t nMixtures, size_t dimension) {
  ...
  if (mixtureSet) {
    if (mixtureSet->nMixtures() != nMixtures) {
      error("Number of labels of alignment (%zd) do not match the number of "
            "mixtures (%d).",
            nMixtures, mixtureSet->nMixtures());
    }
    if (mixtureSet->dimension() != dimension) {
      error("Feature vector size (%zd) do not match the dimension in "
            "mixtures-set (%d).",
...
void MixtureSetTrainer::initializeAccumulation(
  ...
  clear();
  if (firstRun()) {
    estimator_ = Mm::Module::instance().createMixtureSetEstimator(
        config, nMixtures, dimension);
    if (!estimator_) {
...
bool MixtureSetTrainer::combine(const std::vector<std::string> &toCombine) {
  ...
  for (u32 n = 0; n < toCombine.size(); ++n) {
    if (newMixtureSetFilename == toCombine[n]) {
      error("<new-mixture-set-file> must be different from "
  ...
  // createMixtureSetEstimator(); 	if (read(toCombine, *estimator)) { 	    if
  // (estimator_->accumulate(*estimator)) { 		delete estimator; 		return true;
  // 	    }
  // 	} else {
  // 	    error("Failed to read mixture-set from \"%s\".", toCombine.c_str());
  // 	}
  // 	delete estimator;
  // 	error("Combination failed because mixture set '%s' has different
  // topology.", toCombine.c_str());
  //     } else {
...
bool MixtureSetTrainer::combinePartitions(
  ...
  if (Mm::Module_::paramCovarianceTyingType(config) ==
      Mm::Module_::pooledCovariance) {
...
void MixtureSetTrainer::write(const std::string &filename) const {
  if (estimator_) {
    if (filename.empty() ||
        !Mm::Module::instance().writeMixtureSetEstimator(filename, *estimator_))
...
void MixtureSetTrainer::setAssigningFeatureScorer(
  ...
  if (!assigningFeatureScorer) {
    assigningFeatureScorer_ =
        Mm::Module::instance().createAssigningFeatureScorer(config, mixtureSet);
  } else {
...
// 	error("Number of labels of alignment (%zd) do not match the number of
// mixtures (%d).", 	      nMixture, estimator_->nMixtures());
//     }
// }
...
// 	error("Feature vector size (%zd) do not match the dimension in
// mixtures-set (%d).", 	      dimension, estimator_->dimension());
//     }
// }
...
void MixtureSetTrainer::checkCovarianceTying() {
  ...
  case Mm::Module_::pooledCovariance:
    if (nCovariances != 1) {
      log("creating pooled covariance estimator using %d covariance estimators",
          nCovariances);
...
bool MixtureSetTrainer::read(const std::string &filename,
                             Mm::AbstractMixtureSetEstimator &estimator) {
  return (!filename.empty() &&
          Mm::Module::instance().readMixtureSetEstimator(filename, estimator));
}
...
Mm::MixtureSetEstimator *
MlMixtureSetTrainer::createMixtureSetEstimator() const {
