...
// -------------------------------------------------------------------------
class RecordingSegmentation : public std::vector<ConstSegmentRef>,
...
struct BoundedSegment {
  ...
  BoundedSegment()
      : segment(), offset(0), startFrame(Core::Type<s32>::max),
...
BoundedSegmentList buildBoundedSegments(const RecordingSegmentation &recSeg) {
  ...
  f32 recStartTime = recSeg.desc->startTime(),
      recEndTime = recSeg.desc->endTime();
  switch (recSeg.size()) {
  case 0:
    break;
  case 1:
    verify((recStartTime < recSeg.front()->endTime()) &&
           (recSeg.back()->startTime() < recEndTime));
  ...
  default:
    verify((recStartTime < recSeg.front()->endTime()) &&
           (recSeg.back()->startTime() < recEndTime));
    RecordingSegmentation::const_iterator itSegL = recSeg.begin(),
                                          itSeg = recSeg.begin() + 1,
                                          itSegR = recSeg.begin() + 2;
    BoundedSegmentList::iterator itBoundedSegL = boundedSegs.begin(),
                                 itBoundedSeg = boundedSegs.begin() + 1;
    itBoundedSegL->segment = *itSegL;
    itBoundedSegL->offset = 0;
    itBoundedSegL->startFrame = -nFrames(recStartTime, (*itSegL)->startTime());
    itBoundedSegL->endFrame =
        nFrames((*itSegL)->startTime(), (*itSegL)->endTime());
    for (; itSegR != recSeg.end(); ++itSeg, ++itSegR) {
      if ((*itSeg)->endTime() <= (*itSegL)->endTime()) {
        Core::Application::us()->warning(
            ...
            (*itSeg)->segmentIdOrDie().c_str(),
            (*itSegL)->segmentIdOrDie().c_str());
      } else {
        itBoundedSeg->segment = *itSeg;
        itBoundedSeg->offset =
            itBoundedSegL->offset +
            (itBoundedSegL->endFrame - itBoundedSegL->startFrame);
        itBoundedSeg->startFrame =
            -(nFrames(recStartTime, (*itSeg)->startTime()) -
              itBoundedSeg->offset);
        itBoundedSeg->endFrame =
            nFrames((*itSeg)->startTime(), (*itSeg)->endTime());
        itSegL = itSeg;
    ...
    if ((*itSeg)->endTime() <= (*itSegL)->endTime()) {
      ...
      itBoundedSeg->offset =
          ...
          (itBoundedSegL->endFrame - itBoundedSegL->startFrame);
      itBoundedSeg->startFrame = -(
          nFrames(recStartTime, (*itSeg)->startTime()) - itBoundedSeg->offset);
...
void dumpBoundedSegments(std::ostream &os,
                         const BoundedSegmentList &boundedSegs) {
  os << "[segment name]  [relative start-frame - relative end-frame]  [offset "
        "to recording start]\n";
  os << "----------------------------------------------------------------------"
        "------------------\n";
  for (BoundedSegmentList::const_iterator it = boundedSegs.begin();
       it != boundedSegs.end(); ++it)
    os << it->segment->segmentId() << "  " << it->startFrame << "-"
       << it->endFrame << "  " << it->offset << "\n";
}
// -------------------------------------------------------------------------
...
public:
  ...
  typedef Core::hash_map<std::string, RecordingSegmentationRef,
                         Core::StringHash>
      RecordingSegmentationMap;
...
public:
  ...
  bool insertRecording(Flf::ConstSegmentRef desc) {
    verify(desc->hasRecordingId() && desc->hasStartTime() &&
           desc->hasEndTime());
    RecordingSegmentationRef rec =
        RecordingSegmentationRef(new RecordingSegmentation(desc));
...
protected:
  bool init_(NetworkCrawler &crawler,
             const std::vector<std::string> &arguments) {
    if (!connected(0) || !connected(1))
      criticalError("Need segment sources at port 0 and port 1.");
    if (!crawler.init(in()[0].node, arguments) ||
        !crawler.init(in()[1].node, arguments))
      return false;
    NetworkCrawlerRef recSegCrawler =
        Module::instance().processor()->newCrawler();
    for (bool _good = true; _good;) {
      ConstSegmentRef segment = requestSegment(1);
      if (!recSegSet_.insertRecording(segment))
        criticalError(
            "Could not add recording \"%s\", "
    ...
    for (bool _good = true; _good;) {
      ...
      if (!recSegSet_.addSegment(segment))
        warning("Discard segment \"%s\", "
                ...
                segment->segmentId().c_str(), segment->recordingId().c_str());
    ...
    if (itRecSegSet_ == endRecSegSet_)
      return false;
    if (dump_.isOpen()) {
      for (RecordingSegmentationSet::const_iterator itRecSeg =
               recSegSet_.begin();
           itRecSeg != recSegSet_.end(); ++itRecSeg) {
        const RecordingSegmentation &recSeg = **itRecSeg;
        dump_ << "Recording [" << recSeg.desc->recordingId() << ":"
              << Core::form("%.2f-%.2f", recSeg.desc->startTime(),
                            recSeg.desc->endTime())
              << "]" << std::endl;
        for (RecordingSegmentation::const_iterator itSeg = recSeg.begin();
             itSeg != recSeg.end(); ++itSeg)
          dump_ << "\tSegment [" << (*itSeg)->segmentId() << ":"
                << Core::form("%.2f-%.2f", (*itSeg)->startTime(),
                              (*itSeg)->endTime())
                << "]" << std::endl;
        dump_ << std::endl;
      }
...
public:
  ...
  public:
    ...
    protected:
      virtual void exploreState(ConstStateRef sr) {
        verify((sr->id() < sidMap_.size()) &&
               (sidMap_[sr->id()] != Fsa::InvalidStateId));
        ...
        if (sp->isFinal()) {
          ...
          verify(!sp->hasArcs() &&
                 (l->semiring()->compare(sp->weight(), l->semiring()->one()) ==
                  0));
          sp->unsetFinal();
          // unique final state
        ...
        for (State::iterator a = sp->begin(), a_end = sp->end(); a != a_end;
             ++a) {
    ...
    public:
      Concatenator(ConcatenatedLatticeBuilder *builder, ConstLatticeRef l)
          : TraverseState(l), s_(builder->s_), b_(builder->b_),
            sMaxSid_(builder->maxSid_), sFinalSid_(builder->finalSid_),
  ...
  public:
    ...
    ~ConcatenatedLatticeBuilder() { delete s_; }
...
public:
  ConcatenateLatticesNode(const std::string &name,
  ...
  virtual ConstLatticeRef sendLattice(Port to) {
    ...
    BoundedSegmentList boundedSegs = buildBoundedSegments(*recSeg);
    {
      ...
      msg << "Recording \"" << recSeg->desc->recordingId() << ":"
          ...
          << "\"\n"
          << "  duration="
          << (recSeg->desc->endTime() - recSeg->desc->startTime()) << "sec\n";
    ...
    s32 pending = 0;
    for (BoundedSegmentList::iterator it = boundedSegs.begin();
         it != boundedSegs.end(); ++it) {
      ...
      if (!l) {
        ...
        l = fit(l, it->startFrame - pending, it->endFrame,
                forceSentenceEndLabels_);
...
const Core::ParameterBool ConcatenateLatticesNode::paramForceSentenceEndLabels(
    ...
    "put sentence end labels in between concatenated lattices", false);
...
// -------------------------------------------------------------------------
NodeRef createConcatenateCnsNode(const std::string &name,
                                 const Core::Configuration &config) {
...
public:
  ConcatenateFCnsNode(const std::string &name,
                      const Core::Configuration &config)
      : Precursor(name, config), reader_(0) {}
  ...
  virtual ConstPosteriorCnRef sendPosteriorCn(Port to) {
    ...
    BoundedSegmentList boundedSegs = buildBoundedSegments(*recSeg);
    s32 nRecFrames =
        nFrames(recSeg->desc->startTime(), recSeg->desc->endTime());
    {
      Core::Component::Message msg(log());
      msg << "Recording \"" << recSeg->desc->recordingId() << ":"
          << recSeg->desc->startTime() << "-" << recSeg->desc->endTime()
