...
Core::ComponentFactory<MixtureSetReader::Reader, const std::string>
    *MixtureSetReader::reader_ = 0;
...
const Core::ParameterBool MixtureSetReader::RefReader::paramRepeatedCovariance(
    "repeated-covariance",
    "in case of legacy Sietill references the pooled variance is stored once "
    "for each mixture",
    false);

bool MixtureSetReader::RefReader::read(const std::string &filename,
                                       MixtureSetRef &result) const {
#if 1
...
bool MixtureSetReader::FormatReader::read(const std::string &filename,
                                          MixtureSetRef &result) const {
...
bool Mm::MixtureSetReader::CmfReader::read(const std::string &filename,
                                           MixtureSetRef &result) const {
#if 1
...
const Core::ParameterString
    MixtureSetReader::LambdaReader::paramTopologyFilename(
        "topology", "name of mixture set topology file");

bool MixtureSetReader::LambdaReader::read(const std::string &filename,
                                          MixtureSetRef &result) const {
#if 1
  criticalError("Module MM_ADVANCED is not available");
  return false;
...
MixtureSetReader::LambdaReader::readLogLinearMixtureSet(
...
bool MixtureSetReader::MixtureSetEstimatorReader::read(
    const std::string &filename, MixtureSetRef &result) const {
