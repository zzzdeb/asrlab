...
/*****************************************************************************/
{
  ...
  if (b >> datatypeName) {
    const Datatype *datatype =
        Flow::Registry::instance().getDatatype(datatypeName);
...
{
  if (attributes_) {
    Core::ArchiveWriter w(*cache_->archive_, name_ + ".attribs",
                          cache_->compress_);
    if (w.isOpen()) {
      Core::XmlWriter xw(w);
      xw << *attributes_;
...
Core::ParameterInt
    Cache::paramGather("gather",
                       "number of data packets to gather before writing",
                       Core::Type<u32>::max);
Core::ParameterBool
    Cache::paramCompress("compress", "compress data written to archive", false);
Core::ParameterString
    Cache::paramCast("cast", "datatype casted to before writing to archive");

Cache::Cache(const Core::Configuration &c)
    : Core::Component(c), archive_(0),
...
/*****************************************************************************/
{
  if (isOpen())
    close();
  if (path_.empty() || (!Core::isValidPath(path_) &&
...
CacheNode::CacheNode(const Core::Configuration &c)
    : Core::Component(c), Node(c), Cache(c), hasInput_(false),
      hasOutput_(false), isCached_(false), reader_(0), writer_(0), datatype_(0)
/*****************************************************************************/
{}
    ...
    if (r.isOpen()) {
      ...
      if (attributesParser_.buildFromStream(*ca, r)) {
        ...
        if (!datatype.empty()) {
          ...
          if (datatype_ == 0)
            Cache::error("datatype '%s' from archive is unknown",
                         datatype.c_str());
        }
        attributes = ca;
      }
