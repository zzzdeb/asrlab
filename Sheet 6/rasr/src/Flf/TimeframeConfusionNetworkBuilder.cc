    ...
    if (!normalize)
      if ((sum <= 0.99) || (1.01 <= sum))
        Core::Application::us()->warning(
            "Numerical instability: sum over posterior CN slot exceeds [0.99, "
            "1.01]; "
            "sum is %f.",
            sum);
    // Normalize anyway to deal with slight numerical instabilities, e.g.
    // rounding errors
    for (PosteriorCn::Slot::iterator itPdf = pdf.begin(); itPdf != pdf.end();
         ++itPdf)
      itPdf->score /= sum;
 ...
 **/
template <class ArcScores> class FramewiseCollector : public TraverseState {
private:
    ...
    for (State::const_iterator a = sr->begin(), end_a = sr->end(); a != end_a;
      ...
      if (a->input() == Fsa::InvalidLabelId)
        Core::Application::us()->criticalError(
            "Frame-wise score collector: label id is invalid.");
      PosteriorCn::Arc cnArc(a->input(), itScore->score());
      Time t = l->boundary(sr->id()).time(),
           end_t = l->boundary(a->target()).time();
      if ((t == InvalidTime) || (end_t == InvalidTime) || (t > end_t) ||
          (end_t >= 2147483647))
        Core::Application::us()->criticalError(
            "Frame-wise score collector: interval [%d,%d] is invalid.", t,
            end_t);
...
public:
  FramewiseCollector(PosteriorCn &cn, //
                     ConstLatticeRef l, const ArcScores &arcScores)
      : TraverseState(l), arcScores_(arcScores), cn_(cn),
        begin_(Core::Type<Time>::max), end_(0) {
...
public:
  struct const_iterator {
    ...
    Probability score() const { return itArc->score(id); }
  };
...
public:
  PrecalculatedArcScores(ConstLatticeRef l, ScoreId id) : l_(l), id_(id) {}
  std::pair<const_iterator, const_iterator> operator()(Fsa::StateId sid) const {
    ConstStateRef sr = l_->getState(sid);
    return std::make_pair(const_iterator(sr->begin(), id_),
...
public:
  FramePosteriorCnBuilderNode(const std::string &name,
