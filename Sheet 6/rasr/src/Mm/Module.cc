...
const Core::ParameterString
    Module_::paramMixtureSetFilename("file", "name of mixture set file");

const Core::ParameterString
    Module_::paramNewMixtureSetFilename("new-file",
                                        "name of new mixture set file");

const Core::ParameterInt Module_::paramReducedMixtureSetDimension(
    "reduced-mixture-set-dimension", "reduce dimension of densities if > 0", 0,
    0);

const Core::ParameterInt Module_::paramMixtureSetDimension(
    "mixture-set-dimension", "dimension of the (empty) mixture set", 0, 0);

const Core::Choice Module_::choiceCovarianceTyingType(
    "none", noCovarianceTying, "pooled-covariance", pooledCovariance,
    "mixture-specific-covariance", mixtureSpecificCovariance,
    Core::Choice::endMark());
const Core::ParameterChoice
    Module_::paramCovarianceTyingType("covariance-tying",
                                      &choiceCovarianceTyingType,
                                      "type of tying scheme", pooledCovariance);

const Core::Choice Module_::choiceEstimatorType(
    "maximum-likelihood", maximumLikelihood, "discriminative", discriminative,
    "legacy-discriminative-with-i-smoothing",
    legacyDiscriminativeWithISmoothing, "discriminative-with-i-smoothing",
    discriminativeWithISmoothing, "rprop-discriminative", rPropDiscriminative,
    "rprop-discriminative-with-i-smoothing", rPropDiscriminativeWithISmoothing,
    Core::Choice::endMark());
const Core::ParameterChoice
    Module_::paramEstimatorType("estimator-type", &choiceEstimatorType,
                                "type of mixture set estimator",
                                maximumLikelihood);

Module_::Module_()
    : formats_(0), featureScorerFactory_(new FeatureScorerFactory),
      paramFeatureScorerType("feature-scorer-type",
                             &featureScorerFactory_->featureScorerNames(),
...

{
  ...
  featureScorerFactory_
      ->registerFeatureScorer<SimdGaussDiagonalMaximumFeatureScorer, MixtureSet,
                              AbstractMixtureSetLoader>(
  ...
  featureScorerFactory_
      ->registerFeatureScorer<BatchPreselectionFloatFeatureScorer, MixtureSet,
                              AbstractMixtureSetLoader>(
...
Core::Ref<MixtureSet>
Module_::createMixtureSet(const Core::Configuration &configuration,
                          size_t nMixtures, size_t dimension,
                          size_t nDensitiesPerMixture) const {
  return createMixtureSet(
      static_cast<CovarianceTyingType>(paramCovarianceTyingType(configuration)),
      nMixtures, dimension, nDensitiesPerMixture);
}

Core::Ref<MixtureSet>
Module_::createMixtureSet(CovarianceTyingType covarianceTying, size_t nMixtures,
                          size_t dimension, size_t nDensitiesPerMixture) const {
  ...
  case mixtureSpecificCovariance:
    builder =
        new MixtureSpecificCovarianceBuilder(nMixtures, nDensitiesPerMixture);
    break;
...
Core::Ref<MixtureSet>
Module_::createEmptyMixtureSet(const Core::Configuration &configuration) const {
  return createMixtureSet(pooledCovariance, 0,
                          paramMixtureSetDimension(configuration), 0);
}

Core::Ref<MixtureSet>
Module_::readMixtureSet(const std::string &filename,
                        const Core::Configuration &configuration) const {
  Core::Ref<MixtureSet> result;
  if (filename.empty()) {
    return result;
  }
  Core::Application::us()->log("Loading mixture set from file \"%s\"",
                               filename.c_str());
  result = MixtureSetReader(configuration).read<MixtureSet>(filename);
  if (result)
    Core::Application::us()->log("mixtureset read from \"%s\"",
                                 filename.c_str());
  else
    Core::Application::us()->error("failed to read mixtureset from \"%s\"",
                                   filename.c_str());
  ...
  if (result) {
    ...
    if (dim > 0) {
      ...
      Core::Application::us()->log("reduced dimension of mixture set to %d",
                                   dim);
    }

    Core::XmlChannel logNormStatisticsChannel(configuration,
                                              "log-norm-factor-statistics");
...
Core::Ref<MixtureSet>
Module_::readMixtureSet(const Core::Configuration &configuration) const {
...
Core::Ref<AbstractMixtureSet> Module_::readAbstractMixtureSet(
    ...
    const Core::Configuration &configuration) const {
  FeatureScorerType featureScorer =
      static_cast<FeatureScorerType>(paramFeatureScorerType(configuration));
  const AbstractMixtureSetLoader *loader =
      featureScorerFactory_->getMixtureSetLoader(featureScorer);
  verify(loader);
...
Core::Ref<AbstractMixtureSet> Module_::readAbstractMixtureSet(
    const Core::Configuration &configuration) const {
  return readAbstractMixtureSet(paramMixtureSetFilename(configuration),
                                configuration);
}

void Module_::convertMixtureSet(
    const Core::Configuration &configuration) const {
  ...
  Core::Application::us()->log("Saving mixtureset to file \"%s\"",
                               filename.c_str());
...
AbstractMixtureSetEstimator *Module_::createMixtureSetEstimator(
  ...
  default:
    Core::Application::us()->criticalError(
        "Unknown mixture set estimator type.");
  }
  return result;
}
...
Module_::createDiscriminativeMixtureSetEstimator(
    const Core::Configuration &configuration) const {
  ...
  case discriminativeWithISmoothing:
    result =
        new EbwDiscriminativeMixtureSetEstimatorWithISmoothing(configuration);
    break;
  default:
    Core::Application::us()->criticalError(
        "Unknown discriminative mixture set estimator type.");
  }
  return result;
}
#endif
...
Module_::createMixtureSetEstimator(const Core::Configuration &configuration,
                                   size_t nMixtures, size_t dimension,
                                   size_t nDensitiesPerMixture) const {
  AbstractMixtureSetEstimator *result = 0;
  Core::Ref<MixtureSet> mixtureSet = createMixtureSet(
      configuration, nMixtures, dimension, nDensitiesPerMixture);
  if (mixtureSet) {
...
bool Module_::readMixtureSetEstimator(const std::string &filename,
                                      AbstractMixtureSetEstimator &e) {
  MixtureSetReader::MixtureSetEstimatorReader reader(
      Core::Application::us()->getConfiguration());
  return reader.readMixtureSetEstimator(filename, e);
}

bool Module_::writeMixtureSetEstimator(const std::string &filename,
                                       AbstractMixtureSetEstimator &estimator) {
  require(!filename.empty());

  Core::Application::us()->log(
      "Saving mixture set estimator to file \"%s\" ...", filename.c_str());

  std::string backupName = filename + ".bak";
  if (Core::isRegularFile(filename) &&
      rename(filename.c_str(), backupName.c_str()) != 0) {
    Core::Application::us()->error("Could not create backup file \"%s\".",
                                   backupName.c_str());
    return false;
  }
  ...
  if (!bos) {
    Core::Application::us()->error("Failed to open \"%s\" for writing",
                                   filename.c_str());
    return false;
  }
  estimator.write(bos);
  ...
  if (Core::isRegularFile(backupName) && unlink(backupName.c_str()) != 0)
    Core::Application::us()->error("Could not remove backup file \"%s\".",
                                   backupName.c_str());
...
Core::Ref<FeatureScorer>
Module_::createFeatureScorer(const Core::Configuration &c) const {
...
Core::Ref<ScaledFeatureScorer> Module_::createScaledFeatureScorer(
    const Core::Configuration &c,
    Core::Ref<FeatureScorer> featureScorer) const {
  return featureScorer ? Core::Ref<ScaledFeatureScorer>(
                             new FeatureScorerScaling(c, featureScorer))
                       : Core::Ref<ScaledFeatureScorer>();
}

bool Module_::writeMixtureSet(const std::string &filename,
                              const AbstractMixtureSet &ms,
                              u32 precision) const {
  bool result = Mm::Module::instance().formats().write(filename, ms, precision);
  if (result) {
    Core::Application::us()->log("mixtureset written to \"")
        << filename << "\"";
  } else {
    Core::Application::us()->error("failed to write mixtureset to \"")
        << filename << "\"";
  }
  return result;
}
...
Core::FormatSet &Module_::formats() {
  Core::Configuration config(Core::Application::us()->getConfiguration(),
                             "file-format-set");
  if (!formats_) {
    formats_ = new Core::FormatSet(config);
    formats_->registerFormat("bin",
                             new Core::BinaryFormat<AbstractMixtureSet>());
    formats_->registerFormat(
        "ascii", new Core::CompressedPlainTextFormat<AbstractMixtureSet>());
    formats_->registerFormat(
        "_default_", new Core::CompressedPlainTextFormat<AbstractMixtureSet>());
  }
  return *formats_;
}
