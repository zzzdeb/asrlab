...
void NodeFactory::dumpNodeList(std::ostream &o) const {
  ...
  for (NodeCreatorMap::const_iterator it = creatorMap_.begin();
       it != creatorMap_.end(); ++it)
    sortedNodes.push_back(it->second.name);
  std::sort(sortedNodes.begin(), sortedNodes.end());
  o << "List of registered nodes:" << std::endl;
  for (std::vector<std::string>::const_iterator it = sortedNodes.begin();
       it != sortedNodes.end(); ++it)
    o << "    " << *it << std::endl;
}
void NodeFactory::dumpNodeDescription(std::ostream &o,
...
const Core::ParameterString NodeFactory::paramNodeType("type", "node type", "");
// -------------------------------------------------------------------------
