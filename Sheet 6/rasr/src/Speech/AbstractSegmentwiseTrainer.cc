...
const Core::ParameterFloat AbstractSegmentwiseTrainer::paramWeightThreshold(
    "weight-threshold",
    "discard all observations with absolute weight smaller or equal to this "
    "threshold",
    Core::Type<f32>::epsilon, Core::Type<f64>::min);

const Core::ParameterInt AbstractSegmentwiseTrainer::paramPosteriorTolerance(
    "posterior-tolerance",
    "tolerance in posterior computation, i.e., error of forward and backward "
    "flows w.r.t. least significant bits",
    100, 0, Core::Type<s32>::max);

Core::ParameterString AbstractSegmentwiseTrainer::paramLatticeName(
    "lattice-name", "name of lattice with total scores",
...
AbstractSegmentwiseTrainer::AbstractSegmentwiseTrainer(
    ...
    : Core::Component(c), Precursor(c),
      ...
      posteriorTolerance_(paramPosteriorTolerance(config)), orthToLemma_(0),
...
Lattice::ConstWordLatticeRef AbstractSegmentwiseTrainer::extractNumerator(
    const std::string &orth, Lattice::ConstWordLatticeRef denominator) const {
  return Lattice::extractNumerator(orth, denominator, orthToLemma_,
                                   lemmaPronToLemma_, lemmaToLemmaConfusion_);
}

Lattice::ConstWordLatticeRef
AbstractSegmentwiseTrainer::turnOffCompetingHypotheses(
    const std::string &orth, Lattice::ConstWordLatticeRef denominator) {
#if 1
...
Core::Choice AbstractSegmentwiseTrainer::choiceCriterion(
    "MMI", maximumMutualInformation, "weighted-MMI",
    weightedMaximumMutualInformation, "MCE", minimumClassificationError, "ME",
    minimumError, "gis-ME", gisMinimumError, "log-ME", logMinimumError,
    "context-prior", contextPrior, "context-accuracy", contextAccuracy,
    "weighted-ME", weightedMinimumError, "legacy-ME-with-i-smoothing",
    legacyMinimumErrorWithISmoothing, "ME-with-i-smoothing",
    minimumErrorWithISmoothing, "log-ME-with-i-smoothing",
    logMinimumErrorWithISmoothing, "weighted-ME-with-i-smoothing",
    weightedMinimumErrorWithISmoothing, "CORRECTIVE", corrective,
    "minimum-least-squared-error", minimumLeastSquaredError, "plain", plain,
    Core::Choice::endMark());
