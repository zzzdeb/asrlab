...
SearchInterface::SearchInterface(const Core::Configuration &c)
    : Core::Component(c), SearchAlgorithm(c), lexicon_(0), lmScale_(1.0),
      pronunciationScale_(1.0) /* @todo add parameter */
{}

SearchInterface::~SearchInterface() { delete lexicon_; }

bool SearchInterface::setModelCombination(
...
void SearchInterface::getCurrentBestSentence(
    Search::SearchAlgorithm::Traceback &result) const {
  Traceback traceback;
  getResult(traceback);
  result.clear();
  for (Traceback::const_iterator i = traceback.begin(); i != traceback.end();
       ++i) {
    result.push_back(Search::SearchAlgorithm::TracebackItem(
        lexicon_->pronunciation(i->word), i->time, ScoreVector(i->score, 0),
        Lattice::WordBoundary::Transit()));
  }
}
...
Score SearchInterface::getLanguageModelScore(WordSequence ws) const {
  ...
  for (WordSequence::size_type i =
           std::max(ws.size() - 1, (WordSequence::size_type)0);
       i > 0; --i) {
    Lm::extendHistoryByLemmaPronunciation(lm_, lexicon_->pronunciation(ws[i]),
                                          history);
  }
  Lm::addLemmaPronunciationScore(lm_, lexicon_->pronunciation(ws.front()),
                                 pronunciationScale_, lmScale_, history, score);
  return score;
}
...
Score SearchInterface::getTransitionScore(bool isSilence,
                                          State distance) const {
  Am::AcousticModel::StateTransitionIndex i =
      (isSilence ? Am::TransitionModel::silence : Am::TransitionModel::phone0);
