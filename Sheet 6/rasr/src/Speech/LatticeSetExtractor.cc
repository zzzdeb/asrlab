...
const Core::ParameterStringVector LatticeSetExtractor::paramAcousticExtractors(
    "acoustic-rescorers", "set of lattice extractors, type=acoustic", ",");

const Core::ParameterStringVector LatticeSetExtractor::paramEmissionExtractors(
    "emission-rescorers", "set of lattice extractors, type=emission", ",");

const Core::ParameterStringVector LatticeSetExtractor::paramTdpExtractors(
    "tdp-rescorers", "set of lattice extractors, type=tdp", ",");

const Core::ParameterStringVector
    LatticeSetExtractor::paramPronunciationExtractors(
...
const Core::ParameterStringVector LatticeSetExtractor::paramLmExtractors(
    "lm-rescorers", "set of lattice extractors, type=lm", ",");

const Core::ParameterStringVector
    LatticeSetExtractor::paramCombinedLmExtractors(
        "combined-lm-rescorers", "set of lattice extractors, type=combined-lm",
        ",");

const Core::ParameterStringVector LatticeSetExtractor::paramRestorers(
    "restorers", "set of lattice extractors, type=restorer", ",");
...
const Core::ParameterStringVector LatticeSetExtractor::paramDistanceExtractors(
    "distance-rescorers", "set of lattice extractors, type=distance", ",");

const Core::ParameterStringVector LatticeSetExtractor::paramPosteriorExtractors(
    "posterior-rescorers", "set of lattice extractors, type=posterior", ",");

const Core::ParameterStringVector LatticeSetExtractor::paramPassExtractors(
    "pass-extractors", "set of lattice extractors, type=pass", ",");
 ...
 *  LatticeSetGenerator
 */
Core::Choice LatticeSetGenerator::choiceSearchType("exact-match", exactMatch,
                                                   "full-search", fullSearch,
                                                   Core::Choice::endMark());

const Core::ParameterChoice LatticeSetGenerator::paramSearchType(
    "search-type", &choiceSearchType,
...
const Core::ParameterBool LatticeSetGenerator::paramShareAcousticModel(
    ...
    "if alignment generator and rescorer have the same acoustic model, they "
    "can share it",
    false);
...
LatticeSetGenerator::LatticeSetGenerator(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), LatticeSetExtractor(c) {}
...
Core::Ref<SegmentwiseFeatureExtractor>
LatticeSetGenerator::segmentwiseFeatureExtractor() {
  if (!segmentwiseFeatureExtractor_) {
#if 1
    segmentwiseFeatureExtractor_ = Core::ref(new SegmentwiseFeatureExtractor(
...
LatticeSetGenerator::AlignmentGeneratorRef
LatticeSetGenerator::alignmentGenerator() {
  if (!alignmentGenerator_) {
#if 1
    alignmentGenerator_ = Core::ref(
        new PhonemeSequenceAlignmentGenerator(select("segmentwise-alignment")));
#endif
    alignmentGenerator_->setSegmentwiseFeatureExtractor(
...
void LatticeSetGenerator::appendLmRescorers() {
  ...
  for (std::vector<std::string>::const_iterator name = lmNames.begin();
    ...
    LmLatticeRescorer *rescorer = new LmLatticeRescorer(select(*name));
    extractors_.push_back(rescorer);
...
void LatticeSetGenerator::appendCombinedLmRescorers() {
  std::vector<std::string> combinedLmNames = paramCombinedLmExtractors(config);
...
void LatticeSetGenerator::appendReaders() {
  ...
  for (std::vector<std::string>::const_iterator name = readerNames.begin();
    ...
    LatticeReader *reader = new LatticeReader(select(*name), lexicon_);
...
void LatticeSetGenerator::appendDistanceRescorers() {
  std::vector<std::string> distanceRescorersNames =
      paramDistanceExtractors(config);
  for (std::vector<std::string>::const_iterator name =
           distanceRescorersNames.begin();
       name != distanceRescorersNames.end(); ++name) {
    LatticeRescorer *rescorer =
        DistanceLatticeRescorer::createDistanceLatticeRescorer(select(*name),
                                                               lexicon_);
    if (!rescorer) {
      error("Unknown distance type for rescorer.");
    }
    if (dynamic_cast<ApproximatePhoneAccuracyLatticeRescorer *>(rescorer)) {
      dynamic_cast<ApproximatePhoneAccuracyLatticeRescorer *>(rescorer)
...
LatticeSetReader::~LatticeSetReader() { delete archiveReader_; }
...
void LatticeSetReader::leaveSpeechSegment(Bliss::SpeechSegment *s) {
  ...
  Lattice::ConstWordLatticeRef lattice =
      archiveReader_->get(s->fullName(), readers_);
...
LatticeSetWriter::LatticeSetWriter(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), archiveWriter_(0) {}

LatticeSetWriter::~LatticeSetWriter() { delete archiveWriter_; }

void LatticeSetWriter::processWordLattice(Lattice::ConstWordLatticeRef lattice,
...
void LatticeSetWriter::initialize(Bliss::LexiconRef lexicon) {
  ...
  verify(!archiveWriter_);
  archiveWriter_ =
      Lattice::Archive::openForWriting(select("lattice-archive"), lexicon);
