...
public:
  ...
  bool finalize();
  void mergeModels(const DiagCovMonoGaussianModel &x,
                   const DiagCovMonoGaussianModel &y);
  void mergeMeans(const DiagCovMonoGaussianModel &x,
                  const DiagCovMonoGaussianModel &y);
  void mergeVariance(const DiagCovMonoGaussianModel &x,
                     const DiagCovMonoGaussianModel &y);
  void reset(f32 x);
};
...
// ------------------------------------------------------------------------
/**
 * @todo: this class should be derived from the abstract
 * DiagCovMonoGaussianModel class
 */
...
public:
   ...
   */
  void mergeModels(const FullCovMonoGaussianModel &x,
                   const FullCovMonoGaussianModel &y);
   ...
   */
  void mergeMeans(const FullCovMonoGaussianModel &x,
                  const FullCovMonoGaussianModel &y);
   ...
   * @todo: this should be a private function?
   */
  void mergeVariance(const FullCovMonoGaussianModel &x,
                     const FullCovMonoGaussianModel &y);
...
public:
  ...
  BICFullCovMonoGaussianModel(
      u32 dim, const std::vector<BICFullCovMonoGaussianModel> &models)
      : FullCovMonoGaussianModel::FullCovMonoGaussianModel(dim), glr_(0) {}
  BICFullCovMonoGaussianModel(
      u32 dim, f32 alpha,
      const std::vector<BICFullCovMonoGaussianModel> &models)
      : FullCovMonoGaussianModel::FullCovMonoGaussianModel(dim), glr_(0) {}
   ...
   * the BIC criterion.
   *
   * GLR is calculated as 1/2(alldataLikelihood - leftLikelihood -
   * rightLikelihood)
   */
  void computeGLR(const BICFullCovMonoGaussianModel &x,
                  const BICFullCovMonoGaussianModel &y);
   ...
   *
   */
  void mergeModels(const BICFullCovMonoGaussianModel &x,
                   const BICFullCovMonoGaussianModel &y);
...
public:
  ...
  KL2FullCovMonoGaussianModel(
      u32 dim, const std::vector<KL2FullCovMonoGaussianModel> &models)
      : BICFullCovMonoGaussianModel::BICFullCovMonoGaussianModel(dim), kl2_(0) {
  }
  KL2FullCovMonoGaussianModel(
      u32 dim, f32 alpha,
      const std::vector<KL2FullCovMonoGaussianModel> &models)
      : BICFullCovMonoGaussianModel::BICFullCovMonoGaussianModel(dim), kl2_(0) {
  }
  ...
  /** compute the KL2 distance using relative likelihoods of the left and right
   * model */
  void computeKL2(const KL2FullCovMonoGaussianModel &x,
                  const KL2FullCovMonoGaussianModel &y);
   ...
   *
   */
  void mergeModels(const KL2FullCovMonoGaussianModel &x,
                   const KL2FullCovMonoGaussianModel &y);
...
public:
  ...
  CorrFullCovMonoGaussianModel(
      u32 dim, const std::vector<CorrFullCovMonoGaussianModel> &models)
      : BICFullCovMonoGaussianModel::BICFullCovMonoGaussianModel(dim),
        refresh_(1), alpha_(0.0012) {
    models_ = &models;
  }
  ...
  /** updates the alpha weighting parmameter for the relative feature
   * normalizrtion */
  ...
  /** calulate the correlation-based distance between the left model x and right
   * model y */
  void correlationDistance(CorrFullCovMonoGaussianModel &x,
                           CorrFullCovMonoGaussianModel &y);

  /** merges the left model x and right model y into the current
   * model and calculate the correlation distance
   */
  void mergeModels(CorrFullCovMonoGaussianModel &x,
                   CorrFullCovMonoGaussianModel &y);
...
// ------------------------------------------------------------------------
template <class T> class SegmentModelEstimator {
public:
  typedef T Model;
  typedef f32 Value;
...
public:
  ...
  std::vector<Model> &finalize() {
    for (typename std::vector<Model>::iterator iter = models_.begin();
...
public:
  ...
  SegmentClustering(const Core::Configuration &c)
      : Core::Component(c), bestscore_(-99999.9), totalframes_(0),
  ...
  /** set penalty weight in BIC criterion, although only lambda=1 corresponds to
   * the definition of BIC */
  ...
  /** set the minimum alpha scaling value to be used for alpha optimization in
   * computeAlpha() method */
  void setAlphaMax(float value) { alphaMax_ = value; }

  /** set the maximum alpha scaling value to be used for alpha optimization in
   * computeAlpha() method */
  ...
  /** Initializing distance matrix for clustering and initialize the model
   * trackers */
  void initDistMatrix(std::vector<Model> &models) {
    ...
    if (infoChannel_.isOpen()) {
      infoChannel_ << Core::XmlEmpty("init-distance-matrix ") +
                          Core::XmlAttribute("segments", models.size());
    }
  ...
  void initTracker(u32 dim) {
    classid_.resize(dim);
    ext::iota(classid_.begin(), classid_.end(),
              0); // Iota assigns sequentially increasing values to a range
  ...
  f32 computeAverage(const Math::Matrix<f32> &relativeFeature,
                     const f32 alpha) {
  ...
  void computeRelativeFeatures(const std::vector<Model> &models,
                               Math::Matrix<f32> &relativeFeature) {
    // first compute the relative likelihoods of the models
    Core::WithProgressIndicator progressIndicator(
        "computing relative likelihood features");
  ...
  void computeAlpha(std::vector<Model> &models) {
    ...
    while (((average < alphaMin_) || (average > alphaMax_)) &&
           !(Core::isAlmostEqualUlp(step, 0.0f, 100))) {
      if (infoChannel_.isOpen()) {
    ...
    if ((average < alphaMin_) || (average > alphaMax_)) {
      warning("Clustering goals with current alpha settings not reachable, "
              "computation of alpha failed: alpha=%lf, average=%lf, "
              "alphaMin_=%lf, alphaMax_=%lf, step=%lf",
  ...
  void initSegmentClustering(std::vector<Model> &models) {
    ...
    if (alpha_ < 0) {
      if (infoChannel_.isOpen()) {
        infoChannel_ << Core::XmlEmpty("init-segment-clustering") +
                            Core::XmlAttribute("alpha", alpha_);
      }
      computeAlpha(models);
    }
    ...
    if (infoChannel_.isOpen()) {
      infoChannel_ << Core::XmlEmpty("init-segment-clustering") +
                          Core::XmlAttribute("number-of-clusters", nrcluster_);
    }
  }
  ...
  void maxLinkage() {
    for (u32 i = child_ + 1; i < nrsegments_; i++)
      if (matrixTracker_[i])
        distMatrix_[i][parent_] =
            std::max(distMatrix_[i][parent_], distMatrix_[i][child_]);
    for (u32 i = parent_ + 1; i < child_; i++)
      if (matrixTracker_[i])
        distMatrix_[i][parent_] =
            std::max(distMatrix_[i][parent_], distMatrix_[child_][i]);
    for (u32 i = 0; i < parent_; i++)
      if (matrixTracker_[i])
        distMatrix_[parent_][i] =
            std::max(distMatrix_[parent_][i], distMatrix_[child_][i]);
  }
  ...
  void condenseDistMatrix(std::vector<Model> &models) {
    ...
    bestscore_ -=
        threshold_; // threshold_ contains also the model-complexity-based
                    // penalty P scaled by \lambda
    --nrcluster_;
    ...
    /*merge cluster*/
    if (infoChannel_.isOpen()) {
      infoChannel_ << Core::XmlEmpty("merging-clusters") +
                          Core::XmlAttribute("parent", parent_) +
                          Core::XmlAttribute("child", child_);
    }
    insertTracker();
  ...
  const u32 findBestBIC() const {
    ...
    for (s32 i = bestIdx + 1; i < (s32)stop; i++) {
      ...
      if (currentScore < bestScore) {
        ...
        bestIdx = i;
        if (infoChannel_.isOpen()) {
          infoChannel_ << Core::XmlEmpty("find-best-BIC") +
                              Core::XmlAttribute("score", bestScore) +
                              Core::XmlAttribute("cluster", bestIdx) +
                              Core::XmlAttribute("min-cluster", mincluster_) +
                              Core::XmlAttribute("max-cluster", maxcluster_) +
                              Core::XmlAttribute("segments", nrsegments_) +
                              Core::XmlAttribute("stop", stop);
        }
      }
    }
    return (bestIdx);
  ...
  bool cluster(std::vector<Model> &models) {
    ...
    while (nrcluster_ > 1) {
      if (infoChannel_.isOpen()) {
        infoChannel_ << Core::XmlEmpty("clustering ") +
                            Core::XmlAttribute("number-of-clusters",
                                               nrcluster_);
      }
      condenseDistMatrix(models);
      findBestPair();
...
public:
  ...
  SegmentClusteringNode(const Core::Configuration &c)
      : Core::Component(c), SleeveNode(c), SegmentClustering<Model>(c),
  ...
  virtual ~SegmentClusteringNode() {
    cluster(modelEstimator_.finalize()); /// @todo: ugly, this should be moved
                                         /// to the work method, but how?
  }
...
template <typename T>
Core::ParameterInt
    SegmentClusteringNode<T>::paramMincluster("mincluster",
...
Core::ParameterFloat SegmentClusteringNode<T>::paramThreshold(
    "threshold",
    "Threshold for BIC which is added to the model-complexity based penalty",
    0);

template <typename T>
Core::ParameterFloat SegmentClusteringNode<T>::paramLambda(
    "lambda",
    "Weight for the model-complexity-based penalty (only lambda=1 corresponds "
    "to the definition of BIC)",
    1);
...
Core::ParameterFloat SegmentClusteringNode<T>::paramAlphaMin(
    "minalpha",
    "Minimum Alpha scaling value used within distance scaling optimization",
    0.40);

template <typename T>
Core::ParameterFloat SegmentClusteringNode<T>::paramAlphaMax(
    "maxalpha",
    "Maximum Alpha scaling value used within distance scaling optimization",
    0.60);

template <typename T>
Core::ParameterFloat SegmentClusteringNode<T>::paramAlpha(
    "alpha",
    "Weighting Factor for correlation-based distance (default is automatic "
    "alpha estimation using minalpha and maxalpha values)",
    -1);
