...
struct Context : public std::pair<Lm::History, WordBoundary> {
  ...
  bool valid() const { return first.isValid() && second.valid(); }
...
struct ArcWithContext : public Fsa::Arc {
  ...
  ArcWithContext(Fsa::StateId target, Fsa::Weight weight, Fsa::LabelId input,
                 Fsa::LabelId output, const Context &rightContext)
      : Fsa::Arc(target, weight, input, output), rightContext_(rightContext) {}
...
struct ArcWithContextHash {
  size_t operator()(const ArcWithContext &a) const {
    ...
    return ((a.input_ << 24) & ((a.output_ & 0x000000ff) << 16) &
...
typedef Core::hash_map<Context, Fsa::StateId, ContextHash, ContextEquality>
    IndexMap;
typedef Core::hash_set<ArcWithContext, ArcWithContextHash,
                       ArcWithContextEquality>
    ArcsWithContext;
typedef Core::hash_map<Context, ArcsWithContext, ContextHash, ContextEquality>
    ArcsWithContextMap;
...
ArcsWithContextDfsState::ArcsWithContextDfsState(
    ...
    ArcsWithContextMap &arcsWithContext, Contexts &finals)
    : DfsState(), languageModel_(languageModel),
      arcsWithContext_(arcsWithContext), finals_(finals) {}
...
void ArcsWithContextDfsState::discoverState(Fsa::ConstStateRef sp) {
  ...
  for (Fsa::State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
    ...
    arcsWithContext_[context].insert(ArcWithContext(
        Fsa::InvalidStateId, a->weight(), a->input(), a->output(),
...
Fsa::ConstAutomatonRef correct(const std::string &orth,
                               Bliss::OrthographicParser *orthToLemma,
                               Fsa::ConstAutomatonRef lemmaPronToLemma,
                               Fsa::ConstAutomatonRef lemmaToLemmaConfusion) {
  Fsa::ConstAutomatonRef correct = Fsa::cache(Fsa::multiply(
      Fsa::removeEpsilons(Fsa::projectInput(Fsa::composeMatching(
          lemmaPronToLemma, Fsa::projectOutput(Fsa::composeMatching(
...
MergeWordLattice::MergeWordLattice(
...
Fsa::ConstStateRef MergeWordLattice::getState(Fsa::StateId s) const {
  ...
  for (Internal::ArcsWithContext::iterator a = arcs.begin(); a != arcs.end();
       ++a)
    sp->newArc(index(a->rightContext()), a->weight(), a->input(), a->output());
  return Fsa::ConstStateRef(sp);
}

ConstWordLatticeRef merge(ConstWordLatticeRef master, ConstWordLatticeRef slave,
                          Core::Ref<const Lm::LanguageModel> languageModel) {
 ...
 */
ConstWordLatticeRef
extractNumerator(const std::string &orth, ConstWordLatticeRef denominator,
                 Bliss::OrthographicParser *orthToLemma,
                 Fsa::ConstAutomatonRef lemmaPronToLemma,
                 Fsa::ConstAutomatonRef lemmaToLemmaConfusion) {
  Fsa::ConstAutomatonRef correct = Internal::correct(
      orth, orthToLemma, lemmaPronToLemma, lemmaToLemmaConfusion);
  if (correct) {
...
private:
  ...
  struct HypothesisHash {
    size_t operator()(const Hypothesis &h) const {
      return (h.from & 0x00ff) | ((h.to & 0x00ff) << 8) |
             ((h.label & 0xffff) << 16);
    }
  };
  ...
  typedef Core::hash_set<Hypothesis, HypothesisHash, HypothesisEquality>
      CorrectHypotheses;
  ...
  public:
    InitializeCorrectHypothesesDfsState(Fsa::ConstAutomatonRef fsa,
                                        Fsa::ConstMappingRef mapping,
...
TurnOffCompetingHypothesesLattice::TurnOffCompetingHypothesesLattice(
  ...
  Fsa::ConstAutomatonRef fsa = Fsa::composeMatching(
      Internal::correct(orth, orthToLemma, lemmaPronToLemma,
                        lemmaToLemmaConfusion),
...
ConstWordLatticeRef
turnOffCompetingHypotheses(const std::string &orth,
                           ConstWordLatticeRef denominator,
                           Bliss::OrthographicParser *orthToLemma,
                           Fsa::ConstAutomatonRef lemmaPronToLemma,
                           Fsa::ConstAutomatonRef lemmaToLemmaConfusion) {
  Core::Ref<TurnOffCompetingHypothesesLattice> turnedOff(
      new TurnOffCompetingHypothesesLattice(orth, denominator, orthToLemma,
                                            lemmaPronToLemma,
                                            lemmaToLemmaConfusion));
