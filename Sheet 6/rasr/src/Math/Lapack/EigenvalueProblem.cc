...
const Core::Choice EigenvalueProblem::choiceDriverType(
    "expert", driverExpert, "relatively-robust", driverRelativelyRobust,
    "divide-and-conquer", driverDivideAndConquer, Core::Choice::endMark());
const Core::ParameterChoice
    EigenvalueProblem::paramDriverType("driver", &choiceDriverType,
                                       "type of driver routine", driverExpert);
const Core::ParameterBool EigenvalueProblem::paramShouldSupportEigenvectors(
    "support-eigenvectros",
    "zero those eigenvector elements which the algorithm proposes.", false);

EigenvalueProblem::EigenvalueProblem(const Core::Configuration &c)
    : Core::Component(c), eigenvalueLowerBound_(paramEigenvalueLowerBound(c)),
...
bool EigenvalueProblem::isEigenvalueRangeValid() const {
  ...
  if (eigenvalueLowerBound_ == Core::Type<double>::min &&
    ...
    return false;
  warning("Inconsistent eigenvalue range (%f, %f]. All eigenvalues will be "
          "calculated.",
...
void EigenvalueProblem::supportEigenvectors(
    const Matrix<int, RowByRowStorage> &isuppz,
...
//--------------------------------------------------------------------------------------------------------
SymmetricEigenvalueProblem::SymmetricEigenvalueProblem(
    const Core::Configuration &c)
    : Core::Component(c), Math::EigenvalueProblem(c),
      Math::Lapack::EigenvalueProblem(c) {}

bool SymmetricEigenvalueProblem::solve(const Math::Matrix<ValueType> &a,
                                       Math::Vector<ValueType> &eigenvalues,
                                       Math::Matrix<ValueType> &eigenvectors) {
  switch (driverType()) {
  case driverExpert:
  case driverRelativelyRobust:
    return solveWithExpertOrRelativelyRobustDriver(a, eigenvalues,
                                                   eigenvectors);
...
bool SymmetricEigenvalueProblem::solveWithExpertOrRelativelyRobustDriver(
    const Math::Matrix<ValueType> &a, Math::Vector<ValueType> &eigenvalues,
...
bool SymmetricEigenvalueProblem::dsyevx(Matrix<double> &a, int &nEigenvalues,
  ...
  Lapack::dsyevx(&jobz, &range, &uplo, &n, a.buffer(), &lda, &vl, &vu, &il, &iu,
                 &abstol, &nEigenvalues, eigenvalues.buffer(),
                 eigenvectors.buffer(), &ldz, work.buffer(), &lwork,
...
bool SymmetricEigenvalueProblem::dsyevr(Matrix<double> &a, int &nEigenvalues,
                                        Vector<double> &eigenvalues,
                                        Matrix<double> &eigenvectors) const {
  char jobz = 'V';
  char range = translateEigenvalueRange();
  ...
  Lapack::dsyevr(&jobz, &range, &uplo, &n, a.buffer(), &lda, &vl, &vu, &il, &iu,
                 &abstol, &nEigenvalues, eigenvalues.buffer(),
                 eigenvectors.buffer(), &ldz, isuppz.buffer(), work.buffer(),
...
bool SymmetricEigenvalueProblem::dsyevd(Matrix<double> &a,
  ...
  Lapack::dsyevd(&jobz, &uplo, &n, a.buffer(), &lda, eigenvalues.buffer(),
                 work.buffer(), &lwork, iwork.buffer(), &liwork, &info);
  if (info != 0) {
    error("Lapack error code %d recieved while workspace query.", info);
    return false;
  }
...
const Core::ParameterFloat
    GenEigenProblemWithSchurDecomposition::paramRelativeAlphaMinimum(
        "relative-alpha-minimum",
        "If abs(alpha) smaller than max(abs(alphas)) * this, eigenvalue is set "
        "to zero.",
        Core::Type<f64>::epsilon, 0, 1);
const Core::ParameterFloat
    GenEigenProblemWithSchurDecomposition::paramRelativeBetaMinimum(
        "relative-beta-minimum",
        "If abs(beta) smaller than max(abs(betas)) * this, eigenvalue is set "
        "to zero.",
        Core::Type<f64>::epsilon, 0, 1);
const Core::Choice GenEigenProblemWithSchurDecomposition::choiceBalancing(
    "none", noneBalancing, "permute", permute, "scale", scale,
    "permute-and-scale", permuteAndScale, Core::Choice::endMark());
const Core::ParameterChoice
    GenEigenProblemWithSchurDecomposition::paramBalancing("balancing",
                                                          &choiceBalancing,
                                                          "type of balancing",
                                                          permuteAndScale);

GenEigenProblemWithSchurDecomposition::GenEigenProblemWithSchurDecomposition(
    const Core::Configuration &c)
    : Core::Component(c), Math::GeneralizedEigenvalueProblem(c),
      Math::Lapack::EigenvalueProblem(c),
...
bool GenEigenProblemWithSchurDecomposition::solveSymmetric(
  ...
  if (!solve(a, b, complexEigenvalues, complexLeftEigenvectors,
             complexRightEigenvectors))
    success = false;
...
bool GenEigenProblemWithSchurDecomposition::solve(
  ...
  switch (driverType()) {
  case driverExpert:
    return solveWithExpertDriver(a, b, eigenvalues, leftEigenvectors,
                                 rightEigenvectors);
  default:
...
bool GenEigenProblemWithSchurDecomposition::solveWithExpertDriver(
  ...
  if (!dggevx(lapackA, lapackB, realAlphas, imaginaryAlphas, betas,
              ...
              eigenvectorReciprocalConditionNumbers, oneNormOfBalancedA,
              oneNormOfBalancedB, leftMaxPerMinScaling, rightMaxPerMinScaling))
    return false;
...
bool GenEigenProblemWithSchurDecomposition::dggevx(
    Matrix<double> &a, Matrix<double> &b, Vector<double> &realAlphas,
    Vector<double> &imaginaryAlphas, Vector<double> &betas,
  ...
  int info;
  Lapack::dggevx(
      &balanc, &jobvl, &jobvr, &sense, &n, a.buffer(), &lda, b.buffer(), &ldb,
      realAlphas.buffer(), imaginaryAlphas.buffer(), betas.buffer(),
      leftEigenvectors.buffer(), &ldvl, rightEigenvectors.buffer(), &ldvr, &ilo,
      &ihi, leftBalancingScales.buffer(), rightBalancingScales.buffer(),
      &oneNormOfBalancedA, &oneNormOfBalancedB,
      eigenvalueReciprocalConditionNumbers.buffer(),
      eigenvectorReciprocalConditionNumbers.buffer(), work.buffer(), &lwork,
      iwork.buffer(), bwork.buffer(), &info);
  if (info != 0) {
    error("Lapack error code %d recieved while workspace query.", info);
    return false;
  }
...
double GenEigenProblemWithSchurDecomposition::maxPerMinScaling(
  ...
  double result =
      (*std::max_element(scales.begin() + (ilo - 1), scales.begin() + ihi) /
...
void GenEigenProblemWithSchurDecomposition::copyEigenvalues(
  ...
  ValueType alphaMinimum =
      Core::maxAbsoluteElement(alphas) * relativeAlphaMinimum_;
  ...
  static const std::string definitions =
      "Where alpha is diagonal element of Alpha=Q^H*A*Z; "
      "beta is diagonal element of Beta=Q^H*B*Z.";
  eigenvalues.resize(alphas.size());
  for (size_t i = 0; i < alphas.size(); ++i) {
    warningDescription.clear();
    eigenvalues[i] = calculateEigenvalue(alphas[i], alphaMinimum, betas[i],
                                         betaMinimum, warningDescription);
    if (!warningDescription.empty()) {
      warning("While calculating %zdth eigenvalue, %s. %s", i + 1,
              warningDescription.c_str(), definitions.c_str());
    }
  }
}
...
GenEigenProblemWithSchurDecomposition::calculateEigenvalue(
    std::complex<ValueType> alpha, ValueType alphaMinimum, ValueType beta,
    ValueType betaMinimum, std::string &warningDescription) const {
  ValueType absAlpha = Core::abs(alpha);
  ValueType absBeta = Core::abs(beta);
  if (absAlpha < alphaMinimum) {
    warningDescription =
        Core::form("eigenvalue set to zero due to abs(alpha)=%le smaller %le "
                   "(abs(beta)=%le)",
                   (double)absAlpha, (double)alphaMinimum, (double)absBeta);
    return std::complex<ValueType>(0, 0);
  } else if (absBeta < betaMinimum) {
    warningDescription =
        Core::form("eigenvalue set to zero due to abs(beta)=%le smaller %le "
                   "(abs(alpha)=%le)",
...
void GenEigenProblemWithSchurDecomposition::writeConditionNumbers(
    Core::XmlWriter &os, const Math::Vector<std::complex<ValueType> > &alphas,
    const Vector<double> &betas,
  ...
  Math::Vector<std::complex<ValueType> >::const_iterator eigenvalue =
      eigenvalues.begin();
  Vector<double>::ConstIterator e =
      eigenvalueReciprocalConditionNumbers.begin();
  Vector<double>::ConstIterator v =
      eigenvectorReciprocalConditionNumbers.begin();
  ...
  os << Core::XmlOpen("perturbation")
     ...
     << Core::XmlFull("left-max-per-min-balancing-scale-ratio",
                      leftMaxPerMinScaling)
     << Core::XmlFull("right-max-per-min-balancing-scale-ratio",
                      rightMaxPerMinScaling)
     << Core::XmlClose("perturbation");
  for (; alpha != alphas.end(); ++alpha, ++beta, ++eigenvalue, ++e, ++v) {
    os << Core::XmlOpen("component") << Core::XmlFull("alpha", *alpha)
...
char GenEigenProblemWithSchurDecomposition::translateBalancingType(
    BalancingType type) const {
  switch (type) {
  case noneBalancing:
    return 'N';
  case permute:
...
bool GenSymmetricDefiniteEigenProblem::solveWithExpertDriver(
  ...
  Vector<double> lapackEigenvalues;
  Matrix<double> lapackEigenvectors;
  if (!dsygvx(lapackA, lapackB, nEigenvalues, lapackEigenvalues,
              lapackEigenvectors))
    return false;
  copy(lapackEigenvalues, eigenvalues, nEigenvalues);
  copy(lapackEigenvectors, eigenvectors, nEigenvalues);
...
bool GenSymmetricDefiniteEigenProblem::dsygvx(
  ...
  Lapack::dsygvx(&itype, &jobz, &range, &uplo, &n, a.buffer(), &lda, b.buffer(),
                 &ldb, &vl, &vu, &il, &iu, &abstol, &nEigenvalues,
                 eigenvalues.buffer(), eigenvectors.buffer(), &ldz,
                 work.buffer(), &lwork, iwork.buffer(), ifail.buffer(), &info);
  if (info != 0) {
    error("Lapack error code %d recieved while workspace query.", info);
    return false;
  }
...
bool GenSymmetricDefiniteEigenProblem::dsygvd(
  ...
  int ldb = n; // LDB >= max(1,N)
  int lwork = 1 + 6 * n +
              3 * n * n; // If JOBZ = 'V' and N > 1, LWORK >= 1 + 6*N + 2*N**2.
  ...
  Lapack::dsygvd(&itype, &jobz, &uplo, &n, a.buffer(), &lda, b.buffer(), &ldb,
                 eigenvalues.buffer(), work.buffer(), &lwork, iwork.buffer(),
                 &liwork, &info);
  if (info != 0) {
    error("Lapack error code %d recieved.", info);
    return false;
