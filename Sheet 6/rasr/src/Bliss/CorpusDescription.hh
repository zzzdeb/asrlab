...
public:
  ...
  bool isAnonymous() const { return name_ == anonymous; }
};
...
protected:
  ParentEntity(ParentEntity *_parent = 0) : NamedCorpusEntity(_parent) {}
...
template <class T> class Directory : private std::map<std::string, T *> {
...
public:
  ...
  CorpusSection *parent() const {
    verify(Precursor::parent() ==
           dynamic_cast<CorpusSection *>(Precursor::parent()));
...
public:
  ...
  Recording *parent() const {
    verify(Precursor::parent() ==
           dynamic_cast<Recording *>(Precursor::parent()));
 ...
 * - progress-indication: show progess meter while traversing corpus (none,
 * local or global)
...
public:
  ProgressReportingVisitorAdaptor(Core::XmlChannel &ch,
                                  bool reportOrth = false);
