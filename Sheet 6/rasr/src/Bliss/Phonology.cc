...
bool ContextPhonology::SemiContext::empty() const {
  if (Precursor::empty()) {
    ...
    verify(std::find_if(begin(), end(),
                        std::bind2nd(std::not_equal_to<Phoneme::Id>(),
                                     Phoneme::term)) == end());
    return true;
  }
  return false;
...
u32 ContextPhonology::SemiContext::Hash::operator()(
    const SemiContext &sc) const {
  u32 h = 0;
...
// ===========================================================================
ContextPhonology::ContextPhonology(Core::Ref<const PhonemeInventory> pi,
                                   u32 maximumHistoryLength,
                                   u32 maximumFutureLength)
    : pi_(pi), maximumHistoryLength_(maximumHistoryLength),
      maximumFutureLength_(maximumFutureLength) {}

void ContextPhonology::push(SemiContext &ctxt, Phoneme::Id p,
                            s32 maximumLength) {
  ctxt.insert(ctxt.begin(), p);
  if (ctxt.size() > size_t(maximumLength))
    ctxt.erase(maximumLength);
}
void ContextPhonology::pushHistory(ContextPhonology::Context &a,
                                   Phoneme::Id p) const {
  push(a.history, p, maximumHistoryLength_);
}
void ContextPhonology::pushHistory(ContextPhonology::Allophone &a,
                                   Phoneme::Id p) const {
  pushHistory(a.context_, p);
}
void ContextPhonology::pushFuture(ContextPhonology::Context &a,
                                  Phoneme::Id p) const {
  push(a.future, p, maximumFutureLength_);
}
void ContextPhonology::pushFuture(ContextPhonology::Allophone &a,
                                  Phoneme::Id p) const {
  pushFuture(a.context_, p);
}

void ContextPhonology::append(SemiContext &ctxt, Phoneme::Id p,
                              s32 maximumLength) {
  if (ctxt.size() < size_t(maximumLength))
    ctxt.append(1, p);
}
void ContextPhonology::appendHistory(ContextPhonology::Context &a,
                                     Phoneme::Id p) const {
  append(a.history, p, maximumHistoryLength_);
}
void ContextPhonology::appendHistory(ContextPhonology::Allophone &a,
                                     Phoneme::Id p) const {
  appendHistory(a.context_, p);
}
void ContextPhonology::appendFuture(ContextPhonology::Context &a,
                                    Phoneme::Id p) const {
  append(a.future, p, maximumFutureLength_);
}
void ContextPhonology::appendFuture(ContextPhonology::Allophone &a,
                                    Phoneme::Id p) const {
  appendFuture(a.context_, p);
}

ContextPhonology::Allophone ContextPhonology::allophone(const Pronunciation &p,
                                                        int i) const {
...
ContextPhonology::Allophone ContextPhonology::operator()(const Pronunciation &p,
                                                         int i) const {
  return allophone(p, i);
}

u32 ContextPhonology::Allophone::Hash::operator()(
    const ContextPhonology::Allophone &a) const {
  u32 h = ch(a.context_);
...
/**
 * \warning PhonemeGraphConverter does not respect
 * Phoneme::isContextDependent().
 */
...
public:
  PhonemeGraphConverter(const ContextPhonology &phonology,
                        const PhonemeGraph &in)
      : phonology_(phonology), in_(in), histories_(in.nNodeIds()),
        futures_(in.nNodeIds()), pastFutures_(in.nEdgeIds()),
  ...
  int traverseForward(const PhonemeGraph::Node *v, SemiContext c) {
    ...
    if (cs.find(c) == cs.end()) {
      int n = cs.size();
      cs[c] = n;
      for (PhonemeGraph::const_OutgoingEdgeIterator e = v->outgoingEdges();
           e.valid(); ++e) {
        futureHistories_[e->id()].insert(traverseForward(
            e->target(), (e->phoneme())
                             ? phonology_.extendHistory(c, e->phoneme()->id())
                             : c));
      }
    }
    return cs[c];
  ...
  int traverseBackward(const PhonemeGraph::Node *v, SemiContext c) {
    ...
    if (cs.find(c) == cs.end()) {
      int n = cs.size();
      cs[c] = n;
      for (PhonemeGraph::const_IncomingEdgeIterator e = v->incomingEdges();
           e.valid(); ++e) {
        pastFutures_[e->id()].insert(traverseBackward(
            e->source(), (e->phoneme())
                             ? phonology_.extendFuture(c, e->phoneme()->id())
                             : c));
      }
    }
    return cs[c];
  ...
  struct VertexSpawnMap {
    ...
    void build(const PhonemeGraphNodeInfo &proto, AllophoneGraph &out,
               const VertexContextMap &histories,
               const VertexContextMap &futures) {
    ...
    void addVertexContexts(AllophoneGraph &out,
                           const VertexContextMap &histories,
                           const VertexContextMap &futures) {
  ...
  void convert(AllophoneGraph &out) {
    ...
    for (v.begin(); v.valid(); ++v)
      spawn[v->id()].addVertexContexts(out, histories_[v->id()],
                                       futures_[v->id()]);
        ...
        for (fh = fhs.begin(); fh != fhs.end(); ++fh) {
          for (int h = 0; h < sourceSpawn.nHistories; ++h) {
            for (int f = 0; f < targetSpawn.nFutures; ++f) {
              ...
              if ((((e->phoneme())
                        ? phonology_.extendHistory(s->context().history,
                                                   e->phoneme()->id())
                        : s->context().history) == t->context().history) &&
                  (((e->phoneme())
                        ? phonology_.extendFuture(t->context().future,
                                                  e->phoneme()->id())
                        : t->context().future) == s->context().future)) {
...
void ContextPhonology::convert(const PhonemeGraph &in,
...
ContextPhonology::AllophoneGraphEdge::phonemeInContext() const {
  require(phoneme());
  return PhonemeInContext(phoneme()->id(), source()->context().history,
