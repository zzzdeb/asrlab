...
TextOutputStream::Buffer::Buffer(TextOutputStream *m)
    : master_(m), output_(0), indentation_(0), indentationLevel_(0), margin_(0),
      isLineBuffered_(false), bufferThreshold_(defaultBufferThreshold),
      isFastPathActive_(true), hasFailed_(false), lineState_(startOfLine),
      protection_(0), position_(0), length_(0) {}
...
TextOutputStream::Buffer::~Buffer() {
  ...
  if (charsetConversion_.nErrors())
    std::cerr
        << "warning: Text stream encountered character set conversion errors."
        << std::endl;
}
...
std::streamsize TextOutputStream::Buffer::xsputn(const char *s,
                                                 std::streamsize n) {
  if (isFastPathActive_) {
...
TextOutputStream::TextOutputStream(const std::string &filename,
  ...
  buffer_ = dynamic_cast<Buffer *>(rdbuf());
  output_ = new std::fstream(filename.c_str(),
...
void TextOutputStream::setIndentation(u32 d) { buffer_->setIndentation(d); }

void TextOutputStream::setMargin(u32 m) { buffer_->setMargin(m); }
...
int TextInputStream::Buffer::underflow() {
  ...
  // refill buffer
  size_t bufferSize =
      std::min(bufferThreshold_ - putBackBufferSize, formatted_.size());
  formatted_.copy(buffer_ + putBackBufferSize, bufferSize);
  formatted_.erase(formatted_.begin(), formatted_.begin() + bufferSize);
  setg(buffer_, buffer_ + putBackBufferSize,
       buffer_ + putBackBufferSize + bufferSize);
...
// ===========================================================================
TextInputStream::TextInputStream() : std::istream(new Buffer(this)), input_(0) {
  buffer_ = dynamic_cast<Buffer *>(rdbuf());
  setEncoding();
}
...
TextInputStream::TextInputStream(const std::string &filename, int mode /*= 0*/)
  ...
  buffer_ = dynamic_cast<Buffer *>(rdbuf());
  input_ = new std::fstream(filename.c_str(),
...
void TextInputStream::setBufferSize(u32 s) { buffer_->setBufferSize(s); }
