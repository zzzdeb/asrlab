...
private:
  ...
  void buildCompressesLookaheadStructure(const StateTree *,
...
private:
  ...
  typedef Core::hash_map<Lm::History, ContextLookahead *, Lm::History::Hash>
      Map;
  mutable Map map_;
...
public:
  ...
  LanguageModelLookahead(const Core::Configuration &, Lm::Score wpScale,
  ...
  protected:
    ...
    ContextLookahead(const LanguageModelLookahead *, const Lm::History &,
                     u32 nEntries);
  ...
  public:
    ...
    bool checkScores() {
      ...
      int nAbnorm = 0;
      for (std::vector<Score>::iterator i = scores_.begin(); i != scores_.end();
           i++) {
        if ((*i > +1.0e+20F) || (*i < -1.0e+20F))
          nAbnorm++;
      }

      std::cout << "checkScores: abnormal scores:" << nAbnorm << "/" << nScores
                << std::endl;
