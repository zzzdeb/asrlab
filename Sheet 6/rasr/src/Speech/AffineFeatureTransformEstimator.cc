...
const Core::ParameterString
    AffineFeatureTransformEstimator::paramInitialTransform(
        "initial-transform", "name of initial value for transforms", "");

const Core::ParameterString
    AffineFeatureTransformEstimator::paramInitialTransformDirectory(
        "initial-transform-directory",
        "name of directory of initial values for transforms", "");

const Core::ParameterString
    AffineFeatureTransformEstimator::paramTransformDirectory(
        "transform-directory", "name of directory of transforms", "transforms");

const Core::ParameterInt
    AffineFeatureTransformEstimator::paramEstimationIterations(
        "estimation-iterations",
        "number of iterations when estimating transform, 0 = don't estimate",
        10);

const Core::ParameterFloat
    AffineFeatureTransformEstimator::paramMinObservationWeight(
...
const Core::Choice AffineFeatureTransformEstimator::optimizationCriterionChoice(
    "naive", ConcreteAccumulator::naive, "mmi-prime",
    ConcreteAccumulator::mmiPrime, "hlda", ConcreteAccumulator::hlda,
    Core::Choice::endMark());
const Core::ParameterChoice
    AffineFeatureTransformEstimator::paramOptimizationCriterion(
...
AffineFeatureTransformEstimator::AffineFeatureTransformEstimator(
    const Core::Configuration &c, Operation op)
    : Component(c), Precursor(c, op) {
...
void AffineFeatureTransformEstimator::postProcess() {
  ...
  if (initialDirectory && initialFile)
    warning("Value for 'initial-transform-directory' overrides value for "
            "'initial-transform'.");
  else if (initialFile)
    if (!Math::Module::instance().formats().read(paramInitialTransform(config),
                                                 initialTransform))
  ...
  for (std::set<std::string>::iterator key = seenKeys.begin();
       key != seenKeys.end(); ++key) {
    ...
    */
    ...
    if (paramEstimationIterations(config) > 0) {
      ...
      if (initialDirectory) {
        ...
        transform = concreteCurrentAccumulator->estimate(
            paramEstimationIterations(config),
            paramMinObservationWeight(config),
            static_cast<ConcreteAccumulator::Criterion>(
                paramOptimizationCriterion(config)),
            initialTransform);
      } else if (initialFile) {
        transform = concreteCurrentAccumulator->estimate(
            paramEstimationIterations(config),
      ...
      std::string filename =
          Core::joinPaths(outputDirectory, *key + transformExtension());
...
void AffineFeatureTransformEstimator::scoreTransforms() {
  ...
  std::vector<std::pair<std::string, ConcreteAccumulator::Transform> >
      transforms;

  Core::DirectoryFileIterator::FileExtensionFilter filter(transformExtension());

  for (Core::DirectoryFileIterator file(transformDirectory, &filter); file;
       ++file) {
    ConcreteAccumulator::Transform transform;
    Math::Module::instance().formats().read(
        Core::joinPaths(transformDirectory, file.path()), transform);
    std::string fileKey(file.path().substr(
        0, file.path().size() - std::string(transformExtension()).size()));
  ...
  for (std::set<std::string>::iterator key = seenKeys.begin();
    ...
    for (int i = 0; i < transforms.size(); i++) {
      // TODO: Nicer logging!
      Mm::Sum score = concreteCurrentAccumulator->score(
          transforms[i].second, static_cast<ConcreteAccumulator::Criterion>(
                                    paramOptimizationCriterion(config)));
      std::cerr << *key << " " << transforms[i].first << " " << score
                << std::endl;
      log("Score, corpus-key=\"%s\" transform=\"%s\": %f", key->c_str(),
          transforms[i].first.c_str(), score);
    }
  }
}
