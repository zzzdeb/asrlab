...
EbwDiscriminativeMixtureEstimator *
EbwDiscriminativeMixtureSetEstimator::createMixtureEstimator() {
  return new EbwDiscriminativeMixtureEstimator(mixtureEstimatorConfig_);
}

EbwDiscriminativeGaussDensityEstimator *
EbwDiscriminativeMixtureSetEstimator::createDensityEstimator() {
...
EbwDiscriminativeMixtureSetEstimator::createMeanEstimator() {
  return new EbwDiscriminativeMeanEstimator;
}

EbwDiscriminativeMeanEstimator *
EbwDiscriminativeMixtureSetEstimator::createMeanEstimator(const Mean &mean) {
  return new EbwDiscriminativeMeanEstimator(mean.size());
}

EbwDiscriminativeCovarianceEstimator *
EbwDiscriminativeMixtureSetEstimator::createCovarianceEstimator() {
...
void EbwDiscriminativeMixtureSetEstimator::initialize(
  ...
  IterationConstants *ic = IterationConstants::createIterationConstants(
      select("iteration-constants"));
...
void EbwDiscriminativeMixtureSetEstimator::read(Core::BinaryInputStream &is) {
  ...
  log("#numerator-observation-weight ")
      << numObservationWeight << ", #denominator-observation-weight "
 ...
 */
EbwDiscriminativeMixtureSetEstimatorWithISmoothing::
    EbwDiscriminativeMixtureSetEstimatorWithISmoothing(
        const Core::Configuration &c)
    : Core::Component(c), ISmoothing(c), Precursor(c) {
  ISmoothing::set(this);
}
...
EbwDiscriminativeMixtureSetEstimatorWithISmoothing::createMixtureEstimator() {
  return new EbwDiscriminativeMixtureEstimatorWithISmoothing(
      mixtureEstimatorConfig_);
}
...
void EbwDiscriminativeMixtureSetEstimatorWithISmoothing::finalize(
    MixtureSet &toEstimate, const MixtureSetEstimatorIndexMap &indexMaps,
    const CovarianceToMeanSetMap &meanSetMap) {
...
void EbwDiscriminativeMixtureSetEstimatorWithISmoothing::setIMixture(
    Core::Ref<AbstractMixtureEstimator> estimator, const Mixture *mixture,
    Weight iSmoothing) {
  EbwDiscriminativeMixtureEstimatorWithISmoothing *_estimator = required_cast(
      EbwDiscriminativeMixtureEstimatorWithISmoothing *, estimator.get());
...
void EbwDiscriminativeMixtureSetEstimatorWithISmoothing::setIDensity(
    Core::Ref<GaussDensityEstimator> estimator, const GaussDensity *density) {
  required_cast(EbwDiscriminativeGaussDensityEstimatorWithISmoothing *,
                estimator.get())
      ->setIDensity(density);
}

void EbwDiscriminativeMixtureSetEstimatorWithISmoothing::setIMean(
    Core::Ref<AbstractMeanEstimator> estimator, const Mean *mean,
    Weight iSmoothing) {
  EbwDiscriminativeMeanEstimatorWithISmoothing *_estimator = required_cast(
      EbwDiscriminativeMeanEstimatorWithISmoothing *, estimator.get());
...
void EbwDiscriminativeMixtureSetEstimatorWithISmoothing::setICovariance(
    ...
    const Covariance *covariance, Weight iSmoothing) {
  EbwDiscriminativeCovarianceEstimatorWithISmoothing *_estimator =
      required_cast(EbwDiscriminativeCovarianceEstimatorWithISmoothing *,
                    estimator.get());
