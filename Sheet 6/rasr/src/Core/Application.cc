...
const ParameterBool Application::paramLogConfiguration("log-configuration",
                                                       "log-configuration",
                                                       false);
const ParameterBool
    Application::paramLogResolvedResources("log-resolved-resources",
                                           "log-resolved-resources", false);
...
Application::Application()
    : Component(getConfig()), channelManager_(0), debugChannel_(0),
      debugXmlChannel_(0), defaultLoadConfigurationFile_(true),
      defaultOutputXmlHeader_(true), comment_("#;%") {
...
Core::XmlChannel &Application::debugXmlChannel() {
  ...
  if (debugXmlChannel_ == 0) {
    debugXmlChannel_ =
        new Core::XmlChannel(config, "debug", Core::Channel::standard);
  }
...
std::string Application::getBaseName() const { return Application::basename_; }

std::string Application::getPath() const { return Application::path_; }
...
std::string Application::getDefaultParameterDescription() const {
  ...
  sout << "  where 'xxx' serves as a placeholder for the channel's name."
       << std::endl;
  ...
  sout << "  At least the channels 'log', 'warn' and 'err' are supported."
       << std::endl;
  return sout.str();
}
...
std::string Application::getUsage() const {
  ...
  if (!param.empty())
    sout << "options" << std::endl << param << std::endl;
  std::string defaultParam = getDefaultParameterDescription();
  if (!defaultParam.empty())
    sout << "default options" << std::endl << defaultParam << std::endl;
...
void Application::logSystemInfo() const {
  ...
  if (uname(&info) < 0) {
    ...
    c << XmlOpen("system-information") << XmlFull("name", info.nodename)
      << XmlFull("type", info.machine)
      << XmlFull("operating-system", info.sysname)
      << XmlFull("build-date", __DATE__) << XmlClose("system-information");
  }
}
...
void Application::logResourceUsage() const {
  ...
  if (!rc.isOpen())
    return;
  rc << XmlOpen("configuration") << XmlOpen("resources");
  config.writeUsage(rc);
  rc << XmlClose("resources") << XmlClose("configuration");
}
...
void Application::openLogging() {
  ...
  channelManager_ =
      new Channel::Manager(select("channels"), defaultOutputXmlHeader_);
...
int Application::main(int argc, char *argv[]) {
  /**
   * @bug This is a workaround for possible bug in LIBC 2.3.2. We are waiting
   * for an answer on our bug report.
   */
#ifdef OS_linux
  mallopt(M_TOP_PAD,
          1024 * 1024); // 128*1024 default for GLIBC2.3.2 on debian sarge
#endif

  if (!app_) {
    std::cerr << "oops! you did not instantiate any application class!"
              << std::endl;
    return EXIT_FAILURE;
  }
...
Application::setFromCommandline(const std::vector<std::string> &arguments) {
