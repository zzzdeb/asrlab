 ...
 *
 * The lattice processor is an application, creating a lattice processor node
 * chain to parse a corpus lattice by lattice.
 */
...
private:
  ...
  enum ApplicationType { speech, tagging };
...
private:
  ...
  template <class T>
  static Speech::LatticeSetProcessor *
  createProcessor(const Core::Configuration &c) {
    return new T(c);
  }
  static Speech::LatticeSetProcessor *
  createTrainer(const Core::Configuration &config);
  template <class T>
  void registerProcessor(Action action, const char *desc = "",
                         const char *options = "") {
    processorFactory_.registerClass(action, createProcessor<T>);
    addDescription(action, desc, options);
  }
  Core::Ref<Speech::LatticeSetProcessor>
  getProcessor(Action action, const Core::Configuration &config) const {
    Speech::LatticeSetProcessor *obj =
        processorFactory_.getObject(action, config);
    if (!obj)
...
private:
  ...
  void addDescription(Action a, const char *desc, const char *options) {
    ...
    bool dummy;
    Core::tie(i, dummy) =
        descriptions_.insert(std::make_pair(a, ActionDescription()));
    i->second.desc = desc;
...
public:
  ...
  virtual std::string getApplicationDescription() const {
    std::string app =
        ...
        "\n"
        "The output is defined in the general ('*') section of the "
        "configuration\n"
        "\t[*]\n"
        ...
        "\t\n"
        "Using this configuration a file output-channel will be created during "
        "a\n"
        "run of lattice-processor. These file contain a lots of additional\n"
        "information, warnings and errors.\n"
        "\n"
        "Every processor node can have its own part in the configuration "
        "file.\n"
        "E.g.:\n"
        "\t[*.lattice-reader]\n"
        "\treaders                         = total\n"
  ...
  virtual std::string getParameterDescription() const {
    ...
    for (std::vector<Action>::const_iterator a = actions.begin();
      ...
      tmp += "\t\"" + name + "\": ";
      std::map<Action, ActionDescription>::const_iterator desc =
          descriptions_.find(*a);
...
const Core::Choice LatticeProcessor::choiceAction(
    "not-given", actionNotGiven, "read", actionRead, "write", actionWrite,
    "dump-word-boundaries", actionDumpWordBoundaries, "generate-numerator",
    actionGenerateNumerator, "info", actionInfo, "merge", actionMerge, "unite",
    actionUnite, "numerator-from-denominator", actionNumeratorFromDenominator,
    "remove-silences-and-noises", actionRemoveSilencesAndNoises,
    "remove-redundant-silences-and-noises",
    actionRemoveRedundantSilencesAndNoises, "extract-n-best-list",
    actionExtractNBestList, "spoken-and-competing", actionSpokenAndCompeting,
    "prune", actionPrune, "time-conditioned", actionTimeConditioned, "copy",
    actionCopy, "cache", actionCache, "change-semiring", actionChangeSemiring,
    "partial", actionPartial, "skip-empty", actionSkipEmpty, "word-to-phoneme",
    actionWordToPhonemeLattice, "lemma-pronunciation-to-evaluation-token",
    actionLemmaPronunciationToEvaluationToken,
    "evaluation-token-to-lemma-pronunciation",
    actionEvaluationTokenToLemmaPronunciation,
    "lemma-pronunciation-to-syntactic-token",
    actionLemmaPronunciationToSyntacticToken, "multiply", actionMultiply,
    "linear-combination", actionLinearCombination, "expm", actionExpm,
    "extend-best-path", actionExtendBestPath, "rescore", actionRescore,
    "timeframe-error", actionTimeframeError, "density", actionDensity,
    "graph-error-rate", actionGer, "single-best-legacy", actionSingleBestLegacy,
    "word-posterior-confidence", actionWpConfidence,
    "search-minimum-bayes-risk", actionSearchMinimumBayesRisk, "single-best",
    actionSingleBest, "traceback", actionTraceback, "evaluate", actionEvaluate,
    "covariance", actionCovariance, "segment-single-word",
    actionSegmentSingleWord, "map-orth-to-phonemes", actionMapOrthToPhonemes,
    "map-to-non-coarticulated", actionMapToNonCoarticulation,
    "accumulate-discriminatively", actionAccumulateDiscriminatively, "crf",
    actionCrf, Core::Choice::endMark());

const Core::ParameterStringVector
    LatticeProcessor::paramActions("actions", "actions to perform", ",");

const Core::ParameterStringVector LatticeProcessor::paramSelections(
    "selections", "configuration selections corresponding to the actions", ",");

const Core::Choice LatticeProcessor::choiceCorpusType("bliss", bliss, "plain",
                                                      plain,
                                                      Core::Choice::endMark());

const Core::ParameterChoice LatticeProcessor::paramCorpusType(
    "type", &choiceCorpusType,
    "selection of corpus type (one of: bliss(default), plain)", bliss);

Core::Choice LatticeProcessor::choiceApplication("speech", speech, "tagging",
                                                 tagging,
                                                 Core::Choice::endMark());

Core::ParameterChoice LatticeProcessor::paramApplication("application",
                                                         &choiceApplication,
                                                         "type of application",
                                                         speech);

void LatticeProcessor::setupProcessors() {
  registerProcessor<Speech::LatticeSetReader>(
      actionRead, "read lattice from archive\n",
      ...
      "\tlattice-archive.alphabet        = "
      "[lemma-pronunciation]|syntactic-token|evaluation-token|no-lexicon-"
      "check\n");

  registerProcessor<Speech::LatticeSetWriter>(
      actionWrite, "write lattice to archive\n",
      "\tlattice-archive.format          = [fsa]|htk|wolfgang\n"
      "\tlattice-archive.path            = <file-name>\n");

  registerProcessor<Speech::WordLatticeMerger>(
      actionMerge,
      "merge two lattices, e.g. numerator into denominator for discriminative "
      "training\n",
      ...
      "\t# requires a language model to determine history of word-conditioned "
      "lattices\n");

  registerProcessor<Speech::WordLatticeUnion>(
      actionUnite, "unite two lattices\n",
      "\tfsa-prefix                      = [acoustic]|<prefix>\n"
      "\tnumerator-lattice-archive.format = [fsa]|htk|wolfgang\n"
      "\tnumerator-lattice-archive.path  = <file-name>\n");

  registerProcessor<Speech::NumeratorFromDenominatorExtractor>(
      actionNumeratorFromDenominator,
      "extract numerator (i.e., correct hypotheses according to transcription) "
      "from the denominator lattice\n",
      "\t# does not require any additional/specific parameters\n");

  registerProcessor<Speech::PruningLatticeSetNode>(
      actionPrune,
      "prune lattice (only a single score is supported) by forward-backward "
      "pruning\n",
      "\tthreshold                       = 15\n"
  ...
  registerProcessor<Speech::LinearCombinationLatticeProcessorNode>(
      actionLinearCombination,
      "calculate linear combination(s) of parts in lattice based on the passed "
      "scaling factors (e.g. scaled acoustic + lm score)\n",
  ...
  registerProcessor<Speech::DensityLatticeProcessorNode>(
      actionDensity, "calculate lattice density\n",
  ...
  registerProcessor<Speech::GerLatticeProcessorNode>(
      actionGer, "calculate graph error rate\n",
      "\t# does not require any additional/specific parameters\n");

  registerProcessor<Speech::LatticeSetGenerator>(
      actionRescore,
      "annotate a lattice (aka topology) with scores, e.g. acoustic scores or "
      "accuracies for discriminative training\n",
      ...
      "\t[*.distance1]\n"
      "\tdistance-type                   = "
      "[approximate-word-accuracy]|approximate-phone-accuracy|...\n"
      "\t# specify where to obtain the correct hypotheses from to generate "
      "numerator\n"
      "\tspoken-source                   = [orthography]|archive\n"
      "\t[*.approximate-phone-accuracy-lattice-builder]\n"
      "\t...\n");

  processorFactory_.registerClass(actionAccumulateDiscriminatively,
                                  createTrainer);
  addDescription(
      actionAccumulateDiscriminatively,
      ...
      "\t# if the posterior calculation exceeds this threshold, a warning is "
      "produced\n"
      "\tposterior-tolerance             = 100\n"
      "\tapplication                     = [speech]|tagging\n"
      "\t# ME stands for Minimum Error training like for example MWE/MPE\n"
      "\tcriterion                       = "
      "[MMI]|MCE|ME|gis-ME|log-ME|context-prior|context-accuracy|weighted-ME|"
      "ME-with-i-smoothing|log-ME-with-i-smoothing|weighted-ME-with-i-"
      "smoothing|CORRECTIVE|minimum-least-squared-error|plain\n"
      ...
      "\tmodel-type                      = "
      "[gaussian-mixture]|maximum-entropy\n");
}

Speech::LatticeSetProcessor *
LatticeProcessor::createTrainer(const Core::Configuration &config) {
  Speech::AbstractSegmentwiseTrainer *trainer = 0;
  ApplicationType appType =
      static_cast<ApplicationType>(paramApplication(config));
  if (appType == speech)
    trainer = Speech::AbstractAcousticSegmentwiseTrainer::
        createAbstractAcousticSegmentwiseTrainer(config);
  ensure(trainer);
  return trainer;
}
...
int LatticeProcessor::main(const std::vector<std::string> &arguments) {
  ...
  for (u32 i = 0; i < actions.size(); ++i) {
    ...
    default:
      ...
      if (!processor) {
        criticalError(
            "Action '%d' at %d is not known to the lattice processor!",
            actions[i], i);
      }
    }
...
void LatticeProcessor::parseActionsSelections(
  ...
  if (_actions.size() != selections.size()) {
    criticalError(
        "Mismatch in number of actions and configuration selections.");
    return;
  }
...
/**
 * Walk through the corpus.
 *    - Create a CorpusVisitor class. The corpus visitor is the class
 * coordinating the walk through the corpus.
 *    - Call signOn to the root processor node (which will call signOn of the
 * corpus visitor as well).
 *    - Create a corpus description class.
 *    - Call corpusDescription.accept to start the walk through the corpus
 */
void LatticeProcessor::visitCorpus(
    Core::Ref<Speech::LatticeSetProcessorRoot> root) {
