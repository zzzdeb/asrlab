...
const Core::Choice PhoneticDecisionTreeBase::boundaryStyleChoice(
    "pos-indep", noPosDep, "pos-dep", posDep, "super-pos-dep", superPosDep,
    Core::Choice::endMark());

const Core::ParameterChoice PhoneticDecisionTreeBase::paramBoundaryStyle(
    "boundary-style", &PhoneticDecisionTreeBase::boundaryStyleChoice,
    "word boundary indication convention used in the CART file", noPosDep);

PhoneticDecisionTreeBase::PhoneticDecisionTreeBase(
    const Core::Configuration &c, Core::Ref<const Bliss::PhonemeInventory> pi)
    : Core::Component(c), pi_(pi) {
  require(pi);
  boundaryStyle_ =
      PhoneticDecisionTree::BoundaryStyle(paramBoundaryStyle(config));
}

// ===========================================================================

struct PhoneticDecisionTree::LightTree : public ::LightTree {};

const Core::ParameterString PhoneticDecisionTree::paramFilename(
    "file", "name of (legacy) decision tree (aka CART) file to load");
...
PhoneticDecisionTree::PhoneticDecisionTree(const Core::Configuration &config,
    ...
    : Core::Component(config),
      ...
      ClassicStateTying(config, stateModel), phonemeMap_(pi_), tree_(0),
...
PhoneticDecisionTree::~PhoneticDecisionTree() { delete tree_; }
...
bool PhoneticDecisionTree::load(const char *filename) {
  ...
  if (md5.updateFromFile(filename))
    ...
    warning("could not derive md5 sum from file '%s'", filename);
  ...
  log("%d phonemes, %d questions, %d clusters", tree_->n_Phonems,
      tree_->n_Questions, tree_->n_Clusters + 1);

  if (paramUseCentralStateClassesOnly(config))
    removeBeginAndEndBranches();
...
void PhoneticDecisionTree::makePhonemeMapping() {
  ...
  for (int pho = 0; pho < tree_->n_Phonems; ++pho) {
    ...
    if (!ph) {
      ...
      warning("decision tree distinguishes different symbols for same phoneme: "
              "/%s/ vs. /%s/",
              sym, ph->symbol().str());
    } else {
      phonemeMap_[ph] = pho;
  ...
  for (Core::tie(pho, pho_end) = pi_->phonemes(); pho != pho_end; ++pho) {
    if (phonemeMap_[*pho] == -1) {
      error("phoneme \"%s\" not known to decision tree",
            (*pho)->symbol().str());
    } else {
...
inline bool PhoneticDecisionTree::answerQuestion(short quest,
                                                 short phonePosition,
...
inline s16
PhoneticDecisionTree::translateBoundaryFlag(u8 newStyleBoundary) const {
...
u32 PhoneticDecisionTree::classify(const Am::AllophoneState &phone) const {
  ...
  if (phonemeMap_[phone.allophone()->phoneme(0)] == tree_->Silence) {
    ...
    while ((treenode->leftChild != NULL) && (treenode->rightChild != NULL)) {
      if (answerQuestion(treenode->question, treenode->context,
                         *phone.allophone(), phone.state(), boundary))
  ...
  if (classifyDumpChannel_.isOpen()) {
    classifyDumpChannel_ << Core::form(
        ...
        alphabetRef_->toString(phone).c_str(), phone.state(),
        phone.allophone()->boundary, result);
  }
...
Core::BinaryTree::TreeStructure
PhoneticDecisionTree::treeStructure(void) const {
  ...
  while (!stack.empty()) {
    ...
    if ((node->leftChild != NULL) && (node->rightChild != NULL)) { // node
      structure.push_back(Core::BinaryTree::TreeStructureEntry(
          node->number, Core::BinaryTree::invalidId));
      stack.push(node->rightChild);
      stack.push(node->leftChild);
    } else // leaf
      structure.push_back(Core::BinaryTree::TreeStructureEntry(
          node->number, node->question - 1));
  }
  return structure;
}
...
struct BuildState {
  ...
  void reduce(const LightTreeNode *node, bool answer) {
    ...
    switch (q.type) {
    case PhonemClass:
      for (PhonemeSet::iterator i = phone_[1 + node_->context].begin();
           i != phone_[1 + node_->context].end();) {
  ...
  friend std::ostream &operator<<(std::ostream &o, const BuildState &s) {
    ...
    for (u32 ctx = 0; ctx < 3; ctx++) {
      o << "context " << ctx << " ";
      for (PhonemeSet::const_iterator i = s.phone_[ctx].begin();
           i != s.phone_[ctx].end(); ++i)
        o << *i << " ";
      o << std::endl;
    }
    o << "state ";
    for (StateSet::const_iterator i = s.state_.begin(); i != s.state_.end();
         ++i)
      o << *i << " ";
    o << std::endl;
    o << "boundary ";
    for (BoundarySet::const_iterator i = s.boundary_.begin();
         i != s.boundary_.end(); ++i)
      o << *i << " ";
    o << std::endl;
    return o;
...
PhoneticDecisionTree::phonemeToMixtureIndices() const {
  ...
  while (!states.empty()) {
    ...
    if (isLeaf) {
      for (PhonemeSet::const_iterator p = s.phone_[1].begin();
           p != s.phone_[1].end(); ++p) {
        phonemeToMixtureIndices[inversePhonemeMap_[*p]].insert(
            s.node_->question - 1);
      }
  ...
  phonemeToMixtureIndices[inversePhonemeMap_[tree_->Silence]].insert(
      tree_->n_Clusters);
...
/**
 * The Path class stores a sequence of LightTreeNodes from a root node to a leaf
 * node. A path object is initialized with the path from the root to the
 * leftmost node of the tree. The next() function can be used to traverse the
 * tree.
 */
...
public:
  explicit Path(LightTreeNode *root) {
    push_back(root);
...
public:
  ...
  KnownAnswerSet(Path path) {
    for (u32 i = 0; i < path.size() - 1; i++) {
      *this +=
          KnownAnswerSet(path[i], (Answer)(path[i]->leftChild == path[i + 1]));
    }
  }
  Answer answer(short question, short context) const {
    for (KnownAnswerBase::const_iterator i = kab_.begin(); i != kab_.end();
         i++) {
  ...
  KnownAnswerSet operator+(const KnownAnswerSet &knownAnswers) const {
    ...
    result.kab_.insert(result.kab_.end(), knownAnswers.kab_.begin(),
                       knownAnswers.kab_.end());
    return result;
  }
...
u32 getNumberOfLeaves(LightTreeNode *node) {
  u32 result = 0;
  for (Path path(node); !path.empty(); path.next()) {
    result++;
  }
  return result;
}
...
void removeKnownQuestions(LightTreeNode *node,
                          const KnownAnswerSet &knownAnswers) {
  if (node->leftChild == 0)
    return;
  Answer answer = knownAnswers.answer(node->question, node->context);
  switch (answer) {
  case notKnown:
    removeKnownQuestions(node->leftChild,
                         knownAnswers + KnownAnswerSet(node, left));
    removeKnownQuestions(node->rightChild,
                         knownAnswers + KnownAnswerSet(node, right));
    break;
  case yes:
    deleteSubTree(node->rightChild);
...
void PhoneticDecisionTree::removeBeginAndEndBranches() {
  ...
  for (int quest = 0; quest < tree_->n_Questions; quest++) {
    ...
    if (q.type == State) {
      ...
      knownAnswers += KnownAnswerSet(
          quest, 0, (Answer)(q.value.state == centralSegmentIndex));
    }
  }
  removeKnownQuestions(tree_->root, knownAnswers);

  log("removed branches to begin and end states; "
      "%d remaining clusters",
      getNumberOfLeaves(tree_->root) + 1);
}

void PhoneticDecisionTree::checkCompatibility(
    const PhoneticDecisionTree &pdt2) {
  if (pi_ != pdt2.pi_)
