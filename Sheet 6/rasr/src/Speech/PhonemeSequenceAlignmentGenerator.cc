...
const Core::ParameterString
    PhonemeSequenceAlignmentGenerator::Cache::paramCache(
        "path", "directory of observation-to-state alignments cache");

const Core::ParameterBool
    PhonemeSequenceAlignmentGenerator::Cache::paramReadOnly(
...
PhonemeSequenceAlignmentGenerator::Cache::Cache(
    ...
    : Core::Component(c), archive_(0), dirty_(false) {
...
void PhonemeSequenceAlignmentGenerator::Cache::setSpeechSegmentId(
...
bool PhonemeSequenceAlignmentGenerator::Cache::insert(const Key &key,
                                                      Alignment *alignment) {
  AlignmentMap::iterator itAlignment = alignments_.find(key);
  if (itAlignment == alignments_.end()) {
...
void PhonemeSequenceAlignmentGenerator::Cache::initializeArchive(
    const ModelCombination &modelCombination) {
  verify(!archive_);
  ...
  if (!paramCache(config).empty()) {
    ...
    if (ar.isOpen()) {
      ...
      Core::XmlOutputStream xw(
          new Core::ArchiveWriter(*archive_, "DEPENDENCIES"));
...
void PhonemeSequenceAlignmentGenerator::Cache::clear() {
  verify(!dirty());
  for (AlignmentMap::iterator it = alignments_.begin(); it != alignments_.end();
       ++it)
    delete it->second;
  alignments_.clear();
}

void PhonemeSequenceAlignmentGenerator::Cache::read(
    const std::string &segmentId) {
  verify(archive_);
  verify(!dirty());
  ...
  if (reader.isOpen()) {
    ...
    if (!i)
      criticalError("Failed to read observation-to-state alignments \"%s\" "
                    "from cache \"%s\"",
    ...
    if (nScores != nMatchingScores)
      warning("Found %d scores without matching alignment; discard scores.",
              (nScores - nMatchingScores));
    if (!i)
      criticalError("Failed to read observation-to-state alignments \"%s\" "
                    "from cache \"%s\"",
                    segmentId.c_str(), archive_->path().c_str());
...
void PhonemeSequenceAlignmentGenerator::Cache::write(
    const std::string &segmentId) {
  verify(archive_);
  verify(dirty());
  if (archive_->hasAccess(Core::Archive::AccessModeWrite)) {
    ...
    if (writer.isOpen()) {
      ...
      for (AlignmentMap::const_iterator it = alignments_.begin();
           o && it != alignments_.end(); ++it)
        o << it->first << *it->second;
      if (!o)
        error("could not write state lattice \"%s\" to cache \"%s\"",
              segmentId.c_str(), archive_->path().c_str());
...
const Core::ParameterBool
    PhonemeSequenceAlignmentGenerator::paramAddEmissionScores(
        "add-emission-scores", "copy emission scores into alignment weights",
        false);
...
PhonemeSequenceAlignmentGenerator::PhonemeSequenceAlignmentGenerator(
    ...
    : Precursor(c, mc), useAlignmentCache_(true),
      addEmissionScores_(paramAddEmissionScores(config)) {
  modelAcceptorCache_ =
      new FsaCache(select("model-acceptor-cache"), Fsa::storeStates);
...
void PhonemeSequenceAlignmentGenerator::addEmissionScores(
    Alignment &alignment, std::vector<Mm::FeatureScorer::Scorer> &scorers) {
  for (Alignment::iterator a = alignment.begin(); a != alignment.end(); ++a) {
    a->weight =
        scorers[a->time]->score(acousticModel()->emissionIndex(a->emission));
  }
}

void PhonemeSequenceAlignmentGenerator::setSpeechSegment(
    Bliss::SpeechSegment *segment) {
  Precursor::setSpeechSegment(segment);
  update(segment->fullName());
}

void PhonemeSequenceAlignmentGenerator::setSpeechSegmentId(
    const std::string &segmentId) {
...
const Alignment *PhonemeSequenceAlignmentGenerator::getAlignment(
    const Bliss::LemmaPronunciation &p, TimeframeIndex tbeg,
    TimeframeIndex tend) {
  return getAlignment(Bliss::Coarticulated<Bliss::LemmaPronunciation>(p), tbeg,
                      tend);
}
...
const Alignment *PhonemeSequenceAlignmentGenerator::getAlignment(
  ...
  if (!useAlignmentCache_ or
      !alignmentCache_->findForReadAccess(key, &result)) {
    aligner_.setModel(
    ...
    verify(alignment->empty() or
           (alignment->score() != Core::Type<Score>::max));
...
private:
  ...
  void insertAlignment(const Alignment *alignment, Fsa::StateId from,
...
void LatticeToAlignmentFsa::insertAlignment(
    const Alignment *alignment, Fsa::StateId from,
    const Fsa::State::const_iterator &arc) {
  // assumption: Viterbi alignment
  if (alignment->size() == 1) {
    boundaryState(from)->newArc(boundaryState(arc->target())->id(),
                                arc->weight(), alignment->front().emission);
          ...
          coarticulatedPronunciation(
              *pronunciation, wordBoundaries_->transit(sp->id()).final,
              wordBoundaries_->transit(fsa_->getState(a->target())->id())
                  .initial);
      alignment = alignmentGenerator_->getAlignment(coarticulatedPronunciation,
                                                    begtime, endtime);
    } else {
      dummyAlignment_.resize(
          endtime - begtime,
          AlignmentItem(InvalidTimeframeIndex, Fsa::Epsilon));
      alignment = &dummyAlignment_;
    }
...
std::pair<Fsa::ConstAutomatonRef, Fsa::Weight>
LatticeToAlignmentFsa::convert(Lattice::ConstWordLatticeRef lattice) {
  Fsa::Weight totalInv;
  fsa_ = Fsa::cache(Fsa::posterior64(
      Fsa::changeSemiring(lattice->mainPart(), Fsa::LogSemiring), totalInv));
  ...
  f_->setProperties(Fsa::PropertyAcyclic,
                    Fsa::hasProperties(fsa_, Fsa::PropertyAcyclic)
                        ? Fsa::PropertyAcyclic
                        : 0);
...
// for all outgoing arcs from a state (getAlignment(coarticulatedPronunciation,
// begtime, endtime))
...
void LatticeToAlignment::discoverState(Flf::ConstStateRef sp) {
  ...
  for (Flf::State::const_iterator arc = sp->begin(); arc != sp->end(); ++arc) {
    const Bliss::LemmaPronunciation *pronunciation =
        alphabet->lemmaPronunciation(arc->input());
    if (pronunciation) {
      ...
      const Alignment *arcAlignment = alignmentGenerator_->getAlignment(
          coarticulatedPronunciation, begtime, endtime);
      require(arcAlignment);
      const Mm::Weight arcWeight =
          f32(fsa_->semiring()->project(arc->weight()));
      for (Alignment::const_iterator in = arcAlignment->begin();
           in != arcAlignment->end(); ++in) {
        latticeAlignment_.push_back(
            AlignmentItem((*in).time, (*in).emission, arcWeight));
      }
    }
  }
}
