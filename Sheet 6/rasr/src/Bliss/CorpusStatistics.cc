...
void CorpusSizeStatisticsVisitor::writeReport(Core::XmlWriter &os) const {
  os << Core::XmlOpen("size-statistics")
     ...
     << "\n"
     << "gross duration (all recordings)          \t: " << totalGrossDuration_
     << "s"
     << "\n"
     << "net duration (all segments)              \t: " << totalNetDuration_
     << "s"
     << "\n"
...
void CorpusSpeakerStatisticsVisitor::accu(SpeakerStatistics &stat,
                                          const SpeechSegment *segment) {
  stat.nSegments += 1;
  stat.totalDuration += segment->end() - segment->start();
}

void CorpusSpeakerStatisticsVisitor::visitSpeechSegment(
...
void CorpusSpeakerStatisticsVisitor::writeReport(Core::XmlWriter &os) const {
  ...
  os << "number of speakers used                 \t: "
     << u32(speakerStatistics_.size()) << "\n";
  for (int g = 0; g < Speaker::nGenders; ++g) {
    os << u32(genderSpeakers_[g].size()) << " " << Speaker::genderId[g]
       << " speakers account for ";
    writeSpeakerStatistics(genderStatistics_[g], os);
  }
  for (SpeakerStatisticsMap::const_iterator i = speakerStatistics_.begin();
       i != speakerStatistics_.end(); ++i) {
...
void CorpusConditionStatisticsVisitor::accu(ConditionStatistics &stat,
...
void CorpusConditionStatisticsVisitor::writeReport(Core::XmlWriter &os) const {
  ...
  os << "number of conditions used                 \t: "
     << u32(conditionStatistics_.size()) << "\n";
  for (ConditionStatisticsMap::const_iterator i = conditionStatistics_.begin();
       i != conditionStatistics_.end(); ++i) {
...
public:
  typedef Core::hash_map<const Lemma *, u32,
                         Core::conversion<const Lemma *, size_t> >
      LemmaMap;
  ...
  u32 nUnmatchedOrth_;
  u32 nLemmas_, nNonLoopLemmas_, nPronunciations_, nNonLoopPronunciations_,
      nEvaluationSequences_, nNonLoopEvaluationSequences_;
  f64 nEvaluationTokens_, nNonLoopEvaluationTokens_, nPhonemes_;
  ...
  virtual void newEdge(OrthographicParser::Node from,
                       OrthographicParser::Node to, const Lemma *);
  virtual void newUnmatchableEdge(OrthographicParser::Node from,
...
CorpusLexicalStatisticsVisitor::CorpusLexicalStatisticsVisitor(
    const Core::Configuration &c, LexiconRef _lexicon)
    : CorpusStatisticsVisitor(c), lexicon_(_lexicon), orthographicParser_(0),
      internal_(0) {
...
void CorpusLexicalStatisticsVisitor::reset() { internal_->reset(); }

void CorpusLexicalStatisticsVisitor::Internal::initialize(
    const OrthographicParser *parser) {
  OrthographicParser::Handler::initialize(parser);

  drawChannel_ << "ranksep = 1.0;" << std::endl
               << "rankdir = LR;" << std::endl
               << "center = 1;" << std::endl
...
void CorpusLexicalStatisticsVisitor::Internal::newEdge(
  ...
  if (lemma) {
    ...
    for (Core::tie(e, e_end) = lemma->evaluationTokenSequences(); e != e_end;
         ++e)
      nEvaluationTokensInLemma_ += e->length();
    nEvaluationTokens_ += f64(nEvaluationTokensInLemma_) /
                          f64(lemma->nEvaluationTokenSequences());
    if (from != to) {
      nNonLoopLemmas_ += 1;
      nNonLoopPronunciations_ += lemma->nPronunciations();
      nNonLoopEvaluationSequences_ += lemma->nEvaluationTokenSequences();
      nNonLoopEvaluationTokens_ += f64(nEvaluationTokensInLemma_) /
                                   f64(lemma->nEvaluationTokenSequences());
    }
    ...
    if (lemma->nPronunciations()) {
      ...
      for (Core::tie(pron, pron_end) = lemma->pronunciations();
           pron != pron_end; ++pron)
...
void CorpusLexicalStatisticsVisitor::visitSpeechSegment(
    SpeechSegment *segment) {
  internal_->drawChannel_ << "digraph \"" << segment->fullName() << "\" {"
                          << std::endl;
...
void CorpusLexicalStatisticsVisitor::Internal::writeReport(
    Core::XmlWriter &os) const {
  os << "number of orthographic match positions         \t: "
     << nOrthMatchPositions_ << "\n"
     ...
     << "\n"
     << "number of matched pronunciations               \t: "
     << nPronunciations_ << "\n"
     << "number of non-loop matched pronunciations      \t: "
     << nNonLoopPronunciations_ << "\n"
     << "number of matched evaluation token sequences   \t: "
     << nEvaluationSequences_ << "\n"
     << "number of non-loop matched eval token sequences\t: "
     << nNonLoopEvaluationSequences_ << "\n"
     << "avg. number of matched evaluation tokens       \t: "
     << nEvaluationTokens_ << "\n"
     << "avg. number of non-loop matched eval tokens    \t: "
...
void CorpusLexicalStatisticsVisitor::writeReport(Core::XmlWriter &os) const {
  ...
  for (Core::tie(l, l_end) = lexicon_->lemmas(); l != l_end; ++l) {
    ...
    if (internal_->lemmaCounts_.find(lemma) == internal_->lemmaCounts_.end())
      continue;
    const u32 count =
        const_cast<Internal::LemmaMap &>(internal_->lemmaCounts_)[lemma];
    os << Core::XmlOpen("lemma") + Core::XmlAttribute("name", lemma->name()) +
  ...
  for (std::vector<std::string>::const_iterator i = list.begin();
       i != list.end(); ++i) {
    const u32 count = const_cast<Internal::UnmatchedOrthMap &>(
        internal_->unmatchedOrthCounts_)[*i];
  ...
  os << Core::XmlFull(
      ...
      (nWordFailurePositions / (nWordMatchPositions + nWordFailurePositions)) *
          100);
...
void CompositeCorpusStatisticsVisitor::writeReport(Core::XmlWriter &os) const {
  for (ComponentList::const_iterator i = components_.begin();
       i != components_.end(); ++i)
    (*i)->writeReport(os);
}
