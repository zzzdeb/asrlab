...
const Core::ParameterString
    WordLatticeUnion::paramFsaPrefix("fsa-prefix",
...
WordLatticeUnion::WordLatticeUnion(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), prefix_(paramFsaPrefix(config)),
      numeratorArchiveReader_(0) {}

WordLatticeUnion::~WordLatticeUnion() { delete numeratorArchiveReader_; }

void WordLatticeUnion::initialize(Bliss::LexiconRef lexicon) {
  Precursor::initialize(lexicon);

  verify(!numeratorArchiveReader_);
  if (!Lattice::Archive::paramPath(select("numerator-lattice-archive"))
           .empty()) {
...
void WordLatticeUnion::processWordLattice(
  ...
  if (denominator and denominator->nParts() == 1) {
    ...
    if (numerator) {
      ...
      warning("no union for '%s' because there is no numerator lattice",
              s->fullName().c_str());
    }
...
const Core::ParameterBool
    BaseWordLatticeMerger::paramMergeOnlyIfSpokenNotInLattice(
        "merge-only-if-spoken-not-in-lattice",
        "merge denominator and numerator lattice only if spoken sentence is "
        "not in denominator lattice",
        true);

BaseWordLatticeMerger::BaseWordLatticeMerger(const Core::Configuration &c)
    : Core::Component(c), Precursor(c),
      mergeOnlyIfSpokenNotInLattice_(
          paramMergeOnlyIfSpokenNotInLattice(config)),
      orthToLemma_(0) {}

BaseWordLatticeMerger::~BaseWordLatticeMerger() { delete orthToLemma_; }

bool BaseWordLatticeMerger::needsMerging(Lattice::ConstWordLatticeRef lattice,
                                         Bliss::SpeechSegment *s) const {
  if (mergeOnlyIfSpokenNotInLattice_) {
    verify(orthToLemma_);
    Lattice::ConstWordLatticeRef spokenFromLattice =
        Lattice::extractNumerator(s->orth(), lattice, orthToLemma_,
                                  lemmaPronToLemma_, lemmaToLemmaConfusion_);
    if (spokenFromLattice and spokenFromLattice->mainPart() and
...
void BaseWordLatticeMerger::initialize(Bliss::LexiconRef lexicon) {
  ...
  if (mergeOnlyIfSpokenNotInLattice_) {
    verify(!orthToLemma_);
    orthToLemma_ =
        new Bliss::OrthographicParser(select("orthographic-parser"), lexicon);
    ...
    lemmaToLemmaConfusion_ =
        Fsa::composeMatching(lemmaToEval, Fsa::invert(lemmaToEval));
...
void WordLatticeMerger::initialize(Bliss::LexiconRef lexicon) {
  ...
  languageModel_ =
      Lm::Module::instance().createLanguageModel(select("lm"), lexicon);
  if (!languageModel_)
  ...
  if (denominator and denominator->nParts() == 1) {
    if (needsMerging(denominator, s)) {
      ...
      if (numerator) {
        ...
        warning("no merge for '%s' because there is no numerator lattice",
                s->fullName().c_str());
      }
    }
...
const Core::ParameterInt
    SpokenAndCompetingListProcessor::paramNumberOfHypotheses(
        "number-of-hypotheses", "target number of hypotheses after merge", -1);
...
void SpokenAndCompetingListProcessor::processWordLattice(
  ...
  if (l) {
    ...
    hypotheses.push_back(numeratorArchiveReader_->get(s->fullName(), prefix()));
    ...
    for (Fsa::State::const_iterator hIt = lSp->begin();
         ...
         ++hIt) {
      Lattice::ConstWordLatticeRef h =
          Lattice::partial(l, hIt->target(), hIt->weight());
      if (needsMerging(h, s)) {
    ...
    for (u32 n = 0; n < hypotheses.size(); ++n) {
      hypotheses[n] = Lattice::staticCopy(Lattice::normalize(hypotheses[n]));
    }
...
NumeratorFromDenominatorExtractor::NumeratorFromDenominatorExtractor(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c), orthToLemma_(0) {}
...
void NumeratorFromDenominatorExtractor::processWordLattice(
  ...
  Lattice::ConstWordLatticeRef numeratorFromDenominator =
      Lattice::extractNumerator(s->orth(), denominator, orthToLemma_,
                                lemmaPronToLemma_, lemmaToLemmaConfusion_);
