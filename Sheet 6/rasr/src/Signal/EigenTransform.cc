...
const Core::ParameterInt EigenTransform::paramReducedDimension(
    "reduced-dimesion", "projected dimesion, if 0, dimension is not changed.",
    0, 0);

const Core::ParameterFloat EigenTransform::paramReducedDimensionByThreshold(
    "reduced-dimesion-threshold",
    "projected dimesion depending on eigenwert values which will be reduced if "
    "lower than threshold; if 0, dimension is not changed.",
    0.0, 0.0);
...
bool EigenTransform::createProjector() {
  ...
  if ((reducedDimension != 0) && (reducedDimensionThreshold != 0.0)) {
    error("Projected-dimension must be defined by either 'reduced-dimesion' or "
          "'reduced-dimesion-threshold', not both");
  ...
  if (reducedDimensionThreshold != 0.0) {
    ...
    //         std::cout << "*** debug: reduced by threshold " <<
    //         reducedDimensionThreshold
    //                   << " from dimension " << eigenvectors_.nColumns()
    //                   << " to " << reducedDimension
    //                   << std::endl;
    if (reducedDimension == 0)
      warning(
          "Projected-dimension was reduced to zero, i.e. reduction will be "
          "disabled. "
  ...
  if (reducedDimension > eigenvectors_.nColumns()) {
    error("Projected-dimension (%zd) is larger than number of eigenvectors "
          "(%zd).",
          reducedDimension, eigenvectors_.nColumns());
    reducedDimension = eigenvectors_.nColumns();
    success = false;
...
PrincipalComponentAnalysis::PrincipalComponentAnalysis(
    ...
    : Core::Component(configuration), Precursor(configuration),
      ...
      covarianceMatrixChannel_(configuration, "covariance-matrix",
                               Core::Channel::disabled) {
  eigenvalueProblem_ = Math::Module::instance().createEigenvalueProblem(
...
bool PrincipalComponentAnalysis::work(const ScatterMatrix &covarianceMatrix) {
  ...
  if (covarianceMatrixChannel_.isOpen())
    writeScatterMatrix(covarianceMatrixChannel_, totalScatterType,
                       covarianceMatrix);
  bool success = true;
  if (!eigenvalueProblem_->solveSymmetricAndFinalize(
          covarianceMatrix, eigenvalues_, eigenvectors_))
    success = false;
  if (!Precursor::work())
    success = false;
  return success;
...
bool PrincipalComponentAnalysis::work() {
  ...
  return (readScatterMatrix(paramTotalScatterFilename(config), totalScatterType,
                            covarianceMatrix) &&
...
LinearDiscriminantAnalysis::LinearDiscriminantAnalysis(
    const Core::Configuration &configuration)
    : Core::Component(configuration), Precursor(configuration),
      generalizedEigenvalueProblem_(0), eigenvalueProblem_(0),
      betweenClassScatterMatrixChannel_(configuration,
                                        "between-class-scatter-matrix",
                                        Core::Channel::disabled),
      withinClassScatterMatrixChannel_(configuration,
...
bool LinearDiscriminantAnalysis::work(
    const ScatterMatrix &betweenClassScatterMatrix,
  ...
  if (withinClassScatterMatrixChannel_.isOpen()) {
    writeScatterMatrix(withinClassScatterMatrixChannel_, withinClassScatterType,
                       withinClassScatterMatrix, eigenvalueProblem_);
  }

  bool success = true;
  if (!generalizedEigenvalueProblem_->solveSymmetricAndFinalize(
          betweenClassScatterMatrix, withinClassScatterMatrix, eigenvalues_,
          eigenvectors_))
    success = false;
  if (!Precursor::work())
    success = false;
