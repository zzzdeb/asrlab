...
const Core::ParameterBool
    Evaluator::paramWordErrors("word-errors",
                               "compute edit-distance on word level", true);
const Core::ParameterBool Evaluator::paramLetterErrors(
    "letter-errors", "compute edit-distance on letter level", false);
const Core::ParameterBool Evaluator::paramPhonemeErrors(
    "phoneme-errors", "compute edit-distance on phoneme level", false);
...
public:
  ...
  virtual OrthographicParser::Node newNode() { return product_->newState(); }

  void buildString(Fsa::State *source, Fsa::State *target,
  ...
  void newUnmatchableEdge(OrthographicParser::Node from,
  ...
  void finalize(OrthographicParser::Node initial,
...
Evaluator::Evaluator(const Core::Configuration &c, Core::Ref<const Lexicon> l)
    : Core::Component(c), lexicon_(l), orthParser_(), letterAcceptorBuilder_(0),
      editDistance_(0), graphStatisticsChannel_(config, "graph-statistics"),
  ...
  lemmaPronToLemma_ = Fsa::multiply(
      lexicon_->createLemmaPronunciationToLemmaTransducer(), Fsa::Weight(0.0));
  lemmaToSynt_ = Fsa::multiply(
      lexicon_->createLemmaToSyntacticTokenTransducer(), Fsa::Weight(0.0));
  lemmaToEval_ = Fsa::multiply(
      lexicon_->createLemmaToEvaluationTokenTransducer(), Fsa::Weight(0.0));
  //    lemmaToPreferredEval_ =
  //    Fsa::multiply(lexicon_->createLemmaToPreferredEvaluationTokenSequenceTransducer(),
  //    Fsa::Weight(0.0));

  if (shallComputeLetterErrors_ || shallComputePhonemeErrors_) {
    lemmaToLemmaConfusion_ =
        Fsa::composeMatching(lemmaToEval_, Fsa::invert(lemmaToEval_));
  ...
  if (shallComputePhonemeErrors_) {
    ...
    lemmaToPhoneme_ = Fsa::invert(Fsa::removeDisambiguationSymbols(
        Fsa::fuse(Fsa::composeSequencing(phonToPron, lemmaPronToLemma_),
...
void Evaluator::setReferenceTranscription(
    const std::string &referenceTranscription) {
  correct_.lemma = orthParser_->createLemmaAcceptor(referenceTranscription);

  graphStatisticsChannel_ << Core::XmlOpen("reference") +
                                 Core::XmlAttribute("type", "lemma");
  ...
  if (shallComputeWordErrors_) {
    correct_.eval =
        Fsa::staticCopy(Fsa::minimize(Fsa::determinize(Fsa::projectOutput(
            Fsa::composeMatching(correct_.lemma, lemmaToEval_)))));

    graphStatisticsChannel_
        << Core::XmlOpen("reference") + Core::XmlAttribute("type", "eval");
    if (graphStatisticsChannel_.isOpen())
  ...
  if (shallComputeLetterErrors_) {
    ...
    correct_.orth = Fsa::staticCopy(Fsa::minimize(Fsa::determinize(
        Fsa::removeEpsilons(letterAcceptorBuilder_->product()))));

    graphStatisticsChannel_
        << Core::XmlOpen("reference") + Core::XmlAttribute("type", "orth");
    if (graphStatisticsChannel_.isOpen())
  ...
  if (shallComputePhonemeErrors_) {
    Fsa::ConstAutomatonRef confusedLemma =
        Fsa::minimize(Fsa::determinize(Fsa::removeEpsilons(Fsa::projectOutput(
            Fsa::composeMatching(correct_.lemma, lemmaToLemmaConfusion_)))));
    correct_.phon =
        Fsa::staticCopy(Fsa::minimize(Fsa::determinize(Fsa::projectOutput(
            Fsa::composeMatching(confusedLemma, lemmaToPhoneme_)))));

    graphStatisticsChannel_
        << Core::XmlOpen("reference") + Core::XmlAttribute("type", "phon");
    if (graphStatisticsChannel_.isOpen())
    ...
    if (Fsa::isEmpty(correct_.phon)) {
      error("The set of correct phoneme sequences is empty.  Phoneme errors "
            "cannot be computed.");
      correct_.phon.reset();
    }
  } else {
 ...
 */
void Evaluator::reportLatticeDensity(Fsa::ConstAutomatonRef lattice,
...
u32 Evaluator::evaluateWords(Fsa::ConstAutomatonRef lemmaOrPronOrSyntOrEval,
                             const std::string &name) {
  require(!Fsa::isEmpty(lemmaOrPronOrSyntOrEval));
  require(lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
              lexicon_->lemmaPronunciationAlphabet() ||
          lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
              lexicon_->lemmaAlphabet() ||
          lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
              lexicon_->syntacticTokenAlphabet() ||
          lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
              lexicon_->evaluationTokenAlphabet());

  Fsa::ConstAutomatonRef lemma, lemmaPron;
  if (lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
      lexicon_->lemmaPronunciationAlphabet()) {
    lemmaPron = lemmaOrPronOrSyntOrEval;
    lemma = Fsa::cache(Fsa::composeMatching(lemmaPron, lemmaPronToLemma_));
  } else if (lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
             lexicon_->lemmaAlphabet()) {
    lemma = lemmaOrPronOrSyntOrEval;
  } else if (lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
             lexicon_->syntacticTokenAlphabet()) {
    lemma = Fsa::cache(Fsa::composeMatching(lemmaOrPronOrSyntOrEval,
                                            Fsa::invert(lemmaToSynt_)));
  } else if (lemmaOrPronOrSyntOrEval->getOutputAlphabet() ==
             lexicon_->evaluationTokenAlphabet()) {
    lemma = Fsa::cache(Fsa::composeMatching(lemmaOrPronOrSyntOrEval,
                                            Fsa::invert(lemmaToEval_)));
  }
  ...
  if (shallComputeWordErrors_) {
    Fsa::ConstAutomatonRef eval =
        Fsa::projectOutput(Fsa::composeMatching(lemma, lemmaToEval_));
  ...
  if (shallComputeLetterErrors_) {
    Fsa::ConstAutomatonRef orth =
        Fsa::projectOutput(Fsa::composeMatching(lemma, lemmaToLetter_));
  ...
  if (shallComputePhonemeErrors_ && correct_.phon && lemmaPron) {
    Fsa::ConstAutomatonRef phon =
        Fsa::removeDisambiguationSymbols(Fsa::projectOutput(
            Fsa::composeMatching(lemmaPron, lemmaPronToPhoneme_)));
...
u32 Evaluator::evaluatePhonemes(Fsa::ConstAutomatonRef phon,
  ...
  if (shallComputePhonemeErrors_ && correct_.phon) {
    ErrorStatistic errors = evaluateMetric(
        correct_.phon, Fsa::staticCopy(Fsa::trim(phon)), "phon", name);
...
ErrorStatistic Evaluator::evaluateMetric(Fsa::ConstAutomatonRef correct,
                                         Fsa::ConstAutomatonRef candidate,
                                         const std::string &type,
                                         const std::string &name) {
  Core::XmlWriter &os(clog());
  os << Core::XmlOpen("evaluation") + Core::XmlAttribute("name", name) +
  ...
  for (EditDistance::Alignment::const_iterator ai = al.begin(); ai != al.end();
       ++ai)
    if (ai->b)
