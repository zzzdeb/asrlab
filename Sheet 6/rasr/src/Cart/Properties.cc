...
void PropertyMap::reset() {
  if (values_) {
    for (ChoicePtrList::iterator it = values_->begin(); it != values_->end();
         ++it)
      delete *it;
    delete values_;
    values_ = 0;
...
void PropertyMap::write(std::ostream &os) const {
  ...
  for (size_t i = 0; i < size(); ++i) {
    os << std::setw(3) << std::right << (i + 1) << ". " << std::setw(maxKeySize)
       << std::left << key(i) << " : ";
...
void PropertyMap::writeXml(Core::XmlWriter &xml) const {
  ...
  for (size_t i = 0; i < size(); ++i) {
    xml << Core::XmlFull("key", key(i)) << Core::XmlOpen("value-map");
    for (Core::Choice::const_iterator it = operator[](i).begin();
         it != operator[](i).end(); ++it)
      xml << Core::XmlFull("value", it->ident()) +
                 Core::XmlAttribute("id", it->value());
...
bool PropertyMapDiff::equalSymbolic(const std::string &key) {
  ...
  for (Core::Choice::const_iterator it = values1.begin(); it != values1.end();
       ++it)
    if (values2[it->ident()] == Core::Choice::IllegalValue)
      return false;
  for (Core::Choice::const_iterator it = values2.begin(); it != values2.end();
       ++it)
...
void PropertyMapDiff::logKey(Core::XmlWriter &xml, const std::string &name,
                             const PropertyMap &map, std::string key) {
...
void PropertyMapDiff::logUniq(Core::XmlWriter &xml, std::string key, u16 diff) {
...
void PropertyMapDiff::logDiff(Core::XmlWriter &xml, std::string key) {
...
PropertyMapDiff::PropertyMapDiff(const Core::Configuration &config,
                                 ...
                                 const PropertyMap &map2, bool compareSymbolic)
    : Core::Component(config), map1_(map1), map2_(map2), differences_(0) {
...
void Properties::write(std::ostream &os) const {
  ...
  for (size_t i = 0; i < map_->size(); ++i)
    os << std::setw(3) << std::right << (i + 1) << ". " << std::setw(maxKeySize)
       << std::left << map_->key(i) << " : "
       << ((map_->isDefined(id = operator[](i))) ? (*map_)[i][id]
                                                 : "<undefined>")
       << std::endl;
}
...
StoredProperties::StoredProperties(PropertyMapRef map, const StringList &keys,
                                   const StringList &values)
    : Properties(map), valueIndexes_(map->size(), map_->undefinedIndex) {
  require(keys.size() == values.size());
