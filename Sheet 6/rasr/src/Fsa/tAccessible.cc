...
void CoaccessibleDfsState<_Automaton>::discoverState(_ConstStateRef sp) {
  ...
  flags_[s] =
      time_++ |
      FlagUnfinished; // | FlagAccessible; // states are accessible by default
  if (sp->isFinal())
...
void CoaccessibleDfsState<_Automaton>::finishState(Fsa::StateId s) {
  if (s == roots_.top()) {
    ...
    if (!coaccessible) {
      for (Fsa::Stack<Fsa::StateId>::const_iterator i = unfinished_.begin();
           *i != s; ++i)
...
template <class _Automaton>
void CoaccessibleDfsState<_Automaton>::exploreNonTreeArc(_ConstStateRef from,
                                                         const _Arc &a) {
