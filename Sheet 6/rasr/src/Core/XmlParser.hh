...
private:
  static xmlParserInputPtr SAX_resolveEntity_callback(void *ctx,
                                                      const xmlChar *publicId,
                                                      const xmlChar *systemId);
  static void SAX_internalSubset_callback(void *ctx, const xmlChar *name,
                                          const xmlChar *ExternalID,
                                          const xmlChar *SystemID);
  static void SAX_externalSubset_callback(void *ctx, const xmlChar *name,
                                          const xmlChar *ExternalID,
                                          const xmlChar *SystemID);
  static xmlEntityPtr SAX_getEntity_callback(void *ctx, const xmlChar *name);
  static xmlEntityPtr SAX_getParameterEntity_callback(void *ctx,
                                                      const xmlChar *name);
  static void SAX_entityDecl_callback(void *ctx, const xmlChar *name, int type,
                                      const xmlChar *publicId,
                                      const xmlChar *systemId,
                                      xmlChar *content);
  static void SAX_notationDecl_callback(void *ctx, const xmlChar *name,
                                        const xmlChar *publicId,
                                        const xmlChar *systemId);
  static void SAX_attributeDecl_callback(void *ctx, const xmlChar *elem,
                                         const xmlChar *name, int type, int def,
                                         const xmlChar *defaultValue,
                                         xmlEnumerationPtr tree);
  static void SAX_elementDecl_callback(void *ctx, const xmlChar *name, int type,
                                       xmlElementContentPtr content);
  static void SAX_unparsedEntityDecl_callback(void *ctx, const xmlChar *name,
                                              const xmlChar *publicId,
                                              const xmlChar *systemId,
  ...
  static void SAX_startElement_callback(void *ctx, const xmlChar *name,
                                        const xmlChar **atts);
  ...
  static void SAX_ignorableWhitespace_callback(void *ctx, const xmlChar *ch,
                                               int len);
  static void SAX_processingInstruction_callback(void *ctx,
...
protected:
  ...
  virtual xmlParserInputPtr resolveEntity(const char *publicId,
                                          const char *systemId);
  virtual void internalSubset(const char *name, const char *ExternalID,
                              const char *SystemID);
  virtual void externalSubset(const char *name, const char *ExternalID,
                              const char *SystemID);
  virtual xmlEntityPtr getEntity(const char *name);
  virtual xmlEntityPtr getParameterEntity(const char *name);
  virtual void entityDecl(const char *name, int type, const char *publicId,
                          const char *systemId, const char *content);
  virtual void notationDecl(const char *name, const char *publicId,
                            const char *systemId);
  virtual void attributeDecl(const char *elem, const char *name, int type,
                             int def, const char *defaultValue,
                             xmlEnumerationPtr tree);
  virtual void elementDecl(const char *name, int type,
                           xmlElementContentPtr content);
  virtual void unparsedEntityDecl(const char *name, const char *publicId,
                                  const char *systemId,
   ...
   **/
  virtual XmlChannel *vErrorMessage(ErrorType mt, const char *msg,
                                    va_list) const;
...
public:
  ...
  template <class T> static EndHandler endHandler(void (T::*h)()) {
    return static_cast<EndHandler>(h);
  }

  template <class T>
  static CharactersHandler charactersHandler(void (T::*h)(const char *ch,
                                                          int len)) {
...
public:
  virtual void start(const XmlAttributes atts);
  XmlEmptyElementRelay(const char *_name, XmlContext *_context,
...
public:
  ...
  XmlFlattenElementRelay(const char *_name, XmlContext *_context,
...
public:
  ...
  XmlMixedElementRelay(const char *_name, XmlContext *_context,
                       XmlContext::StartHandler = 0, XmlContext::EndHandler = 0,
                       XmlContext::CharactersHandler = 0);

  XmlMixedElementRelay(const char *_name, XmlContext *_context,
                       XmlContext::StartHandler, XmlContext::EndHandler,
                       XmlContext::CharactersHandler, XmlElement *child, ...);
};
...
public:
  ...
  XmlRegularElementRelay(const char *_name, XmlContext *_context,
                         XmlContext::StartHandler = 0,
                         XmlContext::EndHandler = 0);
};
 ...
 * "Design Patterns" (<a
 * href="http://www-bib.informatik.rwth-aachen.de/cgi-bin/bibquery.cgi?nr=800815">available
 * in library</a>) we apply the State pattern to concentrate the state dependent
...
class XmlSchemaParser : public XmlParser, public XmlDocumentElement {
