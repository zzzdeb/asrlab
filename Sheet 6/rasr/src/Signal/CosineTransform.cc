...
void CosineTransform::init(InputType inputType, size_t inputSize,
                           size_t outputSize, bool normalize,
  ...
  Math::UnaryAnalyticFunctionRef derivedWarpingFunction =
      shouldWarpDifferentialUnit
          ? warpingFunction->derive()
          : Math::AnalyticFunctionFactory::createConstant(1);
  ensure(derivedWarpingFunction);

  switch (inputType) {
  case NplusOneData:
    initNplusOneData(inputSize, outputSize, warpingFunction,
                     derivedWarpingFunction);
    break;
  case evenAboutNminusHalf:
    initEvenAboutNminusHalf(inputSize, outputSize, warpingFunction,
                            derivedWarpingFunction);
    break;
  default:
    defect();
...
void CosineTransform::apply(const std::vector<Value> &in,
  ...
  if (normalize_) {
    ...
    std::transform(out.begin(), out.end(), out.begin(),
                   std::bind2nd(std::divides<Value>(), N_));
  }
}

//==================================================================================================
const Core::Choice CosineTransformNode::choiceInputType(
    "N-plus-one", NplusOneData, "even-about-N-minus-half", evenAboutNminusHalf,
    Core::Choice::endMark());
const Core::ParameterChoice CosineTransformNode::paramInputType(
    "input-type", &choiceInputType, "Input: (N+1) / (N and even about N-0.5)",
    evenAboutNminusHalf);

const Core::ParameterInt
    CosineTransformNode::paramOutputSize("nr-outputs", "number of outputs", 1,
                                         1);
...
const Core::ParameterString
    CosineTransformNode::paramWarpingFunction("warping-function",
...
CosineTransformNode::CosineTransformNode(const Core::Configuration &c)
    : Component(c), Node(c), StringExpressionNode(c, 1),
      inputType_(evenAboutNminusHalf), outputSize_(0),
      frequencyDomainSampleRate_(0), normalize_(false),
      shouldWarpDifferenctialUnit_(true), needInit_(false) {
  addInput(0);
  addOutput(0);
...
void CosineTransformNode::init(size_t inputSize) {
  ...
  Math::UnaryAnalyticFunctionRef warpingFunction =
      createWarpingFunction(sampleRate);
  respondToDelayedErrors();
...
CosineTransformNode::createWarpingFunction(f64 sampleRate) {
...
bool CosineTransformNode::setParameter(const std::string &name,
...
bool CosineTransformNode::work(Flow::PortId p) {
  ...
  if (in->size() != inputSize())
    criticalError(
        "Input size (%zd) does not match the expected input size (%zd)",
        in->size(), inputSize());
