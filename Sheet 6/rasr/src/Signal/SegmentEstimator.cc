...
const DynamicProgramingSegmentEstimator::_float
    DynamicProgramingSegmentEstimator::infinite =
        Core::Type<DynamicProgramingSegmentEstimator::_float>::max;

bool DynamicProgramingSegmentEstimator::precalculateSegmentError() {
  if (segmentwise_estimator_ == 0)
    return false;

  for (s32 segment_begin = 0; segment_begin < max_segment_value_ + step_;
       segment_begin += step_) {
    ...
    for (s32 segment_end = segment_begin;
      ...
      else if (!segmentwise_estimator_->work(
                   segmentError__(segment_begin, segment_end))) {
        // return false;
        segmentError__(segment_begin, segment_end) = infinite;
      }
      // else
      // segmentError__(segment_begin, segment_end) /= (_float)((segment_end -
...
bool DynamicProgramingSegmentEstimator::buildBackPointerMatrix() {
  ...
  for (s32 segment_end = step_; segment_end < max_segment_value_ + step_;
       segment_end += step_) {
    ...
    for (u8 nr_segment = 1;
         nr_segment <= std::min((s32)max_nr_segment_, segment_end);
         nr_segment++) {
      ...
      for (s32 segment_begin = (nr_segment - 1) * step_;
        ...
        /*
          std::cout << nr_segment - 1 << " " << segment_begin <<  " " <<
          segment_end << " "
