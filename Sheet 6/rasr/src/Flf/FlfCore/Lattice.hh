 ...
 *
 **/
class StateMap : public Core::Vector<Fsa::StateId>,
...
struct Arc {
  ...
  Arc(Label label, ScoresRef scores)
      : label(label), scores(scores), begin(Speech::InvalidTimeframeIndex),
        duration(0), from(Core::Type<u32>::max), to(Core::Type<u32>::max) {}
  ...
  Arc(Label label, ScoresRef scores, Time begin, Time duration, u32 from,
      u32 to)
      : label(label), scores(scores), begin(begin), duration(duration),
        from(from), to(to) {}
  inline bool operator<(const Arc &a) const { return label < a.label; }
};
...
struct PosteriorArc {
  ...
  inline bool operator<(const PosteriorArc &a) const { return label < a.label; }
};
...
public:
  ...
  struct NormalizedProperties : public Core::ReferenceCounted {
    ...
    Score posteriorScore(const ConfusionNetwork::Slot &slot,
  ...
  typedef Core::Ref<const NBestAlignmentProperties>
      ConstNBestAlignmentPropertiesRef;

  /**
   * Additive to "normalized": probabiliy is normalized over all arcs in slot
   *but the first (aligned) arc Additive to "n-best alignment": each slot has
   *n+1 arcs, where the first arc is the aligned arc
   **/
  ...
  typedef Core::Ref<const OracleAlignmentProperties>
      ConstOracleAlignmentPropertiesRef;

  struct MapProperties : public Core::ReferenceCounted {
    // Mapping might be invalid:
    // - invalid aid means there is no corresponding arc
    // - invalid sid means there is no corresponding state/slot; this implies
    // invalid aid
    struct Mapping {
      Fsa::StateId sid;
      Fsa::StateId aid;
...
public:
  ...
  void scores(ProbabilityList::iterator begin, ProbabilityList::iterator end,
              size_t t, Fsa::LabelId label) const;
};
