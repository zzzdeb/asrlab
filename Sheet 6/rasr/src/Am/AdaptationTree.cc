...
const Core::ParameterInt AdaptationTree::paramNumberOfBaseClasses(
    "base-classes", "number of leafs in MLLR tree", 70, 1);
...
void AdaptationTree::loadLegacyDecisionTree(
  ...
  const Am::Allophone *siAllo = stateModel->allophoneAlphabet().allophone(
      Am::Allophone(silencePhoneme, Am::Allophone::isInitialPhone |
                                        Am::Allophone::isFinalPhone));
  Am::AllophoneState siAlloState =
      stateModel->allophoneStateAlphabet().allophoneState(siAllo, 0);
...
void AdaptationTree::loadDecisionTree(const Core::Configuration &config,
  ...
  // Build tree structure as intermediate representation for getting a
  // Core::BinaryTree deprecated: use directly Cart::DecisionTree instead of
  // Core::BinaryTree deprecated: don't separate explicitly silence, rather
  // verify that silence is in the pruned tree
  Core::BinaryTree::TreeStructure structure;
  {
    ...
    /*
      ...
      the first question asks the central phoneme for being the (unique,
      single-state) silence phoneme, the left node (answer=YES) is a leaf, while
      the right node is splitted further
    */
    ...
    while (!S.empty()) {
      ...
      if (node->isLeaf()) {
        ...
        if (leafNumber > siMixtureId)
          --leafNumber;
        structure.push_back(
            Core::BinaryTree::TreeStructureEntry(order, leafNumber));
      } else {
        structure.push_back(Core::BinaryTree::TreeStructureEntry(
            order, Core::BinaryTree::invalidId));
