...
protected:
  ...
  Flow::PortId portId() const { return portId_; }
  Flow::PortId sparsePortId() const { return sparsePortId_; }
  Flow::PortId accumulationPortId() const { return accumulationPortId_; }
  Flow::PortId accumulationSparsePortId() const {
    return accumulationSparsePortId_;
  }
  ConstSegmentwiseFeaturesRef features(Flow::PortId portId,
...
public:
  ...
  virtual void processWordLattice(Lattice::ConstWordLatticeRef,
                                  Bliss::SpeechSegment *);
  virtual void setSegmentwiseFeatureExtractor(
      Core::Ref<Speech::SegmentwiseFeatureExtractor>);
  virtual void setAlignmentGenerator(
      Core::Ref<Speech::PhonemeSequenceAlignmentGenerator>);

  static AbstractAcousticSegmentwiseTrainer *
  createAbstractAcousticSegmentwiseTrainer(const Core::Configuration &);
};
...
protected:
  ...
  void accumulateNumerator(Fsa::ConstAutomatonRef fsa,
  ...
  void accumulateDenominator(Fsa::ConstAutomatonRef fsa,
                             Core::Ref<const Lattice::WordBoundaries> wb) {
    A *acc = createDenAcc();
    acc->setWordBoundaries(wb);
    acc->setFsa(fsa);
  ...
  void accumulateMle(Fsa::ConstAutomatonRef fsa,
                     Core::Ref<const Lattice::WordBoundaries> wb) {
    A *acc = createMleAcc();
    acc->setWordBoundaries(wb);
    acc->setFsa(fsa);
