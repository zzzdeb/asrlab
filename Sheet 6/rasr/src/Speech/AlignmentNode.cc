...
const Core::ParameterString
    AlignmentBaseNode::paramOrthography("orthography",
                                        "orthography of current segment");

AlignmentBaseNode::AlignmentBaseNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), allophoneStateGraphBuilder_(0),
      modelCache_(0), needInit_(true) {
...
bool AlignmentBaseNode::setParameter(const std::string &name,
...
const Core::ParameterBool
    AlignmentNode::paramStoreLattices("store-lattices",
                                      "store word lattices in archive", false);

const Core::ParameterBool AlignmentNode::paramUseTracebacks(
    "use-tracebacks", "align to tracebacks from archive instead of orthography",
    false);

const Core::Choice AlignmentNode::choicePhonemeSequenceSet(
    "lemma-loop", lemmaLoop, "phone-loop", phoneLoop, "orthography",
    orthography, Core::Choice::endMark());

const Core::ParameterChoice AlignmentNode::paramPhonemeSequenceSet(
    "phoneme-sequence-set", &choicePhonemeSequenceSet,
...
const Core::ParameterBool AlignmentNode::paramNoDependencyCheck(
    "no-dependency-check", "do not check any dependencies", false);

AlignmentNode::AlignmentNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), wordLatticeBuilder_(0),
      phonemeSequenceSet_((PhonemeSequenceSet)paramPhonemeSequenceSet(config)),
      noDependencyCheck_(paramNoDependencyCheck(c)),
      tracebackChannel_(config, "traceback"), latticeArchiveWriter_(0),
      tracebackArchiveReader_(0), transducerCache_(0),
...
void AlignmentNode::createModel() {
  ...
  Fsa::ConstAutomatonRef
      transducer; // allophone state to lemma-pronunciation transducer
  if (wordLatticeBuilder_) {
    ...
    transducer = transducerCache_->get(
        allophoneStateGraphBuilder_->createTransducerFunctor(segmentId_,
                                                             orthography_));
  ...
  if (!model) {
    if (tracebackArchiveReader_) {
      Lattice::ConstWordLatticeRef lattice =
          tracebackArchiveReader_->get(segmentId_);
    ...
    if (transducer) {
      model = modelCache_->get(
          allophoneStateGraphBuilder_->createFinalizationFunctor(segmentId_,
                                                                 transducer));
    } else if (phonemeSequenceSet_ == orthography) {
      model = modelCache_->get(
          allophoneStateGraphBuilder_->createFunctor(segmentId_, orthography_));
    } else if (phonemeSequenceSet_ == lemmaLoop ||
               phonemeSequenceSet_ == phoneLoop) {
      model = aligner_.getModel();
      if (!model) {
        if (phonemeSequenceSet_ == lemmaLoop) {
          model = modelCache_->get(allophoneStateGraphBuilder_->createFunctor(
              segmentId_, AllophoneStateGraphBuilder::lemma));
        } else {
          model = modelCache_->get(allophoneStateGraphBuilder_->createFunctor(
              segmentId_, AllophoneStateGraphBuilder::phone));
        }
      }
    } else {
    ...
    warning("Using model acceptor from cache. Dependencies on Traceback "
...
void AlignmentNode::initialize() {
  ...
  if (tracebackChannel_.isOpen() || paramStoreLattices(config)) {
    wordLatticeBuilder_ = new Aligner::WordLatticeBuilder(
        select("word-lattice-builder"), modelCombination.lexicon(),
        acousticModel_);
    ...
    modelCombination.getDependencies(dependencies);
    transducerCache_ =
        new FsaCache(select("transducer-cache"), Fsa::storeStates);
    transducerCache_->setDependencies(dependencies);
    transducerCache_->forceInputAlphabet(
        acousticModel_->allophoneStateAlphabet());
    transducerCache_->respondToDelayedErrors();
  }

  if (paramUseTracebacks(config)) {
    log("opening traceback archive");
    tracebackArchiveReader_ = Lattice::Archive::openForReading(
        select("traceback-archive"), modelCombination.lexicon());
    if (tracebackArchiveReader_->hasFatalErrors()) {
      delete tracebackArchiveReader_;
      tracebackArchiveReader_ = 0;
    }
    lemmaPronunciationToLemma_ =
        modelCombination.lexicon()->createLemmaPronunciationToLemmaTransducer();
  }
...
bool AlignmentNode::work(Flow::PortId p) {
  ...
  if (!aligner_.reachedFinalState())
    warning("Alignment did not reach any final state in segment '%s'",
            segmentId_.c_str());

  Fsa::ConstAutomatonRef alignmentFsa = aligner_.getAlignmentFsa();
  if (aligner_.reachedFinalState() &&
      (paramStoreLattices(config) || tracebackChannel_.isOpen()))
    createWordLattice(alignmentFsa);

  std::pair<Fsa::ConstAutomatonRef, Fsa::Weight> alignmentPosteriorFsa =
      aligner_.getAlignmentPosteriorFsa(alignmentFsa);
...
void AlignmentNode::createWordLattice(
    Fsa::ConstAutomatonRef alignmentFsa) const {
  verify(wordLatticeBuilder_);

  Lattice::ConstWordLatticeRef wordLattice(
      wordLatticeBuilder_->build(alignmentFsa));
  if (wordLattice) {
    if (paramStoreLattices(config)) {
      verify(latticeArchiveWriter_);
    ...
    error("Failed to generate word lattice for the segment '%s'.",
          segmentId_.c_str());
}

void AlignmentNode::logTraceback(
    Lattice::ConstWordLatticeRef wordLattice) const {
  Fsa::ConstAutomatonRef fn =
      wordLattice->part(Lattice::WordLattice::acousticFsa);
  ...
  while (!state->isFinal()) {
    ...
    tracebackChannel_
        << Core::XmlOpen("item") +
               Core::XmlAttribute("type", "lemma-pronunciation")
        << Core::XmlFull("lemma-pronunciation",
                         ia->symbol(state->begin()->input()))
        << Core::XmlFull("score", f32(state->begin()->weight())) +
               Core::XmlAttribute("type", "acoustic")
        << Core::XmlEmpty("samples") +
               Core::XmlAttribute(
                   "start", f32(featureTimes_[previousIndex].startTime())) +
               Core::XmlAttribute("end",
                                  f32(featureTimes_[index - 1].endTime()))
 ...
 */
const Core::ParameterString
    AlignmentDumpNode::paramFilename("file", "text file for alignment dump",
                                     "");
...
AlignmentDumpNode::AlignmentDumpNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), archive_(0), writer_(0), reader_(0),
      archiveExists_(false), attributesParser_(select("attributes-parser")) {
  addInputs(2);
  addOutputs(1);
...
bool AlignmentDumpNode::hasParameters(const std::string &s) {
  ...
  for (std::vector<std::string>::const_iterator i = requiredParams.begin();
       i != requiredParams.end(); ++i)
    ...
    if (r.isOpen()) {
      ...
      if (attributesParser_.buildFromStream(*ca, r)) {
        ...
        attributes->set("datatype",
                        Flow::DataAdaptor<Alignment>::type()->name());
      }
    }
...
bool AlignmentDumpNode::open(Core::Archive::AccessMode _access) {
  if (isOpen())
    close();
  if (filename_.empty() || (!Core::isValidPath(filename_) &&
...
bool AlignmentDumpNode::createContext(const std::string &id) {
  ...
  if (archiveExists_) {
    ...
    if (!writer_) {
      error("Failed to prepare writing to cache for id \"%s\".", id.c_str());
    }
    return true;
  }
...
bool AlignmentDumpNode::setParameter(const std::string &name,
                                     const std::string &value) {
...
bool AlignmentDumpNode::work(Flow::PortId p) {
  ...
  if (alignmentType_ == standard) {
    ...
    while (getData(0, in)) {
      ...
      for (std::vector<Alignment::Frame>::const_iterator i = frames.begin();
           i != frames.end(); ++i) {
        ...
        if (inputConnected(1)) {
          ...
          if (getData(1, f)) {
            plainData =
                Core::form("%i %.3f %.3f ", time, f->startTime(), f->endTime());
            if (writer_->isOpen()) {
              o << plainData;
              if (!o)
                error("could not write data \"%s\" to archive \"%s\"",
            ...
            error("Feature stream and alignment not synchronized at time %d.",
                  time);
          }
        }
    ...
    while (Core::getline(i, line) != EOF) {
      ...
      if (timeFrame == 0 && firstSegment) {
        ...
        Flow::DataAdaptor<Alignment> *alignment =
            new Flow::DataAdaptor<Alignment>(a);
        alignment->setTimestamp(
            Flow::Timestamp(previousStartTime, timeStampEnd));
        putData(0, alignment);
      ...
      Fsa::LabelId index =
          acousticModel_->allophoneStateAlphabet()->index(allophoneString);
