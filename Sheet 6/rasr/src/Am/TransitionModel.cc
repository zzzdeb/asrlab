...
const Core::ParameterFloat StateTransitionModel::paramScores[nTransitionTypes] =
    {
        Core::ParameterFloat(
            "loop", "negative logarithm of probability for loop transition",
            3.0),
        Core::ParameterFloat(
            "forward",
            "negative logarithm of probability for forward transition", 0.0),
        Core::ParameterFloat(
            "skip", "negative logarithm of probability for skip transition",
            3.0),
        Core::ParameterFloat(
            "exit", "negative logarithm of probability for word end transition",
            0.0),
};
...
void StateTransitionModel::getDependencies(
    Core::DependencySet &dependencies) const {
  std::string value;
  for (int i = 0; i < nTransitionTypes; ++i) {
...
#if 1
    criticalError("cannot load tdp values from file. Module MM_ADVANCED is not "
                  "available");
#endif
  }
  correct();
...
bool TransitionModel::correct() {
  ...
  for (int t = entryM1; t <= entryM2; ++t) {
    if (Core::isSignificantlyLess(
            (*transitionModels_[t])[StateTransitionModel::loop],
 ...
 *   (number of DIFFERENT incoming arc labels > 1) && (number of outgoing arcs >
 *1) Presently it is (number of incoming arcs > 1) && (number of outgoing arcs >
 *1).
...
private:
  ...
  public:
    enum Direction { incoming = 0, outgoing = 2 };
    enum Type { emitting = 0, epsilon = 4, disambiguating = 8 };
    enum { none = 0x00, one = 0x01, many = 0x03 };
  ...
  public:
    ...
    virtual void exploreNonTreeArc(Fsa::ConstStateRef from,
                                   const Fsa::Arc &arc) {
      exploreArc(from, arc);
    }
    ...
    const Degree &operator[](Fsa::StateId ii) const { return degrees_[ii]; }
  };
  ...
  struct State {
    ...
    struct Equality {
      bool operator()(const State &ll, const State &rr) const {
        return (ll.emission == rr.emission) && (ll.mask == rr.mask) &&
               (ll.weights == rr.weights) && (ll.right == rr.right);
      }
    };
    struct Hash {
      size_t operator()(const State &s) const {
        return (((((size_t(s.right) << 12) ^ size_t(s.emission)) << 2) ^
                 size_t(s.weights))
                << 2) ^
               size_t(0x0f ^ s.mask);
      }
  ...
  StateStack todo_;
  typedef Core::hash_map<State, Fsa::StateId, State::Hash, State::Equality>
      StateMap;
  StateMap states_;
  ...
  bool isStateLegitimate(const State &s) const {
    if (alphabet_->isDisambiguator(s.emission))
      return false;
    // word start state
    if ((s.mask == (State::allowForward | State::allowSkip | State::allowExit |
                    State::isFinal)) &&
        (s.weights == TransitionModel::entryM1) && (s.emission == Fsa::Epsilon))
      return true;
    // normal emitting state
    if ((s.mask == (State::allowForward | State::allowLoop | State::allowSkip |
                    State::allowExit | State::isFinal)) &&
        (s.weights >= TransitionModel::silence) && (s.emission != Fsa::Epsilon))
      return true;
    // discharged forward and intermediate skip state
    if ((s.mask == (State::allowForward)) && (s.weights == State::noWeights) &&
        (s.emission == Fsa::Epsilon))
      return true;
    // discharged skip and exit state
    if ((s.mask == (State::allowSkip | State::allowExit)) &&
        (s.weights != State::noWeights) && (s.emission == Fsa::Epsilon))
      return true;
    // post-epsilon state
    if ((s.mask == (State::allowForward | State::allowSkip | State::allowExit |
                    State::isFinal)) &&
  ...
  Fsa::StateId getStateId(State::Mask m, Fsa::LabelId e, StateType t,
                          Fsa::StateId r) {
    State s(m, e, t, r);
  ...
  Core::Ref<Fsa::State> createState(const State &s) {
    ...
    bool isFinal =
        (s.mask & State::isFinal) && ((sr = in_->getState(s.right))->isFinal());
    if (isFinal) {
  ...
  Fsa::Weight weight(const State &s,
                     StateTransitionModel::TransitionType type) const {
    ...
    if (emission == silenceLabel_) {
      ...
      if (dynamic_cast<const CartTransitionModel *>(&transitionModel_)) {
        return dynamic_cast<const EmissionAlphabet *>(alphabet_.get())
                   ? (StateType)emission
                   : transitionModel_.classifyIndex(emission);
      } else {
  ...
  void doEpsilon(const StackItem &current, const Fsa::Arc *ra) {
    ...
    current.result->newArc(getStateId(current.mask & ~(State::allowLoop),
                                      Fsa::Epsilon, current.weights,
                                      ra->target()),
                           ra->weight(), Fsa::Epsilon, ra->output());
  }
  ...
  void doForward(const StackItem &current, const Fsa::Arc *ra) {
    ...
    current.result->newArc(
        getStateId(State::allowLoop | State::allowForward | State::allowSkip |
                       State::allowExit | State::isFinal,
                   ra->input(), stateType(ra->input()), ra->target()),
        t_->semiring()->extend(ra->weight(),
  ...
  void doSkip(const StackItem &current, const Fsa::Arc *ra) {
    ...
    bool isEligbleForSkipOptimization =
        ((targetDegree(StateDegrees::outgoing, StateDegrees::disambiguating) ==
          0) &&
         (targetDegree(StateDegrees::outgoing, StateDegrees::epsilon) == 0) &&
    ...
    if (isEligbleForSkipOptimization) {
      ...
      ca->target_ =
          getStateId(State::allowLoop | State::allowForward | State::allowSkip |
                         State::allowExit | State::isFinal,
                     ras->input(), stateType(ras->input()), ras->target());
      ca->weight_ = t_->semiring()->extend(ca->weight_, ras->weight());
      ca->input_ = ras->input();
    } else {
      ca->target_ = getStateId(State::allowForward, Fsa::Epsilon,
                               State::noWeights, ra->target());
  ...
  void doExit(const StackItem &current, const Fsa::Arc *ra) {
    ...
    current.result->newArc(
        getStateId(State::allowForward | State::allowSkip | State::allowExit |
                       State::isFinal,
                   Fsa::Epsilon, TransitionModel::entryM1, ra->target()),
        t_->semiring()->extend(ra->weight(),
                               weight(current, StateTransitionModel::exit)),
        ra->input(), ra->output());
  }

  void doDischarge(const StackItem &current) {
    current.result->newArc(getStateId(State::allowForward, Fsa::Epsilon,
                                      State::noWeights, current.right),
                           weight(current, StateTransitionModel::forward),
                           Fsa::Epsilon, Fsa::Epsilon);
    current.result->newArc(getStateId(State::allowSkip | State::allowExit,
                                      Fsa::Epsilon, current.weights,
                                      current.right),
                           t_->semiring()->one(), Fsa::Epsilon, Fsa::Epsilon);
  }

public:
  Applicator(const TransitionModel &tm)
      : transitionModel_(tm), silenceLabel_(Fsa::InvalidLabelId),
        applyExitTransitionToFinalStates_(false) {}
  Fsa::ConstAutomatonRef apply(Fsa::ConstAutomatonRef in);
};

Fsa::ConstAutomatonRef
TransitionModel::Applicator::apply(Fsa::ConstAutomatonRef input) {
  require(alphabet_);
  in_ = input;
  ...
  Fsa::StateId initial =
      getStateId(State::allowForward | State::allowSkip | State::allowExit |
                     State::isFinal,
                 Fsa::Epsilon, TransitionModel::entryM1, in_->initialStateId());
  ...
  while (!todo_.empty()) {
    ...
    bool shouldDischarge =
        (degree(StateDegrees::incoming, StateDegrees::emitting) ==
         StateDegrees::many) &&
        ((degree(StateDegrees::outgoing, StateDegrees::emitting) ==
          StateDegrees::many) ||
         (degree(StateDegrees::outgoing, StateDegrees::disambiguating) ==
          StateDegrees::many));

    //	std::cerr << Core::form("expand:
    //mask=%x\temission=%d\tweights=%d\tright=%zd\n", current.mask,
    //current.emission, current.weights, current.right); // DEBUG
    ...
    if (current.emission != Fsa::Epsilon && shouldDischarge) {
      ...
      for (Fsa::State::const_iterator ra = currentRight->begin();
           ra != currentRight->end(); ++ra) {
...
Fsa::ConstAutomatonRef
TransitionModel::apply(Fsa::ConstAutomatonRef in, Fsa::LabelId silenceLabel,
...
const Core::ParameterStringVector
    NonWordAwareTransitionModel::paramNonWordPhones(
        "nonword-phones", "non-word (noise) phone symbols with separate tdps",
        ",");

NonWordAwareTransitionModel::NonWordAwareTransitionModel(
    const Core::Configuration &c, ClassicStateModelRef stateModel)
    : Core::Component(c), Precursor(c), stateModel_(stateModel) {
  std::vector<std::string> nonWordPhones = paramNonWordPhones(config);
  Bliss::PhonemeInventoryRef pi =
      stateModel_->phonology().getPhonemeInventory();
  const AllophoneAlphabet &allophones = stateModel_->allophoneAlphabet();
  for (std::vector<std::string>::const_iterator s = nonWordPhones.begin();
       s != nonWordPhones.end(); ++s) {
    const Bliss::Phoneme *phone = pi->phoneme(*s);
    AllophoneIndex ai = allophones.index(Allophone(
        phone->id(), Allophone::isInitialPhone | Allophone::isFinalPhone));
    const Allophone *allophone = allophones.allophone(ai);
    log("using nonword tdps for allophone %s",
        allophones.toString(*allophone).c_str());
    ...
    for (u32 state = 0; state < nStates; ++state) {
      const AllophoneStateIndex si =
          stateModel_->allophoneStateAlphabet().index(allophone, state);
...
CartTransitionModel::CartTransitionModel(const Core::Configuration &c,
    ...
    : Core::Component(c), Precursor(c), nSubStates_(1) {
  stateTying_ = ClassicStateTying::createClassicStateTyingRef(
      select("state-tying"), stateModel);
  ...
  transitionModels_.resize(silence + (stateTying_->nClasses() * nSubStates_),
                           0);
  transitionModels_[entryM1] = new StateTransitionModel(select("entry-m1"));
  transitionModels_[entryM2] = new StateTransitionModel(select("entry-m2"));
  transitionModels_[silence] = new StateTransitionModel(select("silence"));
  ...
  for (u32 s = 1; s < stateTying_->nClasses(); ++s) {
    ...
    transitionModels_[silence + s] =
        new StateTransitionModel(select(selection));
  }
}

// ===========================================================================
Core::Choice TransitionModel::choiceTyingType("global", global, "cart", cart,
                                              "global-and-nonword",
                                              globalPlusNonWord,
                                              Core::Choice::endMark());

Core::ParameterChoice TransitionModel::paramTyingType("tying-type",
                                                      &choiceTyingType,
...
Core::ParameterString TransitionModel::paramTdpValuesFile(
    "file", "file with tdp values, overwrites paramScores()", "");

TransitionModel *
TransitionModel::createTransitionModel(const Core::Configuration &configuration,
                                       ClassicStateModelRef stateModel) {
  ...
  case TransitionModel::cart:
    CartTransitionModel *result =
        new CartTransitionModel(configuration, stateModel);
    return result;
    break;
  }
  return 0;
...
ScaledTransitionModel::~ScaledTransitionModel() { delete transitionModel_; }
...
CombinedTransitionModel::CombinedTransitionModel(
    ...
    ClassicStateModelRef stateModel)
    : Core::Component(c), Precursor(c, stateModel),
...
void CombinedTransitionModel::distributeScaleUpdate(
    const Mc::ScaleUpdate &scaleUpdate) {
  transitionModel_->clear();
  for (u32 i = 0; i < transitionModels_.size(); ++i) {
