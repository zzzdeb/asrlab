...
public:
  AnalyticUnaryComposition(UnaryAnalyticFunctionRef f,
...
public:
  ...
  virtual UnaryAnalyticFunctionRef derive() const {
    ...
    return (!fDerived || !gDerived)
               ...
               : UnaryAnalyticFunctionRef(
                     new AnalyticSummation(fDerived, gDerived));
  }
  virtual UnaryAnalyticFunctionRef invert() const {
    return UnaryAnalyticFunctionRef(); /* no general rule exists, try to use
                                          nesting if possible */
  }
};
/** Creates an AnalyticSummation object */
inline UnaryAnalyticFunctionRef operator+(UnaryAnalyticFunctionRef f,
                                          UnaryAnalyticFunctionRef g) {
...
public:
  ...
  virtual UnaryAnalyticFunctionRef derive() const {
    ...
    return (!fDerived || !gDerived)
               ? UnaryAnalyticFunctionRef()
               : UnaryAnalyticFunctionRef(
                     new AnalyticMultiplication(fDerived, g_)) +
                     UnaryAnalyticFunctionRef(
                         new AnalyticMultiplication(f_, gDerived));
  }
  virtual UnaryAnalyticFunctionRef invert() const {
    return UnaryAnalyticFunctionRef(); /* no general rule exists, try to use
                                          nesting if possible */
...
public:
  ...
  virtual UnaryAnalyticFunctionRef derive() const {
    ...
    return (!fDerived || !gDerived)
               ? UnaryAnalyticFunctionRef()
               : UnaryAnalyticFunctionRef(new AnalyticNesting(fDerived, g_)) *
                     gDerived;
  }
  virtual UnaryAnalyticFunctionRef invert() const {
    UnaryAnalyticFunctionRef fInverse = f_->invert(), gInverse = g_->invert();
    return (!fInverse || !gInverse)
               ? UnaryAnalyticFunctionRef()
               : UnaryAnalyticFunctionRef(
                     new AnalyticNesting(gInverse, fInverse));
  }
};
/** Creates an AnalyticNesting object. */
inline UnaryAnalyticFunctionRef nest(UnaryAnalyticFunctionRef f,
                                     UnaryAnalyticFunctionRef g) {
