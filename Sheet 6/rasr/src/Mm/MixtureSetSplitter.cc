...
const Core::ParameterFloat MixtureSetSplitter::paramMinMeanObservationWeight(
    "minimum-mean-observation-weight",
    "only means with more observation-weight get splitted", 20, 0);
const Core::ParameterFloat
    MixtureSetSplitter::paramMinCovarianceObservationWeight(
        "minimum-covariance-observation-weight",
        "only covariances with more observation-weight get splitted",
        Core::Type<f32>::max, 0);
const Core::ParameterFloat MixtureSetSplitter::paramPerturbationWeight(
    "perturbation-weight",
    "empirical weight to optimize the epsilon for splitting means", 0.1, 0);
const Core::ParameterBool MixtureSetSplitter::paramNormalizeMixtureWeights(
    "normalize-mixture-weights",
    "normalize mixture weights after split, not required in the Viterbi "
    "approximation ",
    false);
...
MixtureSetSplitter::MixtureSetSplitter(const Core::Configuration &configuration)
    : Precursor(configuration),
      ...
      minCovarianceObservationWeight_(
          paramMinCovarianceObservationWeight(configuration)),
      normalizeMixtureWeights_(paramNormalizeMixtureWeights(configuration)) {}

Core::Ref<MixtureSet>
MixtureSetSplitter::split(AbstractMixtureSetEstimator &estimator) {
  ...
  estimator.estimate(*result, meanObservationWeight,
                     covarianceObservationWeight);
...
void MixtureSetSplitter::splitMeans(MixtureSet &mixtureSet,
  ...
  for (DensityIndex densityIndex = 0; densityIndex < splittedDensities_.size();
       ++densityIndex) {
    ...
    std::vector<MeanType> perturbation =
        mixtureSet.covariance(covarianceIndex)->diagonal();
    for (std::vector<MeanType>::iterator elem = perturbation.begin();
         elem != perturbation.end(); ++elem) {
      *elem = std::sqrt(*elem) * perturbationWeight_ * Core::Type<f32>::epsilon;
    }

    splittedMeans_[meanIndex] =
        (weights[meanIndex] > minMeanObservationWeight_
             ? splitMean(mixtureSet, meanIndex, perturbation)
             : meanIndex);
  }
}

MeanIndex MixtureSetSplitter::splitMean(MixtureSet &mixtureSet, MeanIndex index,
                                        std::vector<MeanType> &perturbation) {
...
void MixtureSetSplitter::splitCovariances(MixtureSet &mixtureSet,
  ...
  for (CovarianceIndex i = 0; i < splittedCovariances_.size(); ++i) {
    splittedCovariances_[i] = (weights[i] > minCovarianceObservationWeight_
                                   ? splitCovariance(mixtureSet, i)
                                   : i);
  }
}

CovarianceIndex MixtureSetSplitter::splitCovariance(MixtureSet &mixtureSet,
                                                    CovarianceIndex index) {
...
void MixtureSetSplitter::splitDensities(MixtureSet &mixtureSet) {
  ...
  for (DensityIndex i = 0; i < splittedDensities_.size(); ++i) {
    ...
    CovarianceIndex splittedCovarianceIndex =
        splittedCovariances_[toSplit->covarianceIndex()];
...
void MixtureSetSplitter::splitDensitiesInMixtures(MixtureSet &mixtureSet) {
  for (MixtureIndex i = 0; i < mixtureSet.nMixtures(); ++i) {
    ...
    for (size_t dns = 0; dns < nDensitiesToSplit; ++dns) {
      DensityIndex splittedDensityIndex =
          splittedDensities_[mixture->densityIndex(dns)];
