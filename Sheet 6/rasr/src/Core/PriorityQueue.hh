...
public:
  ...
  PriorityQueueBase(const PriorityFunction &precedes,
                    u32 maxSize = Type<u32>::max)
...
template <class H, class PF> void PriorityQueueBase<H, PF>::downHeap(Index i) {
  require(1 <= i && i <= Precursor::size());
  Index j;
  Item e = item(i);
  while (i <= Precursor::size() / 2) {
    j = 2 * i;
    if (j < Precursor::size() && precedes_(item(j + 1), item(j)))
      j = j + 1;
    if (!precedes_(item(j), e))
      break;
    move(i, j);
    i = j;
  }
...
template <class H, class PF> void PriorityQueueBase<H, PF>::upHeap(Index i) {
  require(1 <= i && i <= Precursor::size());
  Item e = item(i);
...
template <class H, class PF> bool PriorityQueueBase<H, PF>::invariant() const {
...
class DefaultHashMap
    : public Core::hash_map<T_Key, T_Item, T_HashFunction, std::equal_to<T_Key>,
                            std::allocator<T_Item> > {};

template <class T_Item> class UntracedHeap {
public:
  typedef T_Item Item;
...
public:
  ...
  bool contains(const Key &k) const {
    ...
    return 0 < i && i < Precursor::heap_.size() &&
           key_(Precursor::heap_[i]) == k;
  }
...
bool TracedHeap<T_Item, T_Key, T_KeyFunction, T_Map, T_Hash_Obj>::invariant()
    const {
...
public:
  ...
  PriorityQueue(const T_PriorityFunction &precedes,
                u32 maxSize = Type<u32>::max)
...
template <class T_Item, class T_Key = typename T_Item::first_type,
          ...
          class T_PriorityFunction = Core::binary_compose<
              std::less<typename T_Item::second_type>, Core::select2nd<T_Item>,
              Core::select2nd<T_Item> >,
...
public:
  ...
  TracedPriorityQueue(const T_PriorityFunction &precedes,
                      u32 maxSize = Type<u32>::max)
      : Precursor(precedes, maxSize) {}
  ...
  size_t size() const { return Precursor::heap_.size(); }

  const T_Item &operator[](size_t i) const { return Precursor::heap_[i]; }
