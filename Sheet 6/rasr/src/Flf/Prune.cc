...
public:
  ...
  virtual ConstStateRef getState(Fsa::StateId sid) const {
    ...
    for (State::const_iterator a = sr->begin(); a != sr->end();
         ++a, ++itFbScore)
  ...
  virtual std::string describe() const {
    return Core::form("posterior-pruning(%s,%f)", fsa_->describe().c_str(),
                      threshold_);
  }
};
...
ConstLatticeRef pruneByFwdBwdScores(ConstLatticeRef l, ConstFwdBwdRef fb,
  ...
  return ConstLatticeRef(
      new FwdBwdPruningLattice(l, fb, fb->min() + threshold));
}

namespace {
const Core::ParameterBool
    paramRelative("relative",
                  "threshold is interpreted relative to min. arc score", true);
const Core::ParameterBool paramAsProbability(
    "as-probability",
    "threshold given as probability (like the SRI lattice-tool)", false);
const Core::ParameterFloat paramThreshold("threshold", "threshold",
...
public:
  Internal(const Core::Configuration &config, FwdBwdBuilderRef fbBuilder)
    ...
    this->fbBuilder =
        (fbBuilder) ? fbBuilder : FwdBwdBuilder::create(select("fb"));
    ...
    if (configurationChannel) {
                                  ...
                                  Core::XmlAttribute("component", this->name());
      {
        ...
        if (t == 0.0)
          ...
          os << "threshold = " << t << " (p = " << ::exp(-t) << ")"
             << std::endl;
        os << "thresholds is "
           << (isRelative ? "relative to min. fwd/bwd. score" : "absolute")
           << "." << std::endl;
      }
  ...
  Score threshold(ConstFwdBwdRef fb) {
    Score threshold = (t == Core::Type<Score>::max) ? Core::Type<Score>::max
                      : (isRelative)                ? fb->min() + t
                                                    : t;
...
ConstLatticeRef FwdBwdPruner::prune(ConstLatticeRef l, ConstFwdBwdRef fb,
  ...
  ConstLatticeRef p = ConstLatticeRef(
      new FwdBwdPruningLattice(l, fb, internal_->threshold(fb)));
  if (trim) {
...
FwdBwdPrunerRef FwdBwdPruner::create(const Core::Configuration &config,
                                     FwdBwdBuilderRef fbBuilder) {
...
const Core::ParameterBool
    FwdBwdPruningNode::paramTrim("trim", "trim after applying thresholding",
                                 true);
...
void prune(ConstConfusionNetworkRef cnRef, Score threshold, u32 maxSlotSize,
           bool normalize) {
  if (!cnRef)
    return;
  if (!cnRef->isNormalized())
    Core::Application::us()->criticalError(
        "Confusion network pruning does only work for normalized CNs.");
  ...
  for (ConfusionNetwork::iterator itSlot = cn.begin(), endSlot = cn.end();
    ...
    if (itArc != endArc) {
      ...
      if (normalize) {
        Score norm = 1.0 / sum;
        for (itArc = slot.begin(), endArc = slot.end(); itArc != endArc;
             ++itArc)
...
struct PosteriorCnProbabilityWeakOrder {
  bool operator()(const PosteriorCn::Arc &a1,
                  const PosteriorCn::Arc &a2) const {
    return a1.score > a2.score;
  }
};
} // namespace
void prune(ConstPosteriorCnRef cnRef, Score threshold, u32 maxSlotSize,
           bool normalize) {
  if (!cnRef)
    return;
  PosteriorCn &cn = const_cast<PosteriorCn &>(*cnRef);
  for (PosteriorCn::iterator itSlot = cn.begin(), endSlot = cn.end();
       itSlot != endSlot; ++itSlot) {
...
void removeEpsSlots(ConstConfusionNetworkRef cnRef, Score threshold) {
  if (!cnRef)
    return;
  ConfusionNetwork &cn = const_cast<ConfusionNetwork &>(*cnRef);
  ConfusionNetwork::iterator itFrom = cn.begin(), itTo = cn.begin(),
                             endSlot = cn.end();
  ...
  if (!cnRef->isNormalized()) {
    if (threshold < 1.0)
      Core::Application::us()->warning(
          "Epsilon slot removal for non-normalized CNs does not support "
          "thresholding.");
    for (; itFrom != endSlot; ++itFrom, ++itTargetSlotId) {
      const ConfusionNetwork::Slot &from = *itFrom;
      for (ConfusionNetwork::Slot::const_iterator itArc = from.begin(),
                                                  endArc = from.end();
           itArc != endArc; ++itArc)
    ...
    ScoreId posteriorId = (threshold != Core::Type<Score>::max)
                              ? cnRef->normalizedProperties->posteriorId
                              : Semiring::InvalidId;
    for (; itFrom != endSlot; ++itFrom, ++itTargetSlotId) {
      const ConfusionNetwork::Slot &from = *itFrom;
      if ((from.front().label != Fsa::Epsilon) ||
          ((from.size() > 1) &&
           ((posteriorId == Semiring::InvalidId) ||
            (from.front().scores->get(posteriorId) < threshold)))) {
        if (itTo != itFrom)
          *itTo = from;
        ++itTo;
        *itTargetSlotId = targetSlotId++;
  ...
  if (cn.hasMap()) {
    ConfusionNetwork::MapProperties &map =
        const_cast<ConfusionNetwork::MapProperties &>(*cn.mapProperties);
    std::vector<u32>::const_iterator itTargetSlotId = slotIdMapping.begin();
    for (Core::Vector<Fsa::StateId>::iterator
             itSlotIndex = map.slotIndex.begin(),
             endSlotIndex = map.slotIndex.end();
    ...
    for (ConfusionNetwork::MapProperties::Map::iterator
             itLat2Cn = map.lat2cn.begin(),
             endLat2Cn = map.lat2cn.end();
...
void removeEpsSlots(ConstPosteriorCnRef cnRef, Score threshold) {
  ...
  for (PosteriorCn::iterator itFrom = cn.begin(), endSlot = cn.end();
       itFrom != endSlot; ++itFrom) {
    const PosteriorCn::Slot &from = *itFrom;
    if ((from.front().label != Fsa::Epsilon) ||
        ((from.size() > 1) && (from.front().score < threshold))) {
      if (itTo != itFrom)
        *itTo = from;
      ++itTo;
    }
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    prune_ = (threshold_ != Core::Type<Score>::max) ||
             (maxSlotSize_ != Core::Type<u32>::max);
...
const Core::ParameterFloat
    CnPruningNode::paramThreshold("threshold", "probability threshold",
                                  Core::Type<Score>::max);
const Core::ParameterInt CnPruningNode::paramMaxSlotSize("max-slot-size",
                                                         "max. slot size",
                                                         Core::Type<u32>::max);
const Core::ParameterBool CnPruningNode::paramNormalize("normalize",
                                                        "normalize", true);
const Core::ParameterBool CnPruningNode::paramRemoveEpsSlots("remove-eps-slots",
                                                             "remove eps slots",
                                                             false);
...
public:
  NormalizedCnPruningNode(const std::string &name,
                          const Core::Configuration &config)
...
public:
  PosteriorCnPruningNode(const std::string &name,
                         const Core::Configuration &config)
      : Precursor(name, config) {}
