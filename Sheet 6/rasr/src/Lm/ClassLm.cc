...
const Core::ParameterString
    ClassMapping::paramClassFileName("file", "class mapping file", "");
const Core::ParameterString ClassMapping::paramClassFileEncoding(
    "encoding", "encoding of class mapping file", "utf-8");
...
ClassMapping::ClassMapping(const Core::Configuration &config,
                           Bliss::LexiconRef lexicon)
    : Core::Component(config), lexicon_(lexicon) {
  tokenAlphabet_ =
      Fsa::ConstAlphabetRef(new ClassTokenAlphabet(Core::ref(this)));
}
...
void ClassMapping::load() {
  ...
  while (tis) {
    ...
    if (classMap_[syntTok->id()].first) {
      error("In class file: Duplicated entry for syntactic token \"%s\"; "
            "discard entry.",
            syntWord.c_str());
      continue;
    }
    ClassToken *classTok =
        static_cast<ClassToken *>(tokenInventory_[classWord]);
    if (!classTok)
  ...
  if (nSyntTok) {
    ...
    for (Core::tie(synt_tok_it, synt_tok_end) = lexicon_->syntacticTokens();
      ...
      if (!classMap_[syntTok->id()].first) {
        ...
        ClassToken *classTok =
            dynamic_cast<ClassToken *>(tokenInventory_[classWord]);
        if (!classTok)
          classTok = addToken(classWord);
        classTok->syntacticTokens_.push_back(syntTok);
        ensure(size_t(syntTok->id()) < classMap_.size());
  ...
  for (Bliss::TokenInventory::Iterator itClassTok = tokenInventory_.begin();
    ...
    Score sum = 0.0;
    for (ClassToken::SyntacticTokenList::iterator itSyntTok =
             classTok->syntacticTokens_.begin();
         itSyntTok != classTok->syntacticTokens_.end(); ++itSyntTok)
      sum += classMap_[(*itSyntTok)->id()].second;
    for (ClassToken::SyntacticTokenList::iterator itSyntTok =
             classTok->syntacticTokens_.begin();
         itSyntTok != classTok->syntacticTokens_.end(); ++itSyntTok) {
      std::pair<ClassToken *, Score> &tokenScore =
          classMap_[(*itSyntTok)->id()];
...
public:
  ClassMappingAutomaton(ConstClassMappingRef mapping) : mapping_(mapping) {
    ...
    setProperties(Fsa::PropertyLinear | Fsa::PropertyAcyclic,
                  Fsa::PropertyNone);
  }
  virtual ~ClassMappingAutomaton() {}

  virtual ConstSemiringRef semiring() const { return Fsa::TropicalSemiring; }
  ...
  virtual Fsa::ConstStateRef getState(Fsa::StateId sid) const {
    ...
    for (ClassMapping::ClassMap::const_iterator itClassScore =
             mapping_->classMap_.begin();
         itClassScore != mapping_->classMap_.end();
         ++itClassScore, ++syntTokId) {
      sp->newArc(0, Fsa::Weight(itClassScore->second), syntTokId,
                 itClassScore->first->id());
    }
    return Fsa::ConstStateRef(sp);
  }

  virtual std::string describe() const { return "class-mapping"; }
};

Fsa::ConstAutomatonRef
ClassMapping::createSyntacticTokenToClassTokenTransducer() const {
...
void ClassMapping::writeClasses(Core::XmlWriter &xml) const {
  ...
  for (Bliss::TokenInventory::Iterator itClassTok = tokenInventory_.begin();
    ...
    ClassToken *classTok = static_cast<ClassToken *>(*itClassTok);
    xml << std::setw(4) << std::right << nClass << ". "
        << std::string(classTok->symbol()) << "={";
...
const Core::ParameterFloat
    ClassLm::paramClassEmissionScale("scale", "class emission scale", 1.0);

ClassLm::ClassLm(const Core::Configuration &config) : Core::Component(config) {
