...
NamedCorpusEntity::NamedCorpusEntity(ParentEntity *_parent)
    : parent_(_parent), name_(anonymous) {}
...
const char *Speaker::genderId[] = {"unknown", "male", "female"};

Speaker::Speaker(ParentEntity *_parent)
    : NamedCorpusEntity(_parent), gender_(unknown) {}
...
CorpusSection::CorpusSection(CorpusSection *_parent)
    : ParentEntity(_parent), level_((_parent) ? (_parent->level() + 1) : 0),
      defaultCondition_(0), defaultSpeaker_(0) {}
...
// ========================================================================
// class Corpus
Corpus::Corpus(Corpus *parentCorpus) : CorpusSection(parentCorpus) {}

// ========================================================================
// class Recording

Recording::Recording(Corpus *corpus) : CorpusSection(corpus), duration_(0) {
  require(corpus);
}
...
const char *Segment::typeId[] = {"speech", "other"};

Segment::Segment(Type _type, Recording *_recording)
    : ParentEntity(_recording), recording_(_recording), type_(_type),
      condition_(0) {
  require(_recording);
}

void Segment::accept(SegmentVisitor *v) { v->visitSegment(this); }

// ========================================================================
// class SpeechSegment

SpeechSegment::SpeechSegment(Recording *_recording)
    : Segment(typeSpeech, _recording), speaker_(0) {}

void SpeechSegment::accept(SegmentVisitor *v) { v->visitSpeechSegment(this); }

// ========================================================================
// class CorpusDescription

const Core::ParameterString CorpusDescription::paramFilename("file",
                                                             "file name", "");
const Core::ParameterString
    CorpusDescription::paramEncoding("encoding", "encoding", "utf-8");
const Core::ParameterInt CorpusDescription::paramPartition(
    "partition",
    "divide corpus into partitions with (approximately) equal number of "
    "segments",
    0, 0);
const Core::ParameterInt CorpusDescription::paramPartitionSelection(
    "select-partition", "select a partition of the corpus", 0, 0);
const Core::ParameterInt CorpusDescription::paramSkipFirstSegments(
    "skip-first-segments",
    "skip the first N segments (counted after partitioning)", 0, 0);
const Core::ParameterStringVector
    CorpusDescription::paramSegmentsToSkip("segments-to-skip",
...
const Core::ParameterBool CorpusDescription::paramRecordingBasedPartition(
    ...
    "create corpus partitions based on recordings instead of segments", false);
const Core::ParameterBool CorpusDescription::paramProgressReportingSegmentOrth(
    "report-segment-orth", "output also segment orth in progress report",
    false);

// ---------------------------------------------------------------------------
class CorpusDescription::SegmentPartitionVisitorAdaptor : public CorpusVisitor {
private:
  u32 segmentIndex_, recordingIndex_, nPartitions_, selectedPartition_,
      nSkippedSegments_;
  ...
  bool shouldVisit(Segment *s) {
    ...
    if (recordingBasedPartitions_)
      isSelectedPartition =
          ((recordingIndex_ % nPartitions_) == selectedPartition_);
    else
      isSelectedPartition =
          ((segmentIndex_ % nPartitions_) == selectedPartition_);
    bool hasSkippedEnoughSegments =
        ((segmentIndex_ / nPartitions_) >= nSkippedSegments_);
    ++segmentIndex_;
    if (isSelectedPartition && hasSkippedEnoughSegments &&
        (segmentsToSkip_.empty() ||
         (segmentsToSkip_.find(s->fullName()) == segmentsToSkip_.end())) &&
        (segmentsToKeep_.empty() ||
         (segmentsToKeep_.find(s->fullName()) != segmentsToKeep_.end()) ||
         (segmentsToKeep_.find(s->name()) != segmentsToKeep_.end()))) {
...
public:
  SegmentPartitionVisitorAdaptor() : visitor_(0) {
    nPartitions_ = 1;
    selectedPartition_ = 0;
    recordingBasedPartitions_ = false;
    nSkippedSegments_ = 0;
  }
  void loadSegmentList(const std::string &filename,
                       const std::string &encoding);
  const Core::StringHashSet &segmentsToKeep() const { return segmentsToKeep_; }
  void setPartitioning(u32 nPartitions, u32 selectedPartition,
                       bool recordingBased = false) {
  ...
  virtual void leaveCorpus(Corpus *c) { visitor_->leaveCorpus(c); }
};

void CorpusDescription::SegmentPartitionVisitorAdaptor::loadSegmentList(
    const std::string &filename, const std::string &encoding) {
  if (!filename.empty()) {
    ...
    if (!is)
      Core::Application::us()->criticalError(
          "Failed to open segment list file \"%s\".", filename.c_str());
    std::string s;
...
// ---------------------------------------------------------------------------
ProgressReportingVisitorAdaptor::ProgressReportingVisitorAdaptor(
    Core::XmlChannel &ch, bool reportOrth)
...
void ProgressReportingVisitorAdaptor::openSegment(Segment *s) {
  channel_ << Core::XmlOpen("segment") + Core::XmlAttribute("name", s->name()) +
...
void ProgressReportingVisitorAdaptor::visitSpeechSegment(SpeechSegment *s) {
  ...
  if (s->speaker()) {
    channel_ << Core::XmlEmpty("speaker") +
                    ...
                    Core::XmlAttribute(
                        "gender",
                        Bliss::Speaker::genderId[s->speaker()->gender()]);
  }
  if ((s->orth() != "") && (reportSegmentOrth_)) {
    channel_ << Core::XmlOpen("orth") << s->orth() << Core::XmlClose("orth");
  }
  visitor_->visitSpeechSegment(s);
  closeSegment(s);
}
...
const Core::Choice CorpusDescription::progressIndicationChoice(
    "none", noProgress, "local", localProgress, "global", globalProgress,
    Core::Choice::endMark());

const Core::ParameterChoice CorpusDescription::paramProgressIndication(
    "progress-indication", &progressIndicationChoice,
    "how to display progress in processing the corpus", noProgress);
...
CorpusDescription::CorpusDescription(const Core::Configuration &c)
    : Component(c), selector_(0), progressChannel_(c, "progress"), reporter_(0),
      indicator_(0) {
  ...
  if (partitioning || skipFirstSegments || !segmentFullNames.empty() ||
      !segmentsFilename.empty()) {
  ...
  if (partitioning) {
    ...
    if (selectedPartition == partitioning)
      ...
      error("Invalid partition %d (should be 0 - %d).", selectedPartition,
            partitioning);
    selector_->setPartitioning(partitioning, selectedPartition,
                               recordingBasedPartition);
  }
  ...
  if (!segmentsFilename.empty()) {
    selector_->loadSegmentList(segmentsFilename,
                               paramEncoding(select("segments")));
    if (selector_->segmentsToKeep().empty())
      error("Discard segment whitelist, because file is empty or does not "
            "exist: ")
          << segmentsFilename;
    else
      log("Use a segment whitelist with %d entries, keep only listed segments.",
          u32(selector_->segmentsToKeep().size()));
  }

  progressIndicationMode_ =
      ProgressIndcationMode(paramProgressIndication(config));
  u32 nSegments = 0;
  switch (progressIndicationMode_) {
  case globalProgress: {
    SegmentCountingVisitor *counter = new SegmentCountingVisitor();
    counter->reset();
    // Note: It is necessary that selector_ is set before and reporter_ after
    // this call to accept()
    accept(counter);
  ...
  if (progressChannel_.isOpen()) {
    reporter_ = new ProgressReportingVisitorAdaptor(
        progressChannel_, paramProgressReportingSegmentOrth(config));
  }
}
