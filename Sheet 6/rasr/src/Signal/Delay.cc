...
const Core::ParameterInt
    DelayNode::parameterMaxSize("max-size", "maximum length of sliding window",
                                1, 1);
const Core::ParameterInt
    DelayNode::parameterRight("right", "position of zero time index from right",
                              0, 0);

const Core::Choice DelayNode::choiceMarginPolicyType("copy", marginPolicyCopy,
                                                     "mean", marginPolicyMean,
                                                     "zero", marginPolicyZero,
                                                     "one", marginPolicyOne,
                                                     Core::Choice::endMark());
const Core::ParameterChoice
    DelayNode::paramMarginPolicy("margin-policy", &choiceMarginPolicyType,
                                 "type of margin policy", marginPolicyCopy);

const Core::Choice DelayNode::choiceMarginConditionType(
    "not-empty", marginConditionNotEmpty, "present-not-empty",
    marginConditionPresentNotEmpty, "full", marginConditionFull,
    Core::Choice::endMark());
const Core::ParameterChoice DelayNode::paramMarginCondition(
    "margin-condition", &choiceMarginConditionType,
    "condition when to generate an output", marginConditionPresentNotEmpty);

DelayNode::DelayNode(const Core::Configuration &c)
    : Component(c), Node(c), maxSize_(0), right_(0), marginPolicy_(0),
      marginCondition_(0), needInit_(true) {
...
void DelayNode::init() {
  if (!DataSlidingWindow::init(maxSize_, right_)) {
    error("Cannot initialize with parameters max-size (%zd), right (%zd).",
          maxSize_, right_);
    return;
  }
  needInit_ = false;
...
bool DelayNode::setParameter(const std::string &name,
