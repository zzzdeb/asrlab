...
/** Feature is container for objects comming from different feature stream at
 * the same time. Remark: applications working on sinlge streams should use the
 * "main stream" which is currently the first stream. Note: Currently it can
...
public:
  ...
  /** Creates a feature with a single stream containing the feature vector
   * @param f. */
  explicit Feature(const Core::Ref<const Vector> &f) : streams_(1) {
    set(0, f);
  }
  /** Creates a feature with a single stream containing the feature vector
   * @param f. */
  ...
  void set(const std::vector<size_t> &indices,
           const Core::Ref<const Vector> &v) {
    for (std::vector<size_t>::const_iterator i = indices.begin();
         i != indices.end(); ++i)
      set(*i, v);
  }
   ...
   */
  Core::Ref<const Vector> mainStream() const { return streams_.front(); }
...
protected:
  /*
   *  Initializes the FeatureDescription by parameters of an existing feature
   * object.
   */
...
public:
  ...
  /**
   *  Initializes the FeatureDescription by parameters of an existing feature
   * object. Name of the description is created form the full name of @c parent.
   */
