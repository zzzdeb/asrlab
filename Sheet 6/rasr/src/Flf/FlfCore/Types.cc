...
// -------------------------------------------------------------------------
namespace {
Core::Choice SemiringTypeChoice("log", Fsa::SemiringTypeLog, "tropical",
                                Fsa::SemiringTypeTropical,
                                Core::Choice::endMark());
} // namespace
...
// -------------------------------------------------------------------------
namespace {
const Core::Choice RescoreModeNameChoice("clone", RescoreModeClone,
                                         "in-place-cached",
                                         RescoreModeInPlaceCache, "in-place",
                                         RescoreModeInPlace,
                                         Core::Choice::endMark());
} // namespace
RescoreMode getRescoreMode(const std::string &s) {
  ...
  if (mode == Core::Choice::IllegalValue)
    Core::Application::us()->criticalError("Unknown rescore mode \"%s\".",
                                           s.c_str());
...
// -------------------------------------------------------------------------
namespace {
const Core::Choice InfoTypeNameChoice("cheap", InfoTypeCheap, "normal",
                                      InfoTypeNormal, "extended",
                                      InfoTypeExtended, "memory",
                                      InfoTypeMemory, Core::Choice::endMark());
} // namespace
InfoType getInfoType(const std::string &s) {
  Core::Choice::Value type = InfoTypeNameChoice[s];
  if (type == Core::Choice::IllegalValue)
    Core::Application::us()->criticalError("Unknown info type \"%s\".",
                                           s.c_str());
