...
public:
  CacheAutomaton(_ConstAutomatonRef f, u32 maxAge)
    ...
    this->setProperties(Fsa::PropertyCached | Fsa::PropertyStorage,
                        Fsa::PropertyCached);
  }
  ...
  virtual size_t getMemoryUsed() const {
    size_t size = 0;
    for (typename Core::Vector<_ConstStateRef>::iterator i = states_.begin();
         i != states_.end(); ++i)
      if ((*i) && ((*i)->refCount() == 1))
        size += (*i)->getMemoryUsed();
    return Precursor::fsa_->getMemoryUsed() + 2 * sizeof(u32) +
           ages_.capacity() * sizeof(u8) +
           states_.capacity() * sizeof(_ConstStateRef) + size;
  }
  virtual std::string describe() const {
    return Core::form("cache(%s,%d)", Precursor::fsa_->describe().c_str(),
                      maxAge_);
  }
};
