...
public:
  ...
  public:
    ...
    /** Compares the value of the attribute @param name with the @param
     * expected. If @param critical is true and comparison fails, execution is
     * aborted
     */
    template <class ValueType>
    bool verifyValue(const std::string &name, const ValueType &expected,
...
public:
  static std::string prepareName(const std::string &parent,
...
public:
   ...
   */
  bool verifyNumberOfStreams(size_t expectedNumber,
                             bool critical = false) const;
...
template <class ValueType>
bool Description::Stream::verifyValue(const std::string &name,
  ...
  if (value != expected) {
    Application::us()->error()
        << name_ << ":mismatch in value '" << name << "': expected '"
        << expected << "' retrieved '" << value << "'.";
    if (critical)
