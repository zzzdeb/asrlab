...
struct AlignmentItem {
  ...
  bool operator==(const AlignmentItem &other) const {
    return (time == other.time && emission == other.emission &&
            weight == other.weight);
  }
};
...
Core::BinaryOutputStream &operator<<(Core::BinaryOutputStream &,
                                     const Alignment &);
...
public:
  ...
  /** Returns true, if at least one alignment item has a weight different from
   * one. */
  ...
  /** Raise all weights to the given power gamma. Note that the weights are not
   * normalized afterwards. This function is intended to approximate a
   * re-alignment with a different acoustic-model scale. */
  ...
  friend Core::BinaryInputStream &operator>>(Core::BinaryInputStream &,
                                             Alignment &);
  friend Core::BinaryOutputStream &operator<<(Core::BinaryOutputStream &,
                                              const Alignment &);
...
template <> class NameHelper<Speech::Alignment> : public std::string {
public:
