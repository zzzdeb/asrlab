...
BinaryTree::BinaryTree(const Core::Configuration &config, Node *root)
    : Precursor(config), root_(0), depth_(InvalidDepth),
      internalNodeN_(InvalidSize), leafN_(InvalidSize) {
  if (root)
    setRoot(root);
}
...
u32 BinaryTree::checkTree(const Node &node) {
  ...
  if (node.isLeaf()) {
    ...
    ++internalNodeN_;
    return 1 +
           std::max(checkTree(node.leftChild()), checkTree(node.rightChild()));
  }
}
...
u32 drawBinaryTreeNode(std::ostream &out, const BinaryTree::Node &node,
                       u32 id = 0) {
  if (node.isLeaf()) {
...
void writeBinaryTreeNode(std::ostream &out, const BinaryTree::Node &node,
                         const u32 depth = 0) {
...
void writeXmlBinaryTreeNode(Core::XmlWriter &xml,
