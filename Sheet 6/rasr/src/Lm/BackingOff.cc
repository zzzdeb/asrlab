...
const BackingOffLm::WordScore *
BackingOffLm::Node::findWordScore(const WordScore *base, TokenIndex t) const {
...
void BackingOffLm::Internal::changeWordScoreCapacity(
  ...
  WordScore *newWordScores =
      (WordScore *)realloc(wordScores_, newCapacity * sizeof(WordScore));
...
void BackingOffLm::Internal::reserve(NodeIndex nNodesNeeded,
 ...
 */
void BackingOffLm::Internal::draw(std::ostream &os,
  ...
  for (const Node *n = nodes_; n != nodesTail_; ++n) {
    ...
    os << Core::form("n%p [shape=square label=\"%d\\n%s\\nbo=%f", (void *)n,
                     nodeIndex(n), (tok) ? tok->symbol().str() : "(null)",
                     n->backOffScore());
    for (const WordScore *ws = scoresBegin(n); ws != scoresEnd(n); ++ws)
      os << Core::form("\\n%s: %f", token(ws->token())->symbol().str(),
                       ws->score());
    os << "\"]\n";
    if (n->parent())
...
bool BackingOffLm::Internal::writeImage(int fd, const std::string &info) const {
  ...
  if (write(fd, nodes_, nBytes) != nBytes)
    return false;
  ...
  if (write(fd, wordScores_, nBytes) != nBytes)
    return false;
...
bool BackingOffLm::Internal::mountImage(
    int fd, std::string &info, const Bliss::TokenInventory &inventory) {
  ssize_t nBytes;
  ImageHeader header;
  nBytes = sizeof(ImageHeader);
  ...
  if (header.versionMark != ImageHeader::version) {
    info = Core::form(
        "image has version %d which is incompatible with currect version %d",
        header.versionMark, ImageHeader::version);
    return false;
  }
  ...
  for (TokenIndex ti = 0; ti < TokenIndex(header.nTokens); ++ti) {
    ...
    if (*str) {
      ...
      if (token->id() != ti) {
        info =
            Core::form("wrong token index: \"%s\" has index %d, should be %d",
                       str, token->id(), ti);
        munmap(mmap_, mmapSize_);
        return false;
      }
...
void BackingOffLm::logInitialization() const {
  log("size: %d n-grams in %d histories", internal_->nWordScores(),
      internal_->nNodes());
...
const Core::ParameterString BackingOffLm::paramImage(
    "image", "create and/or use language model binary image file");
...
void BackingOffLm::load() {
  ...
  if (!Core::isRegularFile(image)) {
    ...
    int fd = open(image.c_str(), O_CREAT | O_WRONLY | O_TRUNC,
...
const BackingOffLm::Node *
BackingOffLm::Internal::startHistory(TokenIndex w) const {
...
const BackingOffLm::Node *
BackingOffLm::Internal::extendedHistory(const Node *old, TokenIndex w) const {
...
void BackingOffLm::historyTokens(const History &h, const Bliss::Token **target,
                                 u32 &size, u32 arraySize) const {
  size = 0;
  for (const Node *n = descriptor<Self>(h); n;
       (n = n->parent()) && size < arraySize) {
...
BackingOffLm::BackOffScores
BackingOffLm::getBackOffScores(const Lm::History &history, int depth) const {
...
void BackingOffLm::getBatch(const History &history,
  ...
  for (Node::Depth d = 0; d <= hn->depth(); ++d) {
    ...
    for (const WordScore *ws = internal_->scoresBegin(n);
         ws != internal_->scoresEnd(n); ++ws) {
      const Bliss::SyntacticToken *syntacticToken =
          static_cast<const Bliss::SyntacticToken *>(
              lexicon()->syntacticTokenInventory()[ws->token()]);
      scores[syntacticToken] = ws->score() + backOffScore[d + 1];
    }
  }

  const NonCompiledBatchRequest *ncbr =
      required_cast(const NonCompiledBatchRequest *, cbr);
  const BatchRequest &request(ncbr->request);
  for (BatchRequest::const_iterator r = request.begin(); r != request.end();
       ++r) {
    Score score = 0.0;
...
public:
  Automaton(Core::Ref<const BackingOffLm> lm)
      : LanguageModelAutomaton(
            static_cast<Core::Ref<const LanguageModel> >(lm)),
  ...
  virtual Fsa::ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (node->backOffScore() < Core::Type<Score>::max) {
      ...
      state->newArc(internal_->nodeIndex(node->parent()),
                    Fsa::Weight(node->backOffScore()), backOffLabel_);
    }

    for (const WordScore *ws = internal_->scoresBegin(node);
         ws != internal_->scoresEnd(node); ++ws) {
      ...
      if (ws->token() == sentenceEndToken_) {
        ...
        state->newArc(
            ...
            Fsa::Weight(ws->score()), internal_->token(ws->token())->id());
      }
    }
