...
/*
  ...
  [<input-alphabet name="lemma-pronunciation" format="bin"
  file="input-alphabet.binfsa.gz"/>]
  [<output-alphabet name="evaluation" format="bin"
  file="output-alphabet.binfsa.gz"/>]
...
struct FsaDescriptor {
  ...
  */
  ...
  static FsaDescriptor joinPaths(const FsaDescriptor &desc,
...
struct BoundariesDescriptor {
  ...
  BoundariesDescriptor(const std::string &file) : file(file) {}
  std::string operator()(const std::string &root = "") const;
  operator bool() const { return !file.empty(); }
...
public:
  ...
  void overhaul(FlfDescriptorRef,
                const Fsa::StaticAutomaton &structureFsa) const;
  void adapt(FlfDescriptorRef desc, const Fsa::StaticAutomaton &structureFsa,
             Core::Archive *archive = 0);
  void update(FlfDescriptorRef, const Fsa::StaticAutomaton &structureFsa,
              Core::Archive *archive = 0);
  void clear();
  ...
  void setInputAlphabet(Fsa::ConstAlphabetRef alphabet,
                        const std::string &alphabetName);
  void loadInputAlphabet(const FsaDescriptor &fromQf, Core::Archive *archive,
                         const std::string &alphabetName);

  Lexicon::AlphabetMapRef outputAlphabetMap() const;
  void setOutputAlphabet(Fsa::ConstAlphabetRef alphabet,
                         const std::string &alphabetName);
  void loadOutputAlphabet(const FsaDescriptor &fromQf, Core::Archive *archive,
                          const std::string &alphabetName);
...
private:
  void init();
  void conform(FlfDescriptorRef desc, const Fsa::StaticAutomaton &structureFsa,
...
public:
  ...
  FlfReader(const Core::Configuration &, FlfContextRef,
            u32 contextHandling = TrustContext);
  virtual ~FlfReader();
  ...
  FlfDescriptorRef readDescriptor(const std::string &descFilename,
                                  Core::Archive *archive = 0);

  ConstLatticeRef read(FlfDescriptorRef desc, Core::Archive *archive = 0);

  ConstLatticeRef read(const std::string &descFilename, Core::Archive *archive);
...
public:
  ...
  FlfDescriptorRef buildDescriptor(ConstLatticeRef f,
                                   const std::string &descFilename) const;

  bool writeDescriptor(FlfDescriptorRef desc, const std::string &descFilename,
                       Core::Archive *archive = 0) const;

  bool write(ConstLatticeRef f, FlfDescriptorRef desc,
             const std::string &descFilename, bool storeAlphabet = true,
             Core::Archive *archive = 0) const;

  bool write(ConstLatticeRef f, const std::string &descFilename,
...
public:
  FlfArchiveReader(const Core::Configuration &config,
...
protected:
  ...
  void storeAlphabets();
  FlfDescriptorRef buildDescriptor(ConstLatticeRef l, const std::string &id,
                                   const std::string &filename);

public:
  FlfArchiveWriter(const Core::Configuration &config,
                   const std::string &pathname);
  ~FlfArchiveWriter();
