 ...
 *
 *    -call out to obtain the element at the output point (also called 'right'
 * or 'present'); -call removed to obtain the removed element after the last
 * call to add or flush; (Remark: neither out nor removed change the status of
 * the sliding window) -call get to obtain the element at a index relative to
 * the present position;
 *
 *    -use the iterators and operators to reach any arbitrary element in the
 * sliding window.
 */
template <class T> class SlidingWindow : private std::deque<T> {
...
public:
  ...
  /** Creates non-existing elements by copying the first resp. last element in
   * sliding window. */
  ...
  /** Creates non-existing elements by average of the present elements in the
   * sliding window. (Type of the Data structure T is:
   * Flow::DataPtr<Flow::Vector<f32>)
   */
  struct meanMarginPolicy : public marginPolicy {
    ...
    Flow::DataPtr<Flow::Vector<V> > setMean(Flow::Vector<V> &vec) const {
      ...
      for (int i = -(int)validFirst; i <= (int)validLast; i++) {
        ...
        std::transform(currentData.get()->begin(), currentData.get()->end(),
                       vec.begin(), vec.begin(), std::plus<V>());
      } // End for sum up
...
public:
  ...
  /** Base class for margin conditions retrieving the state of the sliding
   * window when loading or flushing.
   */
  ...
  /** Condition is satisfied if the sliding window contains at least one
   * element. */
  ...
  /** Condition is satisfied if the sliding window contains an element at the
   * present position. */
...
private:
  int absoluteIndex(int relativeIndex) const {
    require_(relativeIndex <= (int)maxFutureSize_ &&
             relativeIndex >= -(int)maxPastSize_);
...
public:
   ...
   *    @param maxSize gives the maximum number of elements in the sliding
   * window.
  ...
  /** Adds a new element
   *  Removes the oldest element if number of past elements exceeded
   * maxPastSize_. (@see init).
   */
   ...
   */
  void flush();
  /** Simulates adding at least so many (n) empty elements that output (@see
   * out) changes. Present element will be the element at t+n. Removes the
   * oldest element if number of past elements exceeded maxPastSize_ (@see
   * init). Use this function instead of flush if one function call has to
   ...
   *  @return is true if sliding window contains an element at position @param
   * relativeIndex.
   */
  bool get(int relativeIndex, T &out) const;
  /** returns the closest existing element to relative index @param
   * relativeIndex. For further explanations @see get. Note: assertion is raised
   * if sliding window is empty.
   */
  void getClosest(int relativeIndex, T &out) const;

  /** returns the present element if contained.
   *  @return is true if sliding window contains an element at 'present'
   * position, i.e. sliding window is filled up so far or is not yet flushed so
   * far.
   */
  bool out(T &out) const { return get(0, out); }

  /** @return is false if sliding window contains an element at 'present'
   * position. */
  ...
  /** @return is false if no element has been removed during the last call to
   * add or flushed. */
...
template <class T> bool SlidingWindow<T>::init(size_t maxSize, size_t right) {
  // special case of collecting a whole segment
  if (maxSize >= (size_t)Core::Type<s32>::max &&
      right >= (size_t)Core::Type<s32>::max)
    --right;
...
template <class T> void SlidingWindow<T>::clear() {
  std::deque<T>::clear();
  indexOfPresent_ = maxFutureSize_;
  removedValid_ = false;
}

template <class T> void SlidingWindow<T>::add(const T &add) {
...
template <class T> void SlidingWindow<T>::flush() {
...
template <class T> void SlidingWindow<T>::flushOut() {
  if ((int)size() < indexOfPresent_)
    indexOfPresent_ = (int)size();
  flush();
}

template <class T> void SlidingWindow<T>::removeOldest() {
...
template <class T> bool SlidingWindow<T>::get(int relativeIndex, T &out) const {
...
void SlidingWindow<T>::getClosest(int relativeIndex, T &out) const {
  require_(!empty());
  int index =
      std::min(std::max(absoluteIndex(relativeIndex), 0), (int)size() - 1);
  out = operator[](index);
}
