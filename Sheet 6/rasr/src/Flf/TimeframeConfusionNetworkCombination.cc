...
// -------------------------------------------------------------------------
namespace {
typedef std::pair<PosteriorCn::Slot::const_iterator,
                  PosteriorCn::Slot::const_iterator>
    SlotRange;
typedef Core::Vector<SlotRange> SlotRangeList;

const PosteriorCn::Slot EpsSlot =
    PosteriorCn::Slot(1, PosteriorCn::Arc(Fsa::Epsilon, 1.0));
} // namespace
 ...
 **/
void minEntropyWeights(WeightsList &weightsList,
  ...
  for (u32 i = 0; i < weightsList.size(); ++i) {
    ...
    for (u32 j = 0; j < cns.size(); ++j, ++itWeight) {
      const PosteriorCn::Slot &slot =
          (i < cns[j]->size()) ? (*cns[j])[i] : EpsSlot;
...
void inverseEntropyWeights(WeightsList &weightsList,
                           const ConstPosteriorCnRefList &cns,
                           const ScoreList &_weights) {
  const std::vector<f64> weights(_weights.begin(), _weights.end());
  ...
  for (u32 i = 0; i < weightsList.size(); ++i) {
    ...
    for (std::vector<f64>::const_iterator itWeightedInverseEntropy =
             weightedInverseEntropies.begin();
         itWeightedInverseEntropy != weightedInverseEntropies.end();
         ++itWeightedInverseEntropy, ++itSlotWeight)
...
public:
  FramePosteriorCnCombinationNode(const std::string &name,
...
const Core::ParameterFloat
    FramePosteriorCnCombinationNode::paramWeight("weight", "weight", 1.0);
const Core::Choice FramePosteriorCnCombinationNode::choiceWeightingScheme(
    ...
    "inverse-entropy",
    FramePosteriorCnCombinationNode::WeightingSchemeInverseEntropy,
    Core::Choice::endMark());
const Core::ParameterChoice
    FramePosteriorCnCombinationNode::paramWeightingScheme(
        "weighting", &FramePosteriorCnCombinationNode::choiceWeightingScheme,
        "weighting scheme",
