...
private:
  ...
  struct StateHypothesis {
    ...
    s32 nRefs; /**< number of references by traceback arcs in the following
                  timeframe */
  ...
  void activateOrUpdateStateHypothesisViterbi(StateId, Score, Trace,
                                              Am::AllophoneStateIndex);
  void activateOrUpdateStateHypothesisBaumWelch(StateId, Score, Trace,
                                                Am::AllophoneStateIndex,
                                                Fsa::Weight);
...
inline void Aligner::SearchSpace::activateOrUpdateStateHypothesisViterbi(
    StateId state, Score score, Trace trace, Am::AllophoneStateIndex emission) {
  ...
  if (shi >= StateHypothesisIndex(stateHypotheses_.size()) ||
      shi < firstHypCurrentFrame_ || stateHypotheses_[shi].state != state) {
...
inline void Aligner::SearchSpace::activateOrUpdateStateHypothesisBaumWelch(
    StateId toState, Score toStatePotential, StateHypothesisIndex fromShi,
    Am::AllophoneStateIndex emission, Fsa::Weight arcScore) {
  ...
  if (toShi >= nHypotheses || toShi < firstHypCurrentFrame_ ||
    ...
    stateHypotheses_.push_back(
        StateHypothesis(toState, toStatePotential, fromShi,
                        emission)); // trace and emission unused
  } else {
    StateHypothesis &sh(stateHypotheses_[toShi]);
    sh.score = (Score)traceback_->semiring()->collect(
        (Fsa::Weight)sh.score, (Fsa::Weight)toStatePotential);
...
void Aligner::SearchSpace::addStartupHypothesis() {
  require_(model_);
  stateHypotheses_.push_back(StateHypothesis(model_->initialStateId(), 0.0,
  ...
  if (!viterbi_) {
    ...
    Fsa::State *final =
        traceback_->newFinalState(traceback_->semiring()->one());
    verify(final->id() == 0);
  }
}
...
void Aligner::SearchSpace::expand(
  ...
  for (StateHypothesisIndex shi = firstHypPreviousFrame;
       shi < firstHypCurrentFrame_; ++shi) {
    ...
    for (Fsa::State::const_iterator a = s->begin(); a != s->end(); ++a) {
      Score emissionScore =
          emissionScores->score(acousticModel->emissionIndex(a->input_));
      ...
      if (viterbi_) {
        ...
        activateOrUpdateStateHypothesisBaumWelch(a->target_, score + arcScore,
                                                 shi, a->input_,
                                                 (Fsa::Weight)arcScore);
      }
    }
  ...
  if (viterbi_) {
    for (StateHypothesisIndex shi = firstHypCurrentFrame_;
         shi < (StateHypothesisIndex)stateHypotheses_.size(); ++shi)
...
Score Aligner::SearchSpace::minimumScore() const {
  ...
  for (StateHypothesisList::const_iterator sh =
           stateHypotheses_.begin() + +firstHypCurrentFrame_;
...
inline void
Aligner::SearchSpace::deleteStateHypothesisViterbi(StateHypothesisIndex shi) {
...
inline void
Aligner::SearchSpace::deleteStateHypothesisBaumWelch(StateHypothesisIndex shi) {
...
void Aligner::SearchSpace::collectGarbage() const {
  ...
  shi_in = shi_out = 0;
  if (viterbi_) {
    StateHypothesisIndex *map =
        new StateHypothesisIndex[stateHypotheses_.size()];
    ...
    firstHypCurrentFrame_ =
        (out - stateHypotheses_.begin()) - nStateHypotheses();
    stateHypotheses_.erase(out, stateHypotheses_.end());
    ...
    for (StateHypothesisIndex shi =
             (StateHypothesisIndex)stateHypotheses_.size();
    ...
    if (s->isFinal()) {
      ...
      if (viterbi_) {
        ...
        if (result == Core::Type<Score>::max) {
          ...
          result = (Score)traceback_->semiring()->collect((Fsa::Weight)result,
                                                          (Fsa::Weight)score);
        }
      }
    }
...
Fsa::ConstAutomatonRef Aligner::SearchSpace::getAlignmentFsaViterbi() const {
  ...
  result->addProperties(Fsa::PropertyLinear | Fsa::PropertyAcyclic |
                        Fsa::PropertySausages);
  ...
  for (TimeframeIndex time = time_; time > 0; --time) {
    ...
    s->newArc(time,
              Fsa::Weight(stateHypotheses_[shi].score -
                          stateHypotheses_[trace].score),
              stateHypotheses_[shi].emission);
    result->setState(s);
    shi = trace;
...
Fsa::ConstAutomatonRef Aligner::SearchSpace::getAlignmentFsaBaumWelch() const {
  ...
  // otherwise wrap "staticCopy" around transpose or use a non-lazy
  // implementation of transpose
  traceback_->deleteState(final->id()); // undo changes in the traceback fsa to
...
const Core::ParameterFloat Aligner::paramMinAcousticPruningThreshold(
    "min-acoustic-pruning", "maximal threshold for pruning of state hypotheses",
    50, Core::Type<f32>::delta);

const Core::ParameterFloat Aligner::paramMaxAcousticPruningThreshold(
    "max-acoustic-pruning", "maximal threshold for pruning of state hypotheses",
    Core::Type<f32>::max, 0.0);

const Core::ParameterFloat
    Aligner::paramAcousticPruningThresholdIncrementFactor(
        "acoustic-pruning-increment-factor",
        "maximal threshold for pruning of state hypotheses", 2, 1);

const Core::ParameterFloat Aligner::paramMinAverageNumberOfStateHypotheses(
    "min-average-number-of-states",
    "alignments with less average number of states per time-frame are repeated "
...
const Core::ParameterBool Aligner::paramIncreasePruningUntilNoScoreDifference(
    ...
    "alignments are repeated with increasing pruning threshold until final "
    "score does not change",
    true);

const Core::Choice Aligner::choiceMode("viterbi", modeViterbi, "baum-welch",
                                       modeBaumWelch, Core::Choice::endMark());

const Core::ParameterChoice Aligner::paramMode(
    "mode", &choiceMode,
    "describes how alignments are created and observations are weighted",
    modeViterbi);

const Core::ParameterFloat Aligner::paramMinWeight(
    "min-weight",
    "alignment items with a weight (i.e. state posterior probability) below "
    "this threshold are pruned",
    0.0);

const Core::ParameterBool Aligner::paramUsePartialSums(
    "use-partial-sums",
    "weigths of alignment items are not normalized over a timeframe", false);

Aligner::Aligner(const Core::Configuration &c)
    : Core::Component(c), ss_(0),
  ...
  acousticPruningThresholdIncrementFactor_ =
      paramAcousticPruningThresholdIncrementFactor(config);
  ...
  minAverageNumberOfStateHypotheses_ =
      paramMinAverageNumberOfStateHypotheses(config);
  increasePruningUntilNoScoreDifference_ =
      paramIncreasePruningUntilNoScoreDifference(config);
...
void Aligner::feed(
    const std::vector<Mm::FeatureScorer::Scorer> &emissionScorers) {
  Score previousScore = Core::Type<Score>::max;
  // make a copy of the feature scorers HERE to reuse the cache over all pruning
  // iterations
  ...
  for (Score t = minAcousticPruningThreshold_;
    ...
    if (reachedFinalState() and
        nStateHypotheses_.average() >= minAverageNumberOfStateHypotheses_) {
      if (!increasePruningUntilNoScoreDifference_)
        break;
      if (t > minAcousticPruningThreshold_ and
          Core::isAlmostEqual(score, previousScore))
        break;
    }
    previousScore = score;
  }
...
Score Aligner::alignmentScore() const { return ss_->finalStatePotential(); }
...
void Aligner::selectMode() { selectMode((Mode)paramMode(config)); }
...
public:
  ...
  virtual void exploreNonTreeArc(Fsa::ConstStateRef from, const Fsa::Arc &a) {
    verify(color(a.target()) ==
           Black); // verify, that the fsa contains no loops
    exploreArc(from, a);
  }
...
void Aligner::getAlignment(Alignment &result,
                           std::pair<Fsa::ConstAutomatonRef, Fsa::Weight>
                               alignmentPosteriorFsa) const {
...
Fsa::ConstAutomatonRef Aligner::getAlignmentFsa() const {
  ...
  if (statisticsChannel_.isOpen()) {
    statisticsChannel_ << Core::XmlOpen("alignment-statistics")
                       << Core::XmlFull("frames", ss_->time())
                       << Core::XmlFull("acoustic-pruning-threshold",
                                        acousticPruningThreshold_)
                       << Core::XmlOpen("score")
                       << Core::XmlFull("avg", ss_->finalStatePotential() /
                                                   f32(ss_->time()))
                       << Core::XmlFull("total", ss_->finalStatePotential())
                       << Core::XmlClose("score") << nStateHypotheses_
...
std::pair<Fsa::ConstAutomatonRef, Fsa::Weight>
Aligner::getAlignmentPosteriorFsa(Fsa::ConstAutomatonRef alignmentFsa) const {
  ...
  if (viterbi_) {
    //	result = Fsa::extend(Fsa::multiply(alignmentFsa, (Fsa::Weight)0.0),
    //(Fsa::Weight)1.0);
    result = Fsa::multiply(alignmentFsa, (Fsa::Weight)0.0);
  } else { // baum-welch
    result = Fsa::posterior64(
        Fsa::prunePosterior(alignmentFsa, Fsa::Weight(getLogWeightThreshold()),
                            false),
        totalInv);
...
class Aligner::WordLatticeBuilder::AddWordBoundaryDisambiguatorsAutomaton
...
protected:
  ...
  bool isDisambiguatorState(Fsa::StateId id) const { return (id % 2 == 1); }

public:
  AddWordBoundaryDisambiguatorsAutomaton(Fsa::ConstAutomatonRef f,
                                         Fsa::LabelId disambiguator)
      : Precursor(f), disambiguator_(disambiguator),
        one_(f->semiring()->one()){};
...
protected:
  ...
  void setCoarticulatedWordBoundary(Fsa::StateId s, TimeframeIndex time,
                                    Fsa::LabelId lastAllophone) {
    Lattice::WordBoundary::Transit transit;
    if (lastAllophone != Fsa::InvalidLabelId) {
      Am::Phonology::SemiContext future =
          allophoneStateAlphabet_->allophoneState(lastAllophone)
              .allophone()
              ->future();
      if (future.size()) {
        transit.initial = allophoneStateAlphabet_->allophoneState(lastAllophone)
                              .allophone()
                              ->central();
  ...
  void exploreArc(Fsa::ConstStateRef fromState, const Fsa::Arc &a) {
    ...
    if (a.input() == Fsa::Epsilon) {
      ...
      setCoarticulatedWordBoundary(a.target(), fromInfo.time_,
                                   fromInfo.lastAllophone_);
...
public:
  ...
  Lattice::ConstWordLatticeRef build() {
    ...
    wordLattice->setFsa(Fsa::ConstAutomatonRef(t),
    ...
    /** the following 2 lines are a workaround for the current
     * Lattice::normalize procedure which produces correct new word boundaries
     * only if all fsa states are accessed afterwards with 'Fsa::getState'
     */
    result->part(Lattice::WordLattice::acousticFsa) = Fsa::ConstAutomatonRef(
        Fsa::staticCopy(result->part(Lattice::WordLattice::acousticFsa)));
    return result;
  }
};

Aligner::WordLatticeBuilder::WordLatticeBuilder(
    const Core::Configuration &configuration, Bliss::LexiconRef lexicon,
    Core::Ref<const Am::AcousticModel> acousticModel)
    : Core::Component(configuration), lexicon_(lexicon),
      acousticModel_(acousticModel),
...
void Aligner::WordLatticeBuilder::setModelTransducer(
    Fsa::ConstAutomatonRef modelTransducer) {
  require(modelTransducer);
  allophoneToLemmaPronunciationTransducer_ = modelTransducer;
}

Fsa::ConstAutomatonRef
Aligner::WordLatticeBuilder::addWordBoundaryDisambiguators(
    Fsa::ConstAutomatonRef f) const {
  return Fsa::ConstAutomatonRef(new AddWordBoundaryDisambiguatorsAutomaton(
      f, allophoneStateAlphabet_->disambiguator(0)));
}

Fsa::ConstAutomatonRef
Aligner::WordLatticeBuilder::buildAlignmentToLemmaPronunciationTransducer(
    Fsa::ConstAutomatonRef alignmentFsaWithDisambiguators) const {
  return Fsa::trim(Fsa::composeMatching(
      alignmentFsaWithDisambiguators,
      Fsa::multiply(allophoneToLemmaPronunciationTransducer_,
                    Fsa::Weight(0.0))));
}
...
Lattice::ConstWordLatticeRef Aligner::WordLatticeBuilder::convertToWordLattice(
  ...
  Converter transducerToLattice(alignmentToLemmaPronunciationTransducer,
                                allophoneStateAlphabet_);
  return transducerToLattice.build();
}

Lattice::ConstWordLatticeRef
Aligner::WordLatticeBuilder::build(Fsa::ConstAutomatonRef alignmentFsa) {
  if (alignmentFsa->initialStateId() == Fsa::InvalidStateId) {
    error("Cannot generate word lattice because alignment did not reach a "
          "final state.");
    return Lattice::ConstWordLatticeRef();
  }

  Fsa::ConstAutomatonRef f = Fsa::staticCopy(Fsa::changeSemiring(
      addWordBoundaryDisambiguators(alignmentFsa), Fsa::TropicalSemiring));
  require(f);
  if (dumpAutomaton_.isOpen()) {
    Fsa::info(f, dumpAutomaton_);
    Fsa::drawDot(f, "/tmp/alignment-with-disambiguators.dot");
    Fsa::write(f, "bin:/tmp/alignment-with-disambiguators.binfsa.gz",
               Fsa::storeStates);
  }
  ...
  if (dumpAutomaton_.isOpen()) {
    ...
    Fsa::write(f,
               "bin:/tmp/alignment-to-lemma-pronunciation-transducer.binfsa.gz",
               Fsa::storeStates);
  }

  f = Fsa::best(
      f); /* <-- you may remove this line to create non-linear lattices from
           * baum-welch alignments. The drawback is that even in viterbi mode
           * the resulting lattice may become very large due to redundant
           * parallel silence arcs. A silence segment of length n induces
           * n(n+1)/2 silence word arcs! Possible solution: disallow consecutive
  ...
  if (dumpAutomaton_.isOpen()) {
    ...
    Fsa::write(f,
               "bin:/tmp/best-alignment-to-lemma-pronunciation-path.binfsa.gz",
               Fsa::storeStates);
  }
  ...
  if (dumpAutomaton_.isOpen()) {
    ...
    Fsa::drawDot(result->part(Lattice::WordLattice::acousticFsa),
                 "/tmp/lattice.dot");
    Lattice::dumpWordBoundaries(result->wordBoundaries(),
                                "/tmp/lattice-word-boundaries.xml");
  }
...
WordSequenceAligner::WordSequenceAligner(
    ...
    Core::Ref<const Am::AcousticModel> acousticModel)
    : Core::Component(c), acousticModel_(acousticModel), lexicon_(lexicon),
      modelBuilder_(0), aligner_(c),
      alignmentChannel_(config, "alignment-fsa") {
  modelBuilder_ =
      new Speech::AllophoneStateGraphBuilder(config, lexicon, acousticModel_);
}

WordSequenceAligner::~WordSequenceAligner() { delete modelBuilder_; }
