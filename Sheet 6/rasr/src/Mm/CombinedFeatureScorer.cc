...
//==================================================================================================
CombinedFeatureScorer::CombinedContextScorer::CombinedContextScorer(
    Core::Ref<const Feature> f, const CombinedFeatureScorer *fs)
    : ContextScorer(), combinedFeatureScorer_(fs),
...
CombinedFeatureScorer::CombinedContextScorer::CombinedContextScorer(
    const FeatureVector &featureVector, const CombinedFeatureScorer *fs)
    : ContextScorer(), combinedFeatureScorer_(fs),
      mixtureIndexTable_(fs->mixtureIndexTable_) {
  require(1 == fs->nModels());
  Core::Ref<const Feature> modelFeature(new Feature(featureVector));
  contextScorers_.push_back(
      fs->featureScorers_.front()->getScorer(modelFeature));
}

Score CombinedFeatureScorer::CombinedContextScorer::score(
    EmissionIndex e) const {
  Score result = 0;
  ...
  MixtureIndexTableRow::const_iterator mixtureIndex =
      mixtureIndexTable_[e].begin();
...
CombinedFeatureScorer::CombinedFeatureScorer(
    ...
    : Core::Component(configuration), Precursor(configuration),
      featureScorers_(featureScorers), mixtureIndexTable_(mixtureIndexTable),
...
void CombinedFeatureScorer::distributeScaleUpdate(
...
void CombinedFeatureScorer::getFeatureDescription(
    FeatureDescription &description) const {
  for (size_t m = 0; m < nModels(); ++m) {
    FeatureDescription::Stream &stream(description[m]);
    FeatureDescription d(
        FeatureDescription::prepareName(fullName(), "sub-feature-description"));
...
void CombinedFeatureScorer::getDependencies(
