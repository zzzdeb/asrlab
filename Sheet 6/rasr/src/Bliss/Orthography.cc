...
OrthographicParser::OrthographicParser(const Core::Configuration &c,
                                       LexiconRef _lexicon)
    : Component(c), lexicon_(_lexicon), allowForSilenceRepetitions_(true) {
...
void OrthographicParser::Handler::newUnmatchableEdge(Node from, Node to,
...
void OrthographicParser::parse(const std::string &orth,
  ...
  for (i = 0; i <= length; ++i) {
    ...
    for (int l = length - i; l >= 0; --l) {
      ...
      for (lemma = matchingLemmas.first; lemma != matchingLemmas.second;
           ++lemma) {
        const bool isSilenceLemma =
            (lemma->second == lexicon_->specialLemma("silence"));
        if (allowForSilenceRepetitions_ or !isSilenceLemma) {
          handler.newEdge(b_nodes[i], b_nodes[j], lemma->second);
        } else {
          handler.newSilenceEdge(b_nodes[i], handler.newNode(), lemma->second,
                                 i == length);
        }
      }
    }
...
// ===========================================================================
OrthographicParser::LemmaRange
OrthographicParser::lemmas(std::string &orth) const {
  Core::enforceTrailingBlank(orth);
  int maxId(orth.size());
  OrthographicPrefixTree::ItemRange range =
      prefixTree_.lookup(orth.c_str(), maxId);
...
void LemmaAcceptorBuilder::newEdge(OrthographicParser::Node from,
                                   OrthographicParser::Node to,
                                   const Lemma *lemma) {
  Fsa::State *source = static_cast<Fsa::State *>(from);
  Fsa::State *target = static_cast<Fsa::State *>(to);
  source->newArc(target->id(), result_->semiring()->one(),
                 (lemma) ? lemma->id() : Fsa::Epsilon);
}

void LemmaAcceptorBuilder::newSilenceEdge(OrthographicParser::Node from,
                                          OrthographicParser::Node to,
  ...
  for (Fsa::State::const_iterator a = source->begin(); a != source->end();
       ++a) {
...
void LemmaAcceptorBuilder::finalize(OrthographicParser::Node initial,
...
// ===========================================================================
Core::Ref<LemmaAcceptor>
OrthographicParser::createLemmaAcceptor(const std::string &orth) const {
