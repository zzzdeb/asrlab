...
void CodeGenerator::finalize() {
  ...
  void *page = mmap(0, bytesNeeded, PROT_EXEC | PROT_READ | PROT_WRITE,
  ...
  msg << "aligned code to address " << (void *)code_ << " (page size is "
      << u32(pagesize) << ").";
}

CodeGenerator::CodeGenerator(const Configuration &c)
    : Component(c), finalized_(false), n_(0), code_(0) {}
