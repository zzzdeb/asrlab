 ...
 */
template <class A, class P> class VectorScalarFunction {
public:
 ...
 *  energy = sum_i |x_i|^2
 */
template <class T> class EnergyFunction : public VectorScalarFunction<T, T> {
public:
...
const Core::ParameterFloat
    paramVectorScalarFunctionParameter("value", "parametric value", 0);

/** VectorScalarFunctionNode maps a vector to a scalar value.
 */
template <class Function> class VectorScalarFunctionNode : public SleeveNode {
public:
...
VectorScalarFunctionNode<Function>::VectorScalarFunctionNode(
    const Core::Configuration &c)
    : Component(c), SleeveNode(c), function_(c) {
  setFunctionParameter(paramVectorScalarFunctionParameter(c));
}

template <class Function> bool VectorScalarFunctionNode<Function>::configure() {
