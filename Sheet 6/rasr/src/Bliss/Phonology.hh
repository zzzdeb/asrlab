...
class ContextPhonology : public Phonology, public Core::ReferenceCounted {
public:
  class SemiContext
...
public:
  ContextPhonology(PhonemeInventoryRef, u32 maximumHistoryLength = 1,
...
public:
  ...
  PhonemeInContext(Phoneme::Id phoneme,
                   const SemiContext &history = SemiContext(),
                   const SemiContext &future = SemiContext())
  ...
  const SemiContext &history() const { return context_.history; }
  void setHistory(const SemiContext &history) { context_.history = history; }
  const SemiContext &future() const { return context_.future; }
  void setFuture(const SemiContext &future) { context_.future = future; }

private:
  std::string formatPhoneme(Core::Ref<const PhonemeInventory>,
                            Phoneme::Id) const;
