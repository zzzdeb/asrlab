 ...
 * Vector
 */
template <class T> class Vector : public Timestamp, public std::vector<T> {
private:
...
template <typename T> bool Vector<T>::read(Core::BinaryInputStream &i) {
  u32 s;
  i >> s;
  this->resize(s);
  T e; // Intoduced for T=bool: std::vector<bool> does not meet the requirements
...
template <typename T> bool Vector<T>::write(Core::BinaryOutputStream &o) const {
  o << (u32)this->size();
  std::copy(this->begin(), this->end(),
            Core::BinaryOutputStream::Iterator<T>(o));
...
namespace Core {
template <typename T> class NameHelper<Flow::Vector<T> > : public std::string {
public:
