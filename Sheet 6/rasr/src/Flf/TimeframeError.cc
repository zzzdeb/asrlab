...
TimeframeError::TimeframeError(Score alpha)
    : alpha(alpha), nWords(0), nFrames(0), nFrameErrors(0), nSmoothedFrames(0),
...
void TimeframeError::add(ConstLatticeRef lHyp, ConstLatticeRef lRef) {
  ...
  if (hypEnd != refEnd)
    Core::Application::us()->warning(
        "Hypothesis starts at %.3f and reference at %.3f; assume epsilon for "
        "preceeding time frames",
  ...
  for (;;) {
    ...
    if (nFrames > 0) {
        ...
        if (srRef->hasArcs()) {
          if (srRef->nArcs() > 1)
            Core::Application::us()->criticalError(
                "Reference lattice \"%s\" is not linear.",
                lRef->describe().c_str());
          ...
          Core::Application::us()->warning(
              "Reference ends at %.3f before hypothesis; assume epsilon for "
      ...
      this->nSmoothedFrames +=
          FramePosteriorCnFeatures::smooth(duration, duration, alpha);
      this->nSmoothedFrameErrors +=
          FramePosteriorCnFeatures::smooth(error, duration, alpha);
    }
      ...
      break;
    {
      ...
      if (srHyp->hasArcs()) {
        if (srHyp->nArcs() > 1)
          Core::Application::us()->criticalError(
              "Hypothesis lattice \"%s\" is not linear.",
              lHyp->describe().c_str());
        ...
        Core::Application::us()->warning(
            "Hypothesis ends at %.3f before reference; assume epsilon for "
            "remaining time frames",
            (f32(hypEnd) / 100.0));
...
void TimeframeError::add(ConstLatticeRef lHyp, ConstPosteriorCnRef cnRef) {
  FramePosteriorCnFeaturesRef features =
      FramePosteriorCnFeatures::create(cnRef);
  ConstBoundariesRef bHyp = lHyp->getBoundaries();
  Time begin, end = bHyp->time(lHyp->initialStateId());
  for (ConstStateRef sr = lHyp->getState(lHyp->initialStateId()), nextSr;
...
Score TimeframeError::error() const { return nFrameErrors / nFrames; }
...
TimeframeErrorRef TimeframeError::create(ConstLatticeRef lHyp,
                                         ConstLatticeRef lRef, Score alpha) {
...
protected:
  ...
  ConstLatticeRef filter(ConstLatticeRef lHyp) {
    ...
    if (!hasAdded_) {
      ...
      sumError_->add(*error);
      xml_ << (Core::XmlOpen("time-frame-error") +
               Core::XmlAttribute("a", error->alpha));
      dump(error);
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (connected(1)) {
      ...
      criticalError("Either reference lattice at port 1 or reference fCN at "
                    "port 2 required.");
    sumError_ = TimeframeError::create(
        FramePosteriorCnFeatures::paramAlpha(config, 0.05));
    msg << "Error smoothing alpha is " << sumError_->alpha << ".";
    hasAdded_ = false;
  }

  virtual void sync() { hasAdded_ = false; }

  virtual void finalize() {
    xml_ << (Core::XmlOpen("sum-time-frame-error") +
             Core::XmlAttribute("a", sumError_->alpha));
    dump(sumError_);
    xml_ << Core::XmlClose("sum-time-frame-error");
  }
};

NodeRef createTimeframeErrorNode(const std::string &name,
...
public:
  ...
  public:
    ...
    virtual ~SearchSpace() {}
    virtual ConstLatticeRef best(const ConstLatticeRefList &lats,
...
public:
  ...
  ~MinimumFrameWerDecoder() { delete ss_; }

  ConstLatticeRef decode(const ConstLatticeRefList &lats,
...
const Core::Choice MinimumFrameWerDecoder::choiceSearchSpaceType =
    Core::Choice("union", SearchSpaceTypeUnion, "mesh", SearchSpaceTypeMesh,
                 "cn", SearchSpaceTypeFrameCn, Core::Choice::endMark());
const Core::ParameterString MinimumFrameWerDecoder::paramSearchSpaceType(
    "search-space", "search spaces: union, mesh or cn(=fCN)", "mesh");
// -------------------------------------------------------------------------
...
protected:
  Score alpha(Fsa::LabelId word) const {
    ...
    if (word > lastNonWord_)
      return alpha_;
    std::vector<Fsa::LabelId>::const_reverse_iterator itNonWord =
        nonWords_.rbegin();
  ...
  */
  Score calcScore(const PosteriorCn &cn, Fsa::LabelId label, Time start,
                  Time end) const {
    if (start == end)
      return 0.0;
  ...
  */
  ...
  /*
    Confidence score
  */
  Score calcConfidence(const PosteriorCn &cn, Fsa::LabelId label, Time start,
                       Time end) const {
    if (start == end)
      return 1.0;
    const PosteriorCn::Arc cnArc(label, 0.0);
  ...
  struct Hyp {
    ...
    Fsa::StateId bptr;
    Hyp()
        : score(Semiring::Max), label(Fsa::InvalidLabelId),
  ...
  struct Trace : Core::Vector<Hyp>, public Core::ReferenceCounted {
    ...
    u32 system;
    Trace()
        : Core::Vector<Hyp>(), score(Semiring::Max), final(Fsa::InvalidStateId),
          system(Core::Type<u32>::max) {}
  };
  typedef Core::Ref<const Trace> ConstTraceRef;

  ConstTraceRef search(const ConstLatticeRefList &lats,
  ...
  ConstLatticeRef trace(const ConstLatticeRefList &lats, const PosteriorCn &cn,
    ...
    for (Fsa::StateId sourceSid = (*trace)[trace->final].bptr,
                      targetSid = trace->final;
         sourceSid != Fsa::InvalidStateId;
         sourceSid = (*trace)[targetSid].bptr) {
      ...
      if (confidenceId != Semiring::InvalidId) {
        ...
        score->set(confidenceId,
                   calcConfidence(cn, target.label, b->time(sp->id()),
                                  b->time(lastSp->id())));
      } else
        score = target.weight;
...
const Core::ParameterFloat
    UnionSearchSpace::paramAlpha("alpha",
                                 "smooth time frame error normalization", 0.05);
const Core::ParameterFloat UnionSearchSpace::paramNonWordAlpha(
    "non-word-alpha", "smooth time frame error normalization for non words",
    0.0);
const Core::ParameterString
    UnionSearchSpace::paramConfKey("confidence-key", "store confidence score",
                                   "");
// -------------------------------------------------------------------------
...
public:
  /*
    ...
    in conjunction with null-length arcs: loops cannot be detected/avoided
    easily. Allowing more boundary information would require a two-step
    procedure:
    1. build mesh, allow loops
    2. detect loops and remove arcs closing a loop (which arc to remove might be
    arbitrary)
  */
  struct BoundaryTimeHash {
    size_t operator()(const Boundary &b) const { return size_t(b.time()); }
  };
  ...
  typedef Fsa::Hash<Boundary, BoundaryTimeHash, BoundaryTimeEqual>
      BoundaryTimeHashList;
  ...
  ConstLatticeRef buildMesh(const ConstLatticeRefList &lats) {
    ...
    for (u32 i = 1; i < lats.size(); ++i)
      if ((semiring.get() != lats[i]->semiring().get()) &&
        ...
        log("Building time conditioned search space: Semirings are not equal "
            "(%s != %s), use %s",
            lats[0]->semiring()->name().c_str(),
            lats[i]->semiring()->name().c_str(), semiring->name().c_str());
        break;
      }
    StaticBoundaries *b = new StaticBoundaries;
    ...
    s->setDescription(
        Core::form("mesh-search-space(%s)", semiring->name().c_str()));
    s->setInputAlphabet(lats[0]->getInputAlphabet());
    s->setSemiring(semiring);
    s->setBoundaries(ConstBoundariesRef(b));
    ...
    for (u32 i = 0; i < lats.size(); ++i) {
      ...
      std::pair<Fsa::StateId, bool> meshInitial =
          meshSids.insertExisting(initialB);
      ...
      for (u32 j = 0; j < topologicalSort->size(); ++j) {
          ...
          if (defaultScore)
            ...
            if (meshSp->isFinal()) {
              if (semiring->compare(meshSp->weight(), sr->weight()) != 0)
                warning("Building time conditioned search space: Final scores "
                        "at time %d are not equal",
                        meshB.time());
            } else
        ...
        for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
          ...
          if (meshB.time() < targetB.time()) {
            std::pair<Fsa::StateId, bool> meshTarget =
                meshSids.insertExisting(targetB);
            ...
            meshSp->newArc(meshTarget.first,
                           (defaultScore ? defaultScore : a->weight()),
                           a->input());
          } else
    ...
    if (initialSids.size() == 1)
      ...
      State *meshInitialSp;
      Boundary initialB(
          0, Boundary::Transit(Bliss::Phoneme::term, Bliss::Phoneme::term));
      std::pair<Fsa::StateId, bool> meshInitial =
          meshSids.insertExisting(initialB);
      if (!meshInitial.second) {
      ...
      for (Core::Vector<Fsa::StateId>::const_iterator itSid =
               initialSids.begin();
           itSid != initialSids.end(); ++itSid)
        if (*itSid != meshInitialSp->id())
          meshInitialSp->newArc(*itSid, s->semiring()->one(), Fsa::Epsilon,
                                Fsa::Epsilon);
...
public:
  MeshSearchSpace(const Core::Configuration &config) : Precursor(config) {}
  ...
  virtual ConstLatticeRef best(const ConstLatticeRefList &lats,
    ...
    dynamic_cast<StaticLattice *>(const_cast<Lattice *>(result.get()))
        ->setDescription(Core::form("min-fWER(mesh,%s)",
                                    result->semiring()->name().c_str()));
    return result;
  }
};
// -------------------------------------------------------------------------
...
public:
  virtual ConstLatticeRef best(const ConstLatticeRefList &lats,
    ...
    s->setSemiring(semiring);
    s->setDescription(
        Core::form("min-fWER(fCN,%s)", s->semiring()->name().c_str()));
    s->setBoundaries(ConstBoundariesRef(b));
    State *lastSp = s->newState();
...
const Core::ParameterFloat
    FrameCnSearchSpace::paramWordPenalty("word-penalty",
                                         "penalty per word change", 0.0);
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
MinimumFrameWerDecoder::SearchSpace *
MinimumFrameWerDecoder::initSearchSpace(const Core::Configuration &config) {
  ...
  case SearchSpaceTypeUnion:
    return new UnionSearchSpace(Core::Configuration(config, "union"));
  case SearchSpaceTypeMesh:
    return new MeshSearchSpace(Core::Configuration(config, "mesh"));
  case SearchSpaceTypeFrameCn:
    return new FrameCnSearchSpace(Core::Configuration(config, "cn"));
  default:
    return 0;
  }
}
...
public:
  MinimumFrameWerDecoderNode(const std::string &name,
                             const Core::Configuration &config)
      : Node(name, config), n_(0), decoder_(0) {}
  virtual ~MinimumFrameWerDecoderNode() { delete decoder_; }
    ...
    if (!best_) {
      ...
      if (connected(0))
        ...
        std::pair<ConstLatticeRef, ConstFwdBwdRef> fbResult =
            (n_ == 1) ? fbBuilder_->build(lats.front())
      ...
      /*
        ...
        for (ConstStateRef sr = best->getState(best->initialStateId());
        sr->hasArcs(); sr = best->getState(sr->begin()->target())) os <<
        best->getInputAlphabet()->symbol(sr->begin()->input()) <<
        Core::XmlBlank(); os <<
        Core::XmlClose("best-time-frame-word-posterior-hypothesis");
      */
    }
    return best_;
  }

  virtual void sync() { best_.reset(); }
};
NodeRef createMinimumFrameWerDecoderNode(const std::string &name,
                                         const Core::Configuration &config) {
