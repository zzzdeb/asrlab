...
struct Allophone : public Phonology::Allophone {
  ...
  Allophone(const Phonology::Allophone &a, s16 b) : Precursor(a), boundary(b) {}
...
private:
  ...
  typedef Core::hash_map<const Allophone *, Fsa::LabelId, AllophonePtrHash,
                         AllophonePtrEq>
      AllophoneMap;
...
protected:
  ...
  */
  ...
  std::pair<AllophoneMap::iterator, bool> insert(const Allophone &allo) const;
  std::pair<AllophoneMap::iterator, bool>
  createAndInsert(const std::vector<bool> &cd, const Bliss::Pronunciation &p,
                  s32 b, s32 c, s32 e, s16 boundary) const;
...
public:
  AllophoneAlphabet(const Core::Configuration &config,
                    ConstPhonologyRef phonology, Bliss::LexiconRef lexicon);
  ...
  Fsa::LabelId index(const Allophone &allo) const { return index(&allo); }
  ...
  const AllophoneList &allophones() const { return allophones_; }

  ConstPhonologyRef phonology() const { return phonology_; }
  Fsa::LabelId nClasses() const { return allophones_.size(); }
...
public:
  AllophoneState() : allo_(0), state_(0) {}
...
public:
  ...
  AllophoneState operator*() const { return allophoneState(); }
...
public:
  AllophoneStateAlphabet(const Core::Configuration &config,
