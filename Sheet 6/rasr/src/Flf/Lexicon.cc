    ...
    if (!Core::strconv(symbol, id)) {
      if (symbol == "*EPS*")
        ...
        Core::Application::us()->error(
            "Failed to convert \"%s\"; it is not a valid integer",
            symbol.c_str());
...
const Core::ParameterString Lexicon::paramFile("file", "file", "");

const Core::ParameterBool Lexicon::paramNormalizePronunciationWeights(
    "normalize-pronunciation", "normalize pronunciation weights/scores", true);

const Core::ParameterBool
    Lexicon::paramReadOnly("read-only", "do not dynamically add unknown tokens",
                           false);

Lexicon::Lexicon(const Core::Configuration &config) : Precursor(config) {
  verify(!us_);
  us_ = LexiconRef(this);
  ...
  if (!paramFile(config).empty()) {
    ...
    for (LemmaIterator l = lemmas().first; l != lemmas().second; ++l) {
      const Lemma *lemma = *l;
      for (Bliss::OrthographicFormList::Iterator
               o = lemma->orthographicForms().begin(),
               o_end = lemma->orthographicForms().end();
           o != o_end; ++o)
        if (!orthographyMap_
                 .insert(std::make_pair(std::string(*o),
                                        const_cast<Lemma *>(lemma)))
                 .second)
          warning("Orthography \"%s\" occurs twice; a lookup for the "
                  "orthography will map to lemma \"%s\".",
                  o->str(), lemma->symbol().str());
    }
    // hash orthographies
  } else
    us_->setPhonemeInventory(
        Core::Ref<Bliss::PhonemeInventory>(new Bliss::PhonemeInventory));
  unknownAlphabet_ = Fsa::ConstAlphabetRef(new UnknownAlphabet);
  phonemeInventory_ =
      const_cast<Bliss::PhonemeInventory *>(phonemeInventory().get());
...
Lexicon::~Lexicon() { delete insertionChannel_; }
  ...
  */
  {
    ...
    if (sentenceEndLemma) {
      ...
      // unk lemma pronunciation
      LemmaPronunciationRange sentenceEndPronRange =
          sentenceEndLemma->pronunciations();
      ...
      sentenceEndLemmaId_ = sentenceEndLemmaPronunciationId_ =
          Fsa::InvalidLabelId;
  }
}
...
Lexicon::ConstLemmaPtrList Lexicon::nonWordLemmas() {
  ...
  for (std::pair<LemmaIterator, LemmaIterator> lRange = lemmas();
       lRange.first != lRange.second; ++lRange.first) {
    const Bliss::Lemma *lemma = *lRange.first;
    std::pair<Bliss::Lemma::EvaluationTokenSequenceIterator,
              Bliss::Lemma::EvaluationTokenSequenceIterator>
        eRange = lemma->evaluationTokenSequences();
...
Lexicon::ConstLemmaPronunciationPtrList Lexicon::nonWordLemmaPronunciations() {
  ...
  for (ConstLemmaPtrList::const_iterator itNonWordLemma =
           _nonWordLemmas.begin();
       itNonWordLemma != _nonWordLemmas.end(); ++itNonWordLemma)
    for (Bliss::Lemma::LemmaPronunciationRange lpRange =
             (*itNonWordLemma)->pronunciations();
...
LabelIdList Lexicon::nonWordLemmaIds() {
  ...
  for (ConstLemmaPtrList::const_iterator itNonWordLemma =
           _nonWordLemmas.begin();
       itNonWordLemma != _nonWordLemmas.end();
...
Fsa::LabelId Lexicon::phonemeId(const std::string &id) {
  ...
  if (insertionChannel_->isOpen())
    *insertionChannel_ << (Core::XmlFull("new-phoneme",
                                         phoneme->symbol().str()) +
...
Fsa::LabelId Lexicon::lemmaId(const std::string &id) {
  ...
  if (!lb.first) {
    ...
    ss << "possible reason: " << id
       << " is unknown and lexicon option 'read-only' is 'true'";
    error(ss.str().c_str());
    defect();
  ...
  if (!lb.second) {
    ...
    if (insertionChannel_->isOpen())
      *insertionChannel_ << (Core::XmlFull("new-lemma",
                                           lb.first->symbol().str()) +
...
Fsa::LabelId Lexicon::lemmaPronunciationId(const std::string &id) {
  ...
  if ((i == std::string::npos) || (k == std::string::npos) || (k < j)) {
    criticalError("Invalid lemma pronunciation symbol \"%s\".", id.c_str());
    ...
    if (phon == lpRange.first->pronunciation()->format(phonemeInventory())) {
      if (insertionChannel_->isOpen())
        *insertionChannel_ << (Core::XmlFull(
                                   ...
                                   lemmaPronunciationAlphabet()->symbol(
                                       lpRange.first->id())) +
...
Fsa::LabelId Lexicon::lemmaPronunciationId(const std::string &orth,
                                           s32 variant) {
...
Fsa::LabelId Lexicon::syntacticTokenId(const std::string &id) {
  ...
  std::pair<Lemma *, bool> lb = getLemma(id);
  if (lb.second) {
    criticalError("Could not add syntactic token \"%s\", "
                  "because a lemma with the same orthography but a different "
                  "syntactic token does already exist",
                  id.c_str());
    return Fsa::InvalidLabelId;
  }
  Precursor::setSyntacticTokenSequence(lb.first,
  ...
  if (insertionChannel_->isOpen())
    *insertionChannel_ << (Core::XmlFull(
                               "new-syntactic-token",
                               syntacticTokenAlphabet()->symbol(label)) +
...
Fsa::LabelId Lexicon::evaluationTokenId(const std::string &id) {
  ...
  Precursor::addEvaluationTokenSequence(_lemma,
                                        std::vector<std::string>(1, id));
  updateLemma(_lemma);
  ...
  if (insertionChannel_->isOpen())
    *insertionChannel_ << (Core::XmlFull(
                               "new-evaluation-token",
                               evaluationTokenAlphabet()->symbol(label)) +
                           Core::XmlAttribute("id", label));
  return label;
}

const LemmaPronunciation *Lexicon::lemmaPronunciation(const Lemma *_lemma,
                                                      s32 variant) {
  require(_lemma);
  ...
  if (insertionChannel_->isOpen())
    *insertionChannel_ << (Core::XmlFull("new-lemma-pronunciation",
                                         lemmaPronunciationAlphabet()->symbol(
                                             lpRange.first->id())) +
                           Core::XmlAttribute("id", lpRange.first->id()));
  return lpRange.first;
...
std::pair<const Lemma *, s32>
Lexicon::lemmaPronunciationVariant(const Bliss::LemmaPronunciation *lp) {
  require(lp);
...
void Lexicon::SymbolMap::indices(const std::string &str,
                                 std::vector<Fsa::LabelId> &ids) const {
...
Lexicon::SymbolMap Lexicon::symbolMap(AlphabetId alphabetId) {
  ...
  case LemmaPronunciationAlphabetId:
    return SymbolMap(lemmaPronunciationAlphabet(),
                     &Lexicon::lemmaPronunciationId);
  ...
  case PhonemeAlphabetId:
    return SymbolMap(phonemeInventory()->phonemeAlphabet(),
                     &Lexicon::phonemeId);
  default:
    return SymbolMap();
  }
...
const Core::Choice Lexicon::AlphabetNameMap(
    "unknown", UnknownAlphabetId, "phoneme", PhonemeAlphabetId, "lemma",
    LemmaAlphabetId, "lemma-pronunciation", LemmaPronunciationAlphabetId,
    "syntax", SyntacticTokenAlphabetId, "evaluation", EvaluationTokenAlphabetId,
    Core::Choice::endMark());

Lexicon::AlphabetId Lexicon::alphabetId(const std::string &name,
                                        bool dieOnFailure) {
...
Lexicon::AlphabetId Lexicon::alphabetId(Fsa::ConstAlphabetRef alphabet,
                                        bool dieOnFailure) {
  if (alphabet == lemmaAlphabet()) {
    ...
    if (dieOnFailure)
      criticalError("Alphabet \"%p\" is not one of the lexicon's alphabets.",
                    alphabet.get());
    return InvalidAlphabetId;
  }
}
...
Lexicon::AlphabetMap::AlphabetMap(Fsa::ConstAlphabetRef from,
                                  Fsa::ConstAlphabetRef to)
    : lexicon_(Lexicon::us().get()), unkLabel_(Fsa::InvalidLabelId),
      from_(from), to_(to) {
  ensure(from_ && to_);
}
...
public:
  PhonemeAlphabetMap(Fsa::ConstAlphabetRef from)
      : AlphabetMap(from,
                    Lexicon::us()->phonemeInventory()->phonemeAlphabet()) {}
};

class UnknownAlphabetMap : public Lexicon::AlphabetMap {
protected:
  virtual Fsa::LabelId index(Fsa::LabelId label) { return label; }
...
Lexicon::AlphabetMapRef Lexicon::alphabetMap(Fsa::ConstAlphabetRef from,
                                             AlphabetId id) {
  switch (id) {
  case LemmaAlphabetId:
...
Lexicon::PhonemeToLemmaPronunciationTransducerRef
Lexicon::phonemeToLemmaPronunciationTransducer() {
  if (!phonemeToLemmaPronunciationTransducer_)
    phonemeToLemmaPronunciationTransducer_ = Precursor::
        createPhonemeToLemmaPronunciationAndStickPunctuationTransducer();
  return phonemeToLemmaPronunciationTransducer_;
}

Lexicon::LemmaPronunciationToLemmaTransducerRef
Lexicon::lemmaPronunciationToLemmaTransducer() {
...
Lexicon::LemmaToSyntacticTokenTransducerRef
Lexicon::lemmaToSyntacticTokenTransducer() {
...
Lexicon::LemmaToEvaluationTokenTransducerRef
Lexicon::lemmaToEvaluationTokenTransducer() {
...
Lexicon::LemmaToEvaluationTokenTransducerRef
Lexicon::lemmaToPreferredEvaluationTokenSequenceTransducer() {
...
private:
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    ...
    if (!alphabet_)
      ...
      verify(Lexicon::us()->alphabetId(alphabet_) ==
...
public:
  WordListExtractorNode(const std::string &name,
...
NodeRef createWordListExtractorNode(const std::string &name,
                                    const Core::Configuration &config) {
...
// -------------------------------------------------------------------------
EditDistance::EditDistance(Fsa::ConstAlphabetRef alphabet,
                           const Bliss::TokenAlphabet *toks)
...
EditDistance::Cost EditDistance::operator()(Fsa::LabelId label1,
                                            Fsa::LabelId label2) const {
  if (label1 == label2)
    return 0;
  ...
  for (Cost l = 0; l <= l1; ++l, ++d)
    *d = l;
  for (const Bliss::Symbol::Char *c2 = tok2->symbol().str(); *c2 != '\0';
       ++c2) {
    d = D_;
    Cost p = *d + 1;
    ++d;
...
ConstEditDistanceRef EditDistance::create(Lexicon::AlphabetId alphabetId) {
  ...
  case Lexicon::LemmaAlphabetId:
    return ConstEditDistanceRef(
        new EditDistance(alphabet, Lexicon::us()->lemmaAlphabet().get()));
  ...
  case Lexicon::SyntacticTokenAlphabetId:
    return ConstEditDistanceRef(new EditDistance(
        alphabet, Lexicon::us()->syntacticTokenAlphabet().get()));
  case Lexicon::EvaluationTokenAlphabetId:
    return ConstEditDistanceRef(new EditDistance(
        alphabet, Lexicon::us()->evaluationTokenAlphabet().get()));
  case Lexicon::PhonemeAlphabetId:
    return ConstEditDistanceRef(new EditDistance(
        alphabet, Lexicon::us()->phonemeInventory()->phonemeAlphabet().get()));
  default:
    return ConstEditDistanceRef();
  }
}
