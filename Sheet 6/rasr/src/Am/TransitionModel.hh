...
public:
  enum StateType { entryM1, entryM2, silence, phone0, phone1, nStateTypes };
  class Applicator;
...
public:
  ...
  const StateTransitionModel *operator[](int i) const {
    require_(0 <= i && i < (int)transitionModels_.size() &&
             transitionModels_[i] != 0);
  ...
  virtual StateType classifyIndex(AllophoneStateIndex e,
                                  s8 subState = 0) const = 0;

  // factory
  static TransitionModel *createTransitionModel(const Core::Configuration &,
...
public:
  NonWordAwareTransitionModel(const Core::Configuration &c,
  ...
  virtual StateType classify(AllophoneState phone, s8 subState = 0) const {
    return (StateType)(TransitionModel::phone0 +
                       2 * static_cast<u32>(isNonWord(phone)) + subState);
  }
  virtual StateType classifyIndex(AllophoneStateIndex e,
                                  s8 subState = 0) const {
    return (StateType)(TransitionModel::phone0 +
                       2 * static_cast<u32>(isNonWord(e)) + subState);
  }
...
public:
  ...
  virtual StateType classify(AllophoneState phone, s8 subState = 0) const {
    return (StateType)(stateTying_->classify(phone) * nSubStates_ +
                       TransitionModel::silence + subState);
  }
  virtual StateType classifyIndex(AllophoneStateIndex e,
                                  s8 subState = 0) const {
    return (StateType)(stateTying_->classifyIndex(e) * nSubStates_ +
                       TransitionModel::silence + subState);
  }
};
...
public:
  ...
  Fsa::ConstAutomatonRef apply(Fsa::ConstAutomatonRef in,
                               Fsa::LabelId silenceLabel,
                               bool applyExitTransitionToFinalStates) const {
    return transitionModel_->apply(in, silenceLabel,
                                   applyExitTransitionToFinalStates);
  }
  TransitionModel::StateType classify(AllophoneState phone,
                                      s8 subState = 0) const {
    return transitionModel_->classify(phone, subState);
  }
  TransitionModel::StateType classifyIndex(AllophoneStateIndex e,
                                           s8 subState = 0) const {
