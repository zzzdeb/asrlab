...
const Core::ParameterStringVector
    TimeAlignmentBasedMetricLatticeBuilder::paramShortPausesLemmata(
...
Core::Choice TimeAlignmentBasedMetricLatticeBuilder::choiceTokenType(
    "none", noneType, "lemma-pronunciation", lemmaPronunciationType, "lemma",
    lemmaType, "phone", phoneType, "state", stateType, Core::Choice::endMark());

Core::ParameterChoice TimeAlignmentBasedMetricLatticeBuilder::paramTokenType(
    "token-type", &choiceTokenType,
    "specify the tokens for the calculation of the metric", noneType);
...
TimeAlignmentBasedMetricLatticeBuilder::TimeAlignmentBasedMetricLatticeBuilder(
    ...
    : Precursor(c), tokenType_(noneType) {}
...
void TimeAlignmentBasedMetricLatticeBuilder::initializeShortPauses(
  ...
  for (std::vector<std::string>::const_iterator it = shortPausesLemmata.begin();
    ...
    case phoneType: {
      if (lemma->nPronunciations() == 0) {
        error("Lemma \"") << lemma->name()
                          << "\" does not have any pronunciation.";
      }
...
OrthographyTimeAlignmentBasedMetricLatticeBuilder::createFunctor(
  ...
  setReference(Lattice::extractNumerator(
      orth, lattice, orthToLemma_, lemmaPronToLemma_, lemmaToLemmaConfusion_));
...
const Core::ParameterString
    ArchiveTimeAlignmentBasedMetricLatticeBuilder::paramName(
        "name", "name of reference fsa", Lattice::WordLattice::acousticFsa);

ArchiveTimeAlignmentBasedMetricLatticeBuilder::
    ArchiveTimeAlignmentBasedMetricLatticeBuilder(const Core::Configuration &c,
                                                  Bliss::LexiconRef lexicon)
    : Precursor(c, lexicon), name_(paramName(config)) {
...
Fsa::ConstAutomatonRef OrthographyApproximateWordAccuracyLatticeBuilder::build(
    Lattice::ConstWordLatticeRef lattice) {
  if (reference_) {
    Lattice::ConstWordLatticeRef result = Lattice::getApproximateWordAccuracy(
        lattice, reference_, shortPauses_, tokenType_ == lemmaType);
    return result->mainPart();
  } else {
    warning("Approximate word accuracies cannot be calculated because "
            "reference is empty.");
...
Fsa::ConstAutomatonRef OrthographyApproximatePhoneAccuracyLatticeBuilder::build(
    Lattice::ConstWordLatticeRef lattice) {
  if (reference_) {
    Lattice::ConstWordLatticeRef result = Lattice::getApproximatePhoneAccuracy(
        lattice, reference_, shortPauses_, alignmentGenerator_);
    return result->mainPart();
  } else {
    warning("Approximate phone accuracies cannot be calculated because "
            "reference is empty.");
    return Fsa::ConstAutomatonRef();
  }
}
...
ApproximatePhoneAccuracyLatticeBuilder::createFunctor(
    const std::string &id, Lattice::ConstWordLatticeRef reference,
    Lattice::ConstWordLatticeRef lattice,
