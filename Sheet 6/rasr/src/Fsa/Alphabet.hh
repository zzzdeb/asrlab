...
public:
  ...
  public:
     ...
     */
    std::string operator*() { return alphabet_->symbol(label_); }

    /**
     * Retrieve the label id of the current symbol.
     */
    operator LabelId() { return label_; }
    operator LabelId() const { return label_; }
  };
...
private:
  struct Symbol {
    ...
    //  this is due to the binary format which only holds 8 bits for a
    //  disambiguator
    AlphabetTag tag_;
    Symbol() {}
...
public:
   ...
   **/
  // bool addIndexedSymbol(const std::string &symbol, LabelId index, bool
  // isDisambiguator = false);
  bool addIndexedSymbol(const std::string &symbol, LabelId index,
                        AlphabetTag tag = 0);
