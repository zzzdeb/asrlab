...
public:
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    for (typename _State::const_iterator a = _sp->begin(); a != _sp->end();
      ...
      if (a->target() == s)
        continue;
      _Weight w = Precursor::fsa_->semiring()->extend(a->weight_,
                                                      potentials_[a->target()]);
...
typename _Automaton::Weight bestscore(typename _Automaton::ConstRef f) {
  ...
  if (f->initialStateId() != Fsa::InvalidStateId &&
    ...
    StatePotentials<typename _Automaton::Weight> backward =
        sssp<_Automaton>(transposed);
...
best(typename _Automaton::ConstRef f,
     const StatePotentials<typename _Automaton::Weight> &backward) {
  return
      typename _Automaton::ConstRef(new BestAutomaton<_Automaton>(f, backward));
}
...
public:
  Trace(Fsa::LabelId input = Fsa::InvalidLabelId,
        Fsa::LabelId output = Fsa::InvalidLabelId, u32 predecessor = 0)
...
// n best traces
template <class _Automaton> class NBestTraces {
...
private:
  ...
  struct PriorityFunction {
    ...
    PriorityFunction(_ConstSemiringRef semiring,
                     const _StatePotentials &potentials)
        : semiring_(semiring), potentials_(potentials) {}
    bool operator()(const Hyp &h1, const Hyp &h2) const {
      return (semiring_->compare(
                  semiring_->extend(h1.potential_, potentials_[h1.state_]),
                  semiring_->extend(h2.potential_, potentials_[h2.state_])) <
              0);
    }
  };
...
public:
  NBestTraces(_ConstAutomatonRef f, size_t n, bool bestSequences)
      : traces_(n), finals_() {
    // check for tropical semiring (or for at least a semiring with property
    // 'ordered')
    if (bestSequences) {
    ...
    _ConstAutomatonRef f2 =
        transpose<_Automaton>(f); // also generates a single final state
    _StatePotentials potentials(sssp<_Automaton>(f, false));
    _ConstSemiringRef semiring = f2->semiring();
    Core::PriorityQueue<Hyp, PriorityFunction> S(
        PriorityFunction(semiring, potentials));
    Core::Vector<size_t> r_;

    finals_.reserve(n + 1);
    finals_.push_back(semiring->one());
    S.insert(
        Hyp(f2->initialStateId(), semiring->one(), traces_.newTrace(Trace())));
    while (!S.empty()) {
      Hyp h = S.top();
      S.pop();
...
public:
  NBestAutomaton(_ConstAutomatonRef f, size_t n, bool bestSequences)
      : Precursor(f), nBestTraces_(f, n, bestSequences) {
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (s == 0) {
      ...
      if (trace.predecessor_) {
        sp = new _State(s);
        sp->newArc(trace.predecessor_, Precursor::fsa_->semiring()->one(),
                   trace.input_, trace.output_);
      } else
        sp = new _State(s, Fsa::StateTagFinal,
                        Precursor::fsa_->semiring()->one());
    }
    return _ConstStateRef(sp);
  }
...
template <class _Automaton>
typename _Automaton::ConstRef nbest(typename _Automaton::ConstRef f, size_t n,
                                    bool bestSequences) {
  return typename _Automaton::ConstRef(
      new NBestAutomaton<_Automaton>(f, n, bestSequences));
}
...
private:
  ...
  public:
    ...
    TracePool() { push_back(Trace(0, Fsa::InvalidLabelId, 0)); }
  };
  ...
  TraceId search(TracePool &traces) {
    ...
    Core::TracedPriorityQueue<Hyp, Fsa::StateId, typename Hyp::Key,
                              typename Hyp::Priority>
        stack(precedes);
    ...
    while (!stack.empty()) {
      ...
      for (typename _State::const_iterator aa = state->begin();
           aa != state->end(); ++aa) {
        newHyp.state = aa->target();
        newHyp.weight = this->semiring()->extend(current.weight, aa->weight());
        if (closed.find(newHyp.state) != closed.end()) {
          //			std::cerr << __LINE__ << "\t" << newHyp.state << "\t" <<
          //f32(newHyp.weight) << "\t" << f32(closed[newHyp.state]) <<
          //std::endl; // DEBUG
          //                      Assertion is true, but dangerous due to
          //                      limited accuracy.
          //			verify(semiring()->compare(newHyp.weight,
          //closed[newHyp.state]) >= 0);
        } else {
          newHyp.trace = traces.newTrace(current.trace, current.state,
                                         aa - state->begin());
      ...
      if (state->isFinal()) {
        ...
        newHyp.weight =
            this->semiring()->extend(current.weight, state->weight_);
        if (closed.find(newHyp.state) != closed.end()) {
          //			std::cerr << __LINE__ << "\t" << newHyp.state << "\t" <<
          //f32(newHyp.weight) << "\t" << f32(closed[newHyp.state]) <<
          //std::endl; // DEBUG
...
public:
  FirstBestAutomaton(_ConstAutomatonRef ff) : Precursor(ff) {
    this->setProperties(Fsa::PropertyStorage | Fsa::PropertyCached,
                        Fsa::PropertyNone);
    this->addProperties(Fsa::PropertySorted);
    this->addProperties(Fsa::PropertyLinear | Fsa::PropertyAcyclic);
  ...
  virtual _ConstStateRef getState(Fsa::StateId si) const {
    ...
    _State *state =
        new _State(origState->id(), origState->tags(), origState->weight_);
    if (arc >= 0) {
