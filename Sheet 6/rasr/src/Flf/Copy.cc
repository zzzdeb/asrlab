...
// -------------------------------------------------------------------------
namespace {
void copyStub(ConstLatticeRef l, StaticLattice *staticLattice,
              StaticBoundaries *staticBoundaries, const std::string &desc) {
  staticLattice->clear();
...
struct CopyStateByValue {
  ...
  State *operator()(ConstStateRef sr) const { return new State(*sr); }
  std::string describe(ConstLatticeRef l) const {
...
template <class CopyState> class CopyBuilder : public TraverseState {
...
public:
  CopyBuilder(ConstLatticeRef l, StaticLattice *staticLattice,
              StaticBoundaries *staticBoundaries, const CopyState &copyState)
      : Precursor(l), lattice_(l), boundaries_(l->getBoundaries()),
        staticLattice_(staticLattice), staticBoundaries_(staticBoundaries),
        copyState_(copyState) {
    copyStub(lattice_, staticLattice_, staticBoundaries_,
             copyState.describe(l));
...
std::pair<bool, bool> copy_(ConstLatticeRef l, StaticLattice *staticLattice,
  ...
  CopyBuilder<CopyState> buildCopy(l, staticLattice, staticBoundaries,
                                   copyState);
  return staticBoundaries ? std::make_pair(true, true)
                          : std::make_pair(true, false);
}

template <class CopyState> ConstLatticeRef copy_(ConstLatticeRef l) {
  if (!l || (l->initialStateId() == Fsa::InvalidStateId))
  ...
  StaticBoundaries *staticBoundaries =
      l->getBoundaries()->valid() ? new StaticBoundaries : 0;
...
// -------------------------------------------------------------------------
std::pair<bool, bool> persistent(ConstLatticeRef l,
                                 StaticLattice *staticLattice,
                                 StaticBoundaries *staticBoundaries) {
  return copy_<CopyStateByReference>(l, staticLattice, staticBoundaries);
}

std::pair<bool, bool> copy(ConstLatticeRef l, StaticLattice *staticLattice,
                           StaticBoundaries *staticBoundaries) {
  return copy_<CopyStateByValue>(l, staticLattice, staticBoundaries);
}

std::pair<bool, bool> deepCopy(ConstLatticeRef l, StaticLattice *staticLattice,
                               StaticBoundaries *staticBoundaries) {
...
ConstLatticeRef copy(ConstLatticeRef l) { return copy_<CopyStateByValue>(l); }
...
public:
  CachingMapBuilder(ConstLatticeRef l) : Precursor(l) { traverse(); }
...
template <class CopyState>
std::pair<bool, bool> normalizeCopy_(ConstLatticeRef l,
                                     StaticLattice *staticLattice,
                                     StaticBoundaries *staticBoundaries) {
  require(staticLattice);
  if (!l || (l->initialStateId() == Fsa::InvalidStateId))
  ...
  copyStub(l, staticLattice, staticBoundaries,
           "normalized-" + copyState.describe(l));
  staticLattice->setInitialStateId(mapping[l->initialStateId()]);
  for (ConstStateRefList::reverse_iterator itSr =
           stateMapping.states().rbegin();
...
template <class CopyState> ConstLatticeRef normalizeCopy_(ConstLatticeRef l) {
  if (!l || (l->initialStateId() == Fsa::InvalidStateId))
    return ConstLatticeRef();
  StaticLattice *staticLattice = new StaticLattice;
  StaticBoundaries *staticBoundaries =
...
// -------------------------------------------------------------------------
std::pair<bool, bool> normalizeCopy(ConstLatticeRef l,
                                    StaticLattice *staticLattice,
                                    StaticBoundaries *staticBoundaries) {
  return normalizeCopy_<CopyStateByValue>(l, staticLattice, staticBoundaries);
}

std::pair<bool, bool> normalizeDeepCopy(ConstLatticeRef l,
                                        StaticLattice *staticLattice,
                                        StaticBoundaries *staticBoundaries) {
  return normalizeCopy_<DeepCopyStateByValue>(l, staticLattice,
                                              staticBoundaries);
}
...
public:
  BoundariesCopyBuilder(ConstLatticeRef l, StaticBoundaries *staticBoundaries)
      : Precursor(l), boundaries_(l->getBoundaries()),
...
protected:
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    ...
    if (trim_ && normalize_) {
      ...
      l = deepCopy_ ? normalizeDeepCopy(trimmedLattice)
                    : normalizeCopy(trimmedLattice);
    } else if (trim_) {
      StaticLatticeRef trimmedLattice = StaticLatticeRef(new StaticLattice);
      if (deepCopy_)
...
const Core::ParameterBool
    CopyNode::paramDeepCopy("deep", "make a deep copy, i.e. copy weights",
                            false);
const Core::ParameterBool CopyNode::paramTrim("trim", "trim lattice", false);
const Core::ParameterBool CopyNode::paramNormalize("normalize",
                                                   "normalize lattice", false);
NodeRef createCopyNode(const std::string &name,
