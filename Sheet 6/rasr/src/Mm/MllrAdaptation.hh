...
struct IdSetEntryLess : public std::binary_function<const IdSetEntry &,
                                                    const IdSetEntry &, bool> {
...
public:
  ...
  virtual void
  adaptMixtureSet(Core::Ref<Mm::MixtureSet> adaptableMixtureSet) const = 0;
  // virtual void adaptMixtureSet(Core::Ref<Mm::MixtureSet> adaptableMixtureSet,
  // Core::Ref<Mm::Adaptor>) const = 0;
...
public:
  ...
  virtual void
  adaptMixtureSet(Core::Ref<Mm::MixtureSet> adaptableMixtureSet) const;
  void adaptMixtureSet(Core::Ref<Mm::MixtureSet> adaptableMixtureSet,
 ...
 */
class AdaptorEstimator : public AbstractAdaptationAccumulator,
...
protected:
  ...
  template <class T2>
  void propagate(const Math::Vector<T2> &leafData, Math::Vector<T2> &nodeData,
                 NodeId id);
...
public:
  ...
  virtual void accumulate(Core::Ref<const Feature::Vector>, DensityIndex,
                          MixtureIndex, Core::Ref<MixtureSet>) = 0;
  virtual void accumulate(Core::Ref<const Feature::Vector>, DensityIndex,
                          MixtureIndex, Core::Ref<MixtureSet>, Mm::Weight) = 0;
  virtual void accumulate(const Speech::Alignment &,
...
private:
  ...
  std::map<NodeId, Matrix>
      shift_; // not a vector since Id:s need not be successive
...
public:
  ...
  virtual void accumulate(Core::Ref<const Feature::Vector> feature,
                          DensityIndex density, MixtureIndex mixture,
                          Core::Ref<MixtureSet> mixtureSet);
  virtual void accumulate(Core::Ref<const Feature::Vector> feature,
                          DensityIndex density, MixtureIndex mixture,
                          Core::Ref<MixtureSet> mixtureSet, Mm::Weight weight);
