  ...
  void nextStep(StateId s) const {
    ...
    if (maximumSize_ && (steps_.size() + 1 >= maximumSize_)) {
      ...
      if (samplingWeight_ != 0.0) {
        ...
        for (State::const_iterator aa = origState->begin();
             aa != origState->end(); ++aa)
        ...
        step.arc =
            std::upper_bound(probs.begin(), probs.end(), rand) - probs.begin();
...
public:
  RandomPathAutomaton(ConstAutomatonRef f)
      : SlaveAutomaton(f), samplingWeight_(0.0), maximumSize_(0) {
  ...
  virtual ConstStateRef getState(StateId s) const {
    ...
    if (step.arc >= 0) {
      ...
      state->newArc(s + 1, origArc->weight(), origArc->input(),
                    origArc->output());
      if (s + 1 >= steps_.size())
...
ConstAutomatonRef random(ConstAutomatonRef f, f32 weight, u32 maximumSize) {
