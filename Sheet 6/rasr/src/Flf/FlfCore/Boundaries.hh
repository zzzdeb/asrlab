  ...
  struct Transit {
    ...
    */
    ...
    Transit(Bliss::Phoneme::Id final = Bliss::Phoneme::term,
        ...
        : final(final), initial(initial), boundary(boundary) {}

    bool operator==(const Transit &t) const {
      return (final == t.final) && (initial == t.initial) &&
             (boundary == t.boundary);
    }
    bool operator!=(const Transit &t) const {
      return (final != t.final) || (initial != t.initial) ||
...
public:
  ...
  Boundary(Speech::TimeframeIndex time) : time_(time), transit_() {}
  ...
  struct Hash {
    size_t operator()(const Boundary &b) const {
      return size_t(b.time()) ^
             ((size_t(b.transit().final) << 25) |
              (size_t(b.transit().final) >> 7)) ^
             ((size_t(b.transit().initial) << 18) |
              (size_t(b.transit().initial) >> 14)) ^
             ((size_t(b.transit().boundary) << 11) |
              (size_t(b.transit().boundary) >> 11));
    }
  };
  struct Equal {
...
const Boundary InvalidBoundary(Speech::InvalidTimeframeIndex,
                               Boundary::Transit());
...
public:
  ...
  virtual bool valid() const { return boundaries_->valid(); }
...
class StaticBoundaries : public Core::Vector<Boundary>, public Boundaries {
...
public:
  ...
  bool operator==(const StaticBoundaries &boundaries) const {
    return (size() != boundaries.size())
               ? false
               : std::equal(begin(), end(), boundaries.begin());
  }
  virtual bool valid() const { return true; }
  virtual bool valid(Fsa::StateId sid) const { return get(sid).valid(); }
  virtual const Boundary &get(Fsa::StateId sid) const {
