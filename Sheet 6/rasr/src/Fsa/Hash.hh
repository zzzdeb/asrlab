...
template <class T> struct IdentityKey {
  size_t operator()(T t) const { return t; }
};

template <class T, class HashKey = IdentityKey<T>,
          class HashEqual = std::equal_to<T> >
class Hash {
public:
  typedef u32 Cursor;
...
public:
  Hash(u32 defaultSize = 10) { bins_.resize(defaultSize, InvalidCursor); }
  ...
  void resize(u32 size) {
    ...
    size = bins_.size();
    for (typename std::vector<Element>::iterator i = elements_.begin();
         i != elements_.end(); ++i) {
  ...
  Cursor insertWithoutResize(const T &d) {
    ...
    for (; (i != InvalidCursor) && (!hashEqual_(elements_[i].data_, d));
         i = elements_[i].next_)
      ;
    if (i == InvalidCursor) {
      i = elements_.size();
...
const typename Hash<T, HashKey, HashEqual>::Cursor
    Hash<T, HashKey, HashEqual>::InvalidCursor =
