...
Adaptor::Adaptor(const Core::Configuration &c)
    : Core::Component(c), clusterId_("Dummy") {}
...
ShiftAdaptor::ShiftAdaptor(const Core::Configuration &c) : Precursor(c) {}

void ShiftAdaptor::setMatrix(std::map<NodeId, Matrix> &m,
                             std::vector<NodeId> &mi) {
  adaptationMatrices_ = m;
  adaptationMatrixIndex_ = mi;
}

void ShiftAdaptor::adaptMixtureSet(
    Core::Ref<Mm::MixtureSet> adaptableMixtureSet) const {
  // Adapt references
  log("adapting references, using ")
      << adaptationMatrices_.size() << " shifts.";
  for (MixtureIndex mix = 0; mix < adaptableMixtureSet->nMixtures(); ++mix) {
    MatrixConstIterator w =
        adaptationMatrices_.find(adaptationMatrixIndex_[mix]);
    ensure(w != adaptationMatrices_.end());
    for (size_t dns = 0; dns < adaptableMixtureSet->mixture(mix)->nDensities();
         ++dns) {

      // Extract the mean vector
      DensityIndex densityIndex =
          adaptableMixtureSet->mixture(mix)->densityIndex(dns);
      MeanIndex meanIndex =
          adaptableMixtureSet->density(densityIndex)->meanIndex();
      Mm::Mean mean = *(adaptableMixtureSet->mean(meanIndex));

      // Shift
      Math::Vector<Mm::MeanType> adaptedMean(w->second[0] +
                                             Math::Vector<Mm::MeanType>(mean));
...
void ShiftAdaptor::adaptMixtureSet(
    Core::Ref<Mm::MixtureSet> adaptableMixtureSet,
    Core::Ref<Mm::ShiftAdaptor> preAdaptor) const {
  if (adaptationMatrices_.size() > 0 ||
      preAdaptor->adaptationMatrices().size() > 0) {
    // Adapt references
    log("adapting references, using ")
        << adaptationMatrices_.size() << " shifts.";
    for (MixtureIndex mix = 0; mix < adaptableMixtureSet->nMixtures(); ++mix) {
      ...
      for (size_t dns = 0;
           dns < adaptableMixtureSet->mixture(mix)->nDensities(); ++dns) {
...
FullAdaptor::FullAdaptor(const Core::Configuration &c) : Precursor(c) {}

void FullAdaptor::setMatrix(std::map<NodeId, Matrix> &m,
                            std::vector<NodeId> &mi) {
  adaptationMatrices_ = m;
  adaptationMatrixIndex_ = mi;
}

void FullAdaptor::adaptMixtureSet(
    Core::Ref<Mm::MixtureSet> adaptableMixtureSet) const {
  ...
  // adapt references
  log("adapting references, using ")
      << adaptationMatrices_.size() << " matrices";
  for (MixtureIndex mix = 0; mix < adaptableMixtureSet->nMixtures(); ++mix) {
    MatrixConstIterator w =
        adaptationMatrices_.find(adaptationMatrixIndex_[mix]);
    ensure(w != adaptationMatrices_.end());
    for (size_t dns = 0; dns < adaptableMixtureSet->mixture(mix)->nDensities();
      ...
      Math::Vector<Mm::MeanType> adaptedMean(
          w->second * Math::Vector<Mm::MeanType>(extendedMean));
...
const Core::ParameterFloat AdaptorEstimator::paramMinAdaptationObservations(
    "min-observation", "minimum number of observations for MLLR estimation",
    1000, 1);

const Core::ParameterFloat AdaptorEstimator::paramMinSilenceObservations(
    "min-silence-observation",
    "minimum number of silence observations for MLLR estimation", 500, 1);
...
AdaptorEstimator::AdaptorEstimator(
    ...
    : Core::Component(c), dimension_(mixtureSet->dimension()),
      leafIndex_(adaptationTree->leafIndex()),
      nLeafs_(adaptationTree->nLeafs()), tree_(adaptationTree->tree()),
  ...
  if ((*leafIndex_).size() != (mixtureSet->nMixtures())) {
    criticalError(
        "size of adaptation class array does not match size of mixture set");
  }
}

AdaptorEstimator::AdaptorEstimator(
    const Core::Configuration &c, ComponentIndex dim,
    const Core::Ref<Am::AdaptationTree> adaptationTree)
    : Core::Component(c), dimension_(dim),
      leafIndex_(adaptationTree->leafIndex()),
      nLeafs_(adaptationTree->nLeafs()), tree_(adaptationTree->tree()),
      silenceMixtures_(adaptationTree->silenceMixtures()),
      adaptationDumpChannel_(config, "dump-adaptation") {
...
AdaptorEstimator::AdaptorEstimator(
    const Core::Configuration &c,
    const Core::Ref<Am::AdaptationTree> adaptationTree)
    : Core::Component(c), leafIndex_(adaptationTree->leafIndex()),
      nLeafs_(adaptationTree->nLeafs()), tree_(adaptationTree->tree()),
      silenceMixtures_(adaptationTree->silenceMixtures()),
      adaptationDumpChannel_(config, "dump-adaptation") {
  //    minAdaptationObservations_=paramMinAdaptationObservations(config);
...
void AdaptorEstimator::propagate(const Math::Vector<T2> &leafData,
  ...
  if (nodeData.size() !=
      size_t(tree_->numberOfNodes() + tree_->numberOfLeafs())) {
...
bool AdaptorEstimator::write(Core::BinaryOutputStream &o) const {
  o << clusterId_ << dimension_ << count_ << minAdaptationObservations_
    << minSilenceObservations_;
  return o.good();
}

bool AdaptorEstimator::read(Core::BinaryInputStream &i) {
  i >> clusterId_ >> dimension_ >> count_ >> minAdaptationObservations_ >>
      minSilenceObservations_;

  // Override values if present in configuration.
  minAdaptationObservations_ =
      paramMinAdaptationObservations(config, minAdaptationObservations_);
  minSilenceObservations_ =
      paramMinSilenceObservations(config, minSilenceObservations_);
...
void AccumulatorBase::reset() { matrix_.fill(0.0); }
...
Core::BinaryOutputStream &Mm::operator<<(Core::BinaryOutputStream &o,
                                         const AccumulatorBase &a) {
  a.write(o);
  return o;
}

Core::BinaryInputStream &Mm::operator>>(Core::BinaryInputStream &i,
                                        AccumulatorBase &a) {
  a.read(i);
  return i;
}
...
ShiftAdaptorViterbiEstimator::ShiftAdaptorViterbiEstimator(
    const Core::Configuration &c, Core::Ref<const Mm::MixtureSet> m,
    const Core::Ref<Am::AdaptationTree> adaptationTree)
...
void ShiftAdaptorViterbiEstimator::accumulate(
    Core::Ref<const Feature::Vector> feature, DensityIndex density,
    MixtureIndex mixture, Core::Ref<MixtureSet> mixtureSet) {
...
void ShiftAdaptorViterbiEstimator::accumulate(
    Core::Ref<const Feature::Vector> feature, DensityIndex density,
    MixtureIndex mixture, Core::Ref<MixtureSet> mixtureSet, Mm::Weight weight) {
  MeanIndex meanIndex = mixtureSet->density(density)->meanIndex();
  const Mm::Mean &mean = *(mixtureSet->mean(meanIndex));
...
void ShiftAdaptorViterbiEstimator::accumulate(
  ...
  for (size_t item = 0; item < alignment.size(); ++item) {
    if (alignment[item].weight < 0.5)
      continue;
    MixtureIndex mixture = unadaptedAdaptor->acousticModel()->emissionIndex(
        alignment[item].emission);
    size_t density =
        unadaptedFeatureScorer->getAssigningScorer(featureSequence[item])
            ->bestDensity(mixture);
    ...
    MeanIndex meanIndex =
        unadaptedAdaptor->mixtureSet()->density(densityIndex)->meanIndex();
    const Mm::Mean &mean = *(unadaptedAdaptor->mixtureSet()->mean(meanIndex));
    CovarianceIndex covIndex = unadaptedAdaptor->mixtureSet()
                                   ->density(densityIndex)
                                   ->covarianceIndex();
    const Mm::Covariance &cov =
        *(unadaptedAdaptor->mixtureSet()->covariance(covIndex));

    countAccumulators_[(*leafIndex_)[mixture]] += 1.0;
    for (u32 d = 0; d < dimension_; d++) {
      betaAccumulators_[(*leafIndex_)[mixture]][d] += 1.0 / cov.diagonal()[d];
      shiftAccumulators_[(*leafIndex_)[mixture]][d] +=
          ((*featureSequence[item]->mainStream())[d] - mean[d]) /
          cov.diagonal()[d];
    }
  }
}
...
Core::Ref<Adaptor> ShiftAdaptorViterbiEstimator::adaptor(void) {
  ...
  if (count_[id] <= minAdaptationObservations_) {
    ...
    log("too few observations for adaptation\n")
        << minAdaptationObservations_ << " observations needed, " << count_[id]
        << " seen.\n"
    ...
    for (Core::BinaryTree::LeafList::const_iterator p =
             tree_->leafList().begin();
    ...
    for (Core::BinaryTree::LeafList::const_iterator p =
      ...
      if (adaptationMatrices.find(id) == adaptationMatrices.end()) {
        MatrixConstIterator p = shift_.find(id);
               ...
               shift_.end()); // something has gone wrong in estimateWMatrices()
  ...
  Core::BinaryTree::Id silenceId =
      tree_->numberOfNodes() + tree_->numberOfLeafs();
    ...
    if (countAccumulators_[silenceLeaf] > minSilenceObservations_) {
      ...
      for (u32 d = 0; d < dimension_; d++)
        adaptationMatrices[silenceId][0][d] =
            shiftAccumulators_[silenceLeaf][d] /
            betaAccumulators_[silenceLeaf][d];
    } else {
  ...
  for (u32 mixture = 0; mixture < leafIndex_->size(); mixture++) {
    ...
    if (adaptationDumpChannel_.isOpen()) {
      adaptationDumpChannel_
          << Core::XmlEmpty("shift") + Core::XmlAttribute("shift-id", mixture) +
...
bool ShiftAdaptorViterbiEstimator::write(Core::BinaryOutputStream &o) const {
  if (Precursor::write(o)) {
    o << countAccumulators_ << betaAccumulators_ << shiftAccumulators_
      << shift_;
  }
  return o.good();
}
...
bool ShiftAdaptorViterbiEstimator::read(Core::BinaryInputStream &i) {
  if (Precursor::read(i)) {
    i >> countAccumulators_ >> betaAccumulators_ >> shiftAccumulators_ >>
        shift_;
  }
  return i.good();
}
...
void FullAdaptorViterbiEstimator::init() {
  leafZAccumulators_.resize(
      nLeafs_, ZAccumulator(dimension_)); // number of leafs in MLLR tree
...
void FullAdaptorViterbiEstimator::ZAccumulator::accumulate(
    const Mm::Mean &mean, const FeatureVector &vec) {
...
void FullAdaptorViterbiEstimator::ZAccumulator::accumulate(
    const Mm::Mean &mean, const FeatureVector &vec, Mm::Weight weight) {
  require(mean.size() == vec.size());
  count_++;
  for (u32 i = 0; i < matrix_.nRows(); i++) {
    for (u32 j = 0; j < matrix_.nColumns(); j++) {
...
Math::Matrix<double>
FullAdaptorViterbiEstimator::ZAccumulator::squareMatrix() const {
  Math::Matrix<double> m(matrix_);
  m.removeColumn(0);
  return m;
}

void FullAdaptorViterbiEstimator::GAccumulator::accumulate(
    const Mm::Mean &mean) {
  count_++;
  for (u32 i = 0; i < matrix_.nRows(); i++) {
    for (u32 j = 0; j < matrix_.nColumns(); j++) {
      matrix_[i][j] +=
          ((i == 0) ? 1.0 : mean[i - 1]) * ((j == 0) ? 1.0 : mean[j - 1]);
    }
  }
}
...
void FullAdaptorViterbiEstimator::GAccumulator::accumulate(const Mm::Mean &mean,
  ...
  for (u32 i = 0; i < matrix_.nRows(); i++) {
    for (u32 j = 0; j < matrix_.nColumns(); j++) {
      matrix_[i][j] += weight * ((i == 0) ? 1.0 : mean[i - 1]) *
                       ((j == 0) ? 1.0 : mean[j - 1]);
    }
  }
}
...
Math::Matrix<double>
FullAdaptorViterbiEstimator::GAccumulator::squareMatrix() const {
  Math::Matrix<double> m(matrix_);
  m.removeRow(0);
  m.removeColumn(0);
  return m;
...
Core::Ref<Adaptor> FullAdaptorViterbiEstimator::adaptor(void) {
  ...
  if (count_[id] <= minAdaptationObservations_) {
    ...
    for (Core::BinaryTree::LeafList::const_iterator p =
      ...
      if (adaptationMatrices.find(id) == adaptationMatrices.end()) {
        ...
        ensure(p != w_.end()); // something has gone wrong in
                               // estimateWMatrices()
        adaptationMatrices[id] = p->second;
      }
      matrixId[leafId] = id;
  ...
  for (std::set<MixtureIndex>::const_iterator p = silenceMixtures_.begin();
    ...
    if (leafGAccumulators_[silenceLeaf].count() > minSilenceObservations_) {
      ...
      pseudoInvert(gInverse);
      adaptationMatrices[silenceId] =
          leafZAccumulators_[silenceLeaf].matrix() * gInverse;
    } else {
      log("too few observations for silence adaptation\n")
          << minSilenceObservations_ << " observations needed, "
  ...
  for (u32 mixture = 0; mixture < leafIndex_->size(); mixture++) {
    ...
    if (adaptationDumpChannel_.isOpen()) {
      adaptationDumpChannel_
          << Core::XmlEmpty("mixture") +
                 Core::XmlAttribute("mixture-id", mixture) +
                 Core::XmlAttribute("matrix-id",
                                    adaptationMatrixIndex[mixture]);
    }
  }
  if (adaptationDumpChannel_.isOpen()) {
