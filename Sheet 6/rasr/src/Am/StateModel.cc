...
AllophoneStateAlphabet::AllophoneStateAlphabet(
    ...
    : pi_(pi), contextLength_(contextLength), nStates_(nStates),
      nDisambiguators_(0) {
  calcNClasses();
}

void AllophoneStateAlphabet::set(Core::Ref<const Bliss::PhonemeInventory> pi,
                                 u32 contextLength, u32 nStates) {
  pi_ = pi;
...
AllophoneStateAlphabet::Index
AllophoneStateAlphabet::index(const AllophoneState &phone) const {
...
AllophoneState
AllophoneStateAlphabet::allophoneState(AllophoneStateAlphabet::Index in) const {
  require(nStates_ && contextLength_);
  require(in != Fsa::Epsilon);
  require(in < nClasses_);
  AllophoneState result;
  Index code = in - 1;
  result.state = code % nStates_;
  code /= nStates_;
...
std::string AllophoneStateAlphabet::symbol(Fsa::LabelId id) const {
  ...
  std::string tmp;
  if (id > nClasses())
    return std::string("#") + Core::itoa(tmp, id - nClasses_ - 1);
  AllophoneState as = allophoneState(id);
  std::string result = as.format(pi_);
  if (as.boundary & Allophone::isInitialPhone)
    result += "@i";
  if (as.boundary & Allophone::isFinalPhone)
    result += "@f";
...
void AllophoneStateAlphabet::writeXml(Core::XmlWriter &os) const {
  os << Core::XmlOpenComment() << nClasses_ << " allophone state labels, "
     << nDisambiguators_ << " disambiguation symbols" << Core::XmlCloseComment()
     << "\n";
}
