...
private:
  ...
  void advancePronunciationSuffix(
      ...
      std::back_insert_iterator<std::vector<PronunciationSuffix> > ending)
      const;
...
public:
  int compareSequences(const PronunciationSuffix &,
                       const PronunciationSuffix &) const;
  ...
  struct StateDesc {
    ...
    bool operator<(const StateDesc &rhs) const {
      return (acousticModel < rhs.acousticModel) ||
             (acousticModel == rhs.acousticModel &&
...
public:
  ...
  StateTree(const Core::Configuration &, Bliss::LexiconRef,
  ...
  std::pair<Bliss::Phoneme::Id, Bliss::Phoneme::Id>
      describeRootState(StateId) const;
  ...
  public:
    ...
    operator bool() const { return current_ >= 0; }

    StateId operator*() const { return current_; }
  ...
  public:
    ...
    bool operator==(const SuccessorIterator &rhs) const { return s_ == rhs.s_; }
    bool operator<(const SuccessorIterator &rhs) const { return s_ < rhs.s_; }
    ...
    operator StateId() const { return s_; }
    StateId operator*() const { return s_; }
    u32 operator-(const SuccessorIterator &rhs) const { return s_ - rhs.s_; }
  };
  ...
  std::pair<const_ExitIterator, const_ExitIterator> wordEnds(StateId s) const {
    require_(0 <= s && s < nStates());
    return std::make_pair(states_[s].exits.begin(), states_[s].exits.end());
  }
