...
AllophoneStateGraphBuilder::AllophoneStateGraphBuilder(
    ...
    Core::Ref<const Am::AcousticModel> acousticModel, bool flatModelAcceptor)
    : Precursor(c), lexicon_(lexicon), acousticModel_(acousticModel),
      orthographicParser_(0), modelChannel_(config, "model-automaton"),
...
void AllophoneStateGraphBuilder::addSilenceOrNoise(
...
void AllophoneStateGraphBuilder::setSilencesAndNoises(
    const std::vector<std::string> &silencesAndNoises) {
...
Bliss::OrthographicParser &AllophoneStateGraphBuilder::orthographicParser() {
  if (!orthographicParser_)
    orthographicParser_ =
        new Bliss::OrthographicParser(select("orthographic-parser"), lexicon_);
  return *orthographicParser_;
}

Fsa::ConstAutomatonRef
AllophoneStateGraphBuilder::lemmaPronunciationToLemmaTransducer() {
  if (!lemmaPronunciationToLemmaTransducer_) {
    ...
    lemmaPronunciationToLemmaTransducer_ =
        Fsa::ConstAutomatonRef(Fsa::staticCompactCopy(Fsa::sort(
            lemmaPronunciationToLemmaTransducer_, Fsa::SortTypeByOutput)));

    Fsa::info(lemmaPronunciationToLemmaTransducer_,
              log("lemma-pronuncation-to-lemma transducer"));
  }
  return lemmaPronunciationToLemmaTransducer_;
}

Fsa::ConstAutomatonRef
AllophoneStateGraphBuilder::phonemeToLemmaPronunciationTransducer() {
  if (!phonemeToLemmaPronunciationTransducer_) {
    phonemeToLemmaPronunciationTransducer_ =
        lexicon_->createPhonemeToLemmaPronunciationTransducer(false);
    // sort transducer by output symbols to accelerate composition operations
    phonemeToLemmaPronunciationTransducer_ =
        Fsa::ConstAutomatonRef(Fsa::staticCompactCopy(Fsa::sort(
            phonemeToLemmaPronunciationTransducer_, Fsa::SortTypeByOutput)));
    Fsa::info(phonemeToLemmaPronunciationTransducer_,
              log("phoneme-to-lemma-pronuncation transducer"));
  }
...
AllophoneStateGraphBuilder::allophoneStateToPhonemeTransducer() {
  if (!allophoneStateToPhonemeTransducer_) {
    ...
    Core::Ref<Am::TransducerBuilder> tb =
        acousticModel_->createTransducerBuilder();

    tb->setDisambiguators(phonemeAlphabet->nDisambiguators());
    tb->selectAllophonesFromLexicon();

    // for efficiency reasons, precompute flat allophoneStateToPhoneme
    // transducer without loop and skip transitions and apply transition model
    // (i.e. loops and skips) afterwards on the final transducer
    tb->selectFlatModel();
    ...
    allophoneStateToPhonemeTransducer_ =
        Fsa::ConstAutomatonRef(Fsa::staticCompactCopy(Fsa::sort(
            allophoneStateToPhonemeTransducer_, Fsa::SortTypeByOutput)));

    // To accelerate the application of context dependency, it would be nice, if
    // allophoneStateToPhonemeTransducer was deterministic wrt. its output
    // symbols: allophoneStateToPhonemeTransducer_ =
    //      Fsa::ConstAutomatonRef(Fsa::staticCompactCopy(Fsa::invert(Fsa::determinize(Fsa::invert(allophoneStateToPhonemeTransducer_)))));
    // Unfortunately, this is currently not possible due to ambiguities at word
    // boundaries
  }
  return allophoneStateToPhonemeTransducer_;
}

Fsa::ConstAutomatonRef AllophoneStateGraphBuilder::
    singlePronunciationAllophoneStateToPhonemeTransducer() {
...
AllophoneStateGraphRef
AllophoneStateGraphBuilder::build(const std::string &orth) {
  return finalizeTransducer(buildTransducer(orth));
}

AllophoneStateGraphRef
AllophoneStateGraphBuilder::build(const InputLevel &level) {
  if (level == lemma) {
...
Fsa::ConstAutomatonRef
AllophoneStateGraphBuilder::buildTransducer(const std::string &orth) {
  return buildTransducer(orthographicParser().createLemmaAcceptor(orth));
}

AllophoneStateGraphRef
AllophoneStateGraphBuilder::build(Fsa::ConstAutomatonRef lemmaAcceptor) {
...
Fsa::ConstAutomatonRef AllophoneStateGraphBuilder::buildTransducer(
  ...
  // remove silence and phrases by choosing the shortest path for a flat model
  // acceptor we trim in order to check for not empty but incomplete graphs
  // without final states
  Fsa::ConstAutomatonRef lemmaPronunciationAcceptor =
      Fsa::projectOutput(Fsa::trim(Fsa::composeMatching(
          flatModelAcceptor_
              ? Fsa::best(Fsa::extend(lemmaAcceptor, Fsa::Weight(1.0)))
              : Fsa::ConstAutomatonRef(lemmaAcceptor),
  ...
  if (modelChannel_.isOpen()) {
    ...
    Fsa::write(lemmaPronunciationAcceptor,
               "bin:/tmp/lemma-pronunciation.binfsa.gz");
  }

  if (lemmaPronunciationAcceptor->initialStateId() == Fsa::InvalidStateId)
    criticalError("lemma-pronuncation graph is empty. Probably the current "
                  "sentence contains a word that has no pronunciation.");

  Fsa::ConstAutomatonRef phon = Fsa::trim(Fsa::composeMatching(
      phonemeToLemmaPronunciationTransducer(), lemmaPronunciationAcceptor));
  if (modelChannel_.isOpen()) {
  ...
  if (phon->initialStateId() == Fsa::InvalidStateId)
    criticalError("phoneme graph is empty.  Probably the current sentence "
                  "contains a word that has no pronunciation.");
...
AllophoneStateGraphRef AllophoneStateGraphBuilder::finalizeTransducer(
  ...
  AllophoneStateGraphRef modelAcceptor =
      Fsa::removeEpsilons(Fsa::removeDisambiguationSymbols(
...
AllophoneStateGraphRef AllophoneStateGraphBuilder::build(
  ...
  if (flatModelAcceptor_)
    ...
    tb->selectTransitionModel();
  tb->selectAllophoneStatesAsInput();
  tb->setSilencesAndNoises(silencesAndNoises_.empty() ? 0
                                                      : &silencesAndNoises_);
  return finalizeTransducer(tb->createPronunciationTransducer(pronunciation));
}

AllophoneStateGraphRef
AllophoneStateGraphBuilder::build(const Alignment &alignment) {
  return build(alignment,
               singlePronunciationAllophoneStateToPhonemeTransducer());
}
...
AllophoneStateGraphRef
AllophoneStateGraphBuilder::build(const Alignment &alignment,
                                  AllophoneStateGraphRef allophoneStateGraph) {
  Fsa::ConstAutomatonRef model = Fsa::trim(Fsa::composeMatching(
      createAlignmentGraph(alignment), allophoneStateGraph));
  if (model->initialStateId() == Fsa::InvalidStateId)
    warning(
        "Allophone state graph generated from alignment has no final state.");
  return finalizeTransducer(model);
}

Fsa::ConstAutomatonRef
AllophoneStateGraphBuilder::createAlignmentGraph(const Alignment &alignment) {
  ...
  for (Alignment::const_iterator al = alignment.begin(); al != alignment.end();
       ++al) {
    sp = f->fastState(sid);
