 ...
 * - we always split/merge sets of substates => special representation for
 * split/merge? [memory]
 * - use specialized hash table for output strings; we waste some memory by
 * using a separate hash table [memory]
 ...
 * - use InvalidLabelId as end marker for strings => discard length field
 * [speed]
 * - store string start within substate and state [speed]
 * - compress substate sequences: gzip gives more than a factor of 4 on these
 * huge vectors [memory]
 */
template <class _Automaton> class DeterminizeAutomaton : public _Automaton {
private:
...
private:
  ...
  struct Substate {
    ...
    Substate(Fsa::StateId state, Fsa::LabelIdStrings::Id output, Weight weight)
        : predecessor_(NoPredecessor), state_(state), output_(output),
          weight_(weight), disconnect_(false) {}
    Substate(Cursor predecessor, Fsa::StateId state,
             Fsa::LabelIdStrings::Id output, Weight weight)
        : predecessor_(predecessor), state_(state), output_(output),
          weight_(weight), disconnect_(false) {}
    bool operator<(const Substate &s) const {
      return (state_ < s.state_) ||
             ((state_ == s.state_) && (output_ < s.output_));
    }
  };
  ...
  private:
    ...
    // CCC   # of bits for output string: 000 = 0, 001 = 8, 010 = 16, 011 = 24,
    // 100 = 32 DD    weight: 00 = default (one()), 01 = individual, 10 = weight
    // equal to previous substate,
    //       11 = UNUSED => ensures that 0xff marks the end of a substate
    //       sequence
  ...
  public:
    ...
    private:
      ...
      void uncompress() {
        Core::Vector<u8>::const_iterator tmp =
            container_.substates_.begin() + current_;
        u8 partition = *tmp;
        if (partition != 0xff) {
          ++tmp;
          if (partition & 0x80)
            substate_.predecessor_ =
                Fsa::getBytesAndIncrement(tmp, sizeof(Cursor));
          else
            substate_.predecessor_ = NoPredecessor;
          Fsa::StateId state =
              Fsa::getBytesAndIncrement(tmp, ((partition >> 5) & 0x03) + 1);
          substate_.state_ = state >> 1;
          substate_.disconnect_ = (state & 0x1);
          substate_.output_ =
              Fsa::getBytesAndIncrement(tmp, (partition >> 2) & 0x07);
  ...
  public:
    Substates(_ConstSemiringRef semiring)
        : nSubstates_(0), bins_(13, Core::Type<Cursor>::max),
          semiring_(semiring) {}
    ...
    Cursor addPredecessor(Cursor p1, Cursor p2) {
      ...
      if (!(p2 & Overflow))
        ...
        predecessors_[p1].insert(predecessors_[p1].end(),
                                 predecessors_[p2 & ~Overflow].begin(),
    ...
    Cursor size(u8 partition) const {
      if (partition == 0xff)
        return 0;
      return (partition & 0x80 ? sizeof(Cursor) : 0) +
             ((partition >> 5) & 0x03) + 1 + ((partition >> 2) & 0x07) +
             (partition & 0x01 ? semiring_->compressedSize() : 0);
    }
    ...
    u32 hash(Cursor pos) const {
      u32 value = 0;
      for (Core::Vector<u8>::const_iterator i =
               substates_.begin() + pos + sizeof(Cursor);
           *i != 0xff;) {
    ...
    bool equal(Cursor pos1, Cursor pos2) const {
      ...
      Core::Vector<u8>::const_iterator i1 =
          substates_.begin() + pos1 + sizeof(Cursor);
      Core::Vector<u8>::const_iterator i2 =
          substates_.begin() + pos2 + sizeof(Cursor);
      for (; *i1 != 0xff;) {
        u8 partition = *(i1++);
        if (partition != *(i2))
          return false;
    ...
    std::pair<Cursor, bool> insert(const Core::Vector<Substate> &v) {
      ...
      for (typename Core::Vector<Substate>::const_iterator s = v.begin();
           s != v.end(); ++s) {
      ...
      if (nSubstates_ > 2 * bins_.size()) {
        ...
        for (Cursor i = 0; i != start; ++i) {
          ...
          bins_[key] = i;
          for (i += sizeof(Cursor); substates_[i] != 0xff;
               i += 1 + size(substates_[i]))
            ;
        }
      }
      Fsa::setBytes(substates_.begin() + start, bins_[key % bins_.size()],
                    sizeof(Cursor));
    ...
    size_t getMemoryUsed() const {
      return bins_.getMemoryUsed() + substates_.getMemoryUsed() +
             sizeof(typename Precursor::ConstSemiringRef);
    }
    size_t getPredecessorMemoryUsed() const {
      size_t predecessors = 0;
      for (typename Core::Vector<Core::Vector<Cursor> >::const_iterator i =
               predecessors_.begin();
  ...
  // state
  struct State_ {
    typename Substates::Cursor
        subset_;                     // state label: subset of state and weights
    Fsa::LabelIdStrings::Id output_; // hash index
  ...
  struct StateHashKey_ {
    ...
    u32 operator()(const State_ &s) {
      u32 value = 0;
      for (typename Substates::const_iterator sub =
               d_.substates_.begin(s.subset_);
           sub.valid(); ++sub)
  ...
  struct StateHashEqual_ {
    ...
    bool operator()(const State_ &s1, const State_ &s2) const {
      if (s1.output_ != s2.output_)
        ...
        typename Substates::const_iterator sub1 =
            d_.substates_.begin(s1.subset_);
        typename Substates::const_iterator sub2 =
            d_.substates_.begin(s2.subset_);
  ...
  struct Arc_ {
    ...
    Fsa::LabelId start_;
    Arc_() {}
    Arc_(typename Substates::Cursor predecessor, Fsa::LabelId input,
         Fsa::StateId target, Weight weight, Fsa::LabelId start)
        : predecessor_(predecessor), input_(input), target_(target),
          weight_(weight), start_(start) {}
...
public:
  DeterminizeAutomaton(_ConstAutomatonRef f, bool disambiguate)
      : disambiguate_(disambiguate), substates_(f->semiring()),
    ...
    // if (!hasProperties(fsa_, PropertyStorage)) fsa_ = cache<_Automaton>(fsa_,
    // 10000);
  ...
  void subsetMarkSiblingsWithAmbiguousCycles() const {
    // we follow the predecessor links until we find the same original state (=
    // cycle)
    for (typename Core::Vector<Substate>::iterator sub = tmpSubstates_.begin();
         sub != tmpSubstates_.end(); ++sub)
      if ((*outputs_.begin(sub->output_) != Fsa::InvalidLabelId) ||
          (!semiring_->isDefault(sub->weight_))) {
        Fsa::Stack<typename Substate::Cursor> S;
        if (sub->predecessor_ & Substates::Overflow) {
          for (typename Substates::const_predecessor_iterator p =
                   substates_.predecessorBegin(sub->predecessor_);
        ...
        while (!S.empty()) {
          ...
          if (substates_.state(substate) == sub->state_) {
            ...
            if (substate & Substates::Overflow)
              for (typename Substates::const_predecessor_iterator p =
                       substates_.predecessorBegin(substate);
  ...
  typename Substates::Cursor subsetStop() const {
    ...
    for (typename Core::Vector<Substate>::const_iterator i =
             tmpSubstates_.begin() + 1;
         i != tmpSubstates_.end(); ++i) {
      if ((j->state_ == i->state_) && (j->output_ == i->output_)) {
        j->weight_ = semiring()->collect(j->weight_, i->weight_);
        if (disambiguate_)
          j->predecessor_ =
              substates_.addPredecessor(j->predecessor_, i->predecessor_);
      } else if (++j < i)
        *j = *i;
    }
    ...
    std::pair<typename Substates::Cursor, bool> id =
        substates_.insert(tmpSubstates_);
    if (disambiguate_) {
      if (id.second) {
        for (typename Core::Vector<Substate>::const_iterator i =
                 tmpSubstates_.begin() + 1;
  ...
  // states
  Fsa::StateId insertState(typename Substates::Cursor subset,
                           Fsa::LabelIdStrings::Id output) const {
  ...
  // string semirings
  void lcp(Core::Vector<Fsa::LabelId> &output,
           Core::Vector<Fsa::LabelId>::const_iterator start) const {
  ...
  bool isPrefix(Core::Vector<Fsa::LabelId> &output,
                Core::Vector<Fsa::LabelId>::const_iterator start) const {
    Core::Vector<Fsa::LabelId>::const_iterator o = output.begin();
    for (; (*o != Fsa::InvalidLabelId) && (*o == *start); ++o, ++start)
      ;
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    if (s < states_.size()) {
      ...
      if (*outputs_.begin(output) ==
          Fsa::InvalidLabelId) { // zero length string
        ...
        for (typename Substates::const_iterator sub =
                 substates_.begin(state->subset_);
             sub.valid(); ++sub)
          if (!sub->disconnect_) {
            // std::cout << " " << sub.cursor() << " " << sub->state_ <<
            // std::endl;
            _ConstStateRef _sp = fsa_->getState(sub->state_);
            const Fsa::LabelIdStrings::Id output = sub->output_;
            for (typename _State::const_iterator a = _sp->begin();
                 a != _sp->end(); ++a) {
              if (disambiguate_)
                predecessor = sub.cursor();
              tmpArcs_.push_back(
                  Arc_(predecessor, a->input(), a->target(),
                       semiring_->extend(sub->weight_, a->weight()),
                       tmpOutputs_.size()));
              for (Core::Vector<Fsa::LabelId>::const_iterator i =
            ...
            // subsequential final strings at final states (yet not determinized
            // on output side)
            if (_sp->isFinal() &&
                (*outputs_.begin(output) != Fsa::InvalidLabelId)) {
              _Arc *a = sp->newArc();
              ...
              subsetStart();
              subsetInsert(Substate(sub->state_, outputs_.insert(output + 1),
                                    sub->weight_));
              ...
              tags |= _sp->tags();
            if (_sp->isFinal()) {
              fw = semiring_->collect(
                  fw, semiring_->extend(sub->weight_, _sp->weight_));
            }
          } else { // disambiguate automaton => disconnect detected, ambiguous
                   // cycles
            subsetStart();
            subsetInsert(Substate(sub->state_, 0, semiring_->one()));
            Fsa::LabelIdStrings::Id id = outputs_.insert(sub->output_ + 1);
            Fsa::StateId target =
                insertState(subsetStop(),
                            *outputs_.begin(sub->output_) == Fsa::InvalidLabelId
                                ? sub->output_
                                : id);
            Fsa::LabelId label =
                *outputs_.begin(sub->output_) == Fsa::InvalidLabelId
            ...
            sp->newArc(target, sub->weight_,
                       (fsa_->type() == Fsa::TypeTransducer) ? Fsa::Epsilon
                                                             : label,
                       label);
          }
        sp->setTags(tags);
        ...
        // one iteration of the following loop creates one arc with a unique
        // input label
        Core::Vector<Fsa::LabelId> outputArc, outputTarget;
        for (typename Core::Vector<Arc_>::const_iterator a = tmpArcs_.begin();
             a != tmpArcs_.end();) {
          ...
          outputArc.resize(0);
          for (Core::Vector<Fsa::LabelId>::const_iterator i =
                   tmpOutputs_.begin() + a->start_;
               *i != Fsa::InvalidLabelId; ++i)
            outputArc.push_back(*i);
          outputArc.push_back(Fsa::InvalidLabelId);
          for (++a; (a != tmpArcs_.end()) && (a->input_ == first->input_);
               ++a) {
          ...
          for (a = first; a < last; ++a) {
            Fsa::LabelIdStrings::Id output = 0;
            Core::Vector<Fsa::LabelId>::const_iterator start =
                tmpOutputs_.begin() + a->start_;
          ...
          state = &states_[s]; // update pointer to state (might change due to
                               // resize of hash vector)
        }
      } else {
        // create new intermediate state with a single arc (also used for
        // subsequential final strings)
        sp->newArc(insertState(state->subset_, outputs_.insert(output + 1)),
                   semiring_->one(),
                   (fsa_->type() == Fsa::TypeTransducer)
                       ? Fsa::Epsilon
                       : *outputs_.begin(output),
                   *outputs_.begin(output));
      }
  ...
  void dumpOutput(Fsa::LabelIdStrings::Id output, std::ostream &o) const {
    ...
    if (*outputs_.begin(s) == Fsa::InvalidLabelId)
      ...
      for (Core::Vector<Fsa::LabelId>::const_iterator i = outputs_.begin(s);
           *i != Fsa::InvalidLabelId; ++i) {
  ...
  virtual void dumpState(Fsa::StateId s, std::ostream &o) const {
    if (s < states_.size()) {
      const State_ *state = &states_[s];
      typename Substates::const_iterator begin =
          substates_.begin(state->subset_);
  ...
  virtual size_t getMemoryUsed() const {
    ...
    return fsa_->getMemoryUsed() + sizeof(_ConstAutomatonRef) +
           2 * sizeof(Fsa::LabelId) + substates_.getMemoryUsed() +
           states_.getMemoryUsed() + outputs_.getMemoryUsed() +
           tmpSubstates_.getMemoryUsed() + tmpArcs_.getMemoryUsed() +
           tmpOutputs_.getMemoryUsed();
  }
  virtual void dumpMemoryUsage(Core::XmlWriter &o) const {
    ...
    o << Core::XmlFull("states", states_.getMemoryUsed())
      ...
      << Core::XmlFull("temporary",
                       tmpArcs_.getMemoryUsed() + tmpOutputs_.getMemoryUsed())
...
typename _Automaton::ConstRef determinize(typename _Automaton::ConstRef f,
                                          bool disambiguate) {
  require(f);
  return typename _Automaton::ConstRef(
      new DeterminizeAutomaton<_Automaton>(f, disambiguate));
}
    ...
    *sp->newArc() = *_a;
    for (++_a; _a != _sp->end(); ++_a) {
      if ((_a->input() == _pa->input()) && (_a->target() == _pa->target()))
        (sp->begin() + sp->nArcs() - 1)->weight_ = semiring_->collect(
            (sp->begin() + sp->nArcs() - 1)->weight(), _a->weight());
      else
        *sp->newArc() = *_a;
      _pa = _a;
    }
...
localDeterminize(typename _Automaton::ConstRef f) {
  require(f);
  return typename _Automaton::ConstRef(
      new LocalDeterminizeAutomaton<_Automaton>(f));
}

template <class _Automaton>
class ReplaceDisambiguationSymbolsAutomaton
...
public:
  ReplaceDisambiguationSymbolsAutomaton(_ConstAutomatonRef f, Fsa::LabelId ll,
                                        bool shouldReplaceInput,
                                        bool shouldReplaceOutput)
      : Precursor(f), shouldReplaceInput_(shouldReplaceInput),
        shouldReplaceOutput_(shouldReplaceOutput), replacement_(ll) {
  ...
  virtual std::string describe() const {
    return "replaceDisambiguationSymbols(" + Precursor::fsa_->describe() +
...
template <class _Automaton>
typename _Automaton::ConstRef
removeDisambiguationSymbols(typename _Automaton::ConstRef f) {
  return typename _Automaton::ConstRef(
      new ReplaceDisambiguationSymbolsAutomaton<_Automaton>(f, Fsa::Epsilon,
                                                            true, true));
}

template <class _Automaton>
typename _Automaton::ConstRef
replaceDisambiguationSymbols(typename _Automaton::ConstRef f, Fsa::LabelId ll) {
  return typename _Automaton::ConstRef(
      new ReplaceDisambiguationSymbolsAutomaton<_Automaton>(f, ll, true, true));
}

template <class _Automaton>
typename _Automaton::ConstRef
removeInputDisambiguationSymbols(typename _Automaton::ConstRef f) {
  return typename _Automaton::ConstRef(
      new ReplaceDisambiguationSymbolsAutomaton<_Automaton>(f, Fsa::Epsilon,
                                                            true, false));
}
...
template <class _Automaton>
typename _Automaton::ConstRef
replaceInputDisambiguationSymbols(typename _Automaton::ConstRef f,
                                  Fsa::LabelId ll) {
  return typename _Automaton::ConstRef(
      new ReplaceDisambiguationSymbolsAutomaton<_Automaton>(f, ll, true,
                                                            false));
}
