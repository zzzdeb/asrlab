...
protected:
  virtual const std::vector<MeanType> &
      iMean(Core::Ref<AbstractMeanEstimator>) const = 0;
...
public:
  ...
  VarianceType iCovariance(const CovarianceToMeanSetMap::MeanSet &,
                           ComponentIndex i) const {
    return iCovariance_[i];
  }
  Accumulator::SumType iSum(const CovarianceToMeanSetMap::MeanSet &,
                            ComponentIndex i) const;
