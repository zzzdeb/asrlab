...
public:
  ...
  virtual std::string describe() const {
    return Core::form("collect(%s,%s)", Precursor::fsa_->describe().c_str(),
                      this->semiring()->asString(value_).c_str());
  }
};

template <class _Automaton>
typename _Automaton::ConstRef collect(typename _Automaton::ConstRef f,
                                      typename _Automaton::Weight value) {
  return
      typename _Automaton::ConstRef(new CollectAutomaton<_Automaton>(f, value));
}
...
public:
  ...
  virtual std::string describe() const {
    return Core::form("extend(%s,%s)", Precursor::fsa_->describe().c_str(),
                      this->semiring()->asString(value_).c_str());
  }
};

template <class _Automaton>
typename _Automaton::ConstRef extend(typename _Automaton::ConstRef f,
                                     typename _Automaton::Weight value) {
  return
      typename _Automaton::ConstRef(new ExtendAutomaton<_Automaton>(f, value));
}
...
public:
  ...
  virtual std::string describe() const {
    return Core::form("extend-final(%s,%s)",
...
template <class _Automaton>
typename _Automaton::ConstRef extendFinal(typename _Automaton::ConstRef f,
                                          typename _Automaton::Weight value) {
  return typename _Automaton::ConstRef(
      new ExtendFinalAutomaton<_Automaton>(f, value));
}
...
template <class _Automaton, class Modifier>
typename _Automaton::ConstRef modifyWeight(typename _Automaton::ConstRef f,
                                           const Modifier &modifier) {
  return typename _Automaton::ConstRef(
      new WeightModifyAutomaton<_Automaton, Modifier>(f, modifier));
}
