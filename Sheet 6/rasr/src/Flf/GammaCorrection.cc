...
f64 gammaCorrectionFunc(const f64 x, const f64 gamma, const f64 brpt = 0.3) {
  ...
  if (x > brpt) {
    ...
    const f64 y =
        ((1.0 - ::pow(1.0 - ((x - brpt) / m1brpt), gamma)) * m1brpt) + brpt;
    return (y < 1e-12) ? 1e-12 : y;
  } else {
...
// -------------------------------------------------------------------------
void gammaCorrection(ConstConfusionNetworkRef cnRef, f64 gamma,
                     bool normalize) {
  if (!cnRef || (gamma == 1.0))
    return;
  ...
  for (ConfusionNetwork::iterator itSlot = cn.begin(), endSlot = cn.end();
    ...
    Score sum = 0.0;
    for (ConfusionNetwork::Slot::iterator itArc = slot.begin(),
                                          endArc = slot.end();
         itArc != endArc; ++itArc) {
...
void gammaCorrection(ConstPosteriorCnRef cnRef, f64 gamma, bool normalize) {
  ...
  for (PosteriorCn::iterator itSlot = cn.begin(), endSlot = cn.end();
    ...
    Probability sum = 0.0;
    for (PosteriorCn::Slot::iterator itArc = slot.begin(), endArc = slot.end();
         itArc != endArc; ++itArc) {
    ...
    if (normalize) {
      ...
      for (PosteriorCn::Slot::iterator itArc = slot.begin(),
                                       endArc = slot.end();
           itArc != endArc; ++itArc) {
        Probability &score = itArc->score;
        score *= norm;
...
public:
  CnGammaCorrectionNode(const std::string &name,
...
const Core::ParameterFloat CnGammaCorrectionNode::paramGamma("gamma", "gamma",
                                                             1.0);
const Core::ParameterBool
    CnGammaCorrectionNode::paramNormalize("normalize", "normalize", true);

class NormalizedCnGammaCorrectionNode : public CnGammaCorrectionNode {
  typedef CnGammaCorrectionNode Precursor;

public:
  NormalizedCnGammaCorrectionNode(const std::string &name,
                                  const Core::Configuration &config)
...
NodeRef
createNormalizedCnGammaCorrectionNode(const std::string &name,
                                      const Core::Configuration &config) {
...
public:
  PosteriorCnGammaCorrectionNode(const std::string &name,
                                 const Core::Configuration &config)
      : Precursor(name, config) {}
...
NodeRef
createPosteriorCnGammaCorrectionNode(const std::string &name,
                                     const Core::Configuration &config) {
