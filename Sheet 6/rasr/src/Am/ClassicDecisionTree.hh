...
public:
  ...
  void set(const StringList &keys, const ListOfIndexedStringList &values);
...
private:
  void logDiff(Core::XmlWriter &xml, PropertyMap::Index l,
               PropertyMap::Index r);
...
public:
  Properties(Cart::PropertyMapRef map)
      : Precursor(map), map_(dynamic_cast<const PropertyMap &>(*(map.get()))),
        alloState_(), cond_(0) {}
  ...
  void set(const Conditions &cond) { cond_ = &cond; }
    ...
    default:
      ...
      if (contextIndex < Index(map_.historySize())) {
        return (contextIndex < maxHistorySize_)
                   ? Index(allo.history()[Index(map_.historySize()) -
                                          contextIndex - 1])
                   : map_.termIndex;
        //			map_.undefinedIndex;
      }
      ...
      if (keyIndex < Index(map_.size()))
        return (cond_) ? Index(cond_->hasCondition(map_.key(keyIndex)))
                       : Index(false);
