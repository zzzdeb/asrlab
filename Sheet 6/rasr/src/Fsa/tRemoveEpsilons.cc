...
private:
  ...
  struct EpsPathCmp {
    ...
    EpsPathCmp(_ConstSemiringRef semiring) : semiring_(semiring) {}
...
public:
  RemoveEpsilons(_ConstAutomatonRef f)
      : Precursor(cache<_Automaton>(
            sort<_Automaton>(f, Fsa::SortTypeByInputAndTarget))) {
    if (f->type() != Fsa::TypeAcceptor)
      std::cerr << "epsilon-removal does only work for acceptors at the moment"
                << std::endl;
  }
    ...
    for (typename std::set<EpsPath, EpsPathCmp>::const_iterator qw =
      ...
      for (typename _State::const_iterator a = _sp->begin(); a != _sp->end();
        ...
        if (a->input() != Fsa::Epsilon) {
          _Arc tmp = *a;
          tmp.weight_ =
              Precursor::fsa_->semiring()->extend(qw->second, a->weight_);
          typename _State::iterator pos =
              sp->lower_bound(tmp, byArc<_Automaton>(this->semiring()));
          if ((pos == sp->end()) || (pos->input() != a->input()) ||
              (pos->target() != a->target()))
            sp->insert(pos, tmp);
          else
            pos->weight_ =
                Precursor::fsa_->semiring()->collect(pos->weight_, tmp.weight_);
        }
      }
      if (_sp->isFinal()) {
        _Weight w =
            Precursor::fsa_->semiring()->extend(qw->second, _sp->weight_);
        if (sp->isFinal())
...
private:
  ...
  public:
    IsEpsilonArc(ConstSemiringRef semiring) : semiring_(semiring) {}
    bool operator()(const Arc &a) const {
      return (a.input() == Fsa::Epsilon) && (a.output() == Fsa::Epsilon) &&
             (semiring_->compare(a.weight(), semiring_->one()) == 0);
    }
  };
  ConstSemiringRef semiring_;
...
public:
  ...
  virtual void modifyState(State *sp) const {
    ...
    while (!S.empty()) {
      ...
      for (typename State::const_iterator a = _sp->begin(); a != _sp->end();
           ++a)
        if ((a->input() == Fsa::Epsilon) && (a->output() == Fsa::Epsilon) &&
            (Precursor::fsa_->semiring()->compare(
                 a->weight(), Precursor::fsa_->semiring()->one()) == 0)) {
          mergeStates.push_back(a->target());
          S.push_back(a->target());
        }
    }
    sp->remove(IsEpsilonArc(Precursor::fsa_->semiring()));
    for (std::vector<Fsa::StateId>::const_iterator i = mergeStates.begin();
         i != mergeStates.end(); ++i) {
...
removeSimpleEpsilonArcs(Core::Ref<const _Automaton> f) {
  return Core::Ref<const _Automaton>(
      new SimpleRemoveEpsilonsAutomaton<_Automaton>(f));
}
