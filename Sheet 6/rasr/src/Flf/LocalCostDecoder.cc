...
public:
  ...
  void build(ConstLatticeRef hypSpaceL, ConstLatticeRef sumSpaceL,
             ConstFwdBwdRef sumSpaceFb);

  std::pair<ConstLatticeRef, Score>
  best(ScoreId confidenceId = Semiring::InvalidId) const;

  std::string name() const;

  Score wordPenalty() const { return wordPenalty_; }
  ...
  static ConstLocalCostDecoderRef create(ConstLatticeRef hypSpaceL,
                                         ConstLatticeRef sumSpaceL,
                                         ConstFwdBwdRef sumSpaceFb,
                                         RiskBuilderRef riskBuilder,
                                         Score wordPenalty);
};
// -------------------------------------------------------------------------
    ...
    struct SortArcByLabelAndEndTime {
      bool operator()(const Arc &arc1, const Arc &arc2) const {
        return (arc1.label < arc2.label) ||
               ((arc1.label == arc2.label) && (arc1.endTime < arc2.endTime));
      }
    };
  } hypSpace;

protected:
  void initializeHypothesisSpace(ConstLatticeRef l,
                                 ConstStateMapRef sortedStates);
  void finalizeHypothesisSpace(ConstLatticeRef l,
                               ConstStateMapRef sortedStates);
  void initializeSummationSpace(ConstLatticeRef l,
                                ConstStateMapRef sortedStates,
                                ConstFwdBwdRef fb);

  inline Time overlap(const HypothesisSpace::Arc &arc1,
                      const SummationSpace::Arc &arc2) const {
    return ((arc1.endTime < arc2.endTime) ? arc1.endTime : arc2.endTime) -
           ((arc1.beginTime > arc2.beginTime) ? arc1.beginTime
                                              : arc2.beginTime);
  }

  inline bool equalRisk(const HypothesisSpace::Arc &arc1,
                        const HypothesisSpace::Arc &arc2) const {
    return (arc1.beginTime == arc2.beginTime) &&
           (arc1.endTime == arc2.endTime) && (arc1.label == arc2.label);
  }
...
public:
  ...
  void set(Core::Vector<u32> *offsets, ScoreList *risks,
           ScoreList *confidences) {
  ...
  virtual void build(ConstLatticeRef hypSpaceL, ConstLatticeRef sumSpaceL,
                     ConstFwdBwdRef sumSpaceFb) = 0;

  virtual std::string name() const = 0;
};

void LocalCostDecoder::RiskBuilder::initializeHypothesisSpace(
    ConstLatticeRef l, ConstStateMapRef sortedStates) {
  ...
  u32 nArcs = 0;
  // collect some lattice statistics, construct list of states, initialize data
  // structure
  states.clear();
  states.grow(sortedStates->maxSid);
  {
    for (StateMap::const_iterator itSid = sortedStates->begin(),
                                  endSid = sortedStates->end();
         itSid != endSid; ++itSid) {
  ...
  // construct list of arcs sorted by 1) start time and 2) end time
  {
    ...
    for (StateMap::const_iterator itSid = sortedStates->begin(),
      ...
      u32 arcOrder = 0;
      for (Flf::State::const_iterator itA = s.begin(), endA = s.end();
           itA != endA; ++itA, ++itArc, ++arcOrder) {
        const Flf::Arc &a = *itA;
...
void LocalCostDecoder::RiskBuilder::finalizeHypothesisSpace(
    ConstLatticeRef l, ConstStateMapRef sortedStates) {
  HypothesisSpace::StateList &states = hypSpace.states;
  HypothesisSpace::ArcList &arcs = hypSpace.arcs;
  // copy risk and confidence to final data structure, restore original arc
  // order
...
void LocalCostDecoder::RiskBuilder::initializeSummationSpace(
    ConstLatticeRef l, ConstStateMapRef sortedStates, ConstFwdBwdRef fb) {
...
// -------------------------------------------------------------------------
class LocalCostDecoder::LocalOverlapRiskBuilder
    : public LocalCostDecoder::RiskBuilder {
...
void LocalCostDecoder::LocalOverlapRiskBuilder::build(
    ConstLatticeRef hypSpaceL, ConstLatticeRef sumSpaceL,
    ConstFwdBwdRef sumSpaceFb) {
  ...
  SummationSpace::ArcList::const_iterator beginRefArc = sumSpace.arcs.begin(),
                                          endRefArc = sumSpace.arcs.end();
  const HypothesisSpace::Arc *lastHypArcPtr = 0;
  pi.setTask(Core::form("#arcs=%d", hypSpace.arcs.size()));
  pi.setTotal(hypSpace.arcs.size());
  for (HypothesisSpace::ArcList::iterator itHypArc = hypSpace.arcs.begin(),
                                          endHypArc = hypSpace.arcs.end();
       itHypArc != endHypArc; ++itHypArc) {
    ...
    if (hypArc.beginTime == hypArc.endTime) {
      ...
      for (SummationSpace::ArcList::const_iterator itRefArc = beginRefArc;
           (itRefArc != endRefArc) && (itRefArc->beginTime < hypArc.endTime);
           ++itRefArc) {
        const SummationSpace::Arc &refArc = *itRefArc;
        if ((hypArc.beginTime < refArc.endTime) &&
            (refArc.beginTime < refArc.endTime)) {
...
// -------------------------------------------------------------------------
class LocalCostDecoder::ArcSymetricFrameErrorRiskBuilder
    : public LocalCostDecoder::RiskBuilder {
  typedef RiskBuilder Precursor;
...
void LocalCostDecoder::ArcSymetricFrameErrorRiskBuilder::build(
    ConstLatticeRef hypSpaceL, ConstLatticeRef sumSpaceL,
    ConstFwdBwdRef sumSpaceFb) {
  Core::ProgressIndicator pi("initialize");
  pi.start();
    ...
    if (hypArc.beginTime == hypArc.endTime) {
      ...
      for (SummationSpace::ArcList::const_iterator itRefArc = beginRefArc;
        ...
        if ((hypArc.beginTime < refArc.endTime) &&
            (refArc.beginTime < refArc.endTime)) {
          verify(refArc.beginTime < hypArc.endTime);
          Time ferr = std::max(refArc.endTime, hypArc.endTime) -
                      std::min(refArc.beginTime, hypArc.beginTime);
          if (hypArc.label == refArc.label)
            ferr -= overlap(hypArc, refArc);
          hypArc.risk +=
              refArc.posterior *
              (f64(ferr) / std::min(refArc.duration, hypArc.duration));
        }
      }
...
// -------------------------------------------------------------------------
class LocalCostDecoder::LocalAlignmentRiskBuilder
    : public LocalCostDecoder::RiskBuilder {
public:
  struct Closure {
    struct Path {
      ...
      Path()
          : target(Core::Type<u32>::max), fbScore(Core::Type<f64>::max),
            nToken(0.0), nCorrect(0.0), accuracy(0.0) {}
    };
  ...
  public:
    ...
    virtual void initialize(Closure::Path &path,
                            const HypothesisSpace::Arc &hypArc,
                            const f64 hypProp,
                            const SummationSpace::Arc &refArc,
                            const f64 refProp) const = 0;
    virtual void expand(Closure::Path &path, const HypothesisSpace::Arc &hypArc,
                        const f64 hypProp, const SummationSpace::Arc &refArc,
                        const f64 refProp) const = 0;
    virtual void addRiskAndConfidence(
        f64 &risk, f64 &confidence, const HypothesisSpace::Arc &hypArc,
        const Closure::Path &path, const Probability pathPosterior) const = 0;
    virtual std::string name() const = 0;
  };
...
public:
  void initialize(Closure::Path &path, const HypothesisSpace::Arc &hypArc,
                  const f64 hypProp, const SummationSpace::Arc &refArc,
                  const f64 refProp) const {
    path.accuracy =
        (hypArc.label == refArc.label) ? -1.0 + 2.0 * refProp : -1.0 + refProp;
  }
  void expand(Closure::Path &path, const HypothesisSpace::Arc &hypArc,
              const f64 hypProp, const SummationSpace::Arc &refArc,
              const f64 refProp) const {
    const f64 accuracy =
        (hypArc.label == refArc.label) ? -1.0 + 2.0 * refProp : -1.0 + refProp;
  ...
  void addRiskAndConfidence(f64 &risk, f64 &confidence,
                            const HypothesisSpace::Arc &hypArc,
                            const Closure::Path &path,
                            const Probability pathPosterior) const {
...
public:
  ...
  void expand(Closure::Path &path, const HypothesisSpace::Arc &hypArc,
    ...
    if (refArc.label != Fsa::Epsilon) {
      ...
      path.nToken += refProp;
      if (hypArc.label != Fsa::Epsilon) {
        // correct substitution + insertion (hyp-insertion=(hypProp-1.0) + alpha
        // * ref-insertion=(refProp-1.0))
        const f64 nCorrect =
            (refArc.label == hypArc.label)
...
void LocalCostDecoder::LocalAlignmentRiskBuilder::build(
    ConstLatticeRef hypSpaceL, ConstLatticeRef sumSpaceL,
    ConstFwdBwdRef sumSpaceFb) {
  ...
  for (HypothesisSpace::ArcList::iterator itHypArc = hypSpace.arcs.begin(),
    ...
    if (hypArc.beginTime == hypArc.endTime) {
      ...
      for (; beginRefArc->endTime <= hypArc.beginTime; ++beginRefArc)
        verify(beginRefArc != endRefArc);
      for (SummationSpace::ArcList::const_iterator itRefArc = beginRefArc;
           (itRefArc != endRefArc) && (itRefArc->beginTime <= hypArc.beginTime);
           ++itRefArc) {
        const SummationSpace::Arc &refArc = *itRefArc;
        if (hypArc.beginTime < refArc.endTime) {
          // initialize
          ...
          path.fbScore = sumSpace.states[refArc.from].fwdScore + refArc.score -
                         refArc.normScore;
          const f64 overlapDuration = f64(overlap(hypArc, refArc));
          ...
          while (!S.empty()) {
            ...
            S.pop_back();
            const SummationSpace::State &state =
                sumSpace.states[lastPath.target];
            if (hypArc.endTime <= state.time) {
              lastPath.fbScore += state.bwdScore;
              Probability pathPosterior = ::exp(-lastPath.fbScore);
              scorer.addRiskAndConfidence(risk, confidence, hypArc, lastPath,
                                          pathPosterior);
            } else {
              const u32 arcOffset = sumSpace.states[lastPath.target].beginArc;
              for (SummationSpace::ArcList::const_iterator
                       itRefArcCtd = sumSpace.arcs.begin() + arcOffset,
                       endRefArcCtd =
                           sumSpace.arcs.begin() + arcOffset + state.nArcs;
                   itRefArcCtd != endRefArcCtd; ++itRefArcCtd) {
                const SummationSpace::Arc &refArc = *itRefArcCtd;
                verify((hypArc.beginTime < refArc.beginTime) &&
                       (refArc.beginTime < hypArc.endTime));
      ...
      hypArc.risk = risk;
      hypArc.confidence =
          (confidence < 0.0) ? 0.0 : ((confidence < 1.0) ? confidence : 1.0);
    }
    pi.notify(u32(itHypArc - hypSpace.arcs.begin()));
    lastHypArcPtr = &hypArc;
  }
...
// -------------------------------------------------------------------------
LocalCostDecoder::LocalCostDecoder(LocalCostDecoder::RiskBuilderRef riskBuilder,
                                   Score wordPenalty)
...
std::string LocalCostDecoder::name() const { return riskBuilder_->name(); }

void LocalCostDecoder::build(ConstLatticeRef hypSpaceL,
                             ConstLatticeRef sumSpaceL,
                             ConstFwdBwdRef sumSpaceFb) {
  verify(hypSpaceL && hypSpaceL->getBoundaries()->valid() && sumSpaceL &&
         sumSpaceL->getBoundaries()->valid() && sumSpaceFb);
  l_ = hypSpaceL;
...
std::pair<ConstLatticeRef, Score>
LocalCostDecoder::best(ScoreId confidenceId) const {
  verify(l_);
  ...
  for (StateMap::const_iterator itSid = topologicalSort->begin(),
    ...
    // ScoreList::const_iterator itHypCost = hypCosts_.begin() + offsets_[sid],
    // itRefCost = refCosts_.begin() + offsets_[sid];

    Fsa::StateId aid = 0;
    for (Flf::State::const_iterator a = s.begin(); a != s.end();
         ++a, ++aid, ++itRisk) { // , ++itHypCost, ++itRefCost
      const Score cost = currentTrace.cost + *itRisk +
                         ((a->input() == Fsa::Epsilon) ? 0.0 : wordPenalty_);
  ...
  // dbg("cost=" << bestTrace.cost << ", hyp-cost=" << bestTrace.hypCost << ",
  // ref-cost=" << bestTrace.refCost);

  // trace
  StaticLattice *s = new StaticLattice(l_->type());
  s->setProperties(l_->knownProperties() | Fsa::PropertyLinear,
                   l_->properties() | Fsa::PropertyLinear);
  ...
  while (bestSid != initialSid) {
    ...
    // dbg("best: " << s->getInputAlphabet()->symbol(bestArc.input()) << ",
    // acc=" << accuracy(trace.sid, trace.aid));
...
ConstLocalCostDecoderRef
LocalCostDecoder::create(ConstLatticeRef hypSpaceL, ConstLatticeRef sumSpaceL,
                         ConstFwdBwdRef sumSpaceFb,
...
/*
// -------------------------------------------------------------------------
std::pair<ConstLatticeRef, Score> decodeByLocalRisk(ConstLatticeRef hypSpaceL,
ConstLatticeRef sumSpaceL, ConstFwdBwdRef sumSpaceFb, Score wordPenalty, ScoreId
confidenceId) { if (!hypSpaceL) return std::make_pair(ConstLatticeRef(),
Semiring::Invalid); verify(sumSpaceL); ConstLocalCostDecoderRef scorer =
        LocalCostDecoder::create(hypSpaceL, sumSpaceL, sumSpaceFb,
LocalCostDecoder::RiskBuilderRef(), wordPenalty); return
scorer->best(confidenceId);
}
// -------------------------------------------------------------------------
*/
 ...
 * local-alignment.scorer                = approximated-accuracy |
 *continous-cost1 | continous-cost2 | discrete-cost
...
typedef Core::Ref<const LocalCostDecoderBuilder>
    ConstLocalCostDecoderBuilderRef;

class LocalCostDecoderBuilder : public Core::Component,
...
public:
  ...
  LocalCostDecoder::RiskBuilderRef riskBuilder() const { return riskBuilder_; }

  Score wordPenalty() const { return wordPenalty_; }

  ConstLocalCostDecoderRef build() const {
    return ConstLocalCostDecoderRef(
        new LocalCostDecoder(riskBuilder_, wordPenalty_));
  }

  ConstLocalCostDecoderRef build(ConstLatticeRef hypSpaceL,
                                 ConstLatticeRef sumSpaceL,
                                 ConstFwdBwdRef sumSpaceFb) const {
    return LocalCostDecoder::create(hypSpaceL, sumSpaceL, sumSpaceFb,
                                    riskBuilder_, wordPenalty_);
  }

  static ConstLocalCostDecoderBuilderRef
...
const Core::ParameterFloat
    LocalCostDecoderBuilder::paramWordPenalty("word-penalty", "word penalty",
                                              0.0);

const Core::ParameterFloat LocalCostDecoderBuilder::paramAlpha("alpha", "alpha",
                                                               0.0);
...
const Core::ParameterChoice LocalCostDecoderBuilder::paramRiskBuilderType(
    "risk-builder", &LocalCostDecoderBuilder::choiceRiskBuilderType,
    "risk builder", LocalCostDecoderBuilder::RiskBuilderTypeOverlap);
...
const Core::Choice LocalCostDecoderBuilder::choiceOverlapScorerType(
    ...
    "arc-symetric",
    LocalCostDecoderBuilder::OverlapScorerTypeArcSymetricFrameError,
    Core::Choice::endMark());
const Core::ParameterChoice LocalCostDecoderBuilder::paramOverlapScorerType(
    "scorer", &LocalCostDecoderBuilder::choiceOverlapScorerType,
    "overlap scorer",
    LocalCostDecoderBuilder::OverlapScorerTypePathSymetricFrameError);

const Core::Choice LocalCostDecoderBuilder::choiceLocalAlignmentScorerType(
    "approximated-accuracy",
    LocalCostDecoderBuilder::LocalAlignmentScorerTypeLocalAccuracy,
    "continous-cost1",
    LocalCostDecoderBuilder::LocalAlignmentScorerTypeContinousCost1,
    "continous-cost2",
    LocalCostDecoderBuilder::LocalAlignmentScorerTypeContinousCost2,
    "discrete-cost",
    LocalCostDecoderBuilder::LocalAlignmentScorerTypeDiscreteCost,
    Core::Choice::endMark());
const Core::ParameterChoice
    LocalCostDecoderBuilder::paramLocalAlignmentScorerType(
        "scorer", &LocalCostDecoderBuilder::choiceLocalAlignmentScorerType,
        "local alignment scorer",
        LocalCostDecoderBuilder::LocalAlignmentScorerTypeContinousCost2);

LocalCostDecoderBuilder::LocalCostDecoderBuilder(
  ...
  const Core::Configuration riskBuilderConfig(
      config, choiceRiskBuilderType[riskBuilderType]);
  ...
  case RiskBuilderTypeOverlap: {
    ...
    const Core::Configuration scorerConfig(riskBuilderConfig,
                                           choiceOverlapScorerType[scorerType]);
  ...
  case RiskBuilderTypeLocalAlignment: {
    ...
    Core::Choice::Value scorerType =
        paramLocalAlignmentScorerType(riskBuilderConfig);
    if (scorerType == Core::Choice::IllegalValue)
      criticalError("Unknown local alignment scorer");
    const Core::Configuration scorerConfig(
        riskBuilderConfig, choiceLocalAlignmentScorerType[scorerType]);
    switch (LocalAlignmentScorerType(scorerType)) {
    case LocalAlignmentScorerTypeLocalAccuracy:
      scorer =
          LocalCostDecoder::LocalAlignmentRiskBuilder::ConstOverlapScorerRef(
              new LocalCostDecoder::LocalAlignmentRiskBuilder::
                  LocalAccuracyScorer);
      break;
    case LocalAlignmentScorerTypeContinousCost1:
      scorer =
          LocalCostDecoder::LocalAlignmentRiskBuilder::ConstOverlapScorerRef(
              new LocalCostDecoder::LocalAlignmentRiskBuilder::
                  ContinousCostScorer1(paramAlpha(scorerConfig, 1.0)));
      break;
    case LocalAlignmentScorerTypeContinousCost2:
      scorer =
          LocalCostDecoder::LocalAlignmentRiskBuilder::ConstOverlapScorerRef(
              new LocalCostDecoder::LocalAlignmentRiskBuilder::
                  ContinousCostScorer2(paramAlpha(scorerConfig, 0.5)));
      break;
    case LocalAlignmentScorerTypeDiscreteCost:
      scorer =
          LocalCostDecoder::LocalAlignmentRiskBuilder::ConstOverlapScorerRef(
              new LocalCostDecoder::LocalAlignmentRiskBuilder::
                  DiscreteCostScorer(paramAlpha(scorerConfig, 0.5)));
      break;
    default:
      defect();
    }
...
public:
  ExtendByLocalRiskLattice(ConstLatticeRef l, ConstLocalCostDecoderRef scorer,
                           ScoreId scoreId, ScoreId confidenceId,
                           RescoreMode rescoreMode)
      : RescoreLattice(l, rescoreMode), scorer_(scorer), scoreId_(scoreId),
  ...
  virtual void rescore(State *sp) const {
    ...
    for (State::const_iterator a = sp->begin(); a != sp->end();
         ++a, ++itRisk, ++itConfidence) {
  ...
  virtual std::string describe() const {
    std::string name =
        "extendByLocalRisk(" + fsa_->describe() + ",risk=" + scorer_->name();
    if (scoreId_ != Semiring::InvalidId)
...
ConstLatticeRef extendByLocalRisk(
    ...
    ConstLocalCostDecoderRef scorer =
        LocalCostDecoder::create(l, l, fb, LocalCostDecoder::RiskBuilderRef(),
wordPenalty); return ConstLatticeRef(new ExtendByLocalRiskLattice(l, scorer,
scoreId, confidenceId, rescoreMode));
}
*/
// -------------------------------------------------------------------------
...
public:
  typedef enum { SearchSpaceTypeUnion, SearchSpaceTypeMesh } SearchSpaceType;
...
protected:
  ...
  ConstLatticeRef getRescoredLattice() {
    ...
    if (!rescored_ && scorer_) {
      computeIds();
      rescored_ = ConstLatticeRef(new ExtendByLocalRiskLattice(
...
public:
  LocalCostDecoderNode(const std::string &name,
                       const Core::Configuration &config)
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    ScoreList requiredScales;
    {
      ...
      Core::Choice::Value ssChoice =
          choiceSearchSpaceType[paramSearchSpaceType(config)];
...
const Core::Choice
    LocalCostDecoderNode::choiceSearchSpaceType("union", SearchSpaceTypeUnion,
                                                "mesh", SearchSpaceTypeMesh,
                                                Core::Choice::endMark());
const Core::ParameterString LocalCostDecoderNode::paramSearchSpaceType(
    "search-space", "search spaces: union or mesh", "mesh");
const Core::ParameterString
    LocalCostDecoderNode::paramScoreKey("score-key", "store arc risk", "");
const Core::ParameterString
    LocalCostDecoderNode::paramConfidenceKey("confidence-key",
