...
// -------------------------------------------------------------------------
namespace {
const Core::ParameterString paramRescoreMode("rescore-mode", "rescore mode",
                                             "clone");
} // namespace
RescoreMode getRescoreMode(const Core::Configuration &config) {
  ...
  case RescoreModeInPlaceCache:
    Core::Application::us()->warning(
        "Cached in-place rescoring can have side effects in conjunction with "
        "non-linear command networks.");
    break;
  case RescoreModeInPlace:
    Core::Application::us()->warning(
        "In-place rescoring can have side effects. "
        "USE WITH CARE: amongst others, side effects will show up for all "
        "nodes traversing a lattice and not caching the visited states, e.g. "
        "\"info\".");
    break;
  default:
    break;
...
// -------------------------------------------------------------------------
RescoreNode::RescoreNode(const std::string &name,
                         const Core::Configuration &config)
    : Precursor(name, config) {}

bool RescoreNode::init_(NetworkCrawler &crawler,
...
const Core::ParameterBool RescoreSingleDimensionNode::paramAppend(
    "append", "append a new dimension to store rescoring results", false);

RescoreSingleDimensionNode::RescoreSingleDimensionNode(
    const std::string &name, const Core::Configuration &config)
    : Precursor(name, config) {}

bool RescoreSingleDimensionNode::init_(
    NetworkCrawler &crawler, const std::vector<std::string> &arguments) {
  bool b = Precursor::init_(crawler, arguments);
  ...
  if (append_) {
    ...
    if (!lastSemiring_ || (lastSemiring_.get() != l->semiring().get())) {
      ...
      if (lastExtendedSemiring_->key(lastExtendedSemiring_->size() - 1) != key_)
        error("Failed to set key \"%s\" for dimension %d; probably key is "
              "already in use.",
    ...
    if (!l->semiring()->hasId(id)) {
      ...
      if (!l->semiring()->hasId(id)) {
        criticalError(
            "Could not rescore lattice \"%s\"; dimension \"%s\" not found",
