...
class PosteriorPruneAutomaton : public SlaveAutomaton<_Automaton>,
...
private:
  void setMinWeight(const _Weight &threshold) {
    if (relative_) {
      ...
      minWeight_ = this->semiring()->extend(
          backward_[Precursor::fsa_->initialStateId()], threshold);
    }
  }

public:
  PosteriorPruneAutomaton(_ConstAutomatonRef f, const _Weight &threshold,
                          bool relative)
      : Precursor(f), DfsState<_Automaton>(f), threshold_(threshold),
        fw_(new _StatePotentials), bw_(new _StatePotentials), forward_(*fw_),
        backward_(*bw_), relative_(relative) {
  ...
  PosteriorPruneAutomaton(_ConstAutomatonRef f, const _Weight &threshold,
                          const _StatePotentials &fw,
                          const _StatePotentials &bw, bool relative)
      : Precursor(f), DfsState<_Automaton>(f), threshold_(threshold), fw_(0),
        bw_(0), forward_(fw), backward_(bw), relative_(relative) {
    this->setProperties(Fsa::PropertyStorage | Fsa::PropertyCached,
                        Fsa::PropertyNone);
    setMinWeight(threshold);
  }
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    if (s < forward_.size()) {
      ...
      for (typename _State::const_iterator a = _s->begin(); a != _s->end();
           ++a) {
        _Weight w = semiring->extend(a->weight(), backward_[a->target()]);
  ...
  virtual size_t getMemoryUsed() const {
    return Precursor::fsa_->getMemoryUsed() +
           2 * sizeof(typename Precursor::Weight) + forward_.getMemoryUsed() +
           backward_.getMemoryUsed();
  }
  virtual std::string describe() const {
    return Core::form("prunePosterior(%s,%s,%s)",
...
prunePosterior(typename _Automaton::ConstRef f,
               const typename _Automaton::Weight &threshold, bool relative) {
  return typename _Automaton::ConstRef(
      new PosteriorPruneAutomaton<_Automaton>(f, threshold, relative));
}
...
prunePosterior(typename _Automaton::ConstRef f,
               ...
               bool relative) {
  return typename _Automaton::ConstRef(
      new PosteriorPruneAutomaton<_Automaton>(f, threshold, fw, bw, relative));
}
...
public:
  SyncPruneAutomaton(_ConstAutomatonRef f, const _Weight &threshold)
      : Precursor(f), threshold_(threshold),
  ...
  void calculateSlice() const {
    ...
    while (!sliceStates_.empty()) {
      ...
      for (Core::Vector<Fsa::StateId>::const_iterator s = sliceStates_.begin();
           s != sliceStates_.end(); ++s) {
          ...
          if (semiring->compare(w, worst) < 0) {
            ...
            if (a->input_ != Fsa::Epsilon) {
              if (semiring->compare(w, potentials_[a->target()]) < 0) {
                ...
                if (semiring->compare(potentials_[a->target()], maxWeight_) ==
                    0)
    ...
    sliceStates_.clear();
    for (std::vector<Fsa::StateId>::iterator i = states.begin();
         i != states.end(); ++i) {
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    if (s < slice_.size()) {
      ...
      // we assume calculateSlice() creates entries in slice_ and potentials_
      // for each target state
      _State *sp = new _State(_sp->id(), _sp->tags(), _sp->weight_);
      _ConstSemiringRef semiring = Precursor::fsa_->semiring();
      for (typename _State::const_iterator a = _sp->begin(); a != _sp->end();
           ++a)
        if ((a->target() < slice_.size()) &&
            (slice_[a->target()] != Fsa::InvalidStateId) &&
            (semiring->compare(semiring->extend(potentials_[s], a->weight()),
                               minPotentials_[slice_[a->target()]]) < 0))
          *sp->newArc() = *a;
      sp->minimize();
      return _ConstStateRef(sp);
    }
  ...
  virtual size_t getMemoryUsed() const {
    return Precursor::fsa_->getMemoryUsed() + sizeof(Fsa::LabelId) +
           2 * sizeof(typename Precursor::Weight) + slice_.getMemoryUsed() +
           potentials_.getMemoryUsed() + minPotentials_.getMemoryUsed() +
           sliceStates_.getMemoryUsed();
  }
  virtual std::string describe() const {
    return Core::form("pruneSync(%s,%s)", Precursor::fsa_->describe().c_str(),
                      this->semiring()->asString(threshold_).c_str());
  }
};
...
pruneSync(typename _Automaton::ConstRef f,
          const typename _Automaton::Weight &threshold) {
  return typename _Automaton::ConstRef(
      new SyncPruneAutomaton<_Automaton>(f, threshold));
}
