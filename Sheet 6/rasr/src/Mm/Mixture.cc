...
Mixture::~Mixture() { clear(); }

Mixture::Mixture(const Mixture &mixture) { operator=(mixture); }
...
void Mixture::normalizeWeights() {
  ...
  std::transform(logWeights_.begin(), logWeights_.end(), logWeights_.begin(),
                 std::bind2nd(std::minus<Weight>(), logNorm));
}
...
void Mixture::removeDensitiesWithLowWeight(Weight minWeight,
                                           bool normalizeWeights) {
  const Weight _minWeight =
      exp(logExpNorm(logWeights_.begin(), logWeights_.end())) * minWeight;
