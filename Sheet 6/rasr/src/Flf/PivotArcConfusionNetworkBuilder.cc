...
public:
  ...
  struct Arc {
    ...
    u8 state;
    Arc()
        : slotDist(Core::Type<Score>::max), slotId(Core::Type<u32>::max),
          state(0) {}
    struct SortByTime {
...
public:
  ~PivotArcCn() {
    for (SlotPtrList::iterator itSlotPtr = slots_.begin();
         itSlotPtr != slots_.end(); ++itSlotPtr)
      delete *itSlotPtr;
    slots_.clear();
    for (ArcPtrList::iterator itArcPtr = arcs_.begin(); itArcPtr != arcs_.end();
         ++itArcPtr)
      delete *itArcPtr;
    arcs_.clear();
  }
  ...
  /*
    The lattice
  */
  ConstLatticeRef getLattice() const { return l_; }
  ...
  ConfusionNetwork::ConstMapPropertiesRef getMapping(bool reduced) const {
    ...
    if (reduced) {
      ...
      for (Fsa::StateId sid = 0; sid < slots_.size(); ++sid) {
        ...
        for (ArcPtrList::const_iterator itArcPtr = pcnSlot.arcs.begin(),
                                        endArcPtr = pcnSlot.arcs.end();
             itArcPtr != endArcPtr; ++itArcPtr) {
          const Arc &pcnArc = **itArcPtr;
          verify_(stateIndex[pcnArc.sid] + pcnArc.aid < lat2cn.size());
          lat2cn[stateIndex[pcnArc.sid] + pcnArc.aid] =
              ConfusionNetwork::MapProperties::Mapping(sid,
                                                       Fsa::InvalidStateId);
        }
      }
    } else {
      ...
      ConfusionNetwork::MapProperties::Map &cn2lat = mapProps->cn2lat;
      {
        ...
        cn2lat.resize(
            slotIndex.back(),
            ConfusionNetwork::MapProperties::Mapping(Fsa::InvalidStateId, 0));
      }
      for (Fsa::StateId sid = 0; sid < slots_.size(); ++sid) {
        Slot &pcnSlot = *slots_[sid];
        Fsa::StateId aid = 0;
        for (ArcPtrList::const_iterator itArcPtr = pcnSlot.arcs.begin(),
          ...
          cn2lat[slotIndex[sid] + aid] =
              ConfusionNetwork::MapProperties::Mapping(pcnArc.sid, pcnArc.aid);
          verify_(stateIndex[pcnArc.sid] + pcnArc.aid < lat2cn.size());
          lat2cn[stateIndex[pcnArc.sid] + pcnArc.aid] =
...
public:
  ConstConfusionNetworkRef getCn(ScoreId posteriorId,
    ...
    for (u32 i = 0; i < slots_.size(); ++i) {
      ...
      for (ArcPtrList::const_iterator itArcPtr = pcnSlot.arcs.begin(),
        ...
        slot.push_back(ConfusionNetwork::Arc(arc.input(), scores, pcnArc.start,
                                             pcnArc.end - pcnArc.start));
      }
    }
    if (mapping)
  ...
  /*
    Create confusion network, where per slot no label occurs twice and scores
    sum up to one, and create lattice storing best result with time boundaries,
    confidence scores, and no two subsequent epsilon arcs.
  */
private:
...
public:
  ...
  getNormalizedCn(ScoreId confidenceId, bool mapping = false) const {
    ...
    for (u32 i = 0; i < slots_.size(); ++i) {
      ...
      for (ArcPtrList::const_iterator itArcPtr = arcs.begin(),
                                      endArcPtr = arcs.end();
           itArcPtr != endArcPtr; ++itArcPtr) {
        ...
        if (pcnArcPtr->label != bestCurrentLabelPcnArcPtr->label) {
          ...
          Score start = ::exp(posteriorScore - colStart->get()),
                end = ::exp(posteriorScore - colEnd->get());
          colStart->reset();
          colEnd->reset();
          const Flf::State &state =
              *l_->getState(bestCurrentLabelPcnArcPtr->sid);
          const Flf::Arc &arc =
              *(state.begin() + bestCurrentLabelPcnArcPtr->aid);
          ScoresRef scores = arc.weight();
          ...
          slot.push_back(ConfusionNetwork::Arc(
              bestCurrentLabelPcnArcPtr->label, scores,
              Time(Core::round(start)),
        ...
        } else if (pcnArcPtr->posteriorScore <
                   bestCurrentLabelPcnArcPtr->posteriorScore) {
          bestCurrentLabelPcnArcPtr = pcnArcPtr;
        }
        col->feed(pcnArcPtr->posteriorScore);
        colStart->feed(pcnArcPtr->posteriorScore -
                       ::log(f64(pcnArcPtr->start)));
      ...
      col->reset();
      {
        ...
        slot.push_back(ConfusionNetwork::Arc(
            bestCurrentLabelPcnArcPtr->label, scores, Time(Core::round(start)),
            std::max(Time(1), Time(Core::round(end - start)))));
        if (posteriorScore < bestPosteriorScore) {
          bestPosteriorScore = posteriorScore;
          bestPcnArcPtr = bestCurrentLabelPcnArcPtr;
        ...
        if (dEpsConfidence < 0.0) {
          ...
          if (dEpsConfidence < -0.01)
            Core::Application::us()->warning(
                "PivotArcCnBuilder: Expected 1.0, got %f",
                (1.0 - dEpsConfidence));
          normalize = true;
        } else {
      ...
      if ((bestPcnArcPtr) && (bestPcnArcPtr->label != Fsa::Epsilon)) {
        if (preferredEndTime < bestPcnArcPtr->start) {
          ...
          preferredEndTime = std::max(
              minEndTime, (preferredEndTime + bestPcnArcPtr->start) / 2);
        ...
        sp->newArc(sp->id() + 1, bestPcnArcScores, bestPcnArcPtr->label,
                   bestPcnArcPtr->label);
        sp = new Flf::State(sp->id() + 1);
        s->setState(sp);
...
public:
  ...
  public:
    ...
    virtual Score distance(const PivotArcCn::Arc &ref,
                           const PivotArcCn::Arc &hyp) const = 0;
    virtual Score distance(const PivotArcCn::Slot &slot,
                           const PivotArcCn::Arc &arc) const {
      Score minDist = Core::Type<Score>::max;
      Time start = std::max(arc.start, slot.start),
           end = std::min(arc.end, slot.end);
      if (start < end) {
        for (PivotArcCn::ArcPtrList::const_iterator
                 itArcPtr = slot.arcs.begin(),
                 endArcPtr = slot.arcs.end();
             itArcPtr != endArcPtr; ++itArcPtr) {
  ...
  public:
    ...
    virtual std::string describe() const {
      return Core::form("weighted time, impact=%f, edit-distance=%s", impact,
                        ((editDist) ? "yes" : "no"));
    }
    virtual Score distance(const PivotArcCn::Arc &ref,
                           const PivotArcCn::Arc &hyp) const {
      Time start = std::max(ref.start, hyp.start),
           end = std::min(ref.end, hyp.end);
      verify(start < end);
      Score score = (impact + ref.posteriorScore * hyp.posteriorScore) / impact;
      if ((ref.isNonWord && hyp.isNonWord) ||
          (!ref.isNonWord && !hyp.isNonWord)) {
        Time dist =
                 ((ref.start < hyp.start) ? hyp.start - ref.start
                                          : ref.start - hyp.start) +
                 ((ref.end < hyp.end) ? hyp.end - ref.end : ref.end - hyp.end),
             norm = (ref.end - ref.start) + (hyp.end - hyp.start);
        score *= Score(dist) / Score(norm);
      }
      if (ref.label != hyp.label)
        score *=
            (editDist) ? (1.0 + editDist->normCost(ref.label, hyp.label)) : 2.0;
      return score;
    }
    static ConstDistanceRef
    create(Score impact = 10.0,
           ConstEditDistanceRef editDist = ConstEditDistanceRef()) {
  ...
  public:
    ...
    virtual std::string describe() const {
      return Core::form("weighted pivot time, impact=%f, edit-distance=%s",
                        impact, ((editDist) ? "yes" : "no"));
    }
    virtual Score distance(const PivotArcCn::Slot &slot,
                           const PivotArcCn::Arc &arc) const {
      Time start = std::max(arc.start, slot.start),
      ...
      return (start < end)
                 ? WeightedTimeDistance::distance(*slot.arcs.front(), arc)
                 : Core::Type<Score>::max;
    }
    static ConstDistanceRef
    create(Score impact = 10.0,
...
private:
  ...
  mutable PivotArcCn::ArcPtrList::iterator beginArcPtr_, endArcPtr_,
      endPivotArcPtr_;
...
private:
  ...
  void dump(PivotArcCn::SlotPtrList &slots) const {
    ...
    u32 slotId = 0;
    for (PivotArcCn::SlotPtrList::const_iterator itSlotPtr = slots.begin(),
                                                 endSlotPtr = slots.end();
         itSlotPtr != endSlotPtr; ++itSlotPtr, ++slotId) {
      const PivotArcCn::Slot &slot = **itSlotPtr;
      std::cout << slotId << ": " << slot.start << "-" << slot.end << std::endl;
      for (PivotArcCn::ArcPtrList::const_iterator itArcPtr = slot.arcs.begin();
           itArcPtr != slot.arcs.end(); ++itArcPtr) {
        const PivotArcCn::Arc &arc = **itArcPtr;
        std::cout << "  " << alphabet->symbol(arc.label) << ": " << arc.start
                  << "-" << arc.end << ", " << arc.posteriorScore << " ";
  ...
  bool isConsistent(PivotArcCn::SlotPtrList &slots) const {
    ...
    if (slots.size() > 1) {
      Time pStart = 0, pEnd = 0, tStart = slots.front()->start,
           tEnd = slots.front()->end;
      if (!(tStart < tEnd)) {
        dbg(1);
        consistent = false;
      }
      for (PivotArcCn::SlotPtrList::const_iterator itSlotPtr =
               slots.begin() + 1;
           (itSlotPtr != slots.end()) && consistent; ++itSlotPtr) {
        const PivotArcCn::Slot &slot = **itSlotPtr;
        pStart = tStart;
        pEnd = tEnd;
        tStart = slot.start;
        ...
        for (PivotArcCn::ArcPtrList::const_iterator itArcPtr =
                 slot.arcs.begin();
             itArcPtr != slot.arcs.end(); ++itArcPtr) {
          const PivotArcCn::Arc &arc = **itArcPtr;
  ...
  bool isArcListCleanedUp(u8 threshold) const {
    ...
    for (PivotArcCn::ArcPtrList::iterator endArcPtr = pcn_->arcs_.end();
         itArcPtr != endArcPtr; ++itArcPtr) {
      verify((*itArcPtr)->state >= threshold);
      verify((*itArcPtr)->slotDist != Core::Type<Score>::max);
    }
  ...
  // move all arcs to the end of the arc vector, where state reaches or exceeds
  // the threshold
  void cleanUpArcs(u8 threshold = 1) const {
    for (; (endArcPtr_ != beginArcPtr_) &&
           ((*(endArcPtr_ - 1))->state >= threshold);
         --endArcPtr_)
      ;
    for (PivotArcCn::ArcPtrList::iterator itArcPtr = beginArcPtr_;
         itArcPtr != endArcPtr_; ++itArcPtr) {
  ...
  void cleanUpArcsAndSlots() const {
    ...
    for (PivotArcCn::SlotPtrList::iterator itSlotPtr = pcn_->slots_.begin(),
                                           endSlotPtr = pcn_->slots_.end();
  ...
  u32 init(ConstLatticeRef l, ConstFwdBwdRef fb) const {
    LabelMapRef nonWordToEpsilonMap = LabelMap::createNonWordToEpsilonMap(
        Lexicon::us()->alphabetId(l->getInputAlphabet(), true));
    ...
    for (u32 i = 0; i < topologicalSort->size(); ++i) {
      ...
      for (State::const_iterator a = sr->begin(), a_begin = sr->begin(),
                                 a_end = sr->end();
           a != a_end; ++a) {
        ...
        /*
          ...
          (the reason is probably that the algorithm assumes that the pivot
          elements/slots occupy the complete time span ...) perhaps it is
          interesting to make it run for system combination
        */
        if (startTime < endTime) {
          ...
          arc->end = endTime;
          arc->label = a->input();
          arc->isNonWord = ((arc->label != Fsa::Epsilon) &&
                            (*nonWordToEpsilonMap)[arc->label].empty())
                               ? false
                               : true;
          arcs.push_back(arc);
        } else if (a->input() != Fsa::Epsilon)
          Core::Application::us()->warning(
              "Non-epsilon arc of length 0 detected: \"%s\", %d-%d, score=%f; "
              "arc is discarded",
              l->getInputAlphabet()->symbol(a->input()).c_str(), startTime,
              endTime, semiring->project(a->weight()));
        TraceElement &trace = traceback[a->target()];
        /*
          Score score = currentTrace.score + semiring->project(a->weight());

          Use best posterior sentence instead of Viterbi sentence for
          initialization. Seems to make no difference no average; sometimes some
          errors better, very sometimes a few errors worse. But it works, too,
          for fwd/bwd combination experiments.
        */
  ...
  // find pivot arcs, initialize a new slot for each pivot arc, and insert new
  // slot preserving time order
  u32 addPivotArcs(PivotArcCn::SlotPtrList &slots) const {
    u32 nAdded = 0;
    std::sort(beginArcPtr_, endArcPtr_,
              PivotArcCn::Arc::SortByPosteriorScore());
    for (PivotArcCn::ArcPtrList::iterator itArcPtr = beginArcPtr_;
         itArcPtr != endArcPtr_; ++itArcPtr) {
      PivotArcCn::Arc &arc = **itArcPtr;
      // binary search for finding correct position for new pivot element; slots
      // are sorted by time
      bool insertSlot = false;
      PivotArcCn::SlotPtrList::iterator itInsertSlot = slots.end();
      if (!slots.empty()) {
        s32 l = 0, r = slots.end() - slots.begin() - 1, m,
            maxM = slots.size() - 1;
        for (;;) {
          // m = (l + r) / 2;
          m = (s32)((u32)(l + r) >> 1);
          ...
          if (arc.start > slot.start) {
            if (m == r) {
              if ((slot.end <= arc.start) &&
                  ((m == maxM) ||
                   (arc.end <= (*(slots.begin() + m + 1))->start))) {
                insertSlot = true;
            ...
            if (l == m) {
              if ((arc.end <= slot.start) &&
                  ((m == 0) ||
                   ((*(slots.begin() + m - 1))->end <= arc.start))) {
                insertSlot = true;
  ...
  // find best matching slot for all arcs; some arcs might be left because of
  // violation of time contraints
  u32 addArcs(PivotArcCn::SlotPtrList &slots) const {
    ...
    do {
      ...
      for (PivotArcCn::SlotPtrList::const_iterator itSlotPtr = slots.begin(),
        ...
        const PivotArcCn::Slot &slot = **itSlotPtr;
        for (;
             (beginArcPtr != endArcPtr_) && ((*beginArcPtr)->end <= slot.start);
             ++beginArcPtr)
          ;
        for (PivotArcCn::ArcPtrList::iterator itArcPtr = beginArcPtr;
             (itArcPtr != endArcPtr_) && ((*itArcPtr)->start < slot.end);
             ++itArcPtr) {
          PivotArcCn::Arc &arc = **itArcPtr;
      ...
      /* dbg
         for (PivotArcCn::ArcPtrList::iterator itArcPtr = beginArcPtr_; itArcPtr
         != endArcPtr_; ++itArcPtr) { PivotArcCn::Arc &arc = **itArcPtr; if
         (arc.slotId < slots.size()) { PivotArcCn::Slot &slot =
         *slots[arc.slotId]; verify(arc.slotDist != Core::Type<Score>::max);
      ...
      */
      ...
      /* dbg
         for (PivotArcCn::ArcPtrList::iterator itArcPtr = beginArcPtr_; itArcPtr
         != endArcPtr_; ++itArcPtr) { PivotArcCn::Arc &arc = **itArcPtr; if
         ((arc.slotId < slots.size()) && (arc.state == 0)) { PivotArcCn::Slot
         &slot = *slots[arc.slotId]; verify(std::max(arc.start, slot.start) >=
         std::min(arc.end, slot.end));
         }
         }
      */
  ...
  void addSlots(PivotArcCn::SlotPtrList &slots,
                PivotArcCn::SlotPtrList &addSlots) const {
    PivotArcCn::SlotPtrList::iterator itSlotPtr = slots.begin(),
                                      itAddSlotPtr = addSlots.begin();
   ...
   * 3) sort arcs by similarity and assign arc to slot until all remaining arcs
   *violate time contraints; remove all assigned arcs goto 2) until no more arcs
   *are assigned 4) sort arcs by posterior and find new pivot arcs; remove pivot
   *arcs goto 2) until no more arcs are left
   *
   * Algorithm 2:
   * 1) Initialize CN with top best arcs; remove top best aka pivot arcs
   * 2) for all arcs finds best matching slot
   * 3) sort arcs by similarity and assign arc to slot until all remaining arcs
   *violate time contraints; temporarily remove all assigned arcs goto 2) until
   *no more arcs are assigned 4) sort arcs by posterior and find new pivot arcs;
   *remove pivot arcs, put back all temporarily removed arcs goto 2) until no
   *more arcs are left
   **/
...
public:
  ...
  ConstPivotArcCnRef build(ConstLatticeRef l,
                           ConstFwdBwdRef fb = ConstFwdBwdRef()) const {
    if (!fb) {
  ...
  static ConstPivotArcCnBuilderRef
  create(ConstDistanceRef distFcn = ConstDistanceRef(), bool fast = false) {
    if (!distFcn)
...
PivotArcCnFactory::PivotArcCnFactory() {
  ...
  internal_->pivotCnBuilder = PivotArcCnBuilder::create(
      PivotArcCnBuilder::WeightedPivotTimeDistance::create(10.0), false);
}

PivotArcCnFactory::~PivotArcCnFactory() { delete internal_; }
...
void PivotArcCnFactory::reset() { internal_->pivotCn.reset(); }

ConstConfusionNetworkRef PivotArcCnFactory::getCn(ScoreId posteriorId,
                                                  bool mapping) const {
...
std::pair<ConstConfusionNetworkRef, ConstLatticeRef>
PivotArcCnFactory::getNormalizedCn(ScoreId confidenceId, bool mapping) const {
  verify(internal_->pivotCn);
...
public:
  typedef enum { WeightedTimeDistance, WeightedPivotTimeDistance } DistanceFcn;
...
private:
  ConstPivotArcCnBuilderRef getPivotArcCnBuilder(ConstLatticeRef l) {
    if (!cnBuilder_) {
      ...
      const Core::Configuration distConfig =
          select(choiceDistanceFcn[distChoice]);
      Score impact = 1.0 / paramPosteriorImpact(distConfig);
      ConstEditDistanceRef editDist;
      if (paramEditDistance(distConfig))
        editDist = EditDistance::create(
      ...
      case WeightedTimeDistance:
        distFcn =
            PivotArcCnBuilder::WeightedTimeDistance::create(impact, editDist);
        break;
      case WeightedPivotTimeDistance:
        distFcn = PivotArcCnBuilder::WeightedPivotTimeDistance::create(
            impact, editDist);
        break;
      default:
        defect();
  ...
  void computeConfidenceId() {
    ...
    if (!lastSemiring_ || (semiring.get() != lastSemiring_.get()) ||
  ...
  ConstConfusionNetworkRef getNormalizedCn() {
    if (!posteriorCn_) {
      ...
      std::pair<ConstConfusionNetworkRef, ConstLatticeRef> result =
          pivotCn_->getNormalizedCn(confidenceId_, map_);
...
public:
  PivotArcCnBuilderNode(const std::string &name,
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    fbBuilder_ =
        FwdBwdBuilder::create(select("fb"), requiredKeys, requiredScales);
...
const Core::ParameterChoice PivotArcCnBuilderNode::paramDistanceFcn(
    "distance", &PivotArcCnBuilderNode::choiceDistanceFcn, "distance function",
    PivotArcCnBuilderNode::WeightedTimeDistance);
const Core::ParameterFloat PivotArcCnBuilderNode::paramPosteriorImpact(
    "posterior-impact", "impact of posterior score on arc distance", 0.1);
const Core::ParameterBool PivotArcCnBuilderNode::paramEditDistance(
    "edit-distance", "calculate edit distance between different arc labels",
    false);
const Core::ParameterBool
    PivotArcCnBuilderNode::paramFast("fast", "use fast algorithm", true);
const Core::ParameterString
    PivotArcCnBuilderNode::paramConfKey("confidence-key",
                                        "store confidence score", "");
const Core::ParameterBool
    PivotArcCnBuilderNode::paramMap("map", "map lattice to CN and vice versa",
                                    false);
NodeRef createPivotArcCnBuilderNode(const std::string &name,
                                    const Core::Configuration &config) {
