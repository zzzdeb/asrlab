 ...
 *
 *	Alternating complex numbers are first transformed to a
 *complex<InputType> object. Operation is transforms the complex<InputType>
 *object: operation must support the function: OutputType operator()(const
 *complex<InputType> &). Operation is a unary function (derived from
 *std::unary_function): Argument must have a constructor with two parameters
...
void transformAlternatingComplex(InputIterator first, InputIterator last,
  ...
  for (InputIterator real = first, imaginary = first + 1; real != last;
       ++result) {
 ...
 */
template <class InputIterator, class OutputIterator, class Operation>
void transformAlternatingComplexToAlternatingComplex(InputIterator first1,
                                                     InputIterator last1,
                                                     InputIterator first2,
                                                     OutputIterator result,
                                                     Operation operation) {

  for (InputIterator real1 = first1, imaginary1 = first1 + 1, real2 = first2,
                     imaginary2 = first2 + 1;
       real1 != last1;
...
struct conjugateMultiplies
    : public std::binary_function<std::complex<T>, std::complex<T>,
                                  std::complex<T> > {

  std::complex<T> operator()(const std::complex<T> &x,
                             const std::complex<T> &y) const {
    return x * conj(y);
  }
};
} // namespace Math
