 ...
 *  Parameters
 *    -expand-timestamp: if true timestamp of output encompasses the timestamps
 * of its element; if false timestamp if the "present" element is copied to
 * timestamp of output. -for further parameters @see DelayNode Note: in output
 * vector oldest frames are first and recent frames last.
 */

extern const Core::ParameterBool paramVectorSequenceConcatExpandTimestamp;

template <class T> class VectorSequenceConcatenation : public DelayNode {
...
public:
  static std::string filterName() {
    return std::string("signal-vector-") + Core::Type<T>::name +
...
template <class T>
VectorSequenceConcatenation<T>::VectorSequenceConcatenation(
...
template <class T>
bool VectorSequenceConcatenation<T>::setParameter(const std::string &name,
...
template <class T> bool VectorSequenceConcatenation<T>::configure() {
  clear();
...
template <class T> bool VectorSequenceConcatenation<T>::putData() {
  Flow::Vector<T> *out = new Flow::Vector<T>;
  out->invalidateTimestamp();
  for (int relativeIndex = -(int)maxPastSize();
       relativeIndex <= (int)maxFutureSize(); ++relativeIndex) {
