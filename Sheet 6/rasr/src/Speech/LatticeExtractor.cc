...
LatticeExtractor::LatticeExtractor(const Core::Configuration &c)
    : Core::Component(c), level_(paramLevel(c)) {}

Lattice::ConstWordLatticeRef
LatticeExtractor::extract(Lattice::ConstWordLatticeRef lattice,
                          Bliss::SpeechSegment *) {
 ...
 */
Lattice::ConstWordLatticeRef
LatticeRescorer::extract(Lattice::ConstWordLatticeRef lattice,
 ...
 * LatticeRescorer
 */
LatticeRescorer::LatticeRescorer(const Core::Configuration &c) : Precursor(c) {}

/*
 * LatticeRescorer: acoustic base class
 */
AcousticLatticeRescorerBase::AcousticLatticeRescorerBase(
...
Lattice::ConstWordLatticeRef
AcousticLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice,
                              Bliss::SpeechSegment *segment) {
 ...
 */
class AlignmentLatticeRescorerAutomaton
    : public CachedLatticeRescorerAutomaton {
...
AlignmentLatticeRescorerAutomaton::AlignmentLatticeRescorerAutomaton(
    ...
    : Precursor(lattice), alignmentGenerator_(alignmentGenerator) {
  require(alignmentGenerator_);
}

Fsa::Weight AlignmentLatticeRescorerAutomaton::score(Fsa::StateId s,
                                                     const Fsa::Arc &a) const {
  ...
  if (pronunciation && begtime != InvalidTimeframeIndex) {
    // fsa_->getState(a.target())->id(): guarantee that wordBoundary at
    // a.target() exists
...
Fsa::Weight AlignmentLatticeRescorerAutomaton::_score(
    const Bliss::Coarticulated<Bliss::LemmaPronunciation>
        &coarticulatedPronunciation,
    TimeframeIndex begtime, TimeframeIndex endtime) const {
  if (begtime < endtime) {
    return Fsa::Weight(alignmentGenerator_->alignmentScore(
        coarticulatedPronunciation, begtime, endtime));
  } else {
    Core::Application::us()->warning("score 0 assigned to arc with begin time ")
        << begtime << " , end time " << endtime << " and label id "
        << coarticulatedPronunciation.object().id();
    return fsa_->semiring()->one();
  }
}
 ...
 */
AlignmentLatticeRescorer::AlignmentLatticeRescorer(const Core::Configuration &c)
    : Precursor(c) {}

Lattice::ConstWordLatticeRef
AlignmentLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice,
                               Bliss::SpeechSegment *segment) {
  alignmentGenerator_->setSpeechSegment(segment);
...
LmLatticeRescorerAutomaton::LmLatticeRescorerAutomaton(
    ...
    : LatticeRescorerAutomaton(lattice), languageModel_(languageModel),
      alphabet_(required_cast(const Bliss::LemmaPronunciationAlphabet *,
                              fsa_->getInputAlphabet().get())),
      pronunciationScale_(pronunciationScale) {}

Fsa::Weight LmLatticeRescorerAutomaton::score(Fsa::StateId s,
                                              const Fsa::Arc &a) const {
  ...
  if (lp) {
    Lm::addLemmaPronunciationScore(languageModel_, lp, pronunciationScale_,
                                   languageModel_->scale(), hist, score);
  }
  ...
  if (!(hist == histories_[a.target()])) {
    languageModel_->error()
        ...
        << a.target() << "' ('" << languageModel_->formatHistory(hist)
        << "' vs. '" << languageModel_->formatHistory(histories_[a.target()])
        << "').\n"
 ...
 * LatticeRescorer: lm
 */
LmLatticeRescorer::LmLatticeRescorer(const Core::Configuration &c,
                                     bool initialize)
    : Precursor(c) {
  if (initialize) {
...
Lattice::ConstWordLatticeRef
LmLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice,
                        Bliss::SpeechSegment *) {
...
Lattice::ConstWordLatticeRef
CombinedLmLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice,
                                Bliss::SpeechSegment *) {
  Fsa::ConstAutomatonRef lm =
      Fsa::ConstAutomatonRef(new LmLatticeRescorerAutomaton(
...
const Core::ParameterString
    LatticeReader::paramFsaPrefix("fsa-prefix",
                                  "prefix of automaton in archive");

LatticeReader::LatticeReader(const Core::Configuration &c,
                             Bliss::LexiconRef lexicon)
    : Precursor(c), archiveReader_(0) {
  ...
  archiveReader_ =
      Lattice::Archive::openForReading(select("lattice-archive"), lexicon);
...
Lattice::ConstWordLatticeRef
LatticeReader::extract(Lattice::ConstWordLatticeRef lattice,
                       Bliss::SpeechSegment *segment) {
  ...
  if (result && result->nParts() == 1) {
    if (result->wordBoundaries() and
        (*result->wordBoundaries()) != (*lattice->wordBoundaries())) {
      error("Mismatch between the word boundaries of the given lattice and the "
            "read lattice.");
    }
  } else {
...
Core::Choice DistanceLatticeRescorer::choiceDistanceType(
    ...
    "levenshtein-on-list", levenshteinOnList, "word-accuracy", wordAccuracy,
    "phoneme-accuracy", phonemeAccuracy, "frame-word-accuracy",
    frameWordAccuracy, "frame-phone-accuracy", framePhoneAccuracy,
    "frame-state-accuracy", frameStateAccuracy, "smoothed-frame-state-accuracy",
    smoothedFrameStateAccuracy, "soft-frame-phone-accuracy",
    softFramePhoneAccuracy, Core::Choice::endMark());

Core::ParameterChoice DistanceLatticeRescorer::paramDistanceType(
    "distance-type", &choiceDistanceType, "distance measure to apply",
    approximateWordAccuracy);

Core::Choice DistanceLatticeRescorer::choiceSpokenSource(
    "orthography", orthography, "archive", archive, Core::Choice::endMark());

Core::ParameterChoice DistanceLatticeRescorer::paramSpokenSource(
    "spoken-source", &choiceSpokenSource,
    "where to get the correct hypotheses from", orthography);

DistanceLatticeRescorer::DistanceLatticeRescorer(const Core::Configuration &c)
    : Precursor(c) {}
...
LatticeRescorer *DistanceLatticeRescorer::createDistanceLatticeRescorer(
  ...
  /*! @todo: move ? */
  return Speech::Module::instance().createDistanceLatticeRescorer(config,
                                                                  lexicon);
}
...
Lattice::ConstWordLatticeRef
ApproximateDistanceLatticeRescorer::work(Lattice::ConstWordLatticeRef lattice,
                                         Bliss::SpeechSegment *segment) {
 ...
 */
ArchiveApproximateWordAccuracyLatticeRescorer::
    ArchiveApproximateWordAccuracyLatticeRescorer(const Core::Configuration &c,
                                                  Bliss::LexiconRef lexicon)
    : Precursor(c, lexicon), builder_(0) {
...
ArchiveApproximateWordAccuracyLatticeRescorer::getDistanceFsa(
    Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment *segment) {
  verify(builder_);
  return builder_
      ->createFunctor(segment->fullName(), segment->fullName(), lattice)
      .build();
}

OrthographyApproximateWordAccuracyLatticeRescorer::
    OrthographyApproximateWordAccuracyLatticeRescorer(
        const Core::Configuration &c, Bliss::LexiconRef lexicon)
    : Precursor(c, lexicon), builder_(0) {
...
Fsa::ConstAutomatonRef
OrthographyApproximateWordAccuracyLatticeRescorer::getDistanceFsa(
    Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment *segment) {
  verify(builder_);
  return builder_->createFunctor(segment->fullName(), segment->orth(), lattice)
      .build();
}
 ...
 */
ApproximatePhoneAccuracyLatticeRescorer::
    ApproximatePhoneAccuracyLatticeRescorer(const Core::Configuration &c,
                                            Bliss::LexiconRef lexicon)
    : Precursor(c, lexicon) {}
...
ArchiveApproximatePhoneAccuracyLatticeRescorer::
    ArchiveApproximatePhoneAccuracyLatticeRescorer(const Core::Configuration &c,
                                                   Bliss::LexiconRef lexicon)
    : Precursor(c, lexicon), builder_(0) {
...
Fsa::ConstAutomatonRef
ArchiveApproximatePhoneAccuracyLatticeRescorer::getDistanceFsa(
    Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment *segment) {
  verify(builder_);
  alignmentGenerator_->setSpeechSegment(segment);
  return builder_
...
OrthographyApproximatePhoneAccuracyLatticeRescorer::
    OrthographyApproximatePhoneAccuracyLatticeRescorer(
        const Core::Configuration &c, Bliss::LexiconRef lexicon)
    : Precursor(c, lexicon), builder_(0) {
...
Fsa::ConstAutomatonRef
OrthographyApproximatePhoneAccuracyLatticeRescorer::getDistanceFsa(
    Lattice::ConstWordLatticeRef lattice, Bliss::SpeechSegment *segment) {
  verify(builder_);
  alignmentGenerator_->setSpeechSegment(segment);
  return builder_
      ->createFunctor(segment->fullName(), segment->orth(), lattice,
                      alignmentGenerator_)
      .build();
}
