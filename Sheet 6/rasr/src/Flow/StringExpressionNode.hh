 ...
 *    ...
 *    <node name="cepstrum" ...
 * warping-function="linear(a=$input(warping-factor))"/> <link from="..."
 * to="cepstrum:warping-factor"/>
 *    ...
 *
 *    Value of the expression in warping-function will be resolved for each
 * input at the port warping-factor.
 *
 *  Synchronization of ports:
 *    Classes derived from StringExpressionNode will call the update(Timestamp
 * ts) function to update the string-expression input-ports. Input ports are
 * read if the timestamp 'ts' does not fall into the intervall
 * [start-time..end-time] of the current parameter value. Thus, parameters valid
 * for a complete segment should have start-time equal segment start-time and
 *    end-time equals segment end-time. It is of course possible to have
 * parameters valid for shorter intervals than the complete segment.
 */
...
private:
  void update(Core::StringHashMap<InputPort>::iterator inputPort,
              const String &value);
...
public:
  ...
  /** Performs configuration and merges its own input attributes into @param
   * result. */
  bool configure(Attributes &result);

  /** Reads the input ports and resolves the string expression.
   *  Input ports are read till @param timestamp falls in the interval
   * [start-time..end-time] of an input object.
