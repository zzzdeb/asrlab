...
/*****************************************************************************/
Archive::Archive(const Core::Configuration &config, const std::string &path,
                 AccessMode access)
...
/*****************************************************************************/
{
  ...
  if (sizes.compressed() > 0) {
    ...
    if (read(name, tmp)) {
      ...
      tmp[base] = 0x78;
      tmp[base + 1] = 0x9c;
      switch (uncompress((Bytef *)&b[0], &tmplen, (Bytef *)&tmp[base],
                         compressedSize)) {
      case Z_MEM_ERROR:
      ...
      case Z_BUF_ERROR:
        std::cerr << "unpack buffer was too small (" << sizes.compressed()
                  << ", " << sizes.uncompressed() << ", " << tmplen << ")."
                  << std::endl;
        break;
      case Z_DATA_ERROR:
        /* CAUTION! Zlib thinks that data was corrupted because we replaced
         * the adler32 checksum by a gzip compatible crc32. So we ignore the
         * error.
         */
        // fall through
      case Z_OK:
...
/*****************************************************************************/
bool Archive::writeFile(const std::string &name, const std::string &b,
                        bool compress)
/*****************************************************************************/
{
  lock();
  ...
  /* from zlib.h:
   *
   * ZEXTERN int ZEXPORT compress OF((Bytef *dest, uLongf *destLen, const Bytef
  ...
  if (compress) {
     ...
     */
    int zstatus = ::compress2((Bytef *)&compressed[header_length - 2],
                              &compressedSize, (const Bytef *)&(b.c_str()[0]),
                              b.size(), Z_DEFAULT_COMPRESSION);
    hope(compressed[header_length - 2] == 0x78 &&
         compressed[header_length - 1] == 0x9c);
    compressed.resize(header_length + compressedSize - 6);
    if (zstatus != Z_OK) {
      std::cerr << "pack buffer was too small (" << b.size() << ", "
                << compressedSize << ". falling back to no compression."
                << std::endl;
      compressed.resize(0);
    } else {
      // gzip header
      ...
      compressed[4] =
          0; // modification time: 4 bytes (0 = no timestamp available)
      compressed[5] = 0;
      compressed[6] = 0;
      compressed[7] = 0;
...
/*****************************************************************************/
{
  ...
  std::string tmp;
  u32 bufferSize =
      (sizes.compressed() ? sizes.compressed() : sizes.uncompressed());
  std::string buffer;
...
/*****************************************************************************/
{
  ...
  s << std::setiosflags(std::ios::left) << std::setw(width - 2 * prec - 3)
    << "name";
  s.flags(old);
  s << std::setiosflags(std::ios::right) << std::setw(prec + 1) << "compressed";
  s << std::setiosflags(std::ios::right) << std::setw(prec + 1) << "size"
    << std::endl;
  s << std::setw(width - 1) << std::setfill('-') << "-" << std::setfill(' ')
    << std::endl;
  for (Archive::const_iterator i = a.files(); i; ++i) {
    s.flags(old);
    s << std::setiosflags(std::ios::left) << std::setw(width - 2 * prec - 3)
    ...
    if (i.sizes().compressed())
      s << ' ' << std::setiosflags(std::ios::right) << std::setw(prec)
        << i.sizes().compressed();
    else
      s << " " << std::setw(prec) << " ";
    s << ' ' << std::setiosflags(std::ios::right) << std::setw(prec)
      << i.sizes().uncompressed() << std::endl;
  ...
  s << std::setw(width - 1) << std::setfill('-') << "-" << std::setfill(' ')
    << std::endl;
  s.flags(old);
  s << "total: " << std::setiosflags(std::ios::left)
    << std::setw(width - 2 * prec - 10) << total;
  if (compressed)
    s << ' ' << std::setiosflags(std::ios::right) << std::setw(prec)
      << compressed;
  else
    s << " " << std::setw(prec) << " ";
  s << ' ' << std::setiosflags(std::ios::right) << std::setw(prec) << size
    << std::endl;
  s.flags(old);
  return s;
}
...
/*****************************************************************************/
Archive *Archive::create(const Configuration &config, const std::string &path,
                         AccessMode access)
/*****************************************************************************/
{
  Archive *result = 0;
  ...
  case TypeBundle:
    ...
    break;
  case TypeUnknown:
    Application::us()->error("unknown type of archive (requested path: '%s').",
                             path.c_str());
    break;
  default:
    defect();
...
/*****************************************************************************/
ArchiveWriter::ArchiveWriter(Archive &archive, const std::string &path,
                             bool compress)
    : std::ostream(new ArchiveWriterBuffer(buffer_)), archive_(archive),
      path_(path), compress_(compress)
/*****************************************************************************/
{
