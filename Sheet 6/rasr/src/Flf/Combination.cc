...
public:
  ...
  virtual ScoreId subId(ScoreId subSemiringIndex,
                        const Key &subScoreKey) const {
    return Semiring::InvalidId;
  }

  virtual void set(ScoresRef scores, ScoreId subSemiringIndex,
                   ScoreId subScoreIndex, Score subScore) const {}
  virtual void set(ScoresRef scores, ScoreId subSemiringIndex,
                   ScoresRef subScores) const {}
};
// -------------------------------------------------------------------------
...
public:
  DiscardSemiringCombination(const KeyList &comboKeys,
  ...
  virtual bool update(const ConstSemiringRefList &subSemirings) {
    ...
    if (!semiring) {
      semiring = Semiring::create(Fsa::SemiringTypeTropical, comboKeys.size(),
                                  comboScales, comboKeys);
      return true;
    } else
      return false;
  }
 ...
 **/
class ConcatenateSemiringCombination
    : public SemiringCombinationHelper::Internal {
  typedef SemiringCombinationHelper::Internal Precursor;
...
public:
  ConcatenateSemiringCombination(const KeyList &comboKeys,
                                 const ScoreList &comboScales)
      : Precursor(comboKeys, comboScales) {
  ...
  virtual bool update(const ConstSemiringRefList &subSemirings) {
    ...
    if (this->subSemirings.size() == subSemirings.size()) {
      for (ConstSemiringRefList::const_iterator
               itOld = this->subSemirings.begin(),
               endOld = this->subSemirings.end(), itNew = subSemirings.begin();
    ...
    if (updated) {
      ...
      for (u32 i = 0; i < subSemirings.size(); ++i) {
        ...
        n += size;
        for (KeyList::const_iterator itKey = subSemiring.keys().begin(),
                                     endKey = subSemiring.keys().end();
             itKey != endKey; ++itKey)
          keys.push_back(Core::form("%d-%s", i, itKey->c_str()));
        for (ScoreList::const_iterator itScale = subSemiring.scales().begin(),
                                       endScale = subSemiring.scales().end();
      ...
      for (ScoreIdList::iterator itComboId = comboIds.begin(),
                                 endComboId = comboIds.end();
           itComboId != endComboId; ++itComboId, ++n)
        *itComboId = n;
      semiring =
          Semiring::create(subSemirings.front()->type(), n, scales, keys);
    }
    return updated;
  }
  ...
  ScoreId subId(ScoreId subSemiringIndex, const Key &subScoreKey) const {
    return subId(subSemiringIndex,
                 subSemirings[subSemiringIndex]->id(subScoreKey));
  }

  void set(ScoresRef scores, ScoreId subSemiringIndex, ScoreId subScoreIndex,
           Score subScore) const {
    scores->set(ranges_[subSemiringIndex].first + subScoreIndex, subScore);
  }

  void set(ScoresRef scores, ScoreId subSemiringIndex,
           ScoresRef subScores) const {
    const std::pair<ScoreId, ScoreId> &range = ranges_[subSemiringIndex];
    Scores::iterator itScore = scores->begin() + range.first;
    for (Scores::const_iterator itSubScore = subScores->begin(),
                                endSubScore = subScores->begin() + range.second;
...
Core::Choice semiringCombinationTypeChoice(
    "discard", SemiringCombinationHelper::TypeDiscard, "concatenate",
    SemiringCombinationHelper::TypeConcatenate, Core::Choice::endMark());
} // namespace
const Core::ParameterString SemiringCombinationHelper::paramType("type", "type",
                                                                 "concatenate");
SemiringCombinationHelper::Type
SemiringCombinationHelper::getType(const std::string &s) {
  ...
  if (comboType == Core::Choice::IllegalValue)
    Core::Application::us()->criticalError(
        "Unknown semiring combination type \"%s\"", s.c_str());
  return Type(comboType);
}

ScoreId SemiringCombinationHelper::getIdOrDie(ConstSemiringRef semiring,
                                              const Key &key) {
  verify_(semiring);
...
SemiringCombinationHelperRef
SemiringCombinationHelper::create(SemiringCombinationHelper::Type comboType,
                                  const KeyList &comboKeys,
                                  const ScoreList &comboScales) {
  verify(comboScales.empty() || (comboScales.size() == comboKeys.size()));
  switch (comboType) {
  case TypeDiscard:
    return SemiringCombinationHelperRef(new SemiringCombinationHelper(
        new DiscardSemiringCombination(comboKeys, comboScales)));
  case TypeConcatenate:
    return SemiringCombinationHelperRef(new SemiringCombinationHelper(
        new ConcatenateSemiringCombination(comboKeys, comboScales)));
  default:
    defect();
...
SemiringCombinationHelper::~SemiringCombinationHelper() { delete internal_; }
...
ConstSemiringRef
SemiringCombinationHelper::subSemiring(u32 subSemiringIndex) const {
...
void SemiringCombinationHelper::set(ScoresRef scores, ScoreId comboIndex,
                                    Score score) const {
  verify_(comboIndex < internal_->comboIds.size());
  scores->set(internal_->comboIds[comboIndex], score);
}

ScoreId SemiringCombinationHelper::subId(ScoreId subSemiringIndex,
                                         ScoreId subScoreIndex) const {
...
void SemiringCombinationHelper::set(ScoresRef scores, ScoreId subSemiringIndex,
                                    ScoreId subScoreIndex,
                                    Score subScore) const {
...
// -------------------------------------------------------------------------
namespace {
void verifyAlphabet(Lexicon::AlphabetId alphabetId,
  ...
  if (Lexicon::us()->alphabetId(alphabet) != alphabetId)
    Core::Application::us()->criticalError(
        ...
        Lexicon::us()
            ->alphabetName(Lexicon::us()->alphabetId(alphabet))
            .c_str());
}
} // namespace

CombinationHelper::CombinationHelper(SemiringCombinationHelper::Type comboType,
                                     const KeyList &comboKeys,
                                     const ScoreList &comboScales) {
  semiringCombination_ =
      SemiringCombinationHelper::create(comboType, comboKeys, comboScales);
}

CombinationHelperRef
CombinationHelper::create(SemiringCombinationHelper::Type comboType,
                          const KeyList &comboKeys,
                          const ScoreList &comboScales) {
  return CombinationHelperRef(
      new CombinationHelper(comboType, comboKeys, comboScales));
}

bool CombinationHelper::update(const ConstLatticeRefList &lats) {
  verify(!lats.empty());
  type_ = lats.front()->type();
  Lexicon::AlphabetId inputAlphabetId =
      Lexicon::us()->alphabetId(lats.front()->getInputAlphabet());
  Lexicon::AlphabetId outputAlphabetId =
  ...
  for (ConstLatticeRefList::const_iterator itLat = lats.begin();
       itLat != lats.end(); ++itLat, ++itSemiring) {
    verify((*itLat));
...
bool CombinationHelper::update(const ConstConfusionNetworkRefList &cns) {
  ...
  ConstSemiringRefList::iterator itSemiring = semirings.begin();
  for (ConstConfusionNetworkRefList::const_iterator itCn = cns.begin();
       itCn != cns.end(); ++itCn, ++itSemiring) {
    verify((*itCn));
