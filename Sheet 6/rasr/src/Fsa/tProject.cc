...
public:
  ProjectInputAutomaton(_ConstAutomatonRef f) : Precursor(f) {
    this->unsetProperties(Fsa::PropertySortedByArc |
...
typename _Automaton::ConstRef projectInput(typename _Automaton::ConstRef f) {
  if (f->type() == Fsa::TypeAcceptor)
    return f;
  return
      typename _Automaton::ConstRef(new ProjectInputAutomaton<_Automaton>(f));
}
...
public:
  ProjectOutputAutomaton(_ConstAutomatonRef f) : Precursor(f) {
    this->unsetProperties(Fsa::PropertySortedByArc |
                          Fsa::PropertySortedByInput |
                          Fsa::PropertySortedByInputAndTarget);
...
typename _Automaton::ConstRef projectOutput(typename _Automaton::ConstRef f) {
  if (f->type() == Fsa::TypeAcceptor)
    return f;
  return
      typename _Automaton::ConstRef(new ProjectOutputAutomaton<_Automaton>(f));
}
