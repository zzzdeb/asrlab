...
public:
   ...
   * type of covariance tying is given in @param Configuration
   * (paramCovarianceTyingType)
   */
  Core::Ref<MixtureSet> createMixtureSet(const Core::Configuration &,
                                         size_t nMixtures, size_t dimension,
                                         size_t nDensitiesPerMixture = 1) const;
  /**
   * Creates an empty mixture set with @param nMixtures of @param dimension.
   */
  Core::Ref<MixtureSet> createMixtureSet(CovarianceTyingType covarianceTying,
                                         size_t nMixtures, size_t dimension,
                                         size_t nDensitiesPerMixture = 1) const;
  /**
   ...
   */
  Core::Ref<MixtureSet>
  createEmptyMixtureSet(const Core::Configuration &) const;

  /**
   *  Loads a MixtureSet object from the file @param file.
   */
  Core::Ref<MixtureSet> readMixtureSet(const std::string &file,
                                       const Core::Configuration &c) const;

  /**
   * Loads a MixtureSet object from the file given in @param Configuration
   * (paramMixtureSetFilename).
   */
  Core::Ref<MixtureSet> readMixtureSet(const Core::Configuration &c) const;
  void convertMixtureSet(const Core::Configuration &) const;
  /**
   * Load mixture set from the file given in @param Configuration
   * (paramMixtureSetFilename). Type of mixture set depends on
   * paramFeatureScorerType
   */
  Core::Ref<AbstractMixtureSet>
  readAbstractMixtureSet(const Core::Configuration &configuration) const;
  /**
   * Load mixture set from file @param filename.
   * Type of mixture set depends on paramFeatureScorerType
   */
  Core::Ref<AbstractMixtureSet>
  readAbstractMixtureSet(const std::string &filename,
                         const Core::Configuration &configuration) const;

  bool writeMixtureSet(const std::string &filename,
                       const AbstractMixtureSet &ms, u32 precision = 6) const;

  /** Creates an empty mixture set estimator. */
  AbstractMixtureSetEstimator *
  createMixtureSetEstimator(const Core::Configuration &) const;
#ifdef MODULE_MM_DT
  /** Creates an empty discriminative mixture set estimator. */
  DiscriminativeMixtureSetEstimator *
  createDiscriminativeMixtureSetEstimator(const Core::Configuration &) const;
#endif
  /** Creates an empty mixture set estimator with @param nMixtures of @param
   * dimension. */
  AbstractMixtureSetEstimator *
  createMixtureSetEstimator(const Core::Configuration &, size_t nMixtures,
                            size_t dimension,
                            size_t nDensitiesPerMixture = 1) const;

  /** Read a MixtureSetEstimator object from file */
  bool readMixtureSetEstimator(const std::string &filename,
                               AbstractMixtureSetEstimator &);
  /** Saves a MixtureSetEstimator object in the file @param filename. */
  bool writeMixtureSetEstimator(const std::string &filename,
                                AbstractMixtureSetEstimator &);

  /** Creates and initializes a FeatureScorer as configured.
   *  @return a newly created instance of FeatureScorer or 0 if an error
   * occured.
   */
  Core::Ref<FeatureScorer>
  createFeatureScorer(const Core::Configuration &c,
                      const Core::Ref<const AbstractMixtureSet> m) const;

  /** Creates and initializes an AssigningFeatureScorer as configured.
   *  @return a newly created instance of AssigningFeatureScorer or 0 if an
   * error occured.
   */
  Core::Ref<AssigningFeatureScorer>
  createAssigningFeatureScorer(const Core::Configuration &,
                               const Core::Ref<const AbstractMixtureSet>) const;

  /** Loads a mixture set and creates and initializes a FeatureScorer with it,
   * as configured.
   *  @return a newly created instance of FeatureScorer or 0 if an error
   * occured.
   */
  Core::Ref<FeatureScorer>
  createFeatureScorer(const Core::Configuration &c) const;

  /** Creates a ScaledFeatureScorer by wraping @param featureScorer.
   *  @return a newly created instance of ScaledFeatureScorer or 0 if an error
   * occured.
   */
  Core::Ref<ScaledFeatureScorer>
  createScaledFeatureScorer(const Core::Configuration &c,
  ...
  Core::Ref<ScaledFeatureScorer>
  createScaledFeatureScorer(const Core::Configuration &c,
                            const Core::Ref<Mm::AbstractMixtureSet> m) const {
    return createScaledFeatureScorer(c, createFeatureScorer(c, m));
  }
  /** Loads a mixture set and creates and initializes a ScaledFeatureScorer with
   * it, as configured.
   *  @return a newly created instance of ScaledFeatureScorer or 0 if an error
   * occured.
   */
  Core::Ref<ScaledFeatureScorer>
  createScaledFeatureScorer(const Core::Configuration &c) const {
