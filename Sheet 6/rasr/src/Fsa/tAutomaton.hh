...
/*
  ...
  Arc();
  Arc(Fsa::StateId target, Weight weight, Fsa::LabelId input, Fsa::LabelId
  output);
 ...
 */
template <class _Arc> class VectorArcContainer : public std::vector<_Arc> {
  typedef _Arc Arc;
...
public:
  ...
  Arc *addArc(Fsa::StateId target, typename _Arc::Weight weight,
              Fsa::LabelId input) {
 ...
 */
template <class _Arc> class ArrayArcContainer {
  typedef _Arc Arc;
 ...
 **/
template <class _Arc,
          template <class T> class _ArcContainer = VectorArcContainer>
...
public:
  ...
  typedef typename ArcContainer::const_reverse_iterator const_reverse_iterator;
  Weight weight_;
...
public:
  ...
  State(const State<_Arc, _OtherArcContainer> &o)
      : idAndTags_((o.id() & Fsa::StateIdMask) |
                   (o.tags() & Fsa::StateTagMask)),
        weight_(o.weight_) {
  ...
  Fsa::StateId id() const { return idAndTags_ & Fsa::StateIdMask; }
  ...
  bool hasTags(Fsa::StateTag tags) const { return idAndTags_ & tags; }
  Fsa::StateTag tags() const { return idAndTags_ & Fsa::StateTagMask; }
  bool isFinal() const { return hasTags(Fsa::StateTagFinal); }
  void setFinal(const Weight &weight) {
    addTags(Fsa::StateTagFinal);
    weight_ = weight;
  }
  void unsetFinal() { idAndTags_ &= ~Fsa::StateTagFinal; }

  const Arc *operator[](unsigned i) const { return &(arcs_[i]); }
  const Arc *getArc(unsigned i) const { return &(arcs_[i]); }

  /// Warning: the returned pointer can become invalid after a new call of
  /// newArc()
  Arc *newArc();
  Arc *newArc(Fsa::StateId target, Weight weight, Fsa::LabelId input);
  Arc *newArc(Fsa::StateId target, Weight weight, Fsa::LabelId input,
              Fsa::LabelId output);
  bool hasArcs() const { return !arcs_.empty(); }
  u32 nArcs() const { return arcs_.size(); }
  iterator begin() { return arcs_.begin(); }
  iterator end() { return arcs_.end(); }
  const_iterator begin() const { return arcs_.begin(); }
  const_iterator end() const { return arcs_.end(); }
  reverse_iterator rbegin() { return arcs_.rbegin(); }
  reverse_iterator rend() { return arcs_.rend(); }
  const_reverse_iterator rbegin() const { return arcs_.rbegin(); }
  const_reverse_iterator rend() const { return arcs_.rend(); }
  void truncate(iterator i) { arcs_.truncate(i); }

  void resize(size_t s) { arcs_.resize(s); }
  void minimize() { arcs_.minimize(); }
  iterator insert(iterator pos, const Arc &a) { return arcs_.insert(pos, a); }
  template <class Predicate> void remove(Predicate pred) {
    arcs_.erase(std::remove_if(arcs_.begin(), arcs_.end(), pred), arcs_.end());
  }
  template <class T, class StrictWeakOrdering>
  iterator lower_bound(const T &value, StrictWeakOrdering comp) {
    return std::lower_bound(arcs_.begin(), arcs_.end(), value, comp);
  }
  template <class T, class StrictWeakOrdering>
  const_iterator lower_bound(const T &value, StrictWeakOrdering comp) const {
    return std::lower_bound(arcs_.begin(), arcs_.end(), value, comp);
  }
 ...
 * automaton classes therefore derive from this class and overload the
 *appropriate methods.
 **/
template <class _Semiring,
          class _State = State<Arc<typename _Semiring::Weight> > >
...
public:
  /*
   Attention:
   If you use a class inhereting from tAutomaton in conjunction with Ftl
   algorithms, you must redefine Ref and ConstRef.

   - todo: get rid of ::ConstRef and ::Ref by replacing them by explicit
   Core::Ref<...> constructs (has to be done in all t-files)
   */
  typedef Core::Ref<Self> Ref;
  typedef Core::Ref<const Self> ConstRef;
...
protected:
  Automaton() : knownProperties_(0), properties_(0) {}
...
public:
   ...
   **/
  virtual Fsa::Type type() const { return Fsa::TypeUnknown; }
  ...
  /** Declare properties as known and their (known) values, i.e. set or unset
   * these properties. */
   ...
   * further computation.
   **/
  Fsa::Property knownProperties() const { return knownProperties_; }
   ...
   **/
  Fsa::Property properties() const { return properties_; }
   ...
   **/
  virtual Fsa::StateId initialStateId() const { return Fsa::InvalidStateId; }

  /**
   * Returns a reference to the input alphabet.
   * @return returns a reference to the lazy input alphabet or an empty
   *reference if the automaton provides no input alphabet
   **/
  virtual Fsa::ConstAlphabetRef getInputAlphabet() const = 0;
  Fsa::ConstAlphabetRef inputAlphabet() const { return getInputAlphabet(); }

  /**
   * Returns a reference to the output alphabet.
   * In case of an acceptor this method should return the same reference than
   * getInputAlphabet().
   * @return returns a reference to the lazy output alphabet or an empty
   *reference if the automaton provides no output alphabet
   **/
  virtual Fsa::ConstAlphabetRef getOutputAlphabet() const {
    return Fsa::ConstAlphabetRef();
  }
  Fsa::ConstAlphabetRef outputAlphabet() const { return getOutputAlphabet(); }
   ...
   * state ids on arcs).
   * @warning Note that in case of lazy algorithms calling this method may take
   *some time as states, i.e. mainly the list of outgoing arcs, are calculated
   *on-demand.
   ...
   **/
  virtual void dumpState(Fsa::StateId s, std::ostream &o) const { o << s; }
...
public:
  ...
  virtual Fsa::StateId initialStateId() const { return fsa_->initialStateId(); }
  ...
  virtual size_t getMemoryUsed() const { return fsa_->getMemoryUsed(); }
  virtual void dumpMemoryUsage(Core::XmlWriter &o) const {
...
template <class _AutomatonFrom, class _AutomatonTo>
WrapperAutomaton<_AutomatonFrom, _AutomatonTo>::WrapperAutomaton(
...
template <class _AutomatonFrom, class _AutomatonTo>
void WrapperAutomaton<_AutomatonFrom, _AutomatonTo>::setMaster(
...
protected:
  ...
  SlaveAutomaton(_ConstAutomatonRef fsa, Fsa::Property mask = Fsa::PropertyAll);

public:
  virtual ~SlaveAutomaton() {}
  virtual Fsa::Type type() const { return fsa_->type(); }
  virtual _ConstSemiringRef semiring() const { return fsa_->semiring(); }
  virtual Fsa::StateId initialStateId() const { return fsa_->initialStateId(); }
  virtual Fsa::ConstAlphabetRef getInputAlphabet() const {
    return fsa_->getInputAlphabet();
  }
...
SlaveAutomaton<_Automaton>::SlaveAutomaton(Core::Ref<const _Automaton> f,
                                           Fsa::Property mask) {
  setFsa(f, mask);
  setMaster(f);
}
template <class _Automaton>
void SlaveAutomaton<_Automaton>::setMaster(Core::Ref<const _Automaton> f) {}
