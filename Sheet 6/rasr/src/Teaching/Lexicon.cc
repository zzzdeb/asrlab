...
Lexicon::Lexicon(Bliss::LexiconRef lexicon, AcousticModelRef am)
    : lexicon_(lexicon), acousticModel_(am),
      pronunciations_(lexicon_->lemmaPronunciations().first) {
  LemmaPronunciationIterator si =
      std::find(lexicon_->lemmaPronunciations().first,
                lexicon_->lemmaPronunciations().second,
...
Word Lexicon::silence() const { return silence_; }
...
bool Lexicon::getMixtureSequence(Word w, MixtureSequence &mixtures) const {
  ...
  Core::Ref<const Am::AllophoneAlphabet> allophoneAlphabet =
      acousticModel_->allophoneAlphabet();
  Core::Ref<const Am::AllophoneStateAlphabet> allophoneStateAlphabet =
  ...
  for (unsigned short p = 0; p < pron->length(); ++p) {
    ...
    const Am::Allophone *allo = allophoneAlphabet->allophone(
        Am::Allophone(phonology(*pron, p), boundary));
    verify(allo);

    const Am::ClassicHmmTopology *hmmTopology =
        acousticModel_->hmmTopology((*pron)[p]);
    verify(hmmTopology);

    for (int s = 0; s < hmmTopology->nPhoneStates(); ++s) {
      Am::AllophoneState allophoneState =
          allophoneStateAlphabet->allophoneState(allo, s);
...
const Mixture Lexicon::silenceMixture() const {
  return acousticModel_->emissionIndex(
      acousticModel_->silenceAllophoneStateIndex());
}

const Am::Allophone *Lexicon::allophone(Word word,
                                        Bliss::Phoneme::Id id) const {
...
const MixtureSequence *Lexicon::mixtures(Word word,
                                         Bliss::Phoneme::Id id) const {
  MixtureSequence *mixtures = new MixtureSequence();

  const Bliss::Pronunciation &pron = *pronunciation(word)->pronunciation();
  const Am::ClassicHmmTopology *hmmTopology =
      acousticModel_->hmmTopology(pron[id]);

  // for all states of phoneme id
  for (int i = 0; i < hmmTopology->nPhoneStates(); i++) {
    Am::AllophoneState allophoneState =
        acousticModel_->allophoneStateAlphabet()->allophoneState(
            allophone(word, id), i);
    Mixture mixture = acousticModel_->emissionIndex(allophoneState);
