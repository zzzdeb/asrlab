...
const Core::ParameterFloat
    KeyedEstimator::paramCombinationWeight("combination-weight",
                                           "combination weight", 1.0);
...
const Core::ParameterString
    KeyedEstimator::paramCorpusKeyMap("corpus-key-map",
                                      "path of corpus-key-map file");

KeyedEstimator::KeyedEstimator(const Core::Configuration &c, Operation op)
    : Component(c), Precursor(c, Am::AcousticModel::noEmissions),
      corpusKey_(new Bliss::CorpusKey(select("corpus-key"))),
      featureDimension_(0), modelDimension_(0),
      featureStream_(paramFeatureStream(config)), operation_(op),
      currentAccumulator_(0),
      accumulatorCache_(select("accumulator-cache"),
                        op != estimate ? Core::reuseObjectCacheMode
                                       : Core::createObjectCacheMode) {
  if (op == combines) {
    Core::Configuration additionConfiguration(select("addition"),
                                              "accumulator-cache");
    accumulatorCacheToAdd_ =
        new AccumulatorCache(additionConfiguration, Core::reuseObjectCacheMode);
...
void KeyedEstimator::createAssigningFeatureScorer() {
  assigningFeatureScorer_ = Mm::Module::instance().createAssigningFeatureScorer(
      config, acousticModel()->mixtureSet());
}
...
void KeyedEstimator::setFeatureDescription(
    const Mm::FeatureDescription &description) {
  description[featureStream_].getValue(Mm::FeatureDescription::nameDimension,
                                       featureDimension_);
...
void KeyedEstimator::processAlignedFeature(Core::Ref<const Feature> f,
                                           Am::AllophoneStateIndex i) {
  ...
  (*currentAccumulator_)
      ->accumulate((*f)[featureStream_], density, mixture, mixtureSet_);
}
...
void KeyedEstimator::processAlignedFeature(Core::Ref<const Feature> f,
  ...
  Mm::DensityIndex density = mixtureSet_->mixture(mixture)->densityIndex(
      assigningFeatureScorer_->getAssigningScorer(f)->bestDensity(mixture));
  (*currentAccumulator_)
      ->accumulate((*f)[featureStream_], density, mixture, mixtureSet_, w);
}
...
void KeyedEstimator::combine() {
  ...
  for (std::set<std::string>::iterator key = keysToAdd.begin();
    ...
    log("Combining accumulator '")
        << *key << "' using weight '" << paramCombinationWeight(config) << "'";
    (*currentAccumulator_)
        ->combine(**accumulatorToAdd, paramCombinationWeight(config));
  }
}
