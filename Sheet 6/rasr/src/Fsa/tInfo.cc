...
public:
  CountDfsState(ConstAutomatonRef f) : Precursor(f) {}
...
public:
  InDegreeDfsState(ConstAutomatonRef f) : Precursor(f) {}
  const Core::Vector<u32> &nInDegree() const { return nInDegree_; }
...
public:
  ...
  size_t nLinearStates() const { return nLinearStates_; }
  virtual void discoverState(ConstStateRef sp) {
...
template <class _Automaton>
Core::Vector<u32> inDegree(typename _Automaton::ConstRef f,
                           bool progress = false) {
  Core::ProgressIndicator *p = 0;
  if (progress)
...
public:
  ...
  size_t matched() const { return nMatched_; }
};

template <class _Automaton>
size_t countInput(typename _Automaton::ConstRef f, Fsa::LabelId label,
                  bool progress) {
  Core::ProgressIndicator *p = 0;
  if (progress)
    p = new Core::ProgressIndicator("counting", "states");
...
template <class _Automaton>
size_t countOutput(typename _Automaton::ConstRef f, Fsa::LabelId label,
                   bool progress) {
  Core::ProgressIndicator *p = 0;
  if (progress)
    p = new Core::ProgressIndicator("counting", "states");
...
void basicInfo(typename _Automaton::ConstRef f, Core::XmlWriter &o) {
  o << Core::XmlFull("type", Fsa::TypeChoice[f->type()])
    << Core::XmlFull("describe", f->describe());
  std::string tmp;
