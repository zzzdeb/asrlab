...
XmlPropertiesDefinitionBuilderElement::XmlPropertiesDefinitionBuilderElement(
    ...
    : Precursor(name, context), mapPtr_(0) {
...
void XmlPropertiesDefinitionBuilderElement::value(
    const PropertyMap::IndexedString &pair) {
...
void XmlPropertiesDefinitionBuilderElement::characters(const char *ch,
                                                       int len) {}
// ============================================================================
...
XmlPropertiesBuilderElement::XmlPropertiesBuilderElement(
    const char *name, Core::XmlContext *context)
    : Precursor(name, context), mapRef_() {
  keyParser_ = new XmlStringBuilderElement("key", this);
  keyParser_->setParsedHandler(*this, &Self::key);
...
XmlQuestionRefListBuilderElement::XmlQuestionRefListBuilderElement(
    const char *name, Core::XmlContext *context)
    : Precursor(name, context), mapRef_(), questionRefs_(0),
      index_(Question::InvalidQuestionIndex), desc_(), key_(), values_(),
      isForEachKey_(false), isForEachValue_(false), keyList_(), valueList_() {
  ...
  questionElement_->addTransition(XmlRegularElementRelay::initial, 1,
                                  keyBuilder_);
  questionElement_->addTransition(XmlRegularElementRelay::initial, 2,
                                  valueBuilder_);
  questionElement_->addTransition(XmlRegularElementRelay::initial, 2,
                                  valuesBuilder_);
...
void XmlQuestionRefListBuilderElement::addQuestion(const std::string &key,
...
void XmlQuestionRefListBuilderElement::forEachValue(const std::string &key) {
  if (isForEachValue_) {
    ...
    if (values_.empty()) {
      parser()->warning("In element \"%s\": no range for question for key "
                        "\"%s\"; assume predicate",
                        name(), key.c_str());
...
void XmlQuestionRefListBuilderElement::key(const std::string &key) {
  if (!Core::strconv(key, key_))
    parser()->criticalError("In element \"%s\": non-parseable list \"%s\"",
                            name(), key.c_str());
}
...
void XmlQuestionRefListBuilderElement::startForEachKey(
    const Core::XmlAttributes attr) {
...
void XmlQuestionRefListBuilderElement::startForEachValue(
    const Core::XmlAttributes attr) {
  ...
  if ((c = attr["index"]))
    if (!Core::strconv(std::string(c), index_)) {
      parser()->criticalError(
          "In element \"%s\": non-interpretable value \"%s\"", name(), c);
...
XmlBinaryTreeRootBuilderElement::XmlBinaryTreeRootBuilderElement(
    const char *name, XmlContext *context, InformationBuilder *infoBuilder)
    : Precursor(name, context), node_(0), infoBuilder_(infoBuilder) {
  if (!infoBuilder_)
...
XmlDecisionTreeBuilderElement::XmlDecisionTreeBuilderElement(
    const char *name, XmlContext *context, InformationBuilder *infoBuilder)
    : Precursor(name, context), tree_(0), mapFromFile_(0) {

  propertyMapParser_ =
      new XmlPropertiesDefinitionBuilderElement("properties-definition", this);
  propertyMapParser_->setParsedHandler(*this, &Self::map);

  questionRefsBuilder_ =
      new XmlQuestionRefListBuilderElement("questions", this);
  ...
  binaryTreeElement_->addTransition(XmlRegularElement::initial, 1,
                                    rootBuilder_);
...
void XmlDecisionTreeBuilderElement::map(const PropertyMap &map) {
  if (mapFromFile_) {
    ...
    PropertyMapDiff diff(parser()->getConfiguration(), tree_->map(),
                         *mapFromFile_, true);
    if (diff.hasDifferences())
      parser()->warning("differences in property map of decision tree and "
                        "property map defined in file");
    delete mapFromFile_;
    mapFromFile_ = 0;
  }
...
void XmlDecisionTreeBuilderElement::questionRefs(
...
bool convert(const std::string &buffer, FloatBox &values) {
  ...
  char *end;
  for (FloatBox::vector_iterator it = values.begin(); it != values.end();
       ++it) {
    *it = ::strtod(start, &end);
    verify_(start != end);
    start = end;
...
// ============================================================================
XmlExampleListBuilderElement::XmlExampleListBuilderElement(const char *name,
                                                           XmlContext *context)
    : Precursor(name, context), example_(0), examples_(0), mapFromFile_(0) {
  ...
  exampleElement_->addTransition(XmlRegularElement::initial, 1,
                                 propertiesBuilder_);
...
void XmlExampleListBuilderElement::map(const PropertyMap &map) {
  if (mapFromFile_) {
    ...
    PropertyMapDiff diff(parser()->getConfiguration(), examples_->map(),
                         *mapFromFile_, true);
    if (diff.hasDifferences())
      parser()->warning("differences in property map of example list and "
                        "property map defined in file");
    delete mapFromFile_;
    mapFromFile_ = 0;
  }
...
void XmlExampleListBuilderElement::set(ExampleList *examples) {
  ...
  if (examples_->map().empty()) {
    propertyMapParser_->set(
        const_cast<PropertyMap *>(examples_->getMap().get()));
    mapFromFile_ = 0;
  } else {
    mapFromFile_ = new PropertyMap;
    propertyMapParser_->set(mapFromFile_);
...
struct XmlExampleListMergerElement::ExampleListMerger {
  ...
  typedef Core::hash_map<Properties *, Example *, Properties::PtrHashFcn,
                         Properties::PtrEqual>
      ExampleMap;
  ExampleMap exampleMap;

  ExampleListMerger(XmlExampleListMergerElement *father) : father(father) {
    for (ExampleList::iterator itExample = father->examples_->begin(),
                               endExample = father->examples_->end();
         itExample != endExample; ++itExample)
      if (!exampleMap
               .insert(std::make_pair((*itExample)->properties, &**itExample))
               .second)
        father->parser()->criticalError(
            "Initial example list contains examples eith equal properties.");
  }

  void merge(Example *example) {
    std::pair<ExampleMap::iterator, bool> result =
        exampleMap.insert(std::make_pair(example->properties, example));
    if (result.second) {
...
XmlExampleListMergerElement::XmlExampleListMergerElement(const char *name,
                                                         XmlContext *context)
    : Precursor(name, context), example_(0), examples_(0), mapFromFile_(0),
      merger_(0) {
...
XmlStepBuilderElement::XmlStepBuilderElement(const char *name,
                                             XmlContext *context)
    : Precursor(name, context), step_(0) {
...
void XmlStepBuilderElement::randomize(const XmlAttributes atts) {
  ...
  if (c) {
    ...
    if (!Core::strconv(s, step_->nRandomQuestion))
      parser()->criticalError("In element \"%s\": non-parseable integer \"%s\"",
                              name(), s.c_str());
...
XmlTrainingPlanBuilderElement::XmlTrainingPlanBuilderElement(
    const char *name, XmlContext *context)
    : Precursor(name, context), trainingPlan_(0), stepCount_(0),
      mapFromFile_(0) {
...
void XmlTrainingPlanBuilderElement::map(const PropertyMap &map) {
  if (mapFromFile_) {
    ...
    PropertyMapDiff diff(parser()->getConfiguration(), *trainingPlan_->map,
                         *mapFromFile_, true);
    if (diff.hasDifferences())
      parser()->warning("differences in property map of training and property "
                        "map defined in file");
    delete mapFromFile_;
    mapFromFile_ = 0;
  }
...
void XmlTrainingPlanBuilderElement::set(TrainingPlan *trainingPlan) {
  ...
  if (trainingPlan_->map->empty()) {
    propertyMapParser_->set(
        const_cast<PropertyMap *>(trainingPlan_->map.get()));
    mapFromFile_ = 0;
  } else {
    mapFromFile_ = new PropertyMap;
    propertyMapParser_->set(mapFromFile_);
