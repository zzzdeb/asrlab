...
const Core::ParameterInt LanguageModelLookahead::paramHistoryLimit(
    "history-limit",
    "length of history considered for look-ahead (effective m-grammity of the "
    "look-ahead model - 1)",
    1, 0);
const Core::ParameterInt LanguageModelLookahead::paramTreeCutoff(
    "tree-cutoff",
    "maximum depth of state tree covered by look-ahead (number of HMM state "
    "covered)",
    Core::Type<s32>::max, 0);
const Core::ParameterInt LanguageModelLookahead::paramMinimumRepresentation(
    "minimum-representation",
    "minimum number of HMM states represented by one look-ahead node", 1, 1);
const Core::ParameterInt LanguageModelLookahead::paramCacheSizeLow(
    "cache-size-low",
    "number of look-ahead tables retained before starting to re-use inactive "
    "tables",
    0, 0);
const Core::ParameterInt LanguageModelLookahead::paramCacheSizeHigh(
    "cache-size-high",
    "number of look-ahead tables allowed before starting to delete inactive "
    "tables",
    0, 0);
...
LanguageModelLookahead::LanguageModelLookahead(
    const Core::Configuration &c, Lm::Score wpScale,
    Core::Ref<const Lm::ScaledLanguageModel> lm, const StateTree *st)
    : Core::Component(c), wpScale_(wpScale), lm_(lm), batchRequest_(0),
      nTables_(0), nFreeTables_(0), statisticsChannel_(config, "statistics") {
...
public:
  bool isUnique() const { return consolidation_ >= unique; }
  ...
  void makeUnique() {
    ...
    successors_.erase(std::unique(successors_.begin(), successors_.end()),
                      successors_.end());
...
private:
  void updateDominiees() const {
    ...
    dominees_ = ends_;
    Ends curr;
    for (Successors::const_iterator s = successors_.begin();
         s != successors_.end(); ++s) {
      dominees_.swap(curr);
      dominees_.clear();
      const Ends &succ((*s)->dominees());
      std::set_union(curr.begin(), curr.end(), succ.begin(), succ.end(),
...
public:
  ConstructionNode() : id(invalidId), consolidation_(dirty) {
  ...
  const Ends &ends() const { return ends_; }
  Ends &ends() {
...
protected:
  struct DominationHash {
    u32 operator()(const ConstructionNode *n) const { return n->hash(); }
  };

private:
  struct DominationEquality {
    bool operator()(const ConstructionNode *l,
                    const ConstructionNode *r) const {
...
public:
  ...
  void purge();
  LookaheadId nNodes() const { return nodeList_.size(); }
...
LanguageModelLookahead::ConstructionTree::~ConstructionTree() {
  for (NodeList::const_iterator i = nodeList_.begin(); i != nodeList_.end();
       ++i)
    delete *i;
}
...
bool LanguageModelLookahead::ConstructionTree::isWellOrdered() const {
  ...
  for (u32 ci = 0; ci < nodeList_.size(); ++ci) {
    ...
    if (cn.id == invalidId)
      continue;
    verify(cn.id == ci);
    for (ConstructionNode::Successors::const_iterator si =
             cn.successors().begin();
         si != cn.successors().end(); ++si) {
...
void LanguageModelLookahead::ConstructionTree::writeStatistics(
  ...
  for (std::map<StateTree::Depth, LevelStatistics>::const_iterator l =
           levels.begin();
       l != levels.end(); ++l) {
    ...
    os << Core::form("level %3d: %6d nodes, branching factor %3.2f, %4d ends\n",
                     depth, thisLevel.nNodes,
                     float(thisLevel.nSuccessors) / float(thisLevel.nNodes),
                     thisLevel.nEnds);
  }
}
...
void LanguageModelLookahead::ConstructionTree::build(const StateTree *st) {
  ...
  typedef Core::hash_set<ConstructionNode *, ConstructionNode::DominationHash,
...
bool LanguageModelLookahead::shouldPruneConstructionNode(
    const ConstructionNode &sn) const {
...
void LanguageModelLookahead::ConstructionTree::prune(
  ...
  for (NodeList::iterator ci = nodeList_.begin(); ci != nodeList_.end(); ++ci) {
    ...
    for (ConstructionNode::Successors::iterator si = cn.successors().begin();
         si != cn.successors().end(); ++si) {
      ConstructionNode &sn(**si);
      if (sn.id == invalidId || master->shouldPruneConstructionNode(sn)) {
        std::copy(sn.ends().begin(), sn.ends().end(),
                  std::back_insert_iterator<Ends>(cn.ends()));
        std::copy(sn.successors().begin(), sn.successors().end(),
                  std::back_insert_iterator<ConstructionNode::Successors>(
                      newSuccessors));
        std::copy(sn.represents.begin(), sn.represents.end(),
                  std::back_insert_iterator<ConstructionNode::Represents>(
                      cn.represents));
...
void LanguageModelLookahead::ConstructionTree::purge() {
  ...
  // reassign ids
  nodeList_.erase(
      std::remove(nodeList_.begin(), nodeList_.end(), (ConstructionNode *)0),
      nodeList_.end());
...
void LanguageModelLookahead::buildCompressesLookaheadStructure(
    const StateTree *st, const ConstructionTree &ct) {
  ...
  for (LookaheadId ci = 0; ci < ct.nNodes(); ++ci) {
    ...
    std::copy(cn.ends().begin(), cn.ends().end(),
              std::back_insert_iterator<Ends>(ends_));
    for (ConstructionNode::Successors::const_iterator si =
             cn.successors().begin();
         si != cn.successors().end(); ++si)
      successors_.push_back((*si)->id);
    for (std::vector<StateTree::StateId>::const_iterator ri =
             cn.represents.begin();
         ri != cn.represents.end(); ++ri)
      nodeId_[*ri] = ci;
  }
...
void LanguageModelLookahead::buildBatchRequest() {
  ...
  for (u32 n = 0; n < nEntries_; ++n) {
    for (Ends::const_iterator e = ends_.begin() + nodes_[n].firstEnd,
      ...
      for (u32 ti = 0; ti < request.tokens.length(); ++ti)
        request.offset +=
            lm_->scale() * request.tokens[ti]->classEmissionScore();
...
void LanguageModelLookahead::draw(std::ostream &os) const {
  ...
  for (u32 ni = 0; ni < nEntries_; ++ni) {
    ...
    for (StateTree::StateId si = 0; si < StateTree::StateId(nodeId_.size());
         ++si)
      if (nodeId_[si] == ni)
        os << Core::form("%d ", si);
    for (u32 e = n.firstEnd; e < nodes_[ni + 1].firstEnd; ++e)
      os << Core::form("\\n%s",
                       ends_[e]->lemma()->preferredOrthographicForm().str());
    os << Core::form("\"]\n");
...
void LanguageModelLookahead::computeScores(const Lm::History &history,
                                           std::vector<Score> &scores) const {
  require(scores.size() == nEntries_);

  //  log("computing look-ahead table for history ") <<
  //  lm_->formatHistory(history);
  ...
  for (std::vector<Node>::const_iterator n = nodes_.begin();
       n != nodes_.end() - 1; ++n) {
...
LanguageModelLookahead::ContextLookahead::ContextLookahead(
    const LanguageModelLookahead *la, const Lm::History &_history, u32 nEntries)
    : la_(la), history_(_history), freePos_(la->freeTables_.end()),
      scores_(nEntries) {}

LanguageModelLookahead::ContextLookahead *
LanguageModelLookahead::acquireTable(const Lm::History &h) const {
...
LanguageModelLookahead::ContextLookahead *
LanguageModelLookahead::getCachedTable(const Lm::History &h) const {
  ContextLookahead *t = 0;
...
const Core::Choice LanguageModelLookahead::CacheStatistics::cacheEventChoice(
    ...
    "number of table calculations ", cacheMiss, Core::Choice::endMark());
...
void LanguageModelLookahead::CacheStatistics::write(Core::XmlWriter &os) const {
  os << Core::XmlOpen("language-model-lookahead-cache-statistics")
     << cacheEvents << nActiveTables << nTables
