...
public:
  ...
  static const Core::ParameterFloat
      paramAcousticPruningThresholdIncrementFactor;
  ...
  private:
    ...
    Core::Ref<const Bliss::LemmaPronunciationAlphabet>
        lemmaPronunciationAlphabet_;
  ...
  private:
    ...
    addWordBoundaryDisambiguators(Fsa::ConstAutomatonRef alignmentFsa) const;
  ...
  public:
    WordLatticeBuilder(const Core::Configuration &, Bliss::LexiconRef,
     ...
     *  output labels.
     *  The shortest path of the composition gives finally to necesary
     * informations (e.i. allophone states, lemma-pronunciations, and acoustic
     * scores) to create a linear word lattice from an alignment traceback.
     */
...
public:
   ...
   *  In either case, an allophone-state acceptor with arc weights being
   * acoustic scores is returned. If the aligner does not reach a final state
   * due pruning settings which are too restrictive, an empty automaton is
   * returned.
   */
  ...
  getAlignmentPosteriorFsa(Fsa::ConstAutomatonRef alignmentFsa) const;
...
public:
  ...
  Speech::Alignment align(const Mm::FeatureScorer &,
