...
// -------------------------------------------------------------------------
const Core::ParameterString FsaDescriptor::paramFormat("format", "format", "");
const Core::ParameterString FsaDescriptor::paramFile("file", "file name", "");
...
FsaDescriptor FsaDescriptor::joinPaths(const FsaDescriptor &desc,
...
std::string FlfDescriptor::info() const {
  ...
  oss << "lattice descriptor:" << std::endl
      << "  semiring type:         " << getSemiringTypeName(semiringType)
      << std::endl
  ...
  if (hasInputAlphabet) {
    ...
    if (inputAlphabetFile)
      oss << "  input-alphabet file:   " << inputAlphabetFile(latticeRoot)
          << std::endl;
    else
  ...
  if (hasOutputAlphabet) {
    ...
    if (outputAlphabetFile)
      oss << "  output-alphabet file:   " << outputAlphabetFile(latticeRoot)
          << std::endl;
    else
  ...
  if (hasBoundaries)
    oss << "  boundaries file:       " << boundariesFile(latticeRoot)
        << std::endl;
  else
...
void append(FlfDescriptorRef desc, const KeyList &keys,
            const ScoreList &scales) {
...
// -------------------------------------------------------------------------
FlfContext::FlfContext() { clear(); }

FlfContext::~FlfContext() { clear(); }

ConstSemiringRef FlfContext::semiring() const { return semiring_; }

void FlfContext::setSemiring(ConstSemiringRef semiring) {
  semiring_ = semiring;
}

Fsa::Type FlfContext::fsaType() const { return fsaType_; }

void FlfContext::setFsaType(Fsa::Type fsaType) { fsaType_ = fsaType; }
...
namespace {
std::pair<Fsa::ConstAlphabetRef, Fsa::ConstAlphabetRef>
loadAlphabets(const FsaDescriptor &qf, Core::Archive *archive) {
  if (!qf)
  ...
  if (!is || !Fsa::read(&s, qf.format, is)) {
    Core::Application::us()->criticalError("Failed to read fsa \"%s\"",
                                           qf.file.c_str());
    return std::make_pair(Fsa::ConstAlphabetRef(), Fsa::ConstAlphabetRef());
  }
...
Lexicon::AlphabetMapRef mapAlphabet(Fsa::ConstAlphabetRef alphabet,
...
void FlfContext::setInputAlphabet(Fsa::ConstAlphabetRef alphabet,
                                  const std::string &alphabetName) {
...
void FlfContext::setOutputAlphabet(Fsa::ConstAlphabetRef alphabet,
                                   const std::string &alphabetName) {
...
void FlfContext::loadInputAlphabet(const FsaDescriptor &alphabetQf,
                                   Core::Archive *archive,
                                   const std::string &alphabetName) {
  require(alphabetQf);
  std::pair<Fsa::ConstAlphabetRef, Fsa::ConstAlphabetRef> alphabets =
      loadAlphabets(alphabetQf, archive);
  Fsa::ConstAlphabetRef alphabet =
      (alphabets.first) ? alphabets.first : alphabets.second;
  if (!alphabet)
    Core::Application::us()->criticalError(
        "Failed to read input alphabet from \"%s\"", alphabetQf.file.c_str());
  setInputAlphabet(alphabet, alphabetName);
}

void FlfContext::loadOutputAlphabet(const FsaDescriptor &alphabetQf,
                                    Core::Archive *archive,
                                    const std::string &alphabetName) {
  require(alphabetQf);
  std::pair<Fsa::ConstAlphabetRef, Fsa::ConstAlphabetRef> alphabets =
      loadAlphabets(alphabetQf, archive);
  Fsa::ConstAlphabetRef alphabet =
      (alphabets.second) ? alphabets.second : alphabets.first;
  if (!alphabet)
    Core::Application::us()->criticalError(
        "Failed to read output alphabet from \"%s\"", alphabetQf.file.c_str());
  setOutputAlphabet(alphabet, alphabetName);
}

void FlfContext::overhaul(FlfDescriptorRef desc,
                          const Fsa::StaticAutomaton &structureFsa) const {
  if (!semiring_)
    Core::Application::us()->criticalError("FlfContext: No semiring set");
  if (desc->scoreFiles.size() != semiring_->size())
    Core::Application::us()->criticalError(
        "FlfContext: Semiring dimension mismatch, expected %d, got %d",
        desc->scoreFiles.size(), (u32)semiring_->size());
  if (fsaType_ == Fsa::TypeUnknown)
    Core::Application::us()->criticalError("FlfContext: No fsa type set");
...
void FlfContext::adapt(FlfDescriptorRef desc,
                       const Fsa::StaticAutomaton &structureFsa,
                       Core::Archive *archive) {
  // adapt semiring
  if (!semiring_) {
    ...
    if ((type != Fsa::SemiringTypeTropical) && (type != Fsa::SemiringTypeLog)) {
      Core::Application::us()->warning(
          "FlfContext: No or unknown semiring type specified, assume tropical "
          "semiring");
      type = Fsa::SemiringTypeTropical;
    }
    setSemiring(Semiring::create(type, desc->scoreFiles.size(), desc->scales,
                                 desc->keys));
  ...
  if (fsaType_ == Fsa::TypeUnknown) {
    ...
    if ((bool)desc->inputAlphabetFile)
      loadInputAlphabet(
          ...
          archive, desc->inputAlphabetName);
    else
      setInputAlphabet(structureFsa.getInputAlphabet(),
                       desc->inputAlphabetName);
    if (structureFsa.type() != Fsa::TypeAcceptor) {
      if ((bool)desc->outputAlphabetFile) {
        loadOutputAlphabet(FsaDescriptor::joinPaths(desc->outputAlphabetFile,
                                                    desc->latticeRoot),
                           archive, desc->outputAlphabetName);
      } else
        setOutputAlphabet(structureFsa.getOutputAlphabet(),
...
void FlfContext::update(FlfDescriptorRef desc,
                        const Fsa::StaticAutomaton &structureFsa,
                        Core::Archive *archive) {
  clear();
...
std::string FlfContext::info() const {
  ...
  oss << "lattice context:" << std::endl
      << "  semiring:              " << ((semiring_) ? semiring_->name() : "")
      << std::endl
      << "  fsa type:              " << Fsa::TypeChoice[fsaType_] << std::endl;
  if (inputAlphabetMap_)
    oss << "  input alphabet name:   "
        << Lexicon::us()->alphabetName(
               Lexicon::us()->alphabetId(inputAlphabetMap_->to()))
        << std::endl;
  else
    oss << "  no input alphabet" << std::endl;
  if (outputAlphabetMap_)
    oss << "  output alphabet name:   "
        << Lexicon::us()->alphabetName(
               Lexicon::us()->alphabetId(outputAlphabetMap_->to()))
        << std::endl;
  else
    oss << "  no output alphabet" << std::endl;
  return oss.str();
...
private:
  ...
  void clearCdata(const Core::XmlAttributes atts) { cdata_.clear(); }
    ...
    if (c) {
      if (*c != '\0') {
        ...
        if (desc_->semiringType == Fsa::SemiringTypeUnknown)
          parser()->error(
              "semiring must be of type \"log\" or \"tropical\" not  \"%s\"",
              c);
      } else
  ...
  void dim(const Core::XmlAttributes atts) {
    ...
    if (n_ == 0) {
      ...
      parser()->error("id %d found, but dimensionality is set to %d", id_,
                      (u32)n_);
  }
...
public:
  FlfDescriptorXmlParser(const Core::Configuration &config)
    ...
    Core::XmlMixedElement *rootElement = new Core::XmlMixedElementRelay(
        ...
        XML_CHILD(new Core::XmlIgnoreElement("head", this)),
        XML_CHILD(new Core::XmlEmptyElementRelay(
            "alphabet", this, startHandler(&Self::inputAlphabetFile))),
        XML_CHILD(new Core::XmlEmptyElementRelay(
            "input-alphabet", this, startHandler(&Self::inputAlphabetFile))),
        XML_CHILD(new Core::XmlEmptyElementRelay(
            "output-alphabet", this, startHandler(&Self::outputAlphabetFile))),
        XML_CHILD(new Core::XmlEmptyElementRelay(
            "boundaries", this, startHandler(&Self::boundariesFile))),
        XML_CHILD(new Core::XmlEmptyElementRelay(
            "structure", this, startHandler(&Self::structureFile))),
        XML_CHILD(new Core::XmlMixedElementRelay(
            ...
            XML_CHILD(new Core::XmlMixedElementRelay(
                ...
                XML_CHILD(new Core::XmlMixedElementRelay(
                    ...
                    endHandler(&Self::name),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlMixedElementRelay(
                    "scale", this, startHandler(&Self::clearCdata),
                    endHandler(&Self::scale),
                    charactersHandler(&Self::appendCdata),
                    XML_NO_MORE_CHILDREN)),
                XML_CHILD(new Core::XmlEmptyElementRelay(
...
const Core::ParameterString
    FlfReader::paramContextMode("context-mode", "context mode", "update");
const Core::Choice ContextModeChoice =
    Core::Choice("trust", 0, "adapt", 1, "update", 5, Core::Choice::endMark());

const Core::ParameterStringVector FlfReader::paramKeys("keys",
                                                       "sequence of keys");
const Core::ParameterFloat FlfReader::paramScale("scale", "scale",
...
FlfReader::FlfReader(const Core::Configuration &config)
  ...
  if (contextHandling == Core::Choice::IllegalValue)
    error("Unknown context mode \"%s\"", paramContextMode(config).c_str());
  else
    setContextHandling(contextHandling);
  init();
}

FlfReader::FlfReader(const Core::Configuration &config, FlfContextRef context,
                     u32 contextHandling)
...
void FlfReader::init() {
  ...
  for (KeyList::const_iterator it = partialKeys.begin();
       it != partialKeys.end(); ++it)
    partialKeys_.insert(*it);
  const Core::Configuration appendConfig(config, "append");
  appendKeys_ = paramKeys(appendConfig);
  for (KeyList::const_iterator it = appendKeys_.begin();
       it != appendKeys_.end(); ++it) {
...
FlfContextRef FlfReader::context() const { return context_; }
...
u32 FlfReader::contextHandling() const { return contextHandling_; }

FlfDescriptorRef FlfReader::readDescriptor(const std::string &filename,
                                           Core::Archive *archive) {
...
void FlfReader::conform(FlfDescriptorRef desc,
                        const Fsa::StaticAutomaton &structureFsa,
                        Core::Archive *archive) {
...
void FlfReader::addStructure(StaticLattice &l, const Fsa::StaticAutomaton &f) {
  ...
  Lexicon::AlphabetMap &mapOutputLabel =
      (context_->fsaType() == Fsa::TypeAcceptor)
          ? *context_->inputAlphabetMap()
          : *context_->outputAlphabetMap();
    ...
    if (fsaSp) {
      State *sp = new State(fsaSp->id(), fsaSp->tags(),
                            fsaSp->isFinal()
                                ...
                                : semiring->one());
      for (Fsa::State::const_iterator fsaA = fsaSp->begin();
           fsaA != fsaSp->end(); ++fsaA) {
        /*
        if (fsaA->input() == Fsa::InvalidLabelId)
            Core::Application::us()->warning("invalid label id");
        */
        sp->newArc(fsaA->target(), semiring->clone(semiring->defaultWeight()),
                   mapInputLabel[fsaA->input()],
                   mapOutputLabel[fsaA->output()]);
      }
      sp->minimize();
      l.setState(sp);
    }
...
void FlfReader::addScores(StaticLattice &l, ScoreId id,
                          const Fsa::StaticAutomaton &f) {
...
ConstLatticeRef FlfReader::read(FlfDescriptorRef desc, Core::Archive *archive) {
  ...
  {
    std::string filename =
        Core::joinPaths(desc->latticeRoot, desc->structureFile.file);
    InputStream is(filename, archive);
    ...
    if (!structureFsa.hasProperty(Fsa::PropertyAcyclic))
      warning("Assume \"%s\" to be acyclic", filename.c_str());
  }
    ...
    for (ScoreId id = 0; id < ScoreId(desc->scoreFiles.size()); ++id)
      if ((bool)desc->scoreFiles[id]) {
        if (desc->scoreFiles[id] == desc->structureFile)
          ...
          std::string filename =
              Core::joinPaths(desc->latticeRoot, desc->scoreFiles[id].file);
          InputStream is(filename, archive);
...
ConstLatticeRef FlfReader::read(const std::string &filename,
                                Core::Archive *archive) {
...
/*
  Flf-Writer
*/
const Core::ParameterStringVector FlfWriter::paramKeys("keys",
                                                       "sequence of keys");
...
FlfWriter::FlfWriter(const Core::Configuration &config) : Precursor(config) {
  KeyList partialKeys = paramKeys(select("partial"));
...
FlfDescriptorRef
FlfWriter::buildDescriptor(ConstLatticeRef f,
  ...
  desc->inputAlphabetFile =
      FsaDescriptor("bin", Core::form("%s-input-alphabet.binfsa%s",
  ...
  if (f->type() != Fsa::TypeAcceptor) {
    ...
    desc->outputAlphabetFile =
        FsaDescriptor("bin", Core::form("%s-output-alphabet.binfsa%s",
                                        basename.c_str(), suffix.c_str()));
  } else
    desc->hasOutputAlphabet = false;
  if (f->getBoundaries()->valid()) {
    ...
    desc->boundariesFile = BoundariesDescriptor(
        Core::form("%s.binwb%s", basename.c_str(), suffix.c_str()));
  } else
    desc->hasBoundaries = false;
  ...
  if (desc->scoreFiles.empty()) {
    ...
    desc->structureFile.file =
        Core::form("%s.binfsa%s", basename.c_str(), suffix.c_str());
  } else {
...
bool FlfWriter::writeDescriptor(FlfDescriptorRef desc,
  ...
  xml.setIndentation(2);
  {
    ...
    openLattice +
        Core::XmlAttribute("semiring", getSemiringTypeName(desc->semiringType));
    if (!desc->id.empty())
  ...
  std::ostringstream oss;
  oss << Core::timestamp;
  xml << Core::XmlOpen("head") << Core::XmlFull("creator", "RWTH-i6")
      << Core::XmlFull("timestamp", oss.str()) << Core::XmlClose("head");
  ...
  if (desc->hasInputAlphabet)
    xml << Core::XmlEmpty(
               (desc->hasOutputAlphabet ? "input-alphabet" : "alphabet")) +
  ...
  if (desc->hasBoundaries)
    xml << Core::XmlEmpty("boundaries") + Core::XmlAttribute("format", "bin") +
  ...
  for (ScoreId id = 0; id < desc->scoreFiles.size();
       ++id, ++keyIt, ++scaleIt, ++fsaIt) {
  ...
  xml << Core::XmlClose("scores") << Core::XmlClose("lattice");
  return xml;
}

bool FlfWriter::write(ConstLatticeRef f, FlfDescriptorRef desc,
                      const std::string &descFilename, bool storeAlphabet,
                      Core::Archive *archive) const {
  verify(desc);
  ...
  if (desc->hasInputAlphabet && storeAlphabet) {
    OutputStream os(
        Core::joinPaths(desc->latticeRoot, desc->inputAlphabetFile.file),
        archive);
    if (!Fsa::write(toUnweightedFsa(f, Fsa::Weight(Semiring::One)),
                    desc->inputAlphabetFile.format, os,
                    Fsa::storeInputAlphabet))
      return false;
  }
  if (desc->hasOutputAlphabet && storeAlphabet) {
    OutputStream os(
        Core::joinPaths(desc->latticeRoot, desc->outputAlphabetFile.file),
        archive);
    if (!Fsa::write(toUnweightedFsa(f, Fsa::Weight(Semiring::One)),
                    desc->outputAlphabetFile.format, os,
                    Fsa::storeOutputAlphabet))
      return false;
  }
  if (desc->hasBoundaries && (bool)desc->boundariesFile) {
    OutputStream os(
        Core::joinPaths(desc->latticeRoot, desc->boundariesFile.file), archive);
  ...
  for (KeyList::const_iterator itKey = desc->keys.begin(),
                               endKey = desc->keys.end();
       itKey != endKey; ++itKey, ++itScoreFile)
    if (*itScoreFile) {
      ScoreId dim = semiring->id(*itKey);
      verify(semiring->hasId(dim));
      OutputStream os(Core::joinPaths(desc->latticeRoot, itScoreFile->file),
                      archive);
  ...
  if (!embeddedStructure && (bool)desc->structureFile) {
    OutputStream os(
        Core::joinPaths(desc->latticeRoot, desc->structureFile.file), archive);
    if (!Fsa::write(toUnweightedFsa(f, Fsa::Weight(Semiring::One)),
                    desc->structureFile.format, os, Fsa::storeStates))
      return false;
  }
  return true;
}

bool FlfWriter::write(ConstLatticeRef f, const std::string &descFilename,
                      bool storeAlphabet, Core::Archive *archive) const {
  return write(f, buildDescriptor(f, descFilename), descFilename, storeAlphabet,
               archive);
}
// -------------------------------------------------------------------------
...
*/
const Core::ParameterString
    FlfArchiveReader::paramLatticeType("lattice-type", "lattice type", "");
const Core::ParameterString FlfArchiveReader::paramName("name", "name", "");

FlfArchiveReader::FlfArchiveReader(const Core::Configuration &config,
                                   const std::string &pathname)
    : Precursor(config, pathname) {
  archive =
      Core::Archive::create(config, pathname, Core::Archive::AccessModeRead);
  if (!archive)
    criticalError("Failed to open lattice archive \"%s\" for reading",
                  pathname.c_str());
...
FlfArchiveReader::~FlfArchiveReader() { delete reader_; }

void FlfArchiveReader::setDescriptor(const Core::Configuration &config,
                                     FlfContextRef context) {
  ...
  std::string structureFormat =
      FsaDescriptor::paramFormat(structureConfig, "bin");
  ...
  if (structureSuffix_.empty())
    criticalError(
        "Discard flf-header, but no suffix for structure file given.");
  ...
  if (!outputAlphabetName.empty()) {
    if (outputAlphabetQf)
      context->loadOutputAlphabet(outputAlphabetQf, archive,
                                  outputAlphabetName);
    else
...
/*
  Flf-Archive-Writer
*/
FlfArchiveWriter::FlfArchiveWriter(const Core::Configuration &config,
                                   const std::string &pathname)
    : Precursor(config, pathname) {
  archive =
      Core::Archive::create(config, pathname, Core::Archive::AccessModeWrite);
  if (!archive)
    criticalError("Failed to open lattice archive \"%s\" for writing",
                  pathname.c_str());
...
FlfArchiveWriter::~FlfArchiveWriter() { delete writer_; }
...
void FlfArchiveWriter::storeAlphabets() {
  Core::Ref<Fsa::StorageAutomaton> f =
      Core::ref(new Fsa::StaticAutomaton(Fsa::TypeTransducer));
  if (inputAlphabet_) {
    f->setInputAlphabet(inputAlphabet_);
    if (inputAlphabetQf_) {
      log("store input alphabet \"%s\"",
          inputAlphabetQf_.qualifiedPath().c_str());
      Core::ArchiveWriter os(*archive, inputAlphabetQf_.file, true);
      if (!Fsa::write(f, inputAlphabetQf_.format, os, Fsa::storeInputAlphabet))
        error("Failed to store input alphabet \"%s\"",
              inputAlphabetQf_.qualifiedPath().c_str());
    }
  }
  if (outputAlphabet_) {
    ...
    if (outputAlphabetQf_) {
      log("store output alphabet \"%s\"",
          outputAlphabetQf_.qualifiedPath().c_str());
      Core::ArchiveWriter os(*archive, outputAlphabetQf_.file, true);
      if (!Fsa::write(f, outputAlphabetQf_.format, os,
                      Fsa::storeOutputAlphabet))
        error("Failed to store output alphabet \"%s\"",
              outputAlphabetQf_.qualifiedPath().c_str());
    }
  }
  if (alphabetQf_) {
    ...
    if (!Fsa::write(f, alphabetQf_.format, os,
                    Fsa::storeInputAlphabet | Fsa::storeOutputAlphabet))
      error("Failed to store alphabet \"%s\"",
            alphabetQf_.qualifiedPath().c_str());
  }
}

FlfDescriptorRef
FlfArchiveWriter::buildDescriptor(ConstLatticeRef l, const std::string &id,
  ...
  if (desc->hasOutputAlphabet) {
    ...
    desc->outputAlphabetFile =
        FsaDescriptor::joinPaths(outputAlphabetQf_, root);
  }
  return desc;
}
