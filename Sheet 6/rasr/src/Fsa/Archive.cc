...
const Core::ParameterString
    Archive::paramAlphabetFilename("alphabet-filename", "filename of alphabet",
                                   "alphabets.binfsa.gz");

const Core::ParameterString
    Archive::paramPath("path", "where to store the automaton archive");

Archive::Archive(const Core::Configuration &config, const std::string &path,
                 Core::Archive::AccessMode accessMode)
    : Core::Component(config), archive_(0) {
...
Archive::~Archive() { delete archive_; }
// -----------------------------------------------------------------------------

// -----------------------------------------------------------------------------
const Core::ParameterInt ArchiveReader::paramReportUnknowns(
    "report-unknowns",
    "maximum number of unknown symbols that should be reported", 10, 0);

const Core::ParameterInt ArchiveReader::paramMapUnknownsToIndex(
    "map-unknowns-to-index", "unknown indices are mapped to this index",
    InvalidLabelId);
...
void ArchiveReader::mapInput(ConstAlphabetRef targetAlphabet) {
  mapAlphabet(inputAlphabet_, targetAlphabet, inputMapping_, unknownId_,
              reportUnknowns_);
}

void ArchiveReader::mapOutput(ConstAlphabetRef targetAlphabet) {
  mapAlphabet(outputAlphabet_, targetAlphabet, outputMapping_, unknownId_,
              reportUnknowns_);
}
