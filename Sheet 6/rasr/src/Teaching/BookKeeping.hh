...
public:
  struct Entry {
    ...
    Entry()
        : word(invalidWord), score(maxScore), backpointer(0), time(0),
          timestamp(-1) {}
...
public:
  BookKeeping() : next_(0), lastTimestamp_(0) {}

  void clear();

  Index size() const { return book_.size(); }

  Entry &entry(Index backpointer) { return book_[backpointer]; }

  const Entry &entry(Index backpointer) const { return book_[backpointer]; }

  Index addEntry(Word word, Score score, Index backpointer, Time t);

  template <class HypothesisIterator>
  void tagActiveEntries(Time t, HypothesisIterator begin,
                        HypothesisIterator end);
...
template <class HypothesisIterator>
void BookKeeping::tagActiveEntries(Time t, HypothesisIterator hypBegin,
                                   HypothesisIterator hypEnd) {
