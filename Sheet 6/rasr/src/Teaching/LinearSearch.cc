...
private:
  struct StateHypothesis {
    ...
    StateHypothesis() : state(0), score(maxScore), backpointer(invalidIndex) {}
  };
  ...
  struct WordHypothesis {
    ...
    WordHypothesis()
        : word(invalidWord), stateHypBegin(invalidIndex),
          stateHypEnd(invalidIndex), entryStateHypothesis(invalidIndex) {}
  };
...
public:
  ...
  /**
   * create hypotheses for starting words using the word end hypothesees in
   * wordEndHyps. language model probability is calculated for starting words,
   * word start hypotheses are written to wordStartHyps
   */
  void bigramRecombination(const WordBoundaryHypotheses &wordEndHyps,
   ...
   * wordStartHypotheses are pruned using the language model pruning threshold
   * pruningThreshold
   */
  void insertWordStartHypotheses(const WordBoundaryHypotheses &wordStartHyps,
   ...
   */
  void pruneStatesAndFindWordEnds(WordBoundaryHypotheses &wordEndHyps,
                                  Score threshold);
  /**
   ...
   */
  void traceback(const WordBoundaryHypotheses &wordEndHyps,
...
private:
  StateHypotheses stateHypotheses_, newStateHypotheses_;
...
LinearSearch::SearchSpace::SearchSpace(const LinearLexicon &lexicon,
                                       Word silence)
    : bookKeeping_(new BookKeeping), lexicon_(lexicon), nWords_(lexicon.size()),
      silence_(silence) {
...
LinearSearch::SearchSpace::~SearchSpace() { delete bookKeeping_; }

void LinearSearch::SearchSpace::setTransitionScores(
...
void LinearSearch::SearchSpace::reset() {
  ...
  std::fill(stateHypothesisMap_.begin(), stateHypothesisMap_.end(),
            invalidIndex);
}
...
void LinearSearch::SearchSpace::addInitialHypothesis(
    WordBoundaryHypotheses &wordEndHyps) {
  // startup word hypothesis
  wordEndHyps.push_back(WordBoundaryHypothesis(silence_, 0, 0));
  addBookKeepingEntries(0, wordEndHyps);
}

void LinearSearch::SearchSpace::bigramRecombination(
    const WordBoundaryHypotheses &wordEndHyps,
    WordBoundaryHypotheses &wordStartHyps,
    const LanguageModelScorer &lmScore) const {
  wordStartHyps.clear();
  wordStartHyps.resize(nWords_);

  for (WordBoundaryHypotheses::const_iterator we = wordEndHyps.begin();
       we != wordEndHyps.end(); ++we) {
    ...
    for (Word w = 0; w < nWords_; ++w) {
      if (w == silence_)
        continue; // no transition to silence
    ...
    if (we->word < nWords_) { // exclude silence copies
      wordStartHyps.push_back(WordBoundaryHypothesis(
          silenceCopy(we->word), we->score, we->backpointer));
    }
  }
}

void LinearSearch::SearchSpace::insertWordStartHypotheses(
    const WordBoundaryHypotheses &wordStartHyps, Score pruningThreshold) {

  for (WordBoundaryHypotheses::const_iterator ws = wordStartHyps.begin();
       ws != wordStartHyps.end(); ++ws) {
...
void LinearSearch::SearchSpace::addEntryStateHypothesis(Word word, Score score,
                                                        Index backpointer) {
  ...
  wordHypotheses_[wordHypothesisMap_[word]].entryStateHypothesis =
...
void LinearSearch::SearchSpace::expandState(Word word,
                                            const StateHypothesis &stateHyp) {
  const MixtureSequence &mixtures = lexicon_[acousticWord(word)];
  for (State successorState = std::max(State(1), stateHyp.state);
       successorState <=
       std::min(State(stateHyp.state + maxSkip_), State(mixtures.size()));
       ++successorState) {

    Score newScore = stateHyp.score;
    State timeDistortion = (successorState - stateHyp.state);
    // don't add transition score for the transition from the virtual entry
    ...
    if (stateHypIndex < currentFirstNewStateHypothesis_ ||
      ...
      newStateHypotheses_.push_back(
          StateHypothesis(successorState, newScore, stateHyp.backpointer));
    } else {
...
void LinearSearch::SearchSpace::addAcousticScores(
    const AcousticModelScorer &featureScore) {
  currentBestScore_ = maxScore;
  for (Index w = 0; w < wordHypotheses_.size(); ++w) {
    Word word = acousticWord(wordHypotheses_[w].word);
    for (Index s = wordHypotheses_[w].stateHypBegin;
         s < wordHypotheses_[w].stateHypEnd; ++s) {
      stateHypotheses_[s].score +=
          featureScore(lexicon_[word][stateHypotheses_[s].state - 1]);
...
void LinearSearch::SearchSpace::pruneStatesAndFindWordEnds(
    WordBoundaryHypotheses &wordEndHyps, Score threshold) {
  wordEndHyps.clear();
  ...
  for (wordHypIn = wordHypOut = wordHypotheses_.begin();
       wordHypIn != wordHypotheses_.end(); ++wordHypIn) {
    ...
    for (stateHypEnd = stateHypBegin + wordHypIn->stateHypEnd;
         stateHypIn < stateHypEnd; ++stateHypIn) {
      Score score = stateHypIn->score +
                    transitionScores_[isSilence(wordHypIn->word)]
                                     [maxSkip_ + 1]; // add exit penalty
      if (score < threshold) {
        *(stateHypOut++) = *stateHypIn;
        if (stateHypIn->state == nStates) {
          // found a word end
          wordEndHyps.push_back(WordBoundaryHypothesis(
              wordHypIn->word, score, stateHypIn->backpointer));
        }
      }
    }
...
void LinearSearch::SearchSpace::mergeSilenceToBigramNodes(
    WordBoundaryHypotheses &wordEndHyps) const {
...
void LinearSearch::SearchSpace::addBookKeepingEntries(
    Time t, WordBoundaryHypotheses &wordEndHyps) {
  if (t % purgeStorageInterval_ == 0) {
    bookKeeping_->tagActiveEntries(t, stateHypotheses_.begin(),
                                   stateHypotheses_.end());
  }
  for (WordBoundaryHypotheses::iterator we = wordEndHyps.begin();
       we != wordEndHyps.end(); ++we) {
    Index newBackpointer = bookKeeping_->addEntry(
        acousticWord(we->word), we->score, we->backpointer, t);
...
void LinearSearch::SearchSpace::traceback(
    const WordBoundaryHypotheses &wordEndHyps, Traceback &result) const {
  result.clear();
  if (wordEndHyps.size() == 0)
    return;
  WordBoundaryHypotheses::const_iterator best =
      std::min_element(wordEndHyps.begin(), wordEndHyps.end());
...
LinearSearch::LinearSearch(const Core::Configuration &c)
    : Core::Component(c), SearchInterface(c), searchSpace_(0),
...
bool LinearSearch::setModelCombination(
...
void LinearSearch::processFrame(Time t) {
  searchSpace_->bigramRecombination(wordEndHypotheses_, wordStartHypotheses_,
                                    *lmScorer_);
  WordBoundaryHypotheses::const_iterator best = std::min_element(
      wordStartHypotheses_.begin(), wordStartHypotheses_.end());
  Score lmPruningThreshold = languageModelPruningThreshold_;
  if (lmPruningThreshold < maxScore)
    lmPruningThreshold += best->score;
  searchSpace_->insertWordStartHypotheses(wordStartHypotheses_,
                                          lmPruningThreshold);
  ...
  searchSpace_->pruneStatesAndFindWordEnds(wordEndHypotheses_,
                                           acPruningThreshold);
