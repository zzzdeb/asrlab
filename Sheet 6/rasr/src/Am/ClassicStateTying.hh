...
public:
  ClassicStateTying(const Core::Configuration &config,
  ...
  */
  ...
  /** creates a classic state tying reference for a given config file and a
   * state model reference. */
  static Core::Ref<const ClassicStateTying>
  createClassicStateTyingRef(const Core::Configuration &config,
                             ClassicStateModelRef stateModel);

  Fsa::ConstAutomatonRef
  createMixtureToAllophoneStateTransducer(s32 nDisambiguators = 0) const;
...
public:
  NoStateTying(const Core::Configuration &config,
               ClassicStateModelRef stateModel)
  ...
  virtual Mm::MixtureIndex nClasses() const { return alphabetRef_->nClasses(); }
...
public:
  MonophoneStateTying(const Core::Configuration &config,
                      ClassicStateModelRef stateModel);
...
public:
  LutStateTying(const Core::Configuration &config,
                ClassicStateModelRef stateModel)
      : Core::Component(config), ClassicStateTying(config, stateModel),
        nClasses_(0) {
    if (!loadLut(paramFilename(config))) {
      error("error while reading lookup table from file \"%s\"",
            paramFilename(config).c_str());
      return;
    }
  }
