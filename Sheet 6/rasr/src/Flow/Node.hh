...
protected:
   ...
   *  If the configuration of the predecessor node fails, an empty Attributes
   * object is generated.
   */
...
public:
  ...
  virtual PortId getInput(const std::string &name) { return IllegalPortId; }
  virtual PortId getOutput(const std::string &name) { return IllegalPortId; }
   ...
   *    -This function is the safest point to reset the algorithms used by the
   * node. The alternative would be to reset the algorithms when a non-data
   * object has been recieved. This solution can lead easily to unintended
   * behaviour, since it is not garatinteed at all that every node receives for
   * example a EOS object!
   */
...
public:
  SinkNode(const Core::Configuration &c) : Core::Component(c), Node(c) {
    addInput(0);
  }
...
public:
  SourceNode(const Core::Configuration &c) : Core::Component(c), Node(c) {
    addOutput(0);
  }
...
public:
  SleeveNode(const Core::Configuration &c) : Core::Component(c), Node(c) {
    addInput(0);
    addOutput(0);
  }
  virtual PortId getInput(const std::string &name) { return 0; }
