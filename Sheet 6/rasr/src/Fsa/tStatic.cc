...
namespace Ftl {
template <class _Automaton> StaticAutomaton<_Automaton>::~StaticAutomaton() {
  clear();
}

template <class _Automaton> void StaticAutomaton<_Automaton>::clear() {
...
template <class _Automaton>
Core::Ref<typename _Automaton::State>
StaticAutomaton<_Automaton>::state(Fsa::StateId s) {
  if (s < states_.size())
    return states_[s];
...
template <class _Automaton>
typename _Automaton::State *
StaticAutomaton<_Automaton>::newState(Fsa::StateTag tags) {
...
typename _Automaton::State *StaticAutomaton<_Automaton>::newFinalState(
  ...
  setState(new typename _Automaton::State(states_.size(), Fsa::StateTagFinal,
                                          weight));
  return states_.back().get();
}
...
void StaticAutomaton<_Automaton>::setStateFinal(
    ...
    const typename _Automaton::Weight &finalWeight) {
...
template <class _Automaton>
Core::Ref<const typename _Automaton::State>
StaticAutomaton<_Automaton>::getState(Fsa::StateId s) const {
  if (s < states_.size())
    return states_[s];
  return Core::Ref<const typename _Automaton::State>();
}

template <class _Automaton> void StaticAutomaton<_Automaton>::normalize() {
    ...
    if (states_[s]) {
      ...
      for (typename _Automaton::State::iterator a = states_[_s]->begin();
        ...
        if ((a->target() >= states_.size()) || (!states_[a->target()]) ||
          ...
          std::cerr << "arc at state " << s
                    << " points to non-existent target state " << a->target()
                    << std::endl;
...
StaticAutomaton<_Automaton>::getStateAndArcMemoryUsed() const {
  size_t states = 0, arcs = 0;
  for (typename Core::Vector<
           Core::Ref<typename _Automaton::State> >::const_iterator sp =
           states_.begin();
       sp != states_.end(); ++sp)
    if (*sp) {
...
void StaticAutomaton<_Automaton>::dumpMemoryUsage(Core::XmlWriter &o) const {
  std::pair<size_t, size_t> memory = getStateAndArcMemoryUsed();
  o << Core::XmlOpen("static") << Core::XmlFull("states", memory.first)
    << Core::XmlFull("arcs", memory.second)
    << Core::XmlFull("total", getMemoryUsed()) << Core::XmlClose("static");
}
...
void StaticAutomaton<_Automaton>::compact(Fsa::StateMap &mapping) {
  ...
  for (typename Core::Vector<Core::Ref<typename _Automaton::State> >::iterator
           s = states_.begin();
       s != states_.end(); ++s)
    for (typename _Automaton::State::iterator a = (*s)->begin();
         a != (*s)->end(); ++a)
