...
LabelingFeatureExtractor::LabelingFeatureExtractor(
    ...
    LabeledFeatureProcessor &labeledFeatureProcessor)
...
void LabelingFeatureExtractor::processFeature(
  ...
  while (!currentTimestamp_.contains(feature->timestamp())) {
    ...
    if (dataSource()->getData(labelsPort_, in)) {
      Core::StringHashMap<LabelIndex>::iterator index =
          labelToIndexMap_.find((*in)());
      ...
      criticalError("No label found for interval [%f..%f].",
                    feature->timestamp().startTime(),
                    feature->timestamp().endTime());
    }
  }
