...
Core::BinaryOutputStream &Speech::operator<<(Core::BinaryOutputStream &os,
                                             const Alignment &a) {
  ...
  if (!a.hasWeights()) {
    ...
    os << u32(version | highestBitU32); // use highest bit to distinguish this
                                        // scheme from the RLE scheme
...
Core::BinaryInputStream &Speech::operator>>(Core::BinaryInputStream &is,
                                            Alignment &a) {
  a.clear();
...
void Alignment::write(std::ostream &os) const {
  for (const_iterator i = begin(); os && i != end(); ++i) {
    os << "time = " << i->time << '\t' << "emission = " << i->emission
...
void Alignment::writeXml(Core::XmlWriter &os) const {
  ...
  for (const_iterator i = begin(); os && i != end(); ++i) {
    os << Core::XmlOpen("align") << Core::XmlFull("time", i->time)
...
void Alignment::combineItems(Fsa::ConstSemiringRef sr) {
  ...
  for (; in != in_end; ++in, ++last) {
    if (in->emission == last->emission and in->time == last->time) {
      out->weight =
          f32(sr->collect(Fsa::Weight(out->weight), Fsa::Weight(in->weight)));
    } else {
      *(++out) = *in;
    }
  }
...
void Alignment::getFrames(
    std::vector<std::pair<Alignment::iterator, Alignment::iterator> > &rows) {
  rows.clear();
  ...
  while (i != i_end) {
    while (i != i_end && i->time == i_firstCurrentFrame->time)
      ++i;
    rows.push_back(std::pair<Alignment::iterator, Alignment::iterator>(
