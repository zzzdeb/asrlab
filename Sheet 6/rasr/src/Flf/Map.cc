...
const Core::Choice alphabetMappingActionChoice(
    "undefined", UndefinedAlphabetMapping, "to-phoneme", MapToPhoneme,
    "to-lemma-pron", MapToLemmaPronunciation, "to-preferred-lemma-pron",
    MapToPreferredLemmaPronunciation, "to-lemma", MapToLemma, "to-synt",
    MapToSyntacticTokenSequence, "to-eval", MapToEvaluationTokenSequences,
...
AlphabetMappingAction getAlphabetMappingAction(const std::string &name) {
  ...
  return c == Core::Choice::IllegalValue ? UndefinedAlphabetMapping
                                         : AlphabetMappingAction(c);
}
  ...
  virtual void modifyState(State *sp) const {
    if (isAcceptor_) {
      for (State::iterator a = sp->begin(); a != sp->end(); ++a)
        if ((Fsa::FirstLabelId <= a->input()) &&
            (a->input() <= Fsa::LastLabelId))
          a->input_ = a->output_ =
              lpAlphabet_->lemmaPronunciation(a->input_)->lemma()->id();
    } else {
      for (State::iterator a = sp->begin(); a != sp->end(); ++a)
        if ((Fsa::FirstLabelId <= a->input()) &&
            (a->input() <= Fsa::LastLabelId))
...
class MapInputFromLemmaToSingleLemmaPronunciationLattice
    : public ModifyLattice {
  typedef ModifyLattice Precursor;
      ...
      for (State::iterator a = sp->begin(); a != sp->end(); ++a)
        if ((Fsa::FirstLabelId <= a->input()) &&
          ...
          const Bliss::LemmaPronunciation *lp =
              lexicon_->lemmaPronunciation(lAlphabet_->lemma(a->input_), 0);
          a->input_ = a->output_ =
              (lp) ? Fsa::LabelId(lp->id()) : Fsa::InvalidLabelId;
        }
    } else {
      for (State::iterator a = sp->begin(); a != sp->end(); ++a)
        if ((Fsa::FirstLabelId <= a->input()) &&
...
class MapOutputFromLemmaToSingleLemmaPronunciationLattice
    : public ModifyLattice {
  typedef ModifyLattice Precursor;
...
public:
  ...
  virtual void modifyState(State *sp) const {
    for (State::iterator a = sp->begin(); a != sp->end(); ++a)
      if ((Fsa::FirstLabelId <= a->output()) &&
          (a->output() <= Fsa::LastLabelId)) {
        const Bliss::LemmaPronunciation *lp =
            lexicon_->lemmaPronunciation(lAlphabet_->lemma(a->output_), 0);
...
ConstLatticeRef mapInput(Fsa::ConstAutomatonRef f, ConstLatticeRef l) {
  return composeSequencing(
      fromUnweightedFsa(f, l->semiring(), l->semiring()->one()), l);
}

ConstLatticeRef mapOutput(ConstLatticeRef l, Fsa::ConstAutomatonRef f) {
  return composeSequencing(
      l, fromUnweightedFsa(f, l->semiring(), l->semiring()->one()));
}
...
ConstLatticeRef mapInput(ConstLatticeRef l, AlphabetMappingAction action) {
  ...
  case Lexicon::PhonemeAlphabetId:
    ...
    case MapToPreferredEvaluationTokenSequence:
      return mapInput(
          Fsa::invert(
              Lexicon::us()
                  ->lemmaToPreferredEvaluationTokenSequenceTransducer()),
          mapInput(
              Fsa::invert(Lexicon::us()->lemmaPronunciationToLemmaTransducer()),
              mapInput(
                  Fsa::invert(
  ...
  case Lexicon::LemmaPronunciationAlphabetId:
    switch (action) {
    case MapToPhoneme:
      return mapInput(Lexicon::us()->phonemeToLemmaPronunciationTransducer(),
                      l);
    case MapToLemmaPronunciation:
    case MapToPreferredLemmaPronunciation:
      return l;
    case MapToLemma:
      return ConstLatticeRef(
          new MapInputFromLemmaPronunciationToLemmaLattice(l));
    case MapToSyntacticTokenSequence:
      return mapInput(
          Fsa::invert(Lexicon::us()->lemmaToSyntacticTokenTransducer()),
  ...
  case Lexicon::LemmaAlphabetId:
    ...
    case MapToLemmaPronunciation:
      return mapInput(Lexicon::us()->lemmaPronunciationToLemmaTransducer(), l);
    case MapToPreferredLemmaPronunciation:
      return ConstLatticeRef(
          new MapInputFromLemmaToSingleLemmaPronunciationLattice(l));
    case MapToLemma:
      return l;
    case MapToSyntacticTokenSequence:
      return mapInput(
  ...
  case Lexicon::SyntacticTokenAlphabetId:
    switch (action) {
    case MapToLemma:
      return mapInput(Lexicon::us()->lemmaToSyntacticTokenTransducer(), l);
    case MapToSyntacticTokenSequence:
      return l;
    default:
      break;
  ...
  case Lexicon::EvaluationTokenAlphabetId:
    switch (action) {
    case MapToLemma:
      return mapInput(Lexicon::us()->lemmaToEvaluationTokenTransducer(), l);
    case MapToEvaluationTokenSequences:
    case MapToPreferredEvaluationTokenSequence:
      return l;
    default:
  ...
  Core::Application::us()->criticalError(
      "The implementation of the \"%s\"-mapping for input alphabet \"%s\" is "
      "pending ...",
      alphabetMappingActionChoice[action].c_str(),
      Lexicon::us()->alphabetName(alphabetId).c_str());
  return ConstLatticeRef();
}
...
ConstLatticeRef mapOutput(ConstLatticeRef l, AlphabetMappingAction action) {
  l = transducer(l);
  Lexicon::AlphabetId alphabetId =
      Lexicon::us()->alphabetId(l->getOutputAlphabet());
  switch (alphabetId) {
  case Lexicon::PhonemeAlphabetId:
    switch (action) {
    case MapToPhoneme:
  ...
  case Lexicon::LemmaPronunciationAlphabetId:
    ...
    case MapToLemma:
      return ConstLatticeRef(
          new MapOutputFromLemmaPronunciationToLemmaLattice(l));
    case MapToSyntacticTokenSequence:
      return mapOutput(
  ...
  case Lexicon::LemmaAlphabetId:
    ...
    case MapToPreferredLemmaPronunciation:
      return ConstLatticeRef(
          new MapOutputFromLemmaToSingleLemmaPronunciationLattice(l));
    case MapToLemma:
      return l;
    case MapToSyntacticTokenSequence:
      return mapOutput(l, Lexicon::us()->lemmaToSyntacticTokenTransducer());
    case MapToEvaluationTokenSequences:
      return mapOutput(l, Lexicon::us()->lemmaToEvaluationTokenTransducer());
    case MapToPreferredEvaluationTokenSequence:
      return mapOutput(
          l,
          Lexicon::us()->lemmaToPreferredEvaluationTokenSequenceTransducer());
  ...
  Core::Application::us()->criticalError(
      "The implementation of the \"%s\"-mapping for output alphabet \"%s\" is "
      "pending ...",
      alphabetMappingActionChoice[action].c_str(),
      Lexicon::us()->alphabetName(alphabetId).c_str());
  return ConstLatticeRef();
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!name.empty()) {
      ...
      if (inputMapping_ == UndefinedAlphabetMapping)
        criticalError("MapNode: Unknown mapping \"%s\"", name.c_str());
    }
...
const Core::ParameterString AlphabetMapNode::paramInputMapping(
    "map-input", "apply mapping to input, result is transducer", "");
const Core::ParameterString AlphabetMapNode::paramOutputMapping(
    "map-output", "apply mapping to output, result is transducer", "");
const Core::ParameterBool AlphabetMapNode::paramProjectInput(
    "project-input", "make lattice an acceptor by mapping input to output",
    false);
const Core::ParameterBool AlphabetMapNode::paramProjectOutput(
    "project-output", "make lattice an acceptor by mapping output to input",
    false);
const Core::ParameterBool
    AlphabetMapNode::paramInvert("invert", "invert input and output", false);

NodeRef createAlphabetMapNode(const std::string &name,
...
u32 LabelMap::nMappings() const {
  u32 n = 0;
  for (MappingList::const_iterator it = mappings.begin(), end = mappings.end();
       it != end; ++it)
    if (it->first)
      ++n;
  return n;
...
protected:
  virtual void createMapping(Fsa::LabelId label, Mapping &mapping) {
    Fsa::LabelId targetLabel =
        symbolMap_.index(Core::convertToLowerCase(from->symbol(label)));
...
public:
  typedef std::pair<Bliss::Lemma::EvaluationTokenSequenceIterator,
                    Bliss::Lemma::EvaluationTokenSequenceIterator>
      EvalTokSeqRange;
...
const LabelMap::ExtendedLabel NonWordToEpsilonMap::EpsilonLabel(Fsa::Epsilon,
                                                                1.0);
...
protected:
  ...
  /**
     Split at whitespace or at "_" or "-", if they are not pending, i.e.
  succeeded or followed by whitespace.
  **/
  virtual void createMapping(Fsa::LabelId label, Mapping &mapping) {
    ...
    if ((b + 2 <= e) &&
      ...
      if (!splitIndices.empty()) {
        ...
        for (LabelMap::Mapping::iterator it = mapping.begin(),
                                         end = mapping.end();
             it != end; ++it)
...
public:
  CompoundWordSplittingMap(Fsa::ConstAlphabetRef alphabet)
      : LabelMap(alphabet, alphabet), alphabet_(alphabet),
        lexicon_(Lexicon::us().get()) {
...
public:
        ...
        if (!mapping.empty()) {
          ...
          if (cols.size() == 1) {
            ...
            for (u32 i = 1; i < cols.size(); ++i) {
              ...
              mapping.push_back(
                  LabelMap::ExtendedLabel(partLabel, f32(cols[i].size())));
              totalLength += mapping.back().length;
            }
            for (LabelMap::Mapping::iterator it = mapping.begin(),
                                             end = mapping.end();
  ...
  static LabelMapRef create(const Core::Configuration &config,
                            Fsa::ConstAlphabetRef defaultAlphabet) {
    ...
    Fsa::ConstAlphabetRef from =
        fromAlphabetName.empty()
            ? defaultAlphabet
            : Lexicon::us()->alphabet(
                  Lexicon::us()->alphabetId(fromAlphabetName, true));
    std::string toAlphabetName = paramToAlphabet(config);
    Fsa::ConstAlphabetRef to =
        toAlphabetName.empty()
            ? from
            : Lexicon::us()->alphabet(
                  Lexicon::us()->alphabetId(toAlphabetName, true));
...
const Core::ParameterString
    StaticLabelMap::paramFromAlphabet("from", "name of source alphabet", "");
const Core::ParameterString
    StaticLabelMap::paramToAlphabet("to", "name of target alphabet", "");
const Core::ParameterString
    StaticLabelMap::paramFile("file", "name of label mapping file", "");
const Core::ParameterString
    StaticLabelMap::paramEncoding("encoding", "encoding of label mapping file",
                                  "utf-8");
// -------------------------------------------------------------------------
...
LabelMapRef
LabelMap::createNonWordToEpsilonMap(Lexicon::AlphabetId alphabetId) {
  ...
  case Lexicon::LemmaPronunciationAlphabetId:
    return (internal_->nonWordToEpsilonMaps[alphabetId] =
                LabelMapRef(new NonLemmaPronunciationToEpsilonMap()));
  case Lexicon::LemmaAlphabetId:
    return (internal_->nonWordToEpsilonMaps[alphabetId] =
                LabelMapRef(new NonLemmaToEpsilonMap()));
  default:
    Core::Application::us()->criticalError(
...
LabelMapRef
LabelMap::createCompoundWordSplittingMap(Lexicon::AlphabetId alphabetId) {
  ...
  case Lexicon::LemmaPronunciationAlphabetId:
    return (internal_->compoundWordMaps[alphabetId] =
                LabelMapRef(new CompoundLemmaPronunciationSplittingMap()));
  case Lexicon::LemmaAlphabetId:
    return (internal_->compoundWordMaps[alphabetId] =
                LabelMapRef(new CompoundLemmaSplittingMap()));
  case Lexicon::SyntacticTokenAlphabetId:
    return (internal_->compoundWordMaps[alphabetId] =
                LabelMapRef(new CompoundSyntacticTokenSplittingMap()));
  case Lexicon::EvaluationTokenAlphabetId:
    return (internal_->compoundWordMaps[alphabetId] =
                LabelMapRef(new CompoundEvaluationTokenSplittingMap()));
  default:
    Core::Application::us()->criticalError(
...
LabelMapRef LabelMap::load(const Core::Configuration &config,
                           Fsa::ConstAlphabetRef defaultAlphabet) {
...
public:
  ...
  virtual std::string describe() const {
    return Core::form("oneToOneLabelMap(%s;%s)", fsa_->describe().c_str(),
                      labelMap_->description.c_str());
  }
  virtual Fsa::ConstAlphabetRef getInputAlphabet() const {
    return labelMap_->to;
  }
...
private:
  ScoresRef toPartialScores(const ExtLabel &extLabel,
                            ScoresRef compoundScores) {
    ScoresRef scores = semiring_->clone(compoundScores);
    for (Scores::iterator itScore = semiring_->begin(scores),
                          endScore = semiring_->end(scores);
  ...
  void splitCompoundArcs() {
    for (Core::Vector<std::pair<Fsa::StateId, State::iterator> >::iterator it =
             compoundArcs_.begin();
      ...
      if (b_) {
        ...
        if (duration < Time(mapping.size()))
          Core::Application::us()->criticalError(
              "Cannot distribute compound word \"%s\" with %d time frames over "
              "%d subwords.",
              labelMap_->from->symbol(a.input()).c_str(), duration,
              u32(mapping.size()));
        ...
        endTimes.front() = compoundStartTime + 1 +
                           Time(std::floor(mapping.front().length * extraTime));
        for (u32 i = 1; i < endTimes.size(); ++i)
          endTimes[i] = endTimes[i - 1] + 1 +
                        Time(std::floor(mapping[i].length * extraTime));
        verify(endTimes.back() <= compoundEndTime);
        // distribute surplus time frames over last subwords;
        // a better strategy would be to distribute them according to the
        // floored value (see above) or relative sub word length, but this
        // requires expensive sorting for doubtful precision gain.
        verify((compoundEndTime - endTimes.back()) <= Time(mapping.size()));
        for (u32 i = mapping.size() - 1,
                 t = (compoundEndTime - endTimes.back());
             t > 0; --i, --t)
          endTimes[i] += t;
        verify(endTimes.back() == compoundEndTime);
      }
      State *nextSp = s_->newState();
      {
        ...
        if (b_)
          b_->set(nextSp->id(),
                  Boundary(endTimes.front(),
                           Boundary::Transit(Bliss::Phoneme::term,
                                             Bliss::Phoneme::term,
                                             WithinWordBoundary)));
      }
      for (u32 i = 1; i < mapping.size() - 1; ++i) {
        ...
        Fsa::LabelId partLabel = toPartialLabel(thisPart);
        thisSp->newArc(nextSp->id(), partScores, partLabel,
                       (isAcceptor_ ? partLabel : Fsa::Epsilon));
        if (b_)
          b_->set(nextSp->id(),
                  Boundary(endTimes[i], Boundary::Transit(Bliss::Phoneme::term,
                                                          Bliss::Phoneme::term,
                                                          WithinWordBoundary)));
      }
      {
        ...
        ScoresRef partScores = toPartialScores(thisPart, compoundScores);
        Fsa::LabelId partLabel = toPartialLabel(thisPart);
        nextSp->newArc(compoundTarget, partScores, partLabel,
                       (isAcceptor_ ? partLabel : Fsa::Epsilon));
      }
    }
  }
...
ConstLatticeRef applyLabelMap(ConstLatticeRef l, LabelMapRef labelMap) {
  ensure(Lexicon::us()->alphabetId(l->getInputAlphabet()) ==
         Lexicon::us()->alphabetId(labelMap->from));
  ...
  s->setDescription(Core::form("labelMap(%s;%s)", l->describe().c_str(),
                               labelMap->description.c_str()));
  ...
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    ...
    if (mapNonWordsToEpsilon_) {
      l = applyOneToOneLabelMap(
          l, LabelMap::createNonWordToEpsilonMap(alphabetId));
    }
    if (splitCompoundWords_) {
      l = applyLabelMap(l,
                        LabelMap::createCompoundWordSplittingMap(alphabetId));
    }
    if (applyUserMap_) {
      ensure(userMap_);
      if (Lexicon::us()->alphabetId(userMap_->to) !=
          Lexicon::us()->alphabetId(l->getInputAlphabet()))
        criticalError(
            "LabelMapNode: Cannot apply user map, because lattice alphabet "
            "\"%s\" does not match mapping target alphabet \"%s\".",
            Lexicon::us()
                ->alphabetName(Lexicon::us()->alphabetId(l->getInputAlphabet()))
                .c_str(),
            Lexicon::us()
                ->alphabetName(Lexicon::us()->alphabetId(userMap_->to))
                .c_str());
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    userMap_ = LabelMap::load(
        select("map"), Lexicon::us()->alphabet(Lexicon::LemmaAlphabetId));
    applyUserMap_ = bool(userMap_);
    projectInput_ = paramProjectInput(config);
  }
};
const Core::ParameterBool
    LabelMapNode::paramMapToLowerCase("map-to-lower-case",
                                      "map words to lower case", false);
const Core::ParameterBool LabelMapNode::paramMapNonWordsToEpsilon(
    "map-non-words-to-eps",
    "map non-words, i.e. lemmas or lemma pronunciations having an empty "
    "evaluation token sequence, to epsilon",
    false);
const Core::ParameterBool
    LabelMapNode::paramSplitCompoundWords("split-compound-words",
                                          "split compound words", false);
const Core::ParameterBool LabelMapNode::paramProjectInput(
    "project-input", "make lattice an acceptor by mapping input to output",
    false);
NodeRef createLabelMapNode(const std::string &name,
                           const Core::Configuration &config) {
