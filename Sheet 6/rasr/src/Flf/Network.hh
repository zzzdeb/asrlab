...
public:
  Node(const std::string &name, const Core::Configuration &config)
      : Core::Component(config), name(name) {}
  virtual ~Node() {}
  ...
  /**
   * Is called at every node before pull is called at the final nodes (see
   *below); all nodes are initailized before sync is called the first time.
   **/
  virtual void sync() {}
...
/*
  - concatenate the scores of two topologically equal (down to state numbering)
  lattices
*/
