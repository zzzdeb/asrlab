...
public:
  DrawDotDfsState(DotDrawer<_Automaton> &dd, _ConstAutomatonRef f)
    ...
    if (dd_.hints_ & Fsa::HintMarkBest) {
      potentials_ =
          sssp<_Automaton>(transpose<_Automaton>(Precursor::fsa_, false));
  ...
  void discoverState(_ConstStateRef sp) {
    ...
    if (sp->isFinal())
      if (!Precursor::fsa_->semiring()->isDefault(sp->weight_))
        dd_.os_ << "/"
                << Precursor::fsa_->semiring()->describe(sp->weight_,
                                                         dd_.hints_);
    dd_.os_ << "\"";
    if (sp->isFinal())
      ...
      for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a)
        if (isOnBestPath_[a->target()]) {
          _Weight w = Precursor::fsa_->semiring()->extend(
              a->weight_, potentials_[a->target()]);
    ...
    for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if (!Precursor::fsa_->semiring()->isDefault(a->weight()))
        dd_.os_ << "/"
                << Precursor::fsa_->semiring()->describe(a->weight(),
                                                         dd_.hints_);
      dd_.os_ << "\"]" << std::endl;
    }
  }
...
template <class _Automaton>
DotDrawer<_Automaton>::DotDrawer(std::ostream &os, Fsa::Hint hints,
                                 bool progress)
...
template <class _Automaton>
bool drawDot(typename _Automaton::ConstRef f, std::ostream &o, Fsa::Hint hint,
             bool progress) {
...
template <class _Automaton>
bool drawDot(typename _Automaton::ConstRef f, const std::string &file,
