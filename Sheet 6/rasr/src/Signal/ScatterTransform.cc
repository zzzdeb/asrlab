...
bool ScatterTransform::write() {
  ...
  if (Math::Module::instance().formats().write(filename, transform_,
                                               paramOutputPrecision(config))) {
    log("Transformation matrix written to '%s'.", filename.c_str());
  } else {
    error("Failed to store transformation matrix to file '%s'.",
          filename.c_str());
    success = false;
  }
  return success;
...
void ScatterTransform::writeScatterMatrix(
  ...
  if (eigenvalueProblem) {
    ...
    if (!eigenvalueProblem->solveSymmetricAndFinalize(
            scatterMatrix, eigenvalues, eigenvectors)) {
      os << Core::XmlFull(
          "error",
          "Error(s) occured while calculating eigenvalues and eigenvectors.");
    }
    os << eigenvalues;
...
ScatterDiagonalNormalization::ScatterDiagonalNormalization(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c, "normalization"),
  ...
  for (size_t i = 0; i < diagonal.size(); ++i) {
    if (diagonal[i] < 0 && Core::abs(diagonal[i]) > tolerance_) {
      error("Normalization failed because "
            "because %dth diagonal element %f is negative.",
            i, diagonal[i]);
      success = false;
    }
  }
