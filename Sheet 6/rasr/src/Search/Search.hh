...
public:
  ...
  struct ScoreVector {
    ...
    operator Score() const { return acoustic + lm; };
  };
  ...
  public:
    ...
    ScoreVector score;
    Transit transit;
    TracebackItem(const Bliss::LemmaPronunciation *p, TimeframeIndex t,
                  ScoreVector s, Transit te)
  ...
  public:
    ...
    Fsa::ConstAutomatonRef
        lemmaPronunciationAcceptor(Core::Ref<const Bliss::Lexicon>) const;
...
public:
  ...
  virtual bool
  setModelCombination(const Speech::ModelCombination &modelCombination) = 0;
   ...
   */
  virtual u32 lookAheadLength() { return 0; }
  /**
   * Called before feed() with as many acoustic feature vectors from the future
   * as returned by lookaheadLength().  At the end of a segment, less than
   * requested, or even zero feature vectors may be given.
   */
