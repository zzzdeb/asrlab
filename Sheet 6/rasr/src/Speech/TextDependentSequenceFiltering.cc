...
bool AlignmentToSequenceSelectionNode::Set::setParameter(
...
bool AlignmentToSequenceSelectionNode::Set::acceptParameterNamePrefix(
    std::string &n) const {
...
void AlignmentToSequenceSelectionNode::PhonemeSet::setSymbols(
  ...
  empty_ = true;
  for (std::vector<std::string>::const_iterator i = symbols.begin();
       i != symbols.end(); i++) {
...
AlignmentToSequenceSelectionNode::AlignmentToSequenceSelectionNode(
...
bool AlignmentToSequenceSelectionNode::setParameter(const std::string &name,
...
bool AlignmentToSequenceSelectionNode::configure() {
  ...
  Core::Ref<Flow::Attributes> attributeRatio(
      new Flow::Attributes(*attributeSelection));

  if (!configureDatatype(attributeSelection,
  ...
  return putOutputAttributes(0, attributeSelection) &&
         putOutputAttributes(1, attributeRatio);
}
