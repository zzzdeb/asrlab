...
public:
  ...
  u32 hashKey() const {
    u32 value = 0;
    for (std::vector<u32>::const_iterator i = bits_.begin(); i != bits_.end();
         ++i)
      value |= *i;
    return value;
  }
...
struct LogWeighting {
  bool isOrdered_;
  f32 baseProbability_; // probability \alpha for "normal direction" (no
                        // reordering)
  ...
  LogWeighting(ConstSemiringRef semiring, f32 baseProbability, u32 windowSize)
      : baseProbability_(baseProbability), semiring_(semiring),
    ...
    for (u32 i = 1; i <= windowSize_; ++i) {
      logProbabilitiesForReorderedPaths[i] =
          -log((1 - baseProbability_) / (i - 1));
      logProbabilitiesWitoutOrderedPath[i] = -log(1.0 / i);
    }
  }
  void processState(const Bitvector &used, const u32 lowerLimit,
                    const u32 upperLimit) {
    u32 b;
    isOrdered_ = true;
    ...
    for (u32 i = lowerLimit; i < upperLimit; ++i)
      if (!used[i])
        nUncoveredPositionsInWindow_++; // Here: ++ only if this position does
                                        // not violate the distortion limit! (to
                                        // be implemented! -> states_[s].depth_
                                        // must come in as a parameter!)
  }
  const void processArc(const bool isNextReordered, Arc *const a) {
    ...
    if (nUncoveredPositionsInWindow_ == 1)
      ...
      if (isNextReordered)
        weight = Weight(
            logProbabilitiesForReorderedPaths[nUncoveredPositionsInWindow_]);
      else
        weight = Weight(logBaseProbability_);
    } else
      weight = Weight(
          logProbabilitiesWitoutOrderedPath[nUncoveredPositionsInWindow_]);
    //    if (isNextOrdered) weight = Weight(-log(baseProbability_));
    //          else weight =  Weight(-log((1 - baseProbability_) /
    //          (nUncoveredPositionsInWindow_ - 1)));
...
public:
  PermuteAutomatonBase(ConstAutomatonRef f, u32 windowSize,
                       Processing *processing)
      : DfsState(cache(f)), processing_(processing) {
    if (!hasProperties(fsa_, PropertyLinear))
      std::cerr << "permute is defined properly for linear automata only."
                << std::endl;
    ...
    if (arcs_.size() > 0)
      ...
      arcs_[0].weight_ =
          fsa_->semiring()->extend(initialWeight_, arcs_[0].weight_);
  ...
  virtual void discoverState(ConstStateRef sp) {
    for (State::const_iterator a = sp->begin(); a != sp->end(); ++a)
      if ((a->input() == Epsilon) && (a->output() == Epsilon)) {
        if (arcs_.size() > 0)
          arcs_[arcs_.size() - 1].weight_ = fsa_->semiring()->extend(
              arcs_[arcs_.size() - 1].weight(), a->weight());
        else
          initialWeight_ =
              fsa_->semiring()->extend(initialWeight_, a->weight());
      } else
        arcs_.push_back(*a);
    if (sp->isFinal())
...
public:
  PermuteAutomaton(ConstAutomatonRef f, u32 windowSize, u16 distortionLimit,
                   Processing *processing)
  ...
  virtual size_t getMemoryUsed() const {
    return Precursor::fsa_->getMemoryUsed() + sizeof(Precursor::windowSize_) +
           sizeof(Arc) * Precursor::arcs_.size() + states_.getMemoryUsed();
  }
};
...
public:
  IBMPermuteAutomaton(ConstAutomatonRef f, u32 windowSize, u16 distortionLimit,
                      Processing *processing)
      : PermuteAutomaton<Processing>(f, windowSize, distortionLimit,
                                     processing){};
  ...
  virtual void permuteArcs(State *sp) const {
    ...
    u32 lowerLimit = 0;
    u32 upperLimit =
        std::min(size_t(Precursor::states_[s].depth_ + Precursor::windowSize_),
                 Precursor::states_[s].used_.size());
    Precursor::processing_->processState(Precursor::states_[s].used_,
                                         lowerLimit, upperLimit);
    for (u32 i = lowerLimit; i < upperLimit; ++i)
      if (!Precursor::states_[s].used_[i] &&
          (size_t(abs(i - Precursor::states_[s].depth_)) <
           Precursor::distortionLimit_)) { // !!! LIMIT
        u32 depth = Precursor::states_[s].depth_ + 1;
        Arc *a = sp->newArc();
        *a = Precursor::arcs_[i];
        a->target_ = insertState(depth, Precursor::states_[s].used_, i);
        Precursor::processing_->processArc(
            i - Precursor::states_[s].depth_,
            a); // processing_->processArc(i - states_[s].depth_,
                // states_[s].used_[i-1],a);
      }
  }
  virtual std::string describe() const {
    return Core::form("ibm-permute(%s,%d)", Precursor::fsa_->describe().c_str(),
                      Precursor::windowSize_);
  }
};
...
public:
  LocalPermuteAutomaton(ConstAutomatonRef f, u32 windowSize,
                        u16 distortionLimit, Processing *processing)
      : PermuteAutomaton<Processing>(f, windowSize, distortionLimit,
                                     processing){};
  ...
  virtual void permuteArcs(State *sp) const {
    StateId s = sp->id();
    u32 firstBitNotSet;
    for (firstBitNotSet = 0; Precursor::states_[s].used_[firstBitNotSet];
         firstBitNotSet++)
      ;
    u32 upperLimit = std::min(size_t(firstBitNotSet + Precursor::windowSize_),
                              Precursor::states_[s].used_.size());
    Precursor::processing_->processState(Precursor::states_[s].used_,
                                         firstBitNotSet, upperLimit);
    for (u32 i = firstBitNotSet; i < upperLimit; ++i)
      if (!Precursor::states_[s].used_[i]) {
        Arc *a = sp->newArc();
        *a = Precursor::arcs_[i];
        a->target_ = insertState(Precursor::states_[s].depth_ + 1,
                                 Precursor::states_[s].used_, i);
        Precursor::processing_->processArc(i - Precursor::states_[s].depth_, a);
      }
  }
  virtual std::string describe() const {
    return Core::form("local-permute(%s,%d)",
...
public:
  InverseIBMPermuteAutomaton(ConstAutomatonRef f, u32 windowSize,
                             u16 distortionLimit, Processing *processing)
      : PermuteAutomaton<Processing>(f, windowSize, distortionLimit,
                                     processing){};
  ...
  virtual void permuteArcs(State *sp) const {
    ...
    for (firstBitNotSet = 0; Precursor::states_[s].used_[firstBitNotSet];
         firstBitNotSet++)
      ;
    Precursor::processing_->processState(
        Precursor::states_[s].used_, firstBitNotSet,
        (Precursor::states_[s].depth_ - firstBitNotSet < Precursor::windowSize_)
            ? Precursor::states_[s].used_.size()
            : firstBitNotSet + 1);
    // monotone transition
    if (size_t(abs(firstBitNotSet - Precursor::states_[s].depth_)) <
        Precursor::distortionLimit_) {
      Arc *a = sp->newArc();
      *a = Precursor::arcs_[firstBitNotSet];
      a->target_ = insertState(Precursor::states_[s].depth_ + 1,
                               Precursor::states_[s].used_, firstBitNotSet);
      Precursor::processing_->processArc(
          firstBitNotSet - Precursor::states_[s].depth_, a);
    }
    if (Precursor::states_[s].depth_ - firstBitNotSet <
        Precursor::windowSize_) { // if we can do non-monotone transitions
      for (u32 i = firstBitNotSet + 1; i < Precursor::states_[s].used_.size();
           i++) {
        if (!Precursor::states_[s].used_[i] &&
            (size_t(abs(i - Precursor::states_[s].depth_)) <
             Precursor::distortionLimit_)) { // DISTORTION LIMIT HERE !
          Arc *a = sp->newArc();
          *a = Precursor::arcs_[i];
          a->target_ = insertState(Precursor::states_[s].depth_ + 1,
                                   Precursor::states_[s].used_, i);
          Precursor::processing_->processArc(i - Precursor::states_[s].depth_,
                                             a);
        }
      }
    }
  ...
  virtual std::string describe() const {
    return Core::form("inverse-ibm-permute(%s,%d)",
                      Precursor::fsa_->describe().c_str(),
                      Precursor::windowSize_);
  }
};
...
public:
  DoubleLocalPermuteAutomaton(ConstAutomatonRef f, u32 windowSize,
                              u16 distortionLimit, Processing *processing)
      : PermuteAutomaton<Processing>(f, windowSize, distortionLimit,
                                     processing){};
  ...
  virtual void permuteArcs(State *sp) const {
    ...
    for (u32 i = lowerLimit; i < upperLimit; ++i)
      if (!Precursor::states_[s].used_[i]) {
        u32 depth;
        //			    depth = upperLimit;
        // 			    for (u32 j = states_[s].depth_; j < upperLimit;
        // ++j) 				if ((j != i) && (!states_[s].used_[j])) { 				    depth =
        // std::min(depth, j); 				    break;
        // 				}
        if (i == Precursor::states_[s].depth_)
          for (depth = Precursor::states_[s].depth_ + 1;
               depth < Precursor::states_[s].used_.size() &&
               Precursor::states_[s].used_[depth];
               depth++)
            ;
        else
    ...
    for (upperLimit = Precursor::states_[s].used_.size() - 1;
         Precursor::states_[s].used_[upperLimit] && upperLimit > 0;
         upperLimit--)
      ;
    lowerLimit = std::max(
        int(upperLimit - Precursor::windowSize_),
        int(Precursor::states_[s].depth_ + Precursor::windowSize_ - 1));
  ...
  virtual std::string describe() const {
    return Core::form("double-local-permute(%s,%d)",
                      Precursor::fsa_->describe().c_str(),
                      Precursor::windowSize_);
  }
};
...
private:
  ...
  struct Spans : public std::vector<Span> {
    ...
    Spans(const Spans &x, u32 newpos) : Parent(x) {
    ...
    friend bool operator==(const Spans &a, const Spans &b) {
      return (a.size() == b.size() &&
              memcmp(&a[0], &b[0], a.size() * sizeof(Span)) == 0);
    }
  };
  ...
  struct State_ {
    ...
    State_(u32 depth, const State_ &x, u32 newpos)
        : depth_(depth), used_(x.used_, newpos),
    ...
    bool operator==(const State_ &s) const {
      return (used_ == s.used_ && spans_ == s.spans_ &&
              (last_ == s.last_ ||
               (last_ < s.last_ ? used_.isCovered(last_, s.last_)
                                : used_.isCovered(s.last_, last_))));
    }
    ...
    friend std::ostream &operator<<(std::ostream &out, const State_ &x) {
      return out << x.depth_ << " " << x.used_ << " " << x.spans_ << " "
                 << x.last_;
    }
  };
  struct StateHashKey_ {
...
public:
  ITGPermuteAutomaton(ConstAutomatonRef f, u32 windowSize, u16 distortionLimit,
                      Processing *processing)
      : PermuteAutomatonBase<Processing>(f, windowSize, processing) {
    states_.insert(State_(0, Bitvector(Precursor::arcs_.size())));
  };
  ...
  virtual void permuteArcs(State *sp) const {
    ...
    u32 lowerLimit = 0;
    u32 upperLimit =
        std::min(size_t(states_[s].depth_ + Precursor::windowSize_),
                 states_[s].used_.size());

    for (u32 i = lowerLimit; i < upperLimit; ++i)
      if (!states_[s].used_[i]) {
        const State_ &localState = states_[s];

        // this checks a necessary condition for a valid ITG reordering (nice
        // speedup)
        ...
        if (isOK) {
          ...
          Arc *a = sp->newArc();
          *a = Precursor::arcs_[i];
          Precursor::processing_->processArc(localState.getLastPosition(), i,
                                             a);
          a->target_ =
              insertState(depth, localState,
                          i); // ATTENTION: insertState() invalidates localState
        }
      }
  }
  ...
  virtual std::string describe() const {
    return Core::form("itg-permute(%s,%d)", Precursor::fsa_->describe().c_str(),
                      Precursor::windowSize_);
  }
};

ConstAutomatonRef permute(ConstAutomatonRef f,
                          u32 windowSize = Core::Type<u32>::max,
                          u16 distortionLimit = Core::Type<u16>::max);
ConstAutomatonRef localPermute(ConstAutomatonRef f,
                               u32 windowSize = Core::Type<u32>::max,
                               u16 distortionLimit = Core::Type<u16>::max);
ConstAutomatonRef inverseIBMPermute(ConstAutomatonRef f,
                                    u32 windowSize = Core::Type<u32>::max,
                                    u16 distortionLimit = Core::Type<u16>::max);
ConstAutomatonRef
doubleLocalPermute(ConstAutomatonRef f, u32 windowSize = Core::Type<u32>::max,
                   u16 distortionLimit = Core::Type<u16>::max);
ConstAutomatonRef ITGPermute(ConstAutomatonRef f,
                             u32 windowSize = Core::Type<u32>::max,
                             u16 distortionLimit = Core::Type<u16>::max);
