...
public:
  ...
  */
  ...
  public:
    row_vector_iterator(f64 *begin, size_t size) : ptr_(begin), size_(size) {}
  ...
  */
  ...
  public:
    column_iterator(f64 *begin, size_t offset) : ptr_(begin), offset_(offset) {}
  ...
  */
  struct PtrLessFcn {
    ...
    size_t operator()(const FloatBox *const &val1,
                      const FloatBox *const &val2) const {
      return (*val1 < *val2);
    }
  };
  ...
  */
  struct PtrHashFcn {
    ...
    size_t operator()(const FloatBox *const &val) const { return hasher(*val); }
  };
...
public:
  FloatBox() : rows_(1), columns_(1), size_(1) {
    init();
    *begin_ = 0.0;
  }
  FloatBox(size_t size) : rows_(1), columns_(size), size_(size) {
    init();
    ::memset((void *)begin_, '\0', size_ * sizeof(f64));
  }
  FloatBox(size_t rows, size_t columns)
      : rows_(rows), columns_(columns), size_(rows * columns) {
    init();
    ::memset((void *)begin_, '\0', size_ * sizeof(f64));
  }
  FloatBox(const FloatBox &floatBox)
      : rows_(floatBox.rows_), columns_(floatBox.columns_),
        size_(floatBox.size_) {
    init();
    ::memcpy((void *)begin_, (const void *)floatBox.begin_,
             size_ * sizeof(f64));
  }
  ~FloatBox() { delete[] begin_; }
  ...
  row_vector_iterator end_row() { return row_vector_iterator(end_, 0); }
...
public:
  ...
  Example() : properties(0), values(0), nObs(0) {}
  Example(Properties *properties, FloatBox *values)
      : properties(properties), values(values), nObs(0) {}
  ~Example() {
    delete properties;
    delete values;
  }
