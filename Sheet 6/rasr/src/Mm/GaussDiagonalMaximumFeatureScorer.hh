...
/** Feature-scorer for MixtureSet of Gauss densities with diagonal covariance
 * matrices Scores are calculated with maximum approximation.
 */
...
public:
  ...
  densitiesInMixture(MixtureIndex mix) const {
...
/** Feature-scorer for MixtureSet of Gauss densities with diagonal covariance
 * matrices The maximum approximation is not applied here.
 */
...
protected:
  ...
  void
  calculateScoresAndNumberOfDensities(const CachedAssigningContextScorer *cs,
                                      MixtureIndex mixtureIndex) const;
  size_t maximumNumberOfDensities() const;

public:
  GaussDiagonalSumFeatureScorer(const Core::Configuration &c,
                                Core::Ref<const MixtureSet> mixtureSet);
  ...
  calculateScoreAndDensity(const CachedAssigningContextScorer *cs,
                           MixtureIndex mixtureIndex) const;
  virtual void
  calculateDensityPosteriorProbabilities(const CachedAssigningContextScorer *,
                                         Score denominator, EmissionIndex e,
