...
const Core::ParameterStringVector
    SegmentwiseAlignmentGenerator::paramSilencesAndNoises(
        "silences-and-noises", "list of silence and noise lemmata (strings)",
        ",");

SegmentwiseAlignmentGenerator::SegmentwiseAlignmentGenerator(
    const Core::Configuration &c,
    Core::Ref<const ModelCombination> modelCombination)
    : Core::Component(c), portId_(Flow::IllegalPortId),
      modelCombination_(modelCombination), allophoneStateGraphBuilder_(0),
      aligner_(select("aligner")) {
  if (!modelCombination_) {
    ModelCombinationRef mc = ModelCombinationRef(new ModelCombination(
  ...
  allophoneStateGraphBuilder_ = new AllophoneStateGraphBuilder(
      select("allophone-state-graph-builder"), modelCombination_->lexicon(),
...
FsaCache *SegmentwiseAlignmentGenerator::createFsaCache(
...
void SegmentwiseAlignmentGenerator::setSpeechSegmentId(
    const std::string &segmentId) {
  update(segmentId);
}

void SegmentwiseAlignmentGenerator::setSpeechSegment(
    Bliss::SpeechSegment *segment) {
  verify(segment);
  update(segment->fullName());
}

void SegmentwiseAlignmentGenerator::signOn(
    Speech::CorpusVisitor &corpusVisitor) {
...
void SegmentwiseAlignmentGenerator::getScorers(
  ...
  Core::Ref<const Mm::FeatureScorer> featureScorer(
      acousticModel()->featureScorer());
  scorers.clear();
  for (SegmentwiseFeatures::const_iterator f = features()->begin(),
                                           featureEnd = features()->end();
       f != featureEnd; ++f) {
...
void SegmentwiseAlignmentGenerator::getScorers(
  ...
  if (features()->size() < tend) {
    if (features()->size() <= tbeg) {
      criticalError(
          "Requested alignment is out of range, [%d,%d] not in [0,%d].", tbeg,
          tend, features()->size());
    } else {
      warning("Requested alignment is out of range, [%d,%d] not in [0,%d]; "
              "align [%d,%d].",
  ...
  Core::Ref<const Mm::FeatureScorer> featureScorer(
      modelCombination()->acousticModel()->featureScorer());
...
OrthographyAlignmentGenerator::OrthographyAlignmentGenerator(
  ...
  lemmaPronunciationToLemma_ =
      modelCombination()
          ->lexicon()
          ->createLemmaPronunciationToLemmaTransducer();
...
void OrthographyAlignmentGenerator::update(const std::string &segmentId,
...
void OrthographyAlignmentGenerator::setSpeechSegment(
    Bliss::SpeechSegment *segment) {
  Precursor::setSpeechSegment(segment);
  update(segment->fullName(), segment->orth());
}

void OrthographyAlignmentGenerator::setSpeechSegmentId(
    const std::string &segmentId) {
  Precursor::setSpeechSegmentId(segmentId);
  update(segmentId, "");
}

Core::Archive *OrthographyAlignmentGenerator::createAlignmentCache(
    const Core::Configuration &config) {
  ...
  if (ar.isOpen()) {
    ...
    Core::XmlOutputStream xw(
        new Core::ArchiveWriter(*alignmentCache, "DEPENDENCIES"));
    dependencies().write(xw);
  }
  return alignmentCache;
}

void OrthographyAlignmentGenerator::setOrthography(
    const std::string &orthography) {
...
Fsa::ConstAutomatonRef OrthographyAlignmentGenerator::getAlignmentFsa() {
  ...
  if (!isValid_) {
    ...
    Fsa::ConstAutomatonRef transducer = transducerCache_->get(
        allophoneStateGraphBuilder_->createTransducerFunctor(
            segmentId_ /* + "/" + orthography_ */, orthography_));
    wordLatticeBuilder_->setModelTransducer(transducer);
    // ???
    Fsa::ConstAutomatonRef modelAcceptor = modelAcceptorCache_->get(
        allophoneStateGraphBuilder_->createFinalizationFunctor(
            segmentId_ /* + "/" + orthography_ */, transducer));
...
const Alignment *
OrthographyAlignmentGenerator::getAlignment(const std::string &orthography) {
  setOrthography(orthography);
  if (alignmentCache_) {
    Core::ArchiveReader reader(*alignmentCache_,
                               segmentId_ /* + "/" + orthography_ */);
    if (reader.isOpen()) {
  ...
  if (alignmentCache_) {
    Core::ArchiveWriter writer(*alignmentCache_,
                               segmentId_ /* + "/" + orthography_ */);
    if (writer.isOpen()) {
...
OrthographyAlignmentGenerator::getWordLattice(const std::string &orthography) {
  setOrthography(orthography);
  Fsa::ConstAutomatonRef alignmentFsa = getAlignmentFsa();
  if (!alignmentFsa)
  ...
  if (!wordLattice)
    error("Failed to generate word lattice for segment '%s'.",
          segmentId_.c_str());
  return wordLattice;
}
