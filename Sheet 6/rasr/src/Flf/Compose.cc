...
protected:
  virtual bool init_(NetworkCrawler &crawler,
...
public:
  ...
  virtual ConstLatticeRef sendLattice(Port to) {
    ...
    if (!l || !r) {
      warning(
          "ComposeNode: At least one of the lattices to be composed is empty.");
...
const Core::ParameterBool ComposeNode::paramUnweightLeft(
    "unweight-left", "make left lattice an unweighted transducer", false);
const Core::ParameterBool ComposeNode::paramUnweightRight(
    "unweight-right", "make right lattice an unweighted transducer", false);
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
void composeBoundaries(ConstLatticeRef composition, ConstLatticeRef l,
                       ConstLatticeRef r) {
  ...
  if (l->getBoundaries()->valid()) {
    if (r->getBoundaries()->valid()) {
      if (l->getBoundaries() != r->getBoundaries()) {
        ...
        b = ConstBoundariesRef(new MappedBoundaries(
            l->getBoundaries(), FtlWrapper::mapToLeft(composition)));
      } else
        b = ConstBoundariesRef(new MappedBoundaries(
            l->getBoundaries(), FtlWrapper::mapToLeft(composition)));
    } else
    ...
    b = ConstBoundariesRef(new MappedBoundaries(
        r->getBoundaries(), FtlWrapper::mapToRight(composition)));
  }
...
public:
  ComposeMatchingNode(const std::string &name,
                      const Core::Configuration &config)
      : ComposeNode(name, config) {}
  ~ComposeMatchingNode() {}
};

NodeRef createComposeMatchingNode(const std::string &name,
                                  const Core::Configuration &config) {
...
public:
  ComposeSequencingNode(const std::string &name,
                        const Core::Configuration &config)
      : ComposeNode(name, config) {}
  ~ComposeSequencingNode() {}
};

NodeRef createComposeSequencingNode(const std::string &name,
                                    const Core::Configuration &config) {
...
NodeRef createDifferenceNode(const std::string &name,
                             const Core::Configuration &config) {
 ...
 **/
ConstLatticeRef intersect(ConstLatticeRef l, ConstLatticeRef r,
                          bool appendScores) {
  if (appendScores) {
...
const Core::ParameterBool
    IntersectionNode::paramAppend("append", "append scores instead of extend",
                                  false);

NodeRef createIntersectionNode(const std::string &name,
                               const Core::Configuration &config) {
 ...
 * compose with fsa
 **/
ConstLatticeRef composeWithFsa(ConstLatticeRef l, Fsa::ConstAutomatonRef f,
                               Score scale) {
...
ConstLatticeRef composeWithFsa(ConstLatticeRef l, Fsa::ConstAutomatonRef f,
  ...
  return composeSequencing(l,
                           fromFsa(f, l->semiring(), id, l->semiring()->one()));
}
...
protected:
  virtual ConstLatticeRef rescore(ConstLatticeRef l1, ScoreId id) {
    ConstLatticeRef l2;
    if (connected(1)) {
      Fsa::ConstAutomatonRef fsa = Fsa::ConstAutomatonRef(
          static_cast<const Fsa::Automaton *>(requestData(1)));
      fsa->releaseReference();
      if (scale() != Semiring::DefaultScale)
        fsa = Fsa::multiply(fsa, Fsa::Weight(scale()));
      l2 = fromFsa(fsa_, l1->semiring(), l1->semiring()->one(), id,
                   inputAlphabetId_, outputAlphabetId_);
    } else {
      verify(fsa_);
      if (!cachedL2_ || (l1->semiring().get() != cachedL2_->semiring().get())) {
        cachedL2_ =
            persistent(fromFsa(fsa_, l1->semiring(), l1->semiring()->one(), id,
                               inputAlphabetId_, outputAlphabetId_));
      }
      l2 = cachedL2_;
    }
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (alphabetName.empty()) {
      ...
      inputAlphabetId_ = outputAlphabetId_ =
          Lexicon::us()->alphabetId(alphabetName, true);
    if (!connected(1)) {
      std::string filename = paramFile(config);
      if (filename.empty())
        criticalError("ComposeWithFsaNode: Need either input at port 1 or a "
                      "valid filename");
...
const Core::ParameterString
    ComposeWithFsaNode::paramFile("file", "name of fsa file", "");
const Core::ParameterString ComposeWithFsaNode::paramName("name", "name",
                                                          "lemma");

NodeRef createComposeWithFsaNode(const std::string &name,
                                 const Core::Configuration &config) {
