...
/** Parameter adaptor functions and functors for passing corpus section
 * parameters to DatasSource objects,
 */
...
public:
  ...
  void set(const std::string &name, const std::string &value) {
    ...
    for (dataSource = dataSources_.begin(); dataSource != dataSources_.end();
         ++dataSource)
...
/** Parameter adaptor functions and functors for passing corpus section
 * parameters to CorpusKeys objects.
 */
...
public:
  ...
  void set(const std::string &name, const std::string &value) {
    ...
    for (corpusKey = corpusKeys_.begin(); corpusKey != corpusKeys_.end();
         ++corpusKey)
...
void setParameter(size_t index, Bliss::Recording *recording,
...
template <class ParameterAdaptor>
void setParameter(size_t index, Bliss::Segment *segment,
                  ParameterAdaptor parameterAdaptor) {
  parameterAdaptor.set("id", segment->fullName());
  parameterAdaptor.set("segment-index", Core::form("%zd", index));
  parameterAdaptor.set("segment-type",
                       std::string(Bliss::Segment::typeId[segment->type()]));
  parameterAdaptor.set("acoustic-condition", segment->condition()
...
template <class ParameterAdaptor>
void clearParameter(Bliss::Segment *segment,
                    ParameterAdaptor parameterAdaptor) {}

template <class ParameterAdaptor>
void setParameter(size_t index, Bliss::SpeechSegment *speechSegment,
                  ParameterAdaptor parameterAdaptor) {
  ...
  if (speechSegment->speaker() != 0) {
    ...
    parameterAdaptor.set(
        "gender", Bliss::Speaker::genderId[speechSegment->speaker()->gender()]);
  }
  parameterAdaptor.set("orthography", speechSegment->orth());
}

template <class ParameterAdaptor>
void clearParameter(Bliss::SpeechSegment *speechSegment,
                    ParameterAdaptor parameterAdaptor) {
...
CorpusVisitor::CorpusVisitor(const Core::Configuration &c)
    : Core::Component(c), recordingIndex_(0), segmentIndex_(0) {}
...
void CorpusVisitor::enterRecording(Bliss::Recording *recording) {
  ...
  setParameter(recordingIndex_, recording,
               DataSourceParameterAdaptor(dataSources_));
  setParameter(recordingIndex_, recording,
               StringExpressionAdaptor(corpusKeys_));
...
void CorpusVisitor::visitSegment(Bliss::Segment *segment) {
  setParameter(segmentIndex_, segment,
               DataSourceParameterAdaptor(dataSources_));
...
void CorpusVisitor::visitSpeechSegment(Bliss::SpeechSegment *speechSegment) {
  setParameter(segmentIndex_, speechSegment,
               DataSourceParameterAdaptor(dataSources_));
  setParameter(segmentIndex_, speechSegment,
               StringExpressionAdaptor(corpusKeys_));
