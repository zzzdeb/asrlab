...
bool Aggregate::read(Core::BinaryInputStream &is) {
  bool sameDatatype;
  is >> sameDatatype;
  return Timestamp::read(is) &&
         (sameDatatype ? readSameType(is) : readDifferentTypes(is));
}

bool Aggregate::write(Core::BinaryOutputStream &os) const {
  bool sameDatatype = isSameDataType();
  os << sameDatatype;
  return Timestamp::write(os) &&
         (sameDatatype ? writeSameType(os) : writeDifferentTypes(os));
}
