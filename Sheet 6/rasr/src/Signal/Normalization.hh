 ...
 *  Processing:
 *    updateStatistics is called if no new data entered or old data left the
 * sliding window finalize() called right before normalization is applied to an
 * output vector apply() called to perform normalization Interface: Use
 * update(..) to put new data into the sliding window and get the latest
 * normalized output Use flush to get normalized output without any input
 */
class Normalization {
public:
  typedef f32 Value;
...
public:
   ...
   *  @param right is the output point calculated from the right side of the
   * sliding window. (Note: data flows into the sliding window from right to
   * left.)
 ...
 */
class MeanAndVarianceNormalization : public Core::Component,
...
/**
 *  Output is divided by the arithmetic mean of the norm of the elements of the
 * sliding window.
 *
 *  In case of the 2nd norm, this normalization ensures that average frame
 * energy will be 1.
 */
