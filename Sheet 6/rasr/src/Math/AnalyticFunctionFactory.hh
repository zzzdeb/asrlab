...
private:
   ...
   *  @return is invalid if declaration at position @param position does not
   * match the syntax.
   *  @param position is set to the furthermost unparsed character (independent
   * of success).
   */
...
public:
  enum DomainType {
    ...
    discreteDomain,
    /** omega = 2 * pi * n / N, where n is discrete frequency index and N is
...
private:
  /** Matches @param s in @param declaration starting at position @param
   * position.
   *  @return if true if match was successfull.
   *  If successfully matched @param position will point at the first unparsed
   * position. Whitespaces are ignored.
   */
   ...
   */
  void acceptWhitespaces(const std::string &declaration,
                         std::string::size_type &position) const;
...
private:
  ...
  /** Creates a function by parsing @param declaration starting at position
   * @param position.
   *  @return is invalid if parsing failed.
   */
  AnalyticFunctionRef create(const std::string &declaration,
                             std::string::size_type &position) const;

  /** Creates a nested function object f(g(x)).
   *  Declaration syntax: nest(g, f).
   *  Remark: f and g are swaped in declaration syntax since g changes maximal
   * argument of f. Parsing could be improved...
   */
  AnalyticFunctionRef parseNesting(const std::string &declaration,
                                   std::string::size_type &position) const;
  /** Creates a inverse function object f(x)^(-1).
   *  Declaration syntax: invert(f).
   */
  AnalyticFunctionRef parseInvertion(const std::string &declaration,
                                     std::string::size_type &position) const;
  /** Creates a maximum normalization function object f(x)^(-1).
   *  Declaration syntax: norm-max(f).
   */
  AnalyticFunctionRef
  parseMaximumNormalization(const std::string &declaration,
                            std::string::size_type &position) const;

  /** Creates an identity function object f(x) = x.
   *  Declaration syntax: identity.
   */
  AnalyticFunctionRef parseIdentity(const std::string &declaration,
                                    std::string::size_type &position) const;
  /** Creates a constant function object f(x) = c.
   *  Declaration syntax: const(c).
   */
  AnalyticFunctionRef parseConstant(const std::string &declaration,
                                    std::string::size_type &position) const;
  /** Creates an offset function object f(x) = x + b.
   *  Declaration syntax: offset(b).
   */
  AnalyticFunctionRef parseOffset(const std::string &declaration,
                                  std::string::size_type &position) const;
  /** Creates an scaling function object f(x) = a * x.
   *  Declaration syntax: scaling(a).
   */
  AnalyticFunctionRef parseScaling(const std::string &declaration,
                                   std::string::size_type &position) const;

  /** Creates a addition function object f(x, y) = x + y.
   *  Declaration syntax: plus.
   */
  AnalyticFunctionRef parseAddition(const std::string &declaration,
                                    std::string::size_type &position) const;
  /** Creates a addition function object f(x, y) = x - y.
   *  Declaration syntax: minus.
   */
  AnalyticFunctionRef parseSubstraction(const std::string &declaration,
                                        std::string::size_type &position) const;
  /** Creates a addition function object f(x, y) = x * y.
   *  Declaration syntax: multiplies.
   */
  AnalyticFunctionRef
  parseMultiplication(const std::string &declaration,
                      std::string::size_type &position) const;
  /** Creates a addition function object f(x, y) = x / y.
   *  Declaration syntax: divides.
   */
  AnalyticFunctionRef parseDivision(const std::string &declaration,
                                    std::string::size_type &position) const;

  /** Creates an Sinh function object f(x) = sinh(x).
   *  Declaration syntax: sinh.
   */
  AnalyticFunctionRef parseSinh(const std::string &declaration,
                                std::string::size_type &position) const;
  /** Creates an ArcSinh function object f(x) = asinh(x).
   *  Declaration syntax: asinh.
   */
  AnalyticFunctionRef parseArcSinh(const std::string &declaration,
                                   std::string::size_type &position) const;
  /** Creates an Sinh function object f(x) = cosh(x).
   *  Declaration syntax: cosh.
   */
  AnalyticFunctionRef parseCosh(const std::string &declaration,
                                std::string::size_type &position) const;
  /** Creates an ArcSinh function object f(x) = acosh(x).
   *  Declaration syntax: acosh.
   */
  AnalyticFunctionRef parseArcCosh(const std::string &declaration,
                                   std::string::size_type &position) const;
   ...
   */
  AnalyticFunctionRef
  parseDiscreteToContinuous(const std::string &declaration,
                            std::string::size_type &position) const;
  /** Creates an scaling function object f(x) = x * sampleRate_.
   *  Declaration syntax: cont-to-disc.
   */
  AnalyticFunctionRef
  parseContinuousToDiscrete(const std::string &declaration,
                            std::string::size_type &position) const;
   ...
   */
  AnalyticFunctionRef
  parseMelWarpingFunction(const std::string &declaration,
                          std::string::size_type &position) const;
  /** Creates a bark warping function object (@see createBarkWarpingFunction).
   *  Declaration syntax: bark.
   */
  AnalyticFunctionRef
  parseBarkWarpingFunction(const std::string &declaration,
                           std::string::size_type &position) const;
  /** Creates a two-piece linear function function object (@see
   * createTwoPieceLinearFunction). Declaration syntax: linear-2(a, limit).
   */
  AnalyticFunctionRef
  parseTwoPieceLinearFunction(const std::string &declaration,
                              std::string::size_type &position) const;
  /** Creates a bilinear warping function object (@see
   * createBilinearWarpingFunction). Declaration syntax: bilinear(a).
   */
  AnalyticFunctionRef
  parseBilinearWarpingFunction(const std::string &declaration,
                               std::string::size_type &position) const;
  /** Creates a three-piece affine function function object (@see
   * createThreePieceAffineFunction). Declaration syntax: affine-3(a, a-shift,
   * lower-limit, upper-limit).
   */
  AnalyticFunctionRef
  parseThreePieceAffineFunction(const std::string &declaration,
                                std::string::size_type &position) const;
   ...
   */
  AnalyticFunctionRef
  parseEqualLoudnessPreemphasis(const std::string &declaration,
                                std::string::size_type &position) const;
   ...
   */
  AnalyticFunctionRef
  parseEqualLoudnessPreemphasis40dB(const std::string &declaration,
                                    std::string::size_type &position) const;
...
public:
   ...
   *  @return is invalid if parsing failed.
   */
  UnaryAnalyticFunctionRef
  createUnaryFunction(const std::string &declaration) const {
    return castToUnaryFunction(create(declaration));
  }
  /** Creates a binary function by parsing @param declaration.
   *  @return is invalid if parsing failed.
   */
  BinaryAnalyticFunctionRef
  createBinaryFunction(const std::string &declaration) const {
...
public:
  ...
  static UnaryAnalyticFunctionRef
  createConstant(UnaryAnalyticFunction::Argument c) {
  ...
  static UnaryAnalyticFunctionRef
  createScaling(UnaryAnalyticFunction::Argument a) {
    return UnaryAnalyticFunctionRef(new ScalingFunction(a));
  }
  /** Creates a OffsetFunction object. */
  static UnaryAnalyticFunctionRef
  createOffset(UnaryAnalyticFunction::Argument b) {
    return UnaryAnalyticFunctionRef(new OffsetFunction(b));
  }
  /** Creates an object for f(x) = a * x + b */
  static UnaryAnalyticFunctionRef
  createLinear(UnaryAnalyticFunction::Argument a,
               UnaryAnalyticFunction::Argument b) {
   ...
   */
  UnaryAnalyticFunctionRef
  createMaximumNormalization(UnaryAnalyticFunctionRef f) const {
    return nest(createScaling(maximalArgument_ / f->value(maximalArgument_)),
                f);
  }
...
public:
   ...
   *
   *  Discrete input:
   *    i_{Mel} = 2595 * log10 (1 + i / frequencySampleRate / 700) *
   * frequencySampleRate
   *
   *    if @param discretizeArgument is true then floorf is applied to the
   * frequency:
   *      -'i / frequencySampleRate' replaced by 'floorf(i /
   * frequencySampleRate)' -floorf gives less skipped warped indices -Acctually
   * applied for combatibility with the old standard system.
   *
   *    Note: sampleRate has to be initialized by sampling rate on frequency
   * axis: i.e.: #FFT-points / time-domain-sampling-rate.
   *
   *  Normalized omega input:
   *    omega_{Mel} = 2595 * log10 (1 + omega * sampleRate / (2 * pi) / 700) *
   * (2 * pi) / sampleRate Note: sampleRate is the one in time domain.
   */
  UnaryAnalyticFunctionRef
  createMelWarpingFunction(bool discretizeArgument = false) const;
  /** Creates an object implementing the Bark warping function
   *  Continuous input:
   *    f_{Bark} = 6 * ln(f / 600 + sqrt((f / 600)^2 + 1)) =  6 * asinh(f /
   * 600); Remark: @see ArchSinh Discrete input: i_{Bark} = 6 * asinh(i /
   * frequencySampleRate / 600) * frequencySampleRate Normalized omega input:
   *    omega_{Mel} = 6 * asinh(omega * sampleRate / (2 * pi) / 600) * (2 * pi)
   * / sampleRate Note: sampleRate is the one in time domain.
   */
   ...
   *   Note: this case is equal to the mathematical inverse of the warping
   * function set up with (limit, 1/a).
   */
  UnaryAnalyticFunctionRef
  createTwoPieceLinearFunction(UnaryAnalyticFunction::Argument warpingFactor,
                               UnaryAnalyticFunction::Argument limit) const;
  /** Bilinear Warping Function.
   *  x_{B} = x - T / pi * arctan(a * sin(2 * pi / T * x) / (1 + a * cos(2 * pi
   * / T * x))), where a = 1 - warping-factor, and T/2 is the point which is
   * mapped on itself.
   */
   ...
   *
   * a <= 1:
   *   f < lowerLimit : f_warped = ((a * lowerLimit + aShift * (a -
   * 1))/lowerLimit) * f lowerLimit <= f < upperLimit : f_warped = a * f +
   * aShift * (a-1) f >= upperLimit : f_warped =  m * f + c where m = (1 - (a *
   * upperLimit + aShift * (a - 1)))(1 - upperLimit)
   *
   * a > 1
   *   Note: this case is equal to the mathematical inverse of the warping
   * function set up with (lowerLimit, upper_limit, 1/a).
   */
