...
u32 TokenAlphabet::nDisambiguators() const { return nDisambiguators_; }
...
PhonemeAlphabet::PhonemeAlphabet(Core::Ref<const PhonemeInventory> pi)
    : TokenAlphabet(LexiconRef(), pi->phonemes_), pi_(pi) {}
...
std::string LemmaPronunciationAlphabet::symbol(Fsa::LabelId id) const {
  ...
  verify(lp);
  return std::string(lp->lemma()->name()) + std::string("   /") +
...
Fsa::LabelId LemmaPronunciationAlphabet::disambiguator(u32 d) const {
  require(d <=
          Core::Type<Fsa::LabelId>::max - lexicon_->nLemmaPronunciations());
  if (d >= nDisambiguators_)
    nDisambiguators_ = d + 1;
...
// ===========================================================================
LemmaAlphabet::LemmaAlphabet(LexiconRef l) : TokenAlphabet(l, l->lemmas_) {}
...
LetterAlphabet::LetterAlphabet(LexiconRef l) : TokenAlphabet(l, l->letters_) {}
