...
private:
  std::pair<Fsa::StateId, Fsa::StateId>
  fromUnionSid(Fsa::StateId unionSid) const {
  ...
  UnionBoundaries(const ConstLatticeRefList &lats)
    ...
    for (u32 i = 1; i < nLats_; ++i) {
      ...
      if (initialBoundary_.valid()) {
        ...
        else if (!(currentInitialBoundary.transit() ==
                   initialBoundary_.transit()))
...
public:
  UnionLattice(const ConstLatticeRefList &lats, ConstSemiringRef semiring)
      : SlaveLattice(lats[0]), nLats_(lats.size()), lats_(lats),
        semiring_(semiring) {
    verify(semiring);
    ...
    for (u32 i = 0; i < nLats_; ++i) {
      ...
      verify(fsa_->getInputAlphabet().get() ==
             currentLattice->getInputAlphabet().get());
    ...
    if (unionSid == 0) {
      ...
      for (u32 i = 0; i < nLats_; ++i) {
        ...
        for (State::const_iterator a = sr->begin(); a != sr->end(); ++a)
          sp->newArc(toUnionSid(i, a->target()), a->weight(), a->input(),
                     a->output());
      }
    } else {
      std::pair<Fsa::StateId, Fsa::StateId> sid = fromUnionSid(unionSid);
...
public:
  ...
  virtual void sync() { union_.reset(); }
};
...
struct MeshedFullBoundaryBuilder {
  ...
  Boundary operator()(const Boundary &b) const { return b; }
};
...
struct MeshedTimeBoundaryBuilder {
  ...
  typedef Fsa::Hash<Boundary, Boundary::Hash, Boundary::Equal> HashList;
  Boundary operator()(const Boundary &b) const { return Boundary(b.time()); }
};
} // namespace
...
ConstLatticeRef buildMesh(const ConstLatticeRefList &lats,
                          ConstSemiringRef semiring,
                          const MeshedBoundaryBuilder &meshedBoundaryBuilder =
                              MeshedBoundaryBuilder()) {
  ...
  for (u32 i = 0; i < lats.size(); ++i) {
    ...
    const Boundary initialB =
        meshedBoundaryBuilder(l->boundary(topologicalSort->front()));
    ...
    for (u32 j = 0; j < topologicalSort->size(); ++j) {
      ...
      for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
        const Boundary targetB =
            meshedBoundaryBuilder(l->boundary(a->target()));
        ...
        } else if ((meshB.time() >= targetB.time()) &&
                   (meshSp->id() != meshTarget.first))
          Core::Application::us()->warning(
              "Lattice \"%s\" contains null/negative-length arcs; meshed "
              "lattice might be cyclic.",
              l->describe().c_str());
        /*
        ...
        */
        if (meshSp->id() != meshTarget.first)
          meshSp->newArc(meshTarget.first, a->weight(), a->input(),
                         a->output());
      }
    }
  }
  if (initialSids.size() == 1)
    ...
    for (Core::Vector<Fsa::StateId>::const_iterator itSid = initialSids.begin();
      ...
      if (*itSid != meshInitialSp->id())
        meshInitialSp->newArc(*itSid, semiring->one(), Fsa::Epsilon,
                              Fsa::Epsilon);
...
ConstLatticeRef mesh(const ConstLatticeRefList &lats, ConstSemiringRef semiring,
                     MeshType meshType) {
  if (lats.empty())
    return ConstLatticeRef();
  if (!semiring) {
...
ConstLatticeRef mesh(ConstLatticeRef l1, ConstLatticeRef l2,
                     ConstSemiringRef semiring, MeshType meshType) {
  ConstLatticeRefList lats(2);
  lats[0] = l1;
  lats[1] = l2;
...
public:
  ...
  virtual void sync() { meshL_.reset(); }
};
const Core::Choice MeshNode::choiceMeshType("full", MeshTypeFullBoundary,
                                            "time", MeshTypeTimeBoundary,
                                            Core::Choice::endMark());
const Core::ParameterChoice MeshNode::paramMeshType("mesh-type",
