...
public:
  ...
  // 2) s * Zero := Zero, s * One = One, i.e. Zero and One are defined
...
private:
  ...
  static inline bool isSentinel(const Scores *obj) { return obj == sentinel(); }
...
public:
  ScoresRef() : obj_(Scores::sentinel()) { obj_->acquireReference(); }
  explicit ScoresRef(Scores *obj) : obj_(obj) {
    require_(obj);
    obj_->acquireReference();
  }
  ScoresRef(const ScoresRef &ref) : obj_(ref.obj_) { obj_->acquireReference(); }
  ~ScoresRef() {
    if (obj_->releaseReference()) {
  ...
  operator bool() const { return Scores::isNotSentinel(obj_); }
  ScoresRef &operator=(const ScoresRef &ref) {
    set(ref.obj_);
    return *this;
  }
  bool operator==(const ScoresRef &ref) const { return obj_ == ref.obj_; }
  bool operator!=(const ScoresRef &ref) const { return obj_ != ref.obj_; }
  bool operator!() const { return Scores::isSentinel(obj_); }
  ...
  Scores *get() const { return Scores::isNotSentinel(obj_) ? obj_ : 0; }
  void reset() { set(Scores::sentinel()); }
};
