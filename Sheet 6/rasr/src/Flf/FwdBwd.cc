...
struct FwdBwd::Internal {
  ...
  Internal(const ConstSemiringRefList &semirings)
      : semirings(semirings), states(0), arcs(0), min(Core::Type<f64>::max),
        max(Core::Type<f64>::min), sum(Core::Type<f64>::min) {}
  ~Internal() {
    delete[] states;
    delete[] arcs;
  }
...
FwdBwd::FwdBwd() : internal_(0) {}

FwdBwd::~FwdBwd() { delete internal_; }

s32 FwdBwd::n() const { return internal_->semirings.size(); }

const ConstSemiringRefList &FwdBwd::semirings() const {
  return internal_->semirings;
}

f64 FwdBwd::min() const { return internal_->min; }

f64 FwdBwd::max() const { return internal_->max; }

f64 FwdBwd::sum() const { return internal_->sum; }
...
protected:
  ...
  protected:
      ...
      if (sr->isFinal()) {
        if (sr->hasArcs())
          Core::Application::us()->error(
              "In Fwd./Bwd. Algorithm: Lattice \"%s\"has final state with "
              "outgoing arcs, "
              "ignore final status",
              fsa_->describe().c_str());
        else
    ...
    virtual void finishState(Fsa::StateId sid) {
      ...
      properties.topologicalSort->maxSid =
          std::max(properties.topologicalSort->maxSid, sid);
    }
    ...
    void traverse() {
      ...
      if (properties.finalStateIds.empty())
        Core::Application::us()->criticalError(
            ...
            fsa_->describe().c_str());
      std::reverse(properties.topologicalSort->begin(),
                   properties.topologicalSort->end());
      fsa_->setTopologicalSort(properties.topologicalSort);
    }

  public:
    TraverseLatticeProperties(ConstLatticeRef l, Properties &properties)
        : DfsState(l), l(l), b(l->getBoundaries()), properties(properties) {
  ...
  public:
    TraverseLattice(ConstLatticeRef l, Properties &properties,
                    StaticLattice &staticL, StaticBoundaries &staticB)
        : Precursor(l, properties), staticL(staticL), staticB(staticB) {
      traverse();
    }
  ...
  protected:
    virtual void discoverState(ConstStateRef sr) {
      ...
      sp->setId(unionSid);
      for (Flf::State::iterator a = sp->begin(), a_end = sp->end(); a != a_end;
           ++a)
  ...
  public:
    TraverseSubLattice(ConstLatticeRef l, Properties &properties,
                       StaticLattice &staticL, StaticBoundaries &staticB)
        : Precursor(l, properties), staticL(staticL), staticB(staticB) {
      traverse();
    }
...
public:
  ...
  */
  ConstLatticeRef operator()(ConstLatticeRef l,
                             const FwdBwd::Parameters &params, FwdBwd *fb = 0) {
    ...
    bool hasScores = ((params.scoreId != Semiring::InvalidId) ||
                      (params.riskId != Semiring::InvalidId));
    /*
    ...
    */
    ...
    if (fb) {
      fbScores = fb->internal_ =
          new FwdBwd::Internal(ConstSemiringRefList(1, posteriorSemiring));
    ...
    */
    ...
    ScoreArc *nextFwdArcScores = fwdArcScores,
             *endFwdArcScores = fwdArcScores + properties.nArcs;
    ScoreArc *nextBwdArcScores = bwdArcScores,
             *endBwdArcScores = bwdArcScores + properties.nArcs;
    /*
    ...
    */
    for (StateMap::const_reverse_iterator itSid = topologicalSort->rbegin(),
      ...
      if (!sp->hasArcs()) {
        ...
        if (hasRisk)
          stateScore.cost = stateScore.genBwdScore =
              sp->weight()->get(params.costId);
      } else {
        stateScore.bwdScore = stateScore.cost = 0.0;
        for (Flf::State::const_iterator a = sp->begin(), a_end = sp->end();
             a != a_end; ++a) {
          ...
          // f64 bwdScore = ((targetStateScore.bwdScore == Semiring::Zero) ||
          // (score == Semiring::Zero)) ? Semiring::Zero :
          // targetStateScore.bwdScore + score;
    ...
    */
    ...
    for (StateMap::const_iterator itSid = topologicalSort->begin() + 1,
      ...
      Fsa::StateId sid = *itSid;
      ScoreState &stateScore = stateScores[sid];
      for (const ScoreArc *a = stateScore.bwdBegin, *a_end = stateScore.bwdEnd;
           a != a_end; ++a) {
        ScoreState &targetStateScore = stateScores[a->target];
        f64 fwdScore = targetStateScore.fwdScore + a->score;

        // f64 fwdScore = ((targetStateScore.fwdScore == Semiring::Zero) ||
        // (a->score == Semiring::Zero)) ? Semiring::Zero :
        // targetStateScore.fwdScore + a->score;
    ...
    */
    ...
    f64 genBwdSum =
        (hasRisk) ? stateScores[topologicalSort->front()].genBwdScore : 0.0;
    for (Core::Vector<Fsa::StateId>::const_iterator
             itSid = properties.finalStateIds.begin(),
             endSid = properties.finalStateIds.end();
         itSid != endSid; ++itSid) {
      ScoreState &stateScore = stateScores[*itSid];
      stateScore.fwdScore += stateScore.bwdScore;

      // stateScore.fwdScore = ((stateScore.fwdScore == Semiring::Zero) ||
      // (stateScore.bwdScore == Semiring::Zero)) ? Semiring::Zero :
      // stateScore.fwdScore + stateScore.bwdScore;
    ...
    col->reset();
    {
      ...
      if ((deviation <= OOneInterval.first) ||
          (OOneInterval.second <= deviation))
        Core::Application::us()->warning(
            "Numerical instability: fwd./bwd. scores are not equal; "
            "fwd-score=%f, bwd-score=%f, probability quotient is %f and "
            "exceeds [0.99, 1.01]; ",
    ...
    if (hasRisk) {
      ...
      if ((deviation <= OOneInterval.first) ||
          (OOneInterval.second <= deviation))
        Core::Application::us()->warning(
            "Numerical instability: generalized fwd./bwd. scores are not "
            "equal; "
            "gen-fwd-score=%f, gen-bwd-score=%f, probability quotient is %f "
            "and exceeds [0.99, 1.01]; ",
    ...
    */
    ...
    if (hasRisk) {
      Core::Application::us()->log("Expected cost for lattice \"%s\" is %f.",
                                   l->describe().c_str(),
                                   0.5 * (genFwdSum + genBwdSum));
    }
    ...
    if (hasScores) {
      ...
      for (StateMap::const_iterator itSid = topologicalSort->begin(),
        ...
        for (const ScoreArc *fa = stateScore.fwdBegin,
                            *fa_end = stateScore.fwdEnd;
             fa != fa_end; ++fa, ++a) {
          ScoreState &targetStateScore = stateScores[fa->target];
          ScoresRef scores = a->weight_ = semiring.clone(a->weight());
          Score fbScore = stateScore.fwdScore + fa->score +
                          targetStateScore.bwdScore - normScore;
          if (params.scoreId != Semiring::InvalidId)
            scores->set(params.scoreId, fbScore);
          if (hasRisk) {
            f64 genFbScore = stateScore.genFwdScore + fa->cost +
                             targetStateScore.genBwdScore - genNormScore;
            Score risk = (genFbScore >= 0.0)
                             ? ::exp(::log(genFbScore) - fbScore)
                             : -::exp(::log(-genFbScore) - fbScore);
    ...
    if (fbScores) {
      ...
      for (StateMap::const_iterator itSid = topologicalSort->begin(),
        ...
        for (const ScoreArc *fa = stateScore.fwdBegin,
                            *fa_end = stateScore.fwdEnd;
             fa != fa_end; ++fa, ++fbState.end_) {
          fbState.end_->arcScore = fa->score;
          fbState.end_->fbScore = stateScore.fwdScore + fa->score +
                                  stateScores[fa->target].bwdScore;
          fbState.end_->normScore = normScore;
          Score posteriorScore =
              fbState.end_->fbScore - fbState.end_->normScore;
  ...
  */
  ConstLatticeRef operator()(const ConstLatticeRefList &_lats,
    ...
    ScoreList weights;
    /*
      Normalize weights, exclude lattices with weight zero; find posterior
      semiring
    */
    {
      ...
      for (u32 i = 0; i < _lats.size(); ++i) {
        ...
        if ((weight > 0.0) && _lats[i] &&
            (_lats[i]->initialStateId() != Fsa::InvalidStateId)) {
          indexMap.push_back(i);
          lats.push_back(_lats[i]);
          weights.push_back(weight);
          posteriorSemirings.push_back(
              (!params.posteriorSemirings.empty() &&
               params.posteriorSemirings[i])
                  ? params.posteriorSemirings[i]
                  : toLogSemiring(_lats[i]->semiring(),
                                  (params.alphas.empty() ? Score(0.0)
                                                         : params.alphas[i])));
        }
        normWeight += weight;
      }
      ...
      for (ScoreList::iterator itWeight = weights.begin();
           itWeight != weights.end(); ++itWeight)
        *itWeight *= normWeight;
    }
    const SemiringCombinationHelper &semiringCombo =
        *params.combination->semiringCombination();
    /*
    ...
    */
    ...
    u32 nUnionStates = 2, nInitialUnionArcs = lats.size(),
        nUnionArcs = lats.size();
    Time unionStartTime = Core::Type<Time>::max,
         unionEndTime = Core::Type<Time>::min;
    ...
    unionFinalSp->setFinal(
        unionL->semiring()->clone(unionL->semiring()->one()));
    ...
    */
    ...
    for (u32 i = 0; i < lats.size(); ++i) {
      ...
      Fsa::LabelId systemLabel =
          hasSystemLabels ? params.systemLabels[paramIndex] : Fsa::Epsilon;
      // DEPR begin
      ...
      */
      for (StateMap::const_reverse_iterator itSid = topologicalSort->rbegin(),
        ...
        if (!unionSp->hasArcs()) {
          ...
          for (Flf::State::const_iterator a = unionSp->begin(),
                                          a_end = unionSp->end();
               a != a_end; ++a) {
      ...
      */
      ...
      for (StateMap::const_iterator itSid = topologicalSort->begin() + 1,
        ...
        Fsa::StateId sid = *itSid;
        ScoreState &stateScore = stateScores[sid];
        for (const ScoreArc *a = stateScore.bwdBegin,
                            *a_end = stateScore.bwdEnd;
             a != a_end; ++a)
      ...
      */
      ...
      if (normId != Semiring::InvalidId) {
        if (params.fsaNorms[paramIndex]) {
          Fsa::ConstAutomatonRef total = Fsa::staticCopy(
              Fsa::changeSemiring(toFsa(lats[i], posteriorSemiring->scales()),
      ...
      /*
        Connect union initial state to sub-initial state and set fwd/bwd scores
        for initial arcs
      */
      {
        ...
        unionInitialSp->newArc(topologicalSort->front() + offsetSid, scores,
                               Fsa::Epsilon, systemLabel);
      }
      /*
        Connect sub-final states to union final state and set fwd/bwd scores for
        remaining arcs
      */
      for (StateMap::const_iterator itSid = topologicalSort->begin(),
                                    endSid = topologicalSort->end();
        ...
        if (sp->isFinal()) {
          ...
          FwdBwd::State &fbState = fbScores->states[sid + offsetSid];
          if (params.scoreId != Semiring::InvalidId)
            scores->set(params.scoreId,
                        fbState.fwdScore + fbState.bwdScore - normScore);
          ...
          for (const ScoreArc *fa = stateScore.fwdBegin,
            ...
            if (params.scoreId != Semiring::InvalidId) {
              Score fbScore = stateScore.fwdScore + fa->score +
                              stateScores[fa->target].bwdScore - normScore;
              scores->set(params.scoreId, fbScore);
            }

            // DEPR begin
            if ((normId != Semiring::InvalidId) ||
                (weightId != Semiring::InvalidId)) {
              Score normDuration =
                  normT * Score(unionB->time(a->target()) - beginT);
              if (normId != Semiring::InvalidId)
    ...
    */
    if (fbScores) {
      f64 unionFwdBwdSum = 0.0;
      {
        ...
        for (FwdBwd::Arc *itArc = fbState.begin_; itArc != fbState.end_;
             ++itArc)
          col->feed(itArc->fbScore - itArc->normScore);
      }
      unionFwdBwdSum = col->get();
      col->reset();
      if ((unionFwdBwdSum <= OOneInterval.first) ||
          (OOneInterval.second <= unionFwdBwdSum))
        Core::Application::us()->warning("Numerical instability: sum over "
                                         "initial arcs exceeds [0.99, 1.01]; "
    ...
    /*
      Change to posterior semiring, preserving the scales used for FB
      calculation
    */
    if (params.setPosteriorSemiring) {
      ...
      ConstSemiringRef unionPosteriorSemiring =
          Semiring::create(Fsa::SemiringTypeLog, unionL->semiring()->size(),
                           unionPosteriorScales, unionL->semiring()->keys());
      ...
      /* {
         ...
         Fsa::SemiringTypeLog, unionL->semiring()->size(), unionPosteriorScales,
         unionL->semiring()->keys());
         //dbg("Union posterior semiring:  " << unionPosteriorSemiring->name());
         //dbg("Single posterior semiring: " <<
         singlePosteriorSemiring->name());
         //dbg("Check log-linear models ...");
         std::pair<ConstLatticeRef, ConstFwdBwdRef> result =
         FwdBwd::build(unionL, unionPosteriorSemiring);
         //dbg("Union: " << result.second->sum() << " = 0.0");
         std::vector<f64> Z(lats.size()), Zp(lats.size());
         for (u32 i = 0; i < lats.size(); ++i) {
         u32 paramIndex = indexMap[i];
         ConstLatticeRef l = persistent(filterByOutput(unionL,
         params.systemLabels[paramIndex])); result = FwdBwd::build(l,
         unionPosteriorSemiring);
         //dbg("Weight[" << i << "]:  " << ::exp(-result.second->sum()) << " = "
         << weights[i]); result = FwdBwd::build(l, singlePosteriorSemiring);
         Z[i] = 0.0;
         for (ConstStateRef sr = l->getState(l->initialStateId());
         sr->hasArcs(); sr = l->getState(sr->begin()->target())) Z[i] +=
         sr->begin()->weight()->get(params.normIds[paramIndex]); Zp[i] =
         -result.second->sum(); Fsa::ConstAutomatonRef total = Fsa::staticCopy(
         Fsa::changeSemiring(
         toFsa(
         l,
         ...
         dbg(Core::form("FB-Norm[%d], Fsa:            %.10f", i,
         f64(totalInv)));
         }
         //dbg("Check weights ...");
...
const std::pair<f64, f64> FwdBwd::Builder::OOneInterval(-0.00995033085316808285,
                                                        0.01005033585350144118);
const std::pair<f64, f64>
    FwdBwd::Builder::OFiveInterval(-0.04879016416943200307,
                                   0.05129329438755053343);

FwdBwd::Parameters::Parameters()
    : alpha(0.0), posteriorSemiring(), costId(Semiring::InvalidId),
      scoreId(Semiring::InvalidId), riskId(Semiring::InvalidId),
      normRisk(false) {}

void FwdBwd::Parameters::verifyConsistency() const {
  if ((riskId != Semiring::InvalidId) && (costId == Semiring::InvalidId))
    Core::Application::us()->criticalError(
        "Risk calculation requires a dimension storing the arc-wise cost.");
}

std::pair<ConstLatticeRef, ConstFwdBwdRef>
FwdBwd::build(ConstLatticeRef l, const FwdBwd::Parameters &params) {
...
std::pair<ConstLatticeRef, ConstFwdBwdRef>
FwdBwd::build(ConstLatticeRef l, ConstSemiringRef posteriorSemiring) {
  Parameters params;
...
FwdBwd::CombinationParameters::CombinationParameters()
    : weights(), alphas(), posteriorSemirings(), systemAlphabet(),
      systemLabels(), combination(), scoreId(Semiring::InvalidId) {}
...
void FwdBwd::CombinationParameters::verifyConsistency(u32 n) const {
  ...
  if (systemAlphabet) {
    ...
    for (LabelIdList::const_iterator itLabel = systemLabels.begin();
         itLabel != systemLabels.end(); ++itLabel)
      if (*itLabel == Fsa::InvalidLabelId)
        Core::Application::us()->criticalError(
            "System label must not be invalid.");
  }
...
std::pair<ConstLatticeRef, ConstFwdBwdRef>
FwdBwd::build(const ConstLatticeRefList &lats,
              const FwdBwd::CombinationParameters &params) {
...
std::pair<ConstLatticeRef, ConstFwdBwdRef>
FwdBwd::build(const ConstLatticeRefList &lats, const ScoreList &weights,
...
// -------------------------------------------------------------------------
namespace {
const Core::ParameterString paramKey("key",
                                     "dimension to store fwd./bwd. score", "");
const Core::ParameterString paramLabel("label", "label/symbol/word", "");
const Core::ParameterFloat paramWeight("weight", "weight", 1.0);
const Core::ParameterFloat paramAlpha("alpha", "fwd./bwd. score alpha", 0.0);
const Core::ParameterBool paramFsa("fsa", "use fsa algorithm", false);
const Core::ParameterBool paramNormalize("normalize", "normalize", false);
const Core::ParameterBool
    paramSystemLabels("system-labels", "store system labels as output", false);
const Core::ParameterBool
    paramSetPosteriorSemiring("set-posterior-semiring",
  ...
  struct SingleConfiguration {
    ...
    void dump(std::ostream &os) const {
      if (semiring) {
        ...
        if (params.scoreId != Semiring::InvalidId)
          os << "Store fwd./bwd. scores in dimension \""
             << semiring->key(params.scoreId) << "\"." << std::endl;
        if (params.riskId != Semiring::InvalidId) {
          os << "Store risk in dimension \"" << semiring->key(params.riskId)
             << "\"." << std::endl;
          if (params.normRisk)
      ...
      if (params.costId != Semiring::InvalidId)
        os << "\tcost-key = \"" << semiring->key(params.costId) << "\""
           << std::endl;
    }
  };
  ...
  struct CombinationConfiguration {
    ...
    void dump(std::ostream &os) const {
      if (params.combination) {
        os << "Target semiring is \"" << params.combination->semiring()->name()
           << "\"." << std::endl;
        if (params.scoreId != Semiring::InvalidId)
          os << "Store fwd./bwd. scores in dimension \""
             << params.combination->semiring()->key(params.scoreId) << "\"."
             << std::endl;
      }
      if (params.setPosteriorSemiring)
        os << "Set semiring used for fwd/bwd-score calculation on resulting "
              "lattice."
           << std::endl;
      for (u32 i = 0; i < params.weights.size(); ++i) {
        ...
        if (params.posteriorSemirings[i]) {
          os << "\tsemiring   = " << params.posteriorSemirings[i]->name()
             << std::endl;
        } else {
        ...
        if (params.systemLabels[i] != Fsa::InvalidLabelId)
          os << "\tlabel      = "
             << params.systemAlphabet->symbol(params.systemLabels[i])
             << std::endl;
        if (params.normIds[i] != Semiring::InvalidId) {
          os << "\tnorm-key   = "
             << params.combination->semiring()->key(params.normIds[i]);
        ...
        if (params.weightIds[i] != Semiring::InvalidId)
          os << "\tweight-key = "
             << params.combination->semiring()->key(params.weightIds[i])
             << std::endl;
      }
    }
  };
...
public:
  Internal(const Core::Configuration &config, const KeyList &extensionKeys,
           const ScoreList &extensionScales)
      : Core::Component(config), configurationChannel(config, "configuration"),
        statisticsChannel(config, "statistics"), singleConfig(0),
        comboConfig(0), extensionKeys(extensionKeys),
  ...
  void update(ConstLatticeRef l) {
    ...
    else if ((singleConfig->semiring.get() == l->semiring().get()) ||
    ...
    params.alpha = (alpha == Core::Type<Core::ParameterFloat::Value>::max)
                       ? Core::Type<Score>::max
                       : Score(alpha);
    params.posteriorSemiring =
        Semiring::create(Core::Configuration(config, "semiring"));
    Key scoreKey = paramKey(select("score"));
    for (KeyList::const_iterator itExtKey = extensionKeys.begin();
         itExtKey != extensionKeys.end(); ++itExtKey)
      if (singleConfig->semiring->id(*itExtKey) == Semiring::InvalidId)
        criticalError("Subsequent processings require a dimension \"%s\", "
                      "which is not provided by semiring \"%s\".",
                      itExtKey->c_str(),
                      singleConfig->semiring->name().c_str());
    if (scoreExtensionId != Semiring::InvalidId)
      singleConfig->params.scoreId = SemiringCombinationHelper::getIdOrDie(
          singleConfig->semiring, extensionKeys[scoreExtensionId]);
    Key riskKey = paramKey(select("risk"));
    if (!riskKey.empty())
      singleConfig->params.riskId = SemiringCombinationHelper::getIdOrDie(
          singleConfig->semiring, riskKey);
    singleConfig->params.normRisk = paramNormalize(select("risk"));
    Key costKey = paramKey(select("cost"));
    if (!costKey.empty())
      singleConfig->params.costId = SemiringCombinationHelper::getIdOrDie(
          singleConfig->semiring, costKey);
    singleConfig->params.verifyConsistency();
    if (configurationChannel.isOpen()) {
      configurationChannel << Core::XmlOpen("configuration") +
                                  Core::XmlAttribute("component",
                                                     this->name()) +
  ...
  void update(const ConstLatticeRefList &lats) {
    if (!comboConfig) {
      ...
      comboConfig->params.combination =
          CombinationHelper::create(SemiringCombinationHelper::getType(
                                        SemiringCombinationHelper::paramType(
                                            select("score-combination"))),
    ...
    if (comboConfig->params.combination->update(lats)) {
      ...
      SemiringCombinationHelper &semiringCombo =
          *params.combination->semiringCombination();
      if (scoreExtensionId != Semiring::InvalidId)
        params.scoreId = semiringCombo.combinationId(scoreExtensionId);
      Lexicon::SymbolMap systemSymbolMap;
      if (paramSystemLabels(config)) {
        params.systemAlphabet =
            Lexicon::us()->alphabet(Lexicon::LemmaAlphabetId);
        systemSymbolMap = Lexicon::us()->symbolMap(Lexicon::LemmaAlphabetId);
      }
      for (u32 i = params.weights.size(); i < lats.size(); ++i) {
        const Core::Configuration subConfig =
            select(Core::form("lattice-%d", i));
        params.weights.push_back(paramWeight(subConfig));
        params.posteriorSemirings.push_back(
            Semiring::create(Core::Configuration(subConfig, "semiring")));
        Core::ParameterFloat::Value alpha = paramAlpha(subConfig);
        params.alphas.push_back(
            (alpha == Core::Type<Core::ParameterFloat::Value>::max)
                ? Core::Type<Score>::max
                : Score(alpha));
        ...
        if (!normKey.empty()) {
          ...
          params.fsaNorms.push_back(
              paramFsa(Core::Configuration(subConfig, "norm")));
        } else {
...
FwdBwdBuilder::~FwdBwdBuilder() { delete internal_; }

std::pair<ConstLatticeRef, ConstFwdBwdRef>
FwdBwdBuilder::build(ConstLatticeRef l) {
  internal_->update(l);
  FwdBwdRef fb = FwdBwdRef(new FwdBwd);
...
std::pair<ConstLatticeRef, ConstFwdBwdRef>
FwdBwdBuilder::build(const ConstLatticeRefList &lats) {
  internal_->update(lats);
  FwdBwdRef fb = FwdBwdRef(new FwdBwd);
  ConstLatticeRef l =
      buildFwdBwd(lats, internal_->comboConfig->params, fb.get());
  internal_->dumpStatistics(fb, "FB-combination");
  return std::make_pair(l, fb);
}

FwdBwdBuilderRef FwdBwdBuilder::create(const Core::Configuration &config,
                                       const KeyList &extensionKeys,
                                       const ScoreList &extensionScales) {
  FwdBwdBuilder *fwdBwdBuilder = new FwdBwdBuilder;
  fwdBwdBuilder->internal_ =
      new FwdBwdBuilder::Internal(config, extensionKeys, extensionScales);
    ...
    if (!hasNext_) {
      ...
      if (!lats.empty()) {
        std::pair<ConstLatticeRef, ConstFwdBwdRef> fbResult =
            (multiLatticeAlgorithm_) ? fbBuilder_->build(lats)
                                     : fbBuilder_->build(lats.front());
        l_ = fbResult.first;
        fb_ = fbResult.second;
      }
      hasNext_ = true;
...
const Core::ParameterBool FwdBwdBuilderNode::paramForce("force", "forc", false);
NodeRef createFwdBwdBuilderNode(const std::string &name,
