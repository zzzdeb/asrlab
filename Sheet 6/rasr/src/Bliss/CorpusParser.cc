...
class Bliss::SpeakerDescriptionElement
    : public Core::XmlBuilderElement<Speaker, Core::XmlMixedElement,
                                     Core::CreateUsingNew> {
  typedef Core::XmlBuilderElement<Speaker, Core::XmlMixedElement,
                                  Core::CreateUsingNew>
      Precursor;
...
SpeakerDescriptionElement::SpeakerDescriptionElement(Core::XmlContext *_context,
                                                     Handler _handler)
    : Precursor("speaker-description", _context, _handler) {
  addChild(collect(new XmlStringBuilderElement(
      "gender", this, XmlStringBuilderElement::handler(&Self::gender))));
...
class Bliss::ConditionDescriptionElement
    : public Core::XmlBuilderElement<AcousticCondition, Core::XmlMixedElement,
                                     Core::CreateUsingNew> {
  typedef Core::XmlBuilderElement<AcousticCondition, Core::XmlMixedElement,
                                  Core::CreateUsingNew>
      Precursor;
...
OrthographyElement::OrthographyElement(Core::XmlContext *_context,
                                       Handler _handler /*= 0*/)
...
const Core::ParameterString CorpusDescriptionParser::paramAudioDir(
    "audio-dir", "directory with audio files", "",
    "This path will be prefixed to all audio file names, unless they are "
    "absolute.  If unset, the directory of the corpus file will be used.");

const Core::ParameterString CorpusDescriptionParser::paramVideoDir(
    "video-dir", "directory with video files", "",
    "This path will be prefixed to all video file names, unless they are "
    "absolute.  If unset, the directory of the corpus file will be used.");

const Core::ParameterBool
    CorpusDescriptionParser::paramCaptializeTranscriptions(
        "capitalize-transcriptions",
        "convert all transcriptions to upper case: yes/no", false);

const Core::ParameterBool CorpusDescriptionParser::paramGemenizeTranscriptions(
    "gemenize-transcriptions",
    "convert all transcriptions to lower case: yes/no", false);
...
void CorpusDescriptionParser::initSchema() {
  ...
  XmlElement *segmentConditionDesc = collect(new ConditionDescriptionElement(
      this,
      ConditionDescriptionElement::handler(&Self::defineSegmentCondition)));
  ...
  XmlElement *description_decl =
      collect(new XmlIgnoreElement("description", this)); // FFE

  XmlRegularElement *segment = new XmlRegularElementRelay(
      "segment", this, startHandler(&Self::startSegment),
  ...
  XmlRegularElement *recording = new XmlRegularElementRelay(
      "recording", this, startHandler(&Self::startRecording),
  ...
  XmlElement *include_decl = collect(
      new XmlEmptyElementRelay("include", this, startHandler(&Self::include)));

  XmlMixedElement *subcorpus = new XmlMixedElementRelay(
      // should use XmlRegularElement, but cannot due to nesting restriction
      "subcorpus", this, startHandler(&Self::startSubcorpus),
      endHandler(&Self::endSubcorpus), 0, XML_NO_MORE_CHILDREN);
  collect(subcorpus);
  ...
  XmlRegularElement *corpus = new XmlRegularElementRelay(
      "corpus", this, startHandler(&Self::startCorpus),
  ...
  corpus->addTransition(0, 0, recording), corpus->addFinalState(0);
...
const Speaker *
CorpusDescriptionParser::getSpeaker(const std::string &name) const {
...
void CorpusDescriptionParser::defineSpeaker(std::auto_ptr<Speaker> &speaker) {
  ...
  if (s->isAnonymous()) {
    ...
    if (currentSection_->speakers_.hasKey(s->name())) {
      error("speaker \"%s\" already defined in this section",
            s->name().c_str());
      delete s;
      return;
    }
...
void CorpusDescriptionParser::selectDefaultSpeaker(
    const Core::XmlAttributes atts) {
  verify(currentSection_); // speaker declaration outside corpus
  verify(!segment_);
...
void CorpusDescriptionParser::defineSegmentSpeaker(
    std::auto_ptr<Speaker> &speaker) {
  verify(segment_);
...
void CorpusDescriptionParser::selectSegmentSpeaker(
    const Core::XmlAttributes atts) {
  verify(segment_);
...
const AcousticCondition *
CorpusDescriptionParser::getCondition(const std::string &name) const {
  require(currentSection_);
...
void CorpusDescriptionParser::defineCondition(
    std::auto_ptr<AcousticCondition> &condition) {
  ...
  if (s->isAnonymous()) {
    ...
    if (currentSection_->conditions_.hasKey(s->name())) {
      error("condition \"%s\" already defined in this section",
            s->name().c_str());
      delete s;
      return;
    }
...
void CorpusDescriptionParser::selectDefaultCondition(
    const Core::XmlAttributes atts) {
  verify(currentSection_); // condition declaration outside corpus
  verify(!segment_);
...
void CorpusDescriptionParser::defineSegmentCondition(
    std::auto_ptr<AcousticCondition> &condition) {
  verify(segment_);
...
void CorpusDescriptionParser::selectSegmentCondition(
    const Core::XmlAttributes atts) {
  verify(segment_);
...
// ===========================================================================
int CorpusDescriptionParser::accept(const std::string &filename,
