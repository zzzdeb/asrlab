...
//======================================================================================
const std::string
    WordLatticeDescription::defaultName("word-lattice-description");
...
void WordLatticeDescription::initialize(ConstWordLatticeRef lattice) {
  ...
  for (size_t i = 0; i < lattice->nParts(); ++i) {
    operator[](i).setValue(Lattice::WordLatticeDescription::nameModel,
                           lattice->name(i));
    operator[](i).setValue(Lattice::WordLatticeDescription::nameLevel,
                           defaultLevel);
  }
}
...
public:
  MaximumTimeDfsState(Fsa::ConstAutomatonRef fsa,
...
StandardWordLattice::StandardWordLattice(
...
void StandardWordLattice::newArc(
    Fsa::State *source, Fsa::State *target,
    const Bliss::LemmaPronunciation *lemmaPronunciation, Speech::Score acoustic,
    Speech::Score lm) {
  source->newArc(target->id(), Fsa::Weight(acoustic),
                 (lemmaPronunciation) ? lemmaPronunciation->id()
                                      : Fsa::Epsilon);

  lm_->state(source->id())
      ->newArc(target->id(), Fsa::Weight(lm),
...
private:
  ...
  struct TimeConditionedStateHash {
    u32 operator()(const TimeConditionedState &s) const {
      return (2239 * s.wordBoundary().time() +
              s.wordBoundary().transit().final);
    }
  };
  struct TimeConditionedStateEqual {
    bool operator()(const TimeConditionedState &s1,
                    const TimeConditionedState &s2) const {
      return (s1.wordBoundary() == s2.wordBoundary());
    }
  };
  typedef Fsa::Hash<TimeConditionedState, TimeConditionedStateHash,
                    TimeConditionedStateEqual>
      States;
  ...
  struct ByInputAndTarget
      : public std::binary_function<Fsa::Arc, Fsa::Arc, bool> {
    bool operator()(const Fsa::Arc &a, const Fsa::Arc &b) const {
      return ((a.input() < b.input()) ||
              ((a.input() == b.input()) && (a.target() < b.target())));
    }
  };
...
public:
  ...
  virtual Fsa::StateId initialStateId() const {
    // this construciton is necessary to guarantee the existence of the word
    // boundary
    Fsa::StateId s = Fsa::SlaveAutomaton::fsa_
                         ->getState(Fsa::SlaveAutomaton::fsa_->initialStateId())
                         ->id();
    return states_.insert(TimeConditionedState((*wordBoundaries_)[s]));
  }
  virtual void discoverState(Fsa::ConstStateRef sp) {
    States::Cursor s =
        states_.insert(TimeConditionedState((*wordBoundaries_)[sp->id()]));
  ...
  virtual Fsa::ConstStateRef getState(Fsa::StateId s) const {
    ...
    for (MergedStates::const_iterator it = states.begin(); it != states.end();
         ++it) {
  ...
  virtual std::string describe() const {
    return Core::form("time-conditioned-word-lattice(%s)",
                      Fsa::SlaveAutomaton::fsa_->describe().c_str());
  }
  virtual size_t getMemoryUsed() const {
    return Fsa::SlaveAutomaton::fsa_->getMemoryUsed() +
           states_.getMemoryUsed() + stateMap_.getMemoryUsed();
  }
...
ConstWordLatticeRef
Lattice::timeConditionedWordLattice(ConstWordLatticeRef in) {
  Core::Ref<const TimeConditionedWordLattice> f(
      new TimeConditionedWordLattice(in));
