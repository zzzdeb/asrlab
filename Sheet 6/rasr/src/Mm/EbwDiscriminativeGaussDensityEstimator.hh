 ...
 */
class EbwDiscriminativeGaussDensityEstimator
...
protected:
  virtual const Mean *
  getMean(Core::Ref<EbwDiscriminativeMeanEstimator> e) const {
    return e->estimate();
  }
...
public:
  ...
  virtual std::vector<Accumulator::SumType>
  dtSum(const CovarianceToMeanSetMap::MeanSet &) const;
  Weight denWeight() const { return denAccumulator_.weight(); }
  virtual Weight dtWeight() const { return weight() - denWeight(); }
};
...
protected:
  virtual const std::vector<MeanType> &
  iMean(Core::Ref<AbstractMeanEstimator> m) const {
    return required_cast(EbwDiscriminativeMeanEstimatorWithISmoothing *,
                         m.get())
        ->iMean();
  }
  virtual const Mean *
...
public:
  EbwDiscriminativeCovarianceEstimatorWithISmoothing(
      ComponentIndex dimension = 0);
  virtual ~EbwDiscriminativeCovarianceEstimatorWithISmoothing() {}

  virtual void reset();

  virtual std::vector<Precursor::Accumulator::SumType>
  dtSum(const CovarianceToMeanSetMap::MeanSet &) const;
  virtual Weight dtWeight() const {
    return dtWeight() + ISmoothing::constant();
  }
};
