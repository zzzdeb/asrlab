...
Node::Node(const Core::Configuration &c)
    : Component(c), Precursor(c), filter_(0), datatype_(0),
  ...
  if (inputLink != 0) {
    if (inputLink->areAttributesAvailable()) {
      ...
      if (predecessorNode->configure()) {
        result = inputLink->attributes();
        // If fails, from-node of the input link did not generate output
        // attributes although configuration did not fail.
        ensure(result);
      } else {
        error() << "Configuration of node '" << predecessorNode->name()
                << "' failed.";
...
/*****************************************************************************/
{
  ...
  if (attributesChannel_.isOpen()) {
    attributesChannel_ << Core::XmlOpen("dump-attributes") +
                              Core::XmlAttribute("node", fullName());
    if (nOutputLinks(out) > 0)
...
/*****************************************************************************/
{
  for (std::vector<std::vector<Link *> >::iterator output = outputs_.begin();
    ...
    for (std::vector<Link *>::iterator link = output->begin();
      ...
      // Erased link does not obviously means that all the successor node output
      // links are erased. E.g. using a cache node.
