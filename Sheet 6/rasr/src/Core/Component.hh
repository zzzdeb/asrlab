 ...
 *   - immediatelly abort execution on errors (immediate-exit) -> andras used to
 * mix up critical-errors and errors
...
public:
  ...
  public:
    operator XmlWriter &() const { return *ostream_; }
    template <typename T> XmlWriter &operator<<(const T &v) {
      *ostream_ << v;
      return *ostream_;
    }

    Message &form(const char *msg, ...) __attribute__((format(printf, 2, 3)));
...
public:
   ...
   **/
  Message log(const char *msg, ...) const __attribute__((format(printf, 2, 3)));
  Message log() const;
