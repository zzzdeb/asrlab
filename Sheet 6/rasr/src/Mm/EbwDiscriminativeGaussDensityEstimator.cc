 ...
 */
void EbwDiscriminativeGaussDensityEstimator::setIterationConstant(
    IterationConstant ic) {
  required_cast(EbwDiscriminativeMeanEstimator *, meanEstimator_.get())
      ->setIterationConstant(ic);
}

/**
 * EbwDiscriminativeMeanEstimator
 */
EbwDiscriminativeMeanEstimator::EbwDiscriminativeMeanEstimator(
    ComponentIndex dimension)
    : Precursor(dimension), denAccumulator_(dimension), iterationConstant_(1) {}
...
void EbwDiscriminativeMeanEstimator::accumulate(
...
void EbwDiscriminativeMeanEstimator::read(Core::BinaryInputStream &i,
                                          u32 version) {
...
bool EbwDiscriminativeMeanEstimator::operator==(
...
void EbwDiscriminativeMeanEstimator::setIterationConstant(
    IterationConstant ic) {
  verify(!std::isnan(ic));
  verify(!std::isinf(ic));
  verify(ic >= 0);
 ...
 */
EbwDiscriminativeMeanEstimatorWithISmoothing::
    EbwDiscriminativeMeanEstimatorWithISmoothing(ComponentIndex dimension)
    : Precursor(dimension) {}
 ...
 */
EbwDiscriminativeCovarianceEstimator::EbwDiscriminativeCovarianceEstimator(
    ComponentIndex dimension)
    : Precursor(dimension), denAccumulator_(dimension) {}
...
void EbwDiscriminativeCovarianceEstimator::read(Core::BinaryInputStream &i,
                                                u32 version) {
  Precursor::read(i, version);
  denAccumulator_.read(i, version);
}

void EbwDiscriminativeCovarianceEstimator::write(
...
Covariance *EbwDiscriminativeCovarianceEstimator::estimate(
  ...
  for (; m != meanSet.end(); ++m) {
    ...
    Weight tmpWeight =
        meanEstimator->dtWeight() + meanEstimator->iterationConstant();
    weight += tmpWeight;
    ...
    for (ComponentIndex i = 0; i < dimension; ++i) {
      ...
      previousSum +=
          meanEstimator->previousMean(i) * meanEstimator->previousMean(i);
  ...
  std::transform(buffer.begin(), buffer.end(), buffer.begin(),
                 std::bind2nd(std::divides<Sum>(), weight));
 ...
 */
EbwDiscriminativeCovarianceEstimatorWithISmoothing::
    EbwDiscriminativeCovarianceEstimatorWithISmoothing(ComponentIndex dimension)
    : Precursor(dimension) {
...
std::vector<EbwDiscriminativeCovarianceEstimatorWithISmoothing::Precursor::
                Accumulator::SumType>
EbwDiscriminativeCovarianceEstimatorWithISmoothing::dtSum(
  ...
  for (u32 i = 0; i < result.size(); ++i) {
    result[i] =
        sum()[i] - denSum()[i] + ISmoothing::constant() * iSum(meanSet, i);
  }
  return result;
}
