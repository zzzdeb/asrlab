...
const Core::ParameterFloat HtkReader::paramTimeframeIndexScale(
    "timeframe-index-scale", "timeframe index = time * scale", 100, 0);

HtkReader::HtkReader(const Core::Configuration &config,
                     Bliss::LexiconRef lexiconRef, f64 amScale, f64 lmScale,
                     f64 penaltyScale, f64 pronunciationScale,
                     bool keepVariants)
    : Precursor(config), lexiconRef_(lexiconRef), amScale_(amScale),
      lmScale_(lmScale), penaltyScale_(penaltyScale),
      keepVariants_(keepVariants), wordPenalty_(0.0), silPenalty_(0.0),
      unkLemmaId_(Fsa::InvalidLabelId), silLemmaId_(Fsa::InvalidLabelId),
      lattice_(0), wordBoundaries_(0), states_(),
  ...
  if (pronunciationScale != 1) {
    lemmaPronunciationToHtkTransducerRef_ =
        Fsa::cache(Fsa::multiply(lemmaPronunciationToHtkTransducerRef_,
                                 Fsa::Weight((f32)pronunciationScale)));
  }
  const Bliss::Lemma *unknown = lexiconRef_->specialLemma("unknown");
  if (unknown)
    unkLemmaId_ = unknown->id();
  else
    warning() << "could not find label id for the unknown word; use the "
                 "invalid label id";
...
HtkReader::~HtkReader() { delete orthParser_; }
...
Fsa::StateId HtkReader::getStateId(u32 id) { return Fsa::StateId(id); }
...
HtkReader::Status HtkReader::addArc(Link &link) {
  require(size_t(link.sourceState) < states_.size() &&
  ...
  if (link.hasLemmas()) {
    for (LemmaIterator it = link.lemmas.first; it != link.lemmas.second; ++it) {
      if (!states_[size_t(link.sourceState)].fsaState->newArc(
              getStateId(link.targetState), getWeight(link, it->id()),
              it->id()))
        return htkAddLinkError;
    }
  } else {
    warning() << "No matching lemma for orthographic form \"" << link.label
              << "\" found.\n"
              << "Substitute by unknown word.";
    if (!states_[size_t(link.sourceState)].fsaState->newArc(
            getStateId(link.targetState), getWeight(link, unkLemmaId_),
            unkLemmaId_))
      return htkAddLinkError;

    //    warning() << "No matching lemma for orthographic form \"" <<
    //    link.label << "\" found.\n"
...
HtkReader::LemmaRange HtkReader::parseLabel(std::string &label,
...
ConstWordLatticeRef HtkReader::read(std::istream &is) {
  ...
  ConstWordLatticeRef l =
      Lattice::composeMatching(lemmaPronunciationToHtkTransducerRef_, f);
  WordLattice *wordLattice = new WordLattice;
  wordLattice->setWordBoundaries(l->wordBoundaries());
  wordLattice->setFsa(
      Fsa::staticCopy(Fsa::projectInput(l->part(WordLattice::totalFsa))),
