...
/** XmlMatrixElement implements XmlBuilderElement for an arbitrary
 * XmlSchemaParser which parses a matrix of a given type.
 *
...
class XmlMatrixElement
    : public XmlBuilderElement<Math::Matrix<T>, XmlElement, CreateByContext> {
public:
  typedef XmlBuilderElement<Math::Matrix<T>, XmlElement, CreateByContext>
      Predecessor;
  typedef XmlMatrixElement<T> Self;
  typedef Math::Matrix<T> *(XmlContext::*CreationHandler)(
...
template <class T>
XmlMatrixElement<T>::XmlMatrixElement(XmlContext *context,
                                      CreationHandler newMatrix,
                                      const char *name)
    : Predecessor(name, context, newMatrix) {}

template <class T> void XmlMatrixElement<T>::start(const XmlAttributes atts) {
  ...
  if (nColumns == 0)
    this->parser()->error(
        "Number of columns attribute (nColumns) is not given.");

  Predecessor::product_->resize(atoi(nRows), atoi(nColumns));
}

template <class T> void XmlMatrixElement<T>::end() {
...
template <class T> class XmlMatrixDocument : public XmlSchemaParser {
...
template <class T>
XmlMatrixDocument<T>::XmlMatrixDocument(const Configuration &c,
                                        Math::Matrix<T> &matrix)
    : Predecessor(c), matrixElement_(0), matrix_(matrix) {
...
template <class T> XmlMatrixDocument<T>::~XmlMatrixDocument() {
