 ...
 *   ?   bytes              path + filename: 4 byte string size + string without
 * '\0' 4   bytes              size of compressed file 4   bytes size of
 * uncompressed file (0 = no compression) 4   bytes              checksum
 * (unused)
 *   ...                    data
...
const ParameterBool
    FileArchive::paramOverwrite("allow-overwrite",
                                "allow overwriting of existing files", false);
...
struct FileArchive::FileInfo {
  ...
  bool isEmpty() const { return (name.empty() && position == 0); }

  struct Empty : public std::unary_function<FileInfo, bool> {
    bool operator()(const FileInfo &f) const { return f.isEmpty(); }
  };
};
...
FileArchive::FileInfo *FileArchive::file(const std::string &name) {
  // Check consistency, since '//', '/./' are normalized by directory archive
  // but are not by file archive.
...
const FileArchive::FileInfo *FileArchive::file(const std::string &name) const {
  ...
  if (normalizePath(name) != name)
    criticalError(
        "Filename \"%s\" contains special character(e.g. \"//\") sequences "
        "that will cause different behaviour of file and directory archives.",
        name.c_str());
...
/*****************************************************************************/
FileArchive::FileArchive(const Configuration &c, const std::string &p,
                         AccessMode access)
    : Archive(c, p, access), allowOverwrite_(paramOverwrite(c)), stream_(0),
      open_(false), changed_(false)
/*****************************************************************************/
{
...
/*****************************************************************************/
{
  ...
  emptyFiles_.clear();
  stream_ =
      new BinaryStream(path(), std::ios::in | std::ios::out | std::ios::trunc);
  stream_->write(&header[0], 8);
  *stream_ << u8(0);
  open_ = true;
  changed_ = true;
...
/*****************************************************************************/
{
  ...
  stream_->clear();
  std::streamoff begin = info.position - (sizeof(recoveryStartTag) +
                                          info.name.size() + sizeof(u32));
  u32 size = (info.sizes.compressed() ? info.sizes.compressed()
                                      : info.sizes.uncompressed());
  stream_->BinaryInputStream::seek(begin, std::ios::beg);
  u32 tag = 0;
  *stream_ >> tag;
  if (tag != recoveryStartTag) {
    error("remove failed: recovery start tag not found at position %ld",
          (long int)begin);
    return false;
  }
  setChanged();
  if (std::streampos(info.position + 3 * sizeof(u32) + size +
                     sizeof(recoveryEndTag)) == endOfArchive_) {
    // segment is the last segment in archive -> shrink file
    endOfArchive_ = begin;
  } else {
    stream_->BinaryOutputStream::seek(begin + sizeof(recoveryStartTag),
                                      std::ios::beg);
    ...
    *stream_ << u32(0) << u32(0); // compressed, checksum
    ...
    stream_->BinaryInputStream::seek(curpos + std::streampos(size),
                                     std::ios::beg);
    *stream_ >> tag;
  ...
  *stream_ >> ok;
  if (ok) {
    ...
    *stream_ >> count;
    ...
    warning(
        "No file info table available in archive \"%s\". We try a manual scan.",
        path().c_str());
    if (!scanArchive())
...
/*****************************************************************************/
{
  if ((changed_) && hasAccess(AccessModeWrite)) {
    ...
    u32 tmp = files_.size() -
              std::count_if(files_.begin(), files_.end(), FileInfo::Empty());
    *stream_ << tmp;
    for (std::vector<FileInfo>::const_iterator i = files_.begin();
         i != files_.end(); ++i) {
      if (i->isEmpty())
        continue;
      *stream_ << i->name;
    ...
    std::streampos emptyFileTableStart =
        stream_->BinaryOutputStream::position();
    tmp = emptyFiles_.size();
    *stream_ << tmp;
    for (std::vector<FileInfo>::const_iterator i = emptyFiles_.begin();
         i != emptyFiles_.end(); ++i) {
      *stream_ << u64(i->position);
      *stream_ << u32(i->sizes.uncompressed());
    }
...
/*****************************************************************************/
bool FileArchive::write(const std::string &name, const std::string &b,
                        const Sizes &sizes)
/*****************************************************************************/
{
  ...
  if (fi) {
    if (!allowOverwrite_) {
      error("Overwriting is not allowed. Change parameter '%s'.",
            paramOverwrite.name().c_str());
      return false;
    } else {
      remove(fi->name);
    }
  ...
  if (!append) {
    stream_->BinaryInputStream::seek(
        emptyFile.position - (sizeof(recoveryStartTag) + sizeof(u32)),
        std::ios::beg);
  } else {
    stream_->BinaryInputStream::seek(endOfArchive_, std::ios::beg);
  }
