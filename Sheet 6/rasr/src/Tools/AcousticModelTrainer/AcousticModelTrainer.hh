...
#if 1
namespace Speech {
class LatticeSetProcessor : public Core::ReferenceCounted {};
} // namespace Speech
#endif
 ...
 * Actions:
 *
 * - dry: drives an aligner (given in *.aligner.type). Useful for generating and
 * saving an alignment.
 * - create-model-acceptors: activates the model-acceptor-cache object for each
 * segment of the corpus
 * - show-mixture-set: dumps mixture set in XML format
 * - combine-mixture-sets: combines mixture sets given in
 * old-mixture-set-template and old-mixture-set-ids
 * - map-mixture-set: maps mixture set given mapping file
 * - accumulate-mixture-set-text-dependent: mixtures set estimator accumulates
 * features according to a given alignment
 * - accumulate-mixture-set-ext-independent: mixtures set estimator accumulates
 * features for set of acoustic conditions
 * - accumulate-tdc-sum: decision tree sum file estimator accumulates features
 * according to a given alignment
 * - accumulate-discriminatively: mixtures set estimator discriminatively
 * accumulates features according to a given (weighted) alignment
 * - score-features: acoustic scores of a given alignment accumulated for set of
 * acoustic conditions
 * - estimate-scatter-matrices-text-dependent: estimates between and within
 * class scatter matrices according to a given alignment
 * - estimate-scatter-matrices-text-independent: estimates between and within
 * class scatter matrices according to set of acoustic conditions
 * - estimate-lda-text-dependent: creates projector matrix by LDA according to a
 * given alignment
 * - estimate-lda-text-independent: creates projector matrix by LDA according to
 * set of acoustic conditions
 * - estimate-lda: creates projector matrix by LDA based reading scatter
 * matrices form files.
 *
 * - accumulate-weight-histogram: generates a histogram over all state weights
 * in the alignment The target histogram file is set using the parameter
 * weight-histogram-accumulator.histogram-file
 *
 * Corpus description: configured in the selection "corpus"
 * Aligner: configured in the selection "aligner" @see Speech::Application
 * Trainers: E.g.: accumulater, scorer etc are configured in the selection
 * "trainer"
 */
