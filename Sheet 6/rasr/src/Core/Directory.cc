...
/*****************************************************************************/
{
  ...
  if (i == std::string::npos) {
    ...
    if (j == std::string::npos) {
      j = 1; // only leading slashed -> remove everything except for a signle
             // leading slash
    } else {
...
/*****************************************************************************/
{
  ...
  for (StringTokenizer::Iterator comp = components.begin();
       comp != components.end(); ++comp) {
    std::string component(*comp);
    if (component.size() == 0 || component == ".")
      continue;
    else if ((component == "..") &&
             !(newComponents.size() == 0 || newComponents.back() == ".."))
  ...
  for (std::vector<std::string>::const_iterator c = newComponents.begin();
       c != newComponents.end(); ++c) {
...
/*****************************************************************************/
{
  for (DirectoryFileIterator file(path, &DirectoryFileIterator::fileFilter);
       file; ++file) {
    if (unlink(joinPaths(file.base(), file.path()).c_str()) != 0)
      return false;
  }
  for (DirectoryFileIterator directory(path,
                                       &DirectoryFileIterator::directoryFilter);
       directory; ++directory) {
...
// ===========================================================================
bool DirectoryFileIterator::AllFilter::operator()(const std::string &path,
...
bool DirectoryFileIterator::DirectoryFilter::operator()(
    const std::string &path, const struct stat64 &state, uid_t uid,
    gid_t gid) const {
  return S_ISDIR(state.st_mode);
}

const DirectoryFileIterator::DirectoryFilter
    DirectoryFileIterator::directoryFilter;

bool DirectoryFileIterator::FileFilter::operator()(const std::string &path,
                                                   const struct stat64 &state,
                                                   uid_t uid, gid_t gid) const {
  return S_ISREG(state.st_mode);
}
...
bool DirectoryFileIterator::FileNameFilter::operator()(
  ...
  return S_ISREG(state.st_mode) &&
         (exactMatch_ ? (baseName(path) == name_)
                      : (baseName(path).find(name_) != std::string::npos));
}
...
struct dirent *DirectoryFileIterator::nextEntry() {
  ...
  } while (result != 0 && (strcmp(result->d_name, ".") == 0 ||
...
DirectoryFileIterator::DirectoryFileIterator(const std::string &path,
                                             const Filter *filter)
