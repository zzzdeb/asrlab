...
identity(Fsa::ConstAlphabetRef ab, typename _Automaton::ConstSemiringRef sr) {
...
public:
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (s > 0) {
      ...
      sp->newArc(Precursor::fsa_->initialStateId() + 1, this->semiring()->one(),
                 Fsa::Epsilon);
    }
...
typename _Automaton::ConstRef kleeneClosure(typename _Automaton::ConstRef f) {
  return
      typename _Automaton::ConstRef(new KleeneClosureAutomaton<_Automaton>(f));
}
...
public:
  ComplementAutomaton(_ConstAutomatonRef f) : Precursor(f) {
    if (f->type() != Fsa::TypeAcceptor)
      std::cerr << "complement only defined for acceptors." << std::endl;
    _State *superFinalState =
        new _State(0, Fsa::StateTagFinal, Precursor::fsa_->semiring()->one());
...
protected:
   ...
   * all alphabets
   **/
  static Fsa::ConstAlphabetRef
  uniteAlphabets(const Core::Vector<Fsa::ConstAlphabetRef> &alphabets,
    ...
    for (u32 k = 0; k < alphabets.size(); ++k) {
      ...
      for (Fsa::Alphabet::const_iterator j = alphabet->begin();
           j != alphabet->end(); ++j) {
...
public:
  ...
  ConcatUnionAutomaton(const Core::Vector<_ConstAutomatonRef> &fsa)
    ...
    for (typename Core::Vector<_ConstAutomatonRef>::const_iterator i =
      ...
      if (semiring_ != (*i)->semiring()) {
        std::cerr << "mixed semirings during union. continuing, but behaviour "
  ...
  virtual Fsa::Type type() const { return type_; }
  virtual _ConstSemiringRef semiring() const { return semiring_; }
  virtual Fsa::ConstAlphabetRef getInputAlphabet() const { return input_; }
  virtual Fsa::ConstAlphabetRef getOutputAlphabet() const { return output_; }
  ...
  u32 subAutomaton(Fsa::StateId s) const { return (s - 1) % fsa_.size(); }
...
public:
  ConcatAutomaton(const Core::Vector<_ConstAutomatonRef> &fsa)
      : Precursor(fsa) {}
  virtual std::string name() const { return "concat"; }
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (s != 0) {
      ...
      if ((k < Precursor::fsa_.size() - 1) && (sp->isFinal())) {
        sp->newArc(
            superStateId(k + 1, Precursor::fsa_[k + 1]->initialStateId()),
            sp->weight_, Fsa::Epsilon);
        sp->unsetFinal();
      }
    } else {
      sp = new _State(s);
      sp->newArc(superStateId(0, Precursor::fsa_[0]->initialStateId()),
...
template <class _Automaton>
typename _Automaton::ConstRef
concat(const Core::Vector<typename _Automaton::ConstRef> &f) {
  if (f.size() > 1)
...
template <class _Automaton>
typename _Automaton::ConstRef concat(typename _Automaton::ConstRef f1,
...
public:
  UnionAutomaton(
      const Core::Vector<_ConstAutomatonRef> &fsa,
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (s != 0) {
      ...
      for (u32 k = 0; k < Precursor::fsa_.size(); ++k)
        sp->newArc(superStateId(k, Precursor::fsa_[k]->initialStateId()),
...
template <class _Automaton>
typename _Automaton::ConstRef
unite(const Core::Vector<typename _Automaton::ConstRef> &f,
      const Core::Vector<typename _Automaton::Weight> &initialWeights) {
  if (f.size() >= 1)
    return typename _Automaton::ConstRef(
        new UnionAutomaton<_Automaton>(f, initialWeights));
  // if (f.size() == 1) return f[0];
  return typename _Automaton::ConstRef();
}

template <class _Automaton>
typename _Automaton::ConstRef unite(typename _Automaton::ConstRef f1,
                                    typename _Automaton::ConstRef f2) {
  Core::Vector<typename _Automaton::ConstRef> f;
  f.push_back(f1);
  f.push_back(f2);
...
template <class _Automaton> class UniteMapping : public Fsa::Mapping {
public:
  typedef typename _Automaton::ConstRef _ConstAutomatonRef;
...
public:
  ...
  virtual Fsa::StateId map(Fsa::StateId target) const {
    return uFsa_->subAutomaton(target) == subAutomaton_
               ? uFsa_->subStateId(target)
               : Fsa::InvalidStateId;
  }
};

template <class _Automaton>
Fsa::ConstMappingRef mapToSubAutomaton(typename _Automaton::ConstRef f,
                                       u32 subAutomaton) {
...
public:
  FusionAutomaton(const Core::Vector<ConstAutomatonRef> &fsa)
      : Precursor(fsa) {}
  virtual std::string name() const { return "fuse"; }
  ...
  virtual ConstStateRef getState(Fsa::StateId s) const {
    ...
    if (s != 0) {
      ...
      hope(!sp->isFinal()); // Patrick Lehnen: "Why do we hope this? In
                            // UnionAutomaton this hope is not set."
    } else {
      sp = new State(s);
      for (u32 k = 0; k < Precursor::fsa_.size(); ++k) {
        const Fsa::AlphabetMapping &input(Precursor::inputMappings_[k]);
        ...
        for (typename State::const_iterator ka = ks->begin(); ka != ks->end();
             ++ka) {
        ...
        if (ks->isFinal()) {
          if (sp->isFinal())
            sp->weight_ =
                Precursor::semiring_->collect(sp->weight(), ks->weight());
          else
...
template <class _Automaton>
typename _Automaton::ConstRef
fuse(const Core::Vector<typename _Automaton::ConstRef> &f) {
  if (f.size() > 1)
...
template <class _Automaton>
typename _Automaton::ConstRef fuse(typename _Automaton::ConstRef f1,
                                   typename _Automaton::ConstRef f2) {
  Core::Vector<typename _Automaton::ConstRef> f;
  f.push_back(f1);
  f.push_back(f2);
...
public:
  TransposeDfsState(_ConstAutomatonRef f) : Precursor(f) {
  ...
  StaticAutomaton<_Automaton> *result() { return fsa_; }
  const std::vector<_ConstStateRef> &finals() const { return finals_; }
...
template <class _Automaton>
typename _Automaton::ConstRef transpose(typename _Automaton::ConstRef f,
                                        bool progress) {
  typedef typename _Automaton::State _State;
  typedef typename _Automaton::ConstStateRef _ConstStateRef;
  typedef typename _Automaton::ConstRef _ConstAutomatonRef;
