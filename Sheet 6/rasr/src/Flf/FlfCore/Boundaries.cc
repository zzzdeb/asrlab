...
const Boundary::Transit Boundary::InvalidTransit = Boundary::Transit(
    Boundary::Transit::InvalidId, Boundary::Transit::InvalidId);

#ifdef MEM_DBG
Boundaries::nBoundaries = 0;

Boundaries::Boundaries() { ++nBoundaries; }
...
public:
  ...
  virtual bool valid(Fsa::StateId sid) const { return false; }
  virtual const Boundary &get(Fsa::StateId sid) const {
    return InvalidBoundary;
  }
};

ConstBoundariesRef InvalidBoundaries =
    ConstBoundariesRef(new InvalidBoundaries_);
