...
void DiscriminativeMixtureEstimator::removeDensitiesWithLowWeight(
  ...
  for (DensityIndex dns = 0; dns < densityEstimators_.size();) {
    ...
    bool hasEnoughRelativeWeight =
        previousMixtureWeight(dns) >= minRelativeWeight;
...
void DiscriminativeMixtureEstimator::accumulate(
  ...
  std::transform(weights_.begin(), weights_.end(), _toAdd->weights_.begin(),
                 weights_.begin(), std::plus<Weight>());
}
...
Weight DiscriminativeMixtureEstimator::logPreviousMixtureWeight(
    DensityIndex dns) const {
  ...
  if (_previousMixtureWeight != previousMixtureWeight(dns)) {
    Core::Application::us()->log("previous-mixture-weight[")
        << dns << "]=" << previousMixtureWeight(dns)
        << " floored for logarithm";
  }
 ...
 */
void DiscriminativeMixtureEstimator::setPreviousMixture(
    const Mixture *mixture) {
  require(mixture && mixture->nDensities() == nDensities());
  previousMixtureWeights_.resize(nDensities());
  for (DensityIndex dnsInMix = 0; dnsInMix < previousMixtureWeights_.size();
       ++dnsInMix) {
...
DensityIndex
DiscriminativeMixtureEstimator::accumulate(Core::BinaryInputStreams &is,
