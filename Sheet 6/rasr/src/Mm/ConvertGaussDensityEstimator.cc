...
void ConvertCovarianceEstimator::setCovariance(
  ...
  Weight weight = 0;
  CovarianceToMeanSetMap::MeanSet::const_iterator meanEstimator =
      meanSet.begin();
  for (; meanEstimator != meanSet.end(); ++meanEstimator) {
    ...
    std::transform(sum.begin(), sum.end(), mean.begin(), sum.begin(),
                   plusSquare<Sum>());
  ...
  std::copy(covariance->diagonal().begin(), covariance->diagonal().end(),
            covariance64.begin());
  std::transform(sum.begin(), sum.end(), covariance64.begin(), sum.begin(),
                 plusWeighted<Sum>(weight));
