 ...
 *  If new size is larger than the current size, the vector is extended by
 * initial-value.
 *
...
template <class T> class VectorResizeNode : public SleeveNode {
public:
...
public:
  ...
  virtual bool configure() {
    ...
    f64 sampleRate =
        sampleRateAttribute.empty() ? 1 : atof(sampleRateAttribute.c_str());
    verify(sampleRate > 0);
    newSize_ = (discreteNewSize_ != 0)
                   ? discreteNewSize_
                   : (u32)rint(continuousNewSize_ * sampleRate);
  ...
  virtual bool work(Flow::PortId p) {
    ...
    in.makePrivate();
    s32 difference =
        relativeChange_ ? newSize_ : (s32)newSize_ - (s32)in->size();
