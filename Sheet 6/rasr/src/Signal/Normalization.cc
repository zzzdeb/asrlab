...
Normalization::Normalization()
    : length_(0), right_(0), sumWeight_(0), changed_(true) {}
...
bool Normalization::flush(Frame &out) { return update(Frame(), out); }
...
void LevelNormalization::apply(Frame &out) { (*out)[index_] -= max_; }
...
void MeanNormalization::updateStatistics(const Frame &add,
                                         const Frame &remove) {
  if (add)
    std::transform(sum_.begin(), sum_.end(), add->begin(), sum_.begin(),
                   std::plus<Sum>());
  if (remove)
    std::transform(sum_.begin(), sum_.end(), remove->begin(), sum_.begin(),
                   std::minus<Sum>());
...
void MeanNormalization::apply(Frame &out) {
  std::transform(out->begin(), out->end(), mean_.begin(), out->begin(),
                 std::minus<Value>());
}

//===================================================================================================
MeanAndVarianceNormalization::MeanAndVarianceNormalization(
...
void MeanAndVarianceNormalization::updateStatistics(const Frame &add,
                                                    const Frame &remove) {
  if (add) {
...
void MeanAndVarianceNormalization::finalize() {
  if (sumWeight_ > 0) {
    for (size_t d = 0; d < standardDeviation_.size(); d++) {
      standardDeviation_[d] = (Value)sqrt(
          (sumSquare_[d] - sum_[d] * sum_[d] / sumWeight_) / sumWeight_);
...
//===================================================================================================
DivideByMean::DivideByMean(const Core::Configuration &c) : Core::Component(c) {}
...
//===================================================================================================
MeanNormNormalization::MeanNormNormalization(const Core::Configuration &c,
                                             f32 norm)
    : Core::Component(c), sumOfNorms_(0), averageNorm_(0), norm_(norm),
      normFunction_(c) {}
...
void MeanNormNormalization::updateStatistics(const Frame &add,
                                             const Frame &remove) {
  if (add)
...
//===================================================================================================
const Choice NormalizationNode::typeChoice("level", typeLevel, "mean", typeMean,
                                           "mean-and-variance",
                                           typeMeanAndVariance,
                                           "divide-by-mean", typeDivideByMean,
                                           "mean-norm", typeMeanNorm,
                                           Choice::endMark());
const ParameterChoice NormalizationNode::paramType("type", &typeChoice,
                                                   "type of normalization",
                                                   typeMean);

const ParameterInt
    NormalizationNode::paramLength("length",
                                   "length of the sliding window in frames");
const ParameterInt NormalizationNode::paramRight("right", "output point");

const ParameterInt
    NormalizationNode::paramLevelIndex("level", "index of level normalization.",
                                       0);
const ParameterFloat
    NormalizationNode::paramNorm("norm", "norm of mean norm normalization.", 2);

NormalizationNode::NormalizationNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), algorithm_(0), type_(typeMean),
      length_(0), right_(0), levelIndex_(0), norm_(0), needInit_(true) {
...
NormalizationNode::~NormalizationNode() { delete algorithm_; }

bool NormalizationNode::setParameter(const std::string &name,
...
bool NormalizationNode::init(size_t dimension) {
  ...
  if (!algorithm_->init(length_, right_, dimension)) {
    error("Cannot initialize with parameters lenght (%zd), right (%zd), and "
          "dimension (%zd).",
...
bool NormalizationNode::update(const Normalization::Frame &in,
                               Normalization::Frame &out) {
  require(in);
