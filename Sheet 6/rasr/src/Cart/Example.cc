...
size_t FloatBox::HashFcn::operator()(const FloatBox &val) const {
  size_t hash = 0;
  for (FloatBox::const_vector_iterator it = val.begin(); it != val.end(); ++it)
...
bool FloatBox::operator==(const FloatBox &values) const {
  ...
  for (it1 = begin_, it2 = values.begin(); ((it1 != end_) && (*it1 == *it2));
       ++it1, ++it2)
    ;
  return (it1 == end_);
}
...
void FloatBox::writeValues(std::ostream &os) const {
  ...
  for (const_row_vector_iterator it = begin_row(); it != end_row(); ++it) {
    for (const_row_iterator itt = it.begin(); itt != it.end(); ++itt)
      os << *itt << " ";
    os << "\n";
  }
...
void FloatBox::writeXml(Core::XmlWriter &xml) const {
  xml << Core::XmlOpen("matrix-f64") + Core::XmlAttribute("nRows", rows_) +
...
void Example::write(std::ostream &os) const {
  os << "example" << std::endl << "observation(s) : " << nObs << std::endl;
  properties->write(os);
  os << "values:" << std::endl;
  values->write(os);
}
...
const Core::ParameterString
    ExampleList::paramExampleFilename("example-file", "name of example file");
const Core::ParameterStringVector
    ExampleList::paramExampleFilenamesToMerge("merge-example-files",
...
void ExampleList::write(std::ostream &out) const {
  ...
  out << std::right;
  u32 index = 1;
  for (const_iterator it = exampleRefs_.begin(); it != exampleRefs_.end(); ++it)
    if (*it) {
