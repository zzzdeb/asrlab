 ...
 *  Features:
 *   -Multithread safe reference counting. I.e. objects can be accessed by
 * DataPtr. -Polymorfic I/O operation.
 */
...
private:
   ...
   *  instead the precursor class. In this way, all the sentinels are Data
   * objects and can be handled equally. @see dump method as an example.
   */
 ...
 *    -makePrivate() which ensures that the object is referenced only by this
 * DataPtr.
 *
 *  Note: for comments more important, please @see Core::Ref and Core::TsRef.
 *
 *  Remark: Objects referenced by DataPtr can be referenced by Core::TsRef as
 * well. However, in certain applications multithread safety is not important,
 * thus usage of the mutex_ is wasted time. In these application, thread safe
 * object needs to be copied into a object derived from ReferenceCounted. For an
 * examles @see Speech::Feature.
 */
...
public:
   ...
   *  in comparisons the compiler uses Precursor::operator bool() instead of
   * Precursor::operator==.
   */
  bool operator==(const DataPtr &r) const { return Precursor::operator==(r); }
...
template <class T> inline DataPtr<T> dataPtr(T *o) { return DataPtr<T>(o); }

} // namespace Flow

namespace Core {
template <typename T> class NameHelper<Flow::DataPtr<T> > : public std::string {
public:
