...
public:
  ...
  Target(const Core::Configuration &, bool isXmlDocument,
         const std::string &defaultFilename,
         std::streambuf *defaultStreamBuf = 0);
  virtual ~Target();
...
const Core::ParameterString
    Channel::Target::paramFilename("file", "name of output file");
...
const Core::ParameterBool Channel::Target::paramShouldPutDeclaration(
    "put-declaration",
    "should put declaration to the beginning of the document.", true);
const Core::ParameterString Channel::Target::paramEncoding(
    "encoding", "output character set encoding", Core::defaultEncoding,
...
const Core::ParameterInt
    Core::Channel::Target::paramMargin("margin", "maximum line length", 0, 0);
const Core::ParameterInt Core::Channel::Target::paramIndentation(
    "indentation", "number of blanks for indenting output", 2, 0);
const Core::ParameterBool
    Channel::Target::paramCompressed("compressed",
                                     "write to gzip compressed file", false);
const Core::ParameterBool Channel::Target::paramAddSprintTags(
    "add-sprint-tags", "write <sprint> tags into channel", true);

Channel::Target::Target(const Core::Configuration &c, bool isXmlDocument,
                        const std::string &defaultFilename,
                        std::streambuf *defaultStreamBuf)
    : Core::Configurable(c), isTty_(false), xml_(*this),
      isXmlDocument_(isXmlDocument), defaultStreamBuf_(defaultStreamBuf) {
...
Channel::Target::Target(const Core::Configuration &c, bool isXmlDocument,
    ...
    : Core::Configurable(c), isTty_(false), xml_(*this),
      isXmlDocument_(isXmlDocument), defaultStreamBuf_(defaultStream->rdbuf()) {
...
void Channel::Target::open(const std::string &filename) {
  ...
  if (!good()) {
    ...
    if (defaultStreamBuf_) {
      ...
      std::cerr << "Target " << fullName()
                << " could not be redirected: no default stream buffer given."
                << std::endl;
      defect();
    }
  }
...
int Channel::Dispatcher::overflow(int c) {
  int result = c;
  if (c != EOF) {
    for (std::vector<Target *>::iterator it = targets_.begin();
         it != targets_.end(); it++) {
      Target *t(*it);
      t->lock();
...
std::streamsize Channel::Dispatcher::xsputn(const char *s,
                                            std::streamsize num) {
...
Channel::Manager::Manager(const Core::Configuration &c, bool outputXmlHeader)
  ...
  Target *stdoutTarget = new Target(select("stdout"), outputXmlHeader,
                                    new std::ostream(std::cout.rdbuf()));
  Target *stderrTarget = new Target(select("stderr"), outputXmlHeader,
...
Channel::Manager *Channel::Manager::us() { return singleton_; }
...
void Channel::Manager::flushTty() {
  lock();
  for (TargetList::const_iterator t = ttyTargets_.begin();
       t != ttyTargets_.end(); ++t)
    (*t)->flush();
  release();
}
...
Channel::Target *Channel::Manager::createTarget(TargetType type,
...
Channel::Target *Channel::Manager::get(TargetType type,
...
// ===========================================================================
const Core::ParameterString
    Core::Channel::paramTargets("channel",
                                "comma separated list of channel targets "
                                "(\"stdout\", \"stderr\", \"nil\" or filename)",
                                "stdout");

Core::Channel::Channel(const Configuration &c, const std::string &name,
                       Default defaultTarget, TargetType type)
    : std::ostream(0), isOpen_(false) {
  ...
  if (!Manager::us()) { // fallback when Channel::Manager is not present
    ...
    std::cerr << "channel warning: Creating Channel " << config.getSelection()
  ...
  switch (defaultTarget) {
  case disabled:
    defaultTargetSpec = "nil";
    break;
  case standard:
    defaultTargetSpec = "stdout";
    break;
  case error:
    defaultTargetSpec = "stderr";
    break;
  default:
    defect();
  }
...
Channel::~Channel() { delete rdbuf(0); }

Core::XmlChannel::XmlChannel(const Configuration &c, const std::string &name,
