...
ProbabilityStatistics::ProbabilityStatistics(const std::string &name,
                                             u32 nBuckets)
    : name_(name), nBuckets_(nBuckets) {
...
void ProbabilityStatistics::writeXml(Core::XmlWriter &xml) const {
  ...
  xml << Core::XmlOpen("statistic") + Core::XmlAttribute("name", name_) +
             Core::XmlAttribute("type", "probability-histogram")
      << Core::XmlFull("count", totalCounts) << Core::XmlFull("sum", totalSum);
  if (totalCounts > 0 && totalSum > 0) {
    xml << Core::XmlOpen("table");
    std::ostream &out = xml;
    out << "prob  rel.counts  rel.sum" << std::endl;
    for (int i = 0; i < nBuckets_; ++i) {
      out << Core::form("%1.3f %8.9f %8.9f", (i + 0.5) / nBuckets_,
                        counts_[i] / totalCounts, sum_[i] / totalSum)
          << std::endl;
    }
