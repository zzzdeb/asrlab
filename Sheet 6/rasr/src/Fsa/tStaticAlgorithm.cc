...
void trimInPlace(Core::Ref<StaticAutomaton<_Automaton> > sa) {
  ...
  vv.dfs();
  // not necesserarily true: we want to trim because of the violation of that
  // property ... verify(vv.maxStateId() == sa->maxStateId());
  ...
  for (Fsa::StateId si = 0; si < sa->size(); ++si) {
    ...
    if (st) {
      if (vv.flags()[si] & CoaccessibleDfsState<_Automaton>::FlagCoaccessible) {
        ...
        for (typename _Automaton::State::iterator in = st->begin();
             in != st->end(); ++in) {
          if (vv.flags()[in->target()] &
              CoaccessibleDfsState<_Automaton>::FlagCoaccessible)
            *out++ = *in;
        }
        st->truncate(out);
      } else {
