...
public:
  bool hasLeaped() const { return hasLeaped_; }
   ...
   */
  Bliss::Phoneme::Id successorPhoneme() const { return successorPhoneme_; }
  ...
  /**
   * Pronuncation suffix for pronuncation @c p with history (= predecessor
   * phoneme) @h. the predecessor phoneme is used for across word context
   */
  PronunciationSuffix(const Bliss::Pronunciation *p,
                      Bliss::Phoneme::Id h = Bliss::Phoneme::term)
      : pronunciation_(p), predecessorPhoneme_(h),
        successorPhoneme_(Bliss::Phoneme::term), phoneme(-1), phoneState(0),
        subState(0), hasLeaped_(false), isEmpty_(false), isHashValid_(false) {}
  ...
  s32 nPhonemesRemaining() const { return pronunciation_->length() - phoneme; }
};
...
struct StateTree::PronunciationSuffix::Ordering {
  ...
  bool operator()(const PronunciationSuffix &l,
                  const PronunciationSuffix &r) const {
...
inline s8 StateTree::nPhoneStates(const PronunciationSuffix &s) const {
  ...
  const Am::ClassicHmmTopology *hmmTopology =
      acousticModel_->hmmTopology(s.currentPhoneme());
 ...
 */
inline const Am::Allophone *
StateTree::getAllophone(const PronunciationSuffix &s) const {
  ...
  ensure_(
      phonemeInventory()->phoneme(allophone.phoneme(0))->isContextDependent() ||
      (allophone.history().size() == 0 && allophone.future().size() == 0));

  // now the get pointer to the unique allophone from the allophone alphabet
  const Am::Allophone *result =
      acousticModel_->allophoneAlphabet()->allophone(allophone);
  verify(result);
  return result;
}
...
inline u16 StateTree::transitionModelIndex(const PronunciationSuffix &s) const {
  if (s.phoneme < 0) {
    ...
    Am::AllophoneState alloState =
        acousticModel_->allophoneStateAlphabet()->allophoneState(allo,
                                                                 s.phoneState);
    require_(acousticModel_->stateTransitionIndex(alloState, s.subState) <
...
void StateTree::advancePronunciationSuffix(
  ...
  if (advanceSubState(s)) {
    ...
    if (acousticModel_->isAcrossWordModelEnabled() && mustAdvancePhoneme &&
        s.phoneme == s.pronunciation_->length() - 1) {
...
int StateTree::compareSequences(const PronunciationSuffix &l,
                                const PronunciationSuffix &r) const {
  ...
  for (s32 p = 0; p < l.nPhonemesRemaining(); ++p) {
    if ((*l.pronunciation_)[l.phoneme + p] !=
        (*r.pronunciation_)[r.phoneme + p])
      return (*l.pronunciation_)[l.phoneme + p] -
             (*r.pronunciation_)[r.phoneme + p];
  }
  ...
  if (la != ra) {
    for (s8 phoneState = l.phoneState; phoneState < nPhoneStates(l);
         ++phoneState) {
      Am::AllophoneState las =
          acousticModel_->allophoneStateAlphabet()->allophoneState(la,
                                                                   phoneState);
      Am::AllophoneState ras =
          acousticModel_->allophoneStateAlphabet()->allophoneState(ra,
                                                                   phoneState);
...
u32 StateTree::hashSequence(const PronunciationSuffix &s) const {
  ...
  for (s16 state = s.phoneState; state < nPhoneStates(s); ++state) {
    Am::AllophoneState as =
        acousticModel_->allophoneStateAlphabet()->allophoneState(a, state);
...
private:
  void sort() {
    std::sort(items_.begin(), items_.end(),
              PronunciationSuffix::Ordering(model_));
...
public:
  ...
  u32 size() const { return items_.size(); }
  ...
  SuffixSet(const StateTree *m)
      : model_(m), hasUniqueHead_(true), consolidation_(hashValid), hash_(0) {}
...
public:
  ...
  void addSorted(const PronunciationSuffix &s) {
    require_(items_.empty() ||
             !PronunciationSuffix::Ordering(model_)(s, items_.back()));
    add(s);
    updateHash(s);
  }
...
void StateTree::SuffixSet::advance(ItemList &endingItems) {
  ...
  for (ItemList::iterator i = items_.begin(); i != items_.end(); ++i)
    model_->advancePronunciationSuffix(*i, std::back_inserter(newItems),
...
inline StateTree::StateId
StateTree::CoarticulationStructure::root(Bliss::Phoneme::Id final,
                                         Bliss::Phoneme::Id initial) const {
  require_(std::find(finalPhonemes.begin(), finalPhonemes.end(), final) !=
           finalPhonemes.end());
  require_(std::find(initialPhonemes.begin(), initialPhonemes.end(), initial) !=
           initialPhonemes.end());
  int l = std::find(finalPhonemes.begin(), finalPhonemes.end(), final) -
          finalPhonemes.begin();
  int r = std::find(initialPhonemes.begin(), initialPhonemes.end(), initial) -
          initialPhonemes.begin();
  return roots[l][r];
}
...
std::pair<Bliss::Phoneme::Id, Bliss::Phoneme::Id>
StateTree::describeRootState(StateId s) const {
  ...
  if (s == root() || !coarticulationStructure_) {
    ...
    if (s < coarticulationStructure_->boundaryPhonemes.size()) {
      ...
      std::string text = Core::form(
          "Unknown transit index %d. Context is set to non-coarticulated. ", s);
      text += "It is likely that it occurs in combination with \"No active "
              "word end hypothesis at sentence end.\"";
      warning(text.c_str());
      final = initial = Bliss::Phoneme::term;
    }
  }
...
struct StateTree::BuildRequest {
  ...
  struct Hash {
    u32 operator()(const BuildRequest *r) const { return r->hash(); }
  };
...
struct StateTree::BatchRequest : public BuildRequest {
  ...
  BatchRequest(StateTree *m, StateId s)
      : BuildRequest(batchRequest), suffixes(m), parentsSuffixes(m) {
  ...
  virtual u32 hash() const { return suffixes.hash() + parentsSuffixes.hash(); }
...
void StateTree::BatchRequest::satisfy(StateTree *t) {
  ...
  for (std::vector<StateId>::const_iterator s = states.begin();
       s != states.end(); ++s) {
...
struct StateTree::LeapRequest : public BatchRequest {
  ...
  virtual u32 hash() const { return ~BatchRequest::hash(); }
};
...
private:
  ...
  typedef Core::hash_set<BatchRequest *, BuildRequest::Hash,
...
// ===========================================================================
StateTree::State::State() { successors = invalidBatch; }

const StateTree::Depth StateTree::invalidDepth =
    Core::Type<StateTree::Depth>::max;
const StateTree::BatchId StateTree::invalidBatch =
    Core::Type<StateTree::BatchId>::max;
const StateTree::StateDesc::AcousticModelIndex StateTree::invalidAcousticModel =
    Core::Type<StateTree::StateDesc::AcousticModelIndex>::max;

const Core::ParameterBool
    StateTree::paramEarlyRecombination("early-recombination",
                                       "enable recobination of word hypothesis "
                                       "before actual word end (EXPERIMENTAL)",
                                       false);
const Core::ParameterInt StateTree::paramEarlyRecombinationLimit(
    "early-recombination-limit",
    "minimum number of states before early recobination is allows (negative: "
    "maximum number of states remaining)",
    0);
const Core::ParameterString StateTree::paramFile("file", "cached state tree",
                                                 "");
const Core::ParameterBool StateTree::paramSkipTransitions(
    "skip-transitions", "build second order structure for skips transitions",
    true);
const Core::ParameterBool StateTree::paramCiCrossWordTransitions(
    "ci-cross-word-transitions",
    "allow context independent cross word transitions a{b+#}@f c{#+d}@i", true);

StateTree::StateTree(const Core::Configuration &c, Bliss::LexiconRef lexicon,
                     Core::Ref<const Am::AcousticModel> acousticModel)
    : Core::Component(c), acousticModel_(acousticModel),
      haveSuccessorBatches_(false), coarticulationStructure_(0), requests_(0) {
  ...
  if (acousticModel_->nStateTransitions() >=
      Core::Type<StateDesc::AcousticModelIndex>::max) {
    criticalError("Too many transition models. Current state tree "
                  "implementation handles max %d transition models",
                  Core::Type<StateDesc::TransitionModelIndex>::max);
  }
  ...
  if (acousticModel_->nEmissions() >=
      Core::Type<StateDesc::AcousticModelIndex>::max) {
    criticalError("Too many mixtures. Current state tree implementation "
                  "handles max %d mixtures",
                  Core::Type<StateDesc::AcousticModelIndex>::max);
  }
...
/**
 * get the transit entry for the final phoneme of s with successor phoneme
 * s.successorPhoneme_. the transit entry is the root state for the
 * coarticulated first state of s.successorPhoneme with s.finalPhonhme() as
 * predecessor.
 */
StateTree::StateId
StateTree::pronunciationSuffixTransitEntry(const PronunciationSuffix &s) const {
  require(s.isEmpty());
  if (s.successorPhoneme() == Bliss::Phoneme::term) {
    if (allowCiCrossWordTransitions_ ||
        !phonemeInventory()->isValidPhonemeId(s.finalPhoneme()) ||
    ...
    return coarticulationStructure_->root(s.finalPhoneme(),
                                          s.successorPhoneme());
  }
  defect();
}
  ...
  for (StateId s = 0; s < nStates(); ++s) {
    ...
    for (tie(ss, ss_end) = successors(s); ss < ss_end; ++ss)
      for (tie(sss, sss_end) = successors(ss); sss < sss_end; ++sss) {
        ...
        result &= r;
        if (!r)
          warning("2nd order mesh structure s=%d, ss=%d, sss=%d, ss2=%d", s, ss,
                  sss, ss2);
      }
    bool r = (ss2 == ss2_end);
    if (!r)
      warning("2nd order mesh structure s=%d, ss2=%d, ss2_end=%d", s, ss2,
              ss2_end);
    result &= r;
  }
...
void StateTree::buildCoarticulatedRootStates(Bliss::LexiconRef lexicon) {
  ...
  if (!allowCiCrossWordTransitions_) {
    ...
    SuffixSet &suffixes(request->suffixes);
    for (tie(pron, pron_end) = lexicon->pronunciations(); pron != pron_end;
         ++pron) {
      const Bliss::Pronunciation &p(**pron);
      if (p.length() < 1 ||
          !phonemeInventory()->phoneme(p[0])->isContextDependent()) {
...
void StateTree::createExits(const SuffixSet::ItemList &endingItems,
                            ExitList &exits) {
  Exit exit;
  for (SuffixSet::ItemList::const_iterator s = endingItems.begin();
       s != endingItems.end(); ++s) {
    require(s->isEmpty());
    exit.transitEntry = pronunciationSuffixTransitEntry(*s);
    if (s->pronunciation()) {
      Bliss::Pronunciation::LemmaIterator lpi, lpi_end;
      for (tie(lpi, lpi_end) = s->pronunciation()->lemmas(); lpi != lpi_end;
           ++lpi) {
...
StateTree::BatchId StateTree::createBatch(BatchRequest *request) {
  ...
  for (SuffixSet::SuffixSetList::iterator c = conts.begin(); c != conts.end();
       ++c) {
    m.desc = c->head(); // before c->advance(), all pronunciation suffixes have
                        // the same head
    c->advance(ends);
...
StateTree::StateId StateTree::createLeapEntry(LeapRequest *request) {
  ...
  for (u32 i = 0; i < request->states.size(); ++i) {
    ...
    for (std::vector<const Bliss::Pronunciation *>::const_iterator p =
             request->leaps[i].begin();
         p != request->leaps[i].end(); ++p) {
      verify(*p);
      Bliss::Pronunciation::LemmaIterator lpi, lpi_end;
...
void StateTree::logStatistics() const {
  ...
  for (LevelStatisticsMap::const_iterator l = levels.begin(); l != levels.end();
       ++l) {
    Depth depth = l->first;
    const LevelStatistics &thisLevel(l->second);
    msg.form("\nlevel %3d: %6d states, branching factor %3.2f, %4d exits",
             depth, thisLevel.nStates,
             float(thisLevel.nChildren) / float(thisLevel.nStates),
             thisLevel.nExits);
  }
...
void StateTree::draw(std::ostream &os,
  ...
  for (StateId si = 0; si < nStates(); ++si) {
    ...
    for (StateId ssi = batches_[s.successors];
         ssi != batches_[s.successors + 1]; ++ssi)
