...
public:
  Archive(const Core::Configuration &, const std::string &pathname);
  virtual ~Archive();
  void close();
...
public:
  LatticeArchive(const Core::Configuration &config, const std::string &pathname)
      : Archive(config, pathname) {}

public:
  static LatticeArchiveReader *getReader(const Core::Configuration &);
  static LatticeArchiveWriter *getWriter(const Core::Configuration &);
};
...
protected:
  const Core::Configuration &latticeConfig() const { return latticeConfig_; }
...
public:
  LatticeArchiveWriter(const Core::Configuration &,
                       const std::string &pathname);
...
public:
  LatticeProcessorArchiveWriter(const Core::Configuration &config,
                                const std::string &pathname);
...
public:
  ConfusionNetworkArchive(const Core::Configuration &config,
                          const std::string &pathname, CnFormat format);
  virtual std::string defaultSuffix() const;

public:
  static ConfusionNetworkArchiveReader *getReader(const Core::Configuration &,
                                                  CnFormat = CnFormatXml);
  static ConfusionNetworkArchiveWriter *getWriter(const Core::Configuration &,
                                                  CnFormat = CnFormatXml);
};
...
public:
  ConfusionNetworkArchiveReader(const Core::Configuration &,
                                const std::string &pathname, CnFormat format);
...
public:
  ConfusionNetworkArchiveWriter(const Core::Configuration &,
                                const std::string &pathname, CnFormat format);
  virtual ~ConfusionNetworkArchiveWriter();
  virtual void store(const std::string &id, ConstConfusionNetworkRef cn,
...
public:
  PosteriorCnArchive(const Core::Configuration &config,
                     const std::string &pathname, PosteriorCnFormat format);
  virtual std::string defaultSuffix() const;

public:
  static PosteriorCnArchiveReader *
  getReader(const Core::Configuration &,
            PosteriorCnFormat = PosteriorCnFormatXml);
  static PosteriorCnArchiveWriter *
  getWriter(const Core::Configuration &,
            PosteriorCnFormat = PosteriorCnFormatXml);
};
...
public:
  PosteriorCnArchiveReader(const Core::Configuration &,
                           const std::string &pathname,
                           PosteriorCnFormat format);
...
public:
  PosteriorCnArchiveWriter(const Core::Configuration &,
                           const std::string &pathname,
                           PosteriorCnFormat format);
  virtual ~PosteriorCnArchiveWriter();
  virtual void store(const std::string &id, ConstPosteriorCnRef cn,
                     ConstSegmentRef segment);
};
