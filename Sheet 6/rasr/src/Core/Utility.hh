 ...
 * @param string resulting string that has been read; will not include a
 *trailing  delimiter
...
inline int wsgetline(std::istream &is, std::string &str,
                     std::string delim = "\n") {
  is >> std::ws;
...
struct conversion : public std::unary_function<S, T> {
  T operator()(S s) const { return T(s); }
};
...
template <class A, class B> class tied {
public:
  inline tied(A &a, B &b) : a_(a), b_(b) {}
  template <class U, class V> inline tied &operator=(const std::pair<U, V> &p) {
    a_ = p.first;
    b_ = p.second;
    return *this;
  }
...
template <class A, class B> inline tied<A, B> tie(A &a, B &b) {
  return tied<A, B>(a, b);
}
...
template <class T> T maxAbsoluteElement(const std::vector<T> &v) {
  T m;
...
/**
 * @return is true if [begin..end) interval does not contain any "inf", "nan"
 * etc. value.
 */
 ...
 */
template <class F> bool isMalformed(F f) {
  return std::isinf(f) || std::isnan(f);
}

/**
 * Checks if @param x is infinite and clips it to the largest representable
 * value.
 * @return is clipped value of @param x.
 */
template <class T> T clip(T x) {
  require(!std::isnan(x));
  if (std::isinf(x))
...
/**
 * @return is true if [begin..end) interval contains any malformed value (@see
 * isMalformed(F f)).
 */
template <class InputIterator>
...
template <class F, class G, class H>
inline composedBinaryFunction<F, G, H>
composeBinaryFunction(const F &f, const G &g, const H &h) {
...
inline bool isAlmostEqual(f32 a, f32 b, f32 tolerance = (f32)1) {
  ...
  f32 d = Core::abs(a - b);
  f32 e = (Core::abs(a) + Core::abs(b) + Type<f32>::delta) *
          Type<f32>::epsilon * tolerance;
  return (d < e);
}
inline bool isAlmostEqual(f64 a, f64 b, f64 tolerance = (f64)1) {
  require_(tolerance > (f64)0);
  f64 d = Core::abs(a - b);
  f64 e = (Core::abs(a) + Core::abs(b) + Type<f64>::delta) *
          Type<f64>::epsilon * tolerance;
  return (d < e);
}

inline bool isAlmostEqual(
    const std::complex<f32> &a, const std::complex<f32> &b,
...
inline bool isAlmostEqual(
    const std::complex<f64> &a, const std::complex<f64> &b,
