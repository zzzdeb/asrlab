...
HtkSlfContext::HtkSlfContext() {
  ...
  if (silId_ == Fsa::InvalidLabelId)
    Core::Application::us()->warning(
        "Lexicon does not provide a pronunciation for silence.");
  unkId_ = Lexicon::us()->unkLemmaPronunciationId();
  if (unkId_ == Fsa::InvalidLabelId)
    Core::Application::us()->warning(
        "Lexicon does not provide a pronunciation for the unknown word.");
  type_ = HtkSlfForward;
  fps_ = 0.0;
  isCapitalize_ = false;
  mergePenalty_ = true;
...
void HtkSlfContext::setType(HtkSlfType type) { type_ = type; }

void HtkSlfContext::setFps(f32 fps) { fps_ = fps; }

void HtkSlfContext::setCapitalize(bool isCapitalize) {
  isCapitalize_ = isCapitalize;
}

void HtkSlfContext::setBase(f32 base) { base_ = base; }
...
namespace {
void setKeyAndId(ConstSemiringRef semiring, const Key &key, ScoreId &id,
                 ScoreId defaultId) {
  id = semiring->id(key);
...
void HtkSlfContext::setLmName(const std::string &lmName) { lmName_ = lmName; }

std::string HtkSlfContext::info() const {
  std::ostringstream oss;
  oss << "htk slf context" << std::endl
      << "  type:            "
      << ((type_ == HtkSlfForward) ? "forward" : "backward") << std::endl
      << "  fps:             " << fps_ << std::endl
      << "  capitalize:      " << (isCapitalize_ ? "true" : "false")
      << std::endl
      << "  merge penalties: " << (mergePenalty_ ? "true" : "false")
      << std::endl;
  oss << std::endl;
  ...
  if (unkId_ != Fsa::InvalidLabelId)
    oss << "  unknown:         \""
        << std::string(lemmaPronunciationAlphabet->symbol(unkId_)) << "\""
        << std::endl;
  else
    oss << "  unknown:         n/a" << std::endl;
  if (silId_ != Fsa::InvalidLabelId)
    oss << "  silence:         \""
        << std::string(lemmaPronunciationAlphabet->symbol(silId_)) << "\""
        << std::endl;
  else
...
namespace {
const Core::Choice TypeChoice("forward", HtkSlfForward, "backward",
                              HtkSlfBackward, Core::Choice::endMark());
const Core::ParameterChoice paramType("slf-type", &TypeChoice, "htk slf type",
                                      HtkSlfForward);
const Core::ParameterFloat paramFps("fps", "timeframe index = time * scale",
                                    100.0, 0.0);
const Core::ParameterBool paramCapitalize("capitalize",
                                          "capitalize orthography", false);
const Core::ParameterFloat paramWordPenalty("word-penalty", "word penalty",
                                            Semiring::Invalid);
const Core::ParameterFloat paramSilPenalty("silence-penalty", "silence penalty",
                                           Semiring::Invalid);
const Core::ParameterBool paramMergePenalties(
    "merge-penalties", "merge word and silence penalties into am-score", false);
const Core::ParameterString paramEpsSymbol("eps-symbol", "Epsilon symbol",
                                           "!NULL");
} // namespace
...
const Core::ParameterBool
    paramSetCoarticulation("set-coarticulation",
                           "set coarticulation for across word models", false);
}
...
protected:
  ...
  struct Node {
    ...
    Node()
        : sp(0), labelId(Fsa::InvalidLabelId), transit(TransitUnchecked),
          initialPhonemeId(InvalidPhonemeId), finalPhonemeId(InvalidPhonemeId) {
    }
  };
...
public:
  HtkSlfBuilder(const Core::Configuration &config, HtkSlfContextRef context);
  ~HtkSlfBuilder();
...
const Bliss::Phoneme::Id HtkSlfBuilder::InvalidPhonemeId =
    Core::Type<Bliss::Phoneme::Id>::max;

HtkSlfBuilder::HtkSlfBuilder(const Core::Configuration &config,
                             HtkSlfContextRef context)
    : Precursor(config), context_(context),
      contextHandling_(HtkSlfReader::UpdateContext), logComments_(false) {
  lexicon_ = Lexicon::us();
...
HtkSlfBuilder::~HtkSlfBuilder() { clear(); }
...
HtkSlfBuilder::Status HtkSlfBuilder::header(const StringPairList &props,
                                            HtkSlfHeader &header) {
  s32 required = 2;
  header.reset();
...
Fsa::LabelId HtkSlfBuilder::lemmaPronunciationId(std::string &symbol,
                                                 s32 variant) const {
  ...
  if (label != Fsa::InvalidLabelId) {
    if (label != Fsa::Epsilon)
      error("No special symbols besides epsilon are treated correctly; found "
            "special symbol \"%s\".",
            symbol.c_str());
    else if (variant != -1)
  ...
  if (label == Fsa::InvalidLabelId) {
    warning("Could not find any pronunciation for word \"%s\", substitute by "
            "unknown.",
            symbol.c_str());
...
HtkSlfBuilder::Status HtkSlfBuilder::node(const StringPairList &props) {
  ...
  for (StringPairList::const_iterator it = props.begin(); it != props.end();
    ...
    case 'v':
      ...
      break;
    case 'd':
      if (id == 0) // We assume that the "I" property always occurs before the
                   // "d" property
  ...
  nodes_[id].sp = sp;
  if (!label.empty()) {
    if ((nodes_[id].labelId = lemmaPronunciationId(label, variant)) ==
        Fsa::InvalidLabelId) {
...
HtkSlfBuilder::Status HtkSlfBuilder::link(const StringPairList &props) {
  ...
  for (StringPairList::const_iterator it = props.begin(); it != props.end();
    ...
    case 'd':
      if (linkId ==
          0) // We assume that the "J" field always occurs before the "d" field
        warning("No support for the div field.");
      break;
    case 'n':
      if (linkId ==
          0) // We assume that the "J" field always occurs before the "n" field
  ...
  Fsa::LabelId labelId;
  if (label.empty()) {
    labelId = (context_->type() == HtkSlfForward) ? nodes_[targetId].labelId
...
HtkSlfBuilder::Status HtkSlfBuilder::setCoarticulation() {
  Lexicon::LemmaPronunciationAlphabetRef lpAlphabet =
      Lexicon::us()->lemmaPronunciationAlphabet();
  Core::Ref<const Bliss::PhonemeInventory> phonemeInventory =
      Lexicon::us()->phonemeInventory();
  ...
  for (NodeList::iterator it = nodes_.begin(); it != nodes_.end(); ++it)
    if (it->transit != TransitUnchecked) {
      ...
      (*boundaries_)[sid].setTransit(Boundary::Transit(
          it->finalPhonemeId, it->initialPhonemeId, AcrossWordBoundary));
    }
...
const char *HtkStatusMessage[] = {
    ...
    "Semiring is not set.",
    "Fps is not set.",
    "Error while parsing orthography; lexicon does not contain any matching "
    "pronunciation.",
...
bool HtkSlfBuilder::checkStatus(Status status) {
  if (status != htkOk) {
    error("Error while parsing \n%5d: %s", lineNo_, HtkStatusMessage[status]);
    return false;
  } else
    return true;
}
...
HtkSlfBuilder::Status HtkSlfBuilder::addProperty(const std::string &s,
  ...
  while (*c != '\0') {
    ...
    props.push_back(StringPair(std::string(keyStart, keyEnd - keyStart),
...
const Core::ParameterString
    HtkSlfReader::paramContextMode("context-mode", "context mode", "update");
const Core::Choice ContextModeChoice =
    Core::Choice("trust", 0, "adapt", 1, "update", 5, Core::Choice::endMark());
const Core::ParameterBool
    HtkSlfReader::paramLogComments("log-comments",
                                   "log all comments in htk slf file", false);
const Core::ParameterString
    HtkSlfReader::paramEncoding("encoding", "encoding of htk lattice", "utf-8");
...
HtkSlfReader::HtkSlfReader(const Core::Configuration &config,
                           HtkSlfContextRef context, u32 contextHandling)
    : Precursor(config) {
  require(context);
  builder_ = new HtkSlfBuilder(config, context);
  builder_->setContextHandling(contextHandling);
...
HtkSlfReader::~HtkSlfReader() { delete builder_; }

HtkSlfContextRef HtkSlfReader::context() const { return builder_->context(); }
...
HtkSlfHeader *HtkSlfReader::readHeader(const std::string &filename,
...
ConstLatticeRef HtkSlfReader::read(const std::string &filename,
                                   Core::Archive *archive) {
  InputStream is(filename, archive);
  if (!is)
...
public:
  ...
  virtual void writeWordVariant(std::ostream &os,
                                Fsa::LabelId labelId) const = 0;
};
...
public:
  ...
  virtual void writeWordVariant(std::ostream &os, Fsa::LabelId labelId) const {
    std::pair<const Bliss::Lemma *, s32> lv =
        lexicon->lemmaPronunciationVariant(
            lpAlphabet->lemmaPronunciation(labelId));
...
void HtkSlfWriter::setFps(f32 fps) { fps_ = fps; }
...
void HtkSlfWriter::writeNodes(ConstLatticeRef f, HtkSlfHeader &header,
                              StateIdMapping &mapping, std::ostream &os) const {
    ...
    if (b.valid()) {
      for (++it; it != mapping.finals.end(); ++it)
        ...
        if (b != boundaries->get((*it)->id())) {
          warning("inconsistent boundaries at '")
              << (*it)->id() << "': " << b.time() << " vs. "
              << boundaries->get((*it)->id()).time();
        }
#endif
      os << " t=" << (b.time() / fps_);
    }
...
void HtkSlfWriter::writeLinks(ConstLatticeRef f, HtkSlfHeader &header,
                              StateIdMapping &mapping, std::ostream &os) const {
  ...
  for (std::vector<Fsa::StateId>::const_iterator it = mapping.htk2fsa.begin();
    ...
    for (State::const_iterator a = sr->begin(); a != sr->end(); ++a, ++linkId) {
      os << "J=" << linkId << " S=" << nodeId
         << " E=" << mapping.fsa2htk[a->target()];
  ...
  if (!mapping.finals.empty()) {
    ...
    for (ConstStateRefList::const_iterator it = mapping.finals.begin();
         it != mapping.finals.end(); ++it, ++linkId) {
      os << "J=" << linkId << " S=" << mapping.fsa2htk[(*it)->id()]
         << " E=" << targetId;
...
public:
  HtkSlfHeaderAndMappingBuilder(ConstLatticeRef f, HtkSlfHeader &header,
                                HtkSlfWriter::StateIdMapping &mapping)
      : TraverseState(f), header_(header), mapping_(mapping) {
    ...
    header_.lmName = "";
    ScoreId lmId = f->semiring()->id("lm");
    header_.lmScale = (f->semiring()->hasId(lmId)) ? f->semiring()->scale(lmId)
                                                   : Semiring::UndefinedScale;
    header_.wrdPenalty = header_.silPenalty = Semiring::Invalid;
    header_.nNodes = header_.nLinks = 0;
    traverse();
    if ((mapping_.finals.size() == 1) && !mapping_.finals.front()->hasArcs() &&
        (f->semiring()->compare(mapping_.finals.front()->weight(),
                                f->semiring()->one()) == 0)) {
...
bool HtkSlfWriter::buildHeaderAndMapping(ConstLatticeRef f,
                                         HtkSlfHeader &header,
  ...
  if (!header.base == -1.0)
    warning("all scores and penalties are considered negative natural "
            "logarithms of probabilities");
  HtkSlfHeaderAndMappingBuilder build(f, header, mapping);
  return true;
}

bool HtkSlfWriter::write(ConstLatticeRef l, HtkSlfHeader &header,
                         StateIdMapping &mapping, const std::string &filename,
...
bool HtkSlfWriter::write(ConstLatticeRef l, const std::string &filename,
                         Core::Archive *archive) const {
...
*/
HtkSlfArchiveReader::HtkSlfArchiveReader(const Core::Configuration &config,
...
HtkSlfArchiveReader::~HtkSlfArchiveReader() { delete reader_; }
...
*/
HtkSlfArchiveWriter::HtkSlfArchiveWriter(const Core::Configuration &config,
                                         const std::string &pathname)
    : Precursor(config, pathname) {
  archive =
...
HtkSlfArchiveWriter::~HtkSlfArchiveWriter() { delete writer_; }
