...
// ===========================================================================
const PhonemeGraph::Edge *
PhonemeGraphNode::findIncomingPhoneme(Phoneme::Id p) const {
  for (PhonemeGraph::const_IncomingEdgeIterator e = incomingEdges(); e.valid();
       ++e) {
...
const PhonemeGraph::Edge *
PhonemeGraphNode::findOutgoingPhoneme(Phoneme::Id p) const {
  for (PhonemeGraph::const_OutgoingEdgeIterator e = outgoingEdges(); e.valid();
       ++e) {
    if (e->phoneme() && e->phoneme()->id() == p)
      return e.pointer();
  }
...
// ===========================================================================
PhonemeGraphBuilder::PhonemeGraphBuilder(const Core::Configuration &c,
                                         LexiconRef _lexicon)
    : Core::Component(c), lexicon_(_lexicon) {
  optimize_ = true;
}
...
void PhonemeGraphBuilder::build(PhonemeGraph &pg, const LemmaGraph &lg) const {
  ...
  std::vector<PhonemeGraph::Node *> nn(lg.nNodeIds(),
                                       (PhonemeGraph::Node *)(0));
  ...
  for (lge.begin(); lge.valid(); ++lge) {
    if (lge->lemma()) {
      ...
      for (Core::tie(pron, pron_end) = lge->lemma()->pronunciations();
           pron != pron_end; ++pron)
        addPronunciation(pg, nn[lge->sourceId()], nn[lge->targetId()], pron);
    } else {
      pg.addEdge(new PhonemeGraph::Edge(0, 0), nn[lge->sourceId()],
                 nn[lge->targetId()]);
    }
  }
...
void PhonemeGraphBuilder::addPronunciation(PhonemeGraph &graph,
  ...
  if (i < j) {
    ...
    PhonemeGraph::Node::Flags flags =
        PhonemeGraph::Node::Flags(PhonemeGraph::Node::withinWord |
  ...
  while (ff != from) {
    ...
    ff->setFlags(PhonemeGraph::Node::Flags(
        ff->flags() & ~PhonemeGraph::Node::uniquePronunciation));
