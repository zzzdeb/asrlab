...
AbstractMixtureSet::~AbstractMixtureSet() { clear(); }
...
void AbstractMixtureSet::removeDensitiesWithLowWeight(Weight minWeight,
 ...
 * MixtureSet
 */
MixtureSet::MixtureSet(ComponentIndex dimension) : Precursor(dimension) {}
...
MixtureSet *MixtureSet::createOneMixtureClusterCopy(
...
void MixtureSet::writeLogNormStatistics(Core::XmlWriter &os) const {
  ...
  if (logNormStatistics.minimum() < 0) {
    Core::Application::us()->error(
        ...
        " variance normalization, e.g. "
        "Signal::ScatterTransform::paramTransformScale");
  }
}
...
bool MixtureSet::write(std::ostream &o) const {
  ...
  o << dimension() << " " << nMixtures() << " " << nDensities() << " "
    << nMeans() << " " << nCovariances() << std::endl;
...
bool MixtureSet::read(std::istream &i) {
  ...
  if (version > 2.0) {
    Core::Application::us()->criticalError("version \"")
        << version << "\" not supported";
  }
  getline(i, line); // CovType
  std::string covtype = line.substr(17);
  if (covtype.compare("DiagonalCovariance") != 0) {
    Core::Application::us()->error("No correct Covariance Type set: %s",
                                   covtype.c_str());
  }
...
bool MixtureSet::read(Core::BinaryInputStream &i) {
  // return Legacy::readMixtureSet(i, *this,
  // paramRepeatedCovariance(configuration));
#if 1
  Core::Application::us()->criticalError(
      "reading of binary mixture sets is not supported in this version");
  return false;
#endif
}
