...
LabelId Alphabet::index(const std::string &symbol) const {
  ...
  std::cerr << "critical error: this alphabet does not support symbol-to-index "
               "mapping."
            << std::endl;
  return InvalidLabelId;
}
...
StaticAlphabet::StaticAlphabet() { clear(); }
...
bool StaticAlphabet::read(Core::BinaryInputStream &i) {
  ...
  while (index != InvalidLabelId) {
    ...
    default:
      ...
      if (symbol.empty())
        std::cerr << "empty symbol string is not support in alphabet.\n"
                  << std::endl;
      if (!addIndexedSymbol(symbol, index, tag)) {
        std::cerr << "duplicate symbol '" << symbol << "' in alphabet.\n"
                  << std::endl;
        return false;
      }
    }
