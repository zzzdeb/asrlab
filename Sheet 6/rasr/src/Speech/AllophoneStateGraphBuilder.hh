...
public:
   ...
   */
  template <class BuilderInput> class FunctorBase {
  protected:
  ...
  public:
    FunctorBase(AllophoneStateGraphBuilder &builder, const std::string &id,
  ...
  public:
    Functor(AllophoneStateGraphBuilder &builder, const std::string &id,
            const BuilderInput &builderInput)
  ...
  /**
   *   Converts a call to one of the finalizeTransducer functions into a
   * functor.
   *
  ...
  public:
    TranducerFunctor(AllophoneStateGraphBuilder &builder, const std::string &id,
                     const BuilderInput &builderInput)
        : FunctorBase<BuilderInput>(builder, id, builderInput) {}
...
private:
  ...
  AllophoneStateGraphRef build(const Alignment &alignment,
                               AllophoneStateGraphRef);

public:
  AllophoneStateGraphBuilder(const Core::Configuration &,
  ...
  Functor<std::string> createFunctor(const std::string &id,
  ...
  /** Builds allophone state acceptor for phoneme loops, cf. phoneme
   * recognition. */
  enum InputLevel { lemma, phone };
  AllophoneStateGraphRef build(const InputLevel &level);
  Functor<InputLevel> createFunctor(const std::string &id,
  ...
  AllophoneStateGraphRef
  build(const Bliss::Coarticulated<Bliss::Pronunciation> &);
  Functor<Bliss::Coarticulated<Bliss::Pronunciation> >
  createFunctor(const Bliss::Coarticulated<Bliss::Pronunciation> &p) {
   ...
   */
  AllophoneStateGraphRef
  build(const Alignment &, const Bliss::Coarticulated<Bliss::Pronunciation> &);

  /** Builds a allophone state to lemma pronunciation transducer from
   * orthography. */
  Fsa::ConstAutomatonRef buildTransducer(const std::string &orth);
  TranducerFunctor<const std::string>
  createTransducerFunctor(const std::string &id, const std::string &orth) {
  ...
  FinalizationFunctor createFinalizationFunctor(const std::string &id,
                                                Fsa::ConstAutomatonRef t) {
  ...
  /** Builds a allophone state to lemma pronunciation transducer from lemma
   * acceptor. */
