...
void SearchAlgorithm::Traceback::write(
  ...
  for (const_iterator tbi = begin(); tbi != end(); ++tbi) {
    ...
    os << "    "
       << ((tbi->transit.final == Bliss::Phoneme::term)
               ? "#"
               : phi->phoneme(tbi->transit.final)->symbol().str())
       << "|"
       << ((tbi->transit.initial == Bliss::Phoneme::term)
               ? "#"
               : phi->phoneme(tbi->transit.initial)->symbol().str())
       << std::endl;
  }
}
...
Fsa::ConstAutomatonRef SearchAlgorithm::Traceback::lemmaAcceptor(
  ...
  for (u32 i = 0; i < size(); ++i)
    if ((*this)[i].pronunciation) {
      s2 = result->newState();
      s1->newArc(s2->id(), result->semiring()->one(),
                 (*this)[i].pronunciation->lemma()->id());
      s1 = s2;
    }
...
Fsa::ConstAutomatonRef SearchAlgorithm::Traceback::lemmaPronunciationAcceptor(
    Core::Ref<const Bliss::Lexicon> lexicon) const {
  Bliss::LemmaPronunciationAcceptor *result =
      new Bliss::LemmaPronunciationAcceptor(lexicon);
  const Bliss::LemmaPronunciationAlphabet *abet =
      result->lemmaPronunciationAlphabet();
  Fsa::State *s1, *s2;
  s1 = result->newState();
  result->setInitialStateId(s1->id());
  for (u32 i = 0; i < size(); ++i) {
    ...
    s2 = result->newState();
    s1->newArc(s2->id(), result->semiring()->one(),
               abet->index((*this)[i].pronunciation));
    s1 = s2;
  }
  result->setStateFinal(s1);
  return Fsa::ConstAutomatonRef(result);
...
Lattice::WordLatticeRef SearchAlgorithm::Traceback::wordLattice(
    Core::Ref<const Bliss::Lexicon> lexicon) const {
  ...
  result->setFsa(lemmaPronunciationAcceptor(lexicon),
