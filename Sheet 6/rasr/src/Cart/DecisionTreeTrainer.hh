...
struct ExamplePtrRange {
  ...
  ExamplePtrRange(ExamplePtrList::const_iterator begin,
                  ExamplePtrList::const_iterator end)
      : begin(begin), end(end) {}
...
public:
  Scorer(const Core::Configuration &config) : Core::Component(config) {}
  virtual ~Scorer() {}
  ...
  */
  ...
  /*
    ...
    The parameters leftExamples and rightExamples are the result of splitting
    the father node, fatherScore is the score assigned to the father node. The
    return value is the gain achieved by the split, i.e. the higher the better,
    and must be positive. leftScore and rightScore are the scores of the left
    and right child node after the split of the father node. Remark: The second
    form is used for initialization.
  */
  virtual Score operator()(const ExamplePtrRange &leftExamples,
                           const ExamplePtrRange &rightExamples,
                           const Score fatherScore, Score &leftChildScore,
                           Score &rightChildScore) const = 0;
  virtual void operator()(const ExamplePtrRange &examples, Score &score) const {
    Score dummy;
    operator()(examples,
               ExamplePtrRange(ExamplePtrList::const_iterator(),
                               ExamplePtrList::const_iterator()),
               0.0, score, dummy);
  }
};
...
public:
  ID3(const Core::Configuration &config) : Precursor(config), map_() {}

  void write(std::ostream &os) const;

  Score entropy(ExamplePtrList::const_iterator begin,
                ExamplePtrList::const_iterator end) const;

  Score operator()(const ExamplePtrRange &leftExamples,
                   const ExamplePtrRange &rightExamples,
  ...
  */
  struct TrainingPlan {
    struct Step {
      ...
      Step()
          : name(), action("split"), minObs(0), minGain(0.0),
            nRandomQuestion(1), questionRefs(0) {}
      ~Step() { delete questionRefs; }
    ...
    TrainingPlan(PropertyMapRef map = PropertyMapRef(new PropertyMap))
        : map(map), maxLeaves(Core::Type<u32>::max), steps() {}
    ~TrainingPlan() {
      for (StepPtrList::iterator it = steps.begin(); it != steps.end(); ++it)
        delete *it;
    }
...
public:
  DecisionTreeTrainer(const Core::Configuration &config,
                      ...
                      TrainingPlan *plan = 0)
      : Precursor(config), map_(), scorer_(scorer), plan_(plan) {
    setMap(map);
  }
  ~DecisionTreeTrainer() { delete plan_; }
  ...
  ClusterList *train(DecisionTree *tree,
                     const std::string &examplesFilename = "");
...
inline std::ostream &operator<<(std::ostream &out,
                                const Cart::DecisionTreeTrainer &t) {
  t.write(out);
  return out;
}
