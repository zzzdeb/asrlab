...
void StorageAutomatonXmlParser::startFsa(const Core::XmlAttributes atts) {
  ...
  if (!tmp) {
    ...
    Core::Choice::Value type =
        TypeChoice[std::string(reinterpret_cast<const char *>(tmp))];
  ...
  if (!tmp) {
    parser()->warning(
        "no semiring specified for fsa. assuming tropical semiring.");
    fsa_->setSemiring(TropicalSemiring);
  } else {
    Core::Choice::Value type =
        SemiringTypeChoice[std::string(reinterpret_cast<const char *>(tmp))];
    if (type != Core::Choice::IllegalValue)
      fsa_->setSemiring(getSemiring(SemiringType(type)));
    else {
      parser()->warning(
          "unknown semiring specified for fsa. assuming tropical semiring.");
      fsa_->setSemiring(TropicalSemiring);
    }
  }
  tmp = atts["initial"];
...
void StorageAutomatonXmlParser::startState(const Core::XmlAttributes atts) {
  ...
  if (idStr) {
    ...
    else if (id > StateIdMask)
      parser()->error("state id too large (%d > %d)", id, StateIdMask);
    state_ = new State(id);
  } else
    parser()->error("no state id specified");
}
void StorageAutomatonXmlParser::endState() { fsa_->setState(state_); }
void StorageAutomatonXmlParser::startUser(const Core::XmlAttributes atts) {
  if (state_)
    state_->addTags(StateTagUser);
}
...
void StorageAutomatonXmlParser::startFinalWeight(
...
void StorageAutomatonXmlParser::startArc(const Core::XmlAttributes atts) {
  ...
  if (idStr) {
    ...
    StateId id = strtoll((const char *)idStr, &end, 0);
    if (*end != '\0')
      parser()->error("invalid target state id \"%s\"", idStr);
    else if (id > StateIdMask)
      parser()->error("target id too large (%d > %d)", id, StateIdMask);
    arc_ = state_->newArc();
    arc_->target_ = id;
    arc_->input_ = Epsilon;
...
StorageAutomatonXmlParser::StorageAutomatonXmlParser(
    const Core::Configuration &c, StorageAutomaton *fsa)
    : Core::XmlSchemaParser(c), fsa_(fsa) {
  fsa_->setType(TypeTransducer);
  Core::Ref<StaticAlphabet> input(new StaticAlphabet),
      output(new StaticAlphabet);
  ...
  setRoot(new Core::XmlMixedElementRelay(
      ...
      XML_CHILD(new Core::XmlMixedElementRelay(
          "state", this, startHandler(&Self::startState),
          endHandler(&Self::endState), 0,
          ...
          XML_CHILD(new Core::XmlMixedElementRelay(
              ...
              endHandler(&Self::endFinalWeight),
              charactersHandler(&Self::charactersFinalWeight),
              XML_NO_MORE_CHILDREN)),
          XML_CHILD(new Core::XmlMixedElementRelay(
              "arc", this, startHandler(&Self::startArc), 0, 0,
              XML_CHILD(new Core::XmlMixedElementRelay(
                  "in", this, startHandler(&Self::startIn),
                  endHandler(&Self::endIn),
                  charactersHandler(&Self::charactersArc),
                  XML_NO_MORE_CHILDREN)),
              XML_CHILD(new Core::XmlMixedElementRelay(
                  "out", this, startHandler(&Self::startOut),
                  endHandler(&Self::endOut),
                  charactersHandler(&Self::charactersArc),
                  XML_NO_MORE_CHILDREN)),
              XML_CHILD(new Core::XmlMixedElementRelay(
                  "weight", this, startHandler(&Self::startArcWeight),
                  endHandler(&Self::endArcWeight),
                  charactersHandler(&Self::charactersArc),
                  XML_NO_MORE_CHILDREN)),
              XML_NO_MORE_CHILDREN)),
          XML_NO_MORE_CHILDREN)),
