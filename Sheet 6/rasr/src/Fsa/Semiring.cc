...
ConstSemiringRef TropicalSemiring =
    ConstSemiringRef(new PredefinedTropicalSemiring_());
...
template <>
Fsa::Weight LogIntegerSemiring_::collect(const Fsa::Weight &a,
                                         const Fsa::Weight &b) const {
  return invalid();
}
template <>
Semiring<Weight>::Accumulator *
LogIntegerSemiring_::getCollector(const Fsa::Weight &initial) const {
  return 0;
}
template <>
int LogIntegerSemiring_::compare(const Fsa::Weight &a,
                                 const Fsa::Weight &b) const {
...
/*
  virtual Core::XmlElement * fromXml(XmlParser &, void (XmlParser::*)(Weight *))
  const { std::cerr << "method \"fromXml\" is not supported" << std::endl;
  return 0; }
*/

template <>
std::string LogIntegerSemiring_::describe(const Fsa::Weight &a,
...
ConstSemiringRef TropicalIntegerSemiring =
    ConstSemiringRef(new TropicalIntegerSemiring_());
...
ConstSemiringRef ProbabilitySemiring =
    ConstSemiringRef(new PredefinedProbabilitySemiring_());
...
public:
  ...
  size_t compressedSize() const { return sizeof(u32); }
...
Core::Choice SemiringTypeChoice("log", SemiringTypeLog, "tropical",
                                SemiringTypeTropical, "tropical-integer",
                                SemiringTypeTropicalInteger, "count",
                                SemiringTypeCount, "probability",
                                SemiringTypeProbability,
                                Core::Choice::endMark());
