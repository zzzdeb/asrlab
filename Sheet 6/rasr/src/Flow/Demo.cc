...
const Core::ParameterInt DemoNode::paramMaxInputs_(
    "max-input-nodes", "maximum number of input ports", Core::Type<u32>::max);

// =======================================================

DemoNode::DemoNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), demoChannel_(c, "demo-info"),
      maxInputs_(1), isFirstInPort_(true), isFirstOutPort_(true) {
  // note that attributesChannel_ and dataChannel_ are inherited from base class
  // Flow::Node and can be set using dump-attributes = <file> and dump-data =
  // <file> i.e. if you just want to dump the attributes and the data seen by
  // the demo node and not of all nodes use demo.dump-data.channel = <file> and
  // demo.dump-attributes.channel = <file>
...
bool DemoNode::configure() {
  if (demoChannel_.isOpen()) {
    ...
    demoChannel_ << Core::XmlComment(
        "configure method of a Demo Node is called");

    // require that the number of input ports equals the number of output
    // ports
    demoChannel_ << Core::XmlEmpty(
        "checking number of input ports == number of output ports");
    demoChannel_ << (Core::XmlEmpty("") +
  ...
  if (demoChannel_.isOpen()) {
    demoChannel_ << Core::XmlComment(
        "number of input ports matches number of output ports");
    demoChannel_ << (Core::XmlEmpty("ports") +
                     Core::XmlAttribute("number of ports", nInputs()));
  }
  bool status = true;
...
PortId DemoNode::getInput(const std::string &name) {
  if (demoChannel_.isOpen()) {
    demoChannel_ << Core::XmlOpen("getInput method of " + fullName())
                 << Core::XmlComment(
                        "allowing arbitrary number of input nodes");
    ...
    demoChannel_ << Core::XmlEmpty("testing if port name=" + name +
                                   " already known");
  }
...
PortId DemoNode::getOutput(const std::string &name) {
  if (demoChannel_.isOpen()) {
    demoChannel_ << Core::XmlOpen("getOutput method of " + fullName())
                 << Core::XmlComment(
                        "allowing arbitrary number of output nodes");
  }
  /* if you want your getOutput method to allow only an arbitrary number of
   * ports with port names given by postive numbers in the flow network file you
   * could programm this like: a similiar method can be found in
   * Image::VitruvianManPatches
   *
   *  char* endptr;
   *  int result = strtol(name.c_str(),&endptr,10);
   *  // check if the string has been a integer number and that is in range 0 to
   * maxInputs_ demoChannel_ << Core::XmlOpen("checking written port number")
   *            << Core::XmlEmpty("port number = " + name)
   *           << Core::XmlClose("checking written port number");
   *  if(*endptr!='\0' || result < 0 || result >= (int)maxInputs_){
   *    demoChannel_ << Core::XmlEmpty("port number no number or not in range 0
   * to max-input-nodes")
