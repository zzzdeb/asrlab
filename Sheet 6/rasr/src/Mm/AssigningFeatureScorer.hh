...
protected:
  ...
  public:
    ...
    virtual void
    getDensityPosteriorProbabilities(EmissionIndex e,
                                     std::vector<Mm::Weight> &result) const = 0;
  };
...
public:
  ...
  virtual void getFeatureDescription(FeatureDescription &description) const {
    description.mainStream().setValue(FeatureDescription::nameDimension,
                                      dimension());
  }
  ...
  getAssigningScorer(Core::Ref<const Feature> feature) const {
  ...
  virtual const std::vector<DensityIndex> &
  densitiesInMixture(MixtureIndex) const {
...
protected:
  ...
  protected:
    CachedAssigningContextScorer(
        const CachedAssigningFeatureScorer *featureScorer,
  ...
  public:
    ...
    virtual Score score(EmissionIndex e) const {
      ...
      if (!cache_.isCalculated(e))
        return cache_.set(e, featureScorer_->calculateScoreAndDensity(this, e))
            .score;
    ...
    virtual void
    getDensityPosteriorProbabilities(EmissionIndex e,
                                     std::vector<Mm::Weight> &result) const {
      featureScorer_->calculateDensityPosteriorProbabilities(this, score(e), e,
                                                             result);
    }
  };
  virtual ScoreAndBestDensity
  calculateScoreAndDensity(const CachedAssigningContextScorer *,
                           MixtureIndex) const = 0;
  virtual Score calculateScore(const CachedAssigningContextScorer *,
                               MixtureIndex, DensityIndex) const {
    criticalError("This feature scorer does not support the calculation of "
                  "scores given a density in the mixture");
    return Core::Type<Score>::max;
  }
  virtual void calculateDensityPosteriorProbabilities(
      const CachedAssigningContextScorer *, Score logDenominator,
      EmissionIndex e, std::vector<Mm::Weight> &result) const {
    criticalError("This feature scorer does not support the calculation of "
                  "density posterior probabilities");
  }
...
class DensitySpecificCachedAssigningFeatureScorer
...
protected:
  ...
  protected:
    ContextScorer(
        const DensitySpecificCachedAssigningFeatureScorer *featureScorer,
        EmissionIndex nEmissions)
        : CachedAssigningContextScorer(featureScorer, nEmissions),
          featureScorer_(featureScorer), scoreCache_(nEmissions) {}
  ...
  public:
    ...
    virtual Score score(EmissionIndex e, DensityIndex dnsInMix) const {
      ...
      if (!scoreCache_.isCalculated(e, dnsInMix)) {
        return scoreCache_.set(
            e, dnsInMix, featureScorer_->calculateScore(this, e, dnsInMix));
      }
      return scoreCache_(e, dnsInMix);
    }
  };
  virtual Score calculateScore(const CachedAssigningContextScorer *,
                               MixtureIndex, DensityIndex) const = 0;
