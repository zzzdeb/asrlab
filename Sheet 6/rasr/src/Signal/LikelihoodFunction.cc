...
bool IndependentSequenceLikelihood::setClasses(
    const std::vector<std::string> &classLabels) {
  if (logLikelihoodFunctions_ == 0)
    return false;
  if (logLikelihoodFunctions_->nMixtures() != classLabels.size()) {
    error("Number of mixtures (%d) does not match to the number of classes "
          "(%zd).",
...
void IndependentSequenceLikelihood::feed(const std::vector<Data> &featureVector,
  ...
  Mm::FeatureScorer::Scorer scorer =
      logLikelihoodFunctions_->getScorer(feature);
