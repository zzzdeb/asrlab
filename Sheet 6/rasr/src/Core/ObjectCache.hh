...
template <class Data>
void ObjectCacheItem<Data>::dump(XmlOutputStream &os,
 ...
 * If an element is accessed the corresponding element is moved (erase,
 * push_front) to the beginning of the linked list. last() returns last key in
 * the linked list.
 */
...
MruObjectCacheList<_Key, _Data, _HashFunction, _EqualKey>::access(
    const Key &key) {
...
template <class _Key, class _Data, class _HashFunction, class _EqualKey>
typename MruObjectCacheList<_Key, _Data, _HashFunction, _EqualKey>::Iterator
MruObjectCacheList<_Key, _Data, _HashFunction, _EqualKey>::insert(
    const Key &key, Data *data) {
  std::pair<Iterator, bool> result =
      table_.insert(std::make_pair(key, new Item(data)));
...
template <class _Key, class _Data, class _HashFunction, class _EqualKey>
void MruObjectCacheList<_Key, _Data, _HashFunction, _EqualKey>::erase(
    Iterator item) {
...
/**
 * RandomObjectCacheList: list of ObjectCacheItems implementing the random
 * strategy
 *
 * ObjectCacheItems are stored in a hash map by their keys.
 ...
 *   -new position of its deque element. The new last element and the
 * ObjectCacheItem -are erased. last() chooses a random element of the deque..
 */
template <class _Key, class _Data, class _HashFunction, class _EqualKey>
...
public:
  ...
  Iterator last() {
    size_t randomIndex =
        size_t(f64(size()) * f64(rand()) / f64(RAND_MAX + 1.0));
...
RandomObjectCacheList<_Key, _Data, _HashFunction, _EqualKey>::insert(
    const Key &key, Data *data) {
  std::pair<Iterator, bool> result =
      table_.insert(std::make_pair(key, new Item(data)));
  verify(result.second);
...
template <class _Key, class _Data, class _HashFunction, class _EqualKey>
void RandomObjectCacheList<_Key, _Data, _HashFunction, _EqualKey>::erase(
    Iterator item) {
...
const ParameterInt
    parameterObjectCacheSize("size",
                             "max number of objects in the cache memory", 1, 1);
const ParameterString
    parameterObjectCacheDirectory("directory", "directory of cached objects");
const ParameterBool
    parameterObjectCacheDump("dump", "on/off dumping of objects", false);
 ...
 * The last object is given by the cache strategy implemented by the CacheList.
 * The files are stored in one directory. The file names are the keys of the
 * Datas. Datas can be explicitly saved (writeBack) and explicitly removed
 * (erase) from the memory. Store references to Datas with care! References are
 * valid until the next access to the cache.
 *
 */
...
private:
  ...
  /** Saves the object stored by @param item and removes the @param item from
   * the list_. Note: iterators on the list_ gets invalid after a call to this
   * funciton.
   */
...
public:
   ...
   *        appendObjectCacheMode reuses existing objects and allows creation of
   * new objects createObjectCacheMode deletes exisiting objects and create new
   * ones
   */
  ObjectCache(const Configuration &c, u8 mode = createObjectCacheMode);
  ...
  const Data *findForReadAccess(const Key &key) {
    ListIterator item;
    return (item = access(key)) != list_.end() ? &(item->second->readAccess())
                                               : 0;
  }
  ...
  Data *findForWriteAccess(const Key &key) {
    ListIterator item;
    return (item = access(key)) != list_.end() ? &(item->second->writeAccess())
                                               : 0;
  }
  ...
  std::set<Key> keys() {
    ...
    if (!name().empty()) {
      ...
      for (DirectoryFileIterator file(directoryName_, &filter); file; ++file)
        result.insert(
            file.path().substr(0, file.path().size() - fileExtension().size()));
    }
...
ObjectCache<List>::ObjectCache(const Configuration &c, u8 mode)
    : Component(c), maxSize_(0), mode_(mode), dump_(false) {
...
template <class List> void ObjectCache<List>::writeBack(ListIterator item) {
...
template <class List>
typename ObjectCache<List>::ListIterator
ObjectCache<List>::access(const Key &key) {
...
template <class List> bool ObjectCache<List>::parameterName(std::string &n) {
  if (n.find(name() + ".") == 0) {
...
template <class List> void ObjectCache<List>::writeBack(const Key &key) {
  ListIterator item = list_.find(key);
  if (item != list_.end())
    writeBack(item);
}

template <class List> void ObjectCache<List>::writeBack() {
  while (list_.size() > 0)
    writeBack(list_.begin());
}

template <class List> void ObjectCache<List>::clear() {
  list_.clear();
  if (!name().empty()) {
    DirectoryFileIterator::FileExtensionFilter filter(fileExtension());
    for (DirectoryFileIterator file(directoryName_, &filter); file; ++file)
...
template <class List> void ObjectCache<List>::setMaxSize(u32 maxSize) {
  maxSize_ = maxSize;
  while (list_.size() > maxSize_)
    writeBack(list_.last());
}
...
template <class List>
bool ObjectCache<List>::setParameter(const std::string &name,
