...
public:
  ...
  static SymbolicMapping loadSymbolicMapping(const Core::Configuration &config,
                                             ConstSemiringRef target) {
    ...
    for (KeyList::const_iterator itKey = target->keys().begin();
      ...
      if (itKey->empty())
        Core::Application::us()->warning(
            "Dimension with no symbolic identifier in \"%s\"",
            target->name().c_str());
      else
...
const Core::ParameterStringVector
    SymbolicMapping::paramRow("row", "row in matrix projection");
...
public:
  ...
  ProjectionMatrix buildLinearMapping(ScoreId offset = 0,
    ...
    ScoreId id = 0;
    for (; (itProjectionMatrix != scaledMapping.end()) && (id < offset);
         ++itProjectionMatrix, ++id)
      itProjectionMatrix->resize(source_->size(), 0.0);
    id = 0;
    for (;
         (itProjectionMatrix != scaledMapping.end()) && (id < source_->size());
         ++itProjectionMatrix, ++id) {
      itProjectionMatrix->resize(source_->size(), 0.0);
      (*itProjectionMatrix)[id] =
          scaled ? source_->scale(id) : Semiring::DefaultScale;
    }
  ...
  ProjectionMatrix buildMapping(const SymbolicMapping &symbolicMapping,
                                bool scaled = true) const {
    require(symbolicMapping.size() == target_->size());
    ProjectionMatrix scaledMapping(target_->size());
    ProjectionMatrix::iterator itProjectionMatrix = scaledMapping.begin();
    for (SymbolicMapping::const_iterator itKeyList = symbolicMapping.begin();
         itKeyList != symbolicMapping.end();
         ++itKeyList, ++itProjectionMatrix) {
      itProjectionMatrix->resize(source_->size(), 0.0);
      for (KeyList::const_iterator itKey = itKeyList->begin();
           itKey != itKeyList->end(); ++itKey) {
        ...
        if (source_->hasId(id))
          ...
          Core::Application::us()->error(
              "No dimension \"%s\" found in semiring \"%s\"", itKey->c_str(),
              source_->name().c_str());
      }
    }
    return scaledMapping;
...
// -------------------------------------------------------------------------
ConstLatticeRef offsetSemiring(ConstLatticeRef l,
                               ConstSemiringRef targetSemiring,
                               ScoreId offset) {
  MappingBuilder mb(l->semiring(), targetSemiring);
  return projectSemiring(l, targetSemiring,
                         mb.buildLinearMapping(offset, false));
}
// -------------------------------------------------------------------------
...
NodeRef createChangeSemiringNode(const std::string &name,
...
public:
  ProjectSemiringNode(const std::string &name,
                      const Core::Configuration &config)
      : FilterNode(name, config) {}
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    symbolicMapping_ =
        SymbolicMapping::loadSymbolicMapping(select("matrix"), semiring_);
    scaled_ = paramScaled(config);
  }
};
const Core::ParameterBool
    ProjectSemiringNode::paramScaled("scaled", "scale projection", true);

NodeRef createProjectSemiringNode(const std::string &name,
                                  const Core::Configuration &config) {
...
public:
  FsaToLattice(Fsa::ConstAutomatonRef fsa, ConstSemiringRef semiring,
               WeightMapRef weightMap, Lexicon::AlphabetMapRef inputMap,
               Lexicon::AlphabetMapRef outputMap)
      : fsa_(fsa), isAcceptor_(fsa->type() == Fsa::TypeAcceptor),
        semiring_(semiring), weightMap_(weightMap), inputMap_(inputMap),
  ...
  virtual ConstStateRef getState(Fsa::StateId sid) const {
    ...
    if (isAcceptor_)
      for (Fsa::State::const_iterator a = sr->begin(); a != sr->end(); ++a)
        sp->newArc(a->target(), (*weightMap_)(a->weight()),
                   (*inputMap_)[a->input()]);
    else
      for (Fsa::State::const_iterator a = sr->begin(); a != sr->end(); ++a)
        sp->newArc(a->target(), (*weightMap_)(a->weight()),
                   (*inputMap_)[a->input()], (*outputMap_)[a->output()]);
...
ConstLatticeRef fromFsa(Fsa::ConstAutomatonRef fsa, ConstSemiringRef semiring,
                        ScoresRef scores, ScoreId id,
  ...
  if (id == Semiring::InvalidId)
    weightMap = FsaToLattice::WeightMapRef(
        new FsaToLattice::ConstantToScoresMap(scores));
  else
    weightMap = FsaToLattice::WeightMapRef(
        new FsaToLattice::WeightToScoresMap(semiring, id, scores));
  ...
  return ConstLatticeRef(
      new FsaToLattice(fsa, semiring, weightMap, inputMap, outputMap));
}
...
ConstLatticeRef fromUnweightedFsa(Fsa::ConstAutomatonRef fsa,
                                  ...
                                  Lexicon::AlphabetId inputAlphabetId,
                                  Lexicon::AlphabetId outputAlphabetId) {
  return fromFsa(fsa, semiring, constScore, Semiring::InvalidId,
                 inputAlphabetId, outputAlphabetId);
}
// -------------------------------------------------------------------------
...
public:
  StringConverterNode(const std::string &name,
                      const Core::Configuration &config)
      : Node(name, config) {}
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    symbolMap_ = Lexicon::us()->symbolMap(
        Lexicon::us()->alphabetId(paramAlphabet(config), true));
    semiring_ = Semiring::create(select("semiring"));
    if (!semiring_)
  ...
  virtual ConstLatticeRef sendLattice(Port to) {
    if (!l_) {
      ...
      for (std::vector<Fsa::LabelId>::const_iterator it = labels_.begin();
           it != labels_.end(); ++it) {
...
const Core::ParameterString
    StringConverterNode::paramAlphabet("alphabet", "name of alphabet", "lemma");
NodeRef createStringConverterNode(const std::string &name,
                                  const Core::Configuration &config) {
