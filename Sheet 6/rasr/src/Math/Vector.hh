...
public:
  ...
  template <typename S>
  Vector(const std::vector<S> &x) : std::vector<T>(x.size()) {
...
template <typename V> std::istream &operator>>(std::istream &is, Vector<V> &v) {
  v.clear();
  std::copy(std::istream_iterator<V>(is), std::istream_iterator<V>(),
            std::back_inserter(v));
...
template <typename T, class P = VectorRangeCheckOffPolicy>
class ReferenceCountedVector : public Core::ReferenceCounted,
                               public Vector<T, P> {
public:
  ...
  template <typename S>
  ReferenceCountedVector(const std::vector<S> &x) : Vector<T, P>(x.size()) {
    for (size_t i = 0; i < size(); ++i)
      (*this)[i] = T(x[i]);
  }
...
#else
  ReferenceCountedVector(typename Base::const_iterator first,
