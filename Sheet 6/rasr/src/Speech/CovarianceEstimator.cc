...
void CovarianceEstimator::processFeature(
    Core::Ref<const Speech::Feature> feature) {
  if (feature->nStreams() and feature->mainStream()->size()) {
    accumulate(*feature->mainStream());
  }
}

void CovarianceEstimator::setFeatureDescription(
...
const Core::ParameterString
    MeanAndDiagonalCovarianceEstimator::paramVarianceFilename(
        "variance-file", "Output filename for variance vector");
const Core::ParameterString
    MeanAndDiagonalCovarianceEstimator::paramStandardDeviationFilename(
        "standard-deviation-file",
        "Output filename for standard deviation vector");
const Core::ParameterString
    MeanAndDiagonalCovarianceEstimator::paramMeanFilename(
        "mean-file", "Output filename for mean");
const Core::ParameterFloat
    MeanAndDiagonalCovarianceEstimator::paramElementThresholdMin(
        "element-threshold-min", "Min threshold for every variance entry", 0.0);
const Core::ParameterInt
    MeanAndDiagonalCovarianceEstimator::paramOutputPrecision(
        "output-precision", "Number of decimal digits in text output formats",
        20);

MeanAndDiagonalCovarianceEstimator::MeanAndDiagonalCovarianceEstimator(
    const Core::Configuration &c)
    : Component(c), Extractor(c), squareSum_(0), sum_(0), count_(0),
      finalized_(false) {}

void MeanAndDiagonalCovarianceEstimator::processFeature(
    Core::Ref<const Speech::Feature> feature) {
...
void MeanAndDiagonalCovarianceEstimator::setFeatureDescription(
    const Mm::FeatureDescription &description) {
  description.verifyNumberOfStreams(1);
  size_t d;
  description.mainStream().getValue(Mm::FeatureDescription::nameDimension, d);
...
bool MeanAndDiagonalCovarianceEstimator::write() {
  ...
  std::string standardDeviationFilename =
      paramStandardDeviationFilename(config);
  if (meanFilename != "") {
    if (Math::Module::instance().formats().write(meanFilename, mean_,
  ...
  if (varianceFilename != "") {
    if (Math::Module::instance().formats().write(varianceFilename, variance_,
                                                 paramOutputPrecision(config)))
      log("Diagonal variance vector written to '%s'.",
          varianceFilename.c_str());
    else {
      error("Failed to write diagonal variance to '%s'.",
            varianceFilename.c_str());
      success = false;
    }
  }
  if (standardDeviationFilename != "") {
    ...
    if (Math::Module::instance().formats().write(
      ...
      log("Standard deviation vector written to '%s'.",
          standardDeviationFilename.c_str());
    else {
      error("Failed to write standard deviation to '%s'.",
            standardDeviationFilename.c_str());
      success = false;
    }
  }
