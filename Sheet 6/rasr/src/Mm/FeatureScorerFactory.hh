...
public:
   ...
   */
  FeatureScorer *
  createFeatureScorer(u32 id, const Core::Configuration &c,
  ...
  /**
   * Return a MixtureSetLoader to read the model (AbstractMixtureSet or
   * derivative) for the given feature scorer id.
   */
...
protected:
  ...
  static FeatureScorer *createInstance(const Core::Configuration &c,
                                       Core::Ref<const AbstractMixtureSet> m) {
    verify(m);
