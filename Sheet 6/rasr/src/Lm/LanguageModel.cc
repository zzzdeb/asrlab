...
LanguageModelAutomaton::LanguageModelAutomaton(Bliss::LexiconRef lexicon)
    : lexicon_(lexicon), alphabet_(lexicon->syntacticTokenAlphabet()) {
  backOffLabel_ = Fsa::Epsilon;
}

LanguageModelAutomaton::LanguageModelAutomaton(
    Core::Ref<const LanguageModel> lm)
    : lexicon_(lm->lexicon()), alphabet_(lm->tokenAlphabet()) {
  backOffLabel_ = Fsa::Epsilon;
}

LanguageModel::LanguageModel(const Core::Configuration &c, Bliss::LexiconRef l)
    : Component(c), historyManager_(0), lexicon_(l), tokenInventory_(0) {
...
void LanguageModel::setTokenInventoryAndAlphabet(
    const TokenInventory *_tokenInventory,
    Fsa::ConstAlphabetRef _tokenAlphabet) {
  tokenInventory_ = _tokenInventory;
  tokenAlphabet_ = _tokenAlphabet;
}

const Bliss::SyntacticToken *
LanguageModel::getSpecialSyntacticToken(const std::string &name,
                                        bool required) const {
  ...
  if (tokenSequence.length() < 1) {
    if (required) {
      error("Empty \"%s\" lm token. "
            "Special lemma \"%s\" specifies a syntactic token sequence of "
            "length zero.",
            name.c_str(), name.c_str());
    }
    return 0;
  ...
  if (tokenSequence.length() > 1) {
    warning("Ambiguous \"%s\" lm token. "
            "Special lemma \"%s\" specifies a syntactic token sequence longer "
            "than one. "
            "Using only \"%s\".",
...
Token LanguageModel::getSpecialToken(const std::string &name,
                                     bool required) const {
  const Bliss::SyntacticToken *syntTok =
      getSpecialSyntacticToken(name, required);
  if (!syntTok)
    return InvalidToken;
  Token tok = getToken(syntTok->symbol());
  if (!tok && required)
    criticalError("Cannot find required special token \"%s\"", name.c_str());
  return tok;
}
...
void LanguageModel::createSpecialTokens() {
  ...
  if (lexicon()->specialLemma("sentence-boundary")) {
    sentenceBeginToken_ = sentenceEndToken_ =
        getSpecialToken("sentence-boundary", requiresSentenceBoundaryToken_);
  } else {
    sentenceBeginToken_ =
        getSpecialToken("sentence-begin", requiresSentenceBoundaryToken_);
    sentenceEndToken_ =
        getSpecialToken("sentence-end", requiresSentenceBoundaryToken_);
  }
...
History LanguageModel::reducedHistory(const History &h, u32) const { return h; }
...
// ===========================================================================
CompiledBatchRequest *
LanguageModel::compileBatchRequest(const BatchRequest &request,
                                   Score scale) const {
...
void LanguageModel::getBatch(const History &history,
