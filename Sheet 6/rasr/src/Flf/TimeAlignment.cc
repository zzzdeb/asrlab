...
public:
  StaticScoreLattice(ConstLatticeRef l, ConstSemiringRef semiring,
                     ScoresRef weight)
  ...
  virtual std::string describe() const {
    return Core::form(
        "static-score(%s;%s)", fsa_->describe().c_str(),
        semiring_->describe(weight_, Fsa::HintShowDetails).c_str());
  }
};
ConstLatticeRef staticScore(ConstLatticeRef l, ConstSemiringRef semiring,
                            ScoresRef weight) {
...
private:
  void distributeTimeOverUnalignedWords(
      StaticBoundaries *b,
      const std::vector<std::pair<Fsa::StateId, Fsa::LabelId> > &wordStarts)
      const {
    if (wordStarts.size() < 3)
      return;
    Time startTime = (*b)[wordStarts.back().first].time();
    Time duration = ((*b)[wordStarts.front().first].time() - startTime) /
                    (wordStarts.size() - 1);
    Core::Component::Message msg(log("Could not align:\n"));
    msg << "\t" << startTime << "\t"
        << lAlphabet_->symbol(wordStarts.back().second) << "\n";
    startTime += duration;
    for (u32 i = wordStarts.size() - 2; 0 < i; --i) {
      msg << "\t" << startTime << "\t"
          << lAlphabet_->symbol(wordStarts[i].second) << "\n";
...
public:
  ...
  void dumpStatistics() {
    if (statisticsChannel_.isOpen()) {
      ...
      statisticsChannel_ << Core::XmlFull("count",
...
protected:
  ConstLatticeRef align(ConstLatticeRef lHyp, ConstPosteriorCnRef cnRef) {
    verify((lHyp->type() == Fsa::TypeAcceptor) &&
           (Lexicon::us()->alphabetId(lHyp->getInputAlphabet()) ==
            Lexicon::LemmaAlphabetId));
    ConstSemiringRef hypSemiring = lHyp->semiring();
    ScoresRef One = hypSemiring->one();
    Core::Vector<std::pair<Fsa::LabelId, ScoresRef> > hyp(
        1, std::make_pair(Fsa::Epsilon, One));
    for (ConstStateRef sr = lHyp->getState(lHyp->initialStateId());
         sr->hasArcs(); sr = lHyp->getState(sr->begin()->target())) {
      if (sr->begin()->input() != Fsa::Epsilon) {
        hyp.push_back(
            std::make_pair(sr->begin()->input(), sr->begin()->weight()));
        hyp.push_back(std::make_pair(Fsa::Epsilon, One));
      } else {
        hyp.back().second =
            hypSemiring->extend(hyp.back().second, sr->begin()->weight());
      }
    }
    ...
    Core::Vector<Score> scores1(hyp.size() + 1, Semiring::Max),
        scores2(hyp.size() + 1, Semiring::Max);
    ...
    for (u32 t = 0; t < cnRef->size(); ++t) {
      ...
      Core::Vector<Score> &lastScores = *lastScoresPtr,
                          &currentScores = *currentScoresPtr;
      ...
      for (s32 i = 0; size_t(i) < hyp.size(); ++i) {
        ...
        PosteriorCn::Slot::const_iterator itPdf =
            std::lower_bound(pdf.begin(), pdf.end(), cnArc);
    ...
    u32 j = ((*lastScoresPtr)[hyp.size() - 1] < (*lastScoresPtr)[hyp.size()])
                ...
                : hyp.size() - 1;
    u32 lastJ = j;
    std::pair<Fsa::LabelId, ScoresRef> currentHyp = std::make_pair(
                                           Fsa::InvalidLabelId, One),
                                       lastHyp = hyp[lastJ];
  ...
  ConstLatticeRef align(ConstLatticeRef lHyp, ConstLatticeRef lRef) {
    ensure(Lexicon::us()->alphabetId(lRef->getInputAlphabet()) ==
           Lexicon::LemmaAlphabetId);
    ConstLatticeRef lMeshedRef =
        persistent(staticScore(mesh(lRef, MeshTypeTimeBoundary),
                               lHyp->semiring(), lHyp->semiring()->one()));
    ConstLatticeRef lAlignedHyp =
        best(composeSequencing(lMeshedRef, lHyp), BellmanFord);
...
const Core::ParameterBool TimeAlignmentBuilder::Internal::paramTryIntersection(
    "intersection", "try alignment by intersection", true);
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
TimeAlignmentBuilder::TimeAlignmentBuilder(const Core::Configuration &config,
                                           FwdBwdBuilderRef fbBuilder) {
  internal_ = new Internal(config, fbBuilder);
}

TimeAlignmentBuilder::~TimeAlignmentBuilder() { delete internal_; }

void TimeAlignmentBuilder::dump(std::ostream &os) const { internal_->dump(os); }

void TimeAlignmentBuilder::dumpStatistics() { internal_->dumpStatistics(); }

ConstLatticeRef TimeAlignmentBuilder::align(ConstLatticeRef lHyp,
...
TimeAlignmentBuilderRef
TimeAlignmentBuilder::create(const Core::Configuration &config,
                             FwdBwdBuilderRef fbBuilder) {
...
protected:
  virtual ConstLatticeRef filter(ConstLatticeRef lHyp) {
    ...
    if (connected(2)) {
      ...
      lRef = applyOneToOneLabelMap(projectInput(mapInput(lRef, MapToLemma)),
                                   nonWordToEpsilonMap_);
    }
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(1) && !connected(2))
      criticalError(
          "Reference fCN at port 1 or reference lattice at port 2 required.");
    nonWordToEpsilonMap_ =
        LabelMap::createNonWordToEpsilonMap(Lexicon::LemmaAlphabetId);
    aligner_ = TimeAlignmentBuilder::create(config);
    aligner_->dump(log());
  }
  virtual void finalize() { aligner_->dumpStatistics(); }
};
NodeRef createTimeAlignmentNode(const std::string &name,
