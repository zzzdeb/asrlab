 ...
 * remarks/ideas:
 *
 * - how to handle uninitialized object space inside blocks? use functions
 * defined in <memory>?
 * - what about classes that allocate memory for member variables?
 * - fill unused and uninitialized memory with 0's to achieve better compression
 * ratios
 */
...
private:
  ...
  private:
    ...
    lzo_align_t __LZO_MMODEL
        workmem[(LZO1X_1_MEM_COMPRESS + (sizeof(lzo_align_t) - 1)) /
                sizeof(lzo_align_t)];
#endif
  public:
    Block()
        : objects_(0), compressedObjects_(0), size_(0), allocated_(0),
          compressedSize_(0) {}
    Block(const Block &b) {
      objects_ = b.objects_;
      b.objects_ = 0;
    ...
    double compress() {
      ...
      int ok = ::compress2(
          reinterpret_cast<Bytef *>(compressedObjects_), &compressedSize_,
          reinterpret_cast<const Bytef *>(objects_), size_, Z_BEST_SPEED);
      require(ok == Z_OK);
#endif
#ifdef _WITH_MINILZO
      compressedSize_ =
          int(sizeof(T) * size_ + (sizeof(T) * size_) / 64 + 16 + 3);
      compressedObjects_ = malloc(compressedSize_);
      int ok = ::lzo1x_1_compress(
          reinterpret_cast<const lzo_byte *>(objects_), size_,
          reinterpret_cast<lzo_byte *>(compressedObjects_), &compressedSize_,
          workmem);
      require(ok == LZO_E_OK);
#endif
      ...
      ::free(objects_);
      objects_ = 0;
      // std::cout << "compressed " << size_ * sizeof(T) << " to " <<
      // compressedSize_ << std::endl;
    ...
    void uncompress() {
      if (isCompressed()) {
        ...
        ::uncompress(reinterpret_cast<Bytef *>(objects_), &size_,
                     reinterpret_cast<const Bytef *>(compressedObjects_),
                     compressedSize_);
#endif
#ifdef _WITH_MINILZO
        objects_ = (T *)malloc(size_t(size_));
        ::lzo1x_decompress(
            reinterpret_cast<const lzo_byte *>(compressedObjects_),
...
public:
  ...
  public:
    Pointer() : manager_(0) {}
    Pointer(CompressedMemoryManager *manager, BlockNumber block,
            ObjectNumber firstObject, ObjectNumber nObjects)
        : manager_(manager), block_(block), firstObject_(firstObject),
          nObjects_(nObjects) {}
