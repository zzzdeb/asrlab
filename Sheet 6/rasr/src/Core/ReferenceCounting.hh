...
template <class T /* -> ReferenceCounted */> class Ref {
private:
  typedef T Object;
  Object *object_;
...
public:
   ...
   */
  Ref(const Ref &r) : object_(r.object_) { ++object_->referenceCount_; }
  /**
  ...
  /** Test for identity */
  bool operator==(const Ref &r) const { return object_ == r.object_; }
...
template <class T> inline Ref<T> ref(T *o) { return Ref<T>(o); }
...
private:
  ...
  void release() { object_->releaseWeakReference(this); }
  void acquire() { object_->acquireWeakReference(this); }
  void set(Object *o) {
    release();
    object_ = o;
    acquire();
  }
  virtual void invalidate() { object_ = sentinel(); }
  friend class ReferenceManager;
...
public:
   ...
   * using the template one.
   */
  WeakRef(const WeakRef &r) : object_(r.object_) { acquire(); }
  ...
  template <class S> WeakRef(const Ref<S> &r) : object_(r._get()) { acquire(); }
  ...
  /** Test for identity */
  bool operator==(const WeakRef &r) const { return object_ == r.object_; }
  template <class S> bool operator==(const WeakRef<S> &r) const {
    return object_ == r._get();
  }
  template <class S> bool operator==(const Ref<S> &r) const {
