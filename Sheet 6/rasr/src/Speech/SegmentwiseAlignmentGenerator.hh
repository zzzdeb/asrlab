...
protected:
  ...
  createLatticeArchive(const Core::Configuration &config);
...
public:
  ...
  // if no orthography is given, the orthography form Bliss::SpeechSegment * is
  // used
  const Alignment *getAlignment(const std::string &orthography = "");
  Lattice::ConstWordLatticeRef
  getWordLattice(const std::string &orthography = "");
};
