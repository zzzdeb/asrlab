...
public:
  ...
  typedef enum { CnFormatText, CnFormatXml } CnFormat;
...
/*
  ...
  file is specified
  - writes fsa or one-dimensional lattice to port 0, id(path as found in file
  minus extension) to port 1, reference (if to be loaded) to port 2
*/
...
/*
  ...
  - for a request: cut time-overlapping sequence from ctm-file and build linear
  lattice
