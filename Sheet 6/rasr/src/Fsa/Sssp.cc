...
size_t countPaths(ConstAutomatonRef f) { return Ftl::countPaths<Automaton>(f); }
...
void StatePotentials64DfsState::finishState(StateId s) {
  ...
  f64 sum = (sp->isFinal() && minimumIterator != sp->end())
                ? exp(minimumScore - f32(sp->weight_))
                : 0;
  for (State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
...
protected:
  ...
  f32 logPosterior(StateId s, const Arc &a) const {
    f64 tmp = forwardPotentials_[s] + f32(a.weight()) +
              backwardPotentials_[a.target()] + totalInv_;
...
public:
  ...
  //	Weight totalInv() const { return Weight(totalInv_ > Core::Type<f32>::min
  //? totalInv_ : Core::Type<f32>::min); }
};

Posterior64Automaton::Posterior64Automaton(ConstAutomatonRef fsa,
                                           bool normalize, s32 tol)
    : ModifyAutomaton(fsa) {
  ConstAutomatonRef transposed = transpose(fsa_);
  sssp64(transposed, forwardPotentials_);
  sssp64(fsa_, backwardPotentials_);
  totalInv_ = normalize ? -backwardPotentials_[fsa_->initialStateId()]
                        : fsa_->semiring()->one();
...
ConstAutomatonRef posterior64(ConstAutomatonRef fsa, Weight &totalInv,
                              s32 tol) {
  Posterior64Automaton *f = new Posterior64Automaton(fsa, true, tol);
  totalInv =
      Fsa::Weight(f->totalInv() > Core::Type<f32>::min ? f->totalInv()
                                                       : Core::Type<f32>::min);
...
public:
  StatePotentialsEDfsState(ConstAutomatonRef, ConstAutomatonRef,
                           StatePotentials64 &, StatePotentials64 &);
...
StatePotentialsEDfsState::StatePotentialsEDfsState(
    ...
    : StatePotentials64DfsState(transposed, potentials), risk_(risk),
...
void ssspE(ConstAutomatonRef f, ConstAutomatonRef r,
           StatePotentials64 &potentials, StatePotentials64 &generalized) {
  require(f->semiring() == LogSemiring);
  require(hasProperties(f, PropertyAcyclic));
...
private:
  f32 risk(StateId s, const Arc &a) const {
    f64 tmp = generalizedForwardPotentials_[s] + f32(a.weight()) +
              generalizedBackwardPotentials_[a.target()] - normalization_;
    return tmp < Core::Type<f32>::max ? tmp : Core::Type<f32>::max;
  }
...
public:
  PosteriorEAutomaton(ConstAutomatonRef, ConstAutomatonRef, bool vNormalized,
                      s32 tol);
...
PosteriorEAutomaton::PosteriorEAutomaton(ConstAutomatonRef f,
                                         ConstAutomatonRef r, bool vNormalized,
                                         s32 tol)
    : Posterior64Automaton(f), r_(r), normalization_(0) {
  ConstAutomatonRef transposed = transpose(fsa_);
  ssspE(transposed, transpose(r_), forwardPotentials_,
        generalizedForwardPotentials_);
...
ConstAutomatonRef posteriorE(ConstAutomatonRef f, ConstAutomatonRef r,
                             Weight &expectation, bool vNormalized, s32 tol) {
...
Weight expectation(ConstAutomatonRef f, ConstAutomatonRef r) {
  ...
  StatePotentialsEDfsState backward(f, r, backwardPotentials,
                                    generalizedBackwardPotentials);
  backward.dfs();
