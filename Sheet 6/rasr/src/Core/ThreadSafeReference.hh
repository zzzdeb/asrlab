...
/** Base class for reference-counted objects used in a multithreaded
 * environment.
 *
 * To enable reference counting for a class it need to inhert this
 * class publically.
 *
 * Remark: ThreadSafeReferenceCounted is by intention not derived from
 * ReferenceCounted to forbit access of thread safe reference count by a not
 * thread safe Core::Ref object.
 */
...
public:
  ...
  } // calling mutex_.lock() might be useful for debugging
 ...
 */
template <class T /* -> ThreadSafeReferenceCounted */> class TsRef {
protected:
  typedef T Object;
  Object *object_;
...
public:
  ...
  explicit TsRef(Object *o) : object_(o) {
    require_(object_);
    reference();
  }
  /** Copy Constructor for the Type Ref<T>
   *  @warning: Implementaion of this constructor is necesseary although the
   * template copy constructor implements exactly the same. If this constructor
   * is not implemented, the compiler creates a default copy constructor instead
   * of using the template one.
   */
   ...
   *  For all classes S which are not derived from T, this constructor will not
   * compile.
   */
  template <class S> TsRef(const TsRef<S> &r) : object_(r._get()) {
    reference();
  }
  ~TsRef() { unreference(); }
  ...
  /** Test for identity */
  bool operator==(const TsRef &r) const { return object_ == r.object_; }
  ...
  operator bool() const { return Object::isNotSentinel(object_); }
  /** Test for voidness. */
  bool operator!() const { return Object::isSentinel(object_); }
   ...
   * Use with caution!
   */
  Object *get() const { return Object::isNotSentinel(object_) ? object_ : 0; }
...
template <class T> inline TsRef<T> tsRef(T *o) { return TsRef<T>(o); }
