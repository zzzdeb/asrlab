...
public:
  ...
  public:
    Node(Node *father = 0, Id id = InvalidId, Information *info = 0)
        : id_(id), info_(info), father_(father), leftChild_(0), rightChild_(0) {
    }
    ~Node() {
      delete leftChild_;
      delete rightChild_;
    ...
    template <class InformationType> const InformationType &info() const {
      require((info_));
...
public:
  BinaryTree(const Core::Configuration &config, Node *root = 0);
  virtual ~BinaryTree() { reset(); }
  ...
  u32 nNodes() const { return internalNodeN_ + leafN_; }
  u32 nLeaves() const { return leafN_; }
  ...
  virtual void writeXml(Core::XmlWriter &xml,
                        const std::string &name = "binary-tree") const;
};
