...
public:
  ...
  */
  virtual void build(ConstLatticeRef l,
                     ConstFwdBwdRef fb = ConstFwdBwdRef()) = 0;
  /*
  ...
  */
  virtual ConstConfusionNetworkRef
  getCn(ScoreId posteriorId = Semiring::InvalidId,
        bool mapping = false) const = 0;
  /*
    Get a CN, i.e. the slots are normalized, and
    get the CN decoding result.
  */
  virtual std::pair<ConstConfusionNetworkRef, ConstLatticeRef>
  getNormalizedCn(ScoreId confidenceId = Semiring::InvalidId,
                  bool mapping = false) const = 0;
...
ConstConfusionNetworkRef normalizeCn(ConstConfusionNetworkRef cn,
                                     ScoreId posterirorId);
bool isNormalizeCn(ConstConfusionNetworkRef cn, ScoreId posteriorId);

/**
 * Decodes a normalized CN;
 * if the CN is not normalized, a valid posteriroId is required and the CN is
 *normalized beforehand.
 **/
ConstLatticeRef decodeCn(ConstConfusionNetworkRef cn,
                         ScoreId posterirorId = Semiring::InvalidId);
...
struct CnFeatureConfiguration {
  ...
  ScoreId cnPosteriorId; // required for posterior distribution based features
                         // like confidence, score, and entropy
  // composition with CN
  bool compose;
 ...
 * Optionally, perform the "composition" of the lattice and the CN derived from
 *the lattice: each path in the resulting lattice has the same length
 **/
