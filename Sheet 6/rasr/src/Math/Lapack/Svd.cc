...
s32 pseudoInvert(DoubleMatrix &result, const DoubleMatrix &A) {
  require(A.nRows() ==
          A.nColumns()); // other cases possible but not implemeted yet
...
s32 solveLinearLeastSquares(DoubleVector &result, const DoubleMatrix &A,
...
s32 solveLinearLeastSquares(DoubleMatrix &result, const DoubleMatrix &A,
...
s32 svd(DoubleMatrix &U, DoubleVector &W, DoubleMatrix &V,
        const DoubleMatrix &A) { // returns V, not(!) VT
  ...
  if (m == n) {
    for (u16 rowIdx = 0; rowIdx < m; rowIdx++) {
      for (u16 columnIdx = 0; columnIdx < m; columnIdx++) {
        ...
        V[columnIdx][rowIdx] =
            vt[columnIdx * ldvt + rowIdx]; // returns V, not VT
      }
      W[rowIdx] = s[rowIdx];
    }
  } else {
