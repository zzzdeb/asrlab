 ...
 *   not-empty: if at least one of the delays contain a frame. This condition
 * ensures that each input frame can be retrieved at all delays.
 *   present-not-empty: if the delay=0 contains a frame. This condition ensures
 * that the number of input frames is equal to the number of output frames.
...
protected:
   ...
   *  If sliding window does not contain an element at position @param
   * relativeIndex, return is an object created by marginPolicy_.
   */
