...
/** MeanEnergyVectorNormalization: divides each element by square of frame mean
 * energy
 *
...
template <class T> class MeanEnergyVectorNormalization {
public:
  typedef T Value;
...
template <class T> class EnergyVectorNormalization {
public:
  typedef T Value;
  ...
  void operator()(std::vector<Value> &v) {
    ...
    Value squareEnergy =
        sqrt(std::inner_product(v.begin(), v.end(), v.begin(), 0.0));
...
/** AmplitudeSpectrumEnergyVectorNormalization: divides each element by square
 * of frame energy calculated form (half) amplitude spectrum
 *
 *  square of frame energy =
 *    ( 1 / N * (2 * sum_{omega=1}^{N/2 - 1} (x^2(omega)) + x^2(0) + x^2(N/2)) )
 * ^{1/2} where N is length of FFT
 *
 *
 * Applied normally to amplitude like values.
 */

template <class T> class AmplitudeSpectrumEnergyVectorNormalization {
public:
  typedef T Value;
  ...
  void operator()(std::vector<Value> &v) {
    ...
    Value squareEnergy =
        sqrt((v.front() * v.front() + v.back() * v.back() +
              2 * std::inner_product(v.begin() + 1, v.end() - 1, v.begin() + 1,
                                     0.0)) /
             T((v.size() - 1) * 2));
...
template <class T> class MeanVectorNormalization {
public:
  typedef T Value;
...
template <class T> class VarianceVectorNormalization {
public:
  typedef T Value;
...
template <class T> class MaximumVectorNormalization {
public:
  typedef T Value;
