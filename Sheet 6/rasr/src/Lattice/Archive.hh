...
public:
  ...
  static ArchiveReader *openForReading(const Core::Configuration &,
                                       Bliss::LexiconRef);

  static ArchiveWriter *openForWriting(const Core::Configuration &,
                                       Bliss::LexiconRef);
};
...
protected:
  ...
  f64 amScale() { return paramAmScale(config, paramAmScale(latticeConfig_)); }
  f64 lmScale() { return paramLmScale(config, paramLmScale(latticeConfig_)); }
  f64 penaltyScale() {
    return paramPenaltyScale(config, paramPenaltyScale(latticeConfig_));
  }
  f64 pronunciationScale() {
    return paramPronunciationScale(config,
                                   paramPronunciationScale(latticeConfig_));
  }

public:
  ArchiveReader(const Core::Configuration &, const std::string &pathname,
                Bliss::LexiconRef);
...
public:
  ArchiveWriter(const Core::Configuration &, const std::string &pathname,
                Bliss::LexiconRef);
  virtual ~ArchiveWriter() {}

  virtual void store(const std::string &id,
                     ConstWordLatticeRef wordLattice) = 0;
};
