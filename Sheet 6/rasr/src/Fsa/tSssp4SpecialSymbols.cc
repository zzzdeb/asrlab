...
template <> Fsa::Weight convertWeight<Fsa::Weight, Fsa::Weight>(Fsa::Weight w) {
  return w;
}
...
template <>
Fsa::ConstSemiring64Ref
convertSemiring<Fsa::ConstSemiring64Ref>(Fsa::ConstSemiringRef semiring) {
...
struct byInputAndOutput
    : public std::binary_function<_leftArc, _rightArc, bool> {
 ...
 */
template <class _Automaton, class _Semiring = typename _Automaton::Semiring,
          class _Weight = typename _Automaton::Weight>
class SsspBackward4SpecialSymbols : public DfsState<_Automaton> {
  typedef SsspBackward4SpecialSymbols<_Automaton, _Semiring, _Weight>
      _SsspBackward4SpecialSymbols;
...
public:
  SsspBackward4SpecialSymbols(Fsa::ConstAutomatonRef f)
      : Precursor(sort<_Automaton>(f, Fsa::SortTypeByInputAndOutput)),
        semiring_(sssp4SpecialSymbolsHelper::convertSemiring<_ConstSemiringRef>(
            f->semiring())) {
...
private:
  struct AddArc {
    inline static void
    regularArcWithoutDirectHits(const _Arc *arc, _Accumulator *collector,
                                _SsspBackward4SpecialSymbols *owner) {
      Fsa::StateId ts = arc->target();
      collector->feed(owner->semiring_->extend(
          sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
              arc->weight()),
          owner->potential_[ts]));
    }

    inline static void
    regularArcWithDirectHits(const _Arc *arc, DirectHits &directHits,
                             typename DirectHits::iterator &darc,
                             _Accumulator *collector,
                             _SsspBackward4SpecialSymbols *owner) {
    ...
    inline static _Weight failArc(const _Arc *arc, DirectHits &directHits,
                                  _Accumulator *collector,
                                  const bool firstLevel,
                                  _SsspBackward4SpecialSymbols *owner) {
      Fsa::StateId ts = arc->target();
      _Weight failWeight;
      failWeight = owner->semiring_->extend(
          sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
              arc->weight()),
          owner->processState<AddArc>(ts, directHits));
    ...
    inline static _Weight failArc(const _Arc *arc, DirectHits &directHits,
      ...
      if (owner->semiring_->collect(currentGet, failWeight) != currentGet)
        failWeight = owner->semiring_->extend(
            sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
                arc->weight()),
            owner->processState<AddArcTropical>(ts, directHits));
      collector->feed(failWeight);
  ...
  struct AddArcNegCollect : AddArc {
    ...
    inline static _Weight failArc(const _Arc *arc, DirectHits &directHits,
      ...
      if (firstLevel) {
        ...
        failWeight = owner->semiring_->invCollect(
            allFailWeights,
            owner->processState<AddArcNegCollect>(ts, directHits));
        failWeight = owner->semiring_->extend(
            sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
                arc->weight()),
            failWeight);
  ...
  _Weight processState(const Fsa::StateId s, DirectHits &directHits) {
    ...
    if (not sssp4SpecialSymbolsHelper::stateHasFailArc<_Automaton>(state))
      if (oldDirectHitsEmpty)
        for (typename _State::const_iterator arc = state->begin();
             arc != state->end(); ++arc) {
        ...
        for (typename _State::const_iterator arc = state->begin();
             arc != state->end(); ++arc)
          _AddArc::regularArcWithDirectHits(&(*arc), directHits, darc,
                                            collector, this);
      }
    else {
      ...
      for (typename _State::const_iterator arc = state->begin();
           arc != state->end(); ++arc) {
        if (noNoneFailArc or not(arc->output() == Fsa::Failure or
                                 arc->input() == Fsa::Failure)) {
          noNoneFailArc = false;
          ...
          verify(arc->output() == arc->input()); // assume that we only have
                                                 // arcs with fail on both sides
          _Weight failWeight = _AddArc::failArc(&(*arc), directHits, collector,
                                                oldDirectHitsEmpty, this);
          // std::cout << "processState(" << s << ") oldDirectHitsEmpty=" <<
          // oldDirectHitsEmpty << " failWeight=" << (f32)failWeight <<
          // std::endl;
    ...
    if (state->isFinal()) {
      collector->feed(semiring_->extend(
          sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
              state->weight()),
          semiring_->one()));
    }
...
protected:
  void finishState(Fsa::StateId s) {
    ...
    _Weight potential;
    if (sssp4SpecialSymbolsHelper::isTropicalSemiring<_ConstSemiringRef>(
            semiring_))
    ...
    // std::cout << "finishState(" << s << ") potential=" <<
    // sssp64Helper::weight2str(potential) << std::endl;
...
class SsspForward4SpecialSymbols {
  typedef SsspForward4SpecialSymbols<_Automaton, _Semiring, _Weight>
      _SsspForward4SpecialSymbols;
...
public:
  ...
  public:
    ForwardPotential(_ConstSemiringRef semiring)
        : semiring_(semiring), forwardPotential_(semiring_->zero()),
          forwardAccumulator_(NULL), forwardAccumulators_(NULL) {}

    ForwardPotential(_ConstSemiringRef semiring, _Weight weight)
        : semiring_(semiring), forwardPotential_(weight),
          forwardAccumulator_(NULL), forwardAccumulators_(NULL) {}
    ...
    ForwardPotential(const ForwardPotential &fp)
        : semiring_(fp.semiring_), forwardPotential_(fp.forwardPotential_),
          forwardAccumulator_(NULL), forwardAccumulators_(NULL) {
    ...
    ~ForwardPotential() {
      ...
      if (forwardAccumulators_ != NULL) {
        for (forwardAccumulatorsPos_ = forwardAccumulators_->begin();
  ...
  private:
    ...
    bool walk2(const _Arc &arc, const Sorter &sorter) const {
      ...
      while ((forwardAccumulatorsPos_ != forwardAccumulators_->begin()) and
             (RSorter::cmp(arc, forwardAccumulatorsPos_->first))) {
        --forwardAccumulatorsPos_;
      }
      // while forwardPotentialsPos < arc
  ...
  public:
    ...
    void feedForwardPotential(const _Arc &arc, const Sorter &sorter,
                              const _Weight &w) {
      if (forwardAccumulators_ == NULL) {
        forwardAccumulators_ = new _ForwardAccumulators();
        forwardAccumulatorsPos_ = forwardAccumulators_->begin();
        _FPArc forwardArc(arc.target(), semiring_->zero(), arc.input(),
                          arc.output());
        _ArcAccumulatorPair pair(forwardArc, semiring_->getCollector(w));
        forwardAccumulatorsPos_ =
            forwardAccumulators_->insert(forwardAccumulatorsPos_, pair);
      } else {
        if (walk2(arc, sorter)) {
    ...
    _Weight getForwardPotential(const _Arc &arc, const Sorter &sorter) const {
      ...
      if ((forwardAccumulators_ != NULL) and (walk2(arc, sorter))) {
        ...
        if (not semiring_->hasInvCollect())
          return semiring_->collect(forwardPotential_,
                                    forwardAccumulatorsPos_->first.weight());
        else
          return semiring_->invCollect(forwardPotential_,
                                       forwardAccumulatorsPos_->first.weight());
      } else {
...
private:
  ...
  inline const ForwardPotential &
  getFinalizedForwardPotential(const std::size_t &index) {
...
private:
  struct AddArc {
    inline static void regularArcWithoutDirectHits(
        const _Arc *arc, const _Weight &sourceForwardPotential,
        ForwardPotential &fpot, _SsspForward4SpecialSymbols *owner) {
      Fsa::StateId ts = arc->target();
      owner->getForwardPotential(ts).feedForwardPotential(
          owner->semiring_->extend(
              sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,
                                                       _Weight>(arc->weight()),
              sourceForwardPotential));
    }
    ...
    inline static void failArc(const _Arc *arc, DirectHits &directHits,
                               const _Weight &sourceForwardPotential,
                               const bool firstLevel,
                               _SsspForward4SpecialSymbols *owner) {
      Fsa::StateId ts = arc->target();
      owner->processState<AddArc>(
          owner->fsa_->getState(ts), directHits,
          owner->semiring_->extend(
              sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight,
                                                       _Weight>(arc->weight()),
              sourceForwardPotential));
  ...
  struct AddArcNegCollect {
    inline static void regularArcWithoutDirectHits(
        ...
        ForwardPotential &fpot, _SsspForward4SpecialSymbols *owner) {
      Fsa::StateId ts = arc->target();
      _Weight projection = owner->semiring_->extend(
          sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
              arc->weight()),
          fpot.getForwardPotential(*arc, owner->sorter_));
    ...
    inline static void failArc(const _Arc *arc, DirectHits &directHits,
      ...
      Fsa::StateId ts = arc->target();
      if (firstLevel) {
        _Weight totalFailWeight = owner->semiring_->extend(
            sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
                arc->weight()),
            sourceForwardPotential);
        owner->getForwardPotential(ts).feedForwardPotential(totalFailWeight);
        owner->processState<AddArcNegCollect>(owner->fsa_->getState(ts),
                                              directHits, totalFailWeight);
      } // else stop recursion
    }
  };

  template <class _AddArc>
  void processState(_ConstStateRef state, DirectHits &directHits,
                    _Weight sourceForwardPotential) {
    ...
    if (not sssp4SpecialSymbolsHelper::stateHasFailArc<_Automaton>(state))
      if (oldDirectHitsEmpty)
        for (typename _State::const_iterator arc = state->begin();
             arc != state->end(); ++arc)
          _AddArc::regularArcWithoutDirectHits(&(*arc), sourceForwardPotential,
                                               fpot, this);
      else {
        typename DirectHits::iterator darc = directHits.begin();
        for (typename _State::const_iterator arc = state->begin();
             arc != state->end(); ++arc)
          _AddArc::regularArcWithDirectHits(&(*arc), directHits, darc,
                                            sourceForwardPotential, fpot, this);
      }
      ...
      for (typename _State::const_iterator arc = state->begin();
        ...
        if (noNoneFailArc or not(arc->output() == Fsa::Failure or
          ...
          if (oldDirectHitsEmpty) {
            _AddArc::regularArcWithoutDirectHits(
                &(*arc), sourceForwardPotential, fpot, this);
            directHits.push_back(&(*arc));
          } else {
            _AddArc::regularArcWithDirectHits(
                &(*arc), directHits, darc, sourceForwardPotential, fpot, this);
          }
        } else {
          verify(arc->output() == arc->input()); // assume that we only have
                                                 // arcs with fail on both sides
          _AddArc::failArc(&(*arc), directHits, sourceForwardPotential,
                           oldDirectHitsEmpty, this);
        } // end else Failure
      }   // end arc
    }
  ...
  void finishStates(const _Stack states) {
    ...
    this->getForwardPotential(initialStateId)
        .feedForwardPotential(semiring_->one());
    ...
    for (_Stack::const_iterator s = states.begin(); s != states.end(); ++s) {
      ...
      if (state->isFinal()) {
        // std::cout << "Forward: final state " << *s << " fStatePot=" <<
        // (f32)fStatePot << " state->weight()=" << (f32)state->weight() <<
        // std::endl; std::cout << " before fwdFlowAcc->get()=" <<
        // (f32)fwdFlowAcc->get() << " feed(" <<
        // (f32)semiring_->extend(fStatePot, state->weight()) << ")" <<
        // std::endl;
        fwdFlowAcc->feed(semiring_->extend(
            fStatePot,
            sssp4SpecialSymbolsHelper::convertWeight<_AutomataWeight, _Weight>(
                state->weight())));
        // std::cout << " after fwdFlowAcc->get()=" << (f32)fwdFlowAcc->get() <<
        // std::endl;
      }
    }
...
public:
  ...
  ~SsspForward4SpecialSymbols() {
    for (typename _StatePotentialsForwardFail::iterator p =
             forwardPotentials_.begin();
         p != forwardPotentials_.end(); ++p)
      delete *p;
  }
...
template <class _Automaton,
          class _SsspForward4SpecialSymbols =
              SsspForward4SpecialSymbols<_Automaton>,
          class _SsspBackward4SpecialSymbols =
              SsspBackward4SpecialSymbols<_Automaton> >
...
public:
  ...
  typedef
      typename _SsspForward4SpecialSymbols::ForwardPotential _ForwardPotential;
  typedef std::list<const _Arc *> DirectHits;
  typedef byInputAndOutput<_Automaton> Sorter;
...
public:
  PosteriorAutomaton4SpecialSymbols(_ConstAutomatonRef f, s32 tol)
      : Precursor(sort<_Automaton>(f, Fsa::SortTypeByInputAndOutput)),
        ...
        ssspForward4SpecialSymbols_(
            this->fsa_, ssspBackward4SpecialSymbols_.getProcessedStates()),
        totalInv_(this->semiring()->invert(
            sssp4SpecialSymbolsHelper::convertWeight<_SsspWeight, _Weight>(
                ssspBackward4SpecialSymbols_.totalFlow()))) {
    require(f->hasProperty(Fsa::PropertyAcyclic));
    const _Weight fwdFlow =
        sssp4SpecialSymbolsHelper::convertWeight<_SsspWeight, _Weight>(
            ssspForward4SpecialSymbols_.totalFlow());
    const _Weight bwdFlow =
        sssp4SpecialSymbolsHelper::convertWeight<_SsspWeight, _Weight>(
            ssspBackward4SpecialSymbols_.totalFlow());
  ...
  PosteriorAutomaton4SpecialSymbols(_ConstAutomatonRef f)
      : Precursor(sort<_Automaton>(f, Fsa::SortTypeByInputAndOutput)),
        ...
        ssspForward4SpecialSymbols_(
            this->fsa_, ssspBackward4SpecialSymbols_.getProcessedStates()),
        totalInv_(
            this->semiring()->invert(ssspBackward4SpecialSymbols_.getPotential(
                this->fsa_->initialStateId()))) {
    require(f->hasProperty(Fsa::PropertyAcyclic));
  }

  const _Weight &totalInv() const { return this->totalInv_; }

  virtual void modifyState(_State *sp) const {
    Fsa::StateId s = sp->id();
    // std::cout << "State s=" << s << std::endl;
    bool noNoneFailArc = true;
    const _ForwardPotential &fpot(
        ssspForward4SpecialSymbols_.forwardPotential(s));

    for (typename _State::iterator a = sp->begin(); a != sp->end(); ++a) {
      Fsa::StateId ts = a->target();
      // std::cout << "Arc " << a->input() << ":" << a->output() << "->" << ts
      // << "/" << (f32)a->weight() << ": ";
      _Weight forwardPot = fpot.getForwardPotential(*a, sorter_);
      _Weight backwardPot;
      if (noNoneFailArc or
          not(a->output() == Fsa::Failure or a->input() == Fsa::Failure)) {
      ...
      // std::cout << "forward: " << (f32)forwardPot << " backward: " <<
      // (f32)backwardPot << " totalInv: " << (f32)totalInv_ << " result: " <<
      // (f32)a->weight_ << std::endl;
      delete extender;
    }
  }
...
posterior4SpecialSymbols(typename _Automaton::ConstRef f) {
  return typename _Automaton::ConstRef(
      new PosteriorAutomaton4SpecialSymbols<_Automaton>(f));
}
...
posterior4SpecialSymbols(typename _Automaton::ConstRef f,
  ...
  typedef PosteriorAutomaton4SpecialSymbols<_Automaton>
      _PosteriorAutomaton4SpecialSymbols;
  _PosteriorAutomaton4SpecialSymbols *result =
      new PosteriorAutomaton4SpecialSymbols<_Automaton>(f, tol);
...
posterior4SpecialSymbols(typename _Automaton::ConstRef f,
                         typename _Automaton::Weight &totalInv, s32 tol) {
  typedef SsspForward4SpecialSymbols<_Automaton, _SsspSemiring, _SsspWeight>
      _SsspForward4SpecialSymbols;
  typedef SsspBackward4SpecialSymbols<_Automaton, _SsspSemiring, _SsspWeight>
      _SsspBackward4SpecialSymbols;
  typedef PosteriorAutomaton4SpecialSymbols<
      _Automaton, _SsspForward4SpecialSymbols, _SsspBackward4SpecialSymbols>
      _PosteriorAutomaton4SpecialSymbols;
  _PosteriorAutomaton4SpecialSymbols *result =
      new _PosteriorAutomaton4SpecialSymbols(f, tol);
  totalInv = result->totalInv();
...
private:
  bool directHistsHasArc(typename _State::const_iterator &arc,
                         DirectHits &directHits,
                         typename DirectHits::iterator &darc) const {
    // while: darc->output < farc->output
  ...
  bool getBestArc(typename _State::const_iterator &bestArc, Fsa::StateId s,
                  DirectHits &directHits) const {
    _ConstStateRef _sp = Precursor::fsa_->getState(s);
    bestArc = _sp->end();
    _Weight minWeight = Precursor::fsa_->semiring()->max();
    bool oldDirectHitsEmpty = directHits.empty();
    bool hasFailArc =
        sssp4SpecialSymbolsHelper::stateHasFailArc<_Automaton>(_sp);
    typename DirectHits::iterator darc = directHits.begin();
    bool noNoneFailArc = true;

    for (typename _State::const_iterator a = _sp->begin(); a != _sp->end();
         ++a) {
      Fsa::StateId ts = a->target();
      if (ts == s)
        continue;
      if (a->output() == Fsa::Failure or a->input() == Fsa::Failure) {
        verify(a->output() == a->input()); // assume that we only have arcs with
                                           // fail on both sides
        _Weight potential;
        if (noNoneFailArc)
        ...
        _Weight w = Precursor::fsa_->semiring()->extend(
            a->weight(), ssspBackward4SpecialSymbols_.getPotential(ts));
        if (Precursor::fsa_->semiring()->compare(w, minWeight) < 0) {
          minWeight = w;
          bestArc = a;
        }
...
public:
  /**
   * \warning best4SpecialSymbols() does not work when there are non-trivial
   * zero-weight loops.
   */
...
best4SpecialSymbols(typename _Automaton::ConstRef f) {
  return typename _Automaton::ConstRef(
      new BestAutomaton4SpecialSymbols<_Automaton>(f));
}
