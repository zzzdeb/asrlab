...
StringTokenizer::Iterator::Iterator() : parent_(), begin_(0), end_(0) {}
...
inline StringTokenizer::Iterator::size_type
StringTokenizer::Iterator::findStart(size_type begin) const {
  return parent_->str_.find_first_not_of(parent_->delim_, begin);
}

inline StringTokenizer::Iterator::size_type
StringTokenizer::Iterator::findNext(size_type begin) const {
...
std::string StringTokenizer::Iterator::operator*() const {
  std::string result(parent_->str_, begin_,
                     std::min(parent_->str_.size() - begin_, (end_ - begin_)));
  if (parent_->trim_)
