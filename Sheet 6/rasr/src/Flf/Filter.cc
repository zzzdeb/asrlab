...
// -------------------------------------------------------------------------
template <class ArcFilter> class ArcFilterLattice : public SlaveLattice {
...
public:
  ...
  virtual std::string describe() const {
    return Core::form("filter(%s,%s)", fsa_->describe().c_str(),
...
struct InputFilter {
  ...
  bool operator()(const Arc &a) const { return a.input() == input; }
...
ConstLatticeRef filterByInput(ConstLatticeRef l, Fsa::LabelId input) {
  return ConstLatticeRef(new ArcFilterLattice<InputFilter>(
      l, InputFilter(l->getInputAlphabet(), input)));
}
...
struct OutputFilter {
  ...
  bool operator()(const Arc &a) const { return a.output() == output; }
  std::string describe() const {
...
ConstLatticeRef filterByOutput(ConstLatticeRef l, Fsa::LabelId output) {
  return ConstLatticeRef(new ArcFilterLattice<OutputFilter>(
      l, OutputFilter(l->getOutputAlphabet(), output)));
}
  ...
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    ...
    if (!input_.empty()) {
      ...
      if (inputLabel != Fsa::InvalidLabelId)
        ...
        criticalError(
            "Label \"%s\" not in intput \"%s\" alphabet.", input_.c_str(),
            Lexicon::us()
    ...
    if (!output_.empty()) {
      ...
      if (outputLabel != Fsa::InvalidLabelId)
        ...
        criticalError("Label \"%s\" not in intput \"%s\" alphabet.",
                      output_.c_str(),
                      Lexicon::us()
                          ->alphabetName(
...
const Core::ParameterString FilterArcNode::paramInput("input", "input");
const Core::ParameterString FilterArcNode::paramOutput("output", "output");
