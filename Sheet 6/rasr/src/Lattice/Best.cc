...
NBestListExtractor::NBestListExtractor()
    : targetNHypotheses_(1), minThreshold_(100),
      maxThreshold_(Core::Type<f32>::max), thresholdIncrement_(10),
      workOnOutput_(true),
...
void NBestListExtractor::initialize(Bliss::LexiconRef lexicon) {
  ...
  lemmaToEval_ = Fsa::multiply(
      lexicon->createLemmaToEvaluationTokenTransducer(), Fsa::Weight(0));
  lemmaPronToLemma_ = Fsa::multiply(
      lexicon->createLemmaPronunciationToLemmaTransducer(), Fsa::Weight(0));
}

Fsa::ConstAutomatonRef
NBestListExtractor::mapLemmaPronToEval(Fsa::ConstAutomatonRef fsa) const {
  return Fsa::cache(Fsa::projectOutput(Fsa::composeMatching(
      Fsa::composeMatching(fsa, lemmaPronToLemma_), lemmaToEval_)));
}

Fsa::ConstAutomatonRef
NBestListExtractor::mapEvalToLemmaPron(Fsa::ConstAutomatonRef fsa) const {
  return Fsa::cache(Fsa::removeEpsilons(Fsa::projectInput(Fsa::composeMatching(
      lemmaPronToLemma_, Fsa::composeMatching(lemmaToEval_, fsa)))));
}
...
Fsa::ConstAutomatonRef NBestListExtractor::getNBestListWithoutWordBoundaries(
  ...
  for (; threshold <= maxThreshold_; threshold += thresholdIncrement_) {
    ...
    Fsa::ConstAutomatonRef pruned =
        Fsa::prunePosterior(fsa, Fsa::Weight(threshold));
    ...
    Fsa::Weight leastWeight =
        nbl->getState(nbl->initialStateId())->getArc(nHypotheses - 1)->weight();
    bool hasNBestHypotheses =
        sr->compare(leastWeight,
                    sr->extend(minWeight, Fsa::Weight(threshold))) < 0;
...
ConstWordLatticeRef
NBestListExtractor::getNBestList(ConstWordLatticeRef lattice) {
  ...
  for (; hIt != hEnd; ++hIt) {
    Fsa::ConstAutomatonRef hf =
        Fsa::partial(nBestList, hIt->target(), hIt->weight());
    if (!workOnOutput_) {
      hf = mapEvalToLemmaPron(hf);
    }
    hf = Fsa::multiply(hf, Fsa::Weight(0));
    hypotheses.push_back(
        staticCopy(normalize(best(composeMatching(hf, lattice)))));
  }
