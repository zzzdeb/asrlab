...
public:
  Parser(const Configuration &c, DependencySet &dependencySet)
    ...
    XmlRegularElement *newDependencySet =
        new XmlRegularElementRelay("dependency-set", this, 0, 0);
...
/*****************************************************************************/
void DependencySet::add(const std::string &name,
...
/*****************************************************************************/
{
  ...
  for (Dependencies::const_iterator d = dependencies_.begin();
       d != dependencies_.end(); ++d) {
    Dependencies::const_iterator found = s.dependencies_.find(d->first);
    if (found == s.dependencies_.end()) {
      Application::us()->warning("dependency named '%s' with value '%s' "
                                 "generated on %s not in second set",
      ...
      if (d->second.value() != found->second.value()) {
        Application::us()->warning(
            "dependency named '%s' with value '%s' generated on %s differs "
            "from value '%s' generated on %s",
            d->first.c_str(), d->second.value().c_str(),
...
/*****************************************************************************/
{
  ...
  for (d = required.dependencies_.begin(); d != required.dependencies_.end();
       ++d) {
...
/*****************************************************************************/
bool DependencySet::weak_satisfies(const DependencySet &required,
                                   bool warn) const
/*****************************************************************************/
{
  bool isEqual = true;
      ...
      if (d->second.value() != found->second.value()) {
        if (d->first == "acoustic-model.stream-extraction") {
          ...
          if (dValue < foundValue) {
            Application::us()->warning(
                "dependency named '%s' with feature value '%s' generated on %s "
                "is smaller than acoustic model value '%s' generated on %s; "
                "should be greater equall.",
                d->first.c_str(), d->second.value().c_str(),
                d->second.date().c_str(), found->second.value().c_str(),
                found->second.date().c_str());
            isEqual = false;
          ...
          Application::us()->warning(
              "dependency named '%s' with feature value '%s' generated on %s "
              "differs from acoustic model value '%s' generated on %s",
              d->first.c_str(), d->second.value().c_str(),
              d->second.date().c_str(), found->second.value().c_str(),
              found->second.date().c_str());
          isEqual = false;
...
/*****************************************************************************/
bool DependencySet::read(const Configuration &config,
