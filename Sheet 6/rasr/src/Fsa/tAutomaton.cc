...
template <class _Arc> inline void ArrayArcContainer<_Arc>::reallocateOneMore() {
  ++nArcs_;
  arcs_ = (_Arc *)::realloc(arcs_, sizeof(_Arc) * nArcs_);
  /*! @todo: check bad alloc */
}
...
template <class _Arc>
ArrayArcContainer<_Arc>::ArrayArcContainer(
    const ArrayArcContainer<_Arc> &other) {
...
template <class _Arc> void ArrayArcContainer<_Arc>::clear() {
  if (arcs_) {
    destroy(begin(), end());
    ::free(arcs_);
    arcs_ = 0;
...
template <class _Arc> _Arc *ArrayArcContainer<_Arc>::addArc() {
  reallocateOneMore();
  return arcs_ + nArcs_ - 1;
}

template <class _Arc>
_Arc *ArrayArcContainer<_Arc>::addArc(Fsa::StateId target,
                                      typename _Arc::Weight weight,
                                      Fsa::LabelId input) {
  reallocateOneMore();
...
template <class _Arc> void ArrayArcContainer<_Arc>::truncate(iterator i) {
  reallocate(i - begin());
}

template <class _Arc> void ArrayArcContainer<_Arc>::resize(size_t s) {
  reallocate(s);
}

template <class _Arc> void ArrayArcContainer<_Arc>::minimize() {
  reallocate(nArcs_);
}

template <class _Arc>
typename ArrayArcContainer<_Arc>::iterator
ArrayArcContainer<_Arc>::insert(iterator pos, const _Arc &arc) {
  size_t p = pos - begin();
  reallocateOneMore();
