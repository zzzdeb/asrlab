...
public:
  Segment()
      : audioFilename_(), hasAudioFilename_(false),
        startTime_(Core::Type<f32>::min), hasStartTime_(false),
        endTime_(Core::Type<f32>::max), hasEndTime_(false), track_(0),
        hasTrack_(false), orthography_(), hasOrthography_(false), speakerId_(),
        hasSpeakerId_(false), conditionId_(), hasConditionId_(false),
        recordingId_(), hasRecordingId_(false), segmentId_(),
        hasSegmentId_(false), blissSpeechSegment_(0) {}

  Segment(const Bliss::SpeechSegment *blissSpeechSegment) {
    setBlissSpeechSegment(blissSpeechSegment);
  }

  bool hasAudioFilename() const { return hasAudioFilename_; }
  const std::string &audioFilename() const { return audioFilename_; }
  void setAudioFilename(const std::string &audioFilename) {
    audioFilename_ = audioFilename;
    hasAudioFilename_ = true;
  }

  bool hasStartTime() const { return hasStartTime_; }
  f32 startTime() const { return startTime_; }
  void setStartTime(f32 startTime) {
    startTime_ = startTime;
    hasStartTime_ = true;
  }

  bool hasEndTime() const { return hasEndTime_; }
  f32 endTime() const { return endTime_; }
  ...
  u32 track() const { return track_; }
  void setTrack(u32 track) {
    track_ = track;
    hasTrack_ = true;
  }

  bool hasOrthography() const { return hasOrthography_; }
  ...
  bool hasSpeakerId() const { return hasSpeakerId_; }
  const std::string &speakerId() const { return speakerId_; }
  void setSpeakerId(const std::string &speakerId) {
    speakerId_ = speakerId;
    hasSpeakerId_ = true;
  }

  bool hasConditionId() const { return hasConditionId_; }
  const std::string &conditionId() const { return conditionId_; }
  void setConditionId(const std::string &conditionId) {
    conditionId_ = conditionId;
    hasConditionId_ = true;
  }

  bool hasRecordingId() const { return hasRecordingId_; }
  const std::string &recordingId() const { return recordingId_; }
  void setRecordingId(const std::string &recordingId) {
    recordingId_ = recordingId;
    hasRecordingId_ = true;
  }

  bool hasSegmentId() const { return hasSegmentId_; }
  ...
  bool hasBlissSpeechSegment() const { return blissSpeechSegment_; }
...
void printSegmentHeader(std::ostream &os, ConstSegmentRef segment,
