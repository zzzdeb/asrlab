...
ClassicAcousticModel::ClassicAcousticModel(
    ...
    : Core::Component(configuration), AcousticModel(configuration),
...
bool ClassicAcousticModel::loadStateModel() {
  HmmTopologySet *hmmTopologySet = new HmmTopologySet(select("hmm"), silence_);
  HmmTopologySetRef hmmTopologySetRef(hmmTopologySet);
  Phonology *phonology =
      new Phonology(select("phonology"), lexiconRef_->phonemeInventory());
  ...
  AllophoneAlphabet *allophoneAlphabet =
      new AllophoneAlphabet(select("allophones"), phonologyRef, lexiconRef_);
  ConstAllophoneAlphabetRef allophoneAlphabetRef(allophoneAlphabet);
  AllophoneStateAlphabet *allophoneStateAlphabet = new AllophoneStateAlphabet(
      select("allophone-states"), allophoneAlphabetRef, hmmTopologySetRef);
  ConstAllophoneStateAlphabetRef allophoneStateAlphabetRef(
      allophoneStateAlphabet);
  stateModelRef_ = ClassicStateModelRef(
      new ClassicStateModel(phonologyRef, allophoneAlphabetRef,
                            allophoneStateAlphabetRef, hmmTopologySetRef));
  determineSilenceAllophoneStateIndex();
  return true;
}

const Core::Choice ClassicAcousticModel::choiceStateTying(
    "none", noTying, "no-tying", noTying, "monophone", monophoneTying, "lut",
    lutTying, "lookup", lutTying, "cart", cartTying, "decision-tree", cartTying,
    "old-cart", oldCartTying, "old-decision-tree", oldCartTying,
    Core::Choice::endMark());
const Core::ParameterChoice
    ClassicAcousticModel::paramType("type", &choiceStateTying,
...
bool ClassicAcousticModel::loadStateTying() {
  ...
  case oldCartTying:
    // for compatibility reasons
    stateTyingRef_ =
        Core::Ref<const ClassicStateTying>(new Legacy::PhoneticDecisionTree(
            select("decision-tree"), stateModelRef_));
    break;
  default:
    stateTyingRef_ = ClassicStateTying::createClassicStateTyingRef(
        select("state-tying"), stateModelRef_);
    break;
  }
...
bool ClassicAcousticModel::loadTransitionModel() {
  transitionModel_ =
      Core::ref(new ScaledTransitionModel(select("tdp"), stateModel()));
...
bool ClassicAcousticModel::loadMixtureSet() {
  ...
  Core::Ref<Mm::ScaledFeatureScorer> featureScorer =
      Mm::Module::instance().createScaledFeatureScorer(select("mixture-set"),
                                                       mixtureSet_);
  if (!featureScorer) {
...
void ClassicAcousticModel::getDependencies(
...
bool ClassicAcousticModel::setFeatureScorer(
    Core::Ref<Mm::ScaledFeatureScorer> featureScorer) {
  require(featureScorer);
  verify(stateTyingRef_);

  if (stateTyingRef_->nClasses() != featureScorer->nMixtures()) {
    error("State-tying (nClasses=%d) and mixture-set (nMixtures=%d) do not "
          "match.",
...
AcousticModel::EmissionIndex ClassicAcousticModel::nEmissions() const {
  ...
  return featureScorer_ ? featureScorer_->nMixtures()
                        : stateTyingRef_->nClasses();
}

void ClassicAcousticModel::distributeScaleUpdate(
