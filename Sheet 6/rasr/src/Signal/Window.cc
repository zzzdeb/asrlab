...
Window::Window() : lengthInS_(0), shiftInS_(0.0), windowFunction_(0) {}
...
const Core::ParameterFloat WindowNode::paramShift("shift", "shift of window");

const Core::ParameterFloat WindowNode::paramLength("length",
                                                   "length of window");

const Core::ParameterBool WindowNode::paramFlushAll(
    "flush-all", "if false, segments stops after the last sample was delivered",
    false);

const Core::ParameterBool WindowNode::paramFlushBeforeGap(
    "flush-before-gap", "if true, flushes before a gap in the input samples",
    true);

WindowNode::WindowNode(const Core::Configuration &c)
    : Component(c), Predecessor(c) {

  setWindowFunction(WindowFunction::create(
      (WindowFunction::Type)WindowFunction::paramType(c)));
...
bool WindowNode::setParameter(const std::string &name,
                              const std::string &value) {

  if (WindowFunction::paramType.match(name))
    setWindowFunction(WindowFunction::create(
        (WindowFunction::Type)WindowFunction::paramType(value)));
