...
inline UnaryAnalyticFunctionRef
...
class PiecewiseConstantFunction
    : public UnaryAnalyticFunction,
      public std::vector<std::pair<UnaryAnalyticFunction::Argument,
                                   UnaryAnalyticFunction::Result> > {
  typedef std::vector<std::pair<UnaryAnalyticFunction::Argument,
                                UnaryAnalyticFunction::Result> >
      Precursor;
...
private:
  ...
  struct greaterOrEqual
      : public std::binary_function<LimitAndValue, Argument, bool> {
    bool operator()(const LimitAndValue &x, const Argument &limit) const {
      return x.first >= limit;
    }
  };
  std::vector<LimitAndValue>::const_iterator
  findSegment(const Argument &limit) const {
...
public:
  ...
  void insert(const LimitAndValue &limitAndValue) {
    iterator pos = std::find_if(
        begin(), end(), std::bind2nd(greaterOrEqual(), limitAndValue.first));
    Precursor::insert(pos, limitAndValue);
  }

  virtual Result value(Argument x) const {
    const_iterator result =
        std::find_if(begin(), end(), std::bind2nd(greaterOrEqual(), x));
    ensure(result != end());
