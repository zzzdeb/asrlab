...
BinaryStreamIos::BinaryStreamIos(Endianess endianess) : ios_(0), fstream_(0) {
  ios_ = fstream_ = new std::fstream;
  setEndianess(endianess);
}

BinaryStreamIos::BinaryStreamIos(std::ios &i, Endianess endianess)
    : ios_(&i), fstream_(dynamic_cast<std::fstream *>(ios_)) {
  setEndianess(endianess);
}
...
bool BinaryOutputStream::write(const T *v, std::streamsize n) {
  if (!swap_)
    return os_->write((char *)v, sizeof(T) * n);
...
// namespace Core has to be defined explicitely here, otherwise gcc-4 will
// produce an ICE
namespace Core {
template <> bool BinaryOutputStream::write(const bool *v, std::streamsize n) {
  u8 *tmp = new u8[n];
...
#ifdef OS_darwin
template bool BinaryOutputStream::write<size_t>(const size_t *,
                                                std::streamsize);
...
template <typename T> bool BinaryInputStream::read(T *v, std::streamsize n) {
...
#warning                                                                       \
    "readsome() is buggy, that is why we are using a work-around, see http://support.microsoft.com/kb/246934/de"
  read(v, n);
  return is_->gcount() / sizeof(T);
}

namespace Core {
template <> bool BinaryInputStream::read(bool *v, std::streamsize n) {
  u8 *tmp = new u8[n];
...
template std::streamsize BinaryInputStream::readSome<s16>(s16 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<u16>(u16 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<s32>(s32 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<u32>(u32 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<s64>(s64 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<u64>(u64 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<f32>(f32 *,
                                                          std::streamsize);
template std::streamsize BinaryInputStream::readSome<f64>(f64 *,
                                                          std::streamsize);
...
BinaryInputStreams::BinaryInputStreams(
    const std::vector<std::string> &filenames, std::ios_base::openmode mode,
