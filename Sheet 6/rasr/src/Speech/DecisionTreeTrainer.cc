...
// ============================================================================
FeatureAccumulator::FeatureAccumulator(const Core::Configuration &config)
    : Core::Component(config),
      Precursor(config, Am::AcousticModel::noEmissions |
                            Am::AcousticModel::noStateTying),
      examples_(config), nObs_(0), nCols_(0), map_() {
  map_ = Cart::PropertyMapRef(new Am::PropertyMap(
...
Cart::Example &FeatureAccumulator::example(Am::AllophoneStateIndex id) {
  if ((size_t(id) >= examples_.size()) || !examples_[id]) {
    ...
    Cart::Example *_example = new Cart::Example(
        new Cart::StoredProperties(props), new Cart::FloatBox(2, nCols_));
...
void FeatureAccumulator::processAlignedFeature(Core::Ref<const Feature> f,
                                               Am::AllophoneStateIndex id) {
  ...
  if ((size_t(id) >= examples_.size()) || !examples_[id]) {
    Am::Properties props(
        map_, acousticModel()->allophoneStateAlphabet()->allophoneState(id));
    examples_.set(id, new Cart::Example(new Cart::StoredProperties(props),
                                        new Cart::FloatBox(2, nCols_)));
  }
  ...
  Cart::FloatBox::vector_iterator itSumOfSquares =
      example.values->row(1).begin();
...
void FeatureAccumulator::writeXml(Core::XmlWriter &xml) const {
  xml << Core::XmlFull("nExamples", examples_.size())
      ...
      << Core::XmlEmpty("matrix-f64") + Core::XmlAttribute("nRows", 2) +
...
const Core::ParameterFloat
    StateTyingDecisionTreeTrainer::LogLikelihoodGain::paramVarianceClipping(
        "variance-clipping", "minimum \\sigma^2", 0.0);

StateTyingDecisionTreeTrainer::LogLikelihoodGain::LogLikelihoodGain(
    const Core::Configuration &config)
    : Precursor(config) {
  minSigmaSquare_ = paramVarianceClipping(config);
}

void StateTyingDecisionTreeTrainer::LogLikelihoodGain::write(
...
/*
  ...
  log N(x; \theta)   = -\frac{1}{2} \left( \sum_1^D \log(2 \pi \sigma^2_d +
  \sum_1^D \frac{(x_d - \mu_d)^2}{\sigma^2_d} \right) -LL(\theta| x_1^N) =
  \frac{1}{2} \left( N D + N \sum_{d=1}^{D} \log( 2 \pi \sigma^2_d) \right)
*/
Cart::Score StateTyingDecisionTreeTrainer::LogLikelihoodGain::logLikelihood(
  ...
  for (Cart::ExamplePtrList::const_iterator it = begin; it != end; ++it) {
    ...
    Cart::FloatBox::const_vector_iterator itSum =
        example.values->row(0).begin();
    Cart::FloatBox::const_vector_iterator itSumOfSquares =
        example.values->row(1).begin();
    for (; itMu != mu_.end();
         ++itMu, ++itSigmaSquare, ++itSum, ++itSumOfSquares) {
      *itMu += *itSum;
...
void StateTyingDecisionTreeTrainer::LogLikelihoodGain::operator()(
    const Cart::ExamplePtrRange &examples, Cart::Score &score) const {
  score = logLikelihood(examples.begin, examples.end);
}

Cart::Score StateTyingDecisionTreeTrainer::LogLikelihoodGain::operator()(
    const Cart::ExamplePtrRange &leftExamples,
    const Cart::ExamplePtrRange &rightExamples,
    const Cart::Score fatherLogLikelihood, Cart::Score &leftChildLogLikelihood,
    Cart::Score &rightChildLogLikelihood) const {
  leftChildLogLikelihood = logLikelihood(leftExamples.begin, leftExamples.end);
  rightChildLogLikelihood =
      logLikelihood(rightExamples.begin, rightExamples.end);
  Cart::Score gain =
      fatherLogLikelihood - (leftChildLogLikelihood + rightChildLogLikelihood);
  /*
...
StateTyingDecisionTreeTrainer::StateTyingDecisionTreeTrainer(
    const Core::Configuration &config)
    : Precursor(config) {
  setScorer(Cart::ConstScorerRef(
      new LogLikelihoodGain(select("log-likelihood-gain"))));
}
// ============================================================================
