...
void copy(StorageAutomaton<_Automaton> *f, const std::string &str) {
  ...
  for (Core::StringTokenizer::Iterator token = tokenizer.begin();
    ...
    if (!tmp.empty()) {
      _Arc *a = sp->newArc();
      a->input_ = a->output_ =
          (sinput ? sinput->addSymbol(tmp) : input->index(tmp));
...
staticCopy(typename _Automaton::ConstRef f) {
...
template <class _Automaton>
Core::Ref<StaticAutomaton<_Automaton> >
staticCompactCopy(typename _Automaton::ConstRef f) {
  StaticAutomaton<_Automaton> *result = new StaticAutomaton<_Automaton>;
  copy(result, f);
...
template <class _Automaton>
Core::Ref<StaticAutomaton<_Automaton> >
staticCopy(const Fsa::Hash<std::string, Core::StringHash> strings,
           typename _Automaton::ConstSemiringRef semiring, u32 sausage = 0) {
  StaticAutomaton<_Automaton> *result = new StaticAutomaton<_Automaton>;
  result->setSemiring(semiring);
