...
SimdGaussDiagonalMaximumFeatureScorer::multiplyAndQuantize(
    const Mm::FeatureVector &featureVector) const {
  ...
  for (u32 c = 0; c < ret.size(); ++c) {
    FeatureScorerOptimization::multiplyAndQuantize(
        featureVector, covarianceTable_[c].inverseSquareRootDiagonal(), ret[c]);
  }
...
void SimdGaussDiagonalMaximumFeatureScorer::buildMixtureTable(
  ...
  for (MixtureIndex mix = 0; mix < mixtureTable_.size(); ++mix) {
    ...
    for (size_t dns = 0; dns < mixture->nDensities(); ++dns) {
      ...
      quantizedMixture.density(dns).covarianceIndex_ =
          density->covarianceIndex();

      Weight scaledMinus2LogWeight =
          scalingSquared_ * -2 * mixture->logWeight(dns);

      FeatureScorerOptimization::createDensityElement(
          scaledMinus2LogWeight, *mixtureSet.mean(density->meanIndex()),
...
MeanType SimdGaussDiagonalMaximumFeatureScorer::getScaling(
...
FeatureType SimdGaussDiagonalMaximumFeatureScorer::quantizationScalingFactor(
  ...
  FeatureType intervalSize =
      2 * std::max(Core::abs(minValue), Core::abs(maxValue));
...
SimdGaussDiagonalMaximumFeatureScorer::calculateScoreAndDensity(
  ...
  std::pair<int, size_t> quantizedResult = quantizedScore(
      mixtureTable_[mixtureIndex], c->featureVectorPerCovariance_);
 ...
 */
std::pair<int, DensityIndex>
SimdGaussDiagonalMaximumFeatureScorer::quantizedScore(
