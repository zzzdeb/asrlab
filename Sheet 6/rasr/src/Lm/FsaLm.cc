...
const Core::ParameterString
    FsaLm::paramFilename("file",
                         "name of fsa file file to load as language model");

Fsa::ConstStateRef FsaLm::invalidHistory(new Fsa::State(Fsa::InvalidStateId,
                                                        Fsa::StateTagFinal));
...
FsaLm::FsaLm(const Core::Configuration &c, Bliss::LexiconRef lexicon)
    : Core::Component(c), LanguageModel(c, lexicon),
...
{
  fsa_ = Fsa::cache(
      Fsa::sort(Fsa::mapInput(f, syntacticTokens_), Fsa::SortTypeByInput));
}
