 ...
 */
template <class In, class Out> struct Converter {
 ...
 */
template <class C> class TypeConverterNode : public SleeveNode {
public:
...
public:
  static std::string filterName() {
    return std::string("generic-convert-") + In::type()->name() + "-to-" +
           Out::type()->name();
  }
...
template <class C> bool TypeConverterNode<C>::configure() {
...
template <class C> bool TypeConverterNode<C>::work(PortId p) {
  DataPtr<In> in;
  if (!getData(0, in))
