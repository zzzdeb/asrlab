...
private:
  /** @return is @param vectorSize rounded up to the next integer divisible by
   * BlockSize. */
...
public:
  FeatureScorerIntelOptimization(const Core::Configuration &c,
  ...
  static void createDensityElement(
      Score scaledMinus2LogWeight, const Mean &mean,
   ...
   * Before using floating point operations resetFloatingPointCalculation HAS to
   * be called.
   */
  int distance(const PreparedFeatureVector &mean,
               const PreparedFeatureVector &featureVector) const {
