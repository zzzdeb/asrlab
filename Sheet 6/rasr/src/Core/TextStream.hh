...
public:
  enum FormattingHint {
    ...
    indentPlus = 1, /**< increase indentation level by one (effective after next
                       newline) */
    indentMinus = 2, /**< decrease indentation level by one (effective after
                        next newline) */
    ...
    softBlank = BEL,      /**< consecutive soft blanks are replaced by a single
                             normal blank */
    lineBreak = LF,       /**< start a new line uncoditionally */
    newLine = 14 /**< start a new line, ignore if the current position is the
                    beginning of a line */
  };
...
public:
   ...
   */
  void setEncoding(
      const std::string &encoding = UnicodeInputConverter::defaultEncoding);
