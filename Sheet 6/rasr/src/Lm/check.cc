...
public:
  ...
  virtual int main(const std::vector<std::string> &arguments) {
    ...
    Core::Ref<Lm::LanguageModel> lm =
        Lm::Module::instance().createLanguageModel(select("lm"), lex);
    if (!lm)
...
const Core::ParameterString
    TestApplication::paramDumpLexiconFsa_("dump-lexicon-fsa",
                                          "dump lexicon as fsa to file");
const Core::ParameterString
    TestApplication::paramDumpLmFsa_("dump-lm-fsa", "dump lm as fsa to file");
const Core::ParameterString
    TestApplication::paramDrawLmFsa_("draw-lm-fsa", "draw lm as fsa to file");
