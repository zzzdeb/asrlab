...
ModelCombinationRef getModelCombination(
    const Core::Configuration &config, AcousticModelRef acousticModel,
    ScaledLanguageModelRef languageModel = ScaledLanguageModelRef());
 ...
 **/
class SegmentwiseFeatureExtractor : public virtual Core::Component,
...
public:
  SegmentwiseFeatureExtractor(const Core::Configuration &config,
 ...
 * 2) process all features in segment, verify consistence with acoustic model ->
 *call processFeature for each extracted feature
 **/
...
public:
  SegmentwiseSpeechProcessor(const Core::Configuration &config,
                             ModelCombinationRef = ModelCombinationRef());
  SegmentwiseSpeechProcessor(SegmentwiseFeatureExtractorRef featureExtractor,
