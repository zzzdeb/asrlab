...
bool write(const Resources<_Automaton> &resources,
           typename _Automaton::ConstRef f, const std::string &format,
           std::ostream &o, Fsa::StoredComponents what, bool progress) {
  typename Resources<_Automaton>::Format *formatter =
      (format.empty()) ? resources.getDefaultFormat()
  ...
  if (!formatter->writer) {
    std::cerr << "format \"" << formatter->name << "\" does not support writing"
              << std::endl;
    return false;
  }
...
template <class _Automaton>
bool write(const Resources<_Automaton> &resources,
           typename _Automaton::ConstRef f, const std::string &file,
           Fsa::StoredComponents what, bool progress) {
 ...
 * - with the RWTH FSA label ids can be in the range [0..InvalidLabelId), they
 *are transformed to [1..InvalidLabelId] and 0 is being used for Epsilon, other
 *special labels are not supported in AT&T's format
 **/
template <class _Automaton>
...
public:
  WriteAttDfsState(_ConstAutomatonRef f, std::ostream &o)
      : Precursor(f), o_(o), input_(f->getInputAlphabet()),
    ...
    for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if (!Precursor::fsa_->semiring()->isDefault(a->weight())) {
        std::string weightStr =
            Precursor::fsa_->semiring()->asString(a->weight());
...
template <class _Automaton>
bool writeAtt(const Resources<_Automaton> &resources,
              typename _Automaton::ConstRef f, std::ostream &o,
              Fsa::StoredComponents what, bool progress) {
  /*
  if (!f->semiring()->hasProperty(Fsa::PropertyStringIo)) {
      std::cerr << "semiring " << f->semiring()->name() << " does not support
  string i/o" << std::endl; return false;
  }
  */
  if (o) {
    // input alphabet
...
template <class _Automaton>
bool writeBinary(const Resources<_Automaton> &resources,
                 typename _Automaton::ConstRef f, std::ostream &o,
                 Fsa::StoredComponents what, bool progress) {
  /*
  if (!f->semiring()->hasProperty(Fsa::PropertyBinaryIo)) {
      std::cerr << "semiring " << f->semiring()->name() << " does not support
  binary i/o" << std::endl; return false;
  }
  */
  if (!o)
    return false;
  ...
  if (what & Fsa::storeStates)
    bo << u32(f->properties()) << u32(f->knownProperties())
  ...
  void discoverState(_ConstStateRef sp) {
    for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if ((Precursor::fsa_->type() == Fsa::TypeTransducer) &&
        ...
        out += ":" + (output_ ? output_->symbol(a->output())
                              : Core::itoa(a->output()));
      if (!out.empty()) {
        if (first_)
          first_ = false;
...
template <class _Automaton>
bool writeLinear(const Resources<_Automaton> &resources,
                 typename _Automaton::ConstRef f, std::ostream &o,
                 Fsa::StoredComponents what, bool progress) {
...
public:
  ...
  void discoverState(_ConstStateRef sp) {
    ...
    if (sp->isFinal())
      if (!Precursor::fsa_->semiring()->isDefault(sp->weight_))
        o_ << Core::XmlFull("weight",
                            Precursor::fsa_->semiring()->asString(sp->weight_));
    o_ << "\n";
    for (typename _State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
      o_ << Core::XmlOpen("arc") + Core::XmlAttribute("target", a->target());
      if (a->input() != Fsa::Epsilon) {
        std::string tmp =
            Precursor::fsa_->getInputAlphabet()->specialSymbol(a->input());
        if (tmp.empty())
      ...
      if (Precursor::fsa_->type() == Fsa::TypeTransducer)
        if (a->output() != Fsa::Epsilon) {
          std::string tmp =
              Precursor::fsa_->getOutputAlphabet()->specialSymbol(a->output());
          if (tmp.empty())
...
template <class _Automaton>
bool writeXml(const Resources<_Automaton> &resources,
              typename _Automaton::ConstRef f, std::ostream &o,
              Fsa::StoredComponents what, bool progress) {
  /*
  if (!f->semiring()->hasProperty(Fsa::PropertyXmlIo)) {
      std::cerr << "semiring " << f->semiring()->name() << " does not support
  xml i/o" << std::endl; return false;
  }
  */
  if (!o)
    return false;
  ...
  xo << "\n"
     << Core::XmlOpenComment() << "Properties were (only saved in bin format) '"
     << f->properties() << "'" << Core::XmlCloseComment() << "\n\n";
...
template <class _Automaton> class EdgeTrWGWriter;

template <class _Automaton> class NodeTrWGWriter : public DfsState<_Automaton> {
...
public:
  ...
  virtual void finish() {
    if (!final_states_.empty()) {
      if (new_end()) {
        ...
        ++max_id_;
        end_id_ = max_id_;
        for (std::set<Fsa::StateId>::iterator s = final_states_.begin();
             s != final_states_.end(); ++s)
  ...
  bool new_end() { return (final_states_.size() > 1); }

  bool hasFinalState() { return (!final_states_.empty()); }

  Fsa::StateId end() { return end_id_; }
};

template <class _Automaton> class EdgeTrWGWriter : public DfsState<_Automaton> {
private:
  typedef DfsState<_Automaton> Precursor;
...
public:
  EdgeTrWGWriter(_ConstAutomatonRef f, NodeTrWGWriter<_Automaton> &nw,
                 std::ostream &o)
  ...
  virtual void exploreTreeArc(_ConstStateRef from, const _Arc &a) {
    ...
    o_ << " <edge weight=\""
       << Precursor::fsa_->semiring()->asString(a.weight()) << "\" label=\""
       << convert_to_xml_encoding(label) << "\" > ";
  ...
  virtual void finish() {
    if (nw_.new_end()) {
      for (std::set<Fsa::StateId>::iterator s = nw_.final_states_.begin();
           s != nw_.final_states_.end(); ++s) {
...
template <class _Automaton>
bool writeTrXml(const Resources<_Automaton> &resources,
                typename _Automaton::ConstRef f, std::ostream &o,
                Fsa::StoredComponents what, bool progress) {
  /*
  if (!f->semiring()->hasProperty(Fsa::PropertyStringIo)) {
