...
template <class _Automaton> class ComposeAutomaton : public _Automaton {
public:
...
protected:
  ...
  mutable Core::Vector<Fsa::LabelIdStrings::Id> stringPotentialsLeft_,
      stringPotentialsRight_;

  void stringPotential(_ConstAutomatonRef f, Fsa::StateId s,
                       Core::Vector<Fsa::LabelIdStrings::Id> &potentials,
                       bool input) const {
    // cache potentials
    ...
    states_.push(s);
    typedef Core::Vector<std::pair<Fsa::StateId, Fsa::LabelIdStrings::Id> >
        StatePotentials;
    ...
    while (!states_.empty()) {
      ...
      // as long as we have only a single state to inspect we'll also store its
      // potential
      if (states_.size() == 1)
        potentials_.push_back(
            std::make_pair(states_.top(), stringPotentials_.start() - start));
      Fsa::LabelId label = Fsa::InvalidLabelId;

      // check wether all arcs leaving states have the same label (stack of
      // states is empty if finished)
      while (!states_.empty()) {
        ...
        FinalOrNoMatchingLabels:
          ...
          for (StatePotentials::const_iterator p = potentials_.begin();
               p != potentials_.end(); ++p)
          ...
          for (StatePotentials::const_iterator p = potentials_.begin();
               p != potentials_.end(); ++p) {
            // std::cout << "store: " << p->first << "(" << start + p->second <<
            // ")" << std::endl;
  ...
  bool areStringPotentialsPrefixes(Fsa::StateId sl, Fsa::StateId sr) const {
    return true;
    Fsa::LabelIdStrings::Id ll = stringPotentialLeft(sl),
                            lr = stringPotentialRight(sr);
    if (ll == lr)
      return true;
    Fsa::LabelIdStrings::const_iterator il = stringPotentials_.begin(ll),
                                        ir = stringPotentials_.begin(lr);
    for (; *il == *ir; ++il, ++ir)
...
public:
  GeneralComposeAutomaton(_ConstAutomatonRef fl, _ConstAutomatonRef fr,
                          bool reportUnknowns) {
    if (fl->semiring() != fr->semiring())
      std::cerr << "inconsistent semirings for compose" << std::endl;
    fl_ = cache<_Automaton>(
        sort<_Automaton>(mapOutput<_Automaton>(fl, fr->getInputAlphabet(),
                                               reportUnknowns ? 10 : 0),
                         Fsa::SortTypeByOutput),
        10000);
    ...
    this->setProperties(Fsa::PropertyStorage | Fsa::PropertyCached,
                        Fsa::PropertyNone);
    if ((fl_->hasProperty(Fsa::PropertyAcyclic)) ||
        (fr_->hasProperty(Fsa::PropertyAcyclic)))
  ...
  virtual Fsa::Type type() const {
    if ((fl_->type() == Fsa::TypeAcceptor) &&
        (fr_->type() == Fsa::TypeAcceptor))
  ...
  Fsa::StateId insertState(Fsa::StateId l, Fsa::StateId f,
                           Fsa::StateId r) const {
  ...
  void composeArcs(_State *sp, _ConstStateRef sl, _ConstStateRef sr,
                   typename _State::const_iterator al,
                   typename _State::const_iterator ar) const {
    // the following factor of 2 comparison is heuristic, but tested on a larger
    // set of automata
    if (((sl->nArcs() << 2) < sr->nArcs()) ||
        (sl->nArcs() > (sr->nArcs() << 2))) {
      for (; (al < sl->end()) && (ar < sr->end());) {
        if ((al->output() > Fsa::LastLabelId) ||
            (ar->input() > Fsa::LastLabelId))
          break;
        if (al->output() == ar->input()) {
          for (typename _State::const_iterator a = ar;
               ((a < sr->end()) && (al->output() == a->input())); ++a)
#ifdef STRING_POTENTIALS
            if (areStringPotentialsPrefixes(al->target(), a->target()))
#endif
              sp->newArc(insertState(al->target(), 0, a->target()),
                         semiring()->extend(al->weight(), a->weight()),
                         al->input(), a->output());
          ++al;
  ...
  void composeSpecialArcs(_State *sp, _ConstStateRef sl,
                          _ConstStateRef sr) const {
    Fsa::LabelId label;
      ...
      case Fsa::Any:
        for (al = sl->begin(); al < sl->end(); ++al) {
          ...
          sp->newArc(insertState(al->target(), 0, sar->target()),
                     semiring()->extend(al->weight(), sar->weight()),
                     al->input(), label);
        }
        break;
      case Fsa::Failure:
        for (al = sl->begin(), ar = sr->begin();
             (al < sl->end()) && (ar < sr->end()); ++ar) {
          if (al->output() > Fsa::LastLabelId)
            break;
  ...
  virtual size_t getMemoryUsed() const {
    return fl_->getMemoryUsed() + fr_->getMemoryUsed() +
           2 * sizeof(_ConstAutomatonRef) + 4 * sizeof(Fsa::LabelId) +
           states_.getMemoryUsed()
#ifdef STRING_POTENTIALS
           + stringPotentials_.getMemoryUsed() +
           stringPotentialsLeft_.getMemoryUsed() +
...
public:
  ComposeMatchingAutomaton(_ConstAutomatonRef fl, _ConstAutomatonRef fr,
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    if (s < Precursor::states_.size()) {
      ...
      _ConstStateRef sl = Precursor::fl_->getState(l),
                     sr = Precursor::fr_->getState(r);
          ...
          if ((ar != sr->end()) || (sr->isFinal()))
            for (; (al != sl->end()) && (al->output() == Fsa::Epsilon); ++al)
              sp->newArc(insertState(al->target(), 1, r),
                         this->semiring()->extend(
                             al->weight(), Precursor::fr_->semiring()->one()),
          ...
          if ((al != sl->end()) || (sl->isFinal()))
            for (; (ar != sr->end()) && (ar->input() == Fsa::Epsilon); ++ar)
              sp->newArc(insertState(l, 2, ar->target()),
                         this->semiring()->extend(
                             Precursor::fl_->semiring()->one(), ar->weight()),
      ...
      case 2:
        for (; (ar != sr->end()) && (ar->input() == Fsa::Epsilon); ++ar)
          sp->newArc(insertState(l, 2, ar->target()),
                     this->semiring()->extend(Precursor::fl_->semiring()->one(),
                                              ar->weight()),
                     Fsa::Epsilon, ar->output());
        break;
      default:
  ...
  virtual std::string describe() const {
    return Core::form("composeMatching(%s,%s)",
                      Precursor::fl_->describe().c_str(),
                      Precursor::fr_->describe().c_str());
  }
};

template <class _Automaton>
typename _Automaton::ConstRef composeMatching(typename _Automaton::ConstRef fl,
                                              typename _Automaton::ConstRef fr,
                                              bool reportUnknowns) {
  return typename _Automaton::ConstRef(
      new ComposeMatchingAutomaton<_Automaton>(fl, fr, reportUnknowns));
}

template <class _Automaton>
typename _Automaton::ConstRef difference(typename _Automaton::ConstRef fl,
                                         typename _Automaton::ConstRef fr) {
  return trim<_Automaton>(
      composeMatching<_Automaton>(fl, complement<_Automaton>(fr)));
}
...
public:
  ComposeSequencingAutomaton(_ConstAutomatonRef fl, _ConstAutomatonRef fr,
                             bool reportUnknowns = true)
      : Precursor(fl, fr, reportUnknowns) {}
  ...
  virtual std::string describe() const {
    return Core::form("composeSequencing(%s,%s)",
                      Precursor::fl_->describe().c_str(),
                      Precursor::fr_->describe().c_str());
  }
};
...
composeSequencing(typename _Automaton::ConstRef fl,
                  typename _Automaton::ConstRef fr, bool reportUnknowns) {
  return typename _Automaton::ConstRef(
      new ComposeSequencingAutomaton<_Automaton>(fl, fr, reportUnknowns));
}

template <class _Automaton> class ComposeMapping : public Fsa::Mapping {
public:
  typedef typename _Automaton::ConstRef _ConstAutomatonRef;
