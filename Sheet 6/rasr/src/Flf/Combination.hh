 ...
 * 1) discard
 *    Discard the scores from source semirings; the new semiring consists only
 *of the combination scores. 2) concatenate Concatenate the scores of the
 *individual semirings and append the combination scores.
 **/
...
public:
  ...
  typedef enum { TypeDiscard = 0, TypeConcatenate = 1 } Type;
...
public:
  ...
  static SemiringCombinationHelperRef
  create(Type comboType, const KeyList &comboScoreKeys,
         const ScoreList &comboScales = ScoreList());
};
...
protected:
  CombinationHelper(SemiringCombinationHelper::Type comboType,
                    const KeyList &comboKeys, const ScoreList &comboScales);
...
public:
  ...
  ConstSemiringRef semiring() const { return semiringCombination_->semiring(); }

  SemiringCombinationHelperRef semiringCombination() const {
    return semiringCombination_;
  }

  static CombinationHelperRef create(SemiringCombinationHelper::Type comboType,
