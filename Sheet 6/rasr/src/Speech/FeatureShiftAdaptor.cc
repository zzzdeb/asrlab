...
const Core::ParameterString
    FeatureShiftAdaptor::paramCorpusKeyMap("corpus-key-map",
                                           "path of corpus-key-map file");

const Core::ParameterString
    FeatureShiftAdaptor::paramKey("key", "key for adaptor selection");
...
FeatureShiftAdaptor::FeatureShiftAdaptor(const Core::Configuration &c)
  ...
  adaptationTree_ = Core::ref(new Am::AdaptationTree(
      select("mllr-tree"),
      (dynamic_cast<const Am::ClassicAcousticModel &>(*acousticModel_))
          .stateModel(),
...
bool FeatureShiftAdaptor::configure() {
  ...
  if (!configureDatatype(attributesSelection,
                         Flow::DataAdaptor<Alignment>::type()))
    return false;
...
bool FeatureShiftAdaptor::work(Flow::PortId p) {
  ...
  while (getData(0, in)) {
    ...
    if (featureIndex_ >= alignment.size())
      criticalError("Input stream (%d) is longer than alignment (%zd).",
                    featureIndex_ + 1, alignment.size());
    Mm::MixtureIndex mixture =
        acousticModel_->emissionIndex(alignment[featureIndex_++].emission);

    Math::Vector<Mm::FeatureType> inVec(
        Flow::Vector<Mm::FeatureType>((*in.get())));
    Flow::Vector<Mm::FeatureType> *out = new Flow::Vector<Mm::FeatureType>(
        inVec - (static_cast<Mm::ShiftAdaptor *>(currentAdaptor_.get())
                     ->shiftVector(mixture)));
...
bool FeatureShiftAdaptor::setKey(const std::string &key) {
  ...
  if (!adaptorCache_.findForReadAccess(currentMappedKey_)) {
    if (!adaptorEstimatorCache_.findForReadAccess(currentMappedKey_)) {
      ...
      criticalError("No adaptation data seen for key ")
          << currentMappedKey_ << ". Exiting!\n";
    } else {
...
bool FeatureShiftAdaptor::setParameter(const std::string &name,
