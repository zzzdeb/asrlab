...
void mapAlphabet(ConstAlphabetRef from, ConstAlphabetRef to,
                 AlphabetMapping &mapping, LabelId unknownId,
                 u32 maximumUnknownsReported) {
  u32 nUnknowns = 0;
      ...
      if (id != InvalidLabelId) {
        ...
        if (nUnknowns < maximumUnknownsReported && maximumUnknownsReported > 0)
          Core::Application::us()->warning(
              ...
              (*i).c_str());
        else if (nUnknowns == maximumUnknownsReported &&
                 maximumUnknownsReported > 0)
          Core::Application::us()->warning(
  ...
  if (nUnknowns > 0 && maximumUnknownsReported > 0)
    Core::Application::us()->warning(
        "Fsa::mapAlphabet: %d symbols were mapped to \"%s\"", nUnknowns,
        ((unknownId == InvalidLabelId) ? "*INVALID*"
                                       : to->symbol(unknownId).c_str()));
}
} // namespace Fsa
