...
const Core::ParameterString NodeBuilder::paramNetworkFileExtension(
    "network-file-extension", "default network file extension", ".flow");

const Core::ParameterString
    NodeBuilder::paramNetworkFilePath("network-file-path",
                                      "path of network files", ".");

/*****************************************************************************/
NetworkParser::NetworkParser(Network &n, const Core::Configuration &c)
    : XmlSchemaParser(c), network(n), nodeBuilder_(c)
/*****************************************************************************/
{
  XmlElement *parameter_decl = collect(new XmlEmptyElementRelay(
      "param", this, startHandler(&Self::start_parameter)));
  XmlElement *input_decl = collect(
      new XmlEmptyElementRelay("in", this, startHandler(&Self::start_input)));
  XmlElement *output_decl = collect(
      new XmlEmptyElementRelay("out", this, startHandler(&Self::start_output)));
  XmlElement *node_decl = collect(
      new XmlEmptyElementRelay("node", this, startHandler(&Self::start_node)));
  XmlElement *link_decl = collect(
      new XmlEmptyElementRelay("link", this, startHandler(&Self::start_link)));
  XmlElement *networknode_decl = collect(new XmlMixedElementRelay(
      "network-node", this, startHandler(&Self::start_networknode),
      endHandler(&Self::end_networknode), 0, XML_CHILD(parameter_decl),
      XML_CHILD(input_decl), XML_CHILD(output_decl), XML_CHILD(node_decl),
      XML_CHILD(link_decl), XML_NO_MORE_CHILDREN));
  XmlElement *network_decl = collect(new XmlMixedElementRelay(
      "network", this, startHandler(&Self::start_network),
      endHandler(&Self::end_network), 0, XML_CHILD(parameter_decl),
      XML_CHILD(input_decl), XML_CHILD(output_decl), XML_CHILD(node_decl),
      XML_CHILD(link_decl), XML_CHILD(networknode_decl), XML_NO_MORE_CHILDREN));
...
/*****************************************************************************/
{
  ...
  NetworkTemplate::NodeAttributes &nodeAttributes =
      nodeBuilder_.currentTemplate().addNodeAttributes(name, filter);
...
/*****************************************************************************/
{
  ...
  // add link to network
  bool status;
  status = nodeBuilder_.currentTemplate().addLink(
      from_node, from_port, to_node, to_port, (buffer ? atoi(buffer) : 0));
  if (!status)
...
/*****************************************************************************/
Network *NodeBuilder::createNetworkNode(const std::string &name,
...
/*****************************************************************************/
Node *NodeBuilder::createFilterNode(const std::string &name,
...
/*****************************************************************************/
bool NodeBuilder::findNetworkFile(const std::string &pathname,
                                  std::string &foundPathname) const
/*****************************************************************************/
{
...
/*****************************************************************************/
void NodeBuilder::addNetworkTemplate(const std::string &filename,
...
/*****************************************************************************/
AbstractNode *
NodeBuilder::createNode(const NetworkTemplate::NodeAttributes &nodeAttributes)
/*****************************************************************************/
{
  using namespace std;
  ...
  error(
      "Could not find filter \"%s\" (neither as built-in nor as network file).",
      filterName.c_str());
  return result;
}
...
/*****************************************************************************/
{
  for (AbstractNode::UnresolvedAttributes::const_iterator attr =
           node->unresolvedAttributes().begin();
    ...
    if ((parameterName != "name") && (parameterName != "filter")) {
      ...
      if (!parameterValueExpression.isConstant()) {
        ...
        bool res = network.addParameterUse(node, parameterName,
      ...
      if (!node->addParameter(parameterName, parameterValueExpression)) {
        error("Failed to add parameter in \"%s\"=\"%s\"", parameterName.c_str(),
