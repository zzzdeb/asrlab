...
// -------------------------------------------------------------------------
ConstLatticeRef changeSemiring(ConstLatticeRef l,
                               ConstSemiringRef targetSemiring) {
...
ConstLatticeRef rescale(ConstLatticeRef l, const ScoreList &scales,
                        const KeyList &keys) {
  return changeSemiring(l, rescaleSemiring(l->semiring(), scales, keys));
}

ConstLatticeRef rescale(ConstLatticeRef l, ScoreId id, Score scale,
                        const Key &key) {
...
public:
  ProjectSemiringLattice(ConstLatticeRef l, ConstSemiringRef targetSemiring,
                         const ProjectionMatrix &mapping)
    ...
    for (ProjectionMatrix::const_iterator itScales = mapping_.begin();
         itScales != mapping_.end(); ++itScales)
      verify(itScales->size() == sourceSemiring->size());
  }
  virtual ~ProjectSemiringLattice() {}

  virtual ConstSemiringRef semiring() const { return semiring_; }
  ...
  virtual std::string describe() const {
    return Core::form("projectSemiring(%s;%s,%s)", fsa_->describe().c_str(),
                      fsa_->semiring()->name().c_str(),
                      semiring_->name().c_str());
  }
};
ConstLatticeRef projectSemiring(ConstLatticeRef l,
                                ConstSemiringRef targetSemiring,
                                const ProjectionMatrix &mapping) {
  return ConstLatticeRef(
      new ProjectSemiringLattice(l, targetSemiring, mapping));
}
// -------------------------------------------------------------------------
...
struct FsaWeightToScores {
  ...
  ScoreId id;
  FsaWeightToScores(ConstSemiringRef semiring, ScoresRef defaultScore,
                    ScoreId id)
...
ConstLatticeRef fromFsa(Fsa::ConstAutomatonRef f, ConstSemiringRef semiring,
                        ScoreId id, ScoresRef defaultScore) {
...
public:
  FsaVectorLattice(const std::vector<Fsa::ConstAutomatonRef> &fsas,
                   ConstSemiringRef semiring)
      : Precursor(), fsas_(fsas), semiring_(semiring), tmpStates_(0),
        tmpArcIterators_(0) {
  ...
  virtual Fsa::Type type() const { return fsas_.front()->type(); }
  ...
  virtual ConstStateRef getState(Fsa::StateId sid) const {
    ...
    std::vector<Fsa::State::const_iterator>::iterator itA =
        tmpArcIterators_.begin();
    for (std::vector<Fsa::ConstAutomatonRef>::const_iterator itFsa =
             fsas_.begin();
         itFsa != fsas_.end(); ++itFsa, ++itSr, itA++) {
    ...
    if (tmpStates_.front()->isFinal()) {
      ...
      for (std::vector<Fsa::ConstStateRef>::const_iterator itSr =
               tmpStates_.begin();
           itSr != tmpStates_.end(); ++itSr, ++itScore)
        *itScore = Score((*itSr)->weight());
    }
    for (Fsa::State::const_iterator a = tmpArcIterators_.front(),
                                    a_end = tmpStates_.front()->end();
         a != a_end; ++a) {
      ScoresRef scores = semiring_->create();
      sp->newArc(a->target(), scores, a->input(), a->output());
      Scores::iterator itScore = scores->begin();
      for (std::vector<Fsa::State::const_iterator>::iterator itA =
               tmpArcIterators_.begin();
           itA != tmpArcIterators_.end(); ++itA, ++itScore) {
...
ConstLatticeRef fromFsaVector(const std::vector<Fsa::ConstAutomatonRef> &fsas,
                              ConstSemiringRef semiring) {
  if (fsas.empty())
    return ConstLatticeRef();
...
struct FsaWeightToConstant {
  ...
  ScoresRef operator()(const Fsa::Weight &w) const { return constScore; }
};
ConstLatticeRef fromUnweightedFsa(Fsa::ConstAutomatonRef f,
                                  ConstSemiringRef semiring,
                                  ScoresRef defaultScore) {
  Fsa::SemiringType type = Fsa::getSemiringType(f->semiring());
  if ((type != Fsa::SemiringTypeTropical) && (type != Fsa::SemiringTypeLog))
    Core::Application::us()->warning(
        "In \"fromUnweightedFsa\": expected tropical or log semiring, found %s "
        "semiring.",
        f->semiring()->name().c_str());
  if (!defaultScore)
    defaultScore = semiring->one();
...
Fsa::ConstAutomatonRef toFsa(ConstLatticeRef l, const ScoreList &scales) {
  ...
  require(semiring);
  return Ftl::convert<Lattice, Fsa::Automaton, LinearCombination>(l, semiring,
                                                                  conv);
}
...
std::vector<Fsa::ConstAutomatonRef> toFsaVector(ConstLatticeRef l,
                                                bool scaled) {
  ...
  for (ScoreId id = 0; id < l->semiring()->size(); ++id) {
    Projection conv(id, (scaled ? l->semiring()->scale(id) : 1.0));
    fsas[id] =
        Ftl::convert<Lattice, Fsa::Automaton, Projection>(l, semiring, conv);
  }
  return fsas;
}
...
Fsa::ConstAutomatonRef toUnweightedFsa(ConstLatticeRef l,
                                       const Fsa::Weight &constWeight) {
...
public:
  FindTopologicalOrder(ConstLatticeRef l, Core::Ref<StateMap> map)
    ...
    if (time_ > 0) {
      if (!isCyclic()) {
        --time_;
        for (StateMap::iterator itOrder = map_->begin(); itOrder != map_->end();
             ++itOrder)
...
ConstStateMapRef findTopologicalOrder(ConstLatticeRef l) {
  ...
  if (!l->getTopologicalSort()) {
    ...
    if (topologicalSort.maxSid == Fsa::InvalidStateId) {
      ...
      map = Core::Ref<StateMap>(
          new StateMap(topologicalSort.maxSid + 1, Fsa::InvalidStateId));
...
struct ChronologicalWeakOrder {
  ...
  ChronologicalWeakOrder(const Boundaries &boundaries,
                         const StateMap &topologicalOrder)
...
ConstStateMapRef sortChronologically(ConstLatticeRef l) {
  ...
  // a stable sort w/o taking the topological order (explicitly) into account
...
public:
  ...
  public:
    TopologicalOrderBoundaries(ConstBoundariesRef boundaries,
                               ConstStateMapRef topologicalSort)
        : boundaries_(boundaries), topologicalSort_(topologicalSort) {}
    virtual ~TopologicalOrderBoundaries() {}
    bool valid() const { return boundaries_->valid(); }
...
public:
  TopologicalOrderLattice(ConstLatticeRef l) : Precursor(l) {
    ...
    verify(((*topologicalSort_)[0] == fsa_->initialStateId()) &&
           ((*topologicalOrder_)[fsa_->initialStateId()] == 0));
    setBoundaries(ConstBoundariesRef(new TopologicalOrderBoundaries(
        fsa_->getBoundaries(), topologicalOrder_)));
  }
  virtual ~TopologicalOrderLattice() {}
  Fsa::StateId initialStateId() const { return 0; }
...
// -------------------------------------------------------------------------
bool isAcyclic(ConstLatticeRef l) { return FtlWrapper::isAcyclic(l); }
...
protected:
  ScoresRef concatenate(const Scores &scores1, const Scores &scores2) const {
    ScoresRef scores = semiring_->create();
    Scores::iterator itScore = scores->begin();
    for (Scores::const_iterator itScore1 = scores1.begin(),
                                endScore1 = scores1.begin() + n1_;
         itScore1 != endScore1; ++itScore1, ++itScore)
      *itScore = *itScore1;
    for (Scores::const_iterator itScore2 = scores2.begin(),
                                endScore2 = scores2.begin() + n2_;
...
public:
  ScoreAppendLattice(ConstLatticeRef l1, ConstLatticeRef l2,
                     ConstSemiringRef semiring)
    ...
    if (!semiring) {
      ...
      scales.insert(scales.end(), semiring2.scales().begin(),
                    semiring2.scales().end());
  ...
  ConstStateRef getState(Fsa::StateId sid) const {
    ...
    State::const_iterator a_append = appendSp->begin(),
                          end_append = appendSp->end();
  ...
  std::string describe() const {
    return Core::form("append-score(%s,%s)", fsa_->describe().c_str(),
                      appendL_->describe().c_str());
  }
};
ConstLatticeRef appendScores(ConstLatticeRef l1, ConstLatticeRef l2,
                             ConstSemiringRef semiring) {
