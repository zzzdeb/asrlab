...
const Core::ParameterFloat
    AbstractMixtureSetEstimator::paramMinObservationWeight(
        "minimum-observation-weight",
        "densities with means of less observation weight get removed", 5, 0);

const Core::ParameterFloat AbstractMixtureSetEstimator::paramMinRelativeWeight(
    "minimum-relative-weight",
    "mixture densities with less relative weight get removed", 0, 0);

const Core::ParameterFloat AbstractMixtureSetEstimator::paramMinVariance(
    "minimum-variance",
    "variances lower will be rounded up to minimum-variance", 0, 0);

const Core::Choice
    AbstractMixtureSetEstimator::choiceMode("viterbi", modeViterbi,
                                            "baum-welch", modeBaumWelch,
                                            Core::Choice::endMark());

const Core::ParameterChoice AbstractMixtureSetEstimator::paramMode(
    "mode", &choiceMode,
    "describes how posterior probabilities of the mixture densities are "
    "comuted",
    modeViterbi);

const Core::ParameterBool AbstractMixtureSetEstimator::paramAllowZeroWeights(
    "allow-zero-weights",
    "allow mixtures with zero observations, needed for mixture specific "
    "training",
    false);

const Core::ParameterBool
    AbstractMixtureSetEstimator::paramNormalizeMixtureWeights(
        "normalize-mixture-weights", "mixture weights are normalized", true);

AbstractMixtureSetEstimator::AbstractMixtureSetEstimator(
    const Core::Configuration &c)
    : Precursor(c), viterbi_(paramMode(c) == modeViterbi),
      ...
      accumulationChannel_(c, "dump-accumulation-statistics"), dimension_(0),
      version_(2) {}

void AbstractMixtureSetEstimator::setTopology(
    Core::Ref<const MixtureSet> topology) {
  require(topology);
  ...
  for (MixtureIndex m = 0; m < mixtureEstimators_.size(); ++m)
    mixtureEstimators_[m] =
        Core::Ref<AbstractMixtureEstimator>(createMixtureEstimator());

  std::vector<Core::Ref<GaussDensityEstimator> > densityEstimators(
      topology->nDensities());
  for (DensityIndex i = 0; i < densityEstimators.size(); ++i)
    densityEstimators[i] =
        Core::ref(createDensityEstimator(*topology->density(i)));

  std::vector<Core::Ref<AbstractMeanEstimator> > meanEstimators(
      topology->nMeans());
  for (MeanIndex i = 0; i < meanEstimators.size(); ++i)
    meanEstimators[i] = Core::ref(createMeanEstimator(*topology->mean(i)));

  std::vector<Core::Ref<AbstractCovarianceEstimator> > covarianceEstimators(
      topology->nCovariances());
  for (CovarianceIndex i = 0; i < covarianceEstimators.size(); ++i)
    covarianceEstimators[i] =
        Core::ref(createCovarianceEstimator(*topology->covariance(i)));
  ...
  for (MixtureIndex m = 0; m < mixtureEstimators_.size(); ++m) {
    ...
    for (DensityIndex dns = 0; dns < mixture->nDensities(); ++dns) {
      ...
      const GaussDensity *density =
          topology->density(mixture->densityIndex(dns));

      densityEstimator->setMean(meanEstimators[density->meanIndex()]);
      densityEstimator->setCovariance(
          covarianceEstimators[density->covarianceIndex()]);
...
void AbstractMixtureSetEstimator::accumulate(
  ...
  if (viterbi_) {
    ...
    getDensityPosteriorProbabilities(mixtureIndex, featureVector,
                                     dnsPosteriors);
    weightStats_ += weight;
...
bool AbstractMixtureSetEstimator::accumulate(
    const AbstractMixtureSetEstimator &toAdd) {
  ...
  verify(indexMaps.covarianceMap().size() ==
         indexMapsToAdd.covarianceMap().size());
  for (CovarianceIndex i = 0; i < indexMaps.covarianceMap().size(); ++i)
    indexMaps.covarianceMap()[i]->accumulate(
        *indexMapsToAdd.covarianceMap()[i]);
...
bool AbstractMixtureSetEstimator::accumulate(Core::BinaryInputStreams &is,
  ...
  for (u32 n = 0; n < is.size(); ++n) {
    ...
    if (strcmp(_magic, magic().c_str()) != 0) {
      criticalError("Mixture set estimator file with magic \"")
          << _magic << "\" could not be read, \"" << magic() << "\" expected.";
      return false;
    }
    u32 _version;
    is[n] >> _version;
  ...
  os << (u32)nCovariances;
  Core::Ref<AbstractCovarianceEstimator> covarianceEstimator(
      createCovarianceEstimator());
  Core::Ref<AbstractCovarianceEstimator> _covarianceEstimator(
      createCovarianceEstimator());
...
void AbstractMixtureSetEstimator::estimate(MixtureSet &toEstimate) {
  ...
  /* Store mean estimators (with non-zero weight) necessary for covariance
   * estimation since some may get removed in the next step. */
  ...
  MixtureSetEstimatorIndexMap indexMaps(*this);
  for (MixtureIndex i = 0; i < mixtureEstimators_.size(); ++i)
    toEstimate.addMixture(
        i, mixtureEstimators_[i]->estimate(indexMaps.densityMap(),
                                           normalizeMixtureWeights_));

  for (DensityIndex i = 0; i < indexMaps.densityMap().size(); ++i)
    toEstimate.addDensity(
        i, indexMaps.densityMap()[i]->estimate(indexMaps.meanMap(),
  ...
  for (CovarianceIndex i = 0; i < indexMaps.covarianceMap().size(); ++i) {
    toEstimate.addCovariance(
        i, indexMaps.covarianceMap()[i]->estimate(meanSetMap, minVariance_));
  }

  log("Mixture set estimated:\n") << "#densities " << toEstimate.nDensities();
}

void AbstractMixtureSetEstimator::estimate(
    MixtureSet &toEstimate, std::vector<Weight> &meanObservationWeights,
...
void AbstractMixtureSetEstimator::removeDensitiesWithZeroWeight() {
  for (MixtureIndex mixture = 0; mixture < mixtureEstimators_.size();
       ++mixture) {
...
void AbstractMixtureSetEstimator::read(Core::BinaryInputStream &is) {
  ...
  log("Mixture set estimator read:\n")
      << "    #mixtures " << nMixtures << ", #densities " << nDensities << ",\n"
      << "    #means " << nMeans << ", #covariances " << nCovariances << ",\n"
      << "    "
...
void AbstractMixtureSetEstimator::write(Core::BinaryOutputStream &os) {
  ...
  for (DensityIndex i = 0; i < indexMaps.densityMap().size(); ++i)
    indexMaps.densityMap()[i]->write(os, indexMaps.meanMap(),
                                     indexMaps.covarianceMap());
  ...
  log("Mixture set estimator saved:\n")
      << "    #mixtures " << mixtureEstimators_.size() << ", #densities "
      << indexMaps.densityMap().size() << ",\n"
      << "    #means " << indexMaps.meanMap().size() << ", #covariances "
      << indexMaps.covarianceMap().size() << ",\n"
      << "    dimension " << dimension_;
}
...
bool AbstractMixtureSetEstimator::equalTopology(
    const AbstractMixtureSetEstimator &toCompare) const {
  ...
  if (mixtureEstimators_.size() != toCompare.mixtureEstimators_.size())
    return false;
  for (MixtureIndex i = 0; i < mixtureEstimators_.size(); ++i) {
    if (!mixtureEstimators_[i]->equalTopology(*toCompare.mixtureEstimators_[i],
                                              indexMaps.densityMap(),
                                              indexMapsToCompare.densityMap()))
      return false;
  }
  ...
  for (DensityIndex i = 0; i < indexMaps.densityMap().size(); ++i) {
    if (!indexMaps.densityMap()[i]->equalTopology(
            *indexMapsToCompare.densityMap()[i], indexMaps.meanMap(),
            indexMaps.covarianceMap(), indexMapsToCompare.meanMap(),
            indexMapsToCompare.covarianceMap()))
      return false;
  }
  ...
  if (indexMaps.covarianceMap().size() !=
      indexMapsToCompare.covarianceMap().size())
    return false;

  return true;
}

bool AbstractMixtureSetEstimator::operator==(
    const AbstractMixtureSetEstimator &toCompare) const {
  ...
  verify(indexMaps.covarianceMap().size() ==
         indexMapsToCompare.covarianceMap().size());
  for (CovarianceIndex i = 0; i < indexMaps.covarianceMap().size(); ++i) {
    if ((*indexMaps.covarianceMap()[i]) !=
        (*indexMapsToCompare.covarianceMap()[i]))
      return false;
  }
  ...
  for (MixtureIndex i = 0; i < mapping.size(); ++i) {
    ...
    for (; m != mapping[i].end(); ++m) {
      if (m == mapping[i].begin()) {
        ...
        mixtureEstimators_[i] = Core::Ref<AbstractMixtureEstimator>(
            new AbstractMixtureEstimator(*toMap[*m]));
      } else {
...
bool AbstractMixtureSetEstimator::changeDensityMixtureAssignment(
    std::vector<std::vector<DensityIndex> > stateDensityTable) {

  MixtureSetEstimatorIndexMap indexMaps(*this);

  bool normalizeWeights_ = true;
  std::vector<std::vector<MixtureIndex> > densityStateTable(
      indexMaps.densityMap().size());
  if (normalizeWeights_)
    for (MixtureIndex m = 0; m < stateDensityTable.size(); ++m)
      for (std::vector<DensityIndex>::const_iterator di =
               stateDensityTable[m].begin();
           di != stateDensityTable[m].end(); ++di)
...
bool AbstractMixtureSetEstimator::accumulateDensity(
    Core::BinaryInputStreams &is, Core::BinaryOutputStream &os) {
...
bool AbstractMixtureSetEstimator::addMixtureEstimators(
    const AbstractMixtureSetEstimator &estimator) {
  ...
  if (mixtureEstimators_.size() == 0) {
    ...
    for (Mm::MixtureIndex m = 0; m < mixtureEstimators_.size(); ++m)
      mixtureEstimators_[m] =
          Core::Ref<Mm::AbstractMixtureEstimator>(new Mm::MixtureEstimator);
  }
  for (Mm::MixtureIndex m = 0; m < mixtureEstimators_.size(); ++m) {
    ...
    if (weight > 0) {
      if (mixture->getWeight() > 0) {
        ...
        std::copy(mixtureToAdd->weights().begin(),
                  mixtureToAdd->weights().end(), mixture->weights().begin());
      }
    }
  }
  return true;
...
bool Mm::AbstractMixtureSetEstimator::
    createMixtureSpecificCovarianceEstimator() {
  /*! @todo diagonal covariance should not be hardcoded! */
  Covariance *cov = new DiagonalCovariance(dimension_);

  for (MixtureIndex m = 0; m < mixtureEstimators_.size(); ++m) {
    Core::Ref<AbstractCovarianceEstimator> covariance(
        createCovarianceEstimator(*cov));
    MixtureEstimator::DensityEstimators::const_iterator density =
        mixtureEstimators_[m]->densityEstimators().begin();
    for (; density != mixtureEstimators_[m]->densityEstimators().end();
         ++density) {
...
bool MixtureToMixtureMap::load(const std::string &filename) {
  Core::Application::us()->log(
      "mixture-to-mixture mapping: loading mapping from file \"%s\" ...",
      filename.c_str());
  if (filename.empty())
    return false;
  ...
  while (Core::getline(is, line) != EOF) {
    ...
    for (u32 i = 1; i < tokens.size(); ++i) {
      ...
      // Core::Application::us()->log("mixture-to-mixture mapping:") << front <<
      // " to "  << m;
    }
  }
  nOfMappedMixtures_ = mappedMixtures_.nMixtures();
  Core::Application::us()->log(
      "mixture-to-mixture mapping: mapped %d mixtures to %d mixtures.",
      nMixtures(), nOfMappedMixtures_);
  return true;
}
...
MixtureSetEstimatorIndexMap::MixtureSetEstimatorIndexMap(
    const AbstractMixtureSetEstimator &estimator) {
