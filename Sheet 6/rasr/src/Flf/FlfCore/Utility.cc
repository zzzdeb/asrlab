...
struct TropicalCollector : public Collector {
  ...
  virtual void reset() { score = Semiring::Zero; }
  virtual void feed(f64 a) {
    if (a < score)
      score = a;
  }
  virtual f64 get() const { return score; }
};
...
struct LogCollector : public Collector {
  ...
  virtual f64 get() const {
    if (scores.size() <= 1)
      return min;
    f64 sum = 0.0;
    for (f64List::const_iterator it = scores.begin() + 1; it != scores.end();
         ++it)
      sum += ::exp(min - *it);
...
// -------------------------------------------------------------------------
void CostCollector::reset() { expectedCosts_.clear(); }

void CostCollector::feed(f64 score, f64 cost) {
  if (cost != 0.0)
    expectedCosts_.push_back((cost >= 0.0)
...
f64 CostCollector::get(f64 norm) const {
  f64 cost = 0.0;
  for (CostList::const_iterator it = expectedCosts_.begin(),
                                end = expectedCosts_.end();
       it != end; ++it)
    cost += (it->first ? ::exp(it->second + norm) : -::exp(it->second + norm));
  return cost;
}

CostCollector *CostCollector::create() { return new CostCollector; }
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
TextFileParser::TextFileParser(const std::string &filename,
