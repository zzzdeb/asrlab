 ...
 *  TypedAggregate<T> collects objects strictly of the same type coming from
 * different streams. Note: -It does not support polymoric I/O. -Aggregate is a
 * general implementation of the same problem supporting polymorfism. Advantage
 * of this class compared to Aggregate is that the type of the elements in
 * known. Thus there is no need for type checking when accessing the elements.
 */
template <class T>
...
template <class T> const Datatype *TypedAggregate<T>::type() {
  static Core::NameHelper<Self> name;
  static DatatypeTemplate<Self> dt(name);
  return &dt;
}

template <class T> bool TypedAggregate<T>::read(Core::BinaryInputStream &is) {
  Timestamp::read(is);
  u32 s;
  is >> s;
  this->resize(s);
  for (typename PrecursorVector::iterator i = this->begin(); i != this->end();
       ++i) {
...
bool TypedAggregate<T>::write(Core::BinaryOutputStream &os) const {
  Timestamp::write(os);
  os << (u32)this->size();
  for (typename PrecursorVector::const_iterator i = this->begin();
       i != this->end(); ++i)
    (*i)->write(os);
  return os;
}
 ...
 *  All input packets must be derived from type TypedAggregate<T>::DataType,
 * which will be aggregated, e.g. their references will be collected in a
 * vector.
 */
template <class T>
class TypedAggregateNode
    : public MergerNode<typename TypedAggregate<T>::DataType,
                        TypedAggregate<T> > {
  typedef MergerNode<typename TypedAggregate<T>::DataType, TypedAggregate<T> >
      Precursor;
...
public:
  ...
  virtual TypedAggregate<DataType> *
  merge(std::vector<DataPtr<DataType> > &inputData) {
    return new TypedAggregate<DataType>(inputData);
  }
};

template <class T> class TypedDisaggregateNode : public Node {
...
template <class T> bool TypedDisaggregateNode<T>::configure() {
...
template <class T> bool TypedDisaggregateNode<T>::work(PortId) {
  DataPtr<TypedAggregate<DataType> > in;
  DataPtr<DataType> out;
  if (!getData(0, in)) {
    for (OutputMap::const_iterator i = outputs_.begin(); i != outputs_.end();
         ++i) {
