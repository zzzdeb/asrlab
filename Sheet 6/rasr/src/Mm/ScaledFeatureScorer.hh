...
class ScaledFeatureScorer : public FeatureScorer, public Mc::Component {
public:
 ...
 *  @todo: add scale to the basic feature scorer to remove the additional vtable
 * lookup and function call for each call to score().
 */
...
public:
  ...
  public:
    ...
    virtual const Score getScale() const { return scale_; }

    virtual Scorer getUnscaledScorer() const { return scorer_; }
  };
...
public:
  FeatureScorerScaling(const Core::Configuration &c,
                       Core::Ref<FeatureScorer> featureScorer)
      : Core::Component(c), ScaledFeatureScorer(c),
  ...
  virtual Scorer getScorer(Core::Ref<const Feature> f) const {
    return Scorer(
        new ScaledContextScorer(featureScorer_->getScorer(f), scale()));
  }
  virtual Scorer getScorer(const FeatureVector &featureVector) const {
    return Scorer(new ScaledContextScorer(
        featureScorer_->getScorer(featureVector), scale()));
  }
  Core::Ref<const AssigningFeatureScorer> assigningFeatureScorer() const {
    return Core::Ref<const AssigningFeatureScorer>(
        dynamic_cast<const AssigningFeatureScorer *>(featureScorer_.get()));
  }

  virtual void reset() const { featureScorer_->reset(); }

  virtual void setSegmentName(const std::string name) const {
    featureScorer_->setSegmentName(name);
  }

  virtual void finalize() const { featureScorer_->finalize(); }

  virtual bool isBuffered() const { return featureScorer_->isBuffered(); }
  ...
  virtual Scorer flush() const { return featureScorer_->flush(); }

  virtual bool bufferFilled() const { return featureScorer_->bufferFilled(); }

  virtual bool bufferEmpty() const { return featureScorer_->bufferEmpty(); }

  virtual u32 bufferSize() const { return featureScorer_->bufferSize(); }
};
