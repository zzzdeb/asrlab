...
BayesClassification::BayesClassification(const Core::Configuration &c)
    : Predecessor(c), aprioriProbability_(0), likelihoodFunction_(0),
      nFeatures_(0), delay_(0), nUsedFeatures_(0), useSlidingWindow_(false),
      firstStartTime_(0), lastEndTime_(0), newData_(false), needInit_(true),
...
void BayesClassification::init(size_t inputSize) {
  ...
  if (!likelihoodFunction_->setClasses(classLabels_) ||
      !likelihoodFunction_->setDimension(inputSize))
...
void BayesClassification::feed(const Flow::Vector<Data> &featureVector,
...
bool BayesClassification::getScores(Flow::Vector<Score> &scores) {
  ...
  for (u32 classIndex = 0; classIndex < nClasses(); ++classIndex)
    scores[classIndex] =
        (*aprioriProbability_)[classIndex] + (*likelihoodFunction_)[classIndex];
  return true;
}
...
const std::string &BayesClassification::argMin() const {
  ...
  for (u32 classIndex = 0; classIndex < nClasses(); ++classIndex) {
    ...
    if (useSlidingWindow_) {
      for (SlidingWindow<ScoreVector>::ConstantIterator i =
               scoreWindow_.begin();
...
void BayesClassification::writeStatistics(
    Core::XmlWriter &os, u32 minClassIndex,
    const Core::Statistics<Score> &scoreStatistics) const {
  os << Core::XmlOpen(name() + "-statistics") << Core::XmlOpen("minimum")
     << Core::XmlOpen("class") << classLabels_[minClassIndex]
     << Core::XmlClose("class") << Core::XmlOpen("apriory-score")
     << (*aprioriProbability_)[minClassIndex] << Core::XmlClose("apriory-score")
     << Core::XmlOpen("likelihood-score")
     << (*likelihoodFunction_)[minClassIndex]
     << Core::XmlClose("likelihood-score")
     << Core::XmlOpen("average-likelihood-score")
     << ((*likelihoodFunction_)[minClassIndex] /
         likelihoodFunction_->sumOfWeights())
     << Core::XmlClose("average-likelihood-score") << Core::XmlClose("minimum")
     << scoreStatistics << Core::XmlClose(name() + "-statistics");
}
...
void BayesClassification::setClassLabels(const std::string &fileName) {
  if (fileName != "") {
    Core::XmlVectorDocument<std::string> parser(getConfiguration(),
                                                classLabels_);
    parser.parseFile(fileName.c_str());

    if (classLabels_.empty())
      criticalError("The file \"%s\" did not contain any class labels.",
                    fileName.c_str());
...
void BayesClassification::setAprioriProbability(AprioriProbabilityType type) {
  ...
  case Uniform:
    aprioriProbability_ =
        new UniformAprioriProbability(select("a-priory-probability"));
    break;
  default:
    defect();
  }
...
void BayesClassification::setLikelihoodFunction(LikelihoodFunctionType type) {
  ...
  case IndependentSequence:
    likelihoodFunction_ =
        new IndependentSequenceLikelihood(select("likelihood-function"));
    break;
  default:
    defect();
  }
...
void BayesClassification::setUseSlidingWindow(bool useWindow, int windowLength,
                                              int windowRight) {
...
const Core::Choice BayesClassificationNode::choiceAprioriProbabilityType(
    "uniform", Uniform, Core::Choice::endMark());
const Core::ParameterChoice
    BayesClassificationNode::paramAprioriProbabilityType(
        "a-priory-probability-type", &choiceAprioriProbabilityType,
        "type of apriori probability", Uniform);

const Core::Choice BayesClassificationNode::choiceLikelihoodFunctionType(
    "independent-sequence", IndependentSequence, Core::Choice::endMark());
const Core::ParameterChoice
    BayesClassificationNode::paramLikelihoodFunctionType(
...
const Core::ParameterInt BayesClassificationNode::paramDelay(
    "delay", "continuous classification starts after delay number of features",
    Core::Type<s32>::max, 0);

const Core::ParameterInt BayesClassificationNode::paramNumUsedFeatures(
    "number-of-features", "number of feature vectors used for classification",
    Core::Type<s32>::max, 1);

const Core::ParameterInt BayesClassificationNode::paramWindowLength(
    "window-length",
    "width of the sliding window, -1 to disable usage of sliding window", -1);
...
BayesClassificationNode::BayesClassificationNode(const Core::Configuration &c)
    : Component(c), SleeveNode(c), BayesClassification(c) {
  ...
  setUseSlidingWindow(paramWindowLength(c) > 0, paramWindowLength(c),
                      paramWindowRight(c));
}
...
bool BayesClassificationNode::setParameter(const std::string &name,
  ...
  if (paramClassLabelsFileName.match(name))
    ...
    setAprioriProbability(
        (AprioriProbabilityType)paramAprioriProbabilityType(value));
  else if (paramLikelihoodFunctionType.match(name))
    setLikelihoodFunction(
        (LikelihoodFunctionType)paramLikelihoodFunctionType(value));
    ...
    if (getData(1, featureScoreWeight)) {
      if (featureTimestamp.contains(*featureScoreWeight)) {
        if ((*featureScoreWeight)() >= 0)
          ...
          criticalError("Weight (%f) smaller then zero.",
                        (*featureScoreWeight)());
      }

      criticalError("Timestamp of feature (%f-%f) does not contain the one of "
                    "score-weight (%f-%f)",
                    featureTimestamp.startTime(), featureTimestamp.endTime(),
                    featureScoreWeight->startTime(),
                    featureScoreWeight->endTime());
    }
...
bool BayesClassificationNode::work(Flow::PortId p) {
  ...
  } while (!classify(*classLabel, *featureVector,
                     featureScoreWeight(*featureVector)));
...
const Core::ParameterInt
    BayesClassificationScoreNode::paramNumberOfClasses("number-of-classes",
                                                       "number of classes", 0);

const Core::ParameterBool
    BayesClassificationScoreNode::paramSingleFrameClassification(
        "single-frame-classification", "classify single frames", false);

BayesClassificationScoreNode::BayesClassificationScoreNode(
...
bool BayesClassificationScoreNode::work(Flow::PortId p) {
  ...
  do {
    ...
    classified =
        getScores(*scores, *featureVector, featureScoreWeight(*featureVector));
