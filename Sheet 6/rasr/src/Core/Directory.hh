 ...
 *  If @param path does not have an extenstion @return is @param path without
 * changes.
 */
...
public:
  ...
  struct Filter {
    virtual bool operator()(const std::string &path, const struct stat64 &state,
...
public:
  DirectoryFileIterator(const std::string &path,
                        const Filter *filter = &allFilter);
