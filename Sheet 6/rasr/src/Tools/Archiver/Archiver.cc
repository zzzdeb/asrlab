...
static const Choice modeChoice_("add", Add, "combine", Combine, "copy", Copy,
                                "extract", Extract, "extractAll", ExtractAll,
                                "list", List, "recover", Recover, "remove",
                                Remove, Choice::endMark());
...
static const ParameterString p_select_("select",
                                       "select only entries from file", "");
...
static const Choice overwriteModeChoice("no", overwriteKeepFirst, "keep-first",
                                        overwriteKeepFirst, "yes",
                                        overwriteReplace, "replace",
                                        overwriteReplace, "check",
                                        overwriteCheckEquality, "save",
                                        overwriteCheckEquality,
                                        Core::Choice::endMark());
static const ParameterChoice paramOverwrite(
    "overwrite", &overwriteModeChoice,
    "what to do when an archive member already exists with the same name",
    overwriteCheckEquality);
static const ParameterString
    paramSelect("select",
...
private:
  std::string getUsage() const {
    std::string usage =
        "manipulate SPRINT archives\n"
        "\n"
        "usage: archiver [OPTION] <archive> <FILE>...\n"
        "\n"
        "options:\n"
        "   --compress <bool>\tcompress new files added to the archive\n"
        "   --mode <mode>\tchoose operational mode (see below for available "
        "modes)\n"
        "   --verbose <bool>\tbe a bit more verbose\n"
        "   --quiet <bool>\tless output\n"
        "   --select <file>\tapply operation only to files listed in <file>; "
        "only supported by combine and copy\n"
        ...
        "   copy\t\tcopy files between archives directly (option 'compress' is "
        "ignored)\n"
        ...
        "   check\tcheck for data equality of archive members with the same "
        "name\n"
        "\n";
    return usage;
  }
...
public:
  ...
  bool addFile(Archive *archive, std::istream &src, const std::string &name) {
    ...
    if (archive->hasFile(name)) {
      ...
      case overwriteCheckEquality: {
        ...
        std::cout << ((isEqual) ? "files are equal" : "FILES DIFFER")
                  << std::endl;
        return isEqual;
      } break;
      default:
  ...
  bool compareFileToArchive(Archive *archive, std::istream &src,
                            const std::string &name) {
    Core::ArchiveReader ref(*archive, name);
    if (!ref.isOpen())
      error("could not open member \"%s\" in archive for reading",
            name.c_str());
    while (!src.eof()) {
  ...
  bool writeFileToArchive(Archive *a, std::istream &src,
                          const std::string &archiveName) {
    Core::ArchiveWriter dest(*a, archiveName, compress_);
    if (!dest.isOpen())
      error("could not open member \"%s\" in archive for writing",
            archiveName.c_str());
    while (!src.eof()) {
      char buf[1024];
      src.read(buf, 1024);
  ...
  void addFile(Archive *a, const std::string &name,
               const std::string &archiveName) {
  ...
  void addDirectory(Archive *a, const std::string &name) {
    for (Core::DirectoryFileIterator it(name,
                                        &DirectoryFileIterator::fileFilter);
         it; ++it) {
  ...
  bool copyAllFiles(Archive *a, const std::string &name) {
    Archive *src =
        Core::Archive::create(config, name, Core::Archive::AccessModeRead);
    if (!src) {
    ...
    for (Archive::const_iterator i = src->files(); i; ++i) {
      if (!a->copyFile(*src, i.name())) {
        err = true;
        std::cout << i.name() << ": could not copy file to archive"
                  << std::endl;
      }
    }
    delete src;
  ...
  bool copySelectedFiles(Archive *a, StringVector::const_iterator namesBegin,
                         StringVector::const_iterator namesEnd,
                         Selection &selection) {
    for (StringVector::const_iterator name = namesBegin; name != namesEnd;
         ++name) {
      Archive *src =
          Core::Archive::create(config, *name, Core::Archive::AccessModeRead);
      if (!src) {
    ...
    u32 missing =
        selection.size() - std::count_if(selection.begin(), selection.end(),
      ...
      for (Archive::const_iterator i = src->files(); i; ++i) {
        ...
        if (reader.isOpen()) {
          ...
          if (!addFile(a, reader, i.name()))
            std::cout << i.name() << ": could not add file to archive"
                      << std::endl;
        } else
          error("could not open file '%s' in archive %s for reading",
                i.name().c_str(), name.c_str());
      }
      delete src;
    }
    return true;
  ...
  bool extractFile(Archive *a, const std::string &name,
                   const std::string outputName = "") {
    Core::ArchiveReader src(*a, name);
    if (!src.isOpen())
      error("could not open file '%s' in archive %s for reading", name.c_str(),
            name.c_str());
    respondToDelayedErrors();
    std::string targetName = (outputName.empty() ? name : outputName);
    if (targetName.size() > 3 &&
        targetName.substr(targetName.size() - 3) == ".gz")
  ...
  int main(const std::vector<std::string> &arguments) {
    ...
    case Combine:
      ...
      if (a) {
        ...
        if (selectFile.empty()) {
          for (u32 i = 1; i < arguments.size(); i++) {
            std::cout << "adding contents from archive " << arguments[i]
                      << std::endl;
          ...
          for (u32 i = 1; (i < arguments.size()) && (n > 0); i++) {
            std::cout << "adding selected content from archive " << arguments[i]
                      << std::endl;
            Archive *srcA = Core::Archive::create(
                config, arguments[i], Core::Archive::AccessModeRead);
            if (srcA) {
              for (Selection::iterator it = selection.begin();
                   (it != selection.end()) && (n > 0); ++it) {
                if (!it->second && srcA->hasFile(it->first)) {
                  ...
                  if (reader.isOpen()) {
                    ...
                    error("could not open file '%s' in archive %s for reading",
                          it->first.c_str(), arguments[i].c_str());
                }
              }
              delete srcA;
            }
          ...
          if (n > 0) {
            ...
            for (Selection::const_iterator it = selection.begin();
                 (it != selection.end()) && (n > 0); ++it)
              if (!it->second)
      ...
      if (arguments.size() < 2) {
        ...
        if (a) {
          if (selectionFile.empty()) {
            for (u32 i = 1; i < arguments.size(); ++i) {
              std::cout << "copy all files from " << arguments[i] << " to "
                        << arguments[0] << std::endl;
              copyAllFiles(a, arguments[i]);
            }
          } else {
            Selection selection;
            loadSelection(selectionFile, selection);
            copySelectedFiles(a, arguments.begin() + 1, arguments.end(),
                              selection);
          }
          delete a;
        }
    ...
    case ExtractAll:
      ...
      if (a) {
        ...
        for (Archive::const_iterator i = a->files(); i; ++i) {
          ...
          std::cout << "extracting file " << i.name() << " to " << output
                    << std::endl;
    ...
    case List:
      for (u32 i = 0; i < arguments.size(); i++) {
        Archive *a = Core::Archive::create(config, arguments[i],
                                           Archive::AccessModeRead);
        if (a) {
          if (quiet_) {
            for (Archive::const_iterator i = a->files(); i; ++i)
