...
protected:
   ...
   *  -"relatively-robust" driver chooses optimal drivers depending on the
   * input.
 ...
 */
class SymmetricEigenvalueProblem : public Math::EigenvalueProblem,
                                   public EigenvalueProblem {
private:
   ...
   *  (eigen-value-lower-bound..eigen-value-upper-bound]. Number of calculated
   * eigenvalues is returned in @c nEigenvalues. Eigenvectors are returned in @c
   * eigenvectors, eigenvalues in @c eigenvalues.
   */
  bool dsyevx(Matrix<double> &a, int &nEigenvalues, Vector<double> &eigenvalues,
              Matrix<double> &eigenvectors) const;
  /**
   *  Relatively robust LAPACK driver which solves the symmetric eigenvalue
   * problem. This driver function can calculate a set of eigenvalues defined by
   *  (eigen-value-lower-bound..eigen-value-upper-bound]. Number of calculated
   * eigenvalues is returned in @c nEigenvalues. Eigenvectors are returned in @c
   * eigenvectors, eigenvalues in @c eigenvalues. If shouldSupportEigenvectors()
   * is true, elements of the eigenvectors smaller than a threshold are set to
   * zero. (@see also LAPACK manual variable izuppz).
   */
  bool dsyevr(Matrix<double> &a, int &nEigenvalues, Vector<double> &eigenvalues,
              Matrix<double> &eigenvectors) const;

  bool
  solveWithDivideAndConquerDriver(const Math::Matrix<ValueType> &a,
                                  Math::Vector<ValueType> &eigenvalues,
                                  Math::Matrix<ValueType> &eigenvectors) const;
  /**
   *  Divide and conquer LAPACK driver which solves the symmetric eigenvalue
   * problem. Eigenvectors are returned in @c a, eigenvalues in @c eigenvalues.
   */
...
class GenEigenProblemWithSchurDecomposition
    : public Math::GeneralizedEigenvalueProblem,
      public EigenvalueProblem {
private:
  /**
   ...
   */
  enum BalancingType { noneBalancing, permute, scale, permuteAndScale };
...
private:
   ...
   */
  bool dggevx(Matrix<double> &a, Matrix<double> &b, Vector<double> &realAlphas,
   ...
   */
  void copyAlphas(const Vector<double> &realAlphas,
                  const Vector<double> &imaginaryAlphas,
   ...
   */
  void
  copyEigenvalues(const Math::Vector<std::complex<ValueType> > &alphas,
   ...
   *  Warnings are generated if abs(alpha) < alphaMinimum or abs(beta) <
   * betaMinimum. For these pairs resulting eigenvalue is set to zero.
   */
  std::complex<ValueType>
  calculateEigenvalue(std::complex<ValueType> alpha, ValueType alphaMinimum,
   ...
   */
  void
  copyEigenvectors(const Math::Vector<std::complex<ValueType> > &alphas,
...
private:
  bool solveWithExpertDriver(const Math::Matrix<ValueType> &a,
                             ...
                             Math::Matrix<ValueType> &eigenvectors) const;
  /**
   *  LAPACK driver function which solves the generalized symmetric definite
   * eigenvalue problem. This expert driver function can calculate a set of
   * eigenvalues defined by (eigen-value-lower-bound..eigen-value-upper-bound].
   * Number of calculated eigenvalues is returned in @c nEigenvalues.
   * Eigenvectors are returned in @c eigenvectors, eigenvalues in @c
   * eigenvalues.
   */
  bool dsygvx(Matrix<double> &a, Matrix<double> &b, int &nEigenvalues,
  ...
  /**
   *  LAPACK driver function which solves the generalized symmetric definite
   * eigenvalue problem Eigenvectors are returned in @c a, eigenvalues in @c
   * eigenvalues.
   *
   *  A divide-and-conquer driver (name ending -GVD) solves the same problem as
   * the simple driver. It is much faster than the simple driver for large
   * matrices, but uses more workspace. The name divide-and-conquer refers to
   * the underlying algorithm.
   */
  bool dsygvd(Matrix<double> &a, Matrix<double> &b,
