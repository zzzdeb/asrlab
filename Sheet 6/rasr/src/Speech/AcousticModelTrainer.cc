...
// ===========================================================================
AcousticModelTrainer::AcousticModelTrainer(const Core::Configuration &c,
  ...
  acousticModel_ = Am::Module::instance().createAcousticModel(
      select("acoustic-model"), lexicon_, mode);
  if (!acousticModel_)
...
// ===========================================================================
TextDependentMixtureSetTrainer::TextDependentMixtureSetTrainer(
    const Core::Configuration &c)
    : Core::Component(c),
      AcousticModelTrainer(c, Am::AcousticModel::noEmissions),
      MlMixtureSetTrainer(c), featureDescription_(*this), initialized_(false) {}

void TextDependentMixtureSetTrainer::setFeatureDescription(
...
const Core::ParameterString TdcSumAccumulator::paramSumFile(
    "sum-file", "name of generated sum file to be used with the TDC software");
...
const Core::ParameterStringVector TdcSumAccumulator::paramSumFilesToCombine(
    "sum-files-to-combine", "combine TDC sum files with current accumulator ",
    " ", 0);
...
struct TdcSumAccumulator::Statistics {
  struct Accumulator {
    ...
    Accumulator &operator+=(const Accumulator &accu) {
      verify((sum.size() == accu.sum.size()) &&
             (sumOfSquares.size() == accu.sumOfSquares.size()));
      count += accu.count;
      {
        std::vector<Mm::Sum>::iterator itSum = sum.begin();
        for (std::vector<Mm::Sum>::const_iterator itAdd = accu.sum.begin(),
                                                  endAdd = accu.sum.end();
             itAdd != endAdd; ++itSum, ++itAdd)
          *itSum += *itAdd;
      }
      {
        std::vector<Mm::Sum>::iterator itSumOfSquares = sumOfSquares.begin();
        for (std::vector<Mm::Sum>::const_iterator
                 itAdd = accu.sumOfSquares.begin(),
                 endAdd = accu.sumOfSquares.end();
             itAdd != endAdd; ++itSumOfSquares, ++itAdd)
...
TdcSumAccumulator::TdcSumAccumulator(const Core::Configuration &c)
    : Core::Component(c),
      AcousticModelTrainer(c, Am::AcousticModel::noEmissions |
                                  Am::AcousticModel::noStateTying),
      Legacy::PhoneticDecisionTreeBase(c, acousticModel()->phonemeInventory()) {
  statistics_ = new Statistics();
}

TdcSumAccumulator::~TdcSumAccumulator() { delete statistics_; }

/**
 * \todo estimation sum files for boundary styles other than 'super-pos-dep'
 */

char TdcSumAccumulator::allophoneBoundaryToBoundaryFlag(
    u8 allophoneBoundary) const {
  ...
  case posDep:
    if (allophoneBoundary == Am::Allophone::isWithinPhone) {
      return 'W';
               ...
               (allophoneBoundary == Am::Allophone::isInitialPhone |
                Am::Allophone::isFinalPhone)) {
      return 'X';
    } else {
...
void TdcSumAccumulator::quantizeCountsToIntegers() {
  for (Statistics::AccumulatorMap::iterator i =
           statistics_->accumulators.begin();
       i != statistics_->accumulators.end(); ++i) {
...
void TdcSumAccumulator::reset() { statistics_->accumulators.clear(); }

void TdcSumAccumulator::processAlignedFeature(Core::Ref<const Feature> f,
  ...
  if (w != 0.0) {
    ...
    if (i == statistics_->accumulators.end())
      i = statistics_->accumulators
              .insert(
                  std::make_pair(e, Statistics::Accumulator(feature.size())))
              .first;
    Statistics::Accumulator &accu = i->second;
    accu.count += w;
...
void TdcSumAccumulator::writeSumFile(std::string filename) {
  ...
  Am::ConstAllophoneStateAlphabetRef allophoneStateAlphabet =
      acousticModel()->allophoneStateAlphabet();

  Core::ProgressIndicator p("writing sum file", "phone states");
  p.start(statistics_->accumulators.size());
  for (Statistics::AccumulatorMap::const_iterator i =
           statistics_->accumulators.begin();
       i != statistics_->accumulators.end(); ++i) {
    const Am::AllophoneState allophoneState =
        allophoneStateAlphabet->allophoneState(i->first);
    ...
    os << allophoneState.allophone()->format(pi_) << ' '
       ...
       << ' ' << u32(accu.count) << ":\n";
    for (Mm::ComponentIndex c = 0; c < accu.sum.size(); ++c)
      os << c << ' ' << accu.sum[c] << ' ' << accu.sumOfSquares[c] << '\n';
    p.notify();
  }
  p.finish();
}
...
s32 nextInt(const char *&c) {
  ...
  if (c == n)
    Core::Application::us()->error("Failed to convert integer \"%s\"",
                                   nextString(c).c_str());
  c = n;
  return result;
}
...
void TdcSumAccumulator::addSumFile(const std::string &filename) {
  ...
  Am::ConstAllophoneAlphabetRef allophoneAlphabet =
      acousticModel()->allophoneAlphabet();
  Am::ConstAllophoneStateAlphabetRef allophoneStateAlphabet =
      acousticModel()->allophoneStateAlphabet();

  Statistics::Accumulator accu(0);
  accu.sum.reserve(256);
  accu.sumOfSquares.reserve(256);
  std::pair<Fsa::LabelId, Statistics::Accumulator> allophoneStateAccu =
      std::make_pair(Fsa::InvalidLabelId, Statistics::Accumulator(0));
  ...
  while (!s.empty()) {
    ...
    allophone.boundary = boundaryFlagToAllophoneBoundary(*c);
    ++c;
    ...
    Statistics::Accumulator &target =
        statistics_->accumulators.insert(allophoneStateAccu).first->second;
    if (target.count == 0)
      target = accu;
    else
      target += accu;
