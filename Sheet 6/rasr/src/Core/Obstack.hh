...
template <class T> class Obstack {
public:
  typedef T Item;
...
private:
  struct Chunk {
    ...
    size_t size() const { return tail - data; }

    size_t room() const { return end - tail; }
...
public:
  ...
  void start() { begin_ = current_->tail; }

  Item *currentBegin() const { return begin_; }

  Item *currentEnd() const { return current_->tail; }
...
template <class T>
typename Obstack<T>::Chunk *Obstack<T>::newChunk(Item *begin, Item *end,
...
template <class T> void Obstack<T>::provide_(size_t n) {
  ...
  verify(current_->data <= begin_ && begin_ <= current_->tail &&
         current_->tail < current_->end);

  ensure(current_->room() >= n);
}

template <class T> Obstack<T>::Obstack(size_t chunkCapacity) {
