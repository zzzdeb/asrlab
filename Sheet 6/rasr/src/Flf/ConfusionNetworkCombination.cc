...
typedef std::vector<ConfusionNetworkAlignment *>
    ConfusionNetworkAlignmentPtrList;
...
public:
  ...
  public:
    ...
    virtual ~Cost() {}
    // called before the alignment of ref and hyp starts, i.e. before the first
    // call to the cost functions
    virtual void init(const ConfusionNetwork &ref,
                      const ConfusionNetwork &hyp) const {
      refPtr_ = &ref;
      hypPtr_ = &hyp;
    }
    // called after the alignment, i.e. after the last call to the cost
    // functions
...
private:
  ...
  void allocate(u32 lenRef, u32 lenHyp) const {
    ...
    u32 requiredBeamWidth =
        (lenRef > lenHyp) ? lenRef - lenHyp : lenHyp - lenRef;
    u32 requestedBeamWidth =
        (requiredBeamWidth > minBeamWidth_) ? requiredBeamWidth : minBeamWidth_;
...
public:
  ConfusionNetworkAlignment(ConstCostRef costFcn, u32 minBeamWidth)
      : costFcn_(costFcn), minBeamWidth_(minBeamWidth), beamWidth_(0),
        beamDiameter_(0), maxLen_(0), D_(0), B_(0) {
  ...
  ConstCostRef cost() { return costFcn_; }

  u32 beamWidth() const { return minBeamWidth_; }

  const_iterator begin() const { return revAlignment_.rbegin(); }

  const_iterator end() const { return revAlignment_.rend(); }
  ...
  Score align(const ConfusionNetwork &ref, const ConfusionNetwork &hyp) const {
    ...
    */
    ...
    D_[beamWidth_] = 0.0;
    B_[beamWidth_] = Start;
    for (u32 refIndex = 0; refIndex < std::min(beamWidth_, lenRef);
         ++refIndex) {
      verify(beamWidth_ + 1 + refIndex < beamDiameter_);
      D_[beamWidth_ + 1 + refIndex] =
          D_[beamWidth_ + refIndex] + costFcn.delCost(refIndex);
    ...
    */
    Core::ProgressIndicator pi(
        Core::form("align(#hyp=%d,beam=%d)", lenHyp, beamDiameter_));
    pi.start(lenHyp);
    for (u32 hypIndex = 0, bptrIndex = beamDiameter_; hypIndex < lenHyp;
         ++hypIndex, bptrIndex += beamDiameter_, pi.notify()) {
      /*
        Adjust beam start.
        Insert insertion at bottom of beam, if still in initialization phase of
        beam.
       */
       ...
       */
      for (u32 refIndex = (hypIndex < beamWidth_) ? 0 : hypIndex - beamWidth_,
               refEnd = std::min(hypIndex + beamWidth_ + 1, lenRef);
           refIndex < refEnd; ++refIndex, ++beamIndex) {
        verify(B_[bptrIndex + beamIndex - beamDiameter_] !=
        ...
        if (beamIndex + 1 < beamDiameter_) {
          verify(B_[bptrIndex + beamIndex - (beamDiameter_ - 1)] !=
                 Invalid); // insertion
    ...
    */
    revAlignment_.clear();
    u32 refIndex = lenRef - 1, hypIndex = lenHyp - 1,
...
// -------------------------------------------------------------------------
namespace {
const Core::ParameterInt paramBeamWidth("beam-width", "minimum beam width", 100,
                                        0);
 ...
 * - Provides the dimension storing the posterior prob. dist. for each source
 *lattice/CN, if required
 **/
class WeightedPosteriorCombinationHelper;
typedef Core::Ref<WeightedPosteriorCombinationHelper>
    WeightedPosteriorCombinationHelperRef;
...
public:
  WeightedPosteriorCombinationHelper(const ScoreList &weights,
                                     const KeyList &subPosteriorKeys,
                                     SemiringCombinationHelper::Type comboType,
                                     const KeyList &comboKeys,
                                     const ScoreList &comboScales)
    ...
    for (u32 i = 0; i < weights.size(); ++i) {
      ...
      if (weight > 0.0) {
        ...
        Core::Application::us()->criticalError(
            "Negative weights are not allowed; %.3f < 0.0", weight);
    }
    if (sum == 0.0)
      Core::Application::us()->criticalError(
          "Sum of weights must not be zero.");
    for (ScoreList::iterator itWeight = weights_.begin();
         itWeight != weights_.end(); ++itWeight)
      *itWeight /= sum;
  ...
  void dump(std::ostream &os) const {
    ...
    for (u32 i = 0; i < size(); ++i) {
      ...
      os << Core::form("  %2d. index=%2d, weight=%.3f", (i + 1), indices_[i],
                       weights_[i]);
      if (subPosteriorIds_[i] != Semiring::InvalidId)
        os << Core::form(
            ", posterior-dimension=\"%s\"(d=%d)",
            semiringCombo.subSemiring(i)->key(subPosteriorIds_[i]).c_str(),
            subPosteriorIds_[i]);
      os << std::endl;
    }
    if (semiring())
    ...
    if (Precursor::update(lats)) {
      ...
      for (u32 i = 0; i < subPosteriorKeys_.size(); ++i)
        if (!subPosteriorKeys_[i].empty())
          subPosteriorIds_[i] = SemiringCombinationHelper::getIdOrDie(
              semiringCombo.subSemiring(i), subPosteriorKeys_[i]);
        ...
        if (subPosteriorKeys_[i].empty()) {
          if (cns[i]->isNormalized()) {
            ...
            if (posteriorIdsOrDie)
              Core::Application::us()->criticalError(
                  "Failed to find posterior dimension: Cn is not normalized "
                  "and no posterior dimension is given.");
  ...
  static WeightedPosteriorCombinationHelperRef
  create(const ScoreList &weights, const KeyList &subPosteriorKeys,
         SemiringCombinationHelper::Type comboType, const KeyList &comboKeys,
         const ScoreList &comboScales) {
    return WeightedPosteriorCombinationHelperRef(
        new WeightedPosteriorCombinationHelper(
            weights, subPosteriorKeys, comboType, comboKeys, comboScales));
  }
  ...
  create(const ScoreList &weights, const KeyList &subPosteriorKeys,
    ...
    if (posteriorKey.empty())
      Core::Application::us()->criticalError(
          "Posterior combination requires a valid posterior dimension key.");
    return create(weights, subPosteriorKeys, comboType,
                  KeyList(1, posteriorKey), ScoreList(1, 0.0));
  }
};
const Core::ParameterFloat
    WeightedPosteriorCombinationHelper::paramWeight("weight", "weight", 1.0,
                                                    0.0);
const Core::ParameterString
    WeightedPosteriorCombinationHelper::paramPosteriorKey("posterior-key",
                                                          "posterior key",
                                                          "posterior");
const Core::ParameterString
    WeightedPosteriorCombinationHelper::paramConfidenceKey("confidence-key",
                                                           "confidence key",
                                                           "confidence");
} // namespace
// -------------------------------------------------------------------------
...
typedef Core::Ref<const ConfusionNetworkCombination>
    ConstConfusionNetworkCombinationRef;

class ConfusionNetworkCombination : public ConfusionNetworkAlignment,
...
public:
  ...
  public:
    WeightedCost()
        : refWeight(0.5), hypWeight(0.5), refNorm(1.0), hypNorm(1.0),
    ...
    void setPosteriorIds(ScoreId refPosteriorId, ScoreId hypPosteriorId) {
      verify((refPosteriorId != Semiring::InvalidId) &&
             (hypPosteriorId != Semiring::InvalidId));
  ...
  public:
    ...
    virtual Score insCost(u32 hypId) const {
      ...
      for (ConfusionNetwork::Slot::const_iterator itHyp = hyp.begin(),
                                                  endHyp = hyp.end();
           itHyp != endHyp; ++itHyp)
    ...
    virtual Score delCost(u32 refId) const {
      ...
      Score cost = 0.001; // prefer substitutions rather than del/ins
      for (ConfusionNetwork::Slot::const_iterator itRef = ref.begin(),
                                                  endRef = ref.end();
           itRef != endRef; ++itRef)
  ...
  private:
    Score delInsCost(const ConfusionNetwork::Slot &slot, ScoreId posteriorId,
                     Score slotWeight, Score epsP) const {
      ...
      for (ConfusionNetwork::Slot::const_iterator itSlot = slot.begin() + 1,
                                                  endSlot = slot.end();
      ...
      return std::max(0.0, 1.0 - maxP) +
             0.001; // prefer substitutions rather than del/ins
    }
  ...
  public:
    ...
    virtual Score subCost(u32 refId, u32 hypId) const {
      ...
      for (Fsa::LabelId lastLabel = Core::Type<Fsa::LabelId>::max,
                        nextLabel = itHyp->label;
        ...
        nextLabel =
            (itRef != endRef) ? itRef->label : Core::Type<Fsa::LabelId>::max;
        ...
        nextLabel =
            (itHyp != endHyp) ? itHyp->label : Core::Type<Fsa::LabelId>::max;
      }
      verify((0.0 <= maxP) && (maxP <= 1.001));
      return std::max(0.0, 1.0 - maxP);
    }
...
private:
  ...
  void init(ConfusionNetwork *combinedCn, ConstConfusionNetworkRef cn) const {
    ...
    const SemiringCombinationHelper &semiringCombo =
        *weightedCombo_->semiringCombination();
    ...
    combinedCn->normalizedProperties =
        ConfusionNetwork::ConstNormalizedPropertiesRef(
    ...
    for (ConfusionNetwork::const_iterator itSlot = cn->begin(),
                                          endSlot = cn->end();
         itSlot != endSlot; ++itSlot, ++itCombinedSlot) {

      // dbg("INS " << combinedCn->alphabet->symbol(itSlot->front().label) << "
      // -> " << combinedCn->alphabet->symbol(Fsa::Epsilon));

      itCombinedSlot->resize(itSlot->size());
      ConfusionNetwork::Slot::iterator itCombinedArc = itCombinedSlot->begin();
      for (ConfusionNetwork::Slot::const_iterator itArc = itSlot->begin(),
                                                  endArc = itSlot->end();
           itArc != endArc; ++itArc, ++itCombinedArc) {
        ...
        combinedScores->set(combinedPosteriorId,
                            weight * arc.scores->get(posteriorId));
  ...
  void add(const ConfusionNetworkAlignment &alignment,
           ConfusionNetwork *combinedCn, ConstConfusionNetworkRef cn,
           u32 i) const {
    ...
    ConfusionNetwork::iterator itCombinedSlot = combinedCn->begin();
    for (const_iterator itPair = alignment.begin(), endPair = alignment.end();
         itPair != endPair; ++itPair) {
      if (itPair->first == Core::Type<u32>::max) {
        ...
        if (itCombinedSlot->front().label != Fsa::Epsilon) {
          ...
          itCombinedSlot->insert(
              itCombinedSlot->begin(),
              ConfusionNetwork::Arc(Fsa::Epsilon, combinedScores,
                                    itCombinedSlot->front().begin +
                                        itCombinedSlot->front().duration / 2,
                                    0));
        } else {
          ConfusionNetwork::Arc &combinedArc = itCombinedSlot->front();
          ScoresRef subScores = combinedArc.scores;
          ScoresRef combinedScores = combinedArc.scores =
              combinedSemiring.clone(insOne);
          semiringCombo.set(combinedScores, i, subScores);
          combinedScores->set(combinedPosteriorId,
                              hypWeight * subScores->get(posteriorId) +
                                  refWeight);
          combinedArc.from = combinedArc.to = Core::Type<u32>::max;
        }
        // other arcs
        for (ConfusionNetwork::Slot::iterator
                 itCombinedArc = itCombinedSlot->begin() + 1,
                 endCombinedArc = itCombinedSlot->end();
             itCombinedArc != endCombinedArc; ++itCombinedArc) {
          ConfusionNetwork::Arc &combinedArc = *itCombinedArc;
          ScoresRef subScores = combinedArc.scores;
          ScoresRef combinedScores = combinedArc.scores =
              combinedSemiring.clone(subOne);
          semiringCombo.set(combinedScores, i, subScores);
          combinedScores->set(combinedPosteriorId,
                              hypWeight * subScores->get(posteriorId));
          combinedArc.from = combinedArc.to = Core::Type<u32>::max;
        ...
        // dbg("DEL " << combinedCn->alphabet->symbol(Fsa::Epsilon) << " -> " <<
        // combinedCn->alphabet->symbol(itCombinedSlot->front().label));
        ...
        for (ConfusionNetwork::Slot::const_iterator itArc = itSlot->begin(),
                                                    endArc = itSlot->end();
             itArc != endArc;) {
          for (; (itCombinedArc != combinedSlot.end()) &&
                 (itCombinedArc->label < itArc->label);
               ++itCombinedArc)
            ;
          if ((itCombinedArc != combinedSlot.end()) &&
              (itCombinedArc->label == itArc->label)) {
            ScoresRef combinedScores = itCombinedArc->scores;
            ScoresRef subScores = itArc->scores;
            Score refPosterior = combinedScores->get(combinedPosteriorId),
                  hypPosterior = hypWeight * subScores->get(posteriorId);
            Score sumPosterior = refPosterior + hypPosterior;
            itCombinedArc->begin =
                Time(Core::round((refPosterior * Score(itCombinedArc->begin) +
                                  hypPosterior * Score(itArc->begin)) /
                                 sumPosterior));
            itCombinedArc->duration = Time(
                Core::round((refPosterior * Score(itCombinedArc->duration) +
                             hypPosterior * Score(itArc->duration)) /
                            sumPosterior));
            semiringCombo.set(combinedScores, i, subScores);
          ...
          for (; (itArc != endArc) && ((itCombinedArc == combinedSlot.end()) ||
                                       (itArc->label < itCombinedArc->label));
               ++itArc, ++itCombinedArc) {
            const ConfusionNetwork::Arc &arc = *itArc;
            ScoresRef combinedScores = combinedSemiring.clone(subOne);
            semiringCombo.set(combinedScores, i, arc.scores);
            combinedScores->set(combinedPosteriorId,
                                hypWeight * arc.scores->get(posteriorId));
            itCombinedArc = combinedSlot.insert(
                itCombinedArc, ConfusionNetwork::Arc(arc.label, combinedScores,
                                                     arc.begin, arc.duration));
          }
        }

        // dbg("SUB " << combinedCn->alphabet->symbol(itSlot->front().label) <<
        // " -> " << combinedCn->alphabet->symbol(itCombinedSlot->front().label)
        //<< " " << hypWeight * itSlot->front().scores->get(posteriorId) << " ->
        //" << itCombinedSlot->front().scores->get(combinedPosteriorId));
  ...
  ConfusionNetworkCombination(
      WeightedPosteriorCombinationHelperRef weightedCombo,
      WeightedCostRef weightedCostFcn, u32 minBeamWidth)
      : Precursor(weightedCostFcn, minBeamWidth), weightedCombo_(weightedCombo),
...
public:
  ...
  bool verifyCn(const ConfusionNetwork &cn, Score norm) const {
    for (u32 i = 0; i < cn.size(); ++i) {
      ...
      for (ConfusionNetwork::Slot::const_iterator itPdf = pdf.begin() + 1;
           itPdf != pdf.end(); ++itPdf) {
  ...
  ConstConfusionNetworkRef
  combine(const ConstConfusionNetworkRefList &cns,
          ConfusionNetworkAlignmentPtrList *alignments = 0) const {
    verify(!cns.empty());
    ...
    if (!cns.front()->isNormalized())
      Core::Application::us()->criticalError(
          "Confusion network is not normalized. Confusion network combination "
          "requires normalized CNs.");
    // add proportional posterior mass
    init(combinedCnPtr, cns.front());

    verify(verifyCn(*combinedCnPtr, weightedCombo_->weights().front()));

    ScoreId combinedPosteriorId =
        combinedCnPtr->normalizedProperties->posteriorId;
    ...
    for (u32 i = 1; i < cns.size(); ++i) {
      ...
      if (!cn->isNormalized())
        Core::Application::us()->criticalError(
            "Confusion network is not normalized. Confusion network "
            "combination requires normalized CNs.");
      Score weight = weightedCombo_->weights()[i];
      Score sumWeight = sumWeights_[i] = sumWeights_[i - 1] + weight;
      // set weights such the two aligned CNs make up a slot-wise posterior
      // distribution, i.e. alignment score is normalized
      weightedCostFcn_->setPosteriorIds(combinedPosteriorId,
                                        cn->normalizedProperties->posteriorId);
      weightedCostFcn_->setNorms(sumWeights_[i - 1], 1.0);
      weightedCostFcn_->setWeights(sumWeights_[i - 1] / sumWeight,
                                   weight / sumWeight);
  ...
  static ConstConfusionNetworkCombinationRef
  create(WeightedPosteriorCombinationHelperRef weightedCombo,
         WeightedCostRef weightedCostFcn, u32 minBeamWidth) {
    verify(weightedCombo);
    verify(weightedCostFcn);
    return ConstConfusionNetworkCombinationRef(new ConfusionNetworkCombination(
        weightedCombo, weightedCostFcn, minBeamWidth));
  }
};
// -------------------------------------------------------------------------
...
public:
  typedef enum { ExpectedLoss, ExpectedError } CncCostFcn;
...
private:
  ConstConfusionNetworkRef getCn() {
    if (!cn_) {
      const WeightedPosteriorCombinationHelper::IdList &indices =
          weightedCombo_->indices();
...
public:
  ConfusionNetworkCombinationNode(const std::string &name,
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    for (u32 i = 0; i < n; ++i) {
      ...
      keys[i] =
          WeightedPosteriorCombinationHelper::paramPosteriorKey(cnConfig, "");
    }
    weightedCombo_ = WeightedPosteriorCombinationHelper::create(
        weights, keys,
        SemiringCombinationHelper::getType(
            SemiringCombinationHelper::paramType(select("score-combination"))),
        WeightedPosteriorCombinationHelper::paramPosteriorKey(config,
                                                              "confidence"));
    ...
    cnCombo_ = ConfusionNetworkCombination::create(weightedCombo_, costFcn,
                                                   paramBeamWidth(config));
    cnCombo_->dump(log());
...
const Core::ParameterChoice ConfusionNetworkCombinationNode::paramCncCostFcn(
    "cost", &ConfusionNetworkCombinationNode::choiceCncCostFcn, "cost function",
    ConfusionNetworkCombinationNode::ExpectedError);
NodeRef
createConfusionNetworkCombinationNode(const std::string &name,
                                      const Core::Configuration &config) {
...
class RoverCombination : public ConfusionNetworkAlignment,
                         public Core::ReferenceCounted {
  typedef ConfusionNetworkAlignment Precursor;
...
public:
  /**
     ...
     a distance function defined by static weights, the sclite DP alignment
  module can use two other distance functions.  The first, called Time-Mediated
  alignment and the second called Word-Weight-Mediated alignment.
     ...
     option is exercised and the input formats for both the reference and
  hypothesis files are in "ctm" format.
  ...
  **/
  ...
  public:
    ...
    virtual Score insCost(u32 hypId) const {
      ...
      return ((hypLabel == Fsa::Epsilon) || (hypLabel == nullLabel)) ? 0.001
                                                                     : 3.0;
    }
    ...
    virtual Score delCost(u32 refId) const {
      ...
      for (ConfusionNetwork::Slot::const_iterator itRef = ref.begin(),
        ...
        tmpCost = ((refLabel == Fsa::Epsilon) || (refLabel == nullLabel))
                      ? 0.001
                      : 3.0;
        if (tmpCost < cost)
          cost = tmpCost;
  ...
  **/
  ...
  public:
    ...
    virtual Score insCost(u32 hypId) const {
      ...
      Score cost = (hypArc.label == Fsa::Epsilon)
                       ? 0.002
                       : Score(hypArc.duration) / 100.0 + 0.003;
    ...
    virtual Score delCost(u32 refId) const {
      ...
      for (ConfusionNetwork::Slot::const_iterator itRef = ref.begin(),
        ...
        tmpCost = (refArc.label == Fsa::Epsilon)
                      ? 0.002
                      : Score(refArc.duration) / 100.0 + 0.003;
        if (tmpCost < cost) {
          label = refArc.label;
          cost = tmpCost;
        }
    ...
    virtual Score subCost(u32 refId, u32 hypId) const {
      ...
      if (hypArc.label == Fsa::Epsilon) {
        ...
        for (ConfusionNetwork::Slot::const_iterator itRef = ref.begin(),
          ...
          if (refArc.label != Fsa::Epsilon) {
            Time refBegin = refArc.begin,
                 refEnd = refArc.begin + refArc.duration,
                 hypBegin = hypArc.begin,
                 hypEnd = hypArc.begin + hypArc.duration;
            tmpCost =
                Score(((refBegin < hypBegin) ? hypBegin - refBegin
                                             : refBegin - hypBegin) +
        ...
        if (cost == Infinity)
          cost = insCost(hypId) - 0.001;
        // else  dbg("SUB: " << alphabet->symbol(label)  << ":" <<
        // alphabet->symbol(hypArc.label) << " " << cost);
      }
      return cost;
    }
    ...
    /*
      ...
      Score cost = Infinity, tmpCost = 0.0;
      Fsa::LabelId label = Fsa::InvalidLabelId;
      for (ConfusionNetwork::Slot::const_iterator itRef = ref.begin(), endRef =
      ref.end(); itRef != endRef; ++itRef) { const ConfusionNetwork::Arc &refArc
      = *itRef; if (refArc.label != Fsa::Epsilon) { Time refBegin =
      refArc.begin, refEnd = refArc.begin + refArc.duration, hypBegin =
      hypArc.begin, hypEnd = hypArc.begin + hypArc.duration; tmpCost = Score(
...
private:
   ...
   * Build CN by merging the aligned slots
   **/
  void init(ConfusionNetwork *combinedCn, std::vector<ScoreList> &confidences,
    ...
    combinedCn->alphabet = cn->alphabet;
    combinedCn->semiring = semiringCombo.semiring();
    combinedCn->nBestAlignmentProperties =
        ConfusionNetwork::ConstNBestAlignmentPropertiesRef(
            new ConfusionNetwork::NBestAlignmentProperties(
                weightedCombo_->size()));
  ...
  void add(const ConfusionNetworkAlignment &alignment,
           ConfusionNetwork *combinedCn, std::vector<ScoreList> &confidences,
           ConstConfusionNetworkRef cn, u32 i) const {
    const SemiringCombinationHelper &semiringCombo =
        *weightedCombo_->semiringCombination();
    const Semiring &combinedSemiring = *semiringCombo.semiring();
    ScoresRef subOne = combinedSemiring.one();
    ...
    ConfusionNetwork::Slot insSlot(
        i + 1, ConfusionNetwork::Arc(Fsa::Epsilon, combinedSemiring.one()));
    if (alpha_ != 1.0) {
      const ScoreIdList &posteriorIds = weightedCombo_->posteriorIds();
      for (u32 j = 0; j < i; ++j) {
    ...
    for (const_iterator itPair = alignment.begin(), endPair = alignment.end();
      ...
      if (itPair->first == Core::Type<u32>::max) {
...

        {
          ...
          for (ConfusionNetwork::Slot::iterator
                   itArc = itCombinedSlot->begin(),
                   endArc = itCombinedSlot->begin() + i;
               itArc != endArc; ++itArc) {
        ...
        // dbg("INS" << weightedCombo_->semirings()[i]->describe(arc.scores,
        // Fsa::HintShowDetails | HintUnscaled)
        //<< " -> " <<
        //combinedCn->semiring->describe(itCombinedSlot->back().scores,
        //Fsa::HintShowDetails | HintUnscaled));
        ...
        itCombinedSlot->push_back(ConfusionNetwork::Arc(
            Fsa::Epsilon, combinedSemiring.clone(delOne),
        ...
        semiringCombo.set(combinedScores, i, arc.scores);
        itCombinedSlot->push_back(ConfusionNetwork::Arc(
            arc.label, combinedScores, arc.begin, arc.duration));

        // dbg("SUB" << weightedCombo_->semirings()[i]->describe(arc.scores,
        // Fsa::HintShowDetails | HintUnscaled)
        //<< " -> " <<
        //combinedCn->semiring->describe(itCombinedSlot->back().scores,
        //Fsa::HintShowDetails | HintUnscaled));
    ...
    if (alpha_ != 1.0) {
      ...
      for (const_iterator itPair = alignment.begin(), endPair = alignment.end();
        ...
        if (itPair->first == Core::Type<u32>::max) {
          const ConfusionNetwork::Arc &arc = itSlot->front();
          {
            itConfidences = confidences.insert(itConfidences,
  ...
  /*
    score[i] = alpha * w[i] + (1 - alpha) * (w[i] * c[i] / [ sum_{j=0}^{n} w[j]
    * c[j] ])
  */
  void calcScores(ConfusionNetwork *combinedCn,
                  const std::vector<ScoreList> &confidences) const {
    ScoreId posteriorId =
        weightedCombo_->semiringCombination()->combinationId(0);
    if (alpha_ == 1.0) {
      ScoreList::const_iterator beginWeight = weightedCombo_->weights().begin(),
                                itWeight;
      for (ConfusionNetwork::iterator itSlot = combinedCn->begin(),
                                      endSlot = combinedCn->end();
           itSlot != endSlot; ++itSlot) {
        itWeight = beginWeight;
        for (ConfusionNetwork::Slot::iterator itArc = itSlot->begin(),
                                              endArc = itSlot->end();
             itArc != endArc; ++itArc, ++itWeight)
          itArc->scores->set(posteriorId, *itWeight);
      }
    } else {
      std::vector<ScoreList>::const_iterator itConfidences =
          confidences.begin();
      for (ConfusionNetwork::iterator itSlot = combinedCn->begin(),
                                      endSlot = combinedCn->end();
           itSlot != endSlot; ++itSlot, ++itConfidences) {
        Score norm = 0.0;
        ScoreList::const_iterator beginWeight =
                                      weightedCombo_->weights().begin(),
                                  itWeight;
        ScoreList::const_iterator beginConfidence = itConfidences->begin(),
                                  endConfidence = itConfidences->end(),
                                  itConfidence;
        itWeight = beginWeight;
        ...
        /*
          {
          Score sum = 0.0;
          for (ConfusionNetwork::Slot::iterator itArc = itSlot->begin(), endArc
          = itSlot->end(); itArc != endArc; ++itArc) {
          //dbg(combinedCn->alphabet->symbol(itArc->label) << " "
          //<< combinedCn->semiring->describe(itArc->scores,
          Fsa::HintShowDetails | HintUnscaled)); sum +=
          itArc->scores->get(posteriorId);
          }
  ...
  ConstConfusionNetworkRef
  _combine(const ConstConfusionNetworkRefList &cns,
           ConfusionNetworkAlignmentPtrList *alignments = 0) const {
    verify(!cns.empty());
    ...
    ConfusionNetwork *combinedCnPtr = new ConfusionNetwork;
    ConstConfusionNetworkRef combinedCn =
        ConstConfusionNetworkRef(combinedCnPtr);
  ...
  RoverCombination(WeightedPosteriorCombinationHelperRef weightedCombo,
                   NistScliteCostRef scliteCostFcn, Score nullConfidence,
                   Score alpha, bool removeEps, u32 minBeamWidth)
      : Precursor(scliteCostFcn, minBeamWidth), weightedCombo_(weightedCombo),
        scliteCostFcn_(scliteCostFcn), nullConfidence_(nullConfidence),
        alpha_(alpha), removeEps_(removeEps) {}
...
public:
  ...
  combine(const ConstConfusionNetworkRefList &cns,
          ConfusionNetworkAlignmentPtrList *alignments = 0) const {
    ConstConfusionNetworkRefList linearCns(cns.size(),
                                           ConstConfusionNetworkRef());
    for (u32 i = 0; i < cns.size(); ++i) {
      const ConfusionNetwork &cn = *cns[i];
      verify(cn.isNormalized());
      ScoreId posteriorId = cn.normalizedProperties->posteriorId;
      ConfusionNetwork *linearCn = new ConfusionNetwork(
          cn.size(), ConfusionNetwork::Slot(1, ConfusionNetwork::Arc()));
      ...
      for (ConfusionNetwork::const_iterator itSlot = cn.begin(),
                                            endSlot = cn.end();
           itSlot != endSlot; ++itSlot, ++itLinearSlot) {
        Score maxPosterior = 0.0;
        ConfusionNetwork::Slot::const_iterator itBestPdf = itSlot->end();
        for (ConfusionNetwork::Slot::const_iterator itPdf = itSlot->begin(),
                                                    endPdf = itSlot->end();
             itPdf != endPdf; ++itPdf) {
  ...
  // Todo: std::pair<ConstConfusionNetworkRef, ConstLatticeRef> combine(const
  // ConstLatticeRefList &lats) const
  // -> output is CN(with normalized posterior scores) + best(with averaged
  // [unnormalized] confidences)
  ConstConfusionNetworkRef
  combine(const ConstLatticeRefList &lats,
          ConfusionNetworkAlignmentPtrList *alignments = 0) const {
  ...
  static ConstRoverCombinationRef
  create(WeightedPosteriorCombinationHelperRef weightedCombo,
         NistScliteCostRef scliteCostFcn, Score nullConfidence, Score alpha,
         bool removeEps, u32 minBeamWidth) {
    verify(weightedCombo);
    verify(scliteCostFcn);
    // verify(0.0 <= nullConfidence);
    if ((nullConfidence < 0.0) && (1.0 < nullConfidence))
      Core::Application::us()->warning("Null-confidence %f is not in [0, 1]",
                                       nullConfidence);
    // verify((0.0 <= alpha) && (alpha <= 1.0));
    if ((alpha < 0.0) && (1.0 < alpha))
      Core::Application::us()->warning(
          "Interpolation alpha %f is not in [0, 1]", alpha);
    return ConstRoverCombinationRef(
        new RoverCombination(weightedCombo, scliteCostFcn, nullConfidence,
                             alpha, removeEps, minBeamWidth));
  }
};
// -------------------------------------------------------------------------
...
public:
public:
  typedef enum { NistScliteWordCost, NistScliteTimeMediatedCost } RoverCostFcn;
...
private:
  ...
  ConstConfusionNetworkRef getCn() {
    if (!cn_) {
      ConstConfusionNetworkRef nBestCn =
          getNBestCn(); // Attention: getNBestCn() updates weightedCombo_
      cn_ = normalizeCn(
          nBestCn, weightedCombo_->semiringCombination()->combinationId(0));
    }
    return cn_;
  }

public:
  RoverCombinationNode(const std::string &name,
                       const Core::Configuration &config)
      : Node(name, config) {}
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    for (u32 i = 0; i < n; ++i) {
      ...
      keys[i] =
          WeightedPosteriorCombinationHelper::paramConfidenceKey(latConfig, "");
    }
    weightedCombo_ = WeightedPosteriorCombinationHelper::create(
        weights, keys,
        SemiringCombinationHelper::getType(
    ...
    case NistScliteWordCost:
      costFcn =
          RoverCombination::NistScliteWordCost::create(paramNullWord(config));
      break;
    case NistScliteTimeMediatedCost:
      costFcn = RoverCombination::NistScliteTimeMediatedCost::create(
          paramNullWord(config));
      break;
    default:
      defect();
    }
    roverCombo_ = RoverCombination::create(
        weightedCombo_, costFcn, paramNullConfidence(config),
        paramAlpha(config), paramRemoveEpsilons(config),
        paramBeamWidth(config));
...
const Core::Choice RoverCombinationNode::choiceRoverCostFcn(
    ...
    "sclite-time-mediated-cost",
    RoverCombinationNode::NistScliteTimeMediatedCost, Core::Choice::endMark());
const Core::ParameterChoice RoverCombinationNode::paramRoverCostFcn(
    "cost", &RoverCombinationNode::choiceRoverCostFcn, "cost function",
    RoverCombinationNode::NistScliteTimeMediatedCost);
const Core::ParameterString
    RoverCombinationNode::paramNullWord("null-word", "null word", "@");
const Core::ParameterFloat
    RoverCombinationNode::paramNullConfidence("null-confidence",
                                              "null confidence", 0.7);
const Core::ParameterFloat RoverCombinationNode::paramAlpha("alpha", "alpha",
                                                            0.0);
const Core::ParameterBool
    RoverCombinationNode::paramRemoveEpsilons("remove-eps",
                                              "remove epsilon arcs", false);

NodeRef createRoverCombinationNode(const std::string &name,
                                   const Core::Configuration &config) {
...
class OracleAlignment : public ConfusionNetworkAlignment,
                        public Core::ReferenceCounted {
  typedef ConfusionNetworkAlignment Precursor;
  ...
  */
  ...
  public:
    ...
    virtual Score insCost(u32 hypId) const {
      ...
      verify(posteriorId != Semiring::InvalidId);
      Score cost = 1.0;
      for (ConfusionNetwork::Slot::const_iterator it = hyp.begin(),
                                                  end = hyp.end();
           it != end; ++it)
...
private:
  void build(ConfusionNetwork *alignedCn, ConstConfusionNetworkRef ref,
             ConstConfusionNetworkRef cn) const {
    ConfusionNetwork::OracleAlignmentProperties *oracleProps =
    ...
    alignedCn->oracleAlignmentProperties =
    ...
    if (alignedCn->isNormalized()) {
      ...
      epsScore->set(alignedCn->normalizedProperties->posteriorId,
                    1.0 / Score(n));
    }
    ConfusionNetwork::Slot epsSlot(
        n, ConfusionNetwork::Arc(Fsa::Epsilon, epsScore));
    ConfusionNetwork::const_iterator itRefSlot = ref->begin();
    ConfusionNetwork::const_iterator itSlot = cn->begin();
    ConfusionNetwork::OracleAlignmentProperties::OracleAlignment
        &oracleAlignment = oracleProps->alignment;
    for (const_iterator itPair = begin(), endPair = end(); itPair != endPair;
         ++itPair) {
      if (itPair->first != Core::Type<u32>::max) {
    ...
    if (cn->hasMap()) {
      ...
      ConfusionNetwork::MapProperties *mapProperties =
          new ConfusionNetwork::MapProperties;
      mapProperties->stateIndex = cn->mapProperties->stateIndex;
      mapProperties->lat2cn = cn->mapProperties->lat2cn;
      for (ConfusionNetwork::MapProperties::Map::iterator
               itMap = mapProperties->lat2cn.begin(),
               endMap = mapProperties->lat2cn.end();
           itMap != endMap; ++itMap)
      ...
      if (!cn->mapProperties->slotIndex.empty()) {
        ...
        Fsa::StateId sid = 0;
        for (StateIdList::const_iterator itSlotMap = slotMap.begin();
             itSlotMap != slotMap.end(); ++itSlotMap, ++sid) {
          verify(sid <= *itSlotMap);
          mapProperties->slotIndex[*itSlotMap] =
              cn->mapProperties->slotIndex[sid];
        }
        u32 previousOffset = mapProperties->cn2lat.size();
        for (Core::Vector<Fsa::StateId>::reverse_iterator
                 itOffset = mapProperties->slotIndex.rbegin(),
                 endOffset = mapProperties->slotIndex.rend();
        ...
        // dbg
        /*
        for (ConfusionNetwork::MapProperties::Map::iterator itMap =
        mapProperties->lat2cn.begin(), endMap = mapProperties->lat2cn.end();
             itMap != endMap; ++itMap) if ((itMap->sid != Fsa::InvalidStateId)
        && (itMap->aid != Fsa::InvalidStateId)) { const
        ConfusionNetwork::MapProperties::Mapping &toLatMap =
        mapProperties->latticeArc(itMap->sid, itMap->aid); const
        ConfusionNetwork::MapProperties::Mapping &toCnMap =
        mapProperties->slotArc(toLatMap.sid, toLatMap.aid); verify(itMap->sid ==
        toCnMap.sid); verify(itMap->aid == toCnMap.aid);
            }
        */
      }
      alignedCn->mapProperties =
          ConfusionNetwork::ConstMapPropertiesRef(mapProperties);
    }
  }
  ...
  void lat2cn(ConfusionNetwork *cn, ConstLatticeRef l) const {
    ...
    if (!b->valid())
      b.reset();
    ConfusionNetwork::Slot tmpSlot(
        1, ConfusionNetwork::Arc(Fsa::Epsilon, cn->semiring->one()));
    for (ConstStateRef sr = l->getState(l->initialStateId()), nextSr;
         sr->hasArcs(); sr = nextSr) {
      if (sr->nArcs() > 1)
        Core::Application::us()->criticalError(
  ...
  OracleAlignment(PosteriorCostRef posteriorCostFcn, const Key &posteriorKey,
                  u32 minBeamWidth)
      : Precursor(posteriorCostFcn, minBeamWidth),
        posteriorCostFcn_(posteriorCostFcn), posteriorKey_(posteriorKey) {
...
public:
  ...
  std::pair<ConstConfusionNetworkRef, Score>
  align(ConstConfusionNetworkRef ref, ConstConfusionNetworkRef cn) const {
    verify(ref && cn);
    ...
    if (posteriorCostFcn_->requiresPosteriorId()) {
      if (posteriorKey_.empty()) {
        if (!cn->isNormalized())
          Core::Application::us()->criticalError(
              "Failed to find posterior dimension: Cn is not normalized and no "
              "posterior dimension is given.");
        posteriorCostFcn_->setPosteriorId(
            cn->normalizedProperties->posteriorId);
      } else {
  ...
  std::pair<ConstConfusionNetworkRef, Score>
  align(ConstLatticeRef ref, ConstConfusionNetworkRef cn) const {
    if (Lexicon::us()->alphabetId(ref->getInputAlphabet()) !=
        Lexicon::us()->alphabetId(cn->alphabet))
      Core::Application::us()->criticalError(
          "Reference and CN use different alphabet; \"%s\" != \"%s\".",
          Lexicon::us()
              ->alphabetName(Lexicon::us()->alphabetId(ref->getInputAlphabet()))
              .c_str(),
          Lexicon::us()
              ->alphabetName(Lexicon::us()->alphabetId(cn->alphabet))
              .c_str());
  ...
  std::pair<ConstConfusionNetworkRef, Score>
  align(const std::string &ref, ConstConfusionNetworkRef cn) const {
    if (Lexicon::us()->alphabetId(cn->alphabet) != Lexicon::LemmaAlphabetId)
      Core::Application::us()->criticalError(
          "Reference is assumed to be a lemma string, but CN's alphabet is "
          "\"%s\".",
          Lexicon::us()
              ->alphabetName(Lexicon::us()->alphabetId(cn->alphabet))
              .c_str());
  ...
  static ConstOracleAlignmentRef create(PosteriorCostRef posteriorCostFcn,
                                        const Key &posteriorKey,
                                        u32 minBeamWidth) {
    verify(posteriorCostFcn);
    return ConstOracleAlignmentRef(
        new OracleAlignment(posteriorCostFcn, posteriorKey, minBeamWidth));
  }
};
const Score OracleAlignment::OracleLoss::ErrorCost = 100.0;
ConstSemiringRef OracleAlignment::emptySemiring_ =
    Semiring::create(Fsa::SemiringTypeTropical, 0);
// -------------------------------------------------------------------------
...
public:
public:
  typedef enum { OracleCost, WeightedOracleCost, OracleLoss } CostFcn;
...
public:
  OracleAlignmentNode(const std::string &name,
                      const Core::Configuration &config)
      : Node(name, config) {}
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!connected(1) && !connected(2) && !connected(3) && !connected(4))
      criticalError(
          "Either a linear lattice at port 1, a string at port 2, a linear CN "
          "at port 3, "
    ...
    case WeightedOracleCost:
      costFcn = OracleAlignment::WeightedOracleCost::create(
          paramAlpha(choiceCostFcn[costChoice]));
      break;
    case OracleLoss:
...
const Core::Choice OracleAlignmentNode::choiceCostFcn(
    "oracle-cost", OracleAlignmentNode::OracleCost, "weighted-oracle-cost",
    OracleAlignmentNode::OracleCost, "oracle-loss",
    OracleAlignmentNode::OracleLoss, Core::Choice::endMark());
const Core::ParameterChoice OracleAlignmentNode::paramCostFcn(
    "cost", &OracleAlignmentNode::choiceCostFcn, "cost function",
    OracleAlignmentNode::OracleCost);
const Core::ParameterFloat OracleAlignmentNode::paramAlpha("alpha", "alpha",
                                                           1.0);

NodeRef createOracleAlignmentNode(const std::string &name,
                                  const Core::Configuration &config) {
