...
private:
  ...
  Amplitude getAverageHeight(const std::vector<Amplitude> &v,
                             const u32 peakIndex) const;
  f32 getHeightConfidence(const Amplitude peakValue,
                          const Amplitude averageHeight) const;
  Amplitude getWidthHeight(const Amplitude peakValue,
                           const Amplitude averageHeight) const;
  u32 getPeakWidth(const std::vector<Amplitude> &v, const u32 peakPos,
                   const Amplitude widthHeight) const;
  u32 getMountainWidth(const std::vector<Amplitude> &v, const u32 peakIndex,
                       const Amplitude widthHeight) const;
  f32 getWidthConfidence(const std::vector<Amplitude> &v, const u32 peakIndex,
...
public:
  ...
  /** getMaximumPosition = position of maximum in continous unit depending on
   * previous nodes */
...
private:
  bool putData(const Flow::PortId port, const Flow::Timestamp &timestamp,
               const f32 v);
