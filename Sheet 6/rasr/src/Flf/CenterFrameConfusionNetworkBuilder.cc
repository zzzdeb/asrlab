...
/*
  ...
  plus algorithm-individual information available from the CN construction
  algorithm
*/
...
public:
  ~CenterFrameCn() {
    for (SlotPtrList::iterator itSlotPtr = slotPtrs_.begin(),
                               endSlotPtr = slotPtrs_.end();
  ...
  ConfusionNetwork::ConstMapPropertiesRef getMapping(bool reduced) const {
    ...
    if (reduced) {
      ...
      for (Fsa::StateId sid = 0; sid < slotPtrs_.size(); ++sid) {
        ...
        for (IdList::const_iterator itArcId = frSlot.arcIds.begin(),
                                    endArcId = frSlot.arcIds.end();
             itArcId != endArcId; ++itArcId) {
          const Arc &frArc = arcs_[*itArcId];
          verify_(stateIndex[frArc.sid] + frArc.aid < lat2cn.size());
          lat2cn[stateIndex[frArc.sid] + frArc.aid] =
      ...
      for (Fsa::StateId sid = 0; sid < slotPtrs_.size(); ++sid) {
        ...
        for (IdList::const_iterator itArcId = frSlot.arcIds.begin(),
          ...
          cn2lat[slotIndex[sid] + aid] =
              ConfusionNetwork::MapProperties::Mapping(frArc.sid, frArc.aid);
          verify_(stateIndex[frArc.sid] + frArc.aid < lat2cn.size());
          lat2cn[stateIndex[frArc.sid] + frArc.aid] =
  ...
  ConstConfusionNetworkRef getCn(ScoreId posteriorId,
    ...
    for (u32 i = 0; i < slotPtrs_.size(); ++i) {
      ...
      for (IdList::const_iterator itArcId = frSlot.arcIds.begin(),
        ...
        slot.push_back(ConfusionNetwork::Arc(frArc.label, scores, frArc.start,
                                             frArc.end - frArc.start));
      }
    }
    if (mapping)
...
public:
  ...
  getNormalizedCn(ScoreId confidenceId, bool mapping = false) const {
    ...
    for (u32 i = 0; i < slotPtrs_.size(); ++i) {
      ...
      for (IdList::const_iterator itArcId = frcnSlot.arcIds.begin(),
                                  endArcId = frcnSlot.arcIds.end();
           itArcId != endArcId; ++itArcId) {
        ...
        if (frcnArcPtr->label != bestCurrentLabelFrcnArcPtr->label) {
          ...
          // *l_->getState(bestCurrentLabelFrcnArcPtr->sid); const Flf::Arc &arc
          // = *(state.begin() + bestCurrentLabelFrcnArcPtr->aid);
          ...
          slot.push_back(ConfusionNetwork::Arc(
              bestCurrentLabelFrcnArcPtr->label, scores,
          ...
          if (frcnArcPtr->posteriorScore <
              bestCurrentLabelFrcnArcPtr->posteriorScore)
            bestCurrentLabelFrcnArcPtr = frcnArcPtr;
        }
        col->feed(frcnArcPtr->posteriorScore);
        colStart->feed(frcnArcPtr->posteriorScore -
                       ::log(f64(frcnArcPtr->start)));
      ...
      {
        const Flf::State &state =
            *l_->getState(bestCurrentLabelFrcnArcPtr->sid);
        const Flf::Arc &arc =
            *(state.begin() + bestCurrentLabelFrcnArcPtr->aid);
        Score confidence = ::exp(-posteriorScore);
        ...
        slot.push_back(ConfusionNetwork::Arc(
            bestCurrentLabelFrcnArcPtr->label, scores, Time(Core::round(start)),
            std::max(Time(1), Time(Core::round(end - start)))));
        if (posteriorScore < bestPosteriorScore) {
          bestPosteriorScore = posteriorScore;
          bestFrcnArcPtr = bestCurrentLabelFrcnArcPtr;
        ...
        if (dEpsConfidence < 0.0) {
          ...
          if (dEpsConfidence < -0.01)
            Core::Application::us()->warning(
                "CenterFrameCnBuilder: Expected 1.0, got %f",
                (1.0 - dEpsConfidence));
          normalize = true;
        } else {
      ...
      if ((bestFrcnArcPtr) && (bestFrcnArcPtr->label != Fsa::Epsilon)) {
        if (preferredEndTime < Time(bestFrcnArcPtr->start)) {
          ...
          preferredEndTime = std::max(
              minEndTime, (preferredEndTime + bestFrcnArcPtr->start) / 2);
        ...
        sp->newArc(sp->id() + 1, bestFrcnArcScores, bestFrcnArcPtr->label,
                   bestFrcnArcPtr->label);
        sp = new Flf::State(sp->id() + 1);
        s->setState(sp);
...
private:
  ...
  struct SortSlotPtrByTime {
    bool operator()(const CenterFrameCn::Slot *slot1,
                    const CenterFrameCn::Slot *slot2) const {
...
private:
  ...
  u32 initialize(ConstFwdBwdRef fb, ConstPosteriorCnRef fCn) {
    ...
    for (Fsa::StateId sid = 0, endSid = staticL_->size(); sid < endSid; ++sid) {
      ...
      if (sp) {
        ...
        for (State::const_iterator a = sp->begin(), a_end = sp->end();
             a != a_end; ++a, ++aid, ++arcId) {
          ...
          if (dur > 0) {
            ...
            fCn->scores(confidences.begin(), confidences.begin() + dur, tBegin,
                        label);
            Probability maxConf = 0.0;
            for (ProbabilityList::const_iterator
                     itConf = confidences.begin(),
                     endConf = confidences.begin() + dur;
            ...
            for (ProbabilityList::const_iterator
              ...
              if (maxConf - *itConf <= 0.01) {
                ...
                // dbg("+ " << frameId << ", " << *itConf << ", " <<
                // staticL_->getInputAlphabet()->symbol(label));
            ...
            Core::Application::us()->warning(
                "Non-epsilon arc of length 0 detected: \"%s\", %d-%d, "
                "score=%f; arc is discarded",
                staticL_->getInputAlphabet()->symbol(arc.label).c_str(),
                arc.start, arc.end, staticL_->semiring()->project(arc.scores));
        }
      }
    }
    ...
    s32 t = 0;
    for (FrameList::iterator itFrame = frames_.begin(),
                             endFrame = frames_.begin() + nFrames;
         itFrame != endFrame; ++itFrame, ++t) {
      Frame &frame = *itFrame;
      frame.t = t;
  ...
  */
  inline bool less(const Frame::Properties &p1,
                   const Frame::Properties &p2) const {
  ...
  Id updateFrames(IdList::iterator beginFrameId, IdList::iterator &endFrameId) {
    ...
    Collector *nonEpsCol = createCollector(Fsa::SemiringTypeLog),
              *centerDevCol = createCollector(Fsa::SemiringTypeLog);
    for (IdList::const_iterator itFrameId = beginFrameId;
         itFrameId != endFrameId; ++itFrameId) {
      Frame &frame = frames_[*itFrameId];
      bool keep = true;
      if (frame.updated) {
        keep = false;
        for (IdList::const_iterator itArcId = frame.arcIds.begin(),
                                    endArcId = frame.arcIds.end();
             itArcId != endArcId; ++itArcId) {
          if (*itArcId != -1) {
            keep = true;
            const CenterFrameCn::Arc &arc = arcs[*itArcId];
            nonEpsCol->feed(arc.posteriorScore);
            const Score centerDev = Score(
                u32(Core::abs(frame.t + frame.t - arc.start - arc.end + 1)) >>
                1);
            if (centerDev > 0.0) {
              // quadratic version
        ...
        if (keep) {
          ...
          nonEpsCol->reset();
          if (frame.props.nonEpsScore <
              0.0010005003335835344) // -log(0.995)=0.0050125418235442863,
                                     // -log(0.999)=0.0010005003335835344
  ...
  void buildSlot(Id frameId) {
    ...
    for (IdList::const_iterator itArcId = frame.arcIds.begin(),
      ...
      if (arcId != -1) {
        ...
        for (IdList::const_iterator itDiscardFrameId = arc.frameIds.begin(),
                                    endDiscardFrameId = arc.frameIds.end();
  ...
  */
  void buildCenterFrameCn(ConstFwdBwdRef fb, ConstPosteriorCnRef fCn) {
    ...
    // build CN
    IdList::iterator beginFrameId = frameIdList.begin(),
                     endFrameId = frameIdList.begin() + nFrames;
    ...
    pi.finish(false);
    std::sort(feCn_->slotPtrs_.begin(), feCn_->slotPtrs_.end(),
              SortSlotPtrByTime());
  }
...
public:
  ...
  ConstCenterFrameCnRef build(ConstLatticeRef l,
                              ConstFwdBwdRef fb = ConstFwdBwdRef(),
                              ConstPosteriorCnRef fCn = ConstPosteriorCnRef()) {
...
namespace {
    ...
    const Core::ParameterChoice paramFrameOrderType(
        ...
        "risk builder",
        FrameOrderByLabelEntropy);
    CenterFrameCnBuilder::Frame::ConstLessRef buildFrameOrder(const
Core::Configuration &config) { Core::Choice::Value frameOrderType =
paramFrameOrderType(config); if (frameOrderType == Core::Choice::IllegalValue)
        ...
        case FrameOrderByCenterDeviation:
            return CenterFrameCnBuilder::Frame::ConstLessRef(new
CenterFrameCnBuilder::LessByCenterDeviation()); case FrameOrderByLabelEntropy:
            return CenterFrameCnBuilder::Frame::ConstLessRef(new
CenterFrameCnBuilder::LessByLabelEntropy()); case FrameOrderByMinLabelScore:
            return CenterFrameCnBuilder::Frame::ConstLessRef(new
CenterFrameCnBuilder::LessByMinLabelScore()); default: defect();
        }
...
CenterFrameCnFactory::~CenterFrameCnFactory() { delete internal_; }
...
void CenterFrameCnFactory::reset() { internal_->feCn.reset(); }

ConstConfusionNetworkRef CenterFrameCnFactory::getCn(ScoreId posteriorId,
                                                     bool mapping) const {
...
std::pair<ConstConfusionNetworkRef, ConstLatticeRef>
CenterFrameCnFactory::getNormalizedCn(ScoreId confidenceId,
                                      bool mapping) const {
  verify(internal_->feCn);
  ...
  void computeCenterFrameCn() {
    if (!feCn_) {
      ...
      if (supportFrames.empty()) {
          ...
          for (FrameList::const_iterator itFrame = supportFrames.begin() + 1;
      itFrame != supportFrames.end(); ++itFrame) msg << ", " << *itFrame; msg <<
      Core::XmlClose("support-frames");
      }
      */
    }
  }
  ...
  ConstConfusionNetworkRef getNormalizedCn() {
    if (!posteriorCn_) {
      ...
      std::pair<ConstConfusionNetworkRef, ConstLatticeRef> result =
          feCn_->getNormalizedCn(confidenceId_, map_);
...
public:
  CenterFrameCnBuilderNode(const std::string &name,
...
const Core::ParameterString
    CenterFrameCnBuilderNode::paramConfKey("confidence-key",
                                           "store confidence score", "");
const Core::ParameterBool CenterFrameCnBuilderNode::paramMap(
    "map", "map lattice to CN and vice versa", false);

NodeRef createCenterFrameCnBuilderNode(const std::string &name,
                                       const Core::Configuration &config) {
