...
MixtureSetBuilder::MixtureSetBuilder(MixtureIndex nMixtures,
    ...
    : nMixtures_(nMixtures), nDensitiesPerMixture_(nDensitiesPerMixture) {}
    ...
    for (size_t dns = 0; dns < nDensitiesPerMixture_; ++dns) {
      if (densityIndex(mix, dns) >= toBuild.nDensities()) {
        ...
        if (covarianceIndex(mix, dns) >= toBuild.nCovariances())
          toBuild.addCovariance(covarianceIndex(mix, dns),
                                new DiagonalCovariance(dimension));

        toBuild.addDensity(
            densityIndex(mix, dns),
            new GaussDensity(meanIndex(mix, dns), covarianceIndex(mix, dns)));
      }
