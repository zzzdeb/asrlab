...
// ===========================================================================
PhonemeInventoryElement::PhonemeInventoryElement(XmlContext *_context,
                                                 Handler _handler)
    : Precursor("phoneme-inventory", _context, _handler) {
  phoneme_ = 0;

  { // build schema
    XmlElement *symbol = collect(new XmlStringBuilderElement(
        "symbol", this,
    ...
    phoneme->addTransition(2, 3,
                           collect(new XmlIgnoreElement("features"))); // FFE
...
void PhonemeInventoryElement::phonemedefSymbol(const std::string &s) {
  ...
  if (test == phoneme_) {
    parser()->warning("Redundant definition of phonetic symbol \"%s\"",
                      symbol.c_str());
  } else if (test != 0) {
    parser()->error(
...
void PhonemeInventoryElement::endPhonemedef() { phoneme_ = 0; }
...
class Bliss::PronunciationElement
    : public Core::XmlBuilderElement<WeightedPhonemeString, XmlEmptyElement,
                                     CreateStatic> {
  typedef Core::XmlBuilderElement<WeightedPhonemeString, XmlEmptyElement,
                                  CreateStatic>
      Precursor;
...
void PronunciationElement::start(const Core::XmlAttributes atts) {
  ...
  if (weightStr && scoreStr) {
    parser()->criticalError(
        "not possible to set both, pronunciation weight and score");
    return;
  }
...
LexiconElement::LexiconElement(XmlContext *_context,
                               CreationHandler _newLexicon,
  ...
  { // build schema
    addTransition(initial, 1,
                  collect(new PhonemeInventoryElement(
                      this, PhonemeInventoryElement::handler(
                                &Self::addPhonemeInventory))));

    XmlElement *orth = collect(new XmlStringBuilderElement(
        "orth", this, XmlStringBuilderElement::handler(&Self::addOrth)));
    XmlElement *phon = collect(new PronunciationElement(
        this, PronunciationElement::handler(&Self::addPhon)));

    XmlRegularElement *synt =
        new XmlRegularElementRelay("synt", this, startHandler(&Self::startSynt),
    ...
    XmlRegularElement *eval =
        new XmlRegularElementRelay("eval", this, startHandler(&Self::startEval),
    ...
    XmlRegularElement *lemma = new XmlRegularElementRelay(
        "lemma", this, startHandler(&Self::startLemma),
...
void LexiconElement::addOrth(const std::string &_orth) {
  ...
  if (specialLemmaName_.empty() && !whitelist_.empty() &&
      (whitelist_.find(orth) == whitelist_.end()))
    return;
  ...
  if (!lemma_) {
    ...
    if (!specialLemmaName_.empty()) {
      if (product_->specialLemma(specialLemmaName_))
        parser()->error("Special lemma \"%s\" already defined",
                        specialLemmaName_.c_str());
      else
...
void LexiconElement::addPhon(const WeightedPhonemeString &phon) {
  ...
  if (!product_->phonemeInventory()) {
    parser()->warning("No phoneme inventory defined. Ingnoring pronunciation");
    return;
  }
...
// ===========================================================================
namespace {
const Core::ParameterString paramFile("file", "file name", "");
const Core::ParameterString paramEncoding("encoding", "encoding", "utf-8");
} // namespace

void LexiconParser::loadWhitelist(const Core::Configuration &config,
                                  Core::StringHashSet &whitelist) {
...
LexiconParser::LexiconParser(const Core::Configuration &c, Lexicon *_lexicon)
  ...
  // build schema
  LexiconElement *lexElement = new LexiconElement(
      this, LexiconElement::creationHandler(&Self::pseudoCreateLexicon), c);
