 ...
 *  Supports filenames with format qualifiers, e.g. bin:foo.matrix,
 * xml:foo.vector, etc. Usage: 1) Instantiate this class in every library,
 * tipically in  Application classes. 2) For your class Foo, derive from the
 * Format<Foo> class your own formats, e.g. FooFormat : FormatSet::Format<Foo>
 * {..}. 3) Implement the required read/write methods in FooFormat class.R 4)
 * Register the your format at your library FormatSet object. 5) To read or
 * store foo objects call the I/O function of the fomat set, e.g.
 * Application::us()->formatSet()->read("xml:foo.xml", foo); Note: -You can have
 * different types of format objects in one format set.
 */
...
public:
   ...
   */
  template <class T>
  void registerFormat(const std::string &qualifier, const Format<T> *format,
                      bool isDefault = false);

  /**
   *  Reads the object @c o of type T from the file @c filename.
   *  @c filename can contain a format qualifier, e.g. xml:foo.matrix,
   * bin:foo.cart.
   *  @return is false if the specified format is not registered for the type T,
   * or if the I/O operation was not successful. In case of an error error
   * messages are generated.
   */
  template <class T> bool read(const std::string &filename, T &o) const;
  /**
   ...
   * messages are generated.
   */
  template <class T> bool write(const std::string &filename, const T &o) const;
  /**
   *  Stores the object @c o of type T in the file @c filename.
   *  @c precision specifies the precision with which numbers are represented.
   *  This functionality is mainly meant for text formats thus the default
   * behaviour is ignoring this parameter.
   *  @c filename can contain a format qualifier, e.g. xml:foo.matrix,
   * bin:foo.cart.
   *  @return is false if the specified format is not registered for the type T,
   * or if the I/O operation was not successful. In case of an error error
...
template <class T>
void FormatSet::getTypeSpecificFormats(
    Ref<TypeSpecificFormats<T> > &typeSpecificFormats) {
  NameHelper<T> nameHelper;
  Formats::iterator f = formats_.find(nameHelper);
  if (f == formats_.end())
    f = formats_
            .insert(
                std::make_pair(nameHelper, ref(new TypeSpecificFormats<T>())))
            .first;
  typeSpecificFormats =
      ref(dynamic_cast<TypeSpecificFormats<T> *>(f->second.get()));
  require(typeSpecificFormats);
}

template <class T>
void FormatSet::getTypeSpecificFormats(
    Ref<TypeSpecificFormats<T> > &typeSpecificFormats) const {
  NameHelper<T> nameHelper;
...
void FormatSet::registerFormat(const std::string &qualifier,
                               const Format<T> *format, bool isDefault) {
...
bool FormatSet::getFormat(const std::string &qualifier,
  ...
  if (found == formats->end()) {
    NameHelper<T> nameHelper;
    error("Format '%s' is not supported by type '%s'.", q.c_str(),
          nameHelper.c_str());
    return false;
  }
  format = found->second;
...
template <class T>
bool FormatSet::write(const std::string &filename, const T &o,
                      int precision) const {
  std::string name(stripQualifier(filename));
  Ref<const Format<T> > format;
  if (checkFile(name, modeWrite) && getFormat(getQualifier(filename), format))
 ...
 */
template <class T> struct BinaryFormat : public FormatSet::Format<T> {
 ...
 *  To register compressed binary format for class Foo in a format set call the
 * function Formatset::registerFormat("bin", new Core::BinaryFormat<Foo>());
 */
template <class T> struct CompressedBinaryFormat : public FormatSet::Format<T> {
  virtual bool read(const std::string &filename, T &o) const {
 ...
 *  To register plain text format for class Foo in a format set call the
 * function Formatset::registerFormat("ascii", new
 * Core::PlainTextFormat<Foo>());
 */
template <class T> struct PlainTextFormat : public FormatSet::Format<T> {
  virtual bool read(const std::string &filename, T &o) const {
