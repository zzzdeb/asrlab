...
protected:
  void reduce(ScoresRef sr) const {
    ...
    for (ScoreIdList::const_iterator it = fromIds_.begin();
         it != fromIds_.end(); ++it)
...
public:
  ReduceScoresLattice(ConstLatticeRef l, const ScoreIdList &fromIds,
                      ScoreId toId, RescoreMode rescoreMode)
...
protected:
  virtual ConstLatticeRef rescore(ConstLatticeRef l) {
    ...
    if (!lastReducedSemiring_ || (lastSemiring_.get() != semiring.get())) {
      ...
      for (KeyList::const_iterator itKey = keys_.begin(); itKey != keys_.end();
           ++itKey, ++itId) {
      ...
      lastReducedSemiring_->setScale(ids_.front(),
                                     semiring->scale(ids_.front()));
    }
...
const Core::ParameterStringVector
    ReduceScoresNode::paramKeys("keys", "key identifiers");
NodeRef createReduceScoresNode(const std::string &name,
 ...
 * f(x;const)
 **/
template <class Func> class ArithmeticLattice : public RescoreLattice {
  typedef RescoreLattice Precursor;
...
public:
  ArithmeticLattice(ConstLatticeRef l, ScoreId id, Score c,
  ...
  virtual std::string describe() const {
    return Core::form("%s(%s,dim=%d,const=%f)", Func::describe().c_str(),
                      fsa_->describe().c_str(), id_, f_.c);
  }
};
template <class Func>
ConstLatticeRef func(ConstLatticeRef l, ScoreId id, Score c,
                     RescoreMode rescoreMode) {
  if (c == Func::neutral())
    return l;
...
template <class Func> class ArithmeticNode : public RescoreSingleDimensionNode {
public:
...
template <class Func>
const Core::ParameterFloat ArithmeticNode<Func>::paramScore("score", "score",
                                                            Semiring::Invalid);
template <class Func>
NodeRef createFuncNode(const std::string &name,
                       const Core::Configuration &config) {
...
ConstLatticeRef add(ConstLatticeRef l, ScoreId id, Score c,
                    RescoreMode rescoreMode) {
  return func<Add>(l, id, c, rescoreMode);
}
NodeRef createAddNode(const std::string &name,
                      const Core::Configuration &config) {
...
ConstLatticeRef multiply(ConstLatticeRef l, ScoreId id, Score c,
                         RescoreMode rescoreMode) {
  return func<Multiply>(l, id, c, rescoreMode);
}
NodeRef createMultiplyNode(const std::string &name,
                           const Core::Configuration &config) {
...
ConstLatticeRef exp(ConstLatticeRef l, ScoreId id, Score c,
                    RescoreMode rescoreMode) {
  return func<Exp>(l, id, c, rescoreMode);
}
NodeRef createExpNode(const std::string &name,
                      const Core::Configuration &config) {
...
ConstLatticeRef log(ConstLatticeRef l, ScoreId id, Score c,
                    RescoreMode rescoreMode) {
  return func<Log>(l, id, c, rescoreMode);
}
NodeRef createLogNode(const std::string &name,
                      const Core::Configuration &config) {
...
public:
  ExtendByPenaltyLattice(ConstLatticeRef l, Score penalty, ScoreId id,
                         Score scale, RescoreMode rescoreMode)
  ...
  virtual std::string describe() const {
    return Core::form("extendByPenalty(%s,dim=%d,penalty=%f)",
                      fsa_->describe().c_str(), id_, penalty_);
  }
};
...
class PenaltyMap : public Core::hash_map<Fsa::LabelId, Score>,
...
PenaltyMapRef buildPenaltyMap(const StringListList &orthClasses,
                              const ScoreList &orthClassPenalties) {
  ...
  for (StringListList::const_iterator itSS = orthClasses.begin();
    ...
    for (StringList::const_iterator itS = itSS->begin(); itS != itSS->end();
         ++itS) {
...
public:
  ExtendByLemmaSpecificPenaltyLattice(ConstLatticeRef l, Score defaultPenalty,
                                      PenaltyMapRef penalties, ScoreId id,
                                      Score scale, RescoreMode rescoreMode)
      : Precursor(l, rescoreMode), defaultPenalty_(defaultPenalty),
        penalties_(penalties), id_(id), scale_(scale) {
    require(penalties);
    ...
    for (State::iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if ((Fsa::FirstLabelId <= labelId) && (labelId <= Fsa::LastLabelId)) {
        const Bliss::Lemma *lemma =
            (lAlphabet_) ? lAlphabet_->lemma(labelId)
  ...
  ConstBoundariesRef getBoundaries() const { return fsa_->getBoundaries(); }

  virtual std::string describe() const {
    return "extendByLemmaSpecificPenalties(" + fsa_->describe() +
           ",dim=" + Core::itoa(id_) + ")";
  }
};
...
protected:
  virtual ConstLatticeRef rescore(ConstLatticeRef l, ScoreId id) {
    if (penalties_) {
      ...
      return ConstLatticeRef(new ExtendByLemmaSpecificPenaltyLattice(
          l, defaultPenalty_, penalties_, id, scale(), rescoreMode));
    } else {
      if (defaultPenalty_ != 0.0)
        return ConstLatticeRef(new ExtendByPenaltyLattice(
            l, defaultPenalty_, id, scale(), rescoreMode));
    }
    return l;
  }

public:
  ExtendByPenaltyNode(const std::string &name,
                      const Core::Configuration &config)
      : RescoreSingleDimensionNode(name, config) {}
  ~ExtendByPenaltyNode() {}
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!classes.empty()) {
      ...
      ScoreList penalties;
      for (StringList::const_iterator it = classes.begin(); it != classes.end();
           ++it) {
...
const Core::ParameterFloat
    ExtendByPenaltyNode::paramPenalty("penalty", "penalty", Semiring::One);
const Core::ParameterStringVector
    ExtendByPenaltyNode::paramPenaltyClasses("classes", "penalty classes");
const Core::ParameterString
    ExtendByPenaltyNode::paramOrthographies("orth",
                                            "list of orthographic forms", "");

NodeRef createExtendByPenaltyNode(const std::string &name,
                                  const Core::Configuration &config) {
...
public:
  ExtendByPronunciationScoreLattice(
      ConstLatticeRef l,
      Core::Ref<const Bliss::LemmaPronunciationAlphabet> lpAlphabet, ScoreId id,
      Score scale, RescoreMode rescoreMode)
      : Precursor(l, rescoreMode), lpAlphabet_(lpAlphabet), id_(id),
        scale_(scale) {
    require(lpAlphabet);
  }
  ...
  virtual void rescore(State *sp) const {
    for (State::iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if ((Fsa::FirstLabelId <= labelId) && (labelId <= Fsa::LastLabelId))
        a->weight_->add(id_,
                        scale_ * lpAlphabet_->lemmaPronunciation(a->output())
                                     ->pronunciationScore());
    }
  }
  ...
  virtual std::string describe() const {
    return Core::form("extendByPronunciation(%s,dim=%d)",
...
ConstLatticeRef extendByPronunciationScore(
  ...
  return ConstLatticeRef(new ExtendByPronunciationScoreLattice(
      l, lpAlphabet, id, scale, rescoreMode));
}

class ExtendByPronunciationScoreNode : public RescoreSingleDimensionNode {
protected:
  virtual ConstLatticeRef rescore(ConstLatticeRef l, ScoreId id) {
    return extendByPronunciationScore(
        l, Lexicon::us()->lemmaPronunciationAlphabet(), id, scale(),
        rescoreMode);
  }

public:
  ExtendByPronunciationScoreNode(const std::string &name,
                                 const Core::Configuration &config)
      : RescoreSingleDimensionNode(name, config) {}
...
NodeRef
createExtendByPronunciationScoreNode(const std::string &name,
                                     const Core::Configuration &config) {
