...
public:
  AlphabetCounts() : maxLabelId_(0), nLabels_(0), nDisambiguators_(0) {}
};
...
public:
  typedef u8 Type;
  static const Type typeUnmapped =
      0x01; /**< all symbols have the same index in FROM and TO */
  static const Type typeComplete =
      0x02; /**< all symbols in FROM are also in TO */
  static const Type typeIdentity =
      0x04; /**< FROM and TO are identical (not just equal) */

private:
  friend void mapAlphabet(ConstAlphabetRef, ConstAlphabetRef, AlphabetMapping &,
                          LabelId, u32);
...
public:
  ...
  bool isModifyingType() const {
    return (type_ & (typeUnmapped | typeComplete)) !=
           (typeUnmapped | typeComplete);
  }

  /**
   * @return a reference to the alphabet from which to map
   **/
  ConstAlphabetRef from() const { return from_; }

  /**
   * @return a reference to the alphabet to which we map
   **/
  ConstAlphabetRef to() const { return to_; }
 ...
 * @param reportUnknowns maximum number of unknown symbols that should be
 *reported
 **/
void mapAlphabet(ConstAlphabetRef from, ConstAlphabetRef to,
                 AlphabetMapping &mapping, LabelId unknownId = InvalidLabelId,
                 u32 reportUnknowns = 0);
} // namespace Fsa
