...
private:
  void startFsa(const Core::XmlAttributes atts) {
    ...
    if (!tmp) {
      ...
      // Fsa::TypeChoice[std::string(reinterpret_cast<const char*>(tmp))];
    ...
    if (!(semiring))
      if (fsa_->semiring())
        parser()->warning(
            "semiring not specified or not found. use %s semiring.",
            fsa_->semiring()->name().c_str());
      else
        parser()->error(
            "semiring not specified or not found. no semiring set.");
  ...
  void endState() { fsa_->setState(state_); }
  ...
  void startFinalWeight(const Core::XmlAttributes atts) { content_.resize(0); }
  void endFinalWeight() {
  ...
  void startIn(const Core::XmlAttributes atts) { content_.resize(0); }
  void endIn() {
    Core::stripWhitespace(content_);
    if (!content_.empty()) {
      if (content_[0] != '*') {
  ...
  void startArcWeight(const Core::XmlAttributes atts) { content_.resize(0); }
  void endArcWeight() {
    Core::stripWhitespace(content_);
...
public:
  StorageAutomatonXmlParser(const _Resources &resources, _StorageAutomaton *fsa)
      : Precursor(resources.getConfiguration()), resources_(resources),
        fsa_(fsa) {
    fsa_->setType(Fsa::TypeTransducer);
    Core::XmlMixedElement *arcElement = new Core::XmlMixedElementRelay(
        ...
        XML_CHILD(new Core::XmlMixedElementRelay(
            "in", this, startHandler(&Self::startIn), endHandler(&Self::endIn),
            charactersHandler(&Self::charactersFsa), XML_NO_MORE_CHILDREN)),
        XML_CHILD(new Core::XmlMixedElementRelay(
    ...
    Core::XmlMixedElement *stateElement = new Core::XmlMixedElementRelay(
        ...
        XML_CHILD(new Core::XmlMixedElementRelay(
            ...
            charactersHandler(&Self::charactersFsa), XML_NO_MORE_CHILDREN)),
        arcElement, XML_NO_MORE_CHILDREN);
    ...
    if (fsa_->getInputAlphabet()) {
      ...
      Core::XmlMixedElement *rootElement = new Core::XmlMixedElementRelay(
          ...
          stateElement, XML_NO_MORE_CHILDREN);
      setRoot(rootElement);
    } else {
      Core::Ref<Fsa::StaticAlphabet> input(new Fsa::StaticAlphabet),
          output(new Fsa::StaticAlphabet);
      ...
      Core::XmlMixedElement *rootElement = new Core::XmlMixedElementRelay(
          ...
          XML_CHILD(
              new Fsa::AlphabetXmlParser("output-alphabet", this, output)),
          stateElement, XML_NO_MORE_CHILDREN);
      setRoot(rootElement);
    }
  }
  ...
  bool parseStream(std::istream &i) { return (Precursor::parseStream(i) == 0); }
