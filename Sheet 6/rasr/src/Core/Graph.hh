...
private:
  template <class T> class AdjacentEdgeIterator {
  public:
    typedef T &Reference;
    typedef T *Pointer;
  ...
  public:
    bool valid() const { return edge_ != 0; }
...
public:
  ...
  protected:
    NodeBase() : id_(invalidNodeId()), outgoing_(0), incoming_(0) {}
  ...
  protected:
    EdgeBase()
        : id_(invalidEdgeId()), source_(0), target_(0), linkOutgoing_(0),
          linkIncoming_(0) {}
  };
...
private:
  template <class T, class I> class Iterator {
  public:
  ...
  public:
    ...
    bool valid() const { return begin_ <= current_ && current_ < end_; }
...
public:
  ...
  typedef Iterator<const Node, typename NodeList::const_iterator>
      const_NodeIterator;

  NodeIterator nodes() { return NodeIterator(nodes_.begin(), nodes_.end()); }
  ...
  typedef Iterator<const Edge, typename EdgeList::const_iterator>
      const_EdgeIterator_;
  class const_EdgeIterator : public const_EdgeIterator_ {
  public:
    const_EdgeIterator(typename EdgeList::const_iterator b,
                       typename EdgeList::const_iterator e)
        : const_EdgeIterator_(b, e) {}
    NodePointer currentNode() const { return item().source(); }
  };

  EdgeIterator edges() { return EdgeIterator(edges_.begin(), edges_.end()); }

  const_EdgeIterator edges() const {
    return const_EdgeIterator(edges_.begin(), edges_.end());
  }

  NodeId nNodeIds() const { return nodes_.size(); }

  NodePointer node(NodeId n) { return nodes_[n]; }

  const NodePointer node(NodeId n) const { return nodes_[n]; }

  EdgeId nEdgeIds() const { return edges_.size(); }

  EdgePointer edge(EdgeId e) { return edges_[e]; }

  const EdgePointer edge(EdgeId e) const { return edges_[e]; }
  ...
  NodePointer newNode() { return addNode(new Node); }
...
template <class N, class E> class GraphWithInitialFinals : public Graph<N, E> {
