...
void IndexMappedLm::mapToken(Token s, InternalClassIndex c) {
  ...
  if (tokenMap_[s] != invalidClass) {
    warning("Remapping \"%s\" to \"%s\", was \"%s\".", s->symbol().str(),
            internalClassName(c), internalClassName(tokenMap_[s]));
  }
  if (classMap_[c] != 0) {
    error("Mapping \"%s\" to \"%s\", which is already occupied by \"%s\".",
          s->symbol().str(), internalClassName(c),
          classMap_[c]->symbol().str());
  }
  tokenMap_[s] = c;
  classMap_[c] = s;
  classSizes_[c] += 1;
...
void IndexMappedLm::checkForUnmappedTokens() {
  InternalClassIndex fallback =
      (fallbackToken()) ? tokenMap_[fallbackToken()] : invalidClass;
    ...
    if (tokenMap_[t] == invalidClass) {
      if (fallback != invalidClass) {
        ...
        warning("Unknown syntactic token \"%s\" mapped to \"%s\".",
                (*s)->symbol().str(), internalClassName(fallback));
  ...
  if (nUnknowns)
    warning("%d unknown syntactic tokens mapped to \"%s\".", nUnknowns,
