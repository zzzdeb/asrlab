...
public:
  ...
  Mutex(const pthread_mutex_t &m) : mutex_(m) {}
...
public:
  ReadWriteLock() { pthread_rwlock_init(&lock_, 0); }

  ~ReadWriteLock() { pthread_rwlock_destroy(&lock_); }

  /**
   * Acquire a read lock
   */
  bool readLock() { return (pthread_rwlock_rdlock(&lock_) == 0); }
  /**
   * try to acquire a read lock
   */
  bool tryReadLock() { return (pthread_rwlock_tryrdlock(&lock_) == 0); }
  /**
   * Acquire a write lock
   */
  bool writeLock() { return (pthread_rwlock_wrlock(&lock_) == 0); }
  /**
   * Try to acquire a write lock
   */
  bool tryWriteLock() { return (pthread_rwlock_trywrlock(&lock_) == 0); }
  /**
   * Unlock
   */
