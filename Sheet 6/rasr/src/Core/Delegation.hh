...
/**
   ...
   do this check.
*/
template <typename T> class Delegation {
public:
  struct Target {
    Target() {}
    virtual ~Target() {}
  ...
  template <class TargetClass> struct Forward : public Target {

    typedef void (TargetClass::*TargetMethod)(T);
    TargetClass &tc;
    TargetMethod tm;

    Forward(TargetClass &tc, TargetMethod tm) : tc(tc), tm(tm) {}
...
public:
  Delegation() : target_(0) {}
  ~Delegation() { delete target_; }
  ...
  inline bool hasTarget() { return target_; }
