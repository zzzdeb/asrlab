...
const Core::ParameterString FeatureScorer::paramWeightPortName(
    "weight-port-name",
    "port name of flow network providing the score weights");

const Core::ParameterInt
    FeatureScorer::paramPrecision("output-precision",
                                  "precision of the output channel", 20);

FeatureScorer::FeatureScorer(const Core::Configuration &c)
    : Component(c), Precursor(c, Am::AcousticModel::complete),
      weightScores_(false), weightPort_(Flow::IllegalPortId),
...
void FeatureScorer::leaveSpeechSegment(Bliss::SpeechSegment *segment) {
  ...
  log("Average weighted score for key '%s': for this segment=%f; for all "
      "segments %f",
      key.c_str(), segmentAccumulator_.weightedAverage(),
      corpusKeyToScoreMap_[key].weightedAverage());

  Precursor::leaveSpeechSegment(segment);
}

void FeatureScorer::processAlignedFeature(Core::Ref<const Feature> f,
                                          Am::AllophoneStateIndex e) {
  Mm::FeatureScorer::Scorer scorer =
      acousticModel()->featureScorer()->getScorer(f);
  segmentAccumulator_.accumulate(
      scorer->score(acousticModel()->emissionIndex(e)),
      featureScoreWeight(f->timestamp()));
}

FeatureScorer::Weight
FeatureScorer::featureScoreWeight(const Flow::Timestamp &featureTimestamp) {
  if (weightScores_) {
...
void FeatureScorer::setFeatureDescription(
...
void FeatureScorer::setNumberOfLabels(size_t nLabels) {
  if (nLabels != acousticModel()->nEmissions()) {
    criticalError("Number of labels of alignment (%zd) do not match the number "
                  "of mixtures (%d).",
...
void FeatureScorer::write() {
  if (outputChannel_.isOpen()) {
    ...
    for (i = corpusKeyToScoreMap_.begin(); i != corpusKeyToScoreMap_.end();
         ++i) {
    ...
    criticalError("Could not dump corpus-key-to-score-map since channel "
                  "\"output\" is not open.");
  }
}
