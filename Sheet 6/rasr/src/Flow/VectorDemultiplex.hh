...
const Core::ParameterInt parameterTrack("track", "track number 0, 1, 2, ...", 0,
                                        0);

template <class T> class VectorDemultiplexNode : public SleeveNode {
private:
  u32 track_;
  u32 nTracks_;
...
public:
  ...
  virtual bool work(Flow::PortId p) {
    ...
    out->resize(in->size() / nTracks_ +
                ((in->size() % nTracks_) > offset_ ? 1 : 0));
