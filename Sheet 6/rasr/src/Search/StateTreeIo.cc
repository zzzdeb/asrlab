...
template <>
void StateTreeIo::write<StateTree::Exit>(Output &o,
                                         const StateTree::Exit &exit) const {
  write(o, exit.transitEntry);
  write(o, exit.pronunciation ? exit.pronunciation->id()
                              : Bliss::LemmaPronunciation::invalidId);
}

template <>
void StateTreeIo::read<StateTree::StateDesc>(Input &i,
                                             StateTree::StateDesc &desc) const {
  read(i, desc.acousticModel);
  read(i, desc.transitionModelIndex);
}

template <>
void StateTreeIo::write<StateTree::StateDesc>(
    Output &o, const StateTree::StateDesc &desc) const {
  write(o, desc.acousticModel);
  write(o, desc.transitionModelIndex);
}

template <>
void StateTreeIo::read<StateTree::State>(Input &i,
                                         StateTree::State &state) const {
  read(i, state.desc);
  read(i, state.depth);
  read(i, state.exits);
...
template <>
void StateTreeIo::write<StateTree::State>(Output &o,
                                          const StateTree::State &state) const {
  write(o, state.desc);
  write(o, state.depth);
  write(o, state.exits);
...
void StateTreeIo::write<StateTree::CoarticulationStructure::PhonemePair>(
    Output &o,
    const StateTree::CoarticulationStructure::PhonemePair &pair) const {
  write(o, pair.final);
...
StateTreeIo::StateTreeIo(Bliss::LexiconRef lexicon,
                         Am::AcousticModelRef acousticModel)
    : lexicon_(lexicon), acousticModel_(acousticModel) {
  getDependencies();
}

void StateTreeIo::getDependencies() {
  const Am::ClassicAcousticModel *am =
      required_cast(const Am::ClassicAcousticModel *, acousticModel_.get());
...
void StateTreeIo::getTreeDependencies(const StateTree &tree) {
  ...
  dependencies_.add("ci-crossword-transitions",
                    tree.allowCiCrossWordTransitions_);
}

StateTreeWriter::StateTreeWriter(Bliss::LexiconRef lexicon,
                                 Am::AcousticModelRef acousticModel)
    : StateTreeIo(lexicon, acousticModel) {}

bool StateTreeWriter::write(const StateTree &tree,
...
StateTreeIo::Position
StateTreeWriter::writeHeader(Core::BinaryOutputStream &out) const {
 ...
 */
void StateTreeWriter::writeDependenciesPosition(Core::BinaryOutputStream &out,
...
bool StateTreeWriter::writeDependencies(const std::string &filename,
                                        Position position) const {
  std::ofstream *ofs =
      new std::ofstream(filename.c_str(), std::ios::binary | std::ios::app);
  verify(ofs->good());
  ofs->seekp(position);
...
bool StateTreeWriter::writeTree(const StateTree &tree,
                                Core::BinaryOutputStream &out) const {
  verify(out.good());
...
bool StateTreeReader::checkDependencies(const StateTree &tree,
                                        const std::string &filename,
                                        Position position) const {
...
bool StateTreeReader::checkHeader(Core::BinaryInputStream &in,
                                  Position &dependencyPosition) const {
...
bool StateTreeReader::readTree(StateTree &tree,
                               Core::BinaryInputStream &in) const {
