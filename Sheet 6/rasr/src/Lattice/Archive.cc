...
const Core::Choice Archive::formatChoice("fsa", formatFsa, "htk", formatHtk,
                                         "luna", formatLuna, "lin", formatLin,
                                         "source", formatSourceFile, "wolfgang",
                                         formatWolfgang,
                                         Core::Choice::endMark());

const Core::ParameterChoice Archive::paramFormat("type", &Archive::formatChoice,
                                                 "format of word lattices",
                                                 formatFsa);
const Core::ParameterString Archive::paramPath("path", "path to word lattices");
...
const Core::Choice Archive::alphabetChoice(
    "unknown", unknownAlphabet, "lemma-pronunciation",
    lemmaPronunciationAlphabet, "lemma", lemmaAlphabet, "syntactic-token",
    syntacticTokenAlphabet, "evaluation-token", evaluationTokenAlphabet,
    "no-lexicon-check", noLexiconCheck, Core::Choice::endMark());

const Core::ParameterChoice Archive::paramAlphabet("alphabet",
                                                   &Archive::alphabetChoice,
                                                   "alphabet of word lattices",
                                                   lemmaPronunciationAlphabet);

Archive::Archive(const Core::Configuration &config, const std::string &pathname,
                 Bliss::LexiconRef lexicon)
    : Core::Component(config), lexicon_(lexicon), pathname_(pathname) {}

// generic interface =========================================================
const Core::ParameterFloat
    ArchiveReader::paramAmScale("am-scale", "acoustic model scale", 1.0);
const Core::ParameterFloat
    ArchiveReader::paramLmScale("lm-scale", "language model scale", 0.0);
const Core::ParameterFloat
    ArchiveReader::paramPenaltyScale("penalty-scale", "penalty scale", 1.0);
const Core::ParameterFloat
    ArchiveReader::paramPronunciationScale("pronunciation-scale",
                                           "pronunciation scale", 0.0);

ArchiveReader::ArchiveReader(const Core::Configuration &config,
                             const std::string &pathname,
                             Bliss::LexiconRef lexicon)
    : Core::Component(config), Precursor(config, pathname, lexicon) {}

void ArchiveReader::loadLatticeConfiguration() {
  if (hasFile(latticeConfigFilename)) {
    if (latticeConfig_.setFromFile(
            Core::joinPaths(pathname_, latticeConfigFilename))) {
      log("read config file from archive");
      latticeConfig_.setSelection("lattice-archive");
    } else {
      error("failed to read config file \"%s\" from archive",
            latticeConfigFilename);
    }
  } else {
    warning("no default config file \"%s\" in archive, use main config only",
            latticeConfigFilename);
  }
}

ArchiveWriter::ArchiveWriter(const Core::Configuration &config,
                             const std::string &pathname,
                             Bliss::LexiconRef lexicon)
    : Core::Component(config), Precursor(config, pathname, lexicon) {}
...
public:
  FsaArchiveWriter(const Core::Configuration &config,
                   const std::string &pathname, Bliss::LexiconRef lexicon);
  virtual ~FsaArchiveWriter();

  virtual void store(const std::string &id, ConstWordLatticeRef wordLattice);
};

Lattice::FsaArchiveWriter::FsaArchiveWriter(const Core::Configuration &config,
                                            const std::string &pathname,
                                            Bliss::LexiconRef lexicon)
    : Core::Component(config), Precursor(config, pathname, lexicon),
      fsaArchiveWriter_(0) {
...
FsaArchiveWriter::~FsaArchiveWriter() { delete fsaArchiveWriter_; }

std::string FsaArchiveWriter::filename(const std::string &id,
                                       const std::string &name) const {
  if (name == WordLattice::acousticFsa || name == WordLattice::mainFsa ||
      name.empty())
    return id + suffix();
  return id + "-" + name + suffix();
}

bool writeWordBoundariesBinary(Core::Ref<const WordBoundaries> wordBoundaries,
                               Core::BinaryOutputStream &bo) {
  return wordBoundaries->writeBinary(bo);
}

void FsaArchiveWriter::store(const std::string &id,
                             ConstWordLatticeRef wordLattice) {
  ...
  if (!fsaArchiveWriter_ and wordLattice->nParts() > 0) {
    ...
    if (fsa->type() == Fsa::TypeAcceptor) {
      fsaArchiveWriter_ =
          new Fsa::ArchiveWriter(config, pathname_, fsa->getInputAlphabet());
    } else {
      fsaArchiveWriter_ = new Fsa::ArchiveWriter(
          config, pathname_, fsa->getInputAlphabet(), fsa->getOutputAlphabet());
    }
  }
  ...
  for (size_t i = 0; i < wordLattice->nParts(); ++i) {
    ...
    if (part)
      fsaArchiveWriter_->store(filename(id, wordLattice->name(i)), part);
  }

  if (wordLattice->wordBoundaries()) {
    Core::ArchiveWriter o(*fsaArchiveWriter_->archive(),
                          id + wordBoundarySuffix, true);
    if (!o) {
...
public:
  FsaArchiveReader(const Core::Configuration &config,
                   const std::string &pathname, Bliss::LexiconRef lexicon);
  ...
  virtual ConstWordLatticeRef get(const std::string &id,
                                  const std::vector<std::string> &names);
};

const Core::ParameterString
    FsaArchiveReader::paramAmFileSuffix("am-file-suffix", "", "");

FsaArchiveReader::FsaArchiveReader(const Core::Configuration &config,
                                   const std::string &pathname,
                                   Bliss::LexiconRef lexicon)
    : Core::Component(config), Precursor(config, pathname, lexicon),
      fsaArchiveReader_(0) {
  ...
  if (penaltyScale() != amScale())
    error("cannot set penalty-scale to %f; penalty-scale for fsa lattices is "
          "always equal to am-scale, which is %f",
          penaltyScale(), amScale());
  if (lmScale() != 0.0)
    error("cannot set lm-scale to %f; lm-scale for fsa lattices is always 0.0",
          lmScale());
  if (pronunciationScale() != 0.0)
    error("cannot set pronunciation-scale to %f; pronunciation-scale for fsa "
          "lattices is always 0.0",
          pronunciationScale());
}

FsaArchiveReader::~FsaArchiveReader() { delete fsaArchiveReader_; }

std::string FsaArchiveReader::filename(const std::string &id,
                                       const std::string &name) const {
  if (name == WordLattice::acousticFsa || name == WordLattice::mainFsa ||
      name.empty()) {
...
ConstWordLatticeRef
FsaArchiveReader::get(const std::string &id,
                      const std::vector<std::string> &names) {
  ...
  for (size_t i = 0; i < names.size(); ++i) {
    wordLattice->setFsa(fsaArchiveReader_->get(filename(id, names[i])),
                        names[i]);
...
public:
  HtkArchiveWriter(const Core::Configuration &config,
                   const std::string &pathname, Bliss::LexiconRef lexicon)
      : Core::Component(config), Precursor(config, pathname, lexicon),
        htkWriter_(lexicon) {
    archive_ =
  ...
  virtual ~HtkArchiveWriter() { delete archive_; }
...
public:
  HtkArchiveReader(const Core::Configuration &config,
                   const std::string &pathname, Bliss::LexiconRef lexicon);
  ~HtkArchiveReader();
...
const Core::ParameterString HtkArchiveReader::paramEncoding(
    "encoding", "encoding of htk lattices in archive",
    Core::UnicodeInputConverter::defaultEncoding);
const Core::ParameterBool HtkArchiveReader::paramCapitalizeTranscriptions(
    "capitalize-transcriptions", "convert all words to upper case: yes/no",
    false);
const Core::ParameterString
    HtkArchiveReader::paramSuffix("suffix", "suffix of htk lattices in archive",
                                  ".lat.gz");
const Core::ParameterFloat
    HtkArchiveReader::paramWordPenalty("word-penalty", "word penalty", 0.0);
const Core::ParameterFloat
    HtkArchiveReader::paramSilencePenalty("silence-penalty", "silence penalty",
                                          0.0);

Fsa::ConstAutomatonRef
createEpsilonToSentenceBoundariesTransducer(Bliss::LexiconRef lexicon,
                                            Fsa::ConstSemiringRef semiring) {
  Fsa::StaticAutomaton *epsilonToSentenceBoundaries =
      new Fsa::StaticAutomaton(Fsa::TypeTransducer);
  epsilonToSentenceBoundaries->setSemiring(semiring);
  epsilonToSentenceBoundaries->setInputAlphabet(
      lexicon->lemmaPronunciationAlphabet());
  ...
  if ((lemma = lexicon->specialLemma("sentence-begin")))
    initialState->newArc(1, Fsa::Weight(0.0), Fsa::Epsilon,
                         lexicon->lemmaAlphabet()->index(lemma));
...
HtkArchiveReader::HtkArchiveReader(const Core::Configuration &config,
                                   const std::string &pathname,
                                   Bliss::LexiconRef lexicon)
    : Core::Component(config), Precursor(config, pathname, lexicon) {
  archive_ =
  ...
  capitalize_ = paramCapitalizeTranscriptions(
      config, paramCapitalizeTranscriptions(latticeConfig_));

  htkReader_ = new HtkReader(config, lexicon_, amScale(), lmScale(),
                             penaltyScale(), pronunciationScale(), false);
...
ConstWordLatticeRef
HtkArchiveReader::get(const std::string &id,
                      const std::vector<std::string> &names) {
...
Lattice::ArchiveReader *
Archive::openForReading(const Core::Configuration &config,
                        Bliss::LexiconRef lexicon) {
...
Lattice::ArchiveWriter *
Archive::openForWriting(const Core::Configuration &config,
                        Bliss::LexiconRef lexicon) {
  std::string pathname = paramPath(config);
  if (pathname.empty())
    return 0;
