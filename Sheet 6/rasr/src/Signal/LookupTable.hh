...
inline LookupTable<Value, Index> operator*(const Value scalar,
                                           const LookupTable<Value, Index> &lt);
template <class Value, class Index>
inline Core::XmlWriter &operator<<(Core::XmlWriter &o,
                                   const LookupTable<Value, Index> &lt);

/**
 * LookupTable: array of Values indexed by arbitrary numbers of type Index
 *
 * Maps the Index interval to the [0..(max_index - min_index) * bucketSize]
 * integer inteval. Integer interval is automatically extended, if a new Index
...
private:
  Index bucketSize_; // interval size of indices, which are assigned to the same
                     // bucket
  BucketIndex offset_;
  std::deque<Value> f_;
  bool grow_;
...
public:
   ...
   *  Index values smaller than min or larger than max will be set back to min
   * resp. max.
   */
  ...
  Value &operator[](const Index index) { return *insert(index); }
   ...
   *    2: strictly monotonous
   @  @param equalityTolerance controls how large jitter is accepted in constant
   part.
   */
  ...
  friend LookupTable<Value, Index> operator*
      <>(const Value scalar, const LookupTable<Value, Index> &lt);

  bool read(Core::BinaryInputStream &i);
  bool write(Core::BinaryOutputStream &o) const;
  friend Core::XmlWriter &operator<< <>(Core::XmlWriter &o,
                                        const LookupTable<Value, Index> &lt);
};

template <class Value, class Index>
LookupTable<Value, Index>::LookupTable(const Index bucketSize, const Index min,
                                       const Index max)
    : bucketSize_(bucketSize), offset_(0), grow_(true) {
...
void LookupTable<Value, Index>::normalizeSurface() {
  ...
  std::transform(begin(), end(), begin(),
                 std::bind2nd(std::divides<Value>(), s));
}
...
int LookupTable<Value, Index>::isMonotonous(Value equalityTolerance) const {
  ...
  for (; Core::isAlmostEqual(*current, *previous, equalityTolerance) &&
         current != end();
       ++previous, ++current)
    ;
  if (current == end())
...
template <class Value, class Index>
void LookupTable<Value, Index>::getInverse(
    LookupTable<Index, Value> &inverse) const {
  verify(isMonotonous());
  ...
  // Note: index is used here rather than iterator since iterators become
  // invalid after each call to insert.
