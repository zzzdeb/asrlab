...
/**
 * @todo pronunciation probabilities are quite slow due to repeated calculation
 * of logarithm
 */
class LemmaPronunciation {
public:
  typedef s32 Id;
  /** Id used for the invalid pronunciation (equivalent to a LemmaPronunciation*
   * with value 0) */
...
public:
   ...
   */
  bool hasName() const { return symbol(); }
  ...
  OrthographicFormList::Size nOrthographicForms() const { return orth_.size(); }

  /** Set of orthographic forms of this lemma. */
  const OrthographicFormList &orthographicForms() const { return orth_; }
  ...
  private:
    ...
    friend class Lemma;
    explicit PronunciationIterator(const LemmaPronunciation *c) : current_(c) {}

  public:
    PronunciationIterator() {}
    operator const LemmaPronunciation *() const { return current_; }
    const LemmaPronunciation *operator->() const { return current_; }
  ...
  typedef std::pair<PronunciationIterator, PronunciationIterator>
      LemmaPronunciationRange;
   ...
   * Under all normal circumstances the answer is "yes".
   * You don't have to test for this. */
  bool hasSyntacticTokenSequence() const { return syntacticTokens_.valid(); }
   ...
   *  > 0, see above. */
  u32 nEvaluationTokenSequences() const { return evaluationTokensList_.size(); }

  typedef EvaluationTokenSequenceList::const_iterator
      EvaluationTokenSequenceIterator;
  /** Iterator-range giving access to the set of pronunciations. */
...
public:
  ...
  private:
    ...
    explicit LemmaIterator(const LemmaPronunciation *c) : current_(c) {}
  ...
  std::pair<LemmaIterator, LemmaIterator> lemmas() const {
    return std::make_pair(LemmaIterator(lemmas_), LemmaIterator(0));
  }

  /** Phoneme sequence terminated by Phoneme::term. */
  const Phoneme::Id *phonemes() const { return phonemes_; }
...
template <class Object> class Coarticulated {
private:
...
public:
  Coarticulated(const Object &object, Phoneme::Id leftContext = Phoneme::term,
                Phoneme::Id rightContext = Phoneme::term)
      : object_(object), leftContext_(leftContext),
...
std::string Coarticulated<Object>::format(
    Core::Ref<const PhonemeInventory> phonemeInventory) const {
  const std::string l(
      leftContext() == Phoneme::term
          ? "#"
          : phonemeInventory->phoneme(leftContext())->symbol().str());
  const std::string r(
      rightContext() == Phoneme::term
          ? "#"
          : phonemeInventory->phoneme(rightContext())->symbol().str());
  return Core::form("%s|%s|%s", l.c_str(),
                    object().format(phonemeInventory).c_str(), r.c_str());
}

/**
 * A syntactic unit aka language model token in the Bliss lexicon.
 * @see <a href="../../doc/Bliss.pdf">Bliss documentation</a>
 * @todo class emission probabilities are quite slow due to repeated calculation
 * of logarithm
 */
...
protected:
  ...
  void addLemma(const Lemma *lemma) { lemmas_.push_back(lemma); }

public:
  /** The number of lemmata this token occurs in. */
  u32 nLemmas() const { return lemmas_.size(); }
   ...
   */
  f32 classEmissionScore() const { return std::log(float(nLemmas())); }
...
class Lexicon : public Core::ReferenceCounted, public Core::Component {
...
protected:
  ...
  Pronunciation *
  getOrCreatePronunciation(const std::vector<Phoneme::Id> &phonemes);

  // syntactic tokens
  friend class Bliss::SyntacticTokenAlphabet;
  Core::Obstack<const SyntacticToken *>
      synts_; /**< syntactic token sequences */
  TokenInventory syntacticTokens_;
  SyntacticToken *getOrCreateSyntacticToken(Symbol);

  // evaluation tokens
  friend class Bliss::EvaluationTokenAlphabet;
  Core::Obstack<const EvaluationToken *>
      evals_; /**< evaluation token sequences */
  ...
  void parsePronunciation(const std::string &,
                          std::vector<Phoneme::Id> &) const;
...
public:
   ...
   */
  void setOrthographicForms(Lemma *lemma,
   ...
   */
  void addPronunciation(Lemma *lemma, Pronunciation *pron, f32 weight = 1.0);
  void normalizePronunciationWeights(Lemma *lemma);

  /**
   * Set the a syntactic token sequence for a lemma.
   */
  void setSyntacticTokenSequence(Lemma *lemma,
   ...
   */
  void addEvaluationTokenSequence(Lemma *lemma,
  ...
  std::pair<LemmaIterator, LemmaIterator> lemmas() const {
    return std::make_pair(LemmaIterator(lemmas_.begin()),
  ...
  u32 nPronunciations() const { return pronunciations_.size(); }

  typedef PronunciationList::const_iterator PronunciationIterator;

  /** Iterator-range giving access to the set of pronunciations. */
  std::pair<PronunciationIterator, PronunciationIterator>
  ...
  std::pair<LemmaPronunciationIterator, LemmaPronunciationIterator>
  lemmaPronunciations() const {
  ...
  lemmaPronunciationAlphabet() const;
  ...
  u32 nSyntacticTokens() const { return syntacticTokens_.size(); }

  typedef const SyntacticToken *const *SyntacticTokenIterator;

  /** Iterator-range giving access to the set of syntactic tokens. */
  std::pair<SyntacticTokenIterator, SyntacticTokenIterator>
  syntacticTokens() const {
    return std::make_pair(SyntacticTokenIterator(syntacticTokens_.begin()),
  ...
  u32 nEvaluationTokens() const { return evaluationTokens_.size(); }

  typedef const EvaluationToken *const *EvaluationTokenIterator;

  /** Iterator-range giving access to the set of syntactic tokens. */
  std::pair<EvaluationTokenIterator, EvaluationTokenIterator>
  evaluationTokens() const {
    return std::make_pair(EvaluationTokenIterator(evaluationTokens_.begin()),
  ...
  const TokenInventory &letterInventory() const { return letters_; }
  ...
  createLemmaToOrthographyTransducer(bool shallIncludeVariants = true) const;
  ...
  createPhonemeToLemmaPronunciationTransducer(bool disambiguate = true) const;
  Core::Ref<PhonemeToLemmaPronunciationTransducer>
  createPhonemeToLemmaPronunciationTransducer(
      size_t nDisambiguators, bool disambiguate,
      bool markInitialPhonesAndSilence = false) const;
  Core::Ref<PhonemeToLemmaPronunciationTransducer>
  createPhonemeToLemmaPronunciationAndStickPunctuationTransducer() const;
  ...
  createLemmaPronunciationToLemmaTransducer(u32 nDisambiguators = 0) const;
  ...
  createLemmaToSyntacticTokenTransducer(
      bool useEmptySyntacticTokenSequences = true,
      size_t nDisambiguators = 0) const;
  ...
  createLemmaToEvaluationTokenTransducer(bool allowAlternatives = true) const;
   ...
   */
  Core::Ref<LemmaToEvaluationTokenTransducer>
  createLemmaToPreferredEvaluationTokenSequenceTransducer() const;
};
