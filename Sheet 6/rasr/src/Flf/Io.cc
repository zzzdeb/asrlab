...
InputStream::InputStream(const std::string &path, Core::Archive *archive)
  ...
  if (archive) {
    ...
    if (!(*ar) || !ar->isOpen()) {
      Core::Application::us()->warning("Could not open archive entry \"%s\".",
                                       filename.c_str());
      delete ar;
      ar = 0;
    } else
    ...
    if (!(*cis)) {
      Core::Application::us()->warning("Could not open file \"%s\".",
                                       filename.c_str());
      delete cis;
      cis = 0;
    } else
...
OutputStream::OutputStream(const std::string &path, Core::Archive *archive)
  ...
  if (archive) {
    ...
    if (!(*aw)) {
      Core::Application::us()->error("Could not open archive entry \"%s\"",
                                     filename.c_str());
      delete aw;
      aw = 0;
    } else
    ...
    if (!(*cos)) {
      Core::Application::us()->error("Could not open file \"%s\"",
                                     filename.c_str());
      delete cos;
      cos = 0;
    } else
...
bool readBinary(StaticBoundaries *b, std::istream &is) {
  ...
  switch (version) {
  case 1: {
    for (StaticBoundaries::iterator it = b->begin(); (it != b->end()) && (bis);
         ++it) {
      u32 time, transitId;
...
bool writeBinary(const StaticBoundaries *b, std::ostream &os) {
  ...
  case 3:
    for (StaticBoundaries::const_iterator it = b->begin(); it != b->end();
         ++it) {
      bos << (u32)it->time();
...
bool writeBoundariesBinary(ConstLatticeRef l, std::ostream &os) {
  bool success = false;
  const StaticBoundaries *b =
      dynamic_cast<const StaticBoundaries *>(l->getBoundaries().get());
  if (b) {
...
const Core::ParameterString
    IoFormat::paramEncoding("encoding", "output character set encoding",
                            "utf-8");
const Core::ParameterInt IoFormat::paramMargin("margin", "maximum line length",
                                               0, 0);
const Core::ParameterInt
    IoFormat::paramIndentation("indentation",
                               "number of blanks for indenting output", 2, 0);
...
const Core::Choice IoFormat::choiceLatticeFormat(
    ...
    "lattice-processor",
    IoFormat::LatticeFormatLatticeProcessor, // deprecated, not supported by all
                                             // i/o routines
    Core::Choice::endMark());
const Core::ParameterChoice
    IoFormat::paramLatticeFormat("format", &IoFormat::choiceLatticeFormat,
                                 "format of word lattices", LatticeFormatFlf);

const Core::Choice IoFormat::choiceCnFormat("text", IoFormat::CnFormatText,
                                            "xml", IoFormat::CnFormatXml,
                                            Core::Choice::endMark());
const Core::ParameterChoice IoFormat::paramCnFormat("format",
                                                    &IoFormat::choiceCnFormat,
                                                    "format of CNs",
                                                    CnFormatXml);

const Core::Choice IoFormat::choicePosteriorCnFormat(
    "text", IoFormat::PosteriorCnFormatText, "xml",
    IoFormat::PosteriorCnFormatXml, "flow-alignment",
    IoFormat::PosteriorCnFormatFlowAlignment, Core::Choice::endMark());
const Core::ParameterChoice IoFormat::paramPosteriorCnFormat(
    "format", &IoFormat::choicePosteriorCnFormat, "format of posterior CNs",
...
protected:
  ...
  const std::string dir() { return dir_; }
  ...
  void load() {
    ...
    isValid_ = true;
    filename_ =
        connected(1) ? requestSegment(1)->segmentIdOrDie() : requestString(2);
    if (!dir().empty())
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    if (!connected(1) && !connected(2))
      criticalError(
          "ReaderNode: Need data source either at port 1 or at port 2.");
...
const Core::ParameterString
    ReaderNode::paramDir("path", "interpret file names relative to path", "");
// -------------------------------------------------------------------------
...
public:
  ...
  virtual ~LatticeReaderNode() { delete reader_; }
...
NodeRef createLatticeReaderNode(const std::string &name,
...
protected:
  virtual bool read(const std::string &filename) {
    ...
    if (!fsa_)
      error("FsaReaderNode: Failed to read fsa \"%s\"", filename.c_str());
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!paramName(select("alphabet")).empty()) {
      ...
      inputAlphabetId_ = outputAlphabetId_ =
          Lexicon::us()->alphabetId(paramName(select("alphabet")), true);
    } else {
      type_ = Fsa::TypeTransducer;
      inputAlphabetId_ =
          Lexicon::us()->alphabetId(paramName(select("input-alphabet")), true);
      outputAlphabetId_ =
          Lexicon::us()->alphabetId(paramName(select("output-alphabet")), true);
    }
  }
  ...
  virtual ConstLatticeRef sendLattice(Port to) {
    verify_(to == 0);
    load();
    return (fsa_) ? fromFsa(fsa_, semiring_, semiring_->one(), 0,
                            inputAlphabetId_, outputAlphabetId_)
                  : ConstLatticeRef();
  }
...
const Core::ParameterString FsaReaderNode::paramName("name", "name", "lemma");

NodeRef createFsaReaderNode(const std::string &name,
                            const Core::Configuration &config) {
...
protected:
  State *grow(StaticLattice *s, State *sp, ScoresRef scores,
              Fsa::LabelId label) {
  ...
  ConstLatticeRef build(const std::string &name, Time start, Time end) {
    ...
    s->setDescription(Core::form("ctm(%s,%d-%d)", name.c_str(), start, end));
    ...
    for (; (rec.cursor != rec.end()) && (rec.cursor->end <= lastEnd);
         ++rec.cursor)
      ;
    ...
    for (; (rec.cursor != rec.end()) && (rec.cursor->end <= end);
         ++rec.cursor) {
      ...
      for (; (itId != ids_.end()) && (itCol != cols.end()); ++itId, ++itCol)
        if (*itId != Semiring::InvalidId)
          if (!Core::strconv(*itCol, scores[*itId]))
            criticalError("CtmReaderNode: Failed to convert score %s",
                          itCol->c_str());
    }
    return scoresRef;
  }
  ...
  void load() {
    ...
    if (!Core::isValidPath(path))
      criticalError("CtmReaderNode: Could not find \"%s\"", path.c_str());
    CtmRecording emptyCtmRecording;
    CtmEntry ctmEntry;
    TextFileParser tf(path, paramEncoding(config));
    if (!tf)
      criticalError("CtmReaderNode: Could not open \"%s\"", path.c_str());
    std::string lastName;
    ...
    for (;; ++nWords) {
      ...
      if (it != corpus_.end()) {
        if (ctmEntry.start < it->second.back().end) {
          if ((it->second.back().start < ctmEntry.start) &&
              ((it->second.back().end - ctmEntry.start) <= 1)) {
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (!semiring_) {
      ...
      semiring_ = Semiring::create(Fsa::SemiringTypeTropical, keys.size(),
                                   ScoreList(), keys);
      for (ScoreId id = 0; id < keys.size(); ++id)
        ids_.push_back(id);
    } else {
      // keys and semiring are defined separately
      for (KeyList::const_iterator itKey = keys.begin(); itKey != keys.end();
           ++itKey)
    ...
    log("Semiring is \"%s\".\n"
        ...
        semiring_->describe(defaultScores_, Fsa::HintShowDetails | HintUnscaled)
            .c_str(),
  ...
  virtual ConstLatticeRef sendLattice(Port to) {
    ...
    if (!isValid_) {
      ...
      if (!segment->hasRecordingId() || !segment->hasStartTime() ||
          !segment->hasEndTime())
        criticalError(
            "CtmReaderNode: Require recording id, start time, and end time.");
      l_ = build(segment->recordingId(),
                 Time(Core::round(segment->startTime() * 100.0)),
                 Time(Core::round(segment->endTime() * 100.0)));
      isValid_ = true;
    }
    return l_;
  }
};
const Core::ParameterString CtmReaderNode::paramPath("path", "path to ctm-file",
                                                     "");
const Core::ParameterString
    CtmReaderNode::paramEncoding("encoding", "encoding of ctm-file", "utf-8");
const Core::ParameterStringVector CtmReaderNode::paramScoreKeys(
    "scores", "CTM file provides scores for the given keys", "");
const Core::ParameterFloat
    CtmReaderNode::paramDefaultScore("default", "default score", Semiring::One);

NodeRef createCtmReaderNode(const std::string &name,
                            const Core::Configuration &config) {
...
protected:
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    std::string filename;
    if (connected(1) || connected(2)) {
      filename = Core::joinPaths(dir_, connected(1)
                                           ? requestSegment(1)->segmentIdOrDie()
                                           : requestString(2));
      if (!prefix_.empty())
        filename = Core::joinPaths(Core::directoryName(filename),
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    ...
    if (connected(1) || connected(2)) {
      ...
      if (filename_.empty())
        criticalError("WriterNode: Need either a data source at port 1, at "
                      "port 2, or a valid filename");
      filename_ = Core::joinPaths(dir_, filename_);
    }
  }
};
const Core::ParameterString
    WriterNode::paramDir("path", "interpret file names relative to path", "");
const Core::ParameterString WriterNode::paramFile("file", "file", "");
const Core::ParameterString WriterNode::paramPrefix("prefix",
                                                    "prefix for filename", "");
const Core::ParameterString WriterNode::paramSuffix("suffix", "suffix filename",
                                                    "");
// -------------------------------------------------------------------------
...
public:
  ...
  virtual ~LatticeWriterNode() { delete writer_; }
  virtual void init(const std::vector<std::string> &arguments) {
    Precursor::init(arguments);
...
NodeRef createLatticeWriterNode(const std::string &name,
                                const Core::Configuration &config) {
...
const Core::Choice DrawerNode::hintChoice("details", Fsa::HintShowDetails,
...
const Core::ParameterStringVector DrawerNode::paramOptions(
    "hints", "drawing options (detailed, best, probability, unscaled)", "");

NodeRef createDrawerNode(const std::string &name,
                         const Core::Configuration &config) {
