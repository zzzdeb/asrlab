...
const Core::ParameterFloat ISmoothingMixtureSetEstimator::paramWeightsConstant(
    "i-smoothing-weights",
    "interpolation constant for i-smoothing of mixture weights", 0, 0);

const Core::ParameterFloat ISmoothingMixtureSetEstimator::paramMeansConstant(
    "i-smoothing-means", "interpolation constant for i-smoothing of means", 0,
    0);

ISmoothingMixtureSetEstimator::ISmoothingMixtureSetEstimator(
    const Core::Configuration &c)
    : Core::Component(c), parent_(0), weightsConstant_(paramWeightsConstant(c)),
      meansConstant_(paramMeansConstant(c)) {}

void ISmoothingMixtureSetEstimator::set(
    DiscriminativeMixtureSetEstimator *parent) {
  parent_ = parent;
}
  ...
  if (mixtureSet) {
    if (!distributeISmoothingMixtureSet(*mixtureSet)) {
      Core::Application::us()->criticalError(
          "i-smoothing mixture set differs in topology");
    }
  } else {
    Core::Application::us()->criticalError(
        "i-smoothing mixture set could not be read");
  }
}
...
bool ISmoothingMixtureSetEstimator::distributeISmoothingMixtureSet(
  ...
  for (MixtureIndex i = 0; i < mixtureSet.nMixtures(); ++i) {
    setIMixture((*parent_->getMixtureEstimators())[i], mixtureSet.mixture(i),
                weightsConstant());
  }
  ...
  for (CovarianceIndex i = 0; i < mixtureSet.nCovariances(); ++i) {
    setICovariance(indexMaps.covarianceMap()[i], mixtureSet.covariance(i),
                   meansConstant());
  }
...
Sum ISmoothingMixtureSetEstimator::getObjectiveFunction(
  ...
  for (CovarianceIndex i = 0; i < indexMaps.covarianceMap().size(); ++i) {
    iOfM += getCovarianceObjectiveFunction(indexMaps.covarianceMap()[i],
                                           meanSetMap);
  }
