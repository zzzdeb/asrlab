 ...
 **/
typedef NodeRef (*NodeCreatorFcn)(const std::string &name,
...
struct NodeCreator {
  ...
  NodeCreatorFcn f;
  // old, deprecated
  NodeCreator(const std::string &name, const std::string &desc,
              NodeCreatorFcn f)
      : name(name), generalDesc(desc), f(f) {
    require(f);
  }
  // new
  NodeCreator(const std::string &name, const std::string &generalDesc,
              const std::string &configDesc, const std::string &portDesc,
              NodeCreatorFcn f)
      : name(name), generalDesc(generalDesc), configDesc(configDesc),
        portDesc(portDesc), f(f) {
    require(f);
  }
  NodeRef operator()(const std::string &name,
                     const Core::Configuration &config) {
...
private:
  typedef Core::hash_map<std::string, NodeCreator, Core::StringHash>
      NodeCreatorMap;
...
public:
  ...
  NodeRef createNode(const std::string &name,
                     const Core::Configuration &config);
