...
// -------------------------------------------------------------------------
bool isEmpty(ConstLatticeRef l) { return FtlWrapper::isEmpty(l); }
...
void basicInfo(ConstLatticeRef l, Core::XmlWriter &o) {
  ...
  std::string tmp;
  for (Core::Choice::const_iterator i = Fsa::PropertyChoice.begin();
       i != Fsa::PropertyChoice.end(); ++i) {
...
struct LatticeStatistics {
  ...
  LatticeStatistics()
      : nInputEpsilonArcs(0), nInputNonWordArcs(0), nInputWordArcs(0),
        ...
        nFinalStates(0), minTime(Core::Type<Time>::max),
...
struct BestStatistics : public LatticeStatistics {
  ...
  f64 densityWordWord;
  BestStatistics()
      : LatticeStatistics(), nNonEps(0), densityAllNonEps(0.0),
        densityNonEpsNonEps(0.0), nWords(0), densityAllWord(0.0),
        densityWordWord(0.0) {}
};
struct TraceElement {
  Score score;
...
void extendedInfo(ConstLatticeRef l, Core::XmlWriter &o,
                  LatticeStatistics &stats, BestStatistics &bestStats) {
  ...
  if (l->initialStateId() != Fsa::InvalidStateId) {
    ...
    if (!isAcceptor) {
      ...
      if ((outputAlphabetId == Lexicon::LemmaAlphabetId) ||
        ...
        outputNonWordMap =
            LabelMap::createNonWordToEpsilonMap(outputAlphabetId);
    }

    // fwd/bwd-scores, topological sort
    std::pair<ConstLatticeRef, ConstFwdBwdRef> result =
        FwdBwd::build(l, posteriorSemiring);
    l = result.first;
    ...
    for (u32 i = 0; i < topologicalSort->size(); ++i) {
      ...
      for (State::const_iterator a = sr->begin(); a != sr->end(); ++a) {
        ...
        if (!isAcceptor) {
          ...
          else if (outputNonWordMap &&
                   !(*outputNonWordMap)[a->output()].empty())
    ...
    while (bestSid != initialSid) {
      ...
      else if (inputNonWordMap &&
               !(*inputNonWordMap)[trace.arc.input()].empty())
      ...
      if (!isAcceptor) {
        if (trace.arc.output() == Fsa::Epsilon)
          ++bestStats.nOutputEpsilonArcs;
                 ...
                 !(*outputNonWordMap)[trace.arc.output()].empty())
    ...
    bestStats.posteriorScore =
        posteriorSemiring->project(bestStats.scores) - fb->sum();
    ...
    u32 nArcs = stats.nInputEpsilonArcs + stats.nInputNonWordArcs +
                stats.nInputWordArcs;
    ...
    if (bestStats.nNonEps > 0) {
      ...
      bestStats.densityAllNonEps =
          f64(stats.nInputEpsilonArcs + stats.nInputNonWordArcs +
              stats.nInputWordArcs) /
          nNonEps;
      bestStats.densityNonEpsNonEps =
          f64(stats.nInputNonWordArcs + stats.nInputWordArcs) / nNonEps;
    }
    ...
    o << Core::XmlFull("scores",
                       semiring->describe(bestStats.scores,
                                          Fsa::HintShowDetails | HintUnscaled));
    o << Core::XmlFull("score", bestStats.score);
    o << Core::XmlFull("posterior-score",
                       Core::form("%.15f", bestStats.posteriorScore));
    o << Core::XmlFull("posterior-probability",
                       Core::form("%.15f", ::exp(-bestStats.posteriorScore)));
    if (l->getBoundaries()->valid()) {
      o << Core::XmlFull("start-timeframe", bestStats.minTime);
      o << Core::XmlFull("end-timeframe", bestStats.maxTime);
    }
    u32 nBestArcs = bestStats.nInputEpsilonArcs + bestStats.nInputNonWordArcs +
                    bestStats.nInputWordArcs;
    ...
    if (!isAcceptor) {
      ...
      if (outputNonWordMap) {
        o << Core::XmlFull("output-non-word-arcs",
                           bestStats.nOutputNonWordArcs);
...
void info(ConstLatticeRef l, Core::XmlWriter &o, InfoType infoType) {
  if (l) {
    ...
    o << Core::XmlOpen("lattice-info") << Core::XmlEmpty("empty-lattice")
    ...
    if (l) {
      if (infoType_ == InfoTypeExtended) {
        ...
        if (bestStats.nNonEps > 0) {
          ...
          sumNonEpsNonEps_ +=
              f64(bestStats.nNonEps) * bestStats.densityNonEpsNonEps;
        }
        if (bestStats.nWords > 0) {
          ++nBestWordLattices_;
      ...
      log() << Core::XmlOpen("lattice-info") << Core::XmlEmpty("empty-lattice")
            << Core::XmlClose("lattice-info");
    return l;
  }
...
public:
  ...
  virtual void finalize() { overallInfo(log()); }
};
const Core::ParameterString InfoNode::paramInfoType("info-type", "info type",
                                                    "normal");
