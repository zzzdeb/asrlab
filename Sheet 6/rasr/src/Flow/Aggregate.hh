 ...
 *     Thus, input objects must be instances of the base class or of one of its
 * derived classes. -I/O is solved using features supported by Data.
 */
...
public:
   ...
   */
  template <class T> bool get(size_t i, DataPtr<T> &d) {
   ...
   *  of one of its descendants.
   */
  template <class T> bool get(std::vector<DataPtr<T> > &d) {
    std::vector<DataPtr<T> > dd(begin(), end());
    if (std::find_if(dd.begin(), dd.end(),
                     std::bind2nd(std::equal_to<bool>(), false)) != dd.end())
      return false;
    d = dd;
    return true;
  }
...
/** Aggregation filter.
 *  All input packets must be derived from type Aggregate::DataType, which will
 * be aggregated, e.g. their references will be collected in a vector.
 */
...
public:
  static std::string filterName() { return std::string("generic-aggregation"); }
  ...
  virtual Aggregate *
  merge(std::vector<DataPtr<Aggregate::DataType> > &inputData) {
...
namespace Core {
template <> class NameHelper<Flow::Aggregate> : public std::string {
public:
