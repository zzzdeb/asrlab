...
public:
  LmComposeLattice(ConstLatticeRef l, ConstLanguageModelRef lm, Score scale,
                   bool forceSentenceEnd, ScoreId id)
      : Precursor(l), lm_(lm), scale_(scale),
    ...
    if (fsa_->getBoundaries() == InvalidBoundaries)
      ...
      setBoundaries(ConstBoundariesRef(new MappedBoundaries(
          fsa_->getBoundaries(), Fsa::ConstMappingRef(new Mapping(this)))));
  }
  ...
  virtual void dumpState(Fsa::StateId sid, std::ostream &os) const {
    if (sid < states_.size()) {
      ...
      os << ", " << lm_->formatHistory(states_[sid].history) << ")";
    } else
      os << "unknown";
  }
    ...
    for (State::iterator a = sp->begin(); a != sp->end(); ++a) {
      ...
      if ((Fsa::FirstLabelId <= labelId) && (labelId <= Fsa::LastLabelId)) {
        ...
        /*
          ...
          Soa is that the syntaxEmissionScore has no influence and is normally
          0, thus introducing a new dimension and a new parameter seems to be
          overhead. Instead, the syntaxEmissionSale is set to lmScale.
        */
  ...
  virtual std::string describe() const {
    return Core::form("composeWithLm(%s;%s,dim=%d)", fsa_->describe().c_str(),
                      lm_->fullName().c_str(), id_);
  }
};

ConstLatticeRef composeWithLm(ConstLatticeRef l, ConstLanguageModelRef lm,
                              ScoreId id, Score scale, bool forceSentenceEnd) {
  l = mapInputToLemmaOrLemmaPronunciation(transducer(l));
  return ConstLatticeRef(
      new LmComposeLattice(l, lm, scale, forceSentenceEnd, id));
}
...
public:
  ...
  virtual void init(const std::vector<std::string> &arguments) {
    lm_ =
        Lm::Module::instance().createLanguageModel(select("lm"), Lexicon::us());
    if (!lm_)
    ...
    if (forceSentenceEnd_)
      log("Treat end of segment as end of sentence; compatible with the "
          "structure of Sprint recognizer lattices.");
  }
};
const Core::ParameterBool ComposeWithLmNode::paramForceSentenceEnd(
    "force-sentence-end", "treat end of segment as end of sentence", true);
const Core::ParameterBool ComposeWithLmNode::paramProjectInput("project-input",
                                                               "project input",
                                                               false);

NodeRef createComposeWithLmNode(const std::string &name,
