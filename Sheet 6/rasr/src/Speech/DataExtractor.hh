...
/**
 * DataExtractor is a corpus visitor application for extraction of any data
 * type.
 *
 * Output (XML format):
...
protected:
  ...
  /** Override this function to achieve the features extacted form data source
   * one-by-one.
   */
...
/**
 * FeatureVectorExtractor is a corpus visitor application for feature vector
 * extraction.
 */
...
protected:
   ...
   */
  virtual void
  setFeatureVectorDescription(const Mm::FeatureDescription::Stream &) {}

  virtual void processFeature(Core::Ref<const Feature> f) {
    processFeatureVector(f->mainStream());
  }
  /** Override this function to achieve the feature vectors extacted form data
   * source one-by-one.
   */
