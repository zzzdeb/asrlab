...
public:
  Resource(const std::string &_name, const std::string &_value,
           const SourceDescriptor *_source)
      : source_(_source), name_(_name), value_(_value),
  ...
  inline bool operator<(const Resource &r) const { return name_ < r.name_; }
  inline bool operator==(const Resource &r) const { return name_ == r.name_; }

  void write(std::ostream &os) const { os << name_ << " = " << value_; }
  ...
  void registerUsage(const std::string &n, const AbstractParameter *p,
 ...
 **/
s32 Configuration::Resource::match(
  ...
  for (StringTokenizer::Iterator itResource = tokenizer.begin(),
                                 endResource = tokenizer.end();
       hasMatch && (itResource != endResource); ++itResource) {
    hasMatch = false;
    if (*itResource == resource_wildcard_string) {
      for (std::vector<s16>::iterator itMatches = nMatches.begin() + 1;
           itMatches != nMatches.end(); ++itMatches)
        if (*(itMatches - 1) > *itMatches)
          *itMatches = *(itMatches - 1);
      hasMatch = true;
    } else {
      std::vector<std::string>::const_reverse_iterator itComponent =
          components.rbegin();
      for (std::vector<s16>::reverse_iterator itMatches = nMatches.rbegin();
           itMatches != nMatches.rend() - 1; ++itMatches, ++itComponent)
...
void Configuration::Resource::writeUsage(XmlWriter &os) const {
  for (std::vector<Usage>::const_iterator u = usage.begin(); u != usage.end();
       ++u) {
    os << "! used as: " << u->fullParameterName << " = " << u->effectiveValue;
    if (u->parameter) {
...
public:
   ...
   */
  void set(const std::string &name, const std::string &value = "true",
  ...
  const Resource *noResource() const { return &noResource_; }

  SourceDescriptor *addSource(const std::string &type,
...
Configuration::ResourceDataBase::~ResourceDataBase() {
  for (SourceList::const_iterator s = sources_.begin(); s != sources_.end();
       ++s)
    delete *s;
}

void Configuration::ResourceDataBase::set(const std::string &name,
...
Configuration::ResourceDataBase::find(const std::string &parameter) const {
  ...
  for (StringTokenizer::Iterator token = tokenizer.begin();
  ...
  // debug
  // std::cerr << "match " << parameter << "(" << resources.size() << "):" <<
  // std::endl;
  ...
  for (it = resources.begin(); it != resources.end(); it++) {
    ...
    // debug
    // if (m > 0) std::cerr << it->getName() << " matches with score " << m <<
    // std::endl;
  ...
  if (ties > 0) {
    std::cerr << "configuration warning: \"" << parameter << "\" is matched by "
  ...
  if (isLogging_) {
    ...
    if (result)
      std::cerr << parameter << "\" is matched by \"" << result->getName()
                << "\"" << std::endl;
    else
      std::cerr << parameter << "\" is not matched." << std::endl;
  }
...
void Configuration::ResourceDataBase::write(std::ostream &os) const {
  for (std::set<Resource>::const_iterator ri = resources.begin();
       ri != resources.end(); ++ri) {
    ri->write(os);
    os << "\n";
  }
...
void Configuration::ResourceDataBase::writeUsage(XmlWriter &os) const {
  for (std::set<Resource>::const_iterator r = resources.begin();
       r != resources.end(); ++r) {
...
bool Configuration::isWellFormedName(const std::string &s,
                                     bool allowWildcards) {
  enum {
    require_component,
    component,
...
Configuration::Configuration()
    : /*****************************************************************************/
      db_(0), isDataBaseOwner_(true), selection_("UNNAMED"), name_("UNNAMED") {
...
Configuration::Configuration(const Configuration &c)
    : /*****************************************************************************/
      db_(c.db_), isDataBaseOwner_(false), selection_(c.selection_),
      name_(c.name_) {}

/*****************************************************************************/
Configuration::Configuration(const Configuration &c,
                             const std::string &add_selection)
    : /*****************************************************************************/
      db_(c.db_), isDataBaseOwner_(false) {
...
/*****************************************************************************/
std::string Configuration::prepareResourceName(const std::string &selection,
...
/*****************************************************************************/
{
  ...
  if (!isWellFormedResourceName(parameter)) {
    std::cerr << "parameter name is not valid: '" << parameter << "'"
              << std::endl;
  }
...
bool Configuration::tryToSet(const std::string &name, const std::string &value,
                             const SourceDescriptor *source) {
  if (isWellFormedResourceName(name)) {
    set(name, value, source);
    return true;
  } else {
    std::cerr << "configuration error: malformed resource name \"" << name
              << "\"" << std::endl;
    return false;
  }
}
...
/*****************************************************************************/
{
  ...
  if (!ifs) {
    std::cerr << "configuration error: failed to open file \"" << filename
              << "\" for reading.";
    if (errno) {
    ...
    if (parseInclude(line, includeFilename, warningDescription)) {
      ...
      if (isRegularFile(f))
        ...
        error(filename, linenr,
              "Include file '" + includeFilename + "' does not exist.");
...
/*****************************************************************************/
{
  ...
  if (token == "include") {
    ...
    if (!token.empty() && token[0] != '=') {
      ...
      token.clear();
      iss >> token;
      if (!token.empty() && Application::us()->getCommentCharacters().find(
                                token[0]) == std::string::npos) {
...
/*****************************************************************************/
{
  ...
  if ((status != EOF) && (!parameter.empty())) {
    ...
    if (status != 0) {
      ...
      iss.unget();    // put the last character back in order to parse the
                      // remainder correctly.
      iss >> std::ws; // Seek to a non-whitespace character.
      int peek = iss.get();
      if (peek == '=') {
        // ... It was '=', so try to parse the value until a comment or new
        // line.
      ...
      } else if (Application::us()->getCommentCharacters().find(peek) ==
                     std::string::npos &&
                 peek != EOF) {
      ...
      // Neither comment nor whitespace nor '=' found but parameter is not
      // empty. Thus, it must be a flag type. Simply set the parameter to
      // 'true'.
      value = "true";
    }
  } else {
    // Line was empty or comment, whitespace or '=' found but parameter was
    // empty.
    return false;
  }
  return true;
...
/*****************************************************************************/
{
  std::cerr << "Configuration warning in " << filename << " line " << lineNumber
            << ": " << description << ". Skipping." << std::endl;
}
...
/*****************************************************************************/
{
  std::cerr << "Configuration error in " << filename << " line " << lineNumber
            << ": " << description << "." << std::endl;
}

/*****************************************************************************/
std::vector<std::string>
Configuration::setFromCommandline(const std::vector<std::string> &arguments)
/*****************************************************************************/
{
  std::string line;
  for (std::vector<std::string>::const_iterator arg = arguments.begin() + 1;
       arg != arguments.end(); ++arg) {
    if (line.size())
      line += " ";
    line += *arg;
  }
  return setFromCommandline(arguments, db_->addSource("command line", line));
}
      ...
      if (((*arg)[0] == '-') && ((*arg)[1] == '-')) {
        if ((*arg)[1] == '\0')
          ...
          if (option[0] == resource_separation_char)
            ...
            option =
                Application::us()->name() + resource_separation_string + option;
...
/*****************************************************************************/
const Configuration::Resource *
Configuration::find(const std::string &parameter) const
/*****************************************************************************/
{
...
/*****************************************************************************/
{
  ...
  while ((begin = value.find("$(", pos)) != std::string::npos) {
    ...
    if (end != std::string::npos) {
      ...
      if (isWellFormedParameterName(query)) {
        ...
        for (std::string scope = getSelection();;) {
          ...
          std::string::size_type sp =
              scope.find_last_of(resource_separation_char);
          if (sp == std::string::npos)
            sp = 0;
          scope.erase(sp);
        }
      } else {
        std::cerr << "configuration error: malformed reference \"" << query
                  << "\"" << std::endl;
      }
      ...
      std::cerr << "configuration error: unclosed bracket in value \"" << value
                << "\"" << std::endl;
      return std::string("");
    }
  }
...
/*****************************************************************************/
std::string
Configuration::resolveArithmeticExpressions(const std::string &value) const
/*****************************************************************************/
{
  std::string result;
  std::string::size_type begin, pos = 0;
  ...
  while ((begin = value.find("$[", pos)) != std::string::npos) {
    ...
    if (error) {
      std::cerr << "configuration error: " << errMsg << " in value \"" << value
                << "\"" << std::endl;
      return std::string("");
    }
  }
...
/*****************************************************************************/
{
  ...
  for (std::set<Resource>::const_iterator itResource =
           db_->getResources().begin();
       itResource != db_->getResources().end(); ++itResource) {
