...
public:
  struct Transit {
    ...
    Transit() : final(Bliss::Phoneme::term), initial(Bliss::Phoneme::term) {}
...
public:
  ...
  u32 size() const { return internal_.size(); }
  void grow(Fsa::StateId size) { internal_.grow(size); }
  bool operator==(const WordBoundaries &wordBoundaries) const {
    if (size() != wordBoundaries.size())
      return false;
    return std::equal(internal_.begin(), internal_.end(),
                      wordBoundaries.begin());
  }
  bool operator!=(const WordBoundaries &wordBoundaries) const {
...
public:
  ...
  const std::string &name(PartIndex index) const { return parts_[index]; }
...
public:
  ...
  void newArc(Fsa::State *source, Fsa::State *target,
              const Bliss::LemmaPronunciation *, Speech::Score acoustic,
              Speech::Score lm);
