...
/** Synchronization: ensures that the input stream has the same number of
 *elements and the same start-times than "target-time" stream.
 *
 *  Algorithm:
 *    -remove all elements with a start-time that do not occure in the
 *"target-time" stream. -Error messages: -there is an element in the
 *"target-time" stream whose start-time do not occure in the input stream.
 */
...
/** Ignore if the algorithm could not create a synchronised element for a target
 * start-time */
extern Core::ParameterBool paramSynchronizationIgnoreErrors;

/** Base class for algorithms which synchronise the input stream to a "target"
 * stream according to the start-times
 *
 *  Inputs:
...
public:
  static std::string filterName() { return Algorithm::name(); }
   ...
   *  creates a synchronized element of the input stream by calling
   * Algorithm::work
   */
...
public:
  static std::string fileterName() { return Algorithm::name(); }
...
template <class Algorithm>
SynchronizationNode<Algorithm>::SynchronizationNode(
    const Core::Configuration &c)
    : Core::Component(c), Node(c), firstData_(true),
...
template <class Algorithm> bool SynchronizationNode<Algorithm>::configure() {
...
bool SynchronizationNode<Algorithm>::nextData(DataPointer &dataPointer) {
  ...
  if (result) {
    ...
    if (!firstData_ &&
        !Core::isSignificantlyLess(previousStartTime_, dataPointer->startTime(),
                                   timeTolerance)) {
      criticalError(
          "Input start-times do not increase monotonously: %f after %f",
...
template <class Algorithm> void SynchronizationNode<Algorithm>::reset() {
  Algorithm::reset();
  firstData_ = true;
  previousStartTime_ = Core::Type<Time>::min;
}
