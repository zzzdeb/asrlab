...
const Core::ParameterString
    paramExternalVectorFileName("file", "name of external vector file to load");

/** ExternalVectorAddition */
template <class T> class ExternalVectorAddition {
public:
  typedef T Value;
  ...
  void operator()(std::vector<Value> &v, Flow::Vector<Value> &u) {
    ...
    for (typename std::vector<Value>::iterator itv = v.begin(); itv != v.end();
         itv++, itu++) {
      *itv += *itu;
    }
  }
...
template <class T> class ExternalVectorSubtraction {
public:
  typedef T Value;
...
template <class T> class ExternalVectorMultiplication {
public:
  typedef T Value;
...
template <class T> class ExternalVectorDivision {
public:
  typedef T Value;
...
private:
  Flow::Vector<typename ArithmeticFunction::Value> *
  loadVector(const std::string &filename) {
...
bool ExternalVectorFunctionNode<ArithmeticFunction>::work(Flow::PortId p) {
  ...
  if (sizeIn != sizeVec) {
    //			warning() << "Input Vector dimension mismatch: is " << sizeIn << "
    //should be " << sizeVec << ".\n";
    if (sizeIn > sizeVec) {
      error() << "Input Vector is to long, should be " << sizeVec << " is "
              << sizeIn << "\n.";
    }
  }
...
template <class ArithmeticFunction>
bool ExternalVectorFunctionNode<ArithmeticFunction>::setParameter(
...
bool ExternalVectorFunctionNode<ArithmeticFunction>::configure() {
  ...
  if (!configureDatatype(
          a, Flow::Vector<typename ArithmeticFunction::Value>::type()))
    return false;
