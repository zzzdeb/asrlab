...
public:
  ...
  /** Feeds new data into the Algorithm and reads data out of the Algorithm if
   * existant. Steps: 1) Read data out of Algorithm and if successful send it on
   * in the network, and exit. 2) If Algorithm cannot deliver an output, read
   * new input from the network and feed it into the algorithm and try step 1)
   * again.
   *
   *   Error handling:
   *     1) If EOS arrives from the network, flush the next output of the
   * algorithm. 2) If Algorithm cannot accept new input, flush all output of the
   * algorithm, put them into the network and put the new input into the
   * algorithm again.
   */
...
template <class Algorithm>
bool SlidingAlgorithmNode<Algorithm>::flushAllAndPut(Flow::PortId p,
                                                     const InputData &in) {
  bool result = true;
