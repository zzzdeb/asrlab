...
protected:
  ...
  void estimateMixtureWeights(std::vector<Weight> &,
                              bool normalizeWeights = true) const;
...
public:
  ...
  virtual void accumulateDenominator(DensityIndex, const FeatureVector &,
                                     Weight);
  virtual void reset();
  virtual Mixture *
...
protected:
  ...
  virtual Weight weight(DensityIndex dns) const {
    return Precursor::weight(dns) +
           iMixtureWeight(dns) * ISmoothing::constant();
  }
