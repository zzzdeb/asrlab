...
typedef Core::hash_set<AugmentedDensityEstimator, AugmentedDensityEstimatorHash,
 ...
 */
template <class Object, class Value> class ReferenceValueMap {
public:
  typedef Core::Ref<Object> ObjectRef;
  typedef Core::hash_map<ObjectRef, Value, hashReference<Object>,
                         std::equal_to<ObjectRef> >
      Map;
...
CovarianceToDensitySetMap::CovarianceToDensitySetMap(
  ...
  for (; m != mixtureEstimators.end(); ++m) {
    ...
    for (DensityIndex dnsInMix = 0; dnsInMix < mixtureEstimator->nDensities();
         ++dnsInMix) {
      AugmentedDensityEstimator augmented;
      augmented.densityEstimator =
          Core::Ref<EbwDiscriminativeGaussDensityEstimator>(required_cast(
      ...
      augmented.previousMixtureWeight =
          mixtureEstimator->previousMixtureWeight(dnsInMix);
...
public:
  ...
  IterationConstant
  operator[](Core::Ref<AbstractMixtureEstimator> mixture) const {
    return map_[mixture];
  }
};
...
Xi::Xi(const Core::Configuration &c, const MixtureEstimators &mixtureEstimators)
    : Core::Component(c), beta_(paramBeta(config)) {
...
IterationConstant
Xi::denominator(Core::Ref<AbstractMixtureEstimator> mixtureEstimator) {
  verify(mixtureEstimator);
  IterationConstant maxAbsDtWeight = 0;
  IterationConstant maxNumDenWeight = 0;
  DensityEstimators::const_iterator d =
      mixtureEstimator->densityEstimators().begin();
  for (; d != mixtureEstimator->densityEstimators().end(); ++d) {
    ...
    if (maxAbsDtWeight < absDtWeight) {
      ...
      maxNumDenWeight =
          std::max(meanEstimator->dtWeight() + meanEstimator->denWeight(),
                   meanEstimator->denWeight());
    }
  }
...
protected:
  ...
  IterationConstant minimumDk(CovarianceToDensitySetMap::const_iterator,
                              const CovarianceToMeanSetMap::MeanSet &);
...
const Core::ParameterFloat Icb::paramSigmaMinimumFactor(
    "sigma-minimum-factor", "factor to determine sigma-minimum dynamically, \
...
Icb::Icb(const Core::Configuration &c,
         ...
         const CovarianceToDensitySetMap &cim, IterationConstant stepSize)
    : Precursor(c), default_(1), stepSize_(stepSize),
...
VarianceType Icb::minimumVariance(const CovarianceToDensitySetMap &cim) const {
  ...
  for (; it != cim.end(); ++it) {
    Core::Ref<const EbwDiscriminativeCovarianceEstimator> covarianceEstimator(
        required_cast(const EbwDiscriminativeCovarianceEstimator *,
                      it->first.get()));
    const std::vector<VarianceType> &prevCov =
        covarianceEstimator->previousCovariance();

    os << Core::XmlOpen("previous-covariance");
    std::copy(prevCov.begin(), prevCov.end(),
              std::ostream_iterator<VarianceType>(os, " "));
    os << Core::XmlClose("previous-covariance");

    result =
        std::min(result, *std::min_element(prevCov.begin(), prevCov.end()));
  }
  return result;
}
...
void Icb::setSigmaMinimum(const CovarianceToDensitySetMap &cim) {
  ...
  sigmaMinimum_ =
      paramSigmaMinimum(config, paramSigmaMinimumFactor(config) * tmp);
...
IterationConstant Icb::minimumDk(
  ...
  Core::Ref<EbwDiscriminativeCovarianceEstimator> covarianceEstimator(
      required_cast(EbwDiscriminativeCovarianceEstimator *,
                    covarianceAndDensitySet->first.get()));
  const std::vector<Sum> dtSum2 = covarianceEstimator->dtSum(meanSet);
  const std::vector<VarianceType> &previousCovariance =
      covarianceEstimator->previousCovariance();
  ...
  for (; d != densitySet.end(); ++d) {
    Core::Ref<EbwDiscriminativeMeanEstimator> meanEstimator(required_cast(
        EbwDiscriminativeMeanEstimator *, d->densityEstimator->mean().get()));
    ...
    for (ComponentIndex i = 0; i < dimension; ++i) {
      ...
      denominator[i] +=
          (previousCovariance[i] - sigmaMinimum_) * d->previousMixtureWeight;
    }
  }
  ...
  for (; it != cim.end(); ++it) {
    ...
    for (; d != densitySet.end(); ++d) {
      if (map_.find(d->mixtureEstimator) == map_.end()) {
        map_.insert(d->mixtureEstimator,
                    stepSize_ * std::max(default_, D_k_min));
      }
      Core::Ref<EbwDiscriminativeMeanEstimator> meanEstimator(required_cast(
          EbwDiscriminativeMeanEstimator *, d->densityEstimator->mean().get()));
      IterationConstant D_k = 1 / xi_[d->mixtureEstimator];
      ...
      map_[d->mixtureEstimator] =
          std::max(stepSize_ * D_k, map_[d->mixtureEstimator]);
    }
  }
}
...
GlobalRwthIterationConstants::GlobalRwthIterationConstants(
    const Core::Configuration &c)
    : Precursor(c), usePooledVariances_(true), ic_(0) {}
...
void GlobalRwthIterationConstants::initialize(
    ...
    const CovarianceToMeanSetMap &meanSetMap) {
  verify(!ic_);
  ic_ = new InternalIc::GlobalIcb(select("icb"), mixtureEstimators, cim,
                                  stepSize_, meanSetMap);
}

void GlobalRwthIterationConstants::set(
    const InternalIc::AugmentedDensityEstimator &d) {
...
LocalRwthIterationConstants::LocalRwthIterationConstants(
    const Core::Configuration &c)
    : Precursor(c), usePooledVariances_(true), ic_(0) {}
...
void LocalRwthIterationConstants::initialize(
    ...
    const CovarianceToMeanSetMap &meanSetMap) {
  verify(!ic_);
  ic_ = new InternalIc::LocalIcb(select("icb"), mixtureEstimators, cim,
                                 stepSize_, meanSetMap);
}

void LocalRwthIterationConstants::set(
    const InternalIc::AugmentedDensityEstimator &d) {
...
const Core::ParameterFloat
    CambridgeIterationConstants::paramMinimumObservationWeight(
...
CambridgeIterationConstants::CambridgeIterationConstants(
    const Core::Configuration &c)
    : Precursor(c),
      minimumObservationWeight_(paramMinimumObservationWeight(config)),
      e_(paramE(config)), ic_(0) {}
...
Sum CambridgeIterationConstants::maximumRoot(
  ...
  const std::vector<VarianceType> &sigma_sl =
      covarianceEstimator->previousCovariance();
  ...
  for (ComponentIndex i = 0; i < dimension; ++i) {
    ...
    const Sum b = Gamma_x2_sl[i] +
                  Gamma_1_sl * (sigma_sl[i] + mu_sl[i] * mu_sl[i]) -
                  2.0 * Gamma_x_sl[i] * mu_sl[i];
...
void CambridgeIterationConstants::set(
    const InternalIc::AugmentedDensityEstimator &d) {
  IterationConstant ic = default_;
  ...
  IterationConstant eHalfmax =
      0.5 * stepSize_ * rootMax / meanEstimator->denWeight();
  verify(eHalfmax >= 0);
 ...
 * IterationConstants
 */
Core::Choice IterationConstants::choiceType("global-rwth", globalRwth,
                                            "local-rwth", localRwth,
                                            "cambridge", cambridge,
                                            Core::Choice::endMark());

Core::ParameterChoice
    IterationConstants::paramType("type", &choiceType,
                                  "type of iteration constants", globalRwth);
...
const Core::ParameterFloat IterationConstants::paramMinimumIcd(
    "minimum-icd", "minimum value for the iteration constants", 1, 0);

IterationConstants::IterationConstants(const Core::Configuration &c)
    : Core::Component(c), default_(paramMinimumIcd(config)),
...
void IterationConstants::dumpIterationConstants(
  ...
  for (MixtureIndex mix = 0; mix < mixtureEstimators.size(); ++mix) {
    ...
    for (DensityIndex dns = 0; dns < densityEstimators.size(); ++dns) {
      Core::Ref<EbwDiscriminativeMeanEstimator> meanEstimator(
          required_cast(EbwDiscriminativeMeanEstimator *,
...
IterationConstants *
IterationConstants::createIterationConstants(const Core::Configuration &c) {
