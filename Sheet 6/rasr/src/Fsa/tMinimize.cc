...
protected:
  ...
  StateMappedAutomaton(_ConstAutomatonRef f) : Precursor(f) {}
...
public:
  ...
  virtual _ConstStateRef getState(Fsa::StateId s) const {
    ...
    for (typename _State::iterator a = sp->begin(), b = sp->begin();
         a != sp->end(); ++a)
...
private:
  ...
  struct ClassEntry {
    Fsa::StateId id_;
    ClassId class_;
    ClassEntry(Fsa::StateId id, ClassId c) : id_(id), class_(c) {}
    bool operator<(const ClassEntry &e) const { return class_ < e.class_; }
  };
  ...
  public:
    HashTargetMappedAndLabels(_ConstAutomatonRef f, const ClassMap &classMap,
    ...
    size_t operator()(Fsa::StateId s) const {
      ...
      for (typename A::State::const_iterator a = sp->begin(); a != sp->end();
           ++a) {
  ...
  public:
    EqualTargetMappedAndLabels(_ConstAutomatonRef f, const ClassMap &classMap,
                               const StatePotentials<_Weight> &statePotentials)
        : fsa_(f), classMap_(classMap), semiring_(f->semiring()),
          statePotentials_(statePotentials) {}
    bool operator()(Fsa::StateId as, Fsa::StateId bs) const {
      if (semiring_->compare(statePotentials_[as], statePotentials_[bs]) != 0)
        return false;
      ...
      for (typename A::State::const_iterator aa = a->begin(), ba = b->begin();
           aa != a->end(); ++aa, ++ba)
        if ((classMap_[aa->target()] != classMap_[ba->target()]) ||
            (aa->input() != ba->input()) || (aa->output() != ba->output()) ||
            (semiring_->compare(aa->weight(), ba->weight()) != 0))
          return false;
      return true;
    }
  };
  ...
  _ConstAutomatonRef minimize(_ConstAutomatonRef f, Fsa::OptimizationHint hint,
                              bool progress) {
    // 1. get state potentials
    StatePotentials<_Weight> statePotentials = ssspBackward<A>(f, progress);
    ...
    //    - the equivalence set: all active states sorted by the class id (start
    //    with 2 classes: final/non-final)
    ...
    EqualTargetMappedAndLabels equalTargetMappedAndLabels(f, classMap,
                                                          statePotentials);
    while (didSplit) {
      // iterate over all classes (they are sorted by class id) and build new
      // equivalence set
      nes.clear();
      didSplit = false;
      ClassId nClasses = 0;
      for (typename EquivalenceSet::const_iterator s = es.begin();
           s != es.end();) {
        ClassId c = s->class_;
        if (classSize[c] == 1) { // singleton classes can't be split anymore
          ...
          // The best size for the following hash table is of course a prime
          // number, which may be costly to calculate. The following is already
          // an optimization over previous versions:
          //
          // 1. A sorted vector for each class where splitted classes get merged
          // again. Performed badly, because
          //    either the sorted vector used ConstStateRefs which was
          //    inefficient memory-wise or state ids which was painfully slow in
          //    case of packed automata.
          //
          // 2. A static global hash table which performed badly, because class
          // sizes continously shrink and
          //    clearing the big same sized hash table all the time becomes a
          //    big burden.
          size_t startOfNewSets = nes.size();
          Fsa::Hash<Fsa::StateId, HashTargetMappedAndLabels,
                    EqualTargetMappedAndLabels>
              states(hashTargetMappedAndLabels, equalTargetMappedAndLabels,
                     classSize[c]);
          ...
          if (states.size() > 1) {
            // just another trick:
            // 1. we only need to sort sets if we know there was more than 1
            // class
            // 2. overall complexity of this is smaller: sum(N_i)*log(N_i) <=
            // sum(N_i)*log(sum(N_i))
...
template <class A>
typename A::ConstRef minimize(typename A::ConstRef f, bool progress) {
  return typename A::ConstRef(
      new MinimizeAutomaton<A>(f, Fsa::OptimizationHintSpeed, progress));
}
template <class A>
...
template <class A> typename A::ConstRef minimizeSimple(typename A::ConstRef f) {
  return Ftl::determinize<A>(
      Ftl::transpose<A>(Ftl::determinize<A>(Ftl::transpose<A>(f))));
}
