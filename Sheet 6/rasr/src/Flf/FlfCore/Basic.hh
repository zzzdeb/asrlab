 ...
 * Replace semiring
 *
 * Rescale semiring: new semiring is old semiring, where scales and/or key names
 *are changed Change semiring: old and new semiring must have same size Project
 *semiring: size(projection-matrix) = length(new semiring) x length(old
 *semiring)
 *
 **/
...
ConstLatticeRef rescale(ConstLatticeRef l, ScoreId id, Score scale,
                        const Key &key = Semiring::UndefinedKey);
...
ConstLatticeRef fromFsa(Fsa::ConstAutomatonRef f, ConstSemiringRef semiring,
                        ScoreId id, ScoresRef defaultScore = ScoresRef());
...
ConstLatticeRef fromUnweightedFsa(Fsa::ConstAutomatonRef f,
                                  ConstSemiringRef semiring,
                                  ScoresRef defaultScore = ScoresRef());

/**
 * Flf to Fsa
 *
 **/
Fsa::ConstAutomatonRef toFsa(ConstLatticeRef l,
                             const ScoreList &scales = ScoreList());
Fsa::ConstAutomatonRef toFsa(ConstLatticeRef l, ScoreId id,
                             bool scaled = false);
std::vector<Fsa::ConstAutomatonRef> toFsaVector(ConstLatticeRef l,
                                                bool scaled = false);
Fsa::ConstAutomatonRef
toUnweightedFsa(ConstLatticeRef l,
                const Fsa::Weight &defaultWeight = Fsa::Weight(Semiring::One));
 ...
 * Append
 *
 * appends the scores of two topologically equivalent (down to state numbering!)
 *lattices; scores are concatenated, i.e. new semiring is the concatentation
 **/
