...
void LemmaPronunciationId_hash_map::init(Bliss::LexiconRef lexicon) {
  ...
  for (Core::tie(lpi, lpi_end) = lexicon->lemmaPronunciations(); lpi != lpi_end;
       ++lpi) {
...
void LemmaPronunciationId_hash_map::read(const std::string &filename) {
  ...
  while (std::getline(stats, line)) {
    ...
    for (Core::tie(lpi, lpi_end) = lexicon->lemmaPronunciations();
         lpi != lpi_end; ++lpi) {
      const Bliss::LemmaPronunciation *lp = (*lpi);
...
Core::ParameterString LatticeArcAccumulatorNode::paramLexiconFilename(
    "lexicon-filename", "filename to write lexicon");

Core::ParameterString
    LatticeArcAccumulatorNode::paramPronunciationStatsFilenameRead(
        "pronunciation-read", "filename to read pronunciation statistics");

Core::ParameterString
    LatticeArcAccumulatorNode::paramPronunciationStatsFilenameWrite(
        "pronunciation-write", "filename to write pronunciation statistics");

const Core::ParameterFloat LatticeArcAccumulatorNode::paramMinimumPronWeight(
    "minimal-pronunciation-weight",
    "minimal-pronunciation-weight that will at least remain", 0.0001);

const Core::ParameterFloat LatticeArcAccumulatorNode::paramTauWeight(
    "tau", "tau parameter in MAP update equation", 0.0);

Core::ParameterString LatticeArcAccumulatorNode::paramEncoding(
    "encoding", "encoding of updated lexicon", "ISO-8859-1");

LatticeArcAccumulatorNode::LatticeArcAccumulatorNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c), tau_(paramTauWeight(config)),
      ...
      encoding_(paramEncoding(c)), needInit_(true) {
  addInputs(2);
  addOutputs(1);
...
LatticeArcAccumulatorNode::~LatticeArcAccumulatorNode() {
  ...
  Core::Ref<const Bliss::LemmaPronunciationAlphabet> lpa =
  ...
  for (Core::tie(lpi, lpi_end) = lexicon->lemmaPronunciations(); lpi != lpi_end;
    ...
    Bliss::LemmaPronunciation *lp =
        const_cast<Bliss::LemmaPronunciation *>(*lpi);
    Fsa::LabelId lemmaId = lp->lemma()->id();
    Fsa::LabelId lemmaPronId = lp->id();
...
bool LatticeArcAccumulatorNode::updateWeightsMap(
    Core::hash_map<Fsa::LabelId, f32> &counts,
    Core::hash_map<Fsa::LabelId, f32> &weights, f32 tau) {
  f32 totalCount = 0.0;
  ...
  if (totalCount > 0.0) {
    ...
    for (pron_i = counts.begin(); pron_i != counts.end(); pron_i++) {
      ...
      weights[pronId] = interpolationWeight * (pron_i->second / totalCount) +
  ...
  if (needInit_) {
    ...
    if (pronunciationStatsFilenameRead_ != "") {
      std::vector<std::string> files =
          Core::split(pronunciationStatsFilenameRead_, " ");
