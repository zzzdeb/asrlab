...
AcousticModelRef getAm(const Core::Configuration &config) {
  return Am::Module::instance().createAcousticModel(
      config, Bliss::LexiconRef(Lexicon::us()));
}

ScaledLanguageModelRef getLm(const Core::Configuration &config) {
  return Lm::Module::instance().createScaledLanguageModel(
      config, Bliss::LexiconRef(Lexicon::us()));
}

ModelCombinationRef getModelCombination(const Core::Configuration &config,
                                        AcousticModelRef acousticModel,
                                        ScaledLanguageModelRef languageModel) {
  return ModelCombinationRef(new Speech::ModelCombination(
      config, Bliss::LexiconRef(Lexicon::us()), acousticModel, languageModel));
}
// -------------------------------------------------------------------------

// -------------------------------------------------------------------------
SegmentwiseFeatureExtractor::SegmentwiseFeatureExtractor(
    const Core::Configuration &config, DataSourceRef dataSource)
...
void SegmentwiseFeatureExtractor::enterRecording(
    const Bliss::Recording *recording) {
  ...
  dataSource_->setParameter("recording-index",
                            Core::form("%d", u32(nRecordings_)));
  lastRecordingName_ = recording->fullName();
  ++nRecordings_;
}

void SegmentwiseFeatureExtractor::enterSegment(
  ...
  dataSource_->setParameter(
      "segment-type", std::string(Bliss::Segment::typeId[segment->type()]));
  dataSource_->setParameter("acoustic-condition",
                            segment->condition() ? segment->condition()->name()
                                                 : "");
  ...
  // disassemble segment fullname: .../segment-1/segment-0 and
  // corpus-0/corpus-1/...
  ...
  while (corpusSection) {
    dataSource_->setParameter("corpus-" +
                                  Core::form("%d", corpusSection->level()),
                              corpusSection->name());
  ...
  if (segment->speaker()) {
    ...
    dataSource_->setParameter(
        "gender", Bliss::Speaker::genderId[segment->speaker()->gender()]);
  }
...
void SegmentwiseFeatureExtractor::leaveSegment(
    const Bliss::SpeechSegment *segment) {
...
SegmentwiseSpeechProcessor::SegmentwiseSpeechProcessor(
    const Core::Configuration &config, ModelCombinationRef modelCombination) {
  Core::Configuration featureExtractionConfig(config, "feature-extraction");
  DataSourceRef dataSource = DataSourceRef(
      Speech::Module::instance().createDataSource(featureExtractionConfig));
  featureExtractor_ = SegmentwiseFeatureExtractorRef(
      new SegmentwiseFeatureExtractor(featureExtractionConfig, dataSource));
  modelAdaptor_ =
      SegmentwiseModelAdaptorRef(new SegmentwiseModelAdaptor(modelCombination));
}

SegmentwiseSpeechProcessor::SegmentwiseSpeechProcessor(
    SegmentwiseFeatureExtractorRef featureExtractor,
    SegmentwiseModelAdaptorRef modelAdaptor)
    : featureExtractor_(featureExtractor), modelAdaptor_(modelAdaptor) {}

SegmentwiseSpeechProcessor::~SegmentwiseSpeechProcessor() {}

void SegmentwiseSpeechProcessor::processSegment(
    const Bliss::SpeechSegment *segment) {
  ...
  if (dataSource->getData(feature)) {
    ...
    AcousticModelRef acousticModel =
        modelAdaptor_->modelCombination()->acousticModel();
    if (acousticModel) {
      Mm::FeatureDescription *description =
          feature->getDescription(*featureExtractor_);
