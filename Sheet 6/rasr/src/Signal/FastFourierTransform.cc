...
bool FastFourierTransform::transform(std::vector<Data> &data) {
  if (data.size() > maximalInputSize()) {
    lastError_ = Core::form(
        "Input data size (%zd) is larger then maximal input size (%d).",
        data.size(), maximalInputSize());
    return false;
  }

  return zeroPadding(data) && applyAlgorithm(data) && estimateContinuous(data);
}
...
bool RealInverseFastFourierTransform::pack(std::vector<Data> &data) {
  ...
  if (data[1] != (Data)0 || data[data.size() - 1] != (Data)0) {
    lastError_ = Core::form("For real inverse FFT, imag(input[0]) and "
                            "imag(input[FFT-length - 1]) need to be zero.");
    return false;
  }
...
bool RealInverseFastFourierTransform::estimateContinuous(
...
bool ComplexInverseFastFourierTransform::applyAlgorithm(
    std::vector<Data> &data) {
...
const Core::ParameterInt Signal::paramFftLength("length",
                                                "number of FFT points", 0, 0);

const Core::ParameterFloat Signal::paramFftMaximumInputSize(
    "maximum-input-size", "number of FFT points = max-input-size * sampe-rate",
    0, 0);
