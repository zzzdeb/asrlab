...
MixtureSetEstimator::createDensityEstimator(const GaussDensity &) {
  return new GaussDensityEstimator;
}
...
CovarianceEstimator *
MixtureSetEstimator::createCovarianceEstimator(const Covariance &covariance) {
  return new CovarianceEstimator(covariance.dimension());
}

bool MixtureSetEstimator::accumulate(Core::BinaryInputStreams &is,
