...
StaticLatticeRef applyEpsClosureFilter(ConstLatticeRef l) {
  ...
  s->setDescription(
      Core::form("eps-closure-filter(%s)", l->describe().c_str()));
  ...
  while (!Q.empty()) {
    ...
    if (visited[sid])
      continue;
    visited[sid] = true;
    verify((hyps[sid].score == Semiring::Max) &&
           (hyps[sid].bptr == Fsa::InvalidStateId));
    ...
    for (StateIdList::const_iterator itSid = epsHull.begin(),
                                     endSid = epsHull.end();
         itSid != endSid; ++itSid) {
      ...
      for (;;) {
        ...
        if (!s->hasState(hyp.bptr)) {
          ...
          s->setState(
              new State(bptrSr->id(), bptrSr->tags(), bptrSr->weight()));
        }
        State *sp = s->fastState(hyp.bptr);
        const Arc &a = *hyp.a;
        State::iterator pos =
            sp->lower_bound(a, Ftl::byInputAndTarget<Lattice>());
        if ((pos == sp->end()) || (a.target() != pos->target()) ||
            (a.input() != pos->input()))
          sp->insert(pos, a);
        epsSid = hyp.bptr;
    ...
    epsHull.clear();
    for (StateIdList::const_iterator itSid = epsClosure.begin(),
                                     endSid = epsClosure.end();
         itSid != endSid; ++itSid) {
      Hyp &hyp = hyps[*itSid];
      hyp.visited = false;
...
StaticLatticeRef applyEpsClosureWeakDeterminizationFilter(ConstLatticeRef l) {
  ...
  while (!Q.empty()) {
    ...
    while (a != a_end) {
      ...
      for (Fsa::LabelId label = a->input();
           (a != a_end) && (a->input() == label); ++a) {
...
StaticLatticeRef applyEpsClosureStrongDeterminizationFilter(ConstLatticeRef l) {
  ...
  typedef Core::Vector<std::pair<
      Fsa::StateId, std::pair<State::const_iterator, State::const_iterator> > >
      StateArcRangeList;
  ...
  HypList leftHyps(topologicalOrderMap->maxSid + 1),
  ...
  while (!Q.empty()) {
    ...
    verify((leftHyps[sid].score == Semiring::Max) &&
           (leftHyps[sid].bptr == Fsa::InvalidStateId));
    ...
    while (!epsQ.empty()) {
      ...
      if (epsA != epsEnd) {
        leftEpsExtendedHull.push_back(
            std::make_pair(epsSid, std::make_pair(epsA, epsEnd)));
    ...
    // add left finals
    for (StateIdList::const_iterator itSid = leftEpsFinalsHull.begin(),
                                     endSid = leftEpsFinalsHull.end();
         itSid != endSid; ++itSid) {
      Fsa::StateId epsSid = *itSid;
      if (!s->hasState(epsSid)) {
        ConstStateRef epsSr = l->getState(epsSid);
    ...
    while (nextLabel != Core::Type<Fsa::LabelId>::max) {
      ...
      for (StateArcRangeList::iterator itSidRange = leftEpsExtendedHull.begin(),
                                       endSidRange = leftEpsExtendedHull.end();
           itSidRange != endSidRange; ++itSidRange) {
        Fsa::StateId epsSid = itSidRange->first;
        Score fwdScore = leftHyps[epsSid].score;
        std::pair<State::const_iterator, State::const_iterator> &epsRange =
            itSidRange->second;
        for (; (epsRange.first != epsRange.second) &&
               (epsRange.first->input() == label);
             ++epsRange.first) {
        ...
        if ((epsRange.first != epsRange.second) &&
            (epsRange.first->input() < nextLabel))
      ...
      // add label and epsilon arcs from right and left closures; reset
      // traceback arrays
      for (StateIdList::const_iterator itSid = rightEpsHull.begin(),
                                       endSid = rightEpsHull.end();
           itSid != endSid; ++itSid) {
      ...
      rightEpsHull.clear();
      for (StateIdList::const_iterator itSid = rightEpsClosure.begin(),
                                       endSid = rightEpsClosure.end();
           itSid != endSid; ++itSid) {
    ...
    for (StateIdList::const_iterator itSid = leftEpsClosure.begin(),
                                     endSid = leftEpsClosure.end();
         itSid != endSid; ++itSid) {
...
protected:
  virtual ConstLatticeRef filter(ConstLatticeRef l) {
    ...
    if (l->type() != Fsa::TypeAcceptor) {
      warning("%s: \"%s\" is a transducer, but result will be an acceptor, "
              "i.e. output will be lost.",
...
public:
  NonWordClosureFilterNode(const std::string &name,
                           const Core::Configuration &config)
      : FilterNode(name, config) {}
  ~NonWordClosureFilterNode() {}
};
NodeRef createNonWordClosureFilterNode(const std::string &name,
                                       const Core::Configuration &config) {
...
public:
  NonWordClosureWeakDeterminizationFilterNode(const std::string &name,
                                              const Core::Configuration &config)
      : FilterNode(name, config) {}
...
NodeRef createNonWordClosureStrongDeterminizationFilterNode(
    const std::string &name, const Core::Configuration &config) {
  return NodeRef(
      new NonWordClosureStrongDeterminizationFilterNode(name, config));
}
// -------------------------------------------------------------------------
...
public:
  NormalizeEpsilonClosureLattice(ConstLatticeRef l)
    ...
    ConstStateMapRef topologicalOrderMap = findTopologicalOrder(l);
    verify(topologicalOrderMap &&
           (topologicalOrderMap->maxSid != Fsa::InvalidStateId));
  ...
  virtual ConstStateRef getState(Fsa::StateId sid) const {
    ...
    if ((a == a_end) || (a->input() != Fsa::Epsilon) ||
    ...
    for (; (a != a_end) && (a->input() == Fsa::Epsilon) &&
           (a->output() == Fsa::Epsilon);
         ++a)
    ...
    while (!epsQ.empty()) {
      ...
      if ((a != a_end) && ((a_end - 1)->input() == Fsa::Epsilon) &&
          ((a_end - 1)->output() == Fsa::Epsilon)) {
        if (epsSr->isFinal()) {
          sp->addTags(Fsa::StateTagFinal);
        ...
        for (; a != a_end; ++a) {
          ...
          if (!epsClosureScores_[a->target()]) {
            ...
            epsClosureScores_[a->target()] =
                semiring_->collect(epsClosureScores_[a->target()],
                                   semiring_->extend(score, a->weight()));
        }
      } else
  ...
  virtual std::string describe() const {
    return Core::form("normalize-epsilon-closure(%s)",
                      fsa_->describe().c_str());
  }
};
...
public:
  NonWordClosureNormalizationFilterNode(const std::string &name,
                                        const Core::Configuration &config)
      : FilterNode(name, config) {}
  ~NonWordClosureNormalizationFilterNode() {}
};
NodeRef
createNonWordClosureNormalizationFilterNode(const std::string &name,
                                            const Core::Configuration &config) {
...
public:
  NonWordClosureRemovalFilterNode(const std::string &name,
                                  const Core::Configuration &config)
      : FilterNode(name, config) {}
  ~NonWordClosureRemovalFilterNode() {}
};
NodeRef
createNonWordClosureRemovalFilterNode(const std::string &name,
                                      const Core::Configuration &config) {
