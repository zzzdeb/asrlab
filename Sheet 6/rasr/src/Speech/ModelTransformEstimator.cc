...
const Core::Choice ModelTransformEstimator::mllrModelingChoice(
    "full", fullMllr, "semi-tied", semiTiedMllr, "band", bandMllr, "shift",
    shiftMllr, Core::Choice::endMark());
...
ModelTransformEstimator::ModelTransformEstimator(const Core::Configuration &c,
                                                 Operation o)
    : Component(c), Precursor(c, o) {
...
void ModelTransformEstimator::postProcess() {
  AdaptorCache adaptorCache(select("adaptor-cache"),
