...
bool IntelMMXL2NormCodeGenerator::appendProlog() {
  ...
  */
  static const u8 code[] = {0x57, 0x56, 0x53, 0x8b, 0x74, 0x24,
                            0x10, 0x8b, 0x5c, 0x24, 0x14};
...
bool IntelMMXL2NormCodeGenerator::appendFetchA(int block, u8 offset) {
  /* block = 0
     movq   mm0,DWORD PTR [esi + offset]   ; copy 64 bits from mean vector in
     mm0

     block = 1
     movq   mm4,DWORD PTR [esi + offset]   ; move 64 bits from mean vector in
     mm4
...
bool IntelMMXL2NormCodeGenerator::appendFetchB(int block, u8 offset) {
  /* block = 0
     movq   mm1,DWORD PTR [ebx + offset]   ; copy 64 bits from feature vector in
     mm1

     block = 1
     movq   mm5,DWORD PTR [ebx + offset]   ; copy 64 bits from feature vector in
     mm5
  */
...
bool IntelMMXL2NormCodeGenerator::appendSubtract(int block) {
  /* block = 0
     psubusb mm0,mm1        ; mm0 -= mm1 (SIMD subtraction of packed unsigned
     integers) psubusb mm1,mm2        ; mm1 -= mm2

     block = 1
     psubusb mm4,mm5
     psubusb mm5,mm6
  */
  static const u8 code[2][6] = {{0x0f, 0xd8, 0xc1, 0x0f, 0xd8, 0xca},
...
bool IntelMMXL2NormCodeGenerator::appendUnpackAndAdd(int block) {
  /* block = 0
     punpckhbw mm1,mm0      ; mm1 = unpack and interleave high order bytes from
     mm1 and mm0 ; mm0 is zero => convert higher 4 byte integers in mm1 to word
     integers punpcklbw mm2,mm0      ; convert lower 4 byte integers in mm2 to
     word integers
                            ;
     pmaddwd   mm1,mm1      ; multiply and add packed mm1 = | mm1[3] * mm1[3] +
     mm1[2] * mm1[2], ;                               | mm1[1] * mm1[1] + mm1[0]
     * mm1[0] pmaddwd   mm2,mm2
...
bool IntelMMXL2NormCodeGenerator::appendAccumulate(int block) {
  /* block = 0
     ...
     paddd  mm7,mm6
  */
  static const u8 code[2][6] = {{0x0f, 0xfe, 0xf9, 0x0f, 0xfe, 0xfa},
...
bool IntelMMXL2NormCodeGenerator::appendEpilog() {
  /*
    ...
    ret
  */
  static const u8 code[] = {0x0f, 0x6f, 0xc7, 0x0f, 0x73, 0xd7,
                            0x20, 0x0f, 0xfe, 0xc7, 0x0f, 0x7e,
                            0xc0, 0x5b, 0x5e, 0x5f, 0xc3};
  return append(code, sizeof(code));
}

IntelMMXL2NormCodeGenerator::IntelMMXL2NormCodeGenerator(
    const Core::Configuration &c, size_t d)
    : CodeGenerator(c) {
  if (d > 128) {
...
IntelMMXResetCodeGenerator::IntelMMXResetCodeGenerator(
