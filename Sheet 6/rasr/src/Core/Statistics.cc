...
template <typename T> void Statistics<T>::write(XmlWriter &os) const {
  os << Core::XmlOpen("statistic") + Core::XmlAttribute("name", name_) +
            Core::XmlAttribute("type", "scalar");
  if (nObs_) {
    os << XmlFull("min", min_) << XmlFull("avg", average())
       << XmlFull("max", max_);
  } else {
    os << "n/a";
  }
...
ChoiceStatistics::ChoiceStatistics(const std::string &n, const Choice &choice)
    : name_(n), choice_(choice) {
  std::vector<Value> values;
  choice_.getValues(values);
  Value max = Type<Value>::min;
  for (std::vector<Value>::const_iterator v = values.begin(); v != values.end();
       ++v) {
    require(*v >= 0);
    if (max < *v)
      max = *v;
...
void ChoiceStatistics::clear() { std::fill(counts_.begin(), counts_.end(), 0); }
  ...
  if (stop_clock == -1) {
    ...
    if (stop_clock == -1)
      Core::Application::us()->error(
          "cannot get process time using times(): %s", strerror(errno));
  }
  // ensure(stop_clock != -1);
}
...
void Timer::write(XmlWriter &os) const {
  os << XmlOpen("timer") << XmlFull("user", user())
     << XmlFull("system", system()) << XmlFull("elapsed", elapsed())
     << XmlClose("timer");
}
...
Core::HistogramStatistics::HistogramStatistics(std::string name, u32 bins)
    : Precursor(name), scale_(1), bins_(bins) {}
...
void Core::HistogramStatistics::operator+=(unsigned int v) {
  ...
  // We keep the array significantly larger than the count of bins, so we have
  // some more precision
...
void Core::HistogramStatistics::write(XmlWriter &os) const {
  os << Core::XmlOpen("statistic") + Core::XmlAttribute("name", name()) +
            Core::XmlAttribute("type", "scalar");
  if (nObservations()) {
    os << XmlFull("min", minimum()) << XmlFull("avg", average())
       << XmlFull("max", maximum());
    ...
    for (u32 currentBin = 0; currentBin < bins_; ++currentBin) {
      ...
      while (arrayEnd < counts_.size() &&
             (currentBin == bins_ - 1 || usedArrayWeight < targetArrayWeight)) {
      ...
      for (u32 a = arrayStart; a < oldArrayEnd; ++a) {
        count += counts_[a];
        if (counts_[a])
          arrayEnd = a + 1; // update arrayEnd so arrayStart and arrayEnd
                            // enclose the real range as exactly as possible
      }

      if (count == 0)
        break;

      os << XmlOpen("bin") + XmlAttribute("start", arrayStart * scale_) +
                XmlAttribute("end", arrayEnd * scale_);
      os << count;
      os << XmlClose("bin");
