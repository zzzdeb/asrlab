...
public:
  ...
  GaussDensity *
  estimate(const ReferenceIndexMap<AbstractMeanEstimator> &,
  ...
  static bool accumulate(Core::BinaryInputStreams &is,
...
public:
   ...
   *  The @c map contains mean estimator references for each covariance
   * estimator. The @c minimumVariance specifies the minimum value in covariance
   * matrix. If minimum variance is not supported by a derived estimator class,
   *    overload the protected estimate function without this additional
   * parameter.
   */
  virtual Covariance *estimate(const CovarianceToMeanSetMap &map,
...
public:
  typedef VectorAccumulator<FeatureType, std::plus<Sum>, plusWeighted<Sum> >
      Accumulator;
...
public:
  typedef VectorAccumulator<FeatureType, plusSquare<Sum>,
                            plusSquareWeighted<Sum> >
      Accumulator;
   ...
   */
  typedef VectorAccumulator<Sum, plusNormalizedSquare<Sum>,
                            plusNormalizedSquare<Sum> >
      WeighedMeanSquareSum;

protected:
  Accumulator accumulator_;

protected:
  void applyMinimumVariance(VarianceType minimumVariance,
                            DiagonalCovariance &) const;
...
class CovarianceToMeanSetMap {
public:
  typedef Core::hash_set<Core::Ref<AbstractMeanEstimator>,
                         hashReference<AbstractMeanEstimator> >
      MeanSet;
  typedef Core::hash_map<Core::Ref<AbstractCovarianceEstimator>, MeanSet,
