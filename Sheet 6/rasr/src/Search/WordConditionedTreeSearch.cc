...
public:
  ...
  void write(std::ostream &os,
    ...
    os << "t=" << std::setw(5) << time << "	s=" << std::setw(8) << score;
    if (pronunciation) {
      os << "	" << std::setw(20) << std::setiosflags(std::ios::left)
 ...
 */
void WordConditionedTreeSearch::traceback(Ref<Trace> end,
...
private:
  ...
  struct TreeInstance {
    ...
    typedef Core::hash_map<
        const Bliss::LemmaPronunciation *, LmCacheItem,
        Core::conversion<const Bliss::LemmaPronunciation *, size_t> >
        LmCache;
    ...
    TreeInstance(Lm::History _history) : inactive(0), history(_history) {
      stateHypotheses.begin = stateHypotheses.end = 0;
    }

    void enter(Ref<Trace> trace, StateTree::StateId entryState, Score score) {
  ...
  struct WordEndHypothesis {
    ...
    inline void addLmScore(Core::Ref<const Lm::ScaledLanguageModel> lm,
                           Score wpScale);
    inline void addLmScore(Core::Ref<const Lm::ScaledLanguageModel> lm,
                           Score wpScale, TreeInstance::LmCache &lmCache);
    ...
    struct Equality {
      bool operator()(const WordEndHypothesis *l,
                      const WordEndHypothesis *r) const {
  ...
  typedef Core::hash_set<WordEndHypothesisList::iterator,
                         WordEndHypothesis::Hash, WordEndHypothesis::Equality>
      WordEndHypothesisRecombinationMap;
  WordEndHypothesisRecombinationMap wordEndHypothesisMap;

  TreeInstance *activateTree(Lm::History);
  void activateOrUpdateTree(const Ref<Trace> &, Lm::History, StateTree::StateId,
                            Score);
  ...
  inline void activateOrUpdateStateHypothesis(StateTree::StateId, TraceId,
                                              Score);
...
public:
  SearchSpace(const StateTree *, const LanguageModelLookahead *,
  ...
  Score minimumStateScore() const {
    return (stateHypotheses.size()) ? bestStateHypothesis()->prospect
                                    : Core::Type<Score>::max;
  }
  ...
  void
  hypothesizeEpsilonPronunciations(Core::Ref<const Lm::ScaledLanguageModel>,
                                   Score wpScale, Score threshold);
  void addStartupWordEndHypothesis(const Lm::History &, TimeframeIndex);
  void dumpWordEnds(std::ostream &,
                    Core::Ref<const Bliss::PhonemeInventory>) const;
  void startNewTrees();
  Ref<Trace> getSentenceEnd(Core::Ref<const Lm::ScaledLanguageModel>,
                            TimeframeIndex, bool shallCreateLattice);
  Ref<Trace> getSentenceEndFallBack(Core::Ref<const Lm::ScaledLanguageModel>,
                                    TimeframeIndex, bool shallCreateLattice);
...
WordConditionedTreeSearch::SearchSpace::SearchSpace(
    const StateTree *tree, const LanguageModelLookahead *lmLookahead,
    u32 nStateHistogramBins, u32 nWordEndHistogramBins)
    : tree_(tree), lmLookahead_(lmLookahead), globalScoreOffset_(0.0),
      treeDeletionLatency_(0), stateHistogram_(nStateHistogramBins),
 ...
 * create a new tree
 */
WordConditionedTreeSearch::SearchSpace::TreeInstance *
WordConditionedTreeSearch::SearchSpace::activateTree(Lm::History history) {
...
void WordConditionedTreeSearch::SearchSpace::activateOrUpdateTree(
    const Ref<Trace> &trace, Lm::History history, StateTree::StateId entry,
    Score score) {
 ...
 */
inline bool
WordConditionedTreeSearch::SearchSpace::mayDeactivateTree(TreeInstance *at) {
 ...
 */
inline void
WordConditionedTreeSearch::SearchSpace::activateOrUpdateStateHypothesis(
    StateTree::StateId state, TraceId trace, Score score) {
  ...
  if (shi >= StateHypothesisIndex(newStateHypotheses.size()) ||
    ...
    newStateHypotheses.push_back(
        StateHypothesis(state, trace, score, prospect));
  } else {
 ...
 */
inline void
WordConditionedTreeSearch::SearchSpace::expandState(const StateHypothesis &sh) {
  ...
  if (score < Core::Type<Score>::max)
    for (tie(state, state_end) = tree_->successors(sh.state); state < state_end;
         ++state)
      activateOrUpdateStateHypothesis(*state, sh.trace, score);
  ...
  if (score < Core::Type<Score>::max)
    for (tie(state, state_end) = tree_->successors2(sh.state);
...
void WordConditionedTreeSearch::SearchSpace::expandTree(TreeInstance &at) {
  ...
  for (std::vector<StateHypothesis>::const_iterator sh =
           at.entryStateHypotheses.begin();
       sh != at.entryStateHypotheses.end(); ++sh)
    expandState(*sh);
  at.entryStateHypotheses.clear();

  for (StateHypothesesList::const_iterator sh =
           stateHypotheses.begin() + at.stateHypotheses.begin;
       sh != stateHypotheses.begin() + at.stateHypotheses.end; ++sh)
    expandState(*sh);
 ...
 * the best state hypothesis (w.r.t s.prospect) is written in
 * bestStateHypothesis_. the aoustic score is added to both s.score and
 * s.prospect
 */
void WordConditionedTreeSearch::SearchSpace::addAcousticScores(
  ...
  for (StateHypothesesList::iterator sh = stateHypotheses.begin();
...
void WordConditionedTreeSearch::SearchSpace::activateLmLookahead(
    s32 lmLookaheadLaziness) {
  require(lmLookahead_);

  for (TreeInstanceList::iterator t = activeTrees.begin();
       t != activeTrees.end(); ++t) {
    TreeInstance &at(**t);
    if (!at.lookahead) {
...
/**
 * calculate s.prospect (using LM lookahead) and find best state hypothesis in
 * all active trees. result: bestStateHypothesis_
 */
void WordConditionedTreeSearch::SearchSpace::determinePruningCriterion() {
  ...
  bestStateHypothesis_ = stateHypotheses.begin();
  Score bestScore = Core::Type<Score>::max;
  for (TreeInstanceList::const_iterator t = activeTrees.begin();
       t != activeTrees.end(); ++t) {
    const TreeInstance &at(**t);
    StateHypothesesList::iterator sh_begin =
        stateHypotheses.begin() + at.stateHypotheses.begin;
    StateHypothesesList::iterator sh_end =
        stateHypotheses.begin() + at.stateHypotheses.end;
 ...
 */
Score WordConditionedTreeSearch::SearchSpace::quantileStateScore(
    Score minScore, Score maxScore, u32 nHyps) const {
  stateHistogram_.clear();
...
void WordConditionedTreeSearch::SearchSpace::pruneStates(Score threshold) {
  ...
  for (treeIn = treeOut = 0; treeIn < activeTrees.size(); ++treeIn) {
    ...
    for (treeHypEnd = hypBegin + at->stateHypotheses.end; hypIn < treeHypEnd;
         ++hypIn) {
 ...
 */
inline void
WordConditionedTreeSearch::SearchSpace::WordEndHypothesis::addLmScore(
    Core::Ref<const Lm::ScaledLanguageModel> lm, Score wpScale) {
  Lm::addLemmaPronunciationScore(lm, pronunciation, wpScale, lm->scale(),
                                 history, score.lm);
}
...
WordConditionedTreeSearch::SearchSpace::WordEndHypothesis::addLmScore(
  ...
  Core::tie(lci, needToCalculate) = lmCache.insert(std::make_pair(
      pronunciation, TreeInstance::LmCacheItem(score.lm, history)));
...
void WordConditionedTreeSearch::SearchSpace::findWordEnds(
  ...
  for (TreeInstanceList::const_iterator t = activeTrees.begin();
       t != activeTrees.end(); ++t) {
    const TreeInstance &at(**t);
    for (StateHypothesesList::iterator sh =
             stateHypotheses.begin() + at.stateHypotheses.begin;
         sh != stateHypotheses.begin() + at.stateHypotheses.end; ++sh) {
      ...
      for (tie(we, we_end) = tree_->wordEnds(sh->state); we != we_end; ++we) {
        ...
        WordEndHypothesis weh(at.history, we->transitEntry, we->pronunciation,
                              score, trace);
        if (weh.pronunciation) {
          weh.score.acoustic += (*tree_->transitionModel(
              sh->state))[Am::StateTransitionModel::exit];
 ...
 */
Score WordConditionedTreeSearch::SearchSpace::quantileWordEndScore(
    Score minScore, Score maxScore, u32 nHyps) const {
  wordEndHistogram_.clear();
...
void WordConditionedTreeSearch::SearchSpace::pruneWordEnds(Score threshold) {
  ...
  for (in = out = wordEndHypotheses.begin(); in != wordEndHypotheses.end();
       ++in)
...
void WordConditionedTreeSearch::SearchSpace::createTraces(TimeframeIndex time) {
  for (WordEndHypothesisList::iterator weh = wordEndHypotheses.begin();
       weh != wordEndHypotheses.end(); ++weh) {
    weh->trace =
        Core::ref(new Trace(weh->trace, weh->pronunciation, time, weh->score,
...
void WordConditionedTreeSearch::SearchSpace::hypothesizeEpsilonPronunciations(
  ...
  for (u32 w = 0; w < wordEndHypotheses.size(); ++w) {
    ...
    for (tie(we, we_end) = tree_->wordEnds(state); we != we_end; ++we) {
      ...
      weh.score.acoustic +=
          (*tree_->transitionModel(state))[Am::StateTransitionModel::exit];
      if (weh.score <= threshold) {
        weh.trace = Core::ref(
            new Trace(weh.trace, weh.pronunciation, weh.trace->time, weh.score,
                      tree_->describeRootState(weh.transitEntry)));
        weh.trace->score.acoustic += globalScoreOffset_;
        wordEndHypotheses.push_back(weh);
      }
...
void WordConditionedTreeSearch::SearchSpace::recombineWordEnds(
  ...
  for (in = out = wordEndHypotheses.begin(); in != wordEndHypotheses.end();
       ++in) {
    WordEndHypothesisRecombinationMap::iterator i =
        wordEndHypothesisMap.find(in);
...
void WordConditionedTreeSearch::SearchSpace::startNewTrees() {
  for (WordEndHypothesisList::const_iterator weh = wordEndHypotheses.begin();
       weh != wordEndHypotheses.end(); ++weh) {
    activateOrUpdateTree(weh->trace, weh->history, weh->transitEntry,
                         weh->score);
  }
}
...
WordConditionedTreeSearch::SearchSpace::getSentenceEnd(
    Core::Ref<const Lm::ScaledLanguageModel> lm, TimeframeIndex time,
    bool shallCreateLattice) {
  Ref<Trace> best;
  for (WordEndHypothesisList::const_iterator weh = wordEndHypotheses.begin();
       weh != wordEndHypotheses.end(); ++weh) {
    if (weh->transitEntry != tree_->root() &&
        weh->transitEntry != tree_->ciRoot())
...
WordConditionedTreeSearch::SearchSpace::getSentenceEndFallBack(
    ...
    bool shallCreateLattice) {
  Ref<Trace> best;
  for (TreeInstanceList::const_iterator iter = activeTrees.begin();
       iter != activeTrees.end(); ++iter) {
    const TreeInstance *at(*iter);
    if (!at->nStateHypotheses())
      continue;
...
WordConditionedTreeSearch::SearchSpace::getLastUnambiguousTrace() const {
  ...
  for (TreeInstanceList::const_iterator at = activeTrees.begin();
       at != activeTrees.end(); ++at)
    for (std::vector<Ref<Trace> >::const_iterator t = (*at)->traces.begin();
         t != (*at)->traces.end(); ++t)
      traces.push_back(t->get());

  Core::hash_map<const Trace *, u32, Core::conversion<const Trace *, size_t> >
      m;
...
struct WordConditionedTreeSearch::SearchSpaceStatistics {
  Core::Statistics<u32> treesBeforePruning, treesAfterPrePruning,
      treesAfterPruning, statesBeforePruning, statesAfterPrePruning,
      statesAfterPruning, wordEndsBeforePruning, wordEndsAfterPruning,
      epsilonWordEndsAdded, wordEndsAfterRecombination,
...
void WordConditionedTreeSearch::SearchSpaceStatistics::write(
    Core::XmlWriter &os) const {
  os << Core::XmlOpen("search-space-statistics") << treesBeforePruning
     << treesAfterPrePruning << treesAfterPruning << statesBeforePruning
     << statesAfterPrePruning << statesAfterPruning << wordEndsBeforePruning
     << wordEndsAfterPruning << epsilonWordEndsAdded
     << wordEndsAfterRecombination << wordEndsAfterSecondPruning
     << acousticHistogramPruningThreshold << lmHistogramPruningThreshold
...
const Core::ParameterFloat
    WordConditionedTreeSearch::paramAcousticPrePruningThreshold(
        "acoustic-prepruning",
        "threshold for pruning of state hypotheses before emission scores",
        Core::Type<Score>::max, 0.0);
const Core::ParameterFloat
    WordConditionedTreeSearch::paramAcousticPruningThreshold(
        "acoustic-pruning", "threshold for pruning of state hypotheses", 1000.0,
        0.0);
...
const Core::ParameterFloat WordConditionedTreeSearch::paramLmPruningThreshold(
    "lm-pruning", "threshold for pruning of word end hypotheses", 1000.0, 0.0);
...
const Core::ParameterInt WordConditionedTreeSearch::paramLmPruningBins(
    "lm-pruning-bins", "number of bins for histogram pruning of word ends", 100,
    2);
const Core::ParameterInt WordConditionedTreeSearch::paramTreeDeletionLatency(
    "tree-deletion-latency",
    "number of time-frames a tree without state hypotheses stays around", 0, 0);
const Core::ParameterBool WordConditionedTreeSearch::paramEnableLmLookahead(
    "lm-lookahead", "enable language model look-ahead", false);
const Core::ParameterInt WordConditionedTreeSearch::paramLmLookaheadLaziness(
    "lm-lookahead-laziness",
    "number of states allowed in a tree before calculation of language model "
    "look-ahead is enforced",
    0, 0);
...
const Core::ParameterBool WordConditionedTreeSearch::paramSentenceEndFallBack(
    ...
    true);
const Core::ParameterFloat
    WordConditionedTreeSearch::paramLatticePruningThreshold(
        "lattice-pruning",
        "threshold for pruning of hypotheses stored in word lattice", 1000.0,
        0.0);
...
const Core::ParameterBool
    WordConditionedTreeSearch::paramAnticipatedLmLookahedPruning(
        "anticipated-lm-lookahead-pruning",
        "prune hypotheses using the lm lookahead score before acoustic score "
        "calculation",
        false);
const Core::Choice WordConditionedTreeSearch::latticeOptimizationMethodChoice(
    ...
    "yes", simpleSilenceLatticeOptimization, // for backward compatibility -
                                             // remove asap
    "none", noLatticeOptimization, "simple", simpleSilenceLatticeOptimization,
    Core::Choice::endMark());
const Core::ParameterChoice WordConditionedTreeSearch::paramOptimizeLattice(
    "optimize-lattice", &latticeOptimizationMethodChoice,
    "optimization method for word lattice generation (default is 'simple "
    "silence approximation')",
...
WordConditionedTreeSearch::WordConditionedTreeSearch(
  ...
  latticePruningThreshold_ =
      paramLatticePruningThreshold(config, lmPruningThreshold_);
  ...
  if (shallCreateLattice_) {
    shallOptimizeLattice_ =
        LatticeOptimizationMethod(paramOptimizeLattice(config));
  } else {
    if (latticePruningThreshold_ != lmPruningThreshold_) {
...
bool WordConditionedTreeSearch::setModelCombination(
  ...
  if (paramEnableLmLookahead(config)) {
    lmLookahead_ = new LanguageModelLookahead(select("lm-lookahead"), wpScale_,
                                              lm_, tree_);
    lmLookaheadLaziness_ = paramLmLookaheadLaziness(config);
    if (anticipatedLookaheadPruning_ && lmLookaheadLaziness_)
      error("anticipated lm lookahead pruning cannot be used with lookahead "
            "laziness > 0");
  } else {
    if (anticipatedLookaheadPruning_)
...
#ifdef MODULE_LM_FSA
  require(lm_);
  const Lm::FsaLm *constFsaLm =
      dynamic_cast<const Lm::FsaLm *>(lm_->unscaled().get());
  require(constFsaLm);
...
void WordConditionedTreeSearch::restart() {
  if (!ss_) {
    ss_ = new SearchSpace(tree_, lmLookahead_, acousticPruningBins_,
                          lmPruningBins_);
...
void WordConditionedTreeSearch::feed(
  ...
  if (ss_->nStateHypotheses() &&
      acousticPrePruningThreshold_ < Core::Type<Score>::max) {
    SearchSpace::StateHypothesesList::const_iterator sh =
        ss_->bestStateHypothesis();
    Mm::MixtureIndex mix = tree_->stateDesc(sh->state).acousticModel;
    verify(mix != StateTree::invalidAcousticModel);
    Score acuThreshold = sh->prospect + emissionScores->score(mix) +
  ...
  ss_->pruneStates(acuThreshold);
  if ((ss_->nStateHypotheses() > acousticPruningLimit_) &&
      (minStateScore < acuThreshold)) {
    acuThreshold = ss_->quantileStateScore(minStateScore, acuThreshold,
                                           acousticPruningLimit_);
    searchSpaceStatistics_->acousticHistogramPruningThreshold +=
        acuThreshold - minStateScore;
    ss_->pruneStates(acuThreshold);
  }
  ...
  if ((ss_->nWordEndHypotheses() > latticePruningLimit_) &&
      (minWordEndScore < lmThreshold)) {
    lmThreshold = ss_->quantileWordEndScore(minWordEndScore, lmThreshold,
                                            latticePruningLimit_);
    searchSpaceStatistics_->lmHistogramPruningThreshold +=
        lmThreshold - minWordEndScore;
    ss_->pruneWordEnds(lmThreshold);
  }
  ...
  searchSpaceStatistics_->epsilonWordEndsAdded +=
      ss_->nWordEndHypotheses() - nWordEnds;
  ...
  searchSpaceStatistics_->wordEndsAfterRecombination +=
      ss_->nWordEndHypotheses();
  ...
  if ((ss_->nWordEndHypotheses() > lmPruningLimit_) &&
      (minWordEndScore < lmThreshold)) {
    lmThreshold = ss_->quantileWordEndScore(minWordEndScore, lmThreshold,
                                            lmPruningLimit_);
    searchSpaceStatistics_->lmHistogramPruningThreshold +=
  ...
  searchSpaceStatistics_->wordEndsAfterSecondPruning +=
      ss_->nWordEndHypotheses();
...
Ref<WordConditionedTreeSearch::Trace>
WordConditionedTreeSearch::sentenceEnd() const {
  if (!sentenceEnd_) {
    ...
    if (!sentenceEnd_) {
      ...
      if (allowSentenceEndFallBack_) {
        sentenceEnd_ =
            ss_->getSentenceEndFallBack(lm_, time_ + 1, shallCreateLattice_);
      }
    }
    if (sentenceEnd_) {
      switch (shallOptimizeLattice_) {
...
void WordConditionedTreeSearch::getCurrentBestSentence(
    Traceback &result) const {
  Ref<Trace> t = sentenceEnd();
  if (!t) {
    error(
        "Cannot determine sentence hypothesis: No active word end hypothesis.");
    result.clear();
    return;
  }
  traceback(t, result);
...
Lattice::ConstWordLatticeRef
WordConditionedTreeSearch::getCurrentWordLattice() const {
  ...
  Core::Ref<Lattice::StandardWordLattice> result(
      new Lattice::StandardWordLattice(lexicon_));
  Core::Ref<Lattice::WordBoundaries> wordBoundaries(
      new Lattice::WordBoundaries);
  Ref<Trace> initialTrace;

  Core::hash_map<const Trace *, Fsa::State *,
                 Core::conversion<const Trace *, size_t> >
      state;
  ...
  while (!stack.empty()) {
    ...
    stack.pop();
    tn = state[tt.get()];
    wordBoundaries->set(tn->id(), Lattice::WordBoundary(tt->time, tt->transit));
