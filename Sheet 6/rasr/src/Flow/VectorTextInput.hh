...
template <class T> class VectorTextInputNode : public VectorTextInputNodeBase {
private:
  size_t givenSize_;
...
protected:
  ...
  virtual void setElements(const std::string &buffer) {
    ...
    if (givenSize_ > 0 && givenSize_ != out->size())
      error("Vector dimension mismatch while processing \"%s\": %zd given and "
            "%zd read.",
            buffer.c_str(), givenSize_, out->size());
  }
...
public:
  ...
  VectorTextInputNode(const Core::Configuration &c)
      : Core::Component(c), VectorTextInputNodeBase(c, Vector<T>::type()),
        givenSize_(0) {}
