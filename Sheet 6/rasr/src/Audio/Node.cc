...
// ===========================================================================
const Node::SampleCount Node::invalidSampleCount = Core::Type<SampleCount>::max;

const Core::ParameterString Node::paramFilename("file", "name of audio file");

Node::Node(const Core::Configuration &c)
    : Core::Component(c), Flow::Node(c), sampleRate_(0), sampleSize_(0),
      trackCount_(0), sampleCount_(0) {
...
const Core::ParameterFloat SourceNode::paramStartTime(
    "start-time", "time in seconds to skip at beginning of file", 0.0, 0.0);
const Core::ParameterFloat
    SourceNode::paramEndTime("end-time",
                             "time in seconds to stop extraction at",
...
bool SourceNode::setParameter(const std::string &name,
...
void SourceNode::setTotalSampleCount(SampleCount c) { totalSampleCount_ = c; }
...
bool SourceNode::work(Flow::PortId out) {
  ...
  u32 samplesToRead = (sampleCount_ + blockSize_ <= endSample)
                          ? blockSize_
  ...
  ensure(v->startTime() >= startTime_ ||
         Core::isAlmostEqual(v->startTime(), startTime_));
  ensure(v->endTime() <= endTime_ ||
         Core::isAlmostEqual(v->endTime(), endTime_));
  return putData(0, v);
}
