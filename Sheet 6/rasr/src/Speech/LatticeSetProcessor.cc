...
LatticeSetProcessor::LatticeSetProcessor(const Core::Configuration &c)
    : Core::Component(c), initialized_(false) {}
...
LatticeSetProcessorRoot::LatticeSetProcessorRoot(const Core::Configuration &c)
    : Core::Component(c), CorpusProcessor(c), Precursor(c) {}
...
void InfoLatticeProcessorNode::processWordLattice(
  ...
  for (size_t i = 0; i < lattice->nParts(); ++i) {
    ...
    if (fsa) {
      Component::Message logChannel =
          log("part \"%s\"", lattice->name(i).c_str());
...
const Core::ParameterBool
    DensityLatticeProcessorNode::paramShallEvaluateArcsPerSpokenWord(
        "shall-evaluate-arcs-per-spoken-word",
        "Fsa::info to calculate lattice density, i.e. number of arcs per "
        "spoken word",
        true);

const Core::ParameterBool
    DensityLatticeProcessorNode::paramShallEvaluateArcsPerTimeframe(
        "shall-evaluate-arcs-per-timeframe",
        "evaluate number of arcs and number of timeframes", true);

DensityLatticeProcessorNode::DensityLatticeProcessorNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c),
      shallEvaluateArcsPerSpokenWord_(
          paramShallEvaluateArcsPerSpokenWord(config)),
      shallEvaluateArcsPerTimeframe_(
          paramShallEvaluateArcsPerTimeframe(config)) {}
...
void DensityLatticeProcessorNode::processWordLattice(
  ...
  if (shallEvaluateArcsPerTimeframe_) {
    ...
    std::pair<u32, u32> info = Lattice::densityInfo(lattice);
    log("lattice-info (n-time-frames, n-arcs): %d, %d", info.first,
        info.second);
  }
  Precursor::processWordLattice(lattice, s);
}
...
GerLatticeProcessorNode::GerLatticeProcessorNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), evaluator_(0) {}

GerLatticeProcessorNode::~GerLatticeProcessorNode() { delete evaluator_; }
...
const Core::ParameterStringVector
    LinearCombinationLatticeProcessorNode::paramOutputs(
        "outputs", "output names/selections for linear combinations");

Core::ParameterFloatVector LinearCombinationLatticeProcessorNode::paramScales(
    "scales", "build linear combination");

LinearCombinationLatticeProcessorNode::LinearCombinationLatticeProcessorNode(
    const Core::Configuration &c)
    : Core::Component(c), Precursor(c) {
...
bool LinearCombinationLatticeProcessorNode::checkNParts(u32 nParts) const {
  ...
  for (; it != scales_.end(); ++it) {
    if (it->second.size() != nParts) {
      success = false;
      error("mismatch in number of lattice parts and number of scales for "
            "output \"")
          << it->first << "\"";
    }
  }
...
void LinearCombinationLatticeProcessorNode::processWordLattice(
  ...
  Precursor::processWordLattice(Lattice::linearCombination(lattice, scales_),
                                s);
}
...
void CacheNode::processWordLattice(Lattice::ConstWordLatticeRef lattice,
...
void CopyNode::processWordLattice(Lattice::ConstWordLatticeRef lattice,
                                  Bliss::SpeechSegment *segment) {
  if (lattice) {
...
const Core::ParameterInt
    PartialNode::paramInitial("initial", "initial state id of partial lattice",
                              Fsa::InvalidStateId);

PartialNode::PartialNode(const Core::Configuration &c)
    : Core::Component(c), Precursor(c), initial_(paramInitial(config)) {}

PartialNode::~PartialNode() {}

void PartialNode::processWordLattice(Lattice::ConstWordLatticeRef lattice,
                                     Bliss::SpeechSegment *segment) {
  if (lattice) {
