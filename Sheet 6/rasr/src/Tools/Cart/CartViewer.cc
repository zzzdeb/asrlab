...
protected:
  ...
  bool readValue(u32 row, u32 col, Cart::PropertyMap::Index keyIndex,
                 std::string &value);
...
public:
  CartViewer()
      : Core::Application(), tree_(0), mode_(0), keys_(), maxKeySize_(0) {
    setTitle("cart-viewer");
  }
  ~CartViewer() { delete tree_; }

  virtual std::string getUsage() const {
    return "Generic CART viewer\n\nSpecify tree either as (first) command "
...
const Core::ParameterBool
    paramInteractive("interactive",
                     "activate interactive mode, i.e. browse the cart", false);
...
int CartViewer::main(const std::vector<std::string> &argv) {
  {
    std::string filename =
        (!argv.empty())
            ? argv[0]
            : DecisionTree::paramCartFilename(select("decision-tree"));
    ...
    Core::Channel plainChannel(select("decision-tree"), "plain",
                               Core::Channel::disabled);
    if (plainChannel.isOpen())
      tree_->write(plainChannel);
    Core::XmlChannel xmlChannel(select("decision-tree"), "xml",
                                Core::Channel::disabled);
    if (xmlChannel.isOpen())
      tree_->writeXml(xmlChannel);
    Core::Channel dotChannel(select("decision-tree"), "dot",
                             Core::Channel::disabled);
...
void CartViewer::displayMask(u32 row, u32 col) {
  for (StringList::iterator it = keys_.begin(); it != keys_.end();
       ++it, ++row) {
...
bool CartViewer::readValue(u32 row, u32 col, PropertyMap::Index keyIndex,
                           std::string &value) {
  value.clear();
  ...
  if (tree_->map().isDefined(choice[value]))
    return true;
  for (Core::Choice::const_iterator it = choice.begin(); it != choice.end();
       ++it) {
...
void CartViewer::displayPath(DecisionTree::Path &path) {
  ...
  cout << endl << "class id is " << path.leaf->id() << endl << endl;
}
...
void CartViewer::displayMultiple(DecisionTree::NodePtrList &list) {
  ...
  case 1:
    ...
    break;
  case 2:
    cout << "class ids are " << list.front()->id() << " and "
         << list.back()->id() << endl;
    break;
  default:
...
bool CartViewer::readCommand() {
  std::string cmd;
  for (;;) {
    cout << "Press enter or choose [q]uit, [c]lassification, [m]ultification"
         << endl
         << ">  " << flush;
    getline(cin, cmd);
    cout << '\b';
...
void CartViewer::run() {
  ...
  u32 headerX = 1 + HEADER_X_OFFSET;
  u32 headerY = 1 + HEADER_Y_OFFSET;
  u32 bodyX = 1 + BODY_X_OFFSET;
  u32 bodyY = headerY + 1 + BODY_Y_OFFSET;
  u32 footerX = 1 + FOOTER_X_OFFSET;
  u32 footerY = bodyY + u32(numberOfKeys) + FOOTER_Y_OFFSET;
  u32 inputY;
  u32 inputX = bodyX + u32(maxKeySize_) + 3;
  ...
  do {
    ...
    for (PropertyMap::Index i = 0; i < numberOfKeys; ++i, ++inputY, ++valueIt) {
      ...
      while (!readValue(inputY, inputX, i, *valueIt)) {
        displayError(headerY, headerX,
                     "Could not find matching value, please try again");
