...
public:
  AffineFeatureTransformAccumulator();
  AffineFeatureTransformAccumulator(size_t featureDimension,
                                    size_t modelDimension_,
                                    const std::string &key);

  void accumulate(Core::Ref<const Feature::Vector>, DensityIndex,
                  Core::Ref<MixtureSet>);
  void accumulate(Core::Ref<const Feature::Vector>, DensityIndex,
                  Core::Ref<MixtureSet>, Mm::Weight);
  void accumulate(Core::Ref<const Feature::Vector> f, DensityIndex di,
                  MixtureIndex mi, Core::Ref<MixtureSet> m) {
    accumulate(f, di, m);
  }
  void accumulate(Core::Ref<const Feature::Vector> f, DensityIndex di,
                  MixtureIndex mi, Core::Ref<MixtureSet> m, Mm::Weight w) {
    accumulate(f, di, m, w);
  }
  void finalize();
  void compact();
