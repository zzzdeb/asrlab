...
public:
  Tester(const Configuration &c) : Component(c), ch(config, "results") {
...
const Choice Tester::flavourChoice("vanilla", vanilla, "strawberry", strawberry,
                                   "chocolate", chocolate,
                                   Core::Choice::endMark());

const ParameterBool Tester::paramBoolean("bool", "a boolean parameter");
const ParameterInt
    Tester::paramInteter("int", "an integer parameter between -10 and 10", 0,
                         -10, 10);
const ParameterFloat Tester::paramFloat("float", "a real valued parameter");
const ParameterString Tester::paramString("string", "a string parameter",
                                          "DEFAULT");
const ParameterChoice Tester::paramFlavour("flavour", &flavourChoice,
                                           "a multiple choice paramter",
                                           Choice::IllegalValue);
...
public:
  ...
  int main(const std::vector<std::string> &arguments) {
    for (std::vector<std::string>::const_iterator itArg = arguments.begin();
         itArg != arguments.end(); ++itArg) {
      StringTokenizer tokenizer(*itArg, ".");
      std::vector<std::string> components = tokenizer();
      Configuration _config(config);
      for (std::vector<std::string>::const_iterator itComponent =
               components.begin();
           itComponent != components.end() - 1; ++itComponent)
    ...
    /*
            Core::XmlOutputStream xw(new
       Core::CompressedOutputStream("/tmp/xm.gz")); xw << Core::XmlOpen("fsm")
               << "HELLO WORLD"
               << Core::XmlOpenComment() << "AAA --- BBB -- CCC ---- DDD" <<
