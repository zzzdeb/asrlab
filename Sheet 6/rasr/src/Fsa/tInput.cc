...
typename _Automaton::ConstRef read(const Resources<_Automaton> &resources,
                                   const std::string &format, std::istream &i) {
  StorageAutomaton<_Automaton> *f = new StaticAutomaton<_Automaton>();
  verify(!f->getInputAlphabet() &&
         ((f->type() == Fsa::TypeAcceptor) || !f->getOutputAlphabet()));
...
template <class _Automaton>
bool read(const Resources<_Automaton> &resources,
          StorageAutomaton<_Automaton> *f, const std::string &format,
          std::istream &i) {
  ...
  if (!formatter->reader) {
    std::cerr << "format \"" << formatter->name << "\" does not support reading"
              << std::endl;
    return false;
  }
...
namespace {
template <class _Automaton>
bool readSingleFile(const Resources<_Automaton> &resources,
                    StorageAutomaton<_Automaton> *f, const std::string &format,
                    const std::string &file) {
  if (format == "combine") {
...
template <class _Automaton>
bool read(const Resources<_Automaton> &resources,
          StorageAutomaton<_Automaton> *f, const std::string &file) {
...
template <class _Automaton>
void assertSemiring(const Resources<_Automaton> &resources,
                    StorageAutomaton<_Automaton> *f) {
  if (!(f->semiring()))
...
template <class _Automaton>
bool readAtt(const Resources<_Automaton> &resources,
             StorageAutomaton<_Automaton> *f, std::istream &i) {
  ...
  while (i) {
    ...
    int n =
        sscanf(line.c_str(), "%s %s %s %s %s", tmp1, tmp2, tmp3, tmp4, tmp5);
    char *error;
    ...
    case 3: // acceptor arc
      ...
      if (*error != '\0') {
        ...
        if (strcmp(tmp3, "eps") == 0)
          ...
          a->setInput(hasAlphabets ? input->index(tmp3)
                                   : sinput->addSymbol(tmp3));
        if (!hasAlphabets && !soutput)
          soutput = new Fsa::StaticAlphabet();
        if (strcmp(tmp4, "eps") == 0)
          a->setOutput(Fsa::Epsilon);
        else
          a->setOutput(hasAlphabets ? output->index(tmp3)
                                    : soutput->addSymbol(tmp3));
      }
      break;
      ...
      if (*error != '\0') {
        ...
        if (*error != '\0') {
          ...
          if (strcmp(tmp4, "eps") == 0)
            ...
            a->setOutput(hasAlphabets ? output->index(tmp4)
                                      : soutput->addSymbol(tmp4));
        }
      } else
        f->setType(Fsa::TypeTransducer);
      break;
  ...
  for (typename Core::Vector<_State *>::iterator i = states.begin();
       i != states.end(); ++i)
    f->setState(*i);
  return true;
}
...
template <class _Automaton>
bool readBinary(const Resources<_Automaton> &resources,
                StorageAutomaton<_Automaton> *f, std::istream &i) {
  typedef typename _Automaton::State _State;
  typedef typename _Automaton::Arc _Arc;
  ...
  if (formatVersion == 0) {
    tmp = what;
    what =
        Fsa::storeStates | Fsa::storeInputAlphabet | Fsa::storeOutputAlphabet;
  ...
  if (what & Fsa::storeStates) {
    ...
    if (!(bi >> tmp))
      return false;
    _ConstSemiringRef semiring = resources.getSemiring(Fsa::SemiringType(tmp));
    if (semiring)
  ...
  if (what & Fsa::storeStates) {
    ...
    if (!(bi >> idAndTags))
      return false;
    while (bi) {
      _State *sp = f->createState(idAndTags & Fsa::StateIdMask,
                                  idAndTags & Fsa::StateTagMask);
...
template <class _Automaton>
bool readLinear(const Resources<_Automaton> &resources,
                StorageAutomaton<_Automaton> *f, std::istream &i) {
  assertSemiring<_Automaton>(resources, f);
  f->clear();
  if (!i)
...
template <class _Automaton>
bool readXml(const Resources<_Automaton> &resources,
             StorageAutomaton<_Automaton> *f, std::istream &i) {
  assertSemiring<_Automaton>(resources, f);
  if (!i)
    return false;
