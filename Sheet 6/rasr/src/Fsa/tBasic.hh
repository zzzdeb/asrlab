 ...
 **/
template <class _Automaton>
typename _Automaton::ConstRef hypothesis(typename _Automaton::ConstRef f,
                                         Fsa::StateId n);
...
changeSemiringAndSet(Core::Ref<const _AutomatonFrom> f,
                     Core::Ref<const _Semiring> semiring,
                     const _Mapping &map = _Mapping()) {
  return convert<_AutomatonFrom,
                 Automaton<State<Arc<typename _Semiring::Weight> > >, _Mapping>(
      f, semiring, map);
}
} // namespace Ftl
