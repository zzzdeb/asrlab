 ...
 *    -Tying of allophone states concerning the emission probabilities.
 * Interface: emissionIndex(...) -Transition probalility set (only preliminary
 * implementaion) -Tying of transition probalility concerning transitions. (Not
 * implemented yet.) -HMM topology. Preliminary interface: hmmTopology(...)
 ...
 *    -Deliver the silence identifier. (Not sure if necessary.) Interface
 * silence(). -Delivers if across-word model is enabled. (Not sure if
 * necessary.) Interface isAcrossWordModelEnabled().
 */
class AcousticModel : public Mc::Component, public Core::ReferenceCounted {
private:
...
public:
  ...
  virtual Core::Ref<const AllophoneStateAlphabet>
  allophoneStateAlphabet() const = 0;
  ...
  virtual const StateTransitionModel *
      stateTransition(StateTransitionIndex) const = 0;
  virtual StateTransitionIndex stateTransitionIndex(AllophoneState,
                                                    s8 subState = 0) const = 0;
  virtual StateTransitionIndex stateTransitionIndex(AllophoneStateIndex,
                                                    s8 subState = 0) const = 0;

  virtual Core::Ref<const ClassicHmmTopologySet> hmmTopologySet() const = 0;
  virtual const ClassicHmmTopology *
  hmmTopology(Bliss::Phoneme::Id phoneme) const = 0;
...
public:
  ...
  virtual Fsa::ConstAutomatonRef
      applyTransitionModel(Fsa::ConstAutomatonRef) = 0;

  virtual Fsa::ConstAutomatonRef
  createEmissionLoopTransducer(bool transitionModel) = 0;
  virtual Fsa::ConstAutomatonRef createMinimizedContextDependencyTransducer(
      Fsa::LabelId initialPhoneAndSilenceOffset) = 0;
};
