...
GaussDensity *GaussDensityEstimator::estimate(
  ...
  verify(meanEstimator_ && covarianceEstimator_);
  return new GaussDensity(meanMap[meanEstimator_],
                          covarianceMap[covarianceEstimator_]);
}

void GaussDensityEstimator::read(
...
void GaussDensityEstimator::write(
...
void GaussDensityEstimator::write(
    Core::XmlWriter &o, const ReferenceIndexMap<AbstractMeanEstimator> &meanMap,
    const ReferenceIndexMap<AbstractCovarianceEstimator> &covarianceMap) const {
  o << Core::XmlEmpty("density-estimator") +
           Core::XmlAttribute("mean-estimator-index", meanMap[meanEstimator_]) +
           Core::XmlAttribute("covariance-estimator-index",
                              covarianceMap[covarianceEstimator_]);
}
...
bool GaussDensityEstimator::equalTopology(
    ...
    const ReferenceIndexMap<AbstractCovarianceEstimator>
        &covarianceMapToCompare) const {
  return (meanMap[meanEstimator_] ==
              meanMapToCompare[toCompare.meanEstimator_] &&
          covarianceMap[covarianceEstimator_] ==
              covarianceMapToCompare[toCompare.covarianceEstimator_]);
}

bool GaussDensityEstimator::accumulate(Core::BinaryInputStreams &is,
...
void MeanEstimator::reset() { accumulator_.reset(); }
...
void MeanEstimator::write(Core::XmlWriter &o) const { accumulator_.write(o); }

bool MeanEstimator::operator==(
...
Mean *MeanEstimator::estimate() {
  ...
  std::transform(accumulator_.sum().begin(), accumulator_.sum().end(),
                 result->begin(),
                 std::bind2nd(std::divides<Accumulator::SumType>(),
                              accumulator_.weight()));
  return result;
}
...
void CovarianceEstimator::reset() { accumulator_.reset(); }
...
void CovarianceEstimator::accumulate(
    const AbstractEstimationAccumulator &other) {
  const CovarianceEstimator *pOther =
      dynamic_cast<const CovarianceEstimator *>(&other);
  verify(pOther != 0);
  accumulator_.accumulate(pOther->accumulator_);
}
...
bool CovarianceEstimator::operator==(
    const AbstractEstimationAccumulator &other) const {
  const CovarianceEstimator *pOther;
...
Covariance *
CovarianceEstimator::estimate(const CovarianceToMeanSetMap &meanSetMap,
  ...
  const CovarianceToMeanSetMap::MeanSet &meanSet =
      meanSetMap[Core::Ref<AbstractCovarianceEstimator>(
          (CovarianceEstimator *)this)];
  WeighedMeanSquareSum weighedMeanSquareSum(accumulator_.size());

  for (CovarianceToMeanSetMap::MeanSet::const_iterator meanEstimator =
           meanSet.begin();
       meanEstimator != meanSet.end(); ++meanEstimator) {
    const MeanEstimator *pMeanEstimator =
        dynamic_cast<const MeanEstimator *>(meanEstimator->get());
    verify(pMeanEstimator);

    if (pMeanEstimator->weight() > 0)
      weighedMeanSquareSum.accumulate(pMeanEstimator->sum(),
                                      pMeanEstimator->weight());
  }
  verify(accumulator_.weight() > 0);
#if 1
  verify(Core::isAlmostEqualUlp(accumulator_.weight(),
                                weighedMeanSquareSum.weight(), s64(1e12)));
#endif
  DiagonalCovariance *result =
      new DiagonalCovariance(accumulator_.sum().size());

  std::transform(accumulator_.sum().begin(), accumulator_.sum().end(),
                 weighedMeanSquareSum.sum().begin(), result->buffer_.begin(),
                 normalizedMinus<Sum>(accumulator_.weight()));

  applyMinimumVariance(minimumVariance, *result);
  return result;
}

void CovarianceEstimator::applyMinimumVariance(
    VarianceType minimumVariance, DiagonalCovariance &result) const {
...
CovarianceToMeanSetMap::CovarianceToMeanSetMap(
  ...
  std::vector<Core::Ref<GaussDensityEstimator> >::const_iterator i =
      densityEstimators.begin();
