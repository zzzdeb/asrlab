 ...
 *  @see Mc::Component.
 */
class ScaledLanguageModel : public LanguageModel, public Mc::Component {
protected:
 ...
 *  Wrapper class which multiplies the scores of an arbitrary language model
 * object by predefined scale.
 */
...
public:
  ...
  virtual CompiledBatchRequest *compileBatchRequest(const BatchRequest &r,
                                                    Score scale = 1.0) const {
    return languageModel_->compileBatchRequest(r,
                                               scale * Mc::Component::scale());
  }
  virtual void getBatch(const History &h, const CompiledBatchRequest *r,
...
inline void addSyntacticTokenScore(Core::Ref<const LanguageModel> lm,
                                   Score lmScale,
...
inline void addSyntacticTokenScore(Core::Ref<const ScaledLanguageModel> lm,
                                   const Bliss::SyntacticToken *st,
                                   Score syntaxEmissionScale,
                                   const History &history, Score &score) {
  addSyntacticTokenScore(lm->unscaled(), lm->scale(), st, syntaxEmissionScale,
                         history, score);
}

/** Language model score convenience function for lemmas. */
inline void addLemmaScore(Core::Ref<const LanguageModel> lm, Score lmScale,
                          const Bliss::Lemma *lemma, Score syntaxEmissionScale,
  ...
  for (u32 ti = 0; ti < tokenSequence.length(); ++ti) {
    ...
    addSyntacticTokenScore(lm, lmScale, st, syntaxEmissionScale, history,
                           score);
...
/** Language model score convenience function for lemmas. */
inline void addLemmaScoreOmitExtension(Core::Ref<const LanguageModel> lm,
                                       Score lmScale, const Bliss::Lemma *lemma,
                                       Score syntaxEmissionScale,
                                       History &history, Score &score) {
  require(lemma);
  const Bliss::SyntacticTokenSequence tokenSequence(
...
inline void addLemmaScore(Core::Ref<const ScaledLanguageModel> lm,
                          const Bliss::Lemma *lemma, Score syntaxEmissionScale,
                          History &history, Score &score) {
  addLemmaScore(lm->unscaled(), lm->scale(), lemma, syntaxEmissionScale,
                history, score);
}

inline void addLemmaScoreOmitExtension(Core::Ref<const ScaledLanguageModel> lm,
                                       const Bliss::Lemma *lemma,
                                       Score syntaxEmissionScale,
                                       History &history, Score &score) {
  addLemmaScoreOmitExtension(lm->unscaled(), lm->scale(), lemma,
                             syntaxEmissionScale, history, score);
}

/** Language model score convenience function for lemma-pronunciations. */
inline void
addLemmaPronunciationScore(Core::Ref<const LanguageModel> lm, Score lmScale,
                           const Bliss::LemmaPronunciation *pronunciation,
                           Score wpScale, Score syntaxEmissionScale,
                           History &history, Score &score) {
  require(pronunciation);
  addLemmaScore(lm, lmScale, pronunciation->lemma(), syntaxEmissionScale,
                history, score);
...
inline void addLemmaPronunciationScoreOmitExtension(
    ...
    const Bliss::LemmaPronunciation *pronunciation, Score wpScale,
    Score syntaxEmissionScale, History &history, Score &score) {
  require(pronunciation);
  addLemmaScoreOmitExtension(lm, pronunciation->lemma(), syntaxEmissionScale,
                             history, score);
  score += wpScale * pronunciation->pronunciationScore();
}
...
/** Language model score convenience function for lemma-pronunciations. */
inline void
addLemmaPronunciationScore(Core::Ref<const ScaledLanguageModel> lm,
                           const Bliss::LemmaPronunciation *pronunciation,
                           Score wpScale, Score syntaxEmissionScale,
                           History &history, Score &score) {
  require(pronunciation);
  addLemmaScore(lm, pronunciation->lemma(), syntaxEmissionScale, history,
                score);
  score += wpScale * pronunciation->pronunciationScore();
}
