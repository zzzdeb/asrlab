...
template <class T, class P = VectorRangeCheckOffPolicy> class Matrix {
public:
  typedef T Type;
...
public:
  ...
  template <class S> Matrix<T> &operator=(const Matrix<S> &x) {
   ...
   */
  std::pair<size_t, size_t> isAlmostEqual(const Matrix<T, P> &m,
                                          T tolerance) const;
...
template <class T, class P>
std::pair<size_t, size_t> Matrix<T, P>::isAlmostEqual(const Matrix<T, P> &m,
                                                      T tolerance) const {
...
template <class T, class P> void Matrix<T, P>::addRow(const Vector<T, P> &v) {
...
template <class T, class P> void Matrix<T, P>::removeRow(size_t position) {
  require(nRows() != 0);
...
template <class T, class P> void Matrix<T, P>::removeColumn(size_t position) {
  require(nColumns() != 0);
  for (size_t row = 0; row < nRows(); ++row)
...
template <class T, class P> const Vector<T, P> Matrix<T, P>::diagonal() const {
...
template <class T, class P> void Matrix<T, P>::fill(T x) {
...
template <class T, class P> Matrix<T, P> Matrix<T, P>::transpose() const {
...
template <class T, class P> T Matrix<T, P>::trace() const {
  T trace = (T)0.0f;
...
template <class T, class P> void Matrix<T, P>::normalizeColumns(f64 norm) {
  for (size_t column = 0; column < nColumns(); column++) {
...
template <class T, class P> void Matrix<T, P>::normalizeColumnDirection() {
  for (size_t column = 0; column < nColumns(); column++) {
...
template <class T, class P> T Matrix<T, P>::vvt(const Vector<T, P> &vec) {
  T result = (T)0.0f;
  require(nRows_ == nColumns_);
...
template <class T, class P> T Matrix<T, P>::l2Norm() const {
  T norm = (T)0.0f;
  for (size_t row = 0; row < nRows(); ++row)
    for (size_t col = 0; col < nColumns(); ++col)
...
template <class T, class P> T Matrix<T, P>::maxElement() const {
...
template <class T, class P> void Matrix<T, P>::print(std::ostream &os) const {
  for (size_t col = 0; col < elem_.size(); col++) {
  ...
  for (size_t col = 0; col < elem_.size(); col++) {
    for (size_t row = 0; row < elem_[col].size(); row++) {
      if (elem_[col][row] > 1e-4 && col != row)
        os << std::setw(8) << std::setprecision(3) << "(" << col << "," << row
           << ") " << elem_[col][row] << "\n ";
    }
  }
}
...
template <class T, class P> void Matrix<T, P>::read(std::istream &is) {
  elem_.clear();
  ...
  while (is) {
    ...
    std::copy(std::istream_iterator<T>(cutOffLineStream),
              std::istream_iterator<T>(), std::back_inserter(rowVector));
    if (rowVector.size() != 0)
      addRow(rowVector);
  }
}
...
template <class T, class P> Vector<T, P> diagonal(const Matrix<T, P> &m) {
  return m.diagonal();
}
...
template <class T, class P>
Matrix<T, P> multiplyElementwise(const Matrix<T, P> &m1,
                                 const Matrix<T, P> &m2) {
...
template <class T, class P> Matrix<T, P> abs(const Matrix<T, P> &m) {
  Matrix<T, P> a;
  abs(m, a);
  return a;
}
...
template <class T, class P> T maxAbsoluteElement(const Matrix<T, P> &m) {
  T r;
...
class RefernceCountedMatrix : public Core::ReferenceCounted,
                              public Matrix<T, P> {
public:
  ...
  template <class S>
  RefernceCountedMatrix(const Matrix<S> &x) : Matrix<T, P>(x) {}
};
