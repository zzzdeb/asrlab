...
/** Accelerated feature-scorer for MixtureSet of Gauss densities with diagonal
 * covariance matrices Scores are calculated on quantized (8 bit) features with
 * maximum approximation.
 *
 *  Data Structure:
 *    -Precalculated data is stored without tying, i.e. the precalculated means
 * and constant weights are stored for each density separated in a
 * QuantizedMixtureFeatureScorerElement object. -The current feature vector is
 * preprocessed by each covariance matrix and the resulting vectors are stored
 * in the Context object.
 *
 *  Acceleration:
 *    -constants are precalculated for each density
 *    -each mean vector is divided by the variance and quantized beforehand
 *    -feature vectors are divided by the variances and quantized once before
 * the distance calculation -distance calculation is reduced (in the previos
 * steps) to addition of u8 integers -addition of u8 integers can be further
 * paralelized using u32 addition
 *
 *  Prerequisite of Acceleration:
 *    -#densities >> #covariances since feature vectors are first divided by all
 * variances thus eventually also with not necesseary ones.
 */
class SimdGaussDiagonalMaximumFeatureScorer
...
public:
  ...
  typedef FeatureScorerOptimization::PreparedFeatureVector
...
public:
  ...
  /**
   * Returns a scaled and quantized version of the given feature-vector for each
   * covariance-matrix
   */
  std::vector<PreparedFeatureVector>
  multiplyAndQuantize(const FeatureVector &features) const;
...
private:
   ...
   *    intervalSize = (maxValue - minValue) is approximated by 2 *
   * max(abs(maxValue), ans(minValue)).
   *
   *  For type u8 the final calculation is:
   *    100.0 / max(abs(maxValue)n, abs(minValue)).
   */
  static FeatureType quantizationScalingFactor(FeatureType minValue,
                                               FeatureType maxValue);

  /** @return is scaling factor for quantization
   *
   *  Assumption: mean values of densities represent the interval of feature
   * vector values.
   *
   *  Algorithm: collect the maximal and minimal mean value over all densities
   * and all dimensions and call the function quantizationScalingFactor which
   * calculates the scaling between this interval and quantized interval. (@see
   * quantizationScalingFactor)
   */
  MeanType getScaling(const MixtureSet &mixtureSet) const;

  ScoreAndBestDensity
  calculateScoreAndDensity(const CachedAssigningContextScorer *cs,
...
public:
  SimdGaussDiagonalMaximumFeatureScorer(const Core::Configuration &c,
  ...
  virtual AssigningScorer
  getAssigningScorer(const FeatureVector &featureVector) const {
