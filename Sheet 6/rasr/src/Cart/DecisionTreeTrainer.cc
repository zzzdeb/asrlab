...
// ============================================================================
void ID3::write(std::ostream &os) const { os << "ID3\n"; }

Score ID3::entropy(ExamplePtrList::const_iterator begin,
                   ExamplePtrList::const_iterator end) const {
  Score entropy = 0.0;
  u32 nObs = 0;
...
Score ID3::operator()(const ExamplePtrRange &leftExamples,
                      const ExamplePtrRange &rightExamples,
                      const Score fatherEntropy, Score &leftChildEntropy,
                      Score &rightChildEntropy) const {
  ...
  Score nObs = leftExamples.size() + rightExamples.size(),
        leftNObs = leftExamples.size(), rightNObs = rightExamples.size();
  return fatherEntropy - (leftNObs / nObs) * leftChildEntropy -
         (rightNObs / nObs) * rightChildEntropy;
}
// ============================================================================
...
void DecisionTreeTrainer::TrainingPlan::Step::writeXml(
    Core::XmlWriter &xml) const {
  xml << (Core::XmlOpen("step") + Core::XmlAttribute("name", name) +
          Core::XmlAttribute("action", action))
      << Core::XmlFull("min-obs", minObs) << Core::XmlFull("min-gain", minGain);
  if (nRandomQuestion > 1)
...
private:
  ...
  */
  struct QuestionStatistic {
    ...
    QuestionStatistic()
        : count(0), minGain(Core::Type<Score>::max), sumGain(Score(0)),
          maxGain(Core::Type<Score>::min), minDepth(Core::Type<u32>::max),
          sumDepth(0), maxDepth(Core::Type<u32>::min) {}
  };
  ...
  */
  struct Node {
    ...
    Node(u32 depth, Score score, u32 nObs, ExamplePtrList *examplePtrs)
        : depth(depth), score(score), nObs(nObs), examplePtrs(examplePtrs),
          questionId(DecisionTree::Node::InvalidId),
          order(DecisionTreeTrainer::Information::InvalidOrder), left(0),
          right(0) {}
  };
  ...
  /**
     ...
     Stores (up to) the N -best hypotheses and returns either the best or a
     randomly choosen hypothesis
  */
  ...
  */
  ...
  private:
    void clear() {
      for (SplitHypothesisPtrList::iterator it = hyps_.begin();
           it != hyps_.end(); ++it)
        delete *it;
      hyps_.clear();
    }
  ...
  public:
    ...
    bool empty() const { return (nHyps_ == 0) ? true : false; }
    ...
    */
    const SplitHypothesis &get() {
      verify(nHyps_ > 0);
      return (nHyps_ == 1) ? *hyps_.front()
                           : **(hyps_.begin() + ::rand() % nHyps_);
    }
  };
  ...
  */
  struct Split {
    ...
    QuestionId questionId() const { return (*questionIds)[questionIdId]; }

    Split(Node *node, QuestionIdList *questionIds, size_t questionIdId,
          Score gain)
        : node(node), questionIds(questionIds), questionIdId(questionIdId),
          gain(gain) {
      require(questionIds);
    }
  };
...
private:
  ...
  void writeXml(Core::XmlWriter &xml, const Split &split) {
    xml << Core::XmlOpen("split") << Core::XmlFull("depth", split.node->depth)
        << Core::XmlFull("gain", split.gain) << Core::XmlOpen("question");
    ...
    xml << Core::XmlClose("question")
        ...
        << Core::XmlClose("right-child") << Core::XmlClose("node")
        << Core::XmlClose("split");
  }
  ...
  */
  Split *splitNode(Node *node, QuestionIdList *questionIds,
                   bool strict = false) {
    ...
    const ExamplePtrList::iterator beginLeftExamplePtr =
        leftExamplePtrs.begin();
    const ExamplePtrList::iterator beginRightExamplePtr =
        rightExamplePtrs.begin();
    SplitHypothesis splitHyp;
    verify(splitHyps_.empty());
    for (splitHyp.questionIdId = 0; splitHyp.questionIdId < questionIds->size();
         ++splitHyp.questionIdId) {
      const Question &question =
          *(questionRefs_[(*questionIds)[splitHyp.questionIdId]]);
      ...
      verify(splitHyp.nLeftExamples + splitHyp.nRightExamples ==
             node->examplePtrs->size());
      if ((splitHyp.nLeftObs < step_->minObs) ||
          (splitHyp.nRightObs < step_->minObs))
        continue;
      if (strict && ((splitHyp.nLeftObs == 0) || (splitHyp.nRightObs == 0)))
        continue;
      splitHyp.gain =
          scorer_(ExamplePtrRange(beginLeftExamplePtr, endLeftExamplePtr),
                  ExamplePtrRange(beginRightExamplePtr, endRightExamplePtr),
                  node->score, splitHyp.leftScore, splitHyp.rightScore);
      if (splitHyp.gain < 0.0) {
        if (!Core::isAlmostEqualUlp(f32(splitHyp.gain), f32(0.0), 20))
          error() << "negative split gain of " << splitHyp.gain
    ...
    if (!splitHyps_.empty()) {
      ...
      const Question &question =
          *(questionRefs_[(*questionIds)[bestSplitHyp.questionIdId]]);
      ExamplePtrList *leftExamplePtrs =
          new ExamplePtrList(bestSplitHyp.nLeftExamples);
      ExamplePtrList *rightExamplePtrs =
          new ExamplePtrList(bestSplitHyp.nRightExamples);
      ...
      for (ExamplePtrList::const_iterator
      ...
      node->examplePtrs = 0;
      node->left = new Node(node->depth + 1, bestSplitHyp.leftScore,
                            bestSplitHyp.nLeftObs, leftExamplePtrs);
      node->right = new Node(node->depth + 1, bestSplitHyp.rightScore,
                             bestSplitHyp.nRightObs, rightExamplePtrs);
      split = new Split(node, questionIds, bestSplitHyp.questionIdId,
                        bestSplitHyp.gain);
    }
    splitHyps_.reset();
    return split;
  ...
  */
  void split() {
    ...
    while (!splits_.empty() &&
           (nLeaf_ + nodes_.size() + splits_.size() < plan_.maxLeaves)) {
  ...
  /*
    action: partition(->finalizeLeftNodes=false) or
    cluster(->finalizeLeftNodes=true) same as split, but put only right node
    back into queue for further splitting; if finalizeLeftNodes=false, then put
    left nodes back into queue for the next step, otherwise each left node will
    become a leaf
  */
  ...
  void start(Node *root) {
    ...
    for (TrainingPlan::StepPtrList::const_iterator it = plan_.steps.begin();
         (it != plan_.steps.end()) &&
         (nLeaf_ + nodes_.size() < plan_.maxLeaves);
         ++it) {
      step_ = *it;
      step_->write(log());
  ...
  DecisionTree::Node *commitNode(Node *node) {
    ...
    DecisionTree::Node *_node =
        new DecisionTree::Node(0, 0,
                               new DecisionTreeTrainer::Information(
                                   node->order, node->nObs, node->score));
    if ((node->left) && (node->right)) {
      _node->id_ = questionIdMap_[node->questionId];
      _node->setChilds(commitNode(node->left), commitNode(node->right));
    } else {
  ...
  ClusterList *finish(Node *root) {
    ...
    for (ExampleList::const_iterator it = examples_.begin();
         it != examples_.end(); ++it)
      clusters_[tree_->classify(*((*it)->properties))]->exampleRefs->push_back(
          *it);
...
public:
  Training(const Core::Configuration &config, const Scorer &scorer,
           const TrainingPlan &plan, const ExampleList &examples,
           DecisionTree *tree)
      : Core::Component(config), scorer_(scorer), plan_(plan),
        examples_(examples), tree_(tree), score_(0.0), nObs_(0), nSplit_(0),
        nNumber_(0), nLeaf_(0), nCluster_(0), gain_(0.0), step_(0) {
    if (plan_.map.get() != examples_.getMap().get()) {
      PropertyMapDiff diff(config, *plan.map, examples_.map(), false);
      if (diff.hasDifferences())
        criticalError(
            "differences in property maps of training and example list");
    }
  }
  ...
  void writeXml(Core::XmlWriter &xml) const {
    ...
    xml << Core::XmlOpen("tree-statistic") << Core::XmlFull("nNode", nSplit_)
        << ((plan_.maxLeaves < Core::Type<u32>::max)
                ? Core::XmlFull("nLeaf", nLeaf_) +
                      Core::XmlAttribute("max", plan_.maxLeaves)
                : Core::XmlFull("nLeaf", nLeaf_))
    ...
    QuestionStatisticList::const_iterator questionStatIt =
        questionStats_.begin();
    for (; questionRefIt != questionRefs_.end();
         ++questionRefIt, ++questionStatIt, ++i) {
      ...
      if (questionStatIt->count > 0)
        xml << Core::XmlFull("total-gain", questionStatIt->sumGain)
            ...
            << Core::XmlFull("avg-gain", questionStatIt->sumGain /
                                             Score(questionStatIt->count))
            << Core::XmlFull("max-gain", questionStatIt->maxGain)
            << Core::XmlFull("min-depth", questionStatIt->minDepth)
            << Core::XmlFull("avg-depth", f32(questionStatIt->sumDepth) /
                                              f32(questionStatIt->count))
...
const Core::ParameterString
    DecisionTreeTrainer::paramTrainingFilename("training-file",
                                               "name of training file");

const Core::ParameterString
    DecisionTreeTrainer::paramClusterFilename("cluster-file",
...
ClusterList *DecisionTreeTrainer::train(DecisionTree *tree,
