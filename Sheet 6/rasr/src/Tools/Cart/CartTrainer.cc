...
static const Core::ParameterBool
    paramDry("dry", "load training file and examples, but don't learn the tree",
             false);
...
*/
static const Core::ParameterString paramScorer("score-function",
                                               "function to minimize", "ID3");
...
int CartTrainer::main(const std::vector<std::string> &arguments) {
  ...
  if (!trainingFilename.empty()) {
    ...
    train = false;
    warning(
        "unable to parse training plan file \"%s\"; cannot proceed training",
  ...
  if (!exampleFilename.empty()) {
    ...
    Core::Channel examplePlainChannel(select("example"), "plain",
                                      Core::Channel::disabled);
    if (examplePlainChannel.isOpen())
      examples.write(examplePlainChannel);
    Core::XmlChannel exampleXmlChannel(select("example"), "xml",
                                       Core::Channel::disabled);
  ...
  if (train) {
    ...
    Core::Channel treePlainChannel(select("decision-tree"), "plain",
                                   Core::Channel::disabled);
    if (treePlainChannel.isOpen())
      tree.write(treePlainChannel);
    Core::XmlChannel treeXmlChannel(select("decision-tree"), "xml",
                                    Core::Channel::disabled);
    if (treeXmlChannel.isOpen())
      tree.writeXml(treeXmlChannel);
    Core::Channel treeDotChannel(select("decision-tree"), "dot",
                                 Core::Channel::disabled);
    if (treeDotChannel.isOpen())
      tree.draw(treeDotChannel);
    tree.writeToFile();

    Core::Channel clusterPlainChannel(select("cluster"), "plain",
                                      Core::Channel::disabled);
    if (clusterPlainChannel.isOpen())
      clusters->write(clusterPlainChannel);
    Core::XmlChannel clusterXmlChannel(select("cluster"), "xml",
                                       Core::Channel::disabled);
