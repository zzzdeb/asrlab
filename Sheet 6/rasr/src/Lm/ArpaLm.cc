...
const Core::ParameterString ArpaLm::paramEncoding(
    "encoding", "encoding of ARPA language model file to load", "utf-8");
const Core::ParameterString
    ArpaLm::paramFilename("file", "name of ARPA language model file to load");

ArpaLm::ArpaLm(const Core::Configuration &c, Bliss::LexiconRef l)
    : Core::Component(c), BackingOffLm(c, l) {}
...
public:
  ...
  void addScore(const Token *newest, const Token *oldest, Token predicted,
                Score score) {
...
void ArpaLm::read() {
  ...
  Core::MD5 md5;
  std::string line;
  u32 lineNumber = 0, totalNGrams = 0, expectedTotalNGrams = 0,
      maxTotalNGrams = 0;
    ...
    if (line.size() == 0)
      ...
      case sizes: {
        ...
        if (sscanf(line.c_str(), "ngram %u=%u", &nGram, &nNGrams) == 2) {
           ...
           * assume that some part of the LM is discarded because of a resricted
           *vocabulary.
           **/
        ...
        for (n = 0; n < nGram; ++n) {
          if (lis >> word) {
            ...
            if (t) {
              ...
              warning("unknown syntactic token '%s' in line %d", word.c_str(),
                      lineNumber);
              break;
            }
          }
  ...
  /*
    log("%d/%d/%d loaded/expected/all n-grams", totalNGrams,
    expectedTotalNGrams, maxTotalNGrams);
  */
...
ArpaClassLm::ArpaClassLm(const Core::Configuration &c, Bliss::LexiconRef l)
    : Core::Component(c), ArpaLm(c, l), ClassLm(c) {
  ClassMapping *mapping = new ClassMapping(select("classes"), lexicon());
  mapping->load();
  mapping_ = ConstClassMappingRef(mapping);
  setTokenInventoryAndAlphabet(&mapping_->tokenInventory(),
                               mapping_->tokenAlphabet());
}

ArpaClassLm::~ArpaClassLm() {}

Token ArpaClassLm::getSpecialToken(const std::string &name,
                                   bool required) const {
  ...
  if (!tok && required)
    error("Cannot find special token \"%s\"", name.c_str());
  return tok;
}
...
Fsa::ConstAutomatonRef ArpaClassLm::getFsa() const {
  return Fsa::composeMatching(
      Fsa::multiply(mapping_->createSyntacticTokenToClassTokenTransducer(),
                    Fsa::Weight(classEmissionScale())),
      ArpaLm::getFsa());
}
