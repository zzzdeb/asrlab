...
public:
  ...
  static std::string prepareResourceName(const std::string &selection,
...
private:
   ...
   *  the including selection will be active until the first selection in the
   * included file.
   */
  bool parseInclude(const std::string &line, std::string &includeFilename,
                    std::string &warningDescription);
  bool parseSelection(const std::string &line, std::string &selection,
                      std::string &warningDescription);
  bool parseComment(const std::string &line, std::string &warningDescription);
  bool parseResource(const std::string &line, std::string &parameter,
                     std::string &value, std::string &warningDescription);
  ...
  setFromCommandline(const std::vector<std::string> &arguments,
...
public:
   ...
   * @see set() */
  bool tryToSet(const std::string &name, const std::string &value = "true",
                const SourceDescriptor *source = 0);
