...
protected:
  ...
  Am::ClassicStateTyingRef
  getStateTying(Core::Ref<Am::ClassicAcousticModel> am);
...
Am::ClassicStateTyingRef
AllophoneTool::getStateTying(Core::Ref<Am::ClassicAcousticModel> am) {
  Am::ClassicStateTyingRef stateTying = am->stateTying();
  if (!stateTying) {
    am->load(Am::AcousticModel::noEmissions |
...
int AllophoneTool::main(const std::vector<std::string> &arguments) {
  ...
  Core::Ref<Am::ClassicAcousticModel> am = Core::ref(
      new Am::ClassicAcousticModel(select("acoustic-model"), lexicon));
  {
...
#warning DEPRECATED!!! because of duplicated code
  {
    ...
    if (dump.isOpen()) {
      ...
      Am::ConstAllophoneStateAlphabetRef alloStateAlphabet =
          am->allophoneStateAlphabet();
      for (std::pair<Am::AllophoneStateIterator, Am::AllophoneStateIterator>
               it = alloStateAlphabet->allophoneStates();
           it.first != it.second; ++it.first)
        os << Core::form(
            "%-32s %9d %9d\n",
            alloStateAlphabet->toString(it.first.allophoneState()).c_str(),
            it.first.id(), stateTying->classify(it.first.allophoneState()));
