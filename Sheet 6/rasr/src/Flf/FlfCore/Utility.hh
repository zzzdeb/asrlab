...
inline Score add(Score a, Score b) {
  return (a == Semiring::Zero) || (b == Semiring::Zero) ? Semiring::Zero
                                                        : a + b;
}
...
inline Score logAdd(Score a, Score b) {
  ...
  if (a == Semiring::Zero)
    return b;
  return std::min(a, b) - ::log1p(::exp(std::min(a, b) - std::max(a, b)));
}
 ...
 * => (p1, c1) + ... + (pN, cN) = (p1 + ... + pN, (p1 * c1 + ... + pN *cN) / (p1
 *+ ... + pN)) Generalized Fwd/Bwd. probability (aka expected cost aka risk) for
 *arc e with incoming (pIN, cIN) and outgoing (pOUT, cOUT) risk(e) = (pIN * p *
 *pOUT) * (cIN + c + cOUT)
 *
 **/
class CostCollector {
...
/**
 * Parse Textfile line by line and split at (sequences of) white spaces into
 *columns; empty lines and lines containing a comment only are discarded.
 *
 * Supports:
 * - Encoding -> see Core::TextInputStream
 * - Escaping -> if \<c> occurs, <c> is appended to the current column
 * - Quoting  -> Columns may be bracketed by "; escaping is active within
 *quoting, comments are not
...
public:
  TextFileParser(const std::string &filename,
