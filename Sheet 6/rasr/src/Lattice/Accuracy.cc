...
ExactAccuracyGenerator::ExactAccuracyGenerator(f32 delCost, f32 insCost,
                                               f32 subCost, f32 corCost)
    : delCost_(delCost), insCost_(insCost), subCost_(subCost),
      corCost_(corCost) {}
...
public:
  ExactWordAccuracyGenerator(const std::string &orth,
                             ...
                             Fsa::ConstAutomatonRef lemmaToEval, f32 delCost,
                             f32 insCost, f32 subCost, f32 corCost);
...
ExactWordAccuracyGenerator::ExactWordAccuracyGenerator(
    const std::string &orth, Bliss::OrthographicParser *orthToLemma,
    Fsa::ConstAutomatonRef lemmaPronToLemma, Fsa::ConstAutomatonRef lemmaToEval,
    f32 delCost, f32 insCost, f32 subCost, f32 corCost)
    : Precursor(delCost, insCost, subCost, corCost),
      lemmaPronToLemma_(lemmaPronToLemma), lemmaToEval_(lemmaToEval) {
  require(orthToLemma);
  correct_ = Fsa::staticCopy(Fsa::minimize(Fsa::determinize(
      Fsa::removeEpsilons(Fsa::projectOutput(Fsa::composeMatching(
          orthToLemma->createLemmaAcceptor(orth), lemmaToEval_))))));
}

Fsa::ConstAutomatonRef ExactWordAccuracyGenerator::mapLemmaPronToEval(
    Fsa::ConstAutomatonRef pronLemma) {
  return Fsa::minimize(Fsa::determinize(Fsa::removeEpsilons(Fsa::cache(
      Fsa::multiply(Fsa::projectOutput(Fsa::composeMatching(
                        Fsa::composeMatching(Fsa::projectInput(pronLemma),
                                             lemmaPronToLemma_),
                        lemmaToEval_)),
                    Fsa::Weight(f32(0)))))));
}
...
ExactWordAccuracyGenerator::mapEvalToLemmaPron(Fsa::ConstAutomatonRef eval) {
  return Fsa::projectInput(Fsa::composeMatching(
      Fsa::multiply(lemmaPronToLemma_, Fsa::Weight(f32(0))),
      Fsa::projectInput(Fsa::composeMatching(
          Fsa::multiply(lemmaToEval_, Fsa::Weight(f32(0))), eval))));
}

ConstWordLatticeRef
ExactWordAccuracyGenerator::getWordLattice(ConstWordLatticeRef lattice) {
  ...
  if (fsa->getInputAlphabet() == lemmaPronToLemma_->getInputAlphabet()) {
    ...
    Fsa::minimize(Fsa::determinize(Fsa::removeEpsilons(Fsa::cache(fsa))));
  }
  fsa = Fsa::cache(
      Fsa::levenshtein(correct_, fsa, delCost_, insCost_, subCost_, corCost_));
  if (lattice->part(0)->getInputAlphabet() ==
      lemmaPronToLemma_->getInputAlphabet()) {
  ...
  return composeMatching(Fsa::multiply(fsa, Fsa::Weight(f32(-1))),
                         multiply(lattice, Fsa::Weight(f32(0))));
}

ConstWordLatticeRef
getExactWordAccuracy(ConstWordLatticeRef lattice, const std::string &orth,
                     Bliss::OrthographicParser *orthToLemma,
                     Fsa::ConstAutomatonRef lemmaPronToLemma,
                     Fsa::ConstAutomatonRef lemmaToEval) {
  // accuracy: 0 1 0 -1
  // levenshtein: 1 1 1 0
  ExactWordAccuracyGenerator generator(orth, orthToLemma, lemmaPronToLemma,
                                       lemmaToEval, 0, 1, 0, -1);
  ConstWordLatticeRef latticeWithAccuracy = generator.getWordLattice(lattice);
...
public:
  ExactPhonemeAccuracyGenerator(const std::string &orth,
                                ...
                                f32 delCost, f32 insCost, f32 subCost,
                                f32 corCost);
...
ExactPhonemeAccuracyGenerator::ExactPhonemeAccuracyGenerator(
    const std::string &orth, Bliss::OrthographicParser *orthToLemma,
    Fsa::ConstAutomatonRef lemmaPronToPhoneme,
    Fsa::ConstAutomatonRef lemmaToPhoneme, f32 delCost, f32 insCost,
    f32 subCost, f32 corCost)
    : Precursor(delCost, insCost, subCost, corCost),
      lemmaPronToPhoneme_(lemmaPronToPhoneme) {
  correct_ = Fsa::staticCopy(Fsa::minimize(Fsa::determinize(
      Fsa::removeEpsilons(Fsa::projectOutput(Fsa::composeMatching(
          orthToLemma->createLemmaAcceptor(orth), lemmaToPhoneme))))));
}

Fsa::ConstAutomatonRef ExactPhonemeAccuracyGenerator::mapLemmaPronToPhon(
    Fsa::ConstAutomatonRef lemmaPron) {
  return Fsa::cache(
      Fsa::projectOutput(Fsa::composeMatching(lemmaPron, lemmaPronToPhoneme_)));
}

Fsa::ConstAutomatonRef
ExactPhonemeAccuracyGenerator::mapPhonToLemmaPron(Fsa::ConstAutomatonRef phon) {
  return Fsa::removeEpsilons(Fsa::cache(
      Fsa::projectInput(Fsa::composeMatching(lemmaPronToPhoneme_, phon))));
}

ConstWordLatticeRef
ExactPhonemeAccuracyGenerator::getWordLattice(ConstWordLatticeRef lattice) {
#if 1
  ...
  return composeMatching(result, multiply(lattice, Fsa::Weight(f32(0))));
}

ConstWordLatticeRef
getExactPhonemeAccuracy(ConstWordLatticeRef lattice, const std::string &orth,
                        Bliss::OrthographicParser *orthToLemma,
                        Fsa::ConstAutomatonRef lemmaPronToPhoneme,
                        Fsa::ConstAutomatonRef lemmaToPhoneme) {
  // accuracy: 0 1 0 -1
  // levenshtein: 1 1 1 0
  ExactPhonemeAccuracyGenerator generator(orth, orthToLemma, lemmaPronToPhoneme,
                                          lemmaToPhoneme, 0, 1, 0, -1);
  ConstWordLatticeRef latticeWithAccuracy = generator.getWordLattice(lattice);
  WordLattice *result = new WordLattice;
  result->setWordBoundaries(latticeWithAccuracy->wordBoundaries());
  result->setFsa(Fsa::multiply(latticeWithAccuracy->part(0), Fsa::Weight(-1)),
...
protected:
  struct TimeInterval {
    ...
    TimeInterval(Speech::TimeframeIndex _startTime,
                 Speech::TimeframeIndex _endTime)
  ...
  struct Hypothesis : public TimeInterval {
    Fsa::LabelId label;
    Hypothesis(Fsa::LabelId _label, Speech::TimeframeIndex _startTime,
               Speech::TimeframeIndex _endTime)
        : TimeInterval(_startTime, _endTime), label(_label) {}
  };
  typedef Core::hash_set<TimeInterval, TimeIntervalHash, TimeIntervalEquality>
      TimeIntervals;
...
public:
  ApproximateAccuracyAutomaton(ConstWordLatticeRef, const ShortPauses &);
};

ApproximateAccuracyAutomaton::ApproximateAccuracyAutomaton(
    ConstWordLatticeRef lattice, const ShortPauses &shortPauses)
    : ModifyWordLattice(lattice), shortPauses_(shortPauses),
...
f32 ApproximateAccuracyAutomaton::accuracy(const States &refs,
                                           const Hypothesis &h) const {
  if (!refs.empty()) {
    f32 acc = Core::Type<f32>::min;
    for (States::const_iterator rIt = refs.begin(); rIt != refs.end(); ++rIt) {
      for (TimeIntervals::const_iterator r = rIt->second.begin();
           r != rIt->second.end(); ++r) {
        f32 overlap = std::min(h.endTime, r->endTime) -
                      std::max(h.startTime, r->startTime);
        verify(overlap >= 0);
        require(r->startTime < r->endTime);
        overlap /= (r->endTime - r->startTime);
        f32 _acc = (label(rIt->first) == label(h.label)) ? -1 + 2 * overlap
                                                         : -1 + overlap;
...
protected:
  virtual Fsa::LabelId label(Fsa::LabelId pronId) const {
    if (useLemmata_) {
      const Bliss::LemmaPronunciation *lp =
          alphabet_->lemmaPronunciation(pronId);
      if (lp)
        return lp->lemma()->id();
      return Fsa::Epsilon;
    } else {
...
public:
  ApproximateWordAccuracyAutomaton(ConstWordLatticeRef, ConstWordLatticeRef,
                                   const ShortPauses &, bool);
  virtual ~ApproximateWordAccuracyAutomaton() {}

  virtual std::string describe() const {
    return Core::form("approximate-word-accuracy(%s)",
                      fsa_->describe().c_str());
  }
...
ApproximateWordAccuracyAutomaton::LatticeToActiveStates::LatticeToActiveStates(
    ...
    : DfsState(correct), stateIds_(stateIds) {}

void ApproximateWordAccuracyAutomaton::LatticeToActiveStates::discoverState(
    Fsa::ConstStateRef sp) {
...
ApproximateWordAccuracyAutomaton::ApproximateWordAccuracyAutomaton(
    ConstWordLatticeRef lattice, ConstWordLatticeRef correct,
    const ShortPauses &shortPauses, bool useLemmata)
...
void ApproximateWordAccuracyAutomaton::modifyState(Fsa::State *sp) const {
  ...
  for (Fsa::State::iterator a = sp->begin(); a != sp->end(); ++a) {
    if (shortPauses_.find(label(a->input())) == shortPauses_.end()) {
      ...
      States refs;
      for (Speech::TimeframeIndex time = startTime; time < endTime; ++time) {
        for (States::const_iterator rIt = stateIds_[time].begin();
             rIt != stateIds_[time].end(); ++rIt) {
...
private:
  ...
  public:
    AlignmentToActiveStates(const Speech::Alignment &,
  ...
  public:
    LatticeToActiveStates(ConstWordLatticeRef,
                          ApproximatePhoneAccuracyAutomaton &parent);
    virtual void discoverState(Fsa::ConstStateRef sp);
  };
...
public:
  ApproximatePhoneAccuracyAutomaton(ConstWordLatticeRef, ConstWordLatticeRef,
                                    const ShortPauses &,
                                    Speech::AlignmentGeneratorRef);
  ApproximatePhoneAccuracyAutomaton(ConstWordLatticeRef,
                                    const Speech::Alignment &,
                                    const ShortPauses &,
                                    Speech::AlignmentGeneratorRef);

  virtual std::string describe() const {
    return Core::form("approximate-phone-accuracy(%s)",
                      fsa_->describe().c_str());
  }
  virtual void modifyState(Fsa::State *sp) const;
};
...
ApproximatePhoneAccuracyAutomaton::LatticeToActiveStates::LatticeToActiveStates(
    ConstWordLatticeRef correct, ApproximatePhoneAccuracyAutomaton &parent)
    : DfsState(correct), parent_(parent) {}

void ApproximatePhoneAccuracyAutomaton::LatticeToActiveStates::discoverState(
    Fsa::ConstStateRef sp) {
  const Speech::TimeframeIndex startTime = wordBoundaries_->time(sp->id());
  for (Fsa::State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
    const Bliss::LemmaPronunciation *pronunciation =
        parent_.alphabet_->lemmaPronunciation(a->input());
    if (!pronunciation) {
      continue;
    }
    const Speech::TimeframeIndex endTime =
    ...
    const Speech::Alignment *alignment =
        parent_.alignmentGenerator_->getAlignment(coarticulatedPronunciation,
                                                  startTime, endTime);
    parent_.stateIds_.grow(endTime, States());
    for (std::vector<Speech::AlignmentItem>::const_iterator al =
             alignment->begin();
         al != alignment->end();) {
      ...
      Speech::TimeframeIndex blTime =
          bl != alignment->end() ? bl->time : endTime;
...
ApproximatePhoneAccuracyAutomaton::AlignmentToActiveStates::
    AlignmentToActiveStates(const Speech::Alignment &alignment,
                            ApproximatePhoneAccuracyAutomaton &parent) {
  ApproximatePhoneAccuracyAutomaton::ActiveStates &stateIds = parent.stateIds_;
  stateIds.grow(alignment.back().time, States());
  for (Speech::Alignment::const_iterator al = alignment.begin();
       al != alignment.end();) {
...
ApproximatePhoneAccuracyAutomaton::ApproximatePhoneAccuracyAutomaton(
    ...
    : ApproximateAccuracyAutomaton(lattice, shortPauses),
      ...
      allophoneStateAlphabet_(
          alignmentGenerator->acousticModel()->allophoneStateAlphabet()) {
  LatticeToActiveStates s(correct, *this);
  s.dfs();
}

ApproximatePhoneAccuracyAutomaton::ApproximatePhoneAccuracyAutomaton(
    ConstWordLatticeRef lattice, const Speech::Alignment &alignment,
    const ShortPauses &shortPauses,
    Speech::AlignmentGeneratorRef alignmentGenerator)
    : ApproximateAccuracyAutomaton(lattice, shortPauses),
      alignmentGenerator_(alignmentGenerator),
...
void ApproximatePhoneAccuracyAutomaton::modifyState(Fsa::State *sp) const {
  ...
  for (Fsa::State::iterator a = sp->begin(); a != sp->end(); ++a) {
    ...
    if (pronunciation) {
      const Speech::TimeframeIndex endTime =
          wordBoundaries_->time(fsa_->getState(a->target())->id());
      Bliss::Coarticulated<Bliss::LemmaPronunciation>
          coarticulatedPronunciation(
              *pronunciation, wordBoundaries_->transit(sp->id()).final,
              wordBoundaries_->transit(fsa_->getState(a->target())->id())
                  .initial);
      const Speech::Alignment *alignment = alignmentGenerator_->getAlignment(
          coarticulatedPronunciation, startTime, endTime);
      for (std::vector<Speech::AlignmentItem>::const_iterator al =
               alignment->begin();
           al != alignment->end();) {
...
ConstWordLatticeRef
getApproximateAccuracy(Core::Ref<const ApproximateAccuracyAutomaton> a) {
...
ConstWordLatticeRef getApproximateWordAccuracy(ConstWordLatticeRef lattice,
                                               ConstWordLatticeRef correct,
                                               const ShortPauses &shortPauses,
                                               bool useLemmata) {
  Core::Ref<ApproximateAccuracyAutomaton> a(
      new ApproximateWordAccuracyAutomaton(lattice, correct, shortPauses,
                                           useLemmata));
  return getApproximateAccuracy(a);
}

ConstWordLatticeRef
getApproximatePhoneAccuracy(ConstWordLatticeRef lattice,
                            ConstWordLatticeRef correct,
                            const ShortPauses &shortPauses,
                            Speech::AlignmentGeneratorRef alignmentGenerator) {
  Core::Ref<ApproximateAccuracyAutomaton> a(
      new ApproximatePhoneAccuracyAutomaton(lattice, correct, shortPauses,
                                            alignmentGenerator));
  return getApproximateAccuracy(a);
}
 ...
 */
class ApproximatePhoneAccuracyMaskAutomaton
    : public ApproximatePhoneAccuracyAutomaton {
protected:
  virtual f32 accuracy(const States &refs, const Hypothesis &h) const;
  const Speech::Confidences &mask_;

public:
  ApproximatePhoneAccuracyMaskAutomaton(ConstWordLatticeRef,
                                        ConstWordLatticeRef,
                                        const Speech::Confidences &,
                                        const ShortPauses &,
                                        Speech::AlignmentGeneratorRef);

  virtual std::string describe() const {
    return Core::form("approximate-phone-accuracy-mask(%s)",
                      fsa_->describe().c_str());
  }
};
...
ApproximatePhoneAccuracyMaskAutomaton::ApproximatePhoneAccuracyMaskAutomaton(
    ConstWordLatticeRef lattice, ConstWordLatticeRef correct,
    const Speech::Confidences &mask, const ShortPauses &shortPauses,
    Speech::AlignmentGeneratorRef alignmentGenerator)
    : ApproximatePhoneAccuracyAutomaton(lattice, correct, shortPauses,
                                        alignmentGenerator),
      mask_(mask) {}

f32 ApproximatePhoneAccuracyMaskAutomaton::accuracy(const States &refs,
                                                    const Hypothesis &h) const {
  f32 weight = 0.0;
...
ConstWordLatticeRef getApproximatePhoneAccuracyMask(
    ...
    Speech::AlignmentGeneratorRef alignmentGenerator) {
  Core::Ref<ApproximateAccuracyAutomaton> a(
      new ApproximatePhoneAccuracyMaskAutomaton(
          lattice, correct, mask, shortPauses, alignmentGenerator));
  return getApproximateAccuracy(a);
}
...
public:
  FramePhoneAccuracyAutomaton(ConstWordLatticeRef, ConstWordLatticeRef,
                              const ShortPauses &,
                              Speech::AlignmentGeneratorRef, f32 normalization);

  virtual std::string describe() const {
    return Core::form("frame-phone-accuracy-%f(%s)", normalization_,
                      fsa_->describe().c_str());
  }
};
...
FramePhoneAccuracyAutomaton::FramePhoneAccuracyAutomaton(
    ConstWordLatticeRef lattice, ConstWordLatticeRef correct,
    const ShortPauses &shortPauses,
    Speech::AlignmentGeneratorRef alignmentGenerator, f32 normalization)
    : ApproximatePhoneAccuracyAutomaton(lattice, correct, shortPauses,
                                        alignmentGenerator),
      normalization_(normalization) {}

f32 FramePhoneAccuracyAutomaton::accuracy(const States &refs,
                                          const Hypothesis &h) const {
  ...
  for (States::const_iterator rIt = refs.begin(); rIt != refs.end(); ++rIt) {
    for (TimeIntervals::const_iterator r = rIt->second.begin();
      ...
      if ((label(rIt->first) == _label)) {
        std::fill(accs.begin() +
                      std::max((s32)r->startTime - (s32)h.startTime, s32(0)),
                  accs.begin() + std::min((s32)r->endTime - (s32)h.startTime,
                                          (s32)accs.size()),
                  1);
      }
    }
  }
  return (f32)std::accumulate(accs.begin(), accs.end(), 0) /
         (1 + normalization_ * (nFrames - 1));
}

ConstWordLatticeRef
getFramePhoneAccuracy(ConstWordLatticeRef lattice, ConstWordLatticeRef correct,
                      const ShortPauses &shortPauses,
                      Speech::AlignmentGeneratorRef alignmentGenerator,
                      f32 normalization) {
  Core::Ref<ApproximateAccuracyAutomaton> a(new FramePhoneAccuracyAutomaton(
      lattice, correct, shortPauses, alignmentGenerator, normalization));
  return getApproximateAccuracy(a);
}
...
private:
  ...
  public:
    LatticeToActiveStates(ConstWordLatticeRef,
                          FrameStateAccuracyAutomaton &parent);
    virtual void discoverState(Fsa::ConstStateRef sp);
  };
...
public:
  FrameStateAccuracyAutomaton(ConstWordLatticeRef, ConstWordLatticeRef,
                              const ShortPauses &,
                              Speech::AlignmentGeneratorRef);
  virtual std::string describe() const {
...
void FrameStateAccuracyAutomaton::LatticeToActiveStates::discoverState(
    Fsa::ConstStateRef sp) {
  const Speech::TimeframeIndex startTime = wordBoundaries_->time(sp->id());
  for (Fsa::State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
    const Bliss::LemmaPronunciation *pronunciation =
...
FrameStateAccuracyAutomaton::FrameStateAccuracyAutomaton(
    ...
    : ModifyWordLattice(lattice),
      alphabet_(required_cast(const Bliss::LemmaPronunciationAlphabet *,
                              lattice->part(0)->getInputAlphabet().get())),
      alignmentGenerator_(alignmentGenerator), shortPauses_(shortPauses) {
  LatticeToActiveStates s(correct, *this);
  s.dfs();
}
...
f32 FrameStateAccuracyAutomaton::accuracy(const States &refs,
                                          const Fsa::LabelId &h) const {
  return isShortPause(h) ? 0 : (refs.find(h) != refs.end() ? 1 : 0);
}

ConstWordLatticeRef
getFrameStateAccuracy(ConstWordLatticeRef lattice, ConstWordLatticeRef correct,
                      const ShortPauses &shortPauses,
                      Speech::AlignmentGeneratorRef alignmentGenerator) {
  Core::Ref<FrameStateAccuracyAutomaton> a(new FrameStateAccuracyAutomaton(
      lattice, correct, shortPauses, alignmentGenerator));
  Core::Ref<WordLattice> result(new WordLattice);
  result->setWordBoundaries(a->wordBoundaries());
  result->setFsa(Fsa::cache(a), WordLattice::accuracyFsa);
  return result;
 ...
 */
class SoftFramePhoneAccuracyAutomaton
    : public ApproximatePhoneAccuracyAutomaton {
private:
  ...
  public:
    AlignmentToSoftAccuracies(SoftFramePhoneAccuracyAutomaton &parent)
        : parent_(parent) {}
    AlignmentToSoftAccuracies(const Speech::Alignment &,
                              SoftFramePhoneAccuracyAutomaton &);
  };
  class LatticeToSoftAccuracies : public AlignmentToSoftAccuracies,
                                  public DfsState {
  public:
    LatticeToSoftAccuracies(ConstWordLatticeRef,
                            SoftFramePhoneAccuracyAutomaton &);
    virtual void discoverState(Fsa::ConstStateRef sp);
  };
...
public:
  SoftFramePhoneAccuracyAutomaton(ConstWordLatticeRef, ConstWordLatticeRef,
                                  const ShortPauses &,
                                  Speech::AlignmentGeneratorRef);
  SoftFramePhoneAccuracyAutomaton(ConstWordLatticeRef,
                                  const Speech::Alignment &,
                                  const ShortPauses &,
                                  Speech::AlignmentGeneratorRef);

  virtual std::string describe() const {
    return Core::form("soft-frame-phone-accuracy(%s)",
                      fsa_->describe().c_str());
  }
};

SoftFramePhoneAccuracyAutomaton::LatticeToSoftAccuracies::
    LatticeToSoftAccuracies(ConstWordLatticeRef lattice,
                            SoftFramePhoneAccuracyAutomaton &parent)
    : SoftFramePhoneAccuracyAutomaton::AlignmentToSoftAccuracies(parent),
      DfsState(lattice) {}

void SoftFramePhoneAccuracyAutomaton::LatticeToSoftAccuracies::discoverState(
    Fsa::ConstStateRef sp) {
  const Speech::TimeframeIndex startTime = wordBoundaries_->time(sp->id());
  ...
  for (Fsa::State::const_iterator a = sp->begin(); a != sp->end(); ++a) {
    const Bliss::LemmaPronunciation *pronunciation =
        alphabet->lemmaPronunciation(a->input());
    if (!pronunciation)
      continue;
    const Speech::TimeframeIndex endTime =
        wordBoundaries_->time(fsa_->getState(a->target())->id());
...
SoftFramePhoneAccuracyAutomaton::AlignmentToSoftAccuracies::
    AlignmentToSoftAccuracies(const Speech::Alignment &alignment,
                              SoftFramePhoneAccuracyAutomaton &parent)
    : parent_(parent) {
  SoftAccuracies &accs = parent_.softAccuracies_;
...
SoftFramePhoneAccuracyAutomaton::SoftFramePhoneAccuracyAutomaton(
    ConstWordLatticeRef lattice, const Speech::Alignment &forcedAlignment,
    const ShortPauses &shortPauses,
    Speech::AlignmentGeneratorRef alignmentGenerator)
    : ApproximatePhoneAccuracyAutomaton(lattice, forcedAlignment, shortPauses,
                                        alignmentGenerator) {
  AlignmentToSoftAccuracies s(forcedAlignment, *this);
}

f32 SoftFramePhoneAccuracyAutomaton::accuracy(const States &refs,
                                              const Hypothesis &h) const {
  const Fsa::LabelId _label = label(h.label);
  const Speech::TimeframeIndex nFrames = h.endTime - h.startTime;
  ...
  for (States::const_iterator rIt = refs.begin(); rIt != refs.end(); ++rIt) {
    for (TimeIntervals::const_iterator r = rIt->second.begin();
      ...
      if ((label(rIt->first) == _label)) {
        ...
        for (; t < (u32)std::min((s32)r->endTime - (s32)h.startTime,
                                 (s32)accs.size());
             ++t, ++at) {
...
ConstWordLatticeRef
getSoftFramePhoneAccuracy(ConstWordLatticeRef lattice,
                          ConstWordLatticeRef correct,
                          const ShortPauses &shortPauses,
                          Speech::AlignmentGeneratorRef alignmentGenerator) {
  Core::Ref<ApproximateAccuracyAutomaton> a(new SoftFramePhoneAccuracyAutomaton(
      lattice, correct, shortPauses, alignmentGenerator));
  return getApproximateAccuracy(a);
}
...
getSoftFramePhoneAccuracy(ConstWordLatticeRef lattice,
                          ...
                          Speech::AlignmentGeneratorRef alignmentGenerator) {
  Core::Ref<ApproximateAccuracyAutomaton> a(new SoftFramePhoneAccuracyAutomaton(
      lattice, forcedAlignment, shortPauses, alignmentGenerator));
  return getApproximateAccuracy(a);
}
 ...
 */
class WeightedFramePhoneAccuracyAutomaton
    : public ApproximatePhoneAccuracyAutomaton {
private:
  ...
  private:
    ...
    struct diffSigmoid : public std::unary_function<f64, f64> {
      ...
      f32 operator()(f64 x) const {
        require(!Core::isSignificantlyLessUlp(x, 0, tol) &&
                !Core::isSignificantlyLessUlp(1, x, tol));
        if (beta != 1) {
          if (Core::isAlmostEqualUlp(x, 0, tol) ||
              Core::isAlmostEqualUlp(x, 1, tol)) {
            return beta > 1 ? 0 : inf;
          } else {
            const f64 s =
                pow(x, beta) / (marginFactor * pow(1 - x, beta) + pow(x, beta));
  ...
  public:
    SetDerivativesDfsState(ConstWordLatticeRef,
                           WeightedFramePhoneAccuracyAutomaton &);
    virtual void discoverState(Fsa::ConstStateRef sp);
    virtual void finish();
  };
...
public:
  WeightedFramePhoneAccuracyAutomaton(ConstWordLatticeRef, ConstWordLatticeRef,
                                      const ShortPauses &,
                                      Speech::AlignmentGeneratorRef, f32 beta,
                                      f32 margin);

  virtual std::string describe() const {
    return Core::form("weighted-frame-phone-accuracy-%f(%s)", beta_,
                      fsa_->describe().c_str());
  }
};

WeightedFramePhoneAccuracyAutomaton::SetDerivativesDfsState::
    SetDerivativesDfsState(ConstWordLatticeRef lattice,
                           WeightedFramePhoneAccuracyAutomaton &parent)
    : DfsState(lattice), parent_(parent) {}

void WeightedFramePhoneAccuracyAutomaton::SetDerivativesDfsState::discoverState(
    Fsa::ConstStateRef sp) {
  const Speech::TimeframeIndex startTime = wordBoundaries_->time(sp->id());
...
void WeightedFramePhoneAccuracyAutomaton::SetDerivativesDfsState::finish() {
  Core::Vector<f32> &drvs = parent_.derivatives_;
  std::transform(drvs.begin(), drvs.end(), drvs.begin(),
                 diffSigmoid(parent_.beta_, parent_.margin_));
}

WeightedFramePhoneAccuracyAutomaton::WeightedFramePhoneAccuracyAutomaton(
    ConstWordLatticeRef lattice, ConstWordLatticeRef correct,
    const ShortPauses &shortPauses,
    Speech::AlignmentGeneratorRef alignmentGenerator, f32 beta, f32 margin)
    : ApproximatePhoneAccuracyAutomaton(lattice, correct, shortPauses,
                                        alignmentGenerator),
      beta_(beta), margin_(margin) {
  require(beta_ > 0);
  SetDerivativesDfsState s(lattice, *this);
  s.dfs();
}

f32 WeightedFramePhoneAccuracyAutomaton::accuracy(const States &refs,
                                                  const Hypothesis &h) const {
  const Fsa::LabelId _label = label(h.label);
  const Speech::TimeframeIndex nFrames = h.endTime - h.startTime;
  std::vector<f32> accs(nFrames, f32(0));
      ...
      for (TimeIntervals::const_iterator r = rIt->second.begin();
        ...
        std::copy(
            derivatives_.begin() +
                std::max((s32)r->startTime - (s32)h.startTime, s32(0)),
            derivatives_.begin() +
                std::min((s32)r->endTime - (s32)h.startTime, (s32)accs.size()),
            accs.begin());
      }
    }
  }
...
ConstWordLatticeRef getWeightedFramePhoneAccuracy(
    ...
    Speech::AlignmentGeneratorRef alignmentGenerator, f32 beta, f32 margin) {
  Core::Ref<ApproximateAccuracyAutomaton> a(
      new WeightedFramePhoneAccuracyAutomaton(
          lattice, correct, shortPauses, alignmentGenerator, beta, margin));
  return getApproximateAccuracy(a);
}
