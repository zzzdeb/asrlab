...
void UnicodeInputConverter::setInputEncoding(const char *inputEncoding) {
  ...
  if (iconvHandle_ == (iconv_t)-1 && errno == EINVAL) {
    std::cerr << "Conversion from \"" << inputEncoding << "\" not available. "
              << "Using default encoding \"" << defaultEncoding << "\" instead."
              << std::endl;
  ...
  if (iconvHandle_ == (iconv_t)-1) {
    perror("iconv_open");
    errno = 0;
    std::cerr << "Failed to set up input conversion. Input will treated as "
                 "UTF-8 encoded."
              << std::endl;
  }
}
...
void UnicodeOutputConverter::setOutputEncoding(const char *outputEncoding) {
  ...
  if (iconvHandle_ == (iconv_t)-1) {
    ...
    errno = 0;
    std::cerr
        << "Failed to set up output conversion. Output will be UTF-8 encoded."
        << std::endl;
  }
}

template <typename InIterator, typename OutIterator>
OutIterator UnicodeInputConverter::convert(const InIterator &begin,
  ...
  for (const char *in = &*begin; in < &*end;) {
    ...
    size_t nconv =
        iconv(iconvHandle_, const_cast<char **>(&in), &todo, &buf, &avail);
...
template std::back_insert_iterator<std::vector<char> >
UnicodeInputConverter::convert(
    const char *const &begin, const char *const &end,
    std::back_insert_iterator<std::vector<char> > out);

template std::back_insert_iterator<std::string>
UnicodeInputConverter::convert(const char *const &begin, const char *const &end,
                               std::back_insert_iterator<std::string> out);

template <typename InIterator, typename OutIterator>
OutIterator UnicodeOutputConverter::convert(const InIterator &begin,
                                            const InIterator &end,
                                            OutIterator out) {
  if (!isConversionActive()) {
    return std::copy(begin, end, out);
...
template std::back_insert_iterator<std::vector<char> >
UnicodeOutputConverter::convert(
    const char *const &begin, const char *const &end,
    std::back_insert_iterator<std::vector<char> > out);

template std::back_insert_iterator<std::string>
UnicodeOutputConverter::convert(const char *const &begin,
                                const char *const &end,
                                std::back_insert_iterator<std::string> out);

template std::ostream_iterator<char>
UnicodeOutputConverter::convert(const std::string::const_iterator &begin,
