...
template <> std::string str<f32>(const f32 &f) {
  return Core::form("%f", double(f));
}
template <> std::string str<f64>(const f64 &f) {
  return Core::form("%f", double(f));
}
} // namespace
...
LogSemiring<_Weight, _Type>::LogSemiring()
    : Precursor(Fsa::PropertySemiringAll), tolerance_(0) {}

template <class _Weight, typename _Type>
LogSemiring<_Weight, _Type>::LogSemiring(s32 tolerance)
    : Precursor(Fsa::PropertySemiringAll), tolerance_(tolerance) {}

template <class _Weight, typename _Type>
std::string LogSemiring<_Weight, _Type>::name() const {
  return Core::form("log-%s(tolerance=%d)", Core::Type<_Type>::name,
                    tolerance_);
}
...
template <class _Weight, typename _Type>
_Weight LogSemiring<_Weight, _Type>::extend(const _Weight &a,
                                            const _Weight &b) const {
...
template <class _Weight, typename _Type>
Accumulator<_Weight> *
LogSemiring<_Weight, _Type>::getExtender(const _Weight &initial) const {
  return new Extender<_Weight, _Type>(initial);
}

template <class _Weight, typename _Type>
_Weight LogSemiring<_Weight, _Type>::collect(const _Weight &a,
                                             const _Weight &b) const {
  if (_Type(b) == _Type(zero()))
    ...
    return _Weight(std::min(_Type(a), _Type(b)) -
                   ::log1p(::exp(std::min(_Type(a), _Type(b)) -
                                 std::max(_Type(a), _Type(b)))));
}

template <class _Weight, typename _Type>
_Weight LogSemiring<_Weight, _Type>::invCollect(const _Weight &a,
                                                const _Weight &b) const {
  if (_Type(b) == _Type(zero()))
    if (_Type(a) == _Type(zero()))
      return zero();
...
template <class _Weight, typename _Type>
Accumulator<_Weight> *
LogSemiring<_Weight, _Type>::getCollector(const _Weight &initial) const {
...
template <class _Weight, typename _Type>
int LogSemiring<_Weight, _Type>::compare(const _Weight &a,
                                         const _Weight &b) const {
  if (_Type(a) == _Type(b))
    return 0;
...
template <class _Weight, typename _Type>
std::string LogSemiring<_Weight, _Type>::describe(const _Weight &a,
                                                  Fsa::Hint hints) const {
...
_Weight LogSemiring<_Weight, _Type>::fromString(const std::string &s) const {
  ...
  if (!Core::strconv(s, tmp))
    std::cerr << "'" << s << "' is not a valid " << name() << " semiring value."
              << std::endl;
  return _Weight(tmp);
}
...
template <class _Weight, typename _Type>
bool LogSemiring<_Weight, _Type>::read(_Weight &a,
                                       Core::BinaryInputStream &i) const {
  _Type tmp;
  i >> tmp;
  a = _Weight(tmp);
  return i;
...
template <class _Weight, typename _Type>
bool LogSemiring<_Weight, _Type>::write(const _Weight &a,
                                        Core::BinaryOutputStream &o) const {
  return o << _Type(a);
}

template <class _Weight, typename _Type>
void LogSemiring<_Weight, _Type>::compress(Core::Vector<u8> &stream,
                                           const _Weight &a) const {
  Fsa::appendBytes(stream, u32(a), sizeof(u32));
}

template <class _Weight, typename _Type>
_Weight LogSemiring<_Weight, _Type>::uncompress(
    Core::Vector<u8>::const_iterator &pos) const {
...
template <class _Weight, typename _Type>
TropicalSemiring<_Weight, _Type>::TropicalSemiring() : Precursor() {}
...
std::string TropicalSemiring<_Weight, _Type>::name() const {
  return Core::form("tropical-%s(tolerance=%d)", Core::Type<_Type>::name,
                    Precursor::tolerance_);
}

template <class _Weight, typename _Type>
_Weight TropicalSemiring<_Weight, _Type>::collect(const _Weight &a,
                                                  const _Weight &b) const {
  return _Weight(std::min(_Type(a), _Type(b)));
}
...
template <class _Weight, typename _Type>
Accumulator<_Weight> *
TropicalSemiring<_Weight, _Type>::getCollector(const _Weight &initial) const {
  return new TropicalCollector<_Weight, _Type>(initial);
}

template <class _Weight, typename _Type>
ProbabilitySemiring<_Weight, _Type>::ProbabilitySemiring() : Precursor() {}
...
std::string ProbabilitySemiring<_Weight, _Type>::name() const {
  return Core::form("probability-%s(tolerance=%d)", Core::Type<_Type>::name,
                    Precursor::tolerance_);
}
...
template <class _Weight, typename _Type>
_Weight ProbabilitySemiring<_Weight, _Type>::extend(const _Weight &a,
                                                    const _Weight &b) const {
  return _Weight(_Type(a) * _Type(b));
}
...
template <class _Weight, typename _Type>
Accumulator<_Weight> *
ProbabilitySemiring<_Weight, _Type>::getExtender(const _Weight &initial) const {
  return new ProbabilityExtender<_Weight, _Type>(initial);
}

template <class _Weight, typename _Type>
_Weight ProbabilitySemiring<_Weight, _Type>::collect(const _Weight &a,
                                                     const _Weight &b) const {
  return _Weight(_Type(a) + _Type(b));
}
...
template <class _Weight, typename _Type>
Accumulator<_Weight> *ProbabilitySemiring<_Weight, _Type>::getCollector(
    const _Weight &initial) const {
...
template <class _Weight, typename _Type>
std::string
ProbabilitySemiring<_Weight, _Type>::describe(const _Weight &a,
                                              Fsa::Hint hints) const {
  return str(_Type(a));
}
...
template <class _Weight, typename _Type>
_Weight
ProbabilitySemiring<_Weight, _Type>::fromString(const std::string &s) const {
