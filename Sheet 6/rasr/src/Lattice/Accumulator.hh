 ...
 * BaseAccumulator
 */
template <class Trainer> class BaseAccumulator : public DfsState {
public:
...
protected:
  virtual void accumulate(Core::Ref<const Mm::Feature::Vector> f,
                          Mm::MixtureIndex m, Mm::Weight w) {
    defect();
  }
  virtual void accumulate(Core::Ref<const Mm::Feature::Vector> f,
                          const PosteriorsAndDensities &p) {
    defect();
  }
  /*
  virtual void accumulate(Core::Ref<const Sparse::Feature::SparseVector> sf,
  Mm::MixtureIndex m, Mm::Weight w) { defect();
  }
  */
  virtual void reset() {}
...
public:
  ...
  virtual void setFsa(Fsa::ConstAutomatonRef fsa) { fsa_ = fsa; }
...
protected:
  typedef Core::Ref<Speech::PhonemeSequenceAlignmentGenerator>
...
protected:
  virtual const Alignment *getAlignment(Fsa::ConstStateRef from,
                                        const Fsa::Arc &a);
...
public:
  AcousticAccumulator(ConstSegmentwiseFeaturesRef, AlignmentGeneratorRef,
                      Trainer *, Mm::Weight,
  ...
  void
  setAccumulationFeatures(ConstSegmentwiseFeaturesRef accumulationFeatures) {
...
protected:
  virtual void process(typename Precursor::TimeframeIndex, Mm::MixtureIndex,
                       Mm::Weight);
  virtual void reset() { collector_.clear(); }

public:
  CachedAcousticAccumulator(typename Precursor::ConstSegmentwiseFeaturesRef,
