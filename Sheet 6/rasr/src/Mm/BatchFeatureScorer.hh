...
protected:
  ...
  public:
    ContextScorer(const BatchFeatureScorerBase *parent, u32 currentFeature,
                  u32 bufferedFeatures)
        : parent_(parent), currentFeature_(currentFeature),
...
public:
   ...
   */
  virtual bool bufferFilled() const { return buffered_ >= bufferSize_ - 1; }

  /**
   * Return true if the feature buffer is empty.
   */
  virtual bool bufferEmpty() const { return buffered_ <= 0; }
...
protected:
  ...
  virtual void fillScoreCache(EmissionIndex e, u32 featureIndex,
                              u32 length) const = 0;
 ...
 *
 * Requires an acoustic model with a single (globally pooled) diagonal
 * covariance. Scores are calculated using the maximum approximation (max. over
 * densities). The distance computation uses SIMD instructions (SSE). Using
 * floats without quantization.
 *
...
protected:
  ...
  inline void fillScoreCacheTpl(EmissionIndex e, u32 featureIndex, u32 length,
 ...
 * Requires an acoustic model with a single (globally pooled) diagonal
 * covariance. Scores are calculated using the maximum approximation (max. over
 * densities). The distance computation uses SIMD instructions (SSE2). Uses
 * quantization to one byte for each vector component.
 *
 * Data is stored in raw arrays with 16-byte alignment.
 * (required by the SSE instructions).
 *
